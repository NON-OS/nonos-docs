...
bridge, the [DHCP](services.md) client, the [DNS](services.md) resolver, and the IPC server that
other capsules call.
...
move the bytes to and from the [driver capsule](drivers.md). This is the seam that keeps the TCP/IP
...
(`respond`); the poll (`iface/poll.rs`) advances smoltcp, processing received frames, running the
...
compiled out when `net-core` is enabled (`src/userspace/init/spawn_plan/network/mod.rs`). The
decomposed form is the maximal-isolation design (one capsule per protocol layer); the consolidated
`net_core` is the form that is [runtime-proven](services.md), bringing the stack up to a bound DHCP
