...
[net_core](../stack.md) over IPC. The kernel holds none of it beyond the broker grants and the IPC.
...
| Page | What it covers |
|------|----------------|
| [driver.md](driver.md) | The RTL8821CE capsule: device discovery, the capability model, the module layout, and the serve loop that presents the interface. |
| [firmware.md](firmware.md) | Loading the on-chip 8051 firmware: reserved-page staging, the DDMA channel, the completion-bit protocol, and the sub-4GB DMA requirement. |
| [mac-and-phy.md](mac-and-phy.md) | Bringing the MAC and PHY up: the register tables, the transmit/receive engine enable, the receive-path and antenna switch, transmit power, and channel tuning. |
| [rings.md](rings.md) | The transmit and receive DMA rings: descriptor formats, the DMA-interface reset, and how frames move between the ring and the air. |
| [scan-and-associate.md](scan-and-associate.md) | Scanning for networks and joining one: the background scanner, the MLME state machine, the WPA2 four-way handshake, and hardware CCMP through the security CAM. |
| [wifi-core.md](wifi-core.md) | The `nonos_wifi_core` crate: the chip-independent 802.11, WPA2, and CCMP logic shared by every Wi-Fi driver, and the traits a chip driver implements. |
...
wireless: [net_core](../stack.md) receives Ethernet frames and has no idea they were 802.11 data
...
from the chip-independent code ([nonos_wifi_core](wifi-core.md): the 802.11 frame formats, the
...
The driver is a capsule, so it holds exactly the authority the [hardware broker](../../hardware-broker/README.md)
