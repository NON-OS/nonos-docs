# Security of the wireless stack
...
no socket code. A capsule holds exactly the authority the [hardware broker](../../hardware-broker/README.md)
...
the [broker's DMA path](../../hardware-broker/README.md), tied to the device claim, and revoked with
...
itself gated: the broker's allowlist (`src/hardware/broker/pci/allowlist.rs`) permits exactly the
...
The WPA2 and CCMP implementations are not stubs. The chip-independent [nonos_wifi_core](wifi-core.md)
...
[network services](../services.md). It is worth naming here only to be precise about the boundary: the
