...
The firmware is a Realtek image with a header and a set of memory sections. `fw/header.rs` parses the
header, confirms it is this chip's firmware, and reads the version, which the driver prints to the
console as the first sign the blob is the right one. `fw/sections.rs` splits the body into its
...
copying from the packet buffer into the 8051's IMEM or DMEM. `fw/download.rs` orchestrates the two
hops for every chunk of every section; the staging is `fw/rsvd.rs`, the DDMA is `fw/ddma.rs`, and the
register work that brackets the whole download is `fw/prep.rs`.

## Staging a chunk (the first hop)

`fw/rsvd.rs` stages one chunk into on-chip packet-buffer page 0. It lays a 48-byte transmit descriptor
...
`fw/ddma.rs` copies one chunk from the packet buffer into 8051 memory over DDMA channel 0. A transfer
...
The fix is in the kernel, not the driver. `src/kernel_core/init/memory.rs` scans the memory map for a
below-4GB region the general allocator is not using, and carves a dedicated low DMA pool from it
(`src/hardware/broker/dma/pool.rs`, the `low32` pool). DMA maps that do not explicitly ask for high
...
With every section copied and marked, `fw/prep.rs` closes out the download and releases the
