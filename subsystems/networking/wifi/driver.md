...
The capsule entry is `_start` in `src/main.rs`. It initialises the heap and then does two things, in
...
reached and serves anyway, and the [Settings panel](../../../userland/capsule-catalog/) reads that
...
With the device identified, `setup/mod.rs` acquires it through the broker in three grants:
...
It does not set the I/O-space bit. The broker's PCI allowlist (`src/hardware/broker/pci/allowlist.rs`)
...
machine, the WPA2 key schedule, and the CCMP cipher, is not here: it is the [nonos_wifi_core](wifi-core.md)
...
scanner is covered in [scan-and-associate.md](scan-and-associate.md); the two protocols are below.

## The net_core link protocol

To [net_core](../stack.md), this capsule is just a NIC: it speaks the same `LinkPort` contract every
...
covered in [scan-and-associate.md](scan-and-associate.md). The service registers under the name
