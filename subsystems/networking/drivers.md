...
frames with the [network stack](stack.md) over IPC. This page documents the driver capsules and the
...
(`src/userspace/init/spawn_plan/drivers_nic.rs`):
...
[IRQ](../hardware-broker/irq.md) wait path) picks it up, and the driver hands the frame to
[net_core](stack.md) over IPC, where the `NicDevice` rx-token feeds it into smoltcp. On transmit, the
...
Putting the NIC driver in a capsule is the same containment argument as the [block drivers](../storage/block-device.md):
