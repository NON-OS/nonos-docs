...
which exposes a BSD-socket API over IPC and drives the [network stack](stack.md) underneath. This is
...
(`src/userspace/capsule_net_sockets/spawn.rs:31`):
...
A client reaches it by name through the [IPC](../ipc/README.md) service registry, sends a socket
...
The wire protocol is the BSD-socket call set (`.../protocol/ops.rs`):
...
against the [net_core](stack.md) stack over IPC, so a `send` becomes a write into a smoltcp TCP or UDP
