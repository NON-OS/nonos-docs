...
Every process in NØNOS is a `ProcessControlBlock` (`src/process/core/pcb.rs:31`). It
...
`state` and `priority` are the two the [scheduler](../scheduler/README.md) reads on every
...
`memory` is the record the [fault handlers](../memory/faults.md) and the exit
...
These four fields are the runtime side of the [capability model](../../security/capabilities-and-tokens.md),
...
[verified spawn](../../security/capsules-and-trust.md)'s `install_spawn` flips this flag
...
into every token, and the syscall [resolver](../../security/revocation.md) compares a
...
field there. This is where the process model and the [context switch](context-switch.md)
meet.
...
that backs [PIO grants](../hardware-broker/README.md): a driver that holds a port grant has
the corresponding bits cleared here so the CPU permits its `in`/`out`. `reply_inbox`
is the name of the private [IPC](../ipc/README.md) inbox this process receives replies on, and
