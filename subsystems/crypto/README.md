...
| Page | What it covers |
|------|----------------|
| [hashes.md](hashes.md) | BLAKE3 (in-tree and the external crate), SHA-2, Keccak-256, HMAC, HKDF, and constant-time comparison. |
| [symmetric.md](symmetric.md) | In-tree AES-256-GCM and ChaCha20-Poly1305, the AEAD core, and the `MkCrypto*` syscall family. |
| [asymmetric.md](asymmetric.md) | In-tree Ed25519 and secp256k1, the kernel signing key, the `AlgId` verify dispatch, and the x25519 caveat. |
| [pqc.md](pqc.md) | ML-DSA-65 and Kyber over PQClean, and the production policy that requires Ed25519 and ML-DSA-65 together. |
| [randomness.md](randomness.md) | The secure RNG: software CSPRNG XOR hardware entropy, entropy sizing, and what draws from it. |
...
and BLAKE3 genuinely exists twice. The [transparent ZK attestation](../../security/attestation.md)
...
`src/security/nonos_id_cert/policy.rs`. Every page is verified against those trees with `file:line`
references.
