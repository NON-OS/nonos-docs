...
`AlgId::verify` (`src/crypto/asymmetric/alg_id/verify.rs:23`) verifies exactly one signature under
...
(`src/security/nonos_id_cert/policy.rs:32`), which the spawn preflight runs before an image loads,
...
(`src/crypto/pqc/ml_dsa_65/api.rs:72`) checks the public-key and signature lengths against the
constants and returns `MlDsa65Error::InvalidLength` before touching the FFI, and the dispatch turns
any error from the underlying verify into a plain `false` (`alg_id/verify.rs:40`,
...
(`ml_dsa_65/ffi.rs` calls `PQCLEAN_MLDSA65_CLEAN_*`), not in-tree Rust. Their known-answer vectors
live in the C library, so unlike the [hashes](hashes.md) and [classical signatures](asymmetric.md),
...
half of the pair are on the [asymmetric](asymmetric.md) page, and the trust chain that runs the dual
policy at spawn is on the [capsules and trust](../../security/capsules-and-trust.md) page.
