...
(`src/crypto/random_api/basic.rs:38`) first fills the buffer from the software CSPRNG, then
...
predictable keys (`src/crypto/error.rs:45`).
...
[the IPC envelope](../ipc/envelope.md)), and wallet and key generation surface
...
error is on the [IPC envelope](../ipc/envelope.md) page, the kernel keypair drawn from this path is
on the [asymmetric](asymmetric.md) page, and the ASLR offset it feeds is on the ELF loader pages.
