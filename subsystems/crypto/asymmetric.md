...
(`userland/crypto_proofs/src/ed25519_tests.rs`), and the verify itself
(`src/crypto/asymmetric/ed25519/signature.rs:119`) does more than the textbook equation. It rejects
...
(`src/crypto/asymmetric/alg_id/verify.rs:23`) returns an `AlgIdError::PubkeyLen` or
...
with ML-DSA-65 is on the [post-quantum](pqc.md) page, the trust chain that verifies these signatures
is on the [capsules and trust](../../security/capsules-and-trust.md) page, and the random path the
kernel keypair is drawn from is on the [randomness](randomness.md) page.
