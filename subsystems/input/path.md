...
An `InputEvent` (`src/kernel_core/surface_registry/types.rs:44`) is a fixed, flat record, `#[repr(C)]`:
...
boundary by value. The userland mirror in `userland/libc/src/surface_registry/types.rs:44` is the
...
`userland/capsule_driver_ps2_input/src/keymap/post.rs:25` as `MOD_SHIFT=1`, `MOD_CTRL=2`,
...
`tag4` (`src/syscall/abi/tag.rs:20`), so they read as ASCII if dumped at low memory
(`src/syscall/numbers/defs.rs:96`):
...
A driver capsule owns its device through the [hardware broker](../hardware-broker/irq.md),
...
event to the consumer that should receive it, over [IPC](../ipc/README.md).

### The driver side

- PS/2 keyboard: scancodes are translated to keycodes and posted at
  `userland/capsule_driver_ps2_input/src/keymap/post.rs:41` as `KEY_DOWN`/`KEY_UP` with the current
...
`server::run` (`userland/capsule_input_router/src/server/runner.rs:31`) is the whole router. Each
...
(`userland/capsule_input_router/src/sources/kernel_ring.rs:27`), which drains up to
...
`route_event` (`userland/capsule_input_router/src/route/dispatch.rs:28`) decides the destination in
this order.

1. Grab. If a capsule holds a grab whose kind mask covers this event's kind
   (`state/grabs/holder_for.rs:20`), the event goes straight to the grab holder and no focus or
...
`route_keyboard` (`userland/capsule_input_router/src/route/keyboard.rs:25`):

- A `KEY_DOWN` resolves the destination by asking the window manager for the focused pid,
  `wm::query_focus` (`clients/wm/query_focus.rs:20`), a synchronous IPC round trip to the
...
`route_pointer` (`userland/capsule_input_router/src/route/pointer/route_pointer.rs:33`):

1. `refresh_display` lazily learns the display size from the compositor the first time, so the
   cursor bounds match the screen (`route/pointer/refresh_display.rs:20`).
2. `cursor.apply` folds the event into the absolute cursor position
   (`state/cursor.rs:43`). A `POINTER_REL` adds `delta * mult_x2 / 2` (a sensitivity multiplier
...
3. `mirror_shell_pointer` sends motion to the shell as a `POINTER_ABS` at the cursor position if the
   ...
   window is focused (`route/pointer/mirror_shell_pointer.rs:24`).
...
6. Buttons, touch, and wheel need a hit test. `topmost_target` asks the WM which window is under the
   cursor, `wm::query_topmost` (`clients/wm/query_topmost.rs:21`), which returns a `Target`: owner
...
`route_to_window` (`route/pointer/route_to_window.rs:27`) is where the coordinate space changes:
...
- Device space. What a driver posts. Relative deltas for a mouse; for absolute devices the
  `0..0x7FFF` normalized range (`state/cursor.rs:19`).
- Screen space. What the router's cursor tracks after `cursor.apply`, unsigned pixels clamped to
  the display, cursor starting centered (`state/cursor.rs:31`).
...
(`server/handlers/grab_request.rs:31`) only accepts three named capsules,
...
(`state/grabs/request.rs:22`). While a grab is held, matching events bypass focus and hit testing
entirely. `grab_release` (`server/handlers/grab_release.rs:21`) drops the caller's grabs.

## Delivery envelope

Every event handed to a consumer is wrapped in the NINP envelope by `encode_delivery`
(`userland/capsule_input_router/src/protocol/delivery.rs:29`) and sent with `mk_ipc_send_to_pid`
(`route/deliver.rs:24`). The frame is 40 bytes: a 4-byte magic `NINP` (`0x4E49_4E50`), a 2-byte
...
(`userland/capsule_desktop_shell/src/server/input.rs:28`) checks length >= 40, magic `0x4E49_4E50`,
...
`mk_debug` once its setup sequence completes (`userland/capsule_driver_ps2_input/src/setup/sequence.rs:43`).
The USB HID driver emits `[USB-HID-ENUM] tablet bound`
(`userland/capsule_driver_usb_hid/src/orchestrator/poll/run.rs`). If neither marker appears, the
...
(`subscriptions.allows`, `route/keyboard.rs:46`), does `wm::query_focus` return a real pid, and is
...
(`route/pointer/route_pointer.rs:52`).
...
(`server/runner.rs:29`) bound how stale a delivered event can be.
