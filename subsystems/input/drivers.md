# Input Driver Capsules

The [ring](ring.md) and the [three syscalls](path.md) are the kernel half of input. This page is the
...
device only through the [hardware broker](../hardware-broker/README.md), reads the hardware, and calls
...
The claim and the grants come from the [broker](../hardware-broker/README.md): a claim is refused if
...
(`userland/capsule_driver_i2c_hid/src/input/post.rs`) is the whole translation layer in miniature:
...
The capsule fills a flat [`InputEvent`](path.md#the-event), hands it to the kernel by value, and is
...
built around this: `capsule_driver_ps2_input/src/main.rs` retries `setup::run` in a yield loop until
...
**Claim** (`setup/claim.rs`) returns the epoch that stamps everything after it:
...
**Port I/O grant** (`setup/pio.rs`) opens the register window, and rolls the claim back if it fails so
...
**IRQ bind** (`setup/irq.rs`) attaches the device's line, and on failure releases the port grant it
...
(`constants/ports.rs`). The status byte it polls has five bits it cares about (`constants/status.rs`):
...
(`constants/pnp.rs`): the keyboard is `0x0001:0x0303`, the AUX mouse `0x0001:0x0304`.

Once configured the driver waits on its IRQ and drains the controller when it fires. The drain
(`poll/drain.rs`) reads at most sixteen bytes per pass, and routes each by the AUX-data bit:
...
the LPSS controller, takes an MMIO grant for its registers (`setup/mmio.rs`) and an IRQ
(`setup/irq.rs`), brings the controller out of reset, programs the SCL clock timing (`init/scl.rs`),
...
each report: the boot-keyboard report through a keymap (`hid/keymap.rs`), a mouse or tablet report
through the pointer path (`hid/tablet.rs`), and posts the result. Its capability mask (`0x200019`) is
...
(`src/userspace/init/capsule_boot/run.rs`), which prints `boot_log::ok(prefix, "capsule spawned")` on
...
usually GSI-to-vector routing through the [IOAPIC](../interrupts/README.md) or, for an ACPI device, a
...
- `userland/capsule_driver_ps2_input/` - the PS/2 driver: `setup/` (`claim.rs`, `pio.rs`, `irq.rs`,
  `sequence.rs`), `constants/` (`ports.rs`, `status.rs`, `pnp.rs`), `poll/drain.rs`, `keymap.rs`,
...
- `userland/capsule_driver_i2c_pci/` - the LPSS i2c bus driver: `setup/` (`mmio.rs`, `irq.rs`,
  `claim.rs`), `init/scl.rs`, `transaction/`. Kernel embed at `src/hardware/i2c_pci_capsule/`.
- `userland/capsule_driver_i2c_hid/` - the i2c-HID driver: `i2c_client/`, `input/parse_report.rs`,
  `input/post.rs`, `hid/`. ACPI discovery in `src/arch/x86_64/acpi/devices/i2c/`.
