...
| Page | What it covers |
|------|----------------|
| [surfaces.md](surfaces.md) | The `SurfaceDescriptor` and slot, `register_surface` with its geometry validation, and the epoch-guarded handle. |
| [sharing.md](sharing.md) | `share` and `attach`: the refcount, the zero-copy cross-address-space frame mapping, the attach map, and release on exit. |
| [presentation.md](presentation.md) | The `MkSurface*` / `MkDisplay*` syscalls, present, and the phase-grid vsync (and the serialization bug it fixed). |
...
is a capsule's job over [IPC](../ipc/README.md). Sharing is zero-copy because the same physical
...
surface registry also houses the [input ring](../input/README.md), since input and display share the
...
`src/syscall/dispatch/router/surface_ops.rs` and `surface_handlers.rs`. Every page is verified
