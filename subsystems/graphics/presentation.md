...
memory draws it from the high-memory display pool (`hardware/broker/dma/pool.rs`), whose capacity is
`DISPLAY_FRAMEBUFFER_PAGES = 8192` (`dma/limits.rs:28`), the same ceiling the DISPLAY class carries
in `dma_page_limit_for_class` (`dma/limits.rs:44`). 8192 pages is one 3840x2160 ARGB surface, so a
...
device-reach boundary as [DMA](../hardware-broker/dma.md) applies to the framebuffer itself.

## Debugging presentation

The display memory path narrates itself at init through the DMA broker. `init_display_pool`
(`dma/pool.rs:31`) tries to reserve a high contiguous run and prints exactly one of two lines:
...
The pool falls back through `8192 -> 4096 -> 2048 -> 1024` pages (`dma/pool.rs:36`), so a `base=`
...
design. `init_after_fb` (`sys/boot_log/init.rs:22`) leaves the bootloader's verified-boot splash in
...
[surfaces](surfaces.md) page, the cross-address-space sharing the compositor reads through is on the
[sharing](sharing.md) page, and the display pool and its per-class ceiling live with the
[DMA broker](../hardware-broker/dma.md).
