...
The IDT is a single lazily-built global (`src/interrupts/idt/table.rs:26`), constructed once
in `build_idt` (`table.rs:29`) in three passes, grouped by what is different about the
vectors rather than by number:

```
  build_idt():                                       # table.rs:29
      table_trampolines::configure(idt)   # table.rs:36  vectors that need a swapgs trampoline
      table_ist::configure(idt)           # table.rs:37  exceptions that also need their own stack
      table_irqs::configure(idt)          # table.rs:38  the device lines and the syscall gate
```

The syscall gate is the one entry deliberately reachable from user mode: `table_irqs`
installs `VECTOR_SYSCALL` and calls `set_privilege_level(PrivilegeLevel::Ring3)`
(`src/interrupts/idt/table_irqs.rs:46`), the only `Ring3` gate in the table, so a capsule
can issue the legacy `int 0x80` while every other gate keeps its ring-0 DPL and a user
attempt to invoke it faults. `table_irqs` also installs the hardware-broker IRQ entries, the
vectors a claimed device's line is routed to, through `install_broker_entries`
(`table_irqs.rs:53`) reading `crate::arch::interrupt::broker` (`table_irqs.rs:54`).
...
why each assignment subtracts one (`table_ist.rs:47` and the five following assignments through
`table_ist.rs:62`). That off-by-one is deliberate and load bearing: getting it wrong points an
exception at the adjacent stack.
...
except one. `table_irqs::configure` (`table_irqs.rs:46`) is the sole call to `set_privilege_level(Ring3)`, on
...
**The table is immutable after build.** `IDT` is a `lazy_static` built once (`table.rs:26`) and never
...
the pre-reserved broker vector slots (`install_broker_entries`, `table_irqs.rs:53`), which point at
...
**The fragile faults run on private stacks.** Six exceptions carry an IST index (`table_ist.rs`), so the CPU
...
because `set_stack_index` is zero-based over one-based hardware slots (`table_ist.rs:47`), and getting that
...
`dump_stack_info` report a stack pointer inside the wrong IST region; the fix is in `table_ist.rs`, not in the
...
```
  ...
  src/interrupts/idt/table.rs     build_idt, the lazy_static IDT, the three configure passes
  src/interrupts/idt/table_trampolines.rs  the swapgs-trampoline vectors
  src/interrupts/idt/table_ist.rs the six IST stack assignments (set_stack_index - 1)
  src/interrupts/idt/table_irqs.rs the device lines, the Ring3 syscall gate, install_broker_entries
