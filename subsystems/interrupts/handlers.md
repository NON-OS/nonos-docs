...
[hardening](../memory/hardening.md) guard pages.
...
in the [IDT](idt.md)) exactly because the normal stack is often what is broken. The honest boundary is
...
the [IDT](idt.md), not at the handler.
...
before these handlers run is on the [trampolines](trampolines.md) page, the interrupt-context guard each
handler takes is on the [safety](safety.md) page, the EOI gate at the tail is on the
[controllers](controllers.md) page, and the guard pages the page-fault path refuses to back are on the
[memory hardening](../memory/hardening.md) page.
