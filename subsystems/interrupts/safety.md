...
[trampolines](trampolines.md) provide before any Rust handler runs. The honest boundary is that this is a
...
GS-base bug seen through this counter, and the fix is on the [trampolines](trampolines.md) page, not
here.
...
depends on is established by the [trampolines](trampolines.md), the handlers that take an interrupt
context at entry are on the [handlers](handlers.md) page, and the per-CPU block layout comes from the
[SMP](../smp/README.md) setup.
