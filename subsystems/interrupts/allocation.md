...
reserved on top of the exception range (`init.rs`) because they carry fixed handlers in the [IDT](idt.md)
...
installed on the [IDT](idt.md) page, and the broker bind path that consumes dynamically-routed vectors is
on the [broker IRQ](../hardware-broker/irq.md) page.
