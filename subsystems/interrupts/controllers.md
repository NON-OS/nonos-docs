...
(`pic/init.rs:25`) before interrupts are ever enabled, so a hardware line can never land on an exception
...
touched only from these kernel modules and from the [broker IRQ](../hardware-broker/irq.md) bind path; a
capsule reaches neither. The [MMIO](../hardware-broker/mmio.md) grant path withholds the MSI-X table and
...
destination programming, and x2APIC mode live in the [system APIC driver](../smp/tlb-shootdown.md), so a
...
the [IDT](idt.md) page, the EOI gate lives at the tail of every [handler](handlers.md), the LAPIC
bring-up and IPIs are on the [SMP](../smp/tlb-shootdown.md) page, and the bind path that programs the
IO-APIC destination is on the [broker IRQ](../hardware-broker/irq.md) page.
