...
happened, so this section reads alongside the [capability model](../../security/capabilities-and-tokens.md).

| Page | What it covers |
|------|----------------|
| [boundary.md](boundary.md) | The `SYSCALL` instruction and LSTAR stub, the register ABI, the number decode, the contract gate, and the `Capability` witness type that makes the check a type-enforced precondition. |
| [numbers.md](numbers.md) | The four-character ASCII tag encoding, decoding a raw number to a typed syscall, and the families: `Crypto*`, `Admin*`, `Graphics*`, and the `Mk*` microkernel surface. |
| [router.md](router.md) | The family dispatch after the contract passes, the per-family handlers, the counters and audit wrapper, and the `ENOSYS` fallback. |
...
[capabilities page](../../security/capabilities-and-tokens.md). The exhaustive per-call
contract with numbers, arguments, and error codes is the [ABI reference](../../abi/syscalls.md).
