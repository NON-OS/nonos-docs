...
(`discover.rs:32`) enumerates only `CLASS_BLOCK` devices, `claim` (`setup/claim.rs`) takes the epoch,
and `mmio::map` (`setup/mmio.rs`) maps `NVME_BAR_INDEX` under that epoch and rolls the claim back if
the map fails.

The **DMA buffers are broker-zeroed and ceiling-bounded.** The queues and data buffers a driver
programs into the controller are `mk_dma_map` grants (`dma/region.rs:28` for NVMe, `setup/dma.rs` for
...
(`src/hardware/broker/dma/limits.rs:40`), so an NVMe submission and completion queue pair
(`admin/queue/types.rs:20`, the `sq` and `cq` DMA regions) plus its data buffers fit inside the
ceiling and a runaway request is refused with `BadLengthForClass` before any RAM is allocated. Each
`DmaRegion` unmaps itself on drop (`dma/region.rs:46`), so the frames return to the allocator when the
...
[DMA grants](../hardware-broker/dma.md#security-analysis) page). The broker bounds what the driver
...
(`src/userspace/init/capsule_boot/run.rs:29`), which prints `capsule spawned` on success and
`boot_log::error(...)` with the spawn reason on failure, tagged with the driver's prefix
(`[DRIVER-NVME]`, `[DRIVER-AHCI]`, `[DRIVER-VIRTIO-BLK]` from `spawn_plan/drivers_storage.rs` and
...
(`error/types.rs:30`): `DeviceNotFound` (30) means `mk_device_list(CLASS_BLOCK, ...)`
(`discover.rs:34`) returned nothing, so the firmware never enumerated an NVMe controller into the
broker table (`src/hardware/broker/table/list.rs`); `ClaimFailed` (31) means another capsule already
...
(`BadLengthForClass`, `dma/map/mod.rs:76`), and a grant against a lapsed claim as
`[DMA] validate stale-epoch`, using the same marker vocabulary as the [DMA grants](../hardware-broker/dma.md#debugging-dma-grants)
...
`irq::bind` (`setup/irq.rs`) treats the MSI-X bind as best effort and, on a failed bind, continues in
...
those is the same GSI-to-vector routing question as any other device (the [IOAPIC](../interrupts/README.md)).
...
specified on the [hardware broker](../hardware-broker/README.md) pages, the DMA ceiling and its markers
on the [DMA grants](../hardware-broker/dma.md) page, and the filesystems that sit above the block layer
on the [VFS routing](vfs-and-paths.md) page.
