...
| Page | What it covers |
|------|----------------|
| [block-device.md](block-device.md) | The `Backend` abstraction, the NVMe -> AHCI -> virtio probe, and the block operations dispatched to the driver capsules over IPC. |
| [ramfs.md](ramfs.md) | The in-kernel RAM filesystem, the `NonosFile`, and the zero-on-drop wipe that ties it to ZeroState. |
| [vfs-and-paths.md](vfs-and-paths.md) | The prefix router (`/ram`, `/data`, else RAMFS), the dormant on-disk blockfs store, and the path-traversal defenses. |
| [vfs-capsule.md](vfs-capsule.md) | The IPC-routed filesystem capsules, the wire protocol, kernel-attested callers, the read-only `/capsules` tree, and the proofs. |
...
[hardware broker](../hardware-broker/README.md), and the modern filesystem is a capsule reached through
[IPC](../ipc/README.md), so a driver or filesystem bug is contained in a capsule. The path and access
