...
The **path defenses stop traversal before dispatch.** `validate_path` (`src/fs/path/validate.rs`)
...
same signed block-driver capsules documented on the [block layer](block-device.md#security-analysis)
...
[filesystem capsule](vfs-capsule.md#caller-attestation) page). The VFS layer guarantees the path is
...
routing outcome: `/ram` went over IPC to the [ramfs capsule](vfs-capsule.md), `/data` went to blockfs,
and everything else went to the in-kernel [RAMFS](ramfs.md). So before suspecting a filesystem bug,
...
[block layer](block-device.md#debugging-the-block-layer) signatures, not a VFS bug.
...
[RAMFS](ramfs.md) page, the IPC-routed `/ram` capsule on the [filesystem capsule](vfs-capsule.md) page,
and the block drivers behind `/data` on the [block layer](block-device.md) page.
