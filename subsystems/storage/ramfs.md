...
half of the [zeroization](../memory/zeroization.md) posture: RAMFS does not depend on the whole-system
wipe to keep file data out of reclaimed RAM, and it complements the [heap](../memory/heap.md)'s own
...
on-disk `/data` store is served here in RAM (`ramfs/mod.rs`), so the live filesystem is memory-resident
and there is no persistent medium for its contents to linger on after a power cycle. The on-disk
[blockfs](vfs-and-paths.md#the-on-disk-store) store and its block drivers are reached only when
...
spawned, so the in-kernel RAMFS backs them while the IPC-routed [ramfs capsule](vfs-capsule.md) serves
...
[VFS router](vfs-and-paths.md#routing), a read that returns nothing is usually a routing question
before it is a RAMFS question: a `/ram` path went to the [ramfs capsule](vfs-capsule.md) over IPC and a
...
(`create_file`, `read_file`, `write_file`, `list_dir` in `ramfs/mod.rs`) return through the ordinary
...
[zeroization](../memory/zeroization.md) page, the heap's own zero-on-free on the [heap](../memory/heap.md)
page, the prefix router that reaches RAMFS on the [VFS routing](vfs-and-paths.md) page, and the
IPC-routed capsule variant on the [filesystem capsule](vfs-capsule.md) page.
