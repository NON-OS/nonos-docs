...
| Page | What it covers |
|------|----------------|
| [calibration.md](calibration.md) | Reading `RDTSC`, the CPUID -> PIT -> HPET calibration chain with its confidence, the stored frequency, and the 2.5 GHz fallback. |
| [time-bases.md](time-bases.md) | Monotonic `now_ns` vs wall-clock `unix_timestamp_ms`, the boot epoch from the handoff or RTC, and the `crate::time` façade. |
| [consumers.md](consumers.md) | Who reads the clock (scheduler deadlines, IPC timestamps, spawn validity, displays) and how the counter doubles as RNG entropy. |
...
re-exported as `crate::time` (`src/lib.rs:80`). The local time-of-day display is `src/sys/clock/`,
the boot seeding is `src/kernel_core/init/entry.rs`, and the entropy use is under `src/crypto/`.
