...
timer interrupt. `on_timer_interrupt` (`src/interrupts/timer/tick.rs:20`) runs on every tick, bumps the
...
and the monotonic deadlines the [scheduler](../scheduler/sleep-wake.md) sets would never be examined. The
tick counter itself (`src/interrupts/timer/state.rs:22`) is a separate quantity from `now_ns`; it counts
...
[time bases](time-bases.md) page, seen from the consumer side.
...
problem, not a clock problem, and is diagnosed on the [interrupts](../interrupts/safety.md) side.
...
mixed alongside the primary secure path documented under [randomness](../crypto/randomness.md). The honest
...
[time bases](time-bases.md) security boundary rather than chasing the consumer.
...
[calibration](calibration.md). The opposite shape, a system that stops switching between tasks entirely,
...
the [time bases](time-bases.md) page, the frequency that scales durations comes from
[calibration](calibration.md), the scheduler's deadline handling is on the
[scheduler](../scheduler/sleep-wake.md) page, and the primary entropy path the TSC only supplements is on
the [randomness](../crypto/randomness.md) page.
