...
scaled by the frequency, with a saturating subtraction (`timer/time.rs:29`), so it is a pure function of
...
clock, which is why the [scheduler](../scheduler/sleep-wake.md) and any timeout use this base rather than
the wall clock.
...
checks `crate::time::timestamp_secs() > entry.expires_at` (`src/security/crypto/key_management/ops.rs:101`)
or the trusted-hash and trusted-key databases stamp `last_update` with `crate::time::timestamp_millis()`
(`src/security/crypto/trusted_hashes.rs:33`), they are comparing against milliseconds since boot, not
against real calendar time. A wall-clock path does exist separately, `crate::sys::clock::unix_ms`
(`src/sys/clock/core.rs:39`), which adds `BOOT_UNIX_MS` to the elapsed TSC and is used by the
[time syscall](../syscall/README.md) and the watchdog, but the security freshness checks named above do
...
`BOOT_UNIX_MS`, which `clock::init` takes from the bootloader handoff (`src/kernel_core/init/entry.rs:43`)
...
set (`timer/time.rs:37`), so a caller seeing `None` is running in the pre-init window before
`timer::init` ran. If `now_ns` itself returns implausible values, the frequency is the thing to check:
`now_ns` substitutes a hard-coded `2_500_000_000` when `TSC_FREQUENCY` is still zero (`timer/time.rs:26`),
...
against is on the [calibration](calibration.md) page.
...
from [calibration](calibration.md); the [consumers](consumers.md) page collects who reads each base; the
scheduler's use of the monotonic base for sleep deadlines is on the
[scheduler](../scheduler/sleep-wake.md) page, and the security checks that read the uptime clock are in
the [security](../../security/README.md) tree.
