...
the ZK attestation gate (`src/kernel_core/process_spawn/capsule_spawn/runner/attest_gate.rs`) is the
...
`capsule_spawn/spec.rs:47` name each: `NonosIdCertRejected`, `ManifestRejected`, `AttestationRejected`).
...
Creating init" and "[UKERNEL] Entering userspace" (`src/kernel_core/init/entry.rs:124`,
