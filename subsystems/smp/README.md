...
| Page | What it covers |
|------|----------------|
| [per-cpu.md](per-cpu.md) | The page-aligned `PerCpuData`, CPU identity through the arch boundary (APIC id, MPIDR, hart id) mapped to a dense index, and the per-CPU `active_asid` that scopes TLB shootdowns. |
| [tlb-shootdown.md](tlb-shootdown.md) | The IPI broadcast-and-wait that invalidates a mapping across every CPU, the acknowledging IPI handler, the `invlpg` and CR3-reload primitives, and how the address-space scope is decided a layer up. |
...
complements is on the [process table](../process/process-table.md) page.
