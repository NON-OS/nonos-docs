...
| Page | What it covers |
|------|----------------|
| [field-and-poseidon.md](field-and-poseidon.md) | The Goldilocks field, and the arithmetization-friendly Poseidon-style hash, described precisely (NUMS constants, all-full-rounds, not the published reference set). |
| [stark.md](stark.md) | The AIR, the prover (LDE, commit, composition, DEEP, FRI), the verifier, the FRI low-degree test, and the two FRI variants. |
| [air-catalog.md](air-catalog.md) | The AIR gadgets, the `wired` mega-AIR, in-circuit Fiat-Shamir, and how the pieces compose into a recursive verifier. |
| [pedersen-attestation.md](pedersen-attestation.md) | The transparent Pedersen commitment (nothing-up-my-sleeve `H`), the Schnorr-style membership proof, and the classical-not-post-quantum caveat. |
