...
| Page | What it covers |
|------|----------------|
| [validation.md](validation.md) | The size floor, the ten header checks (class, endianness, machine, type), the bounds-checked program-header table, and the `ElfError` type. |
| [segments.md](segments.md) | `load_segment`: the W^X gate, permission derivation from `p_flags`, and per-page allocate-map-zero-fill (with `.bss` for free). |
| [layout.md](layout.md) | The load orchestration, the PIE-randomized load base, the relative relocations, and the relocated entry point. |
| [integration.md](integration.md) | Where the loader sits in the spawn pipeline: verify first, then create the address space and load into its ASID. |
