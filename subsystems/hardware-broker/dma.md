...
The **zero-scrub is an information-leak barrier.** `alloc_and_zero` (`dma/map/alloc.rs`) does not trust
...
The **honest boundary is the IOMMU.** NONOS carries an `IommuDomain` abstraction (`src/memory/iommu/`),
...
build), and each line maps to one `DmaMapError` (`dma/types.rs`), so a driver that cannot get its rings
...
the [device claim](claim.md) page, the user MMIO window the install competes for is on the
[MMIO](mmio.md) page, and the revocation paths are on the [revocation](revocation.md) page.
