...
ports `0xCF8`/`0xCFC`, from which it could reprogram any device on the bus. NONOS never grants a capsule
...
A grant refusal is one `PioError` (`pio/types.rs`): `NotClaimed` / `StaleEpoch` (no current claim),
...
[device claim](claim.md) page, and the revocation paths are on the [revocation](revocation.md) page.
