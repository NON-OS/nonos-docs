...
`init_from_pci` (`src/hardware/broker/table/init.rs:28`) turns the PCI enumeration into the broker's
...
capability. The table itself is one global `RwLock<Vec<DeviceRecord>>` (`table/state.rs:24`), so
reads (list, lookup, claim checks) run concurrently and the rare writes (init, platform
registration) take the write lock. The `DeviceRecord` is what the [MMIO](mmio.md), [DMA](dma.md),
[IRQ](irq.md), and [PIO](pio.md) paths resolve a request against; the `PciHandle`
...
`DeviceRecord` (`src/hardware/broker/device/record.rs:21`) is a `repr(C)` struct copied verbatim
...
(`device/bar.rs:31`) is size-asserted at 24 bytes, so the ABI cannot drift silently. A `Bar` carries
`base`, `size`, a `kind` (`None` / `Mmio` / `Pio`, `bar.rs:21`), and flags (prefetchable, 64-bit for
memory BARs). `record_from_pci` (`table/pci_record.rs:21`) fills the record from the PCI enumeration:
...
`classify_pci` (`src/hardware/broker/class.rs:49`) maps a device's PCI class, subclass, and prog-if
...
to drive a controller it does not understand. The same class id also selects the [DMA](dma.md) page
ceiling.

## Platform devices

Not every device comes from PCI. `register_platform_device`
(`src/hardware/broker/table/init.rs:44`) appends a hand-built `DeviceRecord` and assigns it a
`device_id` above the PCI range: the max existing id plus one, or `0x1_0000_0000` if the table is
still empty. The one caller today is `register_legacy` (`src/hardware/broker/platform.rs:31`), which
...
[PIO](pio.md) and [IRQ](irq.md) paths can grant against, exactly like a PCI device. In practice the
...
`src/syscall/microkernel/device.rs`, cap-gated at the contract layer (`MkDeviceList` needs
...
handler snapshots `list_by_class` (a class of 0 means all, `table/list.rs:29`), and if the buffer is
...
`sys_device_release` (`device.rs:88`) is the unbind, covered on the [revocation](revocation.md) page:
...
everything else. The entire authority is one pure validator (`pci/allowlist.rs`):
...
`MkPciConfigWrite` (`pci/write.rs:26`) resolves the caller's ownership through `resolve`
(`pci/ownership.rs:24`, the same claim-pid-epoch check as every grant path), reads the current
register, runs the validator, and applies only the allowed action (`pci/write.rs:34`), so a capsule
...
are programmed only by the kernel on the [IRQ](irq.md) bind path.
...
split the [MMIO](mmio.md) MSI-X clamp enforces from the other side.
...
problem and is diagnosed on the [claim](claim.md) page. The `MkDeviceRelease` path emits a bounded
