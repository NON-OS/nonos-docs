...
revocation interacts with the [claim](claim.md) epoch. The per-class code is in each class's
...
**MMIO** (`mmio/release.rs`) unmaps the user pages of the grant. `unmap_one` (`mmio/release.rs:56`)
calls `unmap_user_mmio` on the grant's `user_va` for its `length`, which walks the pages and, per
the paging manager, emits a per-asid SMP TLB shootdown (`memory/paging/manager/api/mapping.rs:149`).
...
**DMA** (`dma/release.rs:61`) does three ordered steps in `teardown`: scrub the buffer, unmap the
user pages, then free the frames. `scrub_buffer` (`dma/release.rs:80`) zeroes the whole run through
...
(`memory/paging/manager/api/mapping.rs:187`) drops the pages and shoots down the TLB. Then
...
**IRQ** (`irq/release.rs:84`) unwinds by grant kind. An INTx grant masks the IO-APIC line
...
(`irq/release.rs:91`). The IO-APIC redirection entry itself is left programmed on purpose: the line
...
device-side enable bit returns to its post-reset state (`irq/release.rs:104`). The broker vector
...
**PIO** (`pio/release.rs`) simply removes the grant record. There is no physical port state the
...
(`mmio/release.rs:52` calls `claim::release_all_for_pid`), so on the pid drain the claim and its
...
(`src/syscall/microkernel/device.rs:88`). The order there is deliberate: it drains all four grant
...
address space is currently active (`src/process/exit/teardown.rs:32`):
...
The finalize path (`src/process/exit/finalize.rs:18`) runs the same four releases with the flag set
...
The one honest boundary is shared with [DMA](dma.md): revocation unmaps the CPU-side view and frees
...
`StaleEpoch` at its own path, which is the [claim](claim.md) page's diagnostic, not a revocation
