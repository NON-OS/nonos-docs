...
| Page | What it covers |
|------|----------------|
| [inbox.md](inbox.md) | The per-process bounded queue, the registry with an explicit owner pid, the fail-closed `try_enqueue_strict` (`MissingInbox` / `DeadOwner` / `QueueFull`), the receive loop, and the teardown tie. |
| [routing.md](routing.md) | The capability check on every named route, resolving an endpoint to a destination inbox, the kernel-stamped sender identity, the wake, and the send syscalls. |
| [envelope.md](envelope.md) | The `IpcMessage` envelope, the boot-time keyed MAC seed (fatal on RNG failure), the domain-separated BLAKE3 MAC, the constant-comparison integrity check, and its honest scope. |
| [pipe.md](pipe.md) | The anonymous byte FIFO, the ring buffer, the read/write fds, the `EAGAIN` / `EPIPE` / `EBADF` and end-of-stream semantics, and where the fd table consumes it. |
...
read are defined in the [capability model](../../security/capabilities-and-tokens.md), and the
send and receive calls are part of the `Mk*` family on the [syscall boundary](../syscall/boundary.md).
