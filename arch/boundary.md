...
on x86_64 the scheduler proves the root before the switch (see the [x86_64 backend](x86_64.md)).
`flush_tlb_one` invalidates only the calling CPU by construction; cross-CPU invalidation is not this
primitive's job and is handled by the [SMP shootdown](../subsystems/smp/tlb-shootdown.md) layer, so a
...
capsule would want, and they sit behind their own seams (the [hardware broker](../subsystems/hardware-broker/README.md),
the [syscall boundary](../subsystems/syscall/boundary.md)) with their own capability gates, not inside
...
[workflows](../build/workflows.md) page.
...
The x86_64 realisation of each primitive is on the [x86_64 backend](x86_64.md) page, the ARM and RISC-V
realisations are on the [aarch64](aarch64.md) and [riscv64](riscv64.md) pages, and the arch-gated
features that live outside the boundary are on the [platform discovery](platform-discovery.md) page.
