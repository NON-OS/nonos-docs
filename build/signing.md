...
## 5. Attestation enrollment

Signing proves who published a capsule and that its bytes match the manifest.
Attestation is the second, independent layer: a proof that the capsule is enrolled
under the policy root, checked at every spawn (see
[capsule attestation](../security/attestation.md)). In the production posture the
proof is a transparent STARK, generated by `nonos-stark-enroll`
(`nonos-stark-enroll/src/main.rs:209`).

Enrollment is over the whole set at once, not per capsule, because the policy root
commits to every enrolled capsule. The build runs `nonos-stark-enroll capsules`,
passing each capsule as `CAPS:image:trailer-out`, where CAPS is the granted
capability mask. The tool measures every image, commits them to one policy tree
padded to the gate's fixed depth with a domain-separated slot no ELF can occupy,
writes the policy root, and writes each capsule's `NZKSTRK1` trailer bound to its
image and its capabilities. Before it writes a trailer it re-runs the exact
spawn-gate parse over it (`nonos-stark-enroll/src/main.rs:142`), so a trailer that
would be refused at boot fails at build. The Makefile drives this from the
`$(ZK_CAPSULE_ROOT)` rule, and
`make nonos-mk-zerostate` enrolls the set and builds the kernel with the STARK
gate enforced.

The earlier enrolled-secret backend, still the default build, generates its
trailer with the `capsule-attest-proof` tool from the attestation circuit instead.
Both write a `<bin>.zk_trailer.bin` next to the manifest.

## 6. Kernel embedding and verification
