...
Plain `make` with no target builds the full production image instead: the
attested ZeroState kernel, the EFI System Partition, and a bootable ISO.
...
Kernel self-attestation (`NONOS_STARK_KERNEL_ATTEST`) has the bootloader verify
the kernel's own STARK proof against the enrolled kernel root before the jump.
It is on by default, so a clean build ships the full STARK boot for the kernel
and the capsules alike; set `NONOS_STARK_KERNEL_ATTEST := 0` for a build that
only wants signature trust.
