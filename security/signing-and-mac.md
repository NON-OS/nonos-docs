# The Token Signing Key and MAC

The [capability token](capabilities-and-tokens.md) is trustworthy because it
...
(`src/capabilities/token/signing_key.rs:19`):
...
(`src/capabilities/token/material.rs:25`). Every authority field of the token has
...
(`src/capabilities/token/create.rs:25`):
...
a specific process address space and revocation counter (`src/process/caps.rs`).
...
`sign_token` (`src/capabilities/token/sign.rs:24`) is what stamps the signature:
...
Verification is the mirror of signing (`src/capabilities/token/verify.rs:24`):
...
matched. `ct_eq_64` (`src/crypto/util/constant_time/compare.rs:51`) is written to
that end:
