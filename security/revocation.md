...
(`src/capabilities/token/revocation.rs:22`):
...
(`src/capabilities/token/validate.rs:21`):
...
capability module (`src/process/caps.rs`).
...
anchor](trust-anchor.md). Its policy carries three lists, checked during verified
...
key](signing-and-mac.md) is minted fresh each boot, and every token binds the
...
model](capabilities-and-tokens.md) resolver chain is where the runtime ones land:
...
spawn](capsules-and-trust.md), because a capsule that is already running was
