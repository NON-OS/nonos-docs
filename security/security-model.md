...
This is the whole security story of NONOS in one page: what it defends against, the chain that decides
...
page](../architecture/verification.md) is the proof.
...
NONOS is a microkernel that runs untrusted capsules and talks to untrusted peripherals and networks, so
...
Nothing runs on NONOS by default. A capsule becomes a process only by passing an ordered, fail-closed
...
```
  baked trust anchor            (in the signed kernel image, non-optional)
        |  signs
  NONOS-ID certificate          (publisher identity + capability ceiling + namespace)
        |  signs
...
- The **[trust anchor](trust-anchor.md)** is baked into the signed kernel image and is not optional. It
...
- The **[certificate](certificate-schema.md)** binds a publisher identity to a capability ceiling and a
  namespace, and is signed by the anchor. A capsule can never hold a capability above its certificate's
  ceiling.
- The **[manifest](manifest-schema.md)** declares the exact capabilities the capsule requests and the
...
- The **[attestation](attestation.md)** gate binds the running image to an enrolled secret through a
...
- **[Verified spawn](capsules-and-trust.md)** runs the whole pipeline in order, and only after every
...
trust](capsules-and-trust.md) page.
...
Capabilities can be narrowed but never widened. **[Delegation](delegation.md)** lets a capsule hand a
...
**[Revocation](revocation.md)** withdraws authority at four scopes: the per-boot signing key and nonce,
a per-process epoch, a per-token revoked set, and the anchor's lists. **[Signing and
MAC](signing-and-mac.md)** is the per-boot keyed integrity under all of this, a two-pass keyed BLAKE3
...
- **Ring-3 capsules with brokered hardware.** No driver runs in the kernel. A driver reaches its device
  only through the [hardware broker](../subsystems/hardware-broker/README.md), which lends narrow,
  revocable grants: a slice of one BAR ([MMIO](../subsystems/hardware-broker/mmio.md)), a class-capped
  DMA buffer ([DMA](../subsystems/hardware-broker/dma.md)), an interrupt it can wait on but never
  program ([IRQ](../subsystems/hardware-broker/irq.md)), a port window ([PIO](../subsystems/hardware-broker/pio.md)),
  all checked against an exclusive epoch-stamped [claim](../subsystems/hardware-broker/claim.md). A
...
- **Address-space isolation.** Each capsule has its own address space; a capsule reaches another only by
  named [IPC](../subsystems/ipc/README.md) to a registered endpoint, never by a shared pointer, and the
...
- **W^X and guard pages.** No page is writable and executable; device mappings are no-execute; adjacent
  ...
  on the [paging](../subsystems/memory/paging-manager.md) and [MMIO](../subsystems/hardware-broker/mmio.md)
  pages.
- **Zeroization.** Freed memory and freed DMA buffers are scrubbed before reuse, so no capsule receives
  another's residue. See [zeroization](../subsystems/memory/zeroization.md).
...
- **No IOMMU by default.** The `IommuDomain` backend is behind the `nonos-arch-iommu` feature and is not
  ...
  boundary and is stated on the [DMA](../subsystems/hardware-broker/dma.md) page.
- **The security clock is uptime.** Temporal checks read an uptime-based clock with no authenticated
  ...
  pages](../subsystems/time-and-clock/README.md).
...
- **Side channels are out of scope of the proofs.** The crypto is portable software and not everywhere
  constant-time; the [crypto pages](../subsystems/crypto/README.md) mark where.
