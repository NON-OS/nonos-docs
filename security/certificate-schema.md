# The NØNOS-ID Certificate

The certificate is a capsule publisher's durable identity. Where a
[manifest](manifest-schema.md) is a per-build statement that changes every time
the capsule is rebuilt, the certificate is the long-lived root beneath it: it is
signed by the [trust anchor](trust-anchor.md), it sets the hard ceiling on the
...
The type is `NonosIdCertificate` (`src/security/nonos_id_cert/schema/cert.rs:26`).
The current schema version is 2 (`nonos_id_cert/schema/constants.rs:17`).
...
(`nonos_id_cert/schema/constants.rs`) and every text or key field is a fixed
...
the manifest's namespace under `glob_match` (`nonos_id_cert/schema/glob_match.rs`),
...
The publisher keys are the keys that sign manifests (`nonos_id_cert/schema/sub.rs:35`):
...
(`nonos_id_cert/schema/sub.rs:49`):
...
Stage one (`nonos_id_cert/verify/checks.rs`) reads `trust_anchor_epoch` to reject
...
gate](capsules-and-trust.md); the anchor it verifies against is the [trust
