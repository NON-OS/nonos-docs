...
capabilities to another (`src/capabilities/delegation/types.rs:23`):
...
The `sign_delegation` call (`src/capabilities/delegation/sign.rs`) stamps the MAC.
The module also carries an unchecked constructor
(`src/capabilities/delegation/create_unchecked.rs`) that builds and signs a
...
The material is a 48-byte buffer (`src/capabilities/delegation/material.rs:22`):
...
return (`src/capabilities/delegation/verify.rs`).
...
(`src/capabilities/delegation/error.rs:18`):
...
of the [capability token](capabilities-and-tokens.md) and is handled by the chain
