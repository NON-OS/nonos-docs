...
capability check described in [the capability model](../security/capabilities-and-tokens.md).
The authoritative tables in the source are `src/syscall/numbers/defs.rs` for the
numbers and `src/syscall/contract/cap_table/mk.rs` for the capability each call
...
(`src/syscall/abi/tag.rs`). The tags read as mnemonics in a trace, which is the
...
(`src/syscall/contract/dispatch.rs:31`). The "Cap" column below names the
...
`#[cfg(target_arch = "x86_64")]` (`src/hardware/broker/mod.rs`). The aarch64 and
