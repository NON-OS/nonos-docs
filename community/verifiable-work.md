# Contributing verifiable work

NØNOS previously documented a Groth16/ceremony-based NOX receipt lane.
...
The receipt and schema formats are frozen in the kernel repository under
`abi/` (`nox_zk_contribution_receipt.schema.json` and
`nox_zk_work_receipt.schema.json`). The identifier derivations and the
on-chain interface are documented in
[abi/NOX_CONTRACTS.md](https://github.com/NON-OS/nonos-micro-kernel/blob/main/abi/NOX_CONTRACTS.md).
