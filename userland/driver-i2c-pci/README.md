...
capsule at boot through the bus-driver spawn plan (`src/hardware/i2c_pci_capsule/embed.rs:10`,
...
|------|---------|----------------|
| [operations.md](operations.md) | `src/protocol/`, `src/server/` | The `NI2C` wire header, the six opcodes, the status words, and the per-op handlers. |
| [bring-up.md](bring-up.md) | `src/setup/`, `src/init/`, `src/discover.rs` | Intel/PCI discovery, the claim/map/bind broker grants, and the DesignWare reset with its SCL clock program. |
| [transfer-engine.md](transfer-engine.md) | `src/transaction/` | The polled master transfer: address and length checks, the enable/disable bracket, and the four-phase engine loop with its timing budget. |
| [contributing.md](contributing.md) | the whole tree | Where to work, how to add a controller or an operation, the build and sign steps, and the code standards. |
| [debugging.md](debugging.md) | runtime | The boot marker, the bring-up failure strings, the "no SCL clock" trap, and how the transfer status words map to real faults. |
...
words read at init, and the register accessor (`src/driver.rs:3`).
...
this driver parses (`userland/capsule_driver_i2c_hid/src/i2c_client/service.rs:3`,
`userland/capsule_driver_i2c_hid/src/i2c_client/wire.rs:1`). HID-over-I2C sits above this driver and
...
- **Discovery is Intel and PCI only.** A record qualifies only if it is an Intel vendor PCI serial-bus
  device whose PCI id is in the driver's static table (`src/discover.rs:41`, `src/constants/mod.rs:28`).
  A controller enumerated through ACPI rather than PCI, or a PCI id not in the table, is not matched and
  the capsule exits with `controller not found`. See [bring-up.md](bring-up.md).
- **Transfers are polled, not interrupt-driven.** The IRQ is bound and acked once at bring-up so the
  line does not stay asserted (`src/setup/sequence.rs:16`), but transfers busy-wait on FIFO status inside
  a fixed iteration budget rather than blocking on the interrupt (`src/transaction/engine/run.rs:32`).
  See [transfer-engine.md](transfer-engine.md).
- **The IRQ bind is legacy INTx, not MSI-X.** The bind request passes the `irq_line` with zero flags, so
  the broker takes the INTx path (`src/setup/irq.rs:7`). The MSI-X table pages are withheld by the broker
  from the MMIO mapping in any case. See [bring-up.md](bring-up.md).
