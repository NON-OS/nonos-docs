# Contributing to capsule_driver_i2c_pci
...
read the [README](README.md), the [operations](operations.md), the [bring-up](bring-up.md), and the
[transfer engine](transfer-engine.md) pages in this folder.
...
exits 1 (`src/main.rs:19`). The top-level modules are declared there (`src/main.rs:8`).

## Module map

| Folder or file | Owns | Touch it when |
|---|---|---|
| `src/discover.rs` | PCI enumeration and the Intel/PCI match | you add a controller or change the match rule |
| `src/constants/mod.rs` | the PCI-id table and the DesignWare register offsets and bits | you add an id range or a register |
...
| `src/driver.rs`, `src/regs.rs` | the runtime state and the volatile register accessor | rarely |

## Adding a controller

To support another Intel LPSS I2C function, extend the PCI-id match in `device_info` with the new id or
id range and its source clock (`src/constants/mod.rs:28`). Discovery and bring-up pick it up with no
other change: `find_controller` already gates on `device_info` returning `Some`
(`src/discover.rs:44`, `discover.rs:64`), and the clock program derives its counts from whatever source
clock the table returns (`src/init/scl.rs:44`). This only helps a PCI-enumerated controller; an
ACPI-only controller needs discovery work, which is the named gap in [bring-up.md](bring-up.md).

## Adding an operation

The four fixed-width read operations share a shape, and adding one is four edits.

1. Define the opcode constant in `src/protocol/ops.rs` and re-export it from `src/protocol/mod.rs:13`.
2. Write the handler as one file under `src/server/handlers/`, next to the existing ones. A read handler
   ...
   the way `timing.rs` does (`src/server/handlers/timing.rs:6`). Declare it in
   `src/server/handlers/mod.rs`.
3. Wire it into the dispatch match in `src/server/runner.rs:35`. Keep a fixed-width read gated on an empty
   body with the `if body.is_empty()` guard the existing four use, so a stray body falls to `E_INVAL`
   (`src/server/runner.rs:49`).
4. If the operation carries a request body (like `OP_TRANSFER` or `OP_PROBE`), parse and bound it in the
   ...
   declared write length (`src/server/handlers/transfer.rs:30`) and `probe.rs` rejects an address over
   `0x7F` (`src/server/handlers/probe.rs:7`).
...
- No panics in capsule code. No `unwrap`, `expect`, or `panic!`. Every path returns an error as a status
  ...
  saturating and clamped so no input can overflow (`src/init/scl.rs:31`), and every wait loop has a finite
  iteration budget (`src/transaction/engine/run.rs:32`).
- One unit per file. New handlers are one op per file under `server/handlers/`; new transfer helpers are
  ...
  `src/transaction/engine/mod.rs:16` and `src/protocol/mod.rs:1` are.
- The AGPL header is present today on `src/discover.rs:1` and across the `src/transaction/` tree (for
  example `src/transaction/engine/run.rs:1`), but the rest of the tree, including `src/main.rs`,
  `src/protocol/`, `src/server/`, `src/setup/`, and `src/init/`, does not carry it yet. New files should
  add the header byte for byte as it appears on `src/transaction/engine/run.rs:1`, and backfilling the
