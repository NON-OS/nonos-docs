# Controller bring-up and broker grants

This page covers the one-shot path that turns a PCI record into a live I2C master: discovery, the three
broker grants, and the DesignWare reset with its SCL clock program. It mirrors `src/discover.rs`,
`src/setup/`, and `src/init/`. For the request server that runs afterward see
[operations.md](operations.md); for the transfer engine the master drives see
...
(`src/discover.rs:34`, `discover.rs:36`). A record qualifies only if `is_intel_i2c` holds: an Intel
vendor (`0x8086`) PCI (`BUS_KIND_PCI`) device of class serial bus (`0x0c`) whose PCI device id is in the
driver's static table (`src/discover.rs:60`). On top of that it must have a real interrupt pin and line
(`irq_pin != 0` and `irq_line != 0xFF`) and a non-zero MMIO BAR0 (`src/discover.rs:41`, `discover.rs:46`).
...
older parts and 100 MHz on Tiger Lake and later (`src/constants/mod.rs:28`).
...
fails with `i2c-pci: controller not found` (`src/setup/sequence.rs:10`). On a laptop where the LPSS
...
- **Claim.** `mk_device_claim` returns the epoch; a non-positive result is `i2c-pci: device claim failed`
  (`src/setup/claim.rs:3`, `claim.rs:5`). The epoch is the anti-stale token every later grant carries;
  see the [device claim](../../subsystems/hardware-broker/claim.md) page.
- **Map.** `mk_mmio_map` maps BAR0 at offset 0, rounding the BAR0 size up to a page boundary
  ...
  failed` (`src/setup/mmio.rs:8`, `mmio.rs:11`). The broker clamps the mapping to the BAR and withholds
  any MSI-X table pages; see the [MMIO](../../subsystems/hardware-broker/mmio.md) page. The returned user
  VA becomes the register base (`src/setup/sequence.rs:14`).
- **Bind.** `mk_irq_bind` binds the device's PCI interrupt line, and on failure unmaps the MMIO grant and
  releases the claim before returning `i2c-pci: irq bind failed` (`src/setup/irq.rs:5`, `irq.rs:9`). This
  is a legacy INTx bind: the request passes `irq_line` with zero flags, so the broker takes the INTx
  path, not MSI-X (`src/setup/irq.rs:7`; the two bind modes are on the
  [IRQ](../../subsystems/hardware-broker/irq.md) page).

## Controller reset and the clock program

`bring_up` (`src/init/mod.rs:14`) resets the DesignWare master into a known state. It first disables the
controller and spins on `IC_ENABLE_STATUS` until the enable bit clears, failing with `i2c-pci:
controller disable timeout` if it never does (`src/init/mod.rs:47`, `mod.rs:55`). While disabled it
writes `IC_CON` to select master mode, fast speed, restart-enable, and slave-disable
(`src/init/mod.rs:16`, `src/constants/mod.rs:65`), programs the SCL clock, zeroes the RX and TX FIFO
thresholds, masks all interrupts (`IC_INTR_MASK` = 0), and reads `IC_CLR_INTR` once to clear pending
state (`src/init/mod.rs:24`, `mod.rs:26`, `mod.rs:27`). Finally it reads and caches the component type,
component param, enable status, and status registers, which are the values `OP_CONTROLLER_INFO` returns
and the first four words of the register snapshot (`src/init/mod.rs:28`).
...
disabled (`src/init/mod.rs:20`, `mod.rs:36`). `program_clock` writes the standard and fast SCL high/low
pairs and the SDA hold time, all derived from the discovered source clock (`src/init/mod.rs:37`). The
...
(`src/init/scl.rs:7`, `scl.rs:23`). The arithmetic is saturating and every result is clamped into the
`u16` register width and to a floor of 1, so no source clock can produce a zero count, an overflow, or a
panic (`src/init/scl.rs:31`, `scl.rs:39`). This is the "no SCL clock" failure class the
[debugging](debugging.md) page covers: a controller whose count pairs were left at zero looks alive but
never toggles the line.

After bring-up the sequence acks the IRQ grant once (`src/setup/sequence.rs:16`) and assembles the
`Driver` (`src/setup/sequence.rs:17`).
...
and the register accessor (`src/driver.rs:3`). The accessor does raw volatile 32-bit reads and writes at
fixed offsets into the granted window (`src/regs.rs:11`), so every access lands inside the mapping the
broker installed.
