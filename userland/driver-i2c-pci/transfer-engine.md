...
request handlers that call in see [operations.md](operations.md); for the bring-up that programs the
clock this engine relies on see [bring-up.md](bring-up.md).

## Request and result types

A transfer is described by `TransferRequest`: a 7-bit address, a `u16` flags word, a borrowed write
slice, and a read length (`src/transaction/types/transfer_request.rs:16`). The one defined flag is
`FLAG_RESTART_ON_READ` (bit 0, `src/transaction/types/flags.rs:16`). The result is `TransferResult`: a
fixed 64-byte read buffer, the number of bytes actually read, and the DesignWare abort source latched on
a NACK (`src/transaction/types/transfer_result.rs:18`, `src/transaction/types/result_empty.rs:21`).
`TransferError` is the four-way failure enum the handlers map to errnos: `Busy`, `Timeout`, `Nack`,
`Invalid` (`src/transaction/types/transfer_error.rs:16`).

## The transaction bracket

`transfer` brackets the engine with controller state changes (`src/transaction/engine/transfer.rs:21`).
It first validates the address against `0x7F` and the write/read lengths against 64 via `valid_lengths`,
returning `Invalid` on either (`src/transaction/engine/transfer.rs:25`,
`src/transaction/types/valid_lengths.rs:18`). Then it waits for the master to go idle, sets the target
address, enables the controller, runs the engine, and disables the controller again regardless of the
engine's outcome (`src/transaction/engine/transfer.rs:29`, `transfer.rs:33`).
...
- `wait_idle` spins on `IC_STATUS` master-activity and returns `Busy` if the master never goes idle
  (`src/transaction/control/wait_idle.rs:20`, `wait_idle.rs:27`).
- `set_target` disables the controller if it is currently enabled, writes the 7-bit address to `IC_TAR`,
  and re-enables it (`src/transaction/control/set_target.rs:23`). The address change requires the
...
- `enable` writes `IC_ENABLE` and waits for the enable-status bit to reach 1; `disable` writes 0 and waits
  for it to reach 0; both time out with `Timeout` otherwise (`src/transaction/control/enable.rs:23`,
  `src/transaction/control/disable.rs:22`, `src/transaction/control/wait_enable_state.rs:20`).

## The engine loop

The engine itself is `run` (`src/transaction/engine/run.rs:28`). It computes the total command count as
`write.len() + read_len` and loops up to `TIMEOUT_ITERS` (250,000, `src/constants/mod.rs:62`) times over
four phases:

1. **Abort check.** If `IC_RAW_INTR_STAT` shows the TX-abort bit, it latches `IC_TX_ABRT_SOURCE` into the
   result, reads `IC_CLR_TX_ABRT` to clear, and returns `Nack` (`src/transaction/engine/check_abort.rs:20`,
...
2. **Drain RX.** While the RX FIFO is not empty (`IC_STATUS` RFNE bit) and read bytes remain, it pops the
   low byte of `IC_DATA_CMD` into the read buffer (`src/transaction/engine/drain_rx.rs:20`).
3. **Issue commands.** While commands remain and both the TX and RX FIFOs have space, it issues one
   ...
   `src/transaction/engine/take_write.rs:16`), or a read command for the read phase (`read_cmd`,
   `src/transaction/engine/read_cmd.rs:19`), and tags the final command with `IC_DATA_CMD_STOP`
   (`src/transaction/engine/run.rs:36`, `run.rs:42`). A repeated-start is set on the first read after a
   write only when `FLAG_RESTART_ON_READ` is present (`src/transaction/engine/read_cmd.rs:20`). FIFO space
   is computed against a fixed 64-entry depth (`src/transaction/engine/tx_space.rs:19`,
   `src/transaction/engine/rx_space.rs:19`, `src/constants/mod.rs:63`).
4. **Completion.** Once every command has been issued and every expected read byte drained, `done` checks
   ...
   (`src/transaction/engine/run.rs:48`, `src/transaction/engine/done.rs:19`). If the budget runs out
   first, it returns `Timeout` (`src/transaction/engine/run.rs:54`).

## Probe

`probe` is a one-byte read transfer whose outcome is folded to a boolean: `Ok` maps to present, a `Nack`
maps to absent, and any other error propagates (`src/transaction/engine/probe.rs:21`). This is what lets
...
asserted (`src/setup/sequence.rs:16`), but transfers busy-wait on FIFO status inside the fixed 250,000
iteration budget rather than blocking on the interrupt (`src/transaction/engine/run.rs:32`). The transfer
buffers are small and controller-local (64 bytes each), and every wait has a finite budget
(`src/transaction/control/wait_idle.rs:21`, `src/transaction/control/wait_enable_state.rs:22`), so the
