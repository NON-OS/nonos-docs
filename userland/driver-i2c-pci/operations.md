# Operations and the wire protocol
...
[README](README.md); for the transfer engine that two of these operations drive see
[transfer-engine.md](transfer-engine.md).

## The server loop

The server is a blocking receive loop (`src/server/runner.rs:14`). It allocates a fixed receive and
transmit buffer of `HDR_LEN + IPC_PAYLOAD_MAX` bytes (20 + 256, `src/protocol/limits.rs:1`), receives
from the service inbox with the sender pid, parses the header, and dispatches on the opcode. A message
that fails to parse or comes from pid 0 is dropped silently (`src/server/runner.rs:20`). Every reply is
sent back to that pid through `mk_ipc_reply` (`src/server/respond.rs:6`).

## The NI2C envelope

The wire header is the shared capsule envelope: magic `0x4E49_3243` ("NI2C"), version `1`, a 2-byte
opcode, an 8-byte request id, and a 4-byte body length, 20 bytes total (`src/protocol/header.rs:1`). All
multi-byte integers are little-endian. The decoder rejects a short buffer, a wrong magic or version, or a
body length that runs past the buffer (`src/protocol/decode.rs:4`, `decode.rs:9`, `decode.rs:15`).
...
(`src/protocol/encode.rs:3`).

## Status words

The status word is a signed 32-bit errno (`src/protocol/errno.rs:1`):
...
Six opcodes are defined (`src/protocol/ops.rs:1`). The four fixed-width read operations are gated on an
...
(`src/server/runner.rs:35`, `runner.rs:46`, `runner.rs:49`).

| Opcode | Name | Body in | Body out | Handler |
|---|---|---|---|---|
| 1 | `OP_HEALTHCHECK` | empty | 1 byte `1` | `src/server/handlers/health.rs:4` |
| 2 | `OP_CONTROLLER_INFO` | empty | 64-byte identity | `src/server/handlers/controller.rs:5` |
| 3 | `OP_REGISTER_SNAPSHOT` | empty | 40 bytes (ten u32) | `src/server/handlers/snapshot.rs:6` |
| 4 | `OP_TIMING_INFO` | empty | 28 bytes (seven u32) | `src/server/handlers/timing.rs:6` |
| 5 | `OP_TRANSFER` | 8-byte head + write | read length + abort + read bytes | `src/server/handlers/transfer.rs:6` |
| 6 | `OP_PROBE` | 1 byte address | 1 byte present/absent | `src/server/handlers/probe.rs:6` |

### OP_HEALTHCHECK (opcode 1)

Empty body in, `E_OK` with a single byte `1` out (`src/server/handlers/health.rs:5`). The liveness probe;
...
(`src/server/handlers/controller.rs:5`):
...
The family name is copied in truncated to 22 bytes (`src/server/handlers/controller.rs:15`).

### OP_REGISTER_SNAPSHOT (opcode 3)

Empty body in, `E_OK` with a 40-byte body of ten little-endian u32 values
(`src/server/handlers/snapshot.rs:7`). The first four are the values cached at init (`comp_type`,
...
(0x6C), whose offsets are in `src/constants/mod.rs:6`. These are side-effect-free status reads.
...
0x3C) (`src/server/handlers/timing.rs:7`). This is how a client reads back the clock program the driver
applied during bring-up, which is why the "no SCL clock" check in [debugging.md](debugging.md) uses it.

### OP_TRANSFER (opcode 5)

Request: an 8-byte fixed head followed by the write bytes
(`src/server/handlers/transfer.rs:22`):
...
`E_INVAL` (`src/server/handlers/transfer.rs:23`, `transfer.rs:30`). `FLAG_RESTART_ON_READ` (bit 0,
`src/transaction/types/flags.rs:16`) inserts a repeated-start before the first read of a write-then-read
transaction; see [transfer-engine.md](transfer-engine.md). Write and read are each bounded to 64 bytes
...
(`src/server/handlers/transfer.rs:36`). The handler then hands the request to the transfer engine and
maps its error straight to a status word: `E_BUSY`, `E_TIMEOUT`, `E_NACK`, or `E_INVAL`
(`src/server/handlers/transfer.rs:44`).
...
(absent), and any other error is surfaced as its errno (`src/transaction/engine/probe.rs:23`,
`src/server/handlers/probe.rs:11`). The distinction matters: a clean NACK is a definite "nobody home",
not a bus failure.
