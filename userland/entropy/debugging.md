# Debugging capsule_entropy

This page lists the log markers the entropy capsule and its boot path emit, and the concrete failure
modes with where to look for each. For the operations and error codes see the [README](README.md), the
[operations and protocol](operations.md) page, and the [pool](pool.md) page in this folder.
...
spawned")` (`src/userspace/init/capsule_boot/run.rs:29`), and `ok` prints `[<tag>] <msg>`
(`src/sys/boot_log/output.rs:33`). If that line is absent the capsule never started, and the `Err` arm
logged an `[ERROR]` line with the mapped `SpawnError` instead (`src/userspace/init/capsule_boot/run.rs:32`,
`src/sys/boot_log/output.rs:49`), which is the usual signature, manifest, or capability failure.

The kernel-side spawn also carries a debug tag `[ENTROPY-DEBUG] load_elf_executable error:` for an ELF
load failure (`src/security/entropy_capsule/spawn.rs:55`).

## Failure modes

The error codes are the diagnosis. Each op returns an `i32` status in the first four bytes of its reply
payload, and the three codes mean different things (`src/protocol/errno.rs:17`).
...
(`src/pool/fill.rs:49`, `src/server/handlers/getrandom.rs:38`). The `source_failures` counter increments
on every give-up (`src/pool/fill.rs:50`), so the way to tell a dead source from a busy one is `GET_STATS`:
...
The request asked for more than 4096 bytes (`src/server/handlers/getrandom.rs:32`). The kernel client
also rejects an over-4096 request before it ever round-trips
(`src/security/entropy_capsule/client/get_random.rs:28`), so if you see `EMSGSIZE` on the wire the call
...
(`src/server/runner.rs:41`, `src/server/handlers/reseed.rs:34`, `src/server/dispatch.rs:31`). This is
...
never comes (`src/server/runner.rs:41`).
...
leaves the kernel: `CAP_ENTROPY` for stats (`src/security/entropy_capsule/capability.rs:23`) and
`CAP_ADMIN` for reseed (`src/security/entropy_capsule/capability.rs:35`). A denial there is an
access-denied error from the client, not a status code from the capsule. The pid checked is read from
the kernel's process accounting, not from the request (`src/security/entropy_capsule/capability.rs:24`),
...
fails transport (`src/syscall/dispatch/crypto/random.rs:29`), so a caller can be served while the capsule
...
(`src/syscall/dispatch/crypto/random.rs:58`).
