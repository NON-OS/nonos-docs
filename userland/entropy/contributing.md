# Contributing to capsule_entropy
...
together, read the [README](README.md), the [operations and protocol](operations.md) page, and the
[pool](pool.md) page in this folder.
...
(`userland/capsule_entropy/src/main.rs:29`). The three top-level modules are declared there
(`src/main.rs:22`).
...
`src/security/entropy_capsule/protocol.rs` must match it bit-for-bit; the capsule's own protocol comment
calls it out as authoritative (`src/protocol/types.rs:17`).
...
1. Add the opcode constant in `src/protocol/types.rs:30` and the matching constant in the kernel mirror
   `src/security/entropy_capsule/protocol.rs:28`. The two must agree on the number.
...
2. Write the handler as one file under `src/server/handlers/`, next to the existing ones. A handler that
   ...
   `get_random`, `src/server/handlers/getrandom.rs:27`); one that does not touch the pool takes only
   `req` (the shape of `healthcheck`, `src/server/handlers/healthcheck.rs:24`). Build the reply with
   `encode_response`, putting the status word first (`src/protocol/encode.rs:25`). Re-export the handler
   from `src/server/handlers/mod.rs:22`.

3. Wire it into the match in `src/server/dispatch.rs:25`. Add a `use` for the new op constant to the
   import list (`src/server/dispatch.rs:21`) and a match arm that calls the handler
   (`src/server/dispatch.rs:27`). A word no arm matches falls to the `_ =>` arm and returns `EINVAL`
   (`src/server/dispatch.rs:31`).

4. If the kernel needs to call the op, add a client under `src/security/entropy_capsule/client/` and
   re-export it from `src/security/entropy_capsule/client/mod.rs:25`. If the op must be
   capability-checked, gate it in `src/security/entropy_capsule/capability.rs` the way `gate_read`
...
[pool.md](pool.md); the honesty of the randomness posture is a documented property of this capsule.
...
- No panics in capsule code. No `unwrap`, `expect`, or `panic!`. The decoder is explicit that it never
  panics and never unwraps (`src/protocol/decode.rs:28`), and the release profile is `panic = "abort"`
  (`Cargo.toml:25`).
- One unit per file. New handlers are one op per file under `src/server/handlers/`, and `mod.rs` is used
  only for re-exports, matching the existing tree (`src/pool/mod.rs`, `src/server/mod.rs`).
