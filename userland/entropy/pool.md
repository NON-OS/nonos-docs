# The pool and the randomness source
...
the operations that call into the pool, read [operations.md](operations.md). For identity and the
...
(`src/pool/types.rs:26`); it is an accounting object, not a mixed entropy buffer. Random bytes come
verbatim from the CPU hardware random generator, `RDRAND`, word by word, with no whitening, no health
test beyond a retry loop, and no secondary source mixed in (`src/pool/fill.rs:22`). `RESEED` mixes
nothing: it only bumps a counter (`src/pool/record_reseed.rs:21`).
...
this capsule (`src/syscall/dispatch/crypto/random.rs:29`).

## The four counters

`Pool` holds four relaxed `AtomicU64` counters and nothing else (`src/pool/types.rs:26`):
...
reports a source failure (`src/pool/fill.rs:30`). Output is filled eight bytes at a time, the last chunk
truncated to the requested length (`src/pool/fill.rs:34`). There is no software mixing; the bytes are the
hardware generator's output verbatim.

`Pool::fill` (`src/pool/fill.rs:42`) wraps that and does the accounting:
...
The `requests` counter is bumped before the attempt (`src/pool/fill.rs:48`); on a give-up
`source_failures` is bumped and `-5` is returned (`src/pool/fill.rs:50`); otherwise the served count is
added to `bytes_served` and returned (`src/pool/fill.rs:53`). The fill re-clamps to the same 4096 cap the
handler enforces, so nothing downstream can exceed it (`src/pool/fill.rs:43`). A successful request
returns exactly the requested number of hardware-random bytes, and a hardware failure is a clean `-5`
that the handler maps to `EIO` (`src/protocol/errno.rs:17`).

## The stats snapshot

`Pool::snapshot` loads the four counters into a `Stats` struct with relaxed ordering
(`src/pool/snapshot.rs:21`); `requests` is reported under the name `uptime_requests`
(`src/pool/snapshot.rs:23`). `encode_stats` serializes that struct into a fixed 32-byte little-endian
blob (`src/pool/encode_stats.rs:18`):
...
This is exactly the body `GET_STATS` returns after the status word (`src/server/handlers/getstats.rs:24`).

## The reseed breadcrumb

`record_reseed` increments `last_reseed_request` by one and does nothing else
(`src/pool/record_reseed.rs:21`). It takes no bytes and mixes no state, because there is no state to mix
into. The reseed handler still bounds-checks and length-matches the supplied entropy before calling it
(see [operations.md](operations.md)), but the bytes are discarded; only the counter moves. `RESEED` is
...
- There is no boot-time health check of `RDRAND` availability, so a persistent hardware failure surfaces
  as `EIO` at request time rather than at startup (`src/pool/fill.rs:49`).
- The capsule has no software CSPRNG and no secondary source of its own, so its output quality is exactly
  `RDRAND`'s.
- The counters are not persisted across a restart (`src/pool/types.rs:26`).
- `RESEED` here does not reseed anything; it only bumps a counter (`src/pool/record_reseed.rs:21`).
