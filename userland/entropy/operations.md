...
drives them. For the randomness source and the counters, read [pool.md](pool.md). For identity, the
capability mask, and the lifecycle, read the [README](README.md).

## The request loop

The capsule is `no_std`/`no_main`. `_start` initializes the heap and, on success, calls `server::run`
(`src/main.rs:29`). `run` allocates a 4608-byte receive buffer (`MAX_MSG`, `src/server/runner.rs:25`),
builds a fresh `Pool`, and loops (`src/server/runner.rs:27`):
...
A non-positive receive length is skipped (`src/server/runner.rs:32`). A frame that fails to decode is
not dropped: the loop emits a structurally valid `EINVAL` reply so the kernel client gets an answer
rather than a transport timeout (`src/server/runner.rs:41`). Every reply is sent to
`KERNEL_REPLY_ENDPOINT`, the kernel-owned inbox `0x1_0000_0003` (`src/server/runner.rs:43`).
...
`decode_request` (`src/protocol/decode.rs:29`) rejects, in order, a short buffer (`Short`), a wrong
magic (`BadMagic`), a wrong version (`BadVersion`), an over-large `payload_len` (`BadLength`), and a
frame shorter than header plus declared payload (`BadLength`) (`src/protocol/decode.rs:30`). It never
panics and never unwraps (`src/protocol/decode.rs:28`), and on success it returns a `Request` borrowing
the payload slice (`src/protocol/types.rs:57`).

`encode_response` (`src/protocol/encode.rs:25`) reuses the same header and prepends an `i32` status word
to the payload; the status rides in the first four bytes of the payload, little-endian, and a zero
status means success (`src/protocol/encode.rs:26`). `request_id` is echoed so the caller can match
replies, but the capsule never routes on it; IPC handles routing through the per-process inbox
(`src/protocol/types.rs:22`).

## The four operations

Four ops are defined (`src/protocol/types.rs:30`), dispatched by `dispatch` on `req.op`
(`src/server/dispatch.rs:25`); an unknown op returns `EINVAL` (`src/server/dispatch.rs:31`).
...
the request is answered `EIO` (`src/server/handlers/getrandom.rs:38`). The fill itself is the pool's
job; see [pool.md](pool.md).

### GET_STATS (op 2)

`get_stats` (`src/server/handlers/getstats.rs:23`) reads no untrusted input. It snapshots the four
counters and encodes them as a fixed 32-byte little-endian blob (`getstats.rs:24`). Status is always 0.
The blob layout is in [pool.md](pool.md).

### RESEED (op 3)

`reseed` (`src/server/handlers/reseed.rs:26`) bounds-checks the supplied entropy, records the reseed for
...
The ceiling is `MAX_RESEED_BYTES = 256` (`src/protocol/types.rs:37`), checked at `reseed.rs:31`, and the
...
`record_reseed` only increments a counter (`src/pool/record_reseed.rs:21`). So `RESEED` on this capsule
is an observability breadcrumb, not a mixing operation; this is spelled out in [pool.md](pool.md).

### HEALTHCHECK (op 4)

`healthcheck` (`src/server/handlers/healthcheck.rs:24`) takes no input and returns an empty success
...
Three error codes are defined (`src/protocol/errno.rs:17`):
...
request does not leave the caller waiting on a reply that never comes (`src/server/runner.rs:41`).
...
- `handle_crypto_random` (`src/syscall/dispatch/crypto/random.rs:35`) requires `CAP_CRYPTO`, rejects a
...
- `client::reseed` (`src/security/entropy_capsule/client/reseed.rs:25`) gates on `CAP_ADMIN` through
  `gate_reseed` (`src/security/entropy_capsule/capability.rs:35`), rejects an empty seed
...
caller-supplied payload (`src/security/entropy_capsule/capability.rs:24`). A caller that reached the
