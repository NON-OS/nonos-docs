# Client operations and the NNET protocol
...
the [bring-up](bringup.md) and [queues](queues.md) pages; for identity and the capability mask see the
[overview](README.md).
...
+ VIRTIO_NET_HDR_LEN + MAX_ETHERNET_FRAME`, then loops on `mk_ipc_recv_from` (`src/server/runner.rs:36`).
...
version is `1` (`src/protocol/header.rs:30`). The `decode_request` parser reads the fields in
little-endian and rejects a buffer that is short, has the wrong magic, or the wrong version, returning
`None` (`src/protocol/decode.rs:23`); the server turns that `None` into `E_INVAL`.
...
validated and dropped (`src/protocol/header.rs:36`). `encode_response_header` writes the same layout
back, echoing the request's op and flags, zeroing the reserved word, carrying the request_id, and
setting the reply's `payload_len` (`src/protocol/encode.rs:24`). A reply body is a 4-byte little-endian
status word first (`write_status`, `src/protocol/encode.rs:34`), then any payload. Ok is `0`; the error
codes are `E_INVAL` (-22), `E_IO` (-5), `E_AGAIN` (-11), and `E_MSGSIZE` (-90) (`src/protocol/errno.rs:24`).

Every reply is sent with `mk_ipc_reply` to the pid that `mk_ipc_recv_from` reported
(`src/server/error.rs:24`). `reply_with_status` encodes the header, writes the status, and sends a
`RESP_HDR_LEN + STATUS_LEN` reply; `reply_decode_failed` does the same with a zeroed request stub for a
message that never parsed (`src/server/error.rs:27`, `src/server/error.rs:33`).

## The five operations

The five opcodes are defined in one file (`src/protocol/ops.rs:21`).

| Op | Opcode | Request body | Reply payload | Handler |
|---|---|---|---|---|
| `OP_HEALTHCHECK` | 1 | none | status word only | `handlers/health.rs:20` |
| `OP_LINK_STATUS` | 2 | none | status word + 1-byte link flag | `handlers/link_status.rs:24` |
| `OP_MAC_ADDRESS` | 3 | none | status word + 6-byte MAC | `handlers/mac_address.rs:23` |
| `OP_TX_PACKET` | 4 | one Ethernet frame | status word only | `handlers/tx_packet.rs:23` |
| `OP_RX_PACKET` | 5 | none | status word + 4-byte length + frame | `handlers/rx_packet.rs:26` |

An unknown opcode is answered `E_INVAL` (`src/server/runner.rs:63`).

### OP_HEALTHCHECK (1)

Liveness only. The handler writes status `0` and replies; it touches neither the device nor the queues,
so a reply proves the server loop is running (`src/server/handlers/health.rs:20`).
...
(`src/server/handlers/link_status.rs:25`, `src/constants/regs.rs:37`). The reply is the status word
...
else `E_MSGSIZE` (`src/server/handlers/tx_packet.rs:24`). On a valid request the handler calls `tx::send`,
...
transmit path is on the [queues](queues.md) page.
...
the frame bytes (`src/server/handlers/rx_packet.rs:27`). An empty ring is not an error: it replies
...
reaches the handler (`src/rx.rs:52`). net_core drives this op in its poll loop, treating a non-zero
status as no-frame (`userland/capsule_net_core/src/device/rx.rs:45`).
...
(`src/server/handlers/link_status.rs:35`), mac sends `RESP_HDR_LEN + STATUS_LEN + MAC_ADDRESS_PAYLOAD_LEN`
(`src/server/handlers/mac_address.rs:29`), and rx sends `RESP_HDR_LEN + STATUS_LEN + RX_PAYLOAD_PREFIX_LEN
+ frame_len` (`src/server/handlers/rx_packet.rs:47`). The status-only replies (health, tx, and every
error) go through `reply_with_status`, which always sends `RESP_HDR_LEN + STATUS_LEN`
(`src/server/error.rs:30`). The layout constants live in one file, keyed off the frame constants
(`src/protocol/limits.rs:26`).
