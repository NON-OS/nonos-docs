# Device bring-up and broker grants
...
[queues](queues.md) page; for the client protocol see [operations](operations.md). The broker's own view
of each grant is in [claim](../../subsystems/hardware-broker/claim.md),
...
the first that matches (`src/discover/find_virtio_net.rs:24`). A match is virtio vendor `0x1AF4` on a PCI
bus with device id `0x1000` (transitional) or `0x1041` (modern) (`src/discover/is_match.rs:21`,
`src/constants/pci.rs:22`). Discovery deliberately does not filter on the interrupt pin or line: q35
...
leave setup looping forever (`src/discover/find_virtio_net.rs:32`). This is the one place where the
...
(`src/discover/first_register_bar.rs:19`, `src/discover/found.rs:17`).

## The bring-up transaction

`setup::run` is the ordered transaction (`src/setup/sequence.rs:27`). Each step emits a `[net-setup]`
stage marker over `mk_debug` before it runs, so a stuck bring-up names the step it died on
(`src/setup/stage.rs:19`). Each DMA step rolls back all prior grants on failure through the shared
...
(`src/setup/claim.rs:22`). The epoch is the broker's anti-stale linchpin: every later grant quotes it,
and a grant quoting an old epoch after a release is rejected
([claim](../../subsystems/hardware-broker/claim.md)). The claim is exclusive, so once it succeeds no
other capsule can be mapping this device's BARs or taking its interrupts underneath the driver.

### Registers

`registers::map` maps the register BAR according to its kind (`src/setup/registers/map.rs:24`). An MMIO
BAR goes through `mk_mmio_map` with the length page-rounded up (`src/setup/registers/grant_mmio.rs:23`);
a port BAR goes through `mk_pio_grant` (`src/setup/registers/grant_pio.rs:22`). Either way the result is
wrapped in a `RegisterGrant` enum, and `RegisterGrant::regs` turns it into a `Regs` handle so the rest of
the driver reads registers the same way regardless of BAR kind (`src/setup/registers/regs.rs:21`). A map
failure, or an unsupported BAR kind, releases the device claim before returning
(`src/setup/registers/grant_mmio.rs:29`, `src/setup/registers/map.rs:29`). Register access is uncached
device memory, never RAM ([mmio](../../subsystems/hardware-broker/mmio.md)).
...
(`src/setup/irq.rs:28`). The boolean it returns records whether MSI-X won, because MSI-X shifts the
device config layout by 4 bytes and the caller uses that to place the MAC and status offsets
(`src/setup/irq.rs:32`, `src/setup/sequence.rs:42`). A total failure releases the register grant and then
the device claim before returning (`src/setup/irq.rs:36`). The grant delivers the device interrupt on a
kernel-owned vector; the capsule waits and acknowledges through syscalls and never touches the interrupt
controller ([irq](../../subsystems/hardware-broker/irq.md)).

### DMA

`dma_set::map` allocates four device-visible regions in order, and each rolls back all prior grants on
failure (`src/setup/dma_set.rs:29`):

- the RX virtqueue ring, `VQ_REGION_SIZE` = 12288 bytes (`src/setup/dma/rx_queue.rs:23`,
  `src/constants/queue.rs:37`);
- the RX buffers, `RX_BUFFER_LEN * RX_DESC_COUNT` = 2048 * 64 = 131072 bytes
  (`src/setup/dma/rx_buffer.rs:31`, `src/constants/queue.rs:48`);
- the TX virtqueue ring, another `VQ_REGION_SIZE` = 12288 bytes (`src/setup/dma/tx_queue.rs:23`);
- the TX buffer, `TX_BUFFER_LEN * TX_DESC_COUNT` page-rounded, 2048 * 8 = 16384 bytes
  (`src/setup/dma/tx_buffer.rs:33`, `src/constants/queue.rs:49`).
...
([dma](../../subsystems/hardware-broker/dma.md)). The rollback is one shared helper: `rollback::after`
...
(`src/setup/dma/rollback.rs:21`, `src/setup/dma/tx_buffer.rs:38`). The ordering is the reverse of
acquisition, so teardown mirrors bring-up.

## The virtio handshake

`negotiate` walks the legacy virtio status handshake over the register BAR (`src/init/negotiate.rs:24`).
...
(bit 16), and each only if the host advertised it (`src/init/negotiate.rs:30`, `src/constants/regs.rs:33`);
...
config offsets (`src/setup/sequence.rs:41`, `src/setup/config.rs:20`). If the device clears `FEATURES_OK`
after the driver set it, the driver sets the `FAILED` bit and returns `virtio-net: features-ok rejected`
(`src/init/negotiate.rs:37`).
...
bring-up (`src/init/driver_ok.rs:20`, `src/setup/sequence.rs:47`).

## Programming the queues

`queues::build` zeroes both ring regions, programs each queue into the device, and constructs the
`RxQueue`/`TxQueue` state (`src/setup/queues.rs:23`). `program_queue` selects the queue with
`LEG_QUEUE_SEL` (0x0E), reads the device's max ring size from `LEG_QUEUE_NUM` (0x0C), validates it, and
writes the ring physical page frame into `LEG_QUEUE_PFN` (0x08) (`src/init/program_queue.rs:20`). The
...
`virtio-net: invalid queue` on any of them (`src/init/program_queue.rs:29`). The negotiated size is the
minimum of the device max and the driver's hint, `RX_QUEUE_SIZE` = 64 for RX and `TX_QUEUE_SIZE` = 8 for
TX (`src/init/program_queue.rs:33`, `src/constants/queue.rs:30`).

## Read the MAC and arm

`config::read_mac` reads six bytes from the config MAC register, one byte at a time, but only if
`VIRTIO_NET_F_MAC` was negotiated; otherwise it returns zeroes (`src/setup/config.rs:24`). The MAC
register base is `LEG_MAC` (0x14), shifted up by 4 when MSI-X is active, and the link-status word sits at
base plus 6 (`src/setup/sequence.rs:42`, `src/setup/sequence.rs:70`). After the queues are built,
...
interrupt; a failed ack returns `virtio-net: irq ack failed` (`src/setup/sequence.rs:46`). It then hands
back a `Driver` holding the register grant, the IRQ and four DMA grant ids for teardown, the two queues,
the `Regs` handle, the MAC, and the status support flag with its offset (`src/setup/driver.rs:24`). From
this point the [queues](queues.md) engine and the [operations](operations.md) server take over.
...
base pointer or a PIO grant id (`src/regs/io.rs:18`, `src/regs/regs_type.rs:20`). The sized accessors
...
in bytes (`src/regs/r32.rs:24`, `src/regs/pio_read.rs:19`). This is why the mask carries both `Mmio` and
