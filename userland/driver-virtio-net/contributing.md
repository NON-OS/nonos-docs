# Contributing to capsule_driver_virtio_net
...
together, read the [README](README.md), the [operations](operations.md), the [bring-up](bringup.md), and
the [queues](queues.md) pages in this folder.
...
regions did not map, and then enters `server::run`, which never returns (`src/main.rs:41`). The top-level
modules are declared there (`src/main.rs:22`).
...
| `src/rx.rs`, `src/tx.rs` | pulling a received frame and sending one | you change the frame path |
| `src/constants/` | frame, pci, queue, regs, and status constants | you change a device or protocol constant |

Every handler under `src/server/handlers/` is one op per file (`src/server/handlers/mod.rs:17`), and each
...
1. Add the opcode constant in `src/protocol/ops.rs:21` and re-export it from `src/protocol/mod.rs:32`.
   The five existing opcodes are `1..=5`.

2. Write the handler as its own file under `src/server/handlers/`, one op per file. Parse and
   bounds-check the body before any DMA the way `src/server/handlers/tx_packet.rs:24` does, replying a
   ...
   `src/server/handlers/mac_address.rs:23` does. A reply that returns device data must bound the slice to
   the buffer the way `src/rx.rs:52` and `src/queue/used.rs:46` do, never a raw device length.

3. Wire the opcode into the dispatch match in `src/server/runner.rs:57`, and add the handler module to
   `src/server/handlers/mod.rs:17`. A word no arm matches already falls to the `_ =>` arm and replies
   `E_INVAL` (`src/server/runner.rs:63`).

If the operation needs a new virtqueue direction or descriptor shape, extend the ring writers in
`src/queue/post_packet.rs` (TX) or `src/queue/post.rs` (RX) and keep the fixed page-aligned ring offsets
in `src/constants/queue.rs:34`; the whole engine assumes those offsets.
...
`src/hardware/virtio_net_capsule/protocol/header.rs:22` point back at the userland files, and a mismatch
surfaces at runtime as `DriverNetError::ProtocolMismatch` (`src/hardware/virtio_net_capsule/error.rs:33`,
`src/hardware/virtio_net_capsule/protocol/codec.rs:34`). A new opcode is two edits, one per side
(`src/hardware/virtio_net_capsule/protocol/ops.rs:21`). The in-kernel client is reachable only by a
caller holding `CAP_DRIVER` (`src/hardware/virtio_net_capsule/capability.rs:30`).

One stale-source note for a contributor: the kernel transport comment points at a userland file
`src/protocol/endpoint.rs` for the reply-inbox value (`src/hardware/virtio_net_capsule/client/transport.rs:26`),
but this capsule has no such file. The userland driver replies to the sender pid returned by
`mk_ipc_recv_from`, not to a baked endpoint constant (`src/server/runner.rs:44`, `src/server/error.rs:24`).
...
- No panics in capsule code. No `unwrap`, `expect`, or `panic!`. Every handler reports an error as a
  ...
  bring-up loop retries rather than aborting (`src/main.rs:41`).
- One unit per file. Handlers are one op per file, the DMA maps are one region per file, the register
  ...
  (`src/protocol/mod.rs:17`, `src/server/handlers/mod.rs:17`, `src/regs/mod.rs:17`).
