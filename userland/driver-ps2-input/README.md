...
[../../subsystems/hardware-broker/pio.md](../../subsystems/hardware-broker/pio.md)).
...
|------|---------|----------------|
| [protocol.md](protocol.md) | `src/protocol/`, `src/ring/`, `src/mouse/ring.rs` | The `NKBD` wire header, the five ops, the reply records, the reply endpoint, and the two bounded rings with their opposite drop disciplines. |
| [bring-up.md](bring-up.md) | `src/setup/`, `src/init/`, `src/discover.rs`, `src/constants/` | Discovery over ACPI records, the PIO and IRQ grants and their rollback, the i8042 config-byte sequence and its flush fix, and the interrupt pump with its double drain. |
| [decode.md](decode.md) | `src/poll/`, `src/keymap/`, `src/mouse/` | The per-byte drain, the Set 1 scancode tables and modifier tracking, the 3-byte mouse packet assembler, and the two input-post paths into the kernel ring. |
| [contributing.md](contributing.md) | the whole tree | Where to work, how to extend the keymap or add an op, the build and sign steps, and the code standards the CI enforces. |
| [debugging.md](debugging.md) | runtime | The boot marker, the diagnostic counters, and where to look for a dead keyboard, a missing mouse, stuck keys, or a desyncing pointer. |
...
`Driver` to the server loop (`src/main.rs:31`). `setup::run` discovers and claims the i8042 platform
records, mints the PIO grant, binds IRQ1 and IRQ12, brings the controller up, and returns the grant ids
and a mouse-enabled flag (`src/setup/sequence.rs:26`).

Once running, the server does two things in one loop (`src/server/runner.rs:49`). It drains the
...
mapped (`src/hardware/ps2_kbd_capsule/spawn.rs:39`). On a successful bring-up it prints
`[driver_ps2] endpoint driver.ps2_kbd0 ready` through `mk_debug` (`src/setup/sequence.rs:43`); the
[debugging](debugging.md) page covers what that marker and its absence mean.
