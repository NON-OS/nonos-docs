# Debugging capsule_driver_ps2_input

This page lists the markers and counters the driver and its input path emit, and the concrete failure
modes with where to look for each. For how the driver is put together read the [README](README.md), the
[protocol and rings](protocol.md), the [bring-up](bring-up.md), and the [decode](decode.md) pages in this
...
returning (`src/setup/sequence.rs:43`, `src/setup/marker.rs:17`). If that line is absent the device was
...
(`src/setup/sequence.rs:27`), and `_start` retries `setup::run` forever with a yield between attempts, so
...
fault (`src/server/handlers/state.rs:27`, and the [protocol](protocol.md) page for the layout). Index 0
...
i8042 snapshot that does not consume a data byte (`src/server/handlers/controller_status.rs:28`).
...
disable bit got set during the enable sequence (`src/init/enable_keyboard/enable.rs:28`). If index 0
...
[../../subsystems/input/path.md](../../subsystems/input/path.md), not this capsule.
...
(`src/init/enable_mouse/mouse_command.rs:24`, `src/setup/sequence.rs:40`). A non-zero `mouse_enabled`
...
`aux_irq_grant_id = 0` and the keyboard fully live (`src/setup/setup_aux.rs:21`).
...
(`src/ring/push.rs:22`). Parity and timeout counters (indices 2 and 3) rising alongside indicate
line-level corruption rather than a decode bug (`src/poll/drain.rs:39`). The kernel input post itself
...
dropped bytes on the AUX line, not at the parser (`src/mouse/parser.rs:30`). Index 5 (mouse events
dropped) rising means the mouse ring filled and dropped the newest samples, which is consistent with a
consumer that stopped calling `OP_POLL_MOUSE` while motion continued (`src/mouse/ring.rs:38`). A pointer
that moves but never clicks through to a window is the hit-test case in the input path, not a decode bug:
buttons are posted only on transition (`src/mouse/post.rs:35`).

### No IPC reply at all

Every reply goes to the fixed kernel reply endpoint, not back through the recv socket
(`src/protocol/endpoint.rs:16`). A caller that never sees a reply but the driver is otherwise alive is
usually reading the wrong endpoint; the kernel-side transport binds the matching inbox
(`src/hardware/ps2_kbd_capsule/client/transport.rs:30`). A malformed request (bad magic, wrong version,
short buffer, or a non-zero payload length) is answered `E_INVAL` rather than dropped
(`src/server/runner.rs:59`, `:64`).
