# Bring-up, PIO, and IRQ

This page mirrors `src/setup/`, `src/init/`, `src/discover.rs`, and `src/constants/`. It covers how the
...
[../../subsystems/hardware-broker/pio.md](../../subsystems/hardware-broker/pio.md) and
[../../subsystems/hardware-broker/irq.md](../../subsystems/hardware-broker/irq.md).
...
`src/constants/ports.rs:16`), and every read and write is a `mk_pio_read` or `mk_pio_write` against the
grant (`src/init/enable_keyboard/pio_write.rs:18`, `src/poll/read_port.rs:17`). A CI static check rejects
any inline `in`/`out` in this capsule, so every byte crosses the kernel's grant-bounds and stale-epoch
checks (`nonos-ci/run-static-checks.sh:1178`). Two more CI checks forbid this capsule from importing
`crate::drivers` or any kernel memory, paging, or hardware path
(`nonos-ci/run-static-checks.sh:1158`, `:1167`), so its only reach into the machine is the port window.

## Discovery over ACPI records

`find_ps2_kbd` and `find_ps2_aux` list the ACPI platform records with `mk_device_list` (up to 32) and
scan for a matching PnP vendor and device id (`src/discover.rs:26`). The keyboard record is vendor
`0x0001`, device `0x0303`; the AUX record is vendor `0x0001`, device `0x0304`
(`src/constants/pnp.rs:16`). The keyboard match additionally requires a BAR, because the keyboard record
owns the shared port window; the AUX record needs none (`src/discover.rs:32`, `:43`). A match returns the
`device_id` and the record's `irq_line` (`src/discover.rs:46`). The kernel-side broker synthesises these
two platform records with `PS2_KBD_IRQ = 1` and `PS2_AUX_IRQ = 12`
(`src/hardware/broker/platform.rs:28`), and a CI check pins both the device id and `PS2_AUX_IRQ = 12`
(`nonos-ci/run-static-checks.sh:1220`). A missing keyboard record fails startup outright with
`ps2 keyboard not present in device list` (`src/setup/sequence.rs:27`).

## The ordered bring-up

`setup::run` runs a fixed sequence and returns a `Driver { pio_grant_id, irq_grant_id, aux_irq_grant_id,
mouse_enabled }` (`src/setup/sequence.rs:26`, `src/setup/driver.rs:16`).

1. Find and claim the keyboard record. `claim` calls `mk_device_claim` and returns the claim epoch, or
   fails startup (`src/setup/claim.rs:17`).
2. Grant the i8042 PIO window over BAR index 0 with `mk_pio_grant`. On failure it releases the device
   claim with `mk_device_release` and fails startup (`src/setup/pio.rs:18`).
3. Bind IRQ1. `irq::bind` calls `mk_irq_bind` against the keyboard record's `irq_line`; on failure it
   releases the PIO grant with `mk_pio_release` and returns the error (`src/setup/irq.rs:19`). The
...
4. Discover, claim, and bind the AUX record on IRQ12 through `setup_aux`. This path is all soft failure:
   ...
   (`src/setup/setup_aux.rs:21`).
5. Flush stale controller output. `flush_output` drains up to 16 bytes from the data port while the
   output-full bit is set (`src/init/flush_output.rs:20`).
6. Enable the keyboard. This is the careful config-byte step, below (`src/init/enable_keyboard/enable.rs:26`).
...
8. Acknowledge both IRQ lines once through `open_line` so the first real edge is delivered, then emit the
   ready marker (`src/setup/open_line.rs:17`, `src/setup/sequence.rs:41`).

The rollback discipline is enforced by a CI check that requires `setup/pio.rs` to release the device and
`setup/irq.rs` to release the PIO grant on failure (`nonos-ci/run-static-checks.sh:1194`). Because every
...
(`src/init/enable_keyboard/enable.rs:26`, `src/constants/ports.rs:18`). A config byte read back as
...
(`src/init/enable_keyboard/enable.rs:28`): enabling the keyboard can leave an ACK or a stray scancode in
...
buffer flushed the next full byte is genuinely the config response (`src/init/enable_keyboard/read_byte.rs:21`).
...
a wedged controller returns an error rather than hanging (`src/init/enable_keyboard/cmd.rs:20`,
`src/init/enable_keyboard/wait_clear.rs:21`, `read_byte.rs:22`). The keyboard reset writes `0xFF` and
consumes the two response bytes it may produce (`src/init/enable_keyboard/reset.rs:20`).
...
(`src/init/enable_mouse/enable.rs:26`, `src/constants/ports.rs:20`). Each mouse command is routed to the
AUX device by prefixing `CTL_WRITE_AUX` (`0xD4`) and then reading the reply, and the whole sequence fails
unless the reply is `MOUSE_ACK` (`0xFA`) (`src/init/enable_mouse/mouse_command.rs:21`). Because the flag
...
(`src/setup/sequence.rs:40`). A CI check pins the `CTL_WRITE_AUX` and `MOUSE_ENABLE_REPORTING` path and
the IRQ12 ownership (`nonos-ci/run-static-checks.sh:1229`).
...
sequence with `mk_irq_poll` (`src/server/irq_seq.rs:18`), drains the ports once, and if either sequence
advanced past the last one seen it acknowledges both lines and drains a second time
(`src/server/pump.rs:24`). The second sweep is deliberate, and its comment states why
(`src/server/pump.rs:42`): a byte that lands between the pre-ack drain and the IO-APIC unmask raises its
...
previous button mask forward so only button transitions are posted (`src/server/pump.rs:36`).

When no IPC request is waiting, the runner blocks on `mk_irq_wait` with a 100 ms timeout rather than
busy-spinning, so it wakes on the next interrupt (`src/server/runner.rs:51`). A grant id of 0, which is
what the keyboard carries if its IRQ never bound, makes `poll_seq` return 0 without a syscall, so the
polling-only path degrades cleanly (`src/server/irq_seq.rs:19`).
...
InputSource and nothing else (`Capsule.mk:17`, `src/hardware/ps2_kbd_capsule/spawn.rs:51`); the bit
decomposition is on the [README](README.md).
...
([../../subsystems/hardware-broker/pio.md](../../subsystems/hardware-broker/pio.md)). This is the only
...
rings is the ready marker, through `mk_debug` (`src/setup/marker.rs:17`).
...
set of grants (`src/setup/pio.rs:21`, `src/setup/irq.rs:22`, `src/setup/sequence.rs:27`). On exit the
...
(`src/poll/drain.rs:39`), a bad mouse sync byte is counted and dropped (`src/mouse/parser.rs:30`), and a
