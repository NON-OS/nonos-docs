# Contributing to capsule_driver_ps2_input
...
driver does and how it is put together, read the [README](README.md), the
...
hands the `Driver` to the server loop (`src/main.rs:31`). The top-level modules are declared there
(`src/main.rs:19`).
...
| `src/discover.rs` | matching the PS/2 records on the ACPI bus | you change device matching |
...
([decode.md](decode.md)). Printable and named base keys live in `src/keymap/set1/{left,right,function}.rs`
split by scancode range, the extended block in `src/keymap/set1_e0.rs`, and the keycode constants in
`src/keymap/set1/keycodes.rs`. Add a mapping to the right range's match and, if the value is a named key,
add its constant to `keycodes.rs` first. If you add a new modifier key, wire its bit into `modifier_bit`
so the modifier mask tracks it (`src/keymap/post.rs:31`); the `MOD_*` values must stay aligned with the
app-side contract and the USB HID driver (`src/keymap/post.rs:23`).

## Changing mouse decoding

Edit `src/mouse/packet.rs` for the byte layout and sign extension, and `src/mouse/post.rs` for how a
...
every event reaches the kernel twice (`src/mouse/parser.rs:26`, `src/server/pump.rs:36`).

## Adding or changing an op

There are three edits. Add the op constant to `src/protocol/ops.rs` and re-export it through
`src/protocol/mod.rs:32`. Add a handler file under `src/server/handlers/` and declare it in
`src/server/handlers/mod.rs`; a handler builds its reply over `tx` and sends it to the kernel reply
endpoint, the way the existing handlers do. Add the match arm in `run` so the op is dispatched
(`src/server/runner.rs:68`). If the reply carries a payload, add its size constant to
`src/protocol/limits.rs` and confirm the runner's transmit buffer is sized for it: the buffer is the max
of the four payload-bearing replies (`src/server/runner.rs:40`).
...
Several standards are enforced by `nonos-ci/run-static-checks.sh`, so a change that breaks them fails CI:
...
- `setup/pio.rs` rolls back via `mk_device_release` and `setup/irq.rs` via `mk_pio_release`
