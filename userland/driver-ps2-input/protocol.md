# The Protocol and the Rings

This page mirrors `src/protocol/`, `src/ring/`, and `src/mouse/ring.rs`. It covers the `NKBD` wire
...
drop disciplines. For where the bytes on these rings come from see the [decode](decode.md) page; for how
the controller is brought up see [bring-up](bring-up.md). This driver is a pure server: it never opens a
client connection to another service.

## The wire header

Every request and every reply begins with a 20-byte header (`src/protocol/header.rs:16`):
...
`None` (`src/protocol/decode.rs:18`), and the runner answers that failure `E_INVAL` (-22)
(`src/server/runner.rs:59`, `src/protocol/errno.rs:16`). `encode_response_header` echoes the request's
`op`, `flags`, and `request_id` back, writes zero into the reserved field, and sets the reply's own
`payload_len` (`src/protocol/encode.rs:17`). A reply's payload always opens with a little-endian signed
32-bit status word written by `write_status`; status `0` means the operation completed
(`src/protocol/encode.rs:26`, `src/protocol/limits.rs:16`).

Requests carry no payload. The runner rejects any request whose `payload_len` is non-zero with `E_INVAL`
before it ever dispatches (`src/server/runner.rs:64`), so the five ops are distinguished only by the `op`
field.

## The five ops

The op constants are defined once in `src/protocol/ops.rs:16` and dispatched by the match in `run`
(`src/server/runner.rs:68`). An unknown op is answered `E_INVAL` by the wildcard arm
(`src/server/runner.rs:74`).
...
(`src/server/handlers/health.rs:18`). A live driver replies status 0; a dead one never replies at all.
...
(`src/server/handlers/poll.rs:24`, `src/protocol/limits.rs:17`). Each event is packed as three bytes:
the raw scancode, the event flags, and a reserved zero (`src/server/handlers/poll.rs:42`,
...
(`src/server/handlers/state.rs:27`). No drain runs, so the counters are a snapshot.
...
these counters directly (`src/ring/state.rs:22`, `src/mouse/ring.rs:23`).

### Controller status

`OP_CONTROLLER_STATUS` reads only the status port, so it cannot swallow a pending keyboard or mouse byte
the way a data-port read would (`src/server/handlers/controller_status.rs:28`). If that single read
...
The status-bit constants are in `src/constants/status.rs:16`, and the ring depth, head, and tail come
from the ring accessors (`src/ring/state.rs:39`).

### Poll mouse

`OP_POLL_MOUSE` mirrors `OP_POLL_EVENTS`: it drains, acknowledges both IRQ lines, and pops up to 256
events off the mouse ring (`src/server/handlers/mouse.rs:24`). Each event is packed as eight bytes
...
(`src/protocol/endpoint.rs:16`, used in `src/server/error.rs:23` and every handler, for example
`src/server/handlers/poll.rs:51`). That value is slot 10 in the per-service reply table, and the
kernel-side transport binds the matching inbox string `endpoint.4294967306`
(`src/hardware/ps2_kbd_capsule/client/transport.rs:30`); the userland endpoint comment ties the two
together (`src/protocol/endpoint.rs`, `transport.rs:29`).

The runner sizes one transmit buffer large enough for the biggest of the four payload-bearing replies and
reuses it across ops (`src/server/runner.rs:35`). The poll reply is the largest at
...
(`src/ring/state.rs:18`, `RING_CAPACITY = 256` at `src/constants/ports.rs:32`). `push` advances head, and
when the ring is full it also advances tail, dropping the *oldest* event and incrementing
`events_dropped` (`src/ring/push.rs:20`). `pop` returns the event at tail or `None` when head equals tail
(`src/ring/pop.rs:20`). The rationale is that a keystroke that arrived long ago and was never polled is
...
(`src/mouse/ring.rs:19`, `MOUSE_RING_CAPACITY = 128`). Its `push` does the opposite: when the ring is
full it drops the *incoming* event and increments `events_dropped`, leaving the queued samples untouched
(`src/mouse/ring.rs:38`). For a mouse the accumulated motion already queued is what matters; discarding
the newest delta loses the least position information. Both rings expose `queued`, and both count every
arrival in `events_seen` whether or not it was kept (`src/ring/state.rs:39`, `src/mouse/ring.rs:39`).
