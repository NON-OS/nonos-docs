...
`mk_input_event_post`. The bounded rings these decoders feed are on the [protocol](protocol.md) page; how
the drain is triggered is on the [bring-up](bring-up.md) page; the system-wide input path is in
[../../subsystems/input/path.md](../../subsystems/input/path.md).

## The per-byte drain

`drain` reads up to 16 bytes per call (`MAX_BYTES_PER_DRAIN`, `src/poll/drain.rs:23`). For each byte it
reads the status port, stops if the output-full bit is clear, counts the parity and timeout bits into the
keyboard ring's error counters if set, then reads the data byte (`src/poll/drain.rs:31`). It routes the
byte by the AUX-data status bit: if `STATUS_AUX_DATA` (`0x20`) is set the byte goes to the mouse parser,
otherwise to the keyboard absorber (`src/poll/drain.rs:49`). Every port access is `read_port`, which is a
one-byte `mk_pio_read` against the grant (`src/poll/read_port.rs:17`). The 16-byte cap bounds how long a
...
(`src/poll/absorb.rs:23`, `src/ring/flags.rs:16`). This raw scancode plus flags is exactly the 3-byte
...
(`src/poll/absorb.rs:44`). `translate` reads the break bit off the flags, masks the scancode to 7 bits,
...
(`src/keymap/set1/left.rs:20`, `src/keymap/set1/right.rs:47`). Named keys map to keycode constants above
the ASCII range: modifiers such as `KEYCODE_LSHIFT = 0x1003`, function keys such as `KEYCODE_F1 =
0x1101`, and navigation keys such as `KEYCODE_UP = 0x1201` (`src/keymap/set1/keycodes.rs:16`). The E0
table maps the extended block: right control and alt, the arrow cluster, Home/End/PageUp/PageDown,
Insert, Delete, and the two meta keys (`src/keymap/set1_e0.rs:21`). The layout tables are internal to
...
on press and cleared on release (`src/poll/absorb.rs:45`, `src/keymap/post.rs:31`). These `MOD_*` values
are chosen to match the app-side contract and the USB HID driver's encoding, as the comment states
(`src/keymap/post.rs:23`). Every translated key is then handed to `publish`, which builds an `InputEvent`
with kind `INPUT_KIND_KEY_DOWN` or `INPUT_KIND_KEY_UP`, the keycode in `code`, and the current modifier
mask in `flags`, and posts it with `mk_input_event_post` (`src/keymap/post.rs:41`). The `x`, `y`, and
...
`MouseParser::absorb` collects bytes into a 3-slot buffer (`PACKET_LEN = 3`, `src/mouse/parser.rs:16`).
...
(`src/mouse/parser.rs:30`). When the third byte lands it parses the packet and either pushes the result
on the mouse ring or counts a second sync error if the parse rejects it (`src/mouse/parser.rs:36`). The
...
always zero for the base 3-byte protocol (`src/mouse/packet.rs:23`, `src/mouse/event.rs:16`). `parse`
re-checks the sync bit and returns `None` if it is clear, which is the second sync-error path
(`src/mouse/packet.rs:25`).

### The input post

The pump drains the mouse ring and calls `publish_mouse` on each event, carrying the previous button mask
forward (`src/server/pump.rs:36`, `src/mouse/post.rs:24`). `publish` posts up to three separate
`InputEvent`s per mouse event:

- Relative motion as `INPUT_KIND_POINTER_REL` with `delta_x`/`delta_y`, only when `dx` or `dy` is
  non-zero (`src/mouse/post.rs:26`).
- A non-zero wheel as `INPUT_KIND_WHEEL` with the wheel in `delta_y` (`src/mouse/post.rs:29`).
- Each changed button as `INPUT_KIND_BUTTON_DOWN` or `INPUT_KIND_BUTTON_UP`. `publish_buttons` XORs the
  ...
  1` so left is 1, right is 2, middle is 3 (`src/mouse/post.rs:35`). Posting only transitions is why the
...
lost and the boolean is folded into an aggregate `ok` that the caller discards (`src/mouse/post.rs:49`).
