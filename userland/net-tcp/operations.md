...
the [connections](connections.md) page; for how a segment leaves for `net.ip`, read the [ip-link](ip-link.md)
page.

## The NTCP wire format

Every request and reply begins with a fixed twenty-byte header, defined in one place as `HDR_LEN`
(`src/server/parse_req.rs:19`, `src/server/respond.rs:21`). The magic is `NTCP`, stored as the little-endian
`u32` `0x4E544350` (`src/protocol/header.rs:17`).
...
returns the body as a slice bounded to exactly `payload_len` (`src/server/parse_req.rs:27`). A reply reuses
...
with `mk_ipc_reply` (`src/server/respond.rs:23`). The IPC payload is bounded at `IPC_PAYLOAD_MAX`, which is
one maximum segment plus sixty-four bytes of slack (`src/protocol/limits.rs:18`); the segment payload itself
is capped at `SEGMENT_PAYLOAD_MAX = 1460` (`src/protocol/limits.rs:17`).

## The request loop

`server::run` allocates one receive and one transmit buffer of `HDR_LEN + IPC_PAYLOAD_MAX` and loops
forever (`src/server/runner.rs:33`). Each turn does three things in order:

1. Run the timer tick: reap due TimeWait timers, scan the retransmit queue, and drain every pending inbound
   segment from `net.ip` (`src/server/runner.rs:37`, `src/server/tick.rs:24`). This is what makes progress
   between client calls, so a connection advances even when no op arrives.
2. Compute a receive budget and block on `mk_ipc_recv_from` for that long (`src/server/runner.rs:38`). The
   budget is zero when the table is idle, otherwise the time to the next timer deadline clamped to `[10,
   250]` milliseconds, so an idle capsule parks and a busy one wakes on its own timers (`src/server/tick.rs:44`).
3. Parse the request and dispatch on the opcode to a handler (`src/server/runner.rs:45`). A parse failure or
   an empty receive is skipped silently; an unknown op replies `E_BAD_OP` (`src/server/runner.rs:54`).

The dispatch maps `OP_CLOSE` and `OP_SHUTDOWN` to the same close handler; the other seven ops each have
their own handler module (`src/server/runner.rs:46`).

## The operations

The opcodes are `u16` constants (`src/protocol/ops.rs:17`). Handles are the per-connection identifiers the
table mints; see the [state](state.md) page.

| Op | Value | Request body | Reply body | Handler |
|---|---|---|---|---|
| `OP_HEALTHCHECK` | 1 | none | none | `src/server/handlers/health.rs:21` |
| `OP_LISTEN` | 2 | `u16` port | `u32` listener handle | `src/server/handlers/listen.rs:24` |
| `OP_CONNECT` | 3 | `[u8;4]` ip, `u16` port | `u32` connection handle | `src/server/handlers/connect/reply.rs:24` |
| `OP_ACCEPT` | 4 | `u32` listener handle | `u32` child handle | `src/server/handlers/accept.rs:28` |
| `OP_SEND` | 5 | `u32` handle, bytes | none | `src/server/handlers/send.rs:24` |
| `OP_RECV` | 6 | `u32` handle | stream bytes | `src/server/handlers/recv.rs:28` |
| `OP_CLOSE` | 7 | `u32` handle | none | `src/server/handlers/close.rs:27` |
| `OP_SHUTDOWN` | 8 | `u32` handle | none | `src/server/handlers/close.rs:27` |
| `OP_STATE` | 9 | `u32` handle | `u8` state code | `src/server/handlers/state.rs:23` |
...
`State::Listen` control block owned by the caller, replying the new handle (`src/server/handlers/listen.rs:24`).
...
(`src/server/handlers/accept.rs:28`). Children are enqueued when the handshake completes on a passively
opened connection, worked through on the [connections](connections.md) page.

### Connect

`OP_CONNECT` is a multi-file handler under `src/server/handlers/connect/`. `body::parse` reads the four-byte
destination IP and `u16` port, requiring at least six body bytes (`src/server/handlers/connect/body.rs:20`).
`open::connection` allocates an ephemeral local port, derives the ISS, builds a `SynSent` control block, and
inserts it (`src/server/handlers/connect/open.rs:20`). `reply::handle` sends the SYN, then `wait::established`
blocks until the connection reaches `Established` or an eight-second deadline expires, replying the handle on
success and `E_TIMEOUT` on failure after removing the dead entry (`src/server/handlers/connect/reply.rs:24`,
`src/server/handlers/connect/wait.rs:27`).

### Send and receive

`OP_SEND` reads the `u32` handle and treats the rest of the body as stream bytes, rejecting a body under
four bytes or a payload over `SEGMENT_PAYLOAD_MAX` with `E_BAD_LEN` (`src/server/handlers/send.rs:24`). It
...
(`src/server/handlers/recv.rs:28`).
...
replying `E_OK`; from any other state it just removes the entry (`src/server/handlers/close.rs:27`). An
unknown handle is `E_NO_SOCKET`. `OP_STATE` replies a single byte, the numeric `State` discriminant, and is
the probe a client uses to watch a connection walk the machine (`src/server/handlers/state.rs:23`); the code
values are listed on the [connections](connections.md) page.
...
port, timeout, closed, and empty errnos are what a client sees on the wire (`src/server/parse_req.rs:27`,
`src/server/runner.rs:44`).
