...
runtime failure modes with where to look for each. For the shape of the capsule read the [README](README.md),
the [operations](operations.md), [connections](connections.md), [segments](segments.md), [state](state.md),
and [ip-link](ip-link.md) pages.
...
(`src/userspace/init/spawn_plan/network/spawn_tcp.rs:21`), whose `Ok` arm calls `boot_log::ok(prefix,
"capsule spawned")` (`src/userspace/init/capsule_boot/run.rs:29`). An absent line means the capsule never
started, usually a signature, manifest, or capability failure; the `Err` arm prints an `[ERROR]` line
instead (`src/userspace/init/capsule_boot/run.rs:32`). The spawn is feature-gated on `nonos-capsule-net-tcp`,
so an image built without that feature will not print the line at all
(`src/userspace/init/spawn_plan/network/spawn_tcp.rs:18`).
...
init failure, before setup even runs (`src/main.rs:35`). After that, `wait_for_setup` calls `setup::run` in a
loop, yielding sixty-four times between attempts, and only enters the server once setup returns `Ok`
(`src/main.rs:41`). So a capsule that spawned but is not answering has almost always not finished setup. The
three ways setup fails, each returned and retried rather than logged, are:

| Setup error | Cause | Where |
|---|---|---|
| `EntropyMissing` | `crypto_random` returned fewer than sixteen bytes for the ISS key | `src/setup.rs:33` |
| `IpMissing` | `net.ip` is not registered, or its lookup reported port zero | `src/setup.rs:46` |
| `ConfigMissing` | the local-config read failed, or the address came back all zero | `src/setup.rs:50` |
...
returns a non-zero address (`src/setup.rs:51`). This is expected, not an error to chase; the capsule recovers
...
within the eight-second wait (`src/server/handlers/connect/reply.rs:32`, `src/server/handlers/connect/wait.rs:25`).
...
[connections](connections.md) page describes. The dead entry is removed before the reply
(`src/server/handlers/connect/reply.rs:33`).
...
and cannot take the bytes (`src/server/handlers/send.rs:33`). The `E_TIMEOUT` case is backpressure: the peer
window or the congestion window has stalled the sender and the 64 KiB send buffer filled
(`src/state/entry.rs:65`, `src/tcp/mod.rs:40`). It clears once ACKs advance the window and the sender pump
drains the buffer.
...
queued (`src/server/handlers/recv.rs:33`). `E_RX_EMPTY` is normal on an idle connection; a client polls again.
The distinction from `E_NO_SOCKET` is the ownership check at the end of the wait (`src/server/handlers/recv.rs:42`).
...
(`src/server/handlers/accept.rs:40`). A child is only enqueued when the passive-open handshake reaches its
final ACK (`src/server/tcp_rx/existing.rs:77`), so a persistent `E_RX_EMPTY` on a listener that should be
seeing traffic points at SYNs not arriving (a `net.ip` or DHCP problem) or the handshake not completing (a
checksum or sequence rejection in the receive path, which the [connections](connections.md) page covers).
...
receive window reaps the entry (`src/server/tcp_rx/existing.rs:47`), a segment retransmitted past `MAX_RETX`
times aborts the connection (`src/server/retransmit.rs:43`, `src/tcp/mod.rs:44`), and an expired TimeWait
timer reaps it (`src/server/tick.rs:31`). After any of these, the next `OP_SEND` or `OP_RECV` on that handle
