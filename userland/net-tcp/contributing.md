...
[README](README.md), the [operations](operations.md), [connections](connections.md), [segments](segments.md),
[state](state.md), and [ip-link](ip-link.md) pages.
...
(`src/main.rs:33`). The top-level modules are declared there (`src/main.rs:22`).
...
| `src/setup.rs` | the ISS seed, `net.ip` lookup, and config read | you change bring-up |
| `src/clock.rs` | the millisecond time source | you change the time base |

## Adding an op

There are three edits, and the dispatch wiring is the load-bearing one.

1. Add the opcode constant to `src/protocol/ops.rs:17` and re-export it from `src/protocol/mod.rs:28`. If it
   carries a fixed payload, express the bound with the existing `SEGMENT_PAYLOAD_MAX` / `IPC_PAYLOAD_MAX`
   limits (`src/protocol/limits.rs:17`) rather than a new magic number.
2. Write the handler as one file under `src/server/handlers/`, exposing a `handle` function that reads its
   body with the `io` helpers (`src/server/handlers/io.rs:19`), touches state through `TABLE.lock()`, and
   replies through `respond` (`src/server/respond.rs:23`). `health.rs` is the minimal shape and `send.rs` is
   the shape for an op that mutates a connection. Declare the module in `src/server/handlers/mod.rs:17`.
3. Wire it into the dispatch match in `src/server/runner.rs:45`. An unrecognized op already falls through to
   the `E_BAD_OP` arm (`src/server/runner.rs:54`), so the new arm is what makes the op reachable.
...
matched segment to a transition by state (`src/server/tcp_rx/existing.rs:49`); the transitions themselves are
...
`RxAction` (`src/server/tcp_rx/action.rs:21`), keeping the sequence checks going through `src/tcp/seq.rs`
rather than open-coding modular comparisons. A new state means a variant in `src/tcp/state.rs:19` and a
matching arm wherever `accepts_data` or `is_closing` is consulted; keep the `repr(u8)` discriminant stable,
because `OP_STATE` returns it on the wire (`src/server/handlers/state.rs:28`).
...
`src/userspace/capsule_net_tcp/embed.rs:17`), and the network spawn plan spawns it at boot
(`src/userspace/init/spawn_plan/network/spawn_tcp.rs:17`).
...
- Keep timers, retransmission, buffers, and sequence state in this capsule. The kernel owns no TCP state, and
  a change that pushes any of it into a syscall breaks the boundary the [README](README.md) documents.
- Bound every per-connection and per-client structure. The receive queue, send buffer, reassembly map,
  ...
  `src/tcp/mod.rs:38`; a new queue needs one too.
