# The policy client and write gate
...
trigger these writes see [panels.md](panels.md); for the wider capsule see the
...
`policy` on port 4108 with reply port 4109 (`userland/policy_proto/src/service.rs:17`). Each message is a
12-byte header (`op`, `field`, `kind`, a status word, and a payload length) followed by the payload
(`userland/policy_proto/src/hdr.rs:17`). There are two operations: `OP_GET` (0x0001) and `OP_SET` (0x0002)
(`userland/policy_proto/src/ops.rs:17`).

## Resolving the service

`lookup_policy_port` calls `mk_service_lookup` for the shared `POLICY_SERVICE_NAME` and fails with
`NotFound` if the returned port is zero (`src/settings/ipc/lookup.rs:24`). `Settings::new` calls it once
at construction and, if it fails, `ensure_ready` retries on each later event until it succeeds
(`src/settings/app.rs:42`). Until it does, the status bar shows the policy service as unavailable and the
...
`KindMismatch` if not (`src/settings/ipc/op_get.rs:26`, `:30`). Each decoder rejects an empty payload as a
short reply, and the string decoder clamps to the local `STRING_CAP` (`src/settings/ipc/op_get.rs:63`).
...
starve the scheduler, and it is a no-op if the port is not ready (`src/settings/ipc/hydrate.rs:24`). The
`App` layer runs it exactly once, guarded by a `hydrated` flag (`src/settings/app.rs:49`).

## Writing the store

The four setters `op_set_bool`, `op_set_u8`, `op_set_i8`, and `op_set_str` each encode the value and send
an `OP_SET` with the field's kind (`src/settings/ipc/op_set_bool.rs:22`, `op_set_u8.rs`, `op_set_i8.rs`,
...
reply timeout (`src/settings/ipc/timeout.rs:17`), and validates the reply in order: a non-positive length
...
`Status(code)` error (`src/settings/ipc/call.rs:28`). Only a clean `E_OK` reply returns `Ok`, and only
then does the caller update the cache. Requests too large to frame are refused before any send
(`src/settings/ipc/call.rs:36`).

The full error set is `NotFound`, `SendFailed`, `RecvTimeout`, `ShortReply`, `BadHeader`, `KindMismatch`,
and `Status(u16)` (`src/settings/ipc/error.rs:17`). `report` maps each to a specific red status-bar line:
a timeout to `policy timeout`, a send failure to `ipc send failed`, a returned status to `policy
rejected`, and so on (`src/settings/event/report.rs:22`).

## The shell toast

After any successful `OP_SET`, `call` calls `notify_applied` (`src/settings/ipc/call.rs:73`). That looks
...
(`src/settings/ipc/notify_shell.rs:32`). The send is best-effort: if the shell is not registered or the
...
(`userland/capsule_policy/src/server/recv.rs:21`) and hands it to `dispatch`. `dispatch` calls
...
(`userland/capsule_policy/src/server/handle_set.rs:40`, `:41`).
...
(`userland/capsule_policy/src/server/handle_set.rs:23`, `:36`). So the right to write policy is not a
...
`policy rejected`; it has no way to force the write (`userland/capsule_policy/src/server/handlers/set_str.rs:24`,
`:29`, `src/settings/event/report.rs:29`). Hostname and domain name are re-validated against the same
`[A-Za-z0-9._-]` set the capsule filters on (`userland/capsule_policy/src/store/str_validate.rs:17`), so
