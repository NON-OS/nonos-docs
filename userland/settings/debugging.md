# Debugging capsule_settings

This page lists the log marker the settings boot path emits and the concrete failure modes with where to
look for each. For the model see the [README](README.md), the [panels reference](panels.md), the
[policy client](policy.md), the [rendering](rendering.md), and the [input](input.md) pages in this folder.
...
(`src/userspace/init/spawn_plan/apps_tools.rs:40`). On a successful boot the `Ok` arm of that path logs
`[APP-SETTINGS] capsule spawned` (`src/userspace/init/capsule_boot/run.rs:29`). If that line is absent the
...
failure caught at verified spawn (`src/userspace/capsule_settings/spawn.rs:56`).
...
static defaults` before consulting any status (`src/settings/ipc/lookup.rs:32`,
`src/settings/paint/paint_status.rs:31`). Confirm the policy capsule is registered; the values stay blank
until the lookup succeeds and `ensure_ready` runs hydration once (`src/settings/app.rs:42`). The lookup is
...
`IpcError::Status` and `report` maps to `policy rejected` (`src/settings/ipc/call.rs:70`,
`src/settings/event/report.rs:29`). The usual cause is an out-of-range numeric value or an invalid hostname
character, rejected by the store handler with `E_INVAL` or `E_BAD_LEN`
(`userland/capsule_policy/src/server/handlers/set_str.rs:24`, `userland/policy_proto/src/errno.rs:18`). The
...
(`userland/capsule_policy/src/server/handle_set.rs:41`).

### A change shows `policy timeout` or `ipc send failed`

The request did not get a reply within 500 ms, or the send itself failed
(`src/settings/ipc/timeout.rs:17`, `src/settings/ipc/error.rs`, `src/settings/event/report.rs:24`). This
...
`push_text_char` (`src/settings/event/push_text_char.rs:33`). This is by design and matches the server-side
...
that `State` into the surface (`src/settings/paint/paint.rs:31`). A blank frame with a live model points at
...
that is not a key-down or a button-down (`src/settings/event/on_event.rs:29`).
