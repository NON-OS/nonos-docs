...
into a filtered buffer. A change that touches the store goes out through the [policy client](policy.md) and
sets the status line; the model it drives is described in [panels.md](panels.md).

## Event gate

`on_event` is the top of the tree (`src/settings/event/on_event.rs:25`):
...
(`src/settings/event/on_event_browsing.rs:30`). Every arm returns `Repaint` except `Esc`, which closes the
window, and unmatched keys, which return `Idle`:

| Key | Action | Source |
|---|---|---|
| Up | move the cursor up one row | `on_event_browsing.rs:34`, `state/cursor_up.rs` |
| Down | move the cursor down one row | `on_event_browsing.rs:35`, `state/cursor_down.rs` |
| Left | decrement the current numeric or enum field | `on_event_browsing.rs:36`, `event/adjust.rs:24` |
| Right | increment the current numeric or enum field | `on_event_browsing.rs:37`, `event/adjust.rs:24` |
| Space or Enter | toggle a bool, cycle an enum, or open the string editor | `on_event_browsing.rs:38`, `event/toggle_or_inc.rs:24` |
| Tab or `]` | switch to the next tab | `on_event_browsing.rs:33`, `:40`, `event/next_category.rs` |
| `[` | switch to the previous tab | `on_event_browsing.rs:39`, `event/prev_category.rs` |
...
(`src/settings/event/toggle_or_inc.rs:29`). Left and Right go through `adjust`, which dispatches to the u8
or i8 adjuster by kind and does nothing for a bool or string field
(`src/settings/event/adjust.rs:29`). The u8 adjuster clamps to the field's max and the i8 adjuster clamps
to -12..=14 (`src/settings/event/adjust_u8.rs:26`, `src/settings/event/adjust_i8.rs:25`).
...
(`src/settings/event/commit_bool.rs:27`). Numeric adjusters follow the same accept-then-store shape.

## Editing a string field

`on_event_editing` runs while a hostname or domain-name editor is open
(`src/settings/event/on_event_editing.rs:24`):
...
| Esc | cancel the edit and discard the buffer | `on_event_editing.rs:26`, `state/edit_cancel.rs` |

`push_text_char` is the filter. It accepts only a printable byte in `0x20..=0x7E` that also passes the
`[A-Za-z0-9._-]` allow-list, and silently drops anything else (`src/settings/event/push_text_char.rs:23`,
`:33`). This is the capsule's half of the hostname rule; the policy server re-validates the same set on the
write, so the two must agree (see [policy.md](policy.md)). `commit_string` reads the buffer, sends the
`OP_SET`, and on success snapshots the bytes into the cache and shows `updated`; on failure it reports the
error and does not touch the cache (`src/settings/event/commit_string.rs:24`).

## Pointer input

`on_pointer` handles a button-down at a pixel (`src/settings/event/on_pointer.rs:34`). A negative
...
(`src/settings/event/commit_bool.rs:30`), and a failure goes through `report`, which maps each `IpcError`
to a fixed message: `policy timeout`, `ipc send failed`, `policy rejected`, `short reply`, `bad header`,
`kind mismatch`, or `policy service not registered` (`src/settings/event/report.rs:22`). The renderer
projects that line and its colour; see [rendering.md](rendering.md).
