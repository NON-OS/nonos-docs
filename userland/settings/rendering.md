...
whenever an event returns `Repaint`; for what produces those events see [input.md](input.md), and for the
model it draws see [panels.md](panels.md).

## The frame

`paint` is the top of the renderer (`src/settings/paint/paint.rs:31`). It runs a fixed sequence:
...
`Category::Identity`, and `Category::Kernel` (`src/settings/paint/paint_tabs.rs:25`). Each tab is a third
...
[input.md](input.md), which splits the same strip into thirds.

## Rows

`paint` walks the visible slice and calls `paint_field_row` for each, passing the absolute row index, the
y offset, and whether that row is under the cursor (`src/settings/paint/paint.rs:42`). A row fills its
background (selected, or one of two alternating shades by parity), draws the field label from `label_of`
on the left, and hands the value area to `paint_field_value` (`src/settings/paint/paint_field_row.rs:35`).

`paint_field_value` is where the value renderer branches on kind (`src/settings/paint/paint_field_value.rs:34`):
...
tabs, and status bands, and the count is that divided by the row height (`src/settings/paint/visible_rows.rs:21`).
...
visible window (`src/settings/paint/scroll_indicator.rs`).

## The status bar

`paint_status` draws the bottom band and picks its text and colour in three cases
(`src/settings/paint/paint_status.rs:28`):
