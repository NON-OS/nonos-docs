# Contributing to capsule_settings
...
[README](README.md), the [panels and controls reference](panels.md), the
[policy client and write gate](policy.md), the [rendering](rendering.md), and the [input](input.md) pages
in this folder.
...
subscription, and paint loop (`userland/capsule_settings/src/main.rs:28`). The module tree is declared in
...
1. Add the field to the shared protocol. Give it an id in the right category range in
   `userland/policy_proto/src/field.rs` (`0x01xx` user, `0x02xx` kernel, `0x03xx` identity), a label in
...
2. Register it in the capsule schema. Add the field to `ALL_FIELDS`
   (`src/settings/schema/all_fields.rs:19`) so it gets a cache slot and is hydrated at startup, and add it
   to the right group in `visible_for` (`src/settings/schema/visible_for.rs:19`) so it shows up as a row on
...
shared crate, the `adjust` and `toggle_or_inc` dispatch (`src/settings/event/adjust.rs:29`,
`src/settings/event/toggle_or_inc.rs:29`), the per-kind paint renderers (`src/settings/paint/`), and the
...
- One unit per file. Each event handler, each setter, and each paint pass is its own file, and `mod.rs` is
  used only for re-exports, matching the existing tree (`src/settings/mod.rs`, `src/settings/event/mod.rs`,
  `src/settings/ipc/mod.rs`, `src/settings/paint/mod.rs`).
