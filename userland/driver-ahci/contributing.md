# Contributing to capsule_driver_ahci
...
change has to meet. For what the driver does and how it is put together, read the [README](README.md),
the [operation surface](operations.md), the [bring-up](bringup.md), and the [command engine](engine.md)
...
(`userland/capsule_driver_ahci/src/main.rs:37`). The module tree is declared there
(`src/main.rs:22`).
...
| `src/discover.rs` | the PCI storage/SATA/AHCI match | you change what counts as a candidate controller |
...
1. Add the opcode to `src/protocol/ops.rs`, following the existing constants (`ops.rs:17`). Add any fixed
   payload size to `src/protocol/limits.rs` (`limits.rs:17`).
2. Write the handler as one file under `src/server/handlers/`, exposing a `pub fn handle(...)` that builds
   ...
   an errno through `reply_with_status` (`src/server/error.rs:23`, `src/server/handlers/capacity.rs:26`
   is a good reference for a fixed-size reply that guards on `driver.block`). Re-export it from
   `src/server/handlers/mod.rs`.
3. Wire it into the match in `src/server/runner.rs` (`runner.rs:55`). If it is a fixed-size op that must
   carry no payload, add it to the leading guard arm alongside the existing three
   (`src/server/runner.rs:57`); otherwise add a plain arm.
4. If the op transfers sectors, parse and range-check the body through `super::rw_parse::parse` the way
   ...
   (`src/server/handlers/rw_parse.rs:20`).

## Adding an ATA command

1. Add the command byte to `src/constants/ata.rs` next to the existing ones (`ata.rs:17`).
2. Add an engine wrapper under `src/engine/` that calls `build::build_slot0` (or hand-builds the FIS for a
   ...
   `transfer` and `flush` do (`src/engine/transfer.rs:22`, `src/engine/flush.rs:27`). Re-export it from
   `src/engine/mod.rs` if a handler needs to call it (`src/engine/mod.rs:35`).
3. Keep every register offset and bit mask in `src/constants/regs.rs`; the engine code should reference a
...
(`src/userspace/init/spawn_plan/drivers_storage.rs:23`).
...
- No panics in capsule code. No `unwrap`, `expect`, or `panic!`. Every fallible bring-up path returns an
  `AhciError` (`src/error/types.rs:17`) and every request path returns an errno, never a panic; the
...
- No path to hardware except the broker. The driver holds no `Pio` bit and never touches an I/O port; MMIO
  ...
  (`src/engine/region.rs:28`). Do not import kernel driver, memory, or paging code; the only hardware path
  stays the broker.
