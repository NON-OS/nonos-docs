...
mapped, see [bringup.md](bringup.md); for the requests that reach `transfer` and `flush`, see
...
and a grant id (`src/engine/region.rs:28`). The driver never invents a physical address; it only programs
the ones the broker handed it. Each region unmaps itself on drop with `mk_dma_unmap`
(`src/engine/region.rs:46`), so the port's four regions are freed when the `Port` drops. The
[dma](../../subsystems/hardware-broker/dma.md) broker page describes how the allocation is bounded and
tracked.

`init_port` allocates four regions (`src/engine/init.rs:24`):
...
32 KiB (`src/constants/ata.rs:29`). The `fb` region is held only to keep the mapping alive; the port
stores it as `_fb` (`src/engine/port.rs:22`).
...
The PRDT sits inside the command table at byte offset 128, `PRDT_OFFSET` (`src/engine/cmd_table.rs:29`).
...
(`src/engine/init.rs:30`).

`stop` clears `PxCMD.ST`, spins until the command-list-running and FIS-receive-running bits (`CR`, `FR`)
clear or the poll limit is hit, then clears `PxCMD.FRE` (`src/engine/stop.rs:21`). Idling the engines is
...
(`src/engine/program.rs:24`, `src/constants/regs.rs:43`).

`start` waits for `CR` to clear, then sets `FRE | ST` to re-enable the FIS-receive and command engines
(`src/engine/start.rs:21`). Both spins in stop and start are bounded by `COMPLETION_POLL_LIMIT`
...
controllers as well as on QEMU's `ich9-ahci` (`src/engine/stop.rs:21`, `src/engine/start.rs:21`,
`src/discover.rs:60`).
...
data command (`src/engine/build.rs:26`):
...
4. It writes command header 0 into the command-list region with the FIS length in dwords
   ...
   `CMD_HEADER_WRITE` (`1 << 6`) when the transfer is outbound (`src/engine/build.rs:54`,
   `src/constants/ata.rs:22`).
...
(`src/engine/flush.rs:27`).

## Issuing and polling

`issue_slot0` is the single completion path for every command (`src/engine/issue.rs:22`):

1. Clear `PxIS` by writing all-ones.
2. Spin until the task file leaves `BSY | DRQ`, returning `Timeout` if the spin passes
   `COMPLETION_POLL_LIMIT` (`src/engine/issue.rs:26`).
3. Write 1 to `PxCI` to issue command slot 0 (`src/engine/issue.rs:33`).
4. Loop: if an error bit appears in `PxIS` (`IS_ERR_MASK`, the four highest error bits) or `PxTFD.ERR` is
   ...
   the poll limit yields `Timeout` (`src/engine/issue.rs:35`).

Completion is decided entirely here by polling, never by the interrupt. `COMPLETION_POLL_LIMIT` is
5,000,000 spin iterations (`src/constants/ata.rs:33`).
...
(`src/engine/recover.rs:20`). Reissuing the same request after a recover is safe.
...
- `identify` issues `IDENTIFY` (0xEC) into the data buffer, then reads the 48-bit sector count from
  ...
  zero, and stores the result as `port.capacity_sectors` (`src/engine/identify.rs:22`,
  `src/constants/ata.rs:17`). This runs once at bring-up and is what `OP_CAPACITY` and the read/write
  range check read back.
- `transfer` issues `READ DMA EXT` (0x25) or `WRITE DMA EXT` (0x35) for the requested LBA and count,
  chosen by the `write` flag (`src/engine/transfer.rs:22`, `src/constants/ata.rs:18`).
- `flush` issues `FLUSH CACHE EXT` (0xEA) with no PRDT (`src/engine/flush.rs:27`,
  `src/constants/ata.rs:20`).

All three run `recover` and return the error on an issue failure, so a caller sees a clean `Err` and the
port is left re-idled and restarted (`src/engine/identify.rs:24`, `src/engine/transfer.rs:25`,
`src/engine/flush.rs:48`).

## The port model

A `Port` owns its four DMA regions, its register base offset, and its identified capacity
(`src/engine/port.rs:19`). The base is `PORT_BASE + index * PORT_STRIDE` computed once at bring-up
(`src/engine/init.rs:29`) and reused for every register access, so the engine never needs the port index
...
device-visible views of the same buffer separate and explicit (`src/engine/build.rs:27`,
`src/server/handlers/read.rs:48`).
