...
[bringup.md](bringup.md); for the hardware path a read or write drives, see [engine.md](engine.md); for
identity and the capability mask, see the [README](README.md).

## The wire format

Every request and every reply begins with the same 20-byte header (`src/protocol/header.rs:19`,
`src/protocol/decode.rs`, `src/protocol/encode.rs:19`):
...
are not `NAHC` or whose version is not 1, returning `None` (`src/protocol/decode.rs:19`). A `None` decode
does not touch a port: the loop answers it with `E_INVAL` through the decode-failed path and moves on
(`src/server/runner.rs:45`, `src/server/error.rs:29`).
...
`src/protocol/encode.rs:19`). A non-negative status is success; a negative status is one of the errno
values below.
...
(`src/server/runner.rs:29`). `MAX_RW_PAYLOAD_BYTES` is the 32 KiB data-buffer size,
`crate::constants::ata::DATA_BUF_BYTES` (`src/protocol/limits.rs:24`, `src/constants/ata.rs:31`).

Each iteration does four things (`src/server/runner.rs:37`):
...
The dispatch is a single match (`src/server/runner.rs:55`). The fixed-size ops, `OP_HEALTHCHECK`,
`OP_CONTROLLER_INFO`, and `OP_PORT_LIST`, are guarded by a leading arm that rejects any of them carrying a
non-zero `payload_len` with `E_INVAL` before the handler runs (`src/server/runner.rs:57`). An opcode no
arm matches also falls to `E_INVAL` (`src/server/runner.rs:67`).
...
`RESP_HDR_LEN + 4` bytes (`src/server/error.rs:23`).
...
the new sequence (`src/server/runner.rs:71`). This keeps the interrupt drained but does not gate command
completion: completion is decided entirely by polling `PxCI` and the status registers inside the engine
(see [engine.md](engine.md)). That separation is why a controller with no routed legacy interrupt line
still works.
...
(`src/server/handlers/health.rs:20`). It is the liveness probe: a reply at all means the capsule spawned,
...
(`src/server/handlers/controller_info.rs:25`, `src/controller/info.rs:31`). The reply payload is the
4-byte status word plus that 24-byte record (`src/protocol/limits.rs:18`).
...
`PxCI` (`src/server/handlers/port_list.rs:26`, `src/protocol/limits.rs:20`). The kind byte is `1` for
SATA, `2` ATAPI, `3` SEMB, `4` port multiplier, `0` none, `255` unknown
(`src/constants/port.rs:19`). Because it is the setup snapshot, port list is the probe for a disk that
never came up: check each entry's `present` flag and its `PxSSTS` and `PxSIG` to see what the controller
reported (see [debugging.md](debugging.md)).

### Capacity

`OP_CAPACITY` returns the block port's identified sector count as an 8-byte little-endian value
(`src/server/handlers/capacity.rs:26`, `src/protocol/limits.rs:23`). If no port was brought up,
`driver.block` is `None` and the handler replies `E_NODEV` before building any payload
(`src/server/handlers/capacity.rs:27`). The count itself is filled in by `IDENTIFY` at bring-up (see
[engine.md](engine.md)).
...
(`src/server/handlers/rw_parse.rs:20`, `src/constants/ata.rs:30`). Both handlers reply `E_NODEV` first if
there is no block port (`src/server/handlers/read.rs:32`, `src/server/handlers/write.rs:27`).

`OP_READ_BLOCKS` additionally requires `payload_len` to equal the 12-byte request length before parsing
(`src/server/handlers/read.rs:34`). It calls `transfer(..., write=false)`, and on success copies the
completed `count * 512` bytes out of the DMA data region into the reply body after the status word
(`src/server/handlers/read.rs:41`). A transfer error becomes `E_IO`.

`OP_WRITE_BLOCKS` parses first, then checks that both the body length and `payload_len` are exactly
`12 + count * 512` before it copies (`src/server/handlers/write.rs:34`). It copies the request body into
the DMA data region, issues `transfer(..., write=true)`, and replies with a status word only, `E_OK` on
success or `E_IO` on failure (`src/server/handlers/write.rs:37`).

### Flush

`OP_FLUSH` issues `FLUSH CACHE EXT` with no data transfer and replies with a status word, `E_OK` or
`E_IO` (`src/server/handlers/flush.rs:22`, `src/engine/flush.rs:27`). It replies `E_NODEV` if there is no
block port.
...
- bad magic or version: dropped in `decode_request`, answered `E_INVAL` (`src/protocol/decode.rs:23`).
- a fixed-size op carrying a payload: `E_INVAL` from the guard arm (`src/server/runner.rs:57`).
- an unknown opcode: `E_INVAL` from the fallthrough arm (`src/server/runner.rs:67`).
- no block port for a data op: `E_NODEV` before any parse (`src/server/handlers/capacity.rs:27`).
- wrong length for read or write: `E_MSGSIZE` (`src/server/handlers/read.rs:34`,
  `src/server/handlers/write.rs:34`).
- zero or over-64 sector count: `E_INVAL` (`src/server/handlers/rw_parse.rs:26`).
- LBA range past capacity: `E_NXIO` (`src/server/handlers/rw_parse.rs:30`).
