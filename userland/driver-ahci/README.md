# The AHCI Driver Capsule

`capsule_driver_ahci` is the SATA storage driver in the NONOS tree: a signed userland capsule that owns
...
(`userland/capsule_driver_ahci/src/main.rs:38`). The two halves stay clean: `setup::run` performs every
...
and acknowledge the controller interrupt (`src/setup/sequence.rs:26`, `src/server/runner.rs:29`).
...
|------|---------|----------------|
| [operations.md](operations.md) | `src/protocol/`, `src/server/` | The `NAHC` wire format, the request loop and dispatch, the IRQ poll, and every one of the seven ops with its opcode, payloads, and errno set. |
| [bringup.md](bringup.md) | `src/discover.rs`, `src/setup/`, `src/controller/`, `src/handles/` | The one-shot bring-up: PCI discovery, the broker claim, bus-master enable, ABAR mapping, IRQ bind, AHCI-mode enable, port scan, and the grant owners that free everything on drop. |
| [engine.md](engine.md) | `src/engine/`, `src/regs/` | The command engine: DMA regions, the command list, FIS, command table and PRDT layouts, port program and stop/start, and issue, identify, transfer, flush, and recover. |
| [contributing.md](contributing.md) | the whole tree | Where each behaviour lives, how to add an op or an ATA command, the build and sign steps, and the code standards. |
| [debugging.md](debugging.md) | runtime | The boot marker, the setup exit codes, the no-port and command-timeout failure modes, and the real-hardware notes. |
...
is its ELF mapped and run (`src/hardware/ahci_capsule/spawn.rs:37`). `setup::run` executes the whole
privileged path once and returns a `Driver`, or exits the process with a setup exit code
(`src/setup/sequence.rs:26`, `src/main.rs:43`). On success the storage fleet logs `[DRIVER-AHCI] capsule
spawned` (`src/userspace/init/spawn_plan/drivers_storage.rs:27`, `docs/userland/drivers.md:294`), and the
server enters its request loop. The [debugging](debugging.md) page covers what each later exit code and
errno means.

## Operation summary

The service accepts one request at a time on its endpoint, decodes the 20-byte `NAHC` header, and
dispatches on the operation id (`src/server/runner.rs:39`). The seven ops and their opcodes
