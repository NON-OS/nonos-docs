# Contributing to capsule_driver_usb_hid
...
steps. For what the driver does and how it is put together, read the [README](README.md), the
...
(`src/main.rs:33`). The seven top-level modules are declared there (`src/main.rs:22`).
...
1. Add the op code to `src/protocol/ops.rs:17` and re-export it from `src/protocol/mod.rs:33`, next to
...
2. Write the handler as one file under `src/server/handlers/`, exposing a `pub fn handle(...)` that
   ...
   never panics; declare it in `src/server/handlers/mod.rs:17`. The state ops
   (`src/server/handlers/get_state.rs:21`) and poll ops (`src/server/handlers/poll_keys.rs:21`) are the
   reference shape for a payload reply.
3. Wire the op into the match in `src/server/dispatch.rs:22`. Gate it on an empty body in the match
   arm if it takes no input, the way the poll and state ops do (`src/server/dispatch.rs:27`), or
   validate the body length inside the handler the way the feed ops do
   (`src/server/handlers/feed_key.rs:22`). A body that does not fit an op's expectation returns
   `E_INVAL` through the dispatch fall-through (`src/server/dispatch.rs:33`).

## Extending the HID parse or the input mapping

Edit the relevant unit under `src/hid/`. The keyboard diff is `src/hid/keyboard/feed.rs`, the
usage-to-ASCII mapping is `src/hid/keymap.rs` and `src/hid/punctuation.rs`, the navigation-key and
flag mapping is `src/hid/post_key.rs`, the mouse is `src/hid/mouse.rs` with its post in
`src/hid/post_mouse.rs`, and the absolute tablet path is `src/hid/tablet.rs`. The actual
`mk_input_event_post` call is centralized in `src/hid/post_wire.rs`, so a change to how any event
reaches the ring belongs there and nowhere else. If a new device shape needs a new endpoint kind, add
it to `HidKind` (`src/descriptors/types.rs:26`) and to the classification in
`src/descriptors/binding.rs:32` and the routing in `src/orchestrator/poll/feed_report.rs:29`.
...
- No panics in capsule code. No `unwrap`, `expect`, or `panic!`. Every handler returns an error as a
  ...
  (`userland/capsule_driver_usb_hid/Cargo.toml:26`).
