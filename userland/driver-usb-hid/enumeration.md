# Enumeration, the transport client, and the poll loop
...
[input-post path](input-post.md).

## The transport client

The client speaks the `NXHC` protocol (magic `0x4E58_4843`, version 1) to `driver.xhci0`
(`src/xhci/wire/constants.rs:19`). It resolves the transport port once, by name, at startup:
`lookup` calls `mk_service_lookup("driver.xhci0")` and returns the port on success
(`src/xhci/lookup.rs:19`, `src/xhci/lookup.rs:24`). Every op is then a synchronous `mk_ipc_call`
...
(`src/xhci/call.rs:29`, `src/xhci/call.rs:45`). The request id comes from a monotonic counter that
skips zero (`src/xhci/seq.rs:21`).

The ops the client uses, one file each under `src/xhci/ops/`
(`src/xhci/ops/mod.rs:17`):
...
(`src/xhci/ops/port_status.rs:41`, `src/xhci/ops/port_status.rs:42`); a `get_config_descriptor` reply
is a 2-byte actual length then the bytes, bounded to the caller's buffer
(`src/xhci/ops/config_descriptor.rs:39`); an `interrupt_in` reply is a 2-byte actual length then up to
8 report bytes, and a status of `E_AGAIN` (-11) is returned as `Ok(None)` meaning no report is pending
(`src/xhci/ops/interrupt_in.rs:38`, `src/xhci/ops/interrupt_in.rs:45`).
...
(`src/descriptors/parse.rs:24`). It first validates the header: the buffer must be at least 9 bytes,
...
(`src/descriptors/parse.rs:48`). It then walks the record list from offset 9 up to `wTotalLength`,
rejecting any record whose length is under 2 or runs past the total
(`src/descriptors/parse.rs:30`, `src/descriptors/parse.rs:32`). An interface descriptor of length 9 or
more sets the current interface; an endpoint descriptor of length 7 or more is paired with that
interface (`src/descriptors/parse.rs:36`). The walk caps at 8 bindings (`src/protocol/limits.rs:21`,
`src/descriptors/parse.rs:40`).

`HidBinding::from_pair` decides what an interface-endpoint pair becomes
(`src/descriptors/binding.rs:32`). It accepts only a HID-class interface (class `0x03`) on an
interrupt IN endpoint, where interrupt IN is the direction bit set on the address and the interrupt
transfer type on the attributes (`src/descriptors/binding.rs:33`, `src/descriptors/binding.rs:55`).
...
(`src/descriptors/binding.rs:36`). The boot subclass and keyboard/mouse protocol constants are in
`src/descriptors/types.rs:21`. This one classification is shared by the live enumerator and the
`OP_PROBE_CONFIG` handler, so a probed descriptor is classified identically to a live one.

## Discovery

`enumerate` is discovery over the transport (`src/orchestrator/enumerate/run.rs:25`). It asks for a
port-status snapshot of up to 255 ports, and for each port whose `portsc` has the connected bit set it
calls `configure_port` (`src/orchestrator/enumerate/run.rs:31`, `src/orchestrator/enumerate/run.rs:32`).

`configure_port` drives the standard USB bring-up as a sequence of xHCI ops
(`src/orchestrator/enumerate/configure_port.rs:28`): enable a slot, address the device, and sanity-check
the returned slot id, port id, speed, and max packet size before proceeding
(`src/orchestrator/enumerate/configure_port.rs:35`). It reads a 64-byte configuration descriptor
(`src/orchestrator/enumerate/constants.rs:17`, `src/orchestrator/enumerate/configure_port.rs:43`),
...
(`src/orchestrator/enumerate/configure_port.rs:46`, `src/orchestrator/enumerate/configure_port.rs:53`).

For each binding `configure_binding` selects the boot protocol and allocates the interrupt ring
(`src/orchestrator/binding.rs:24`). For a boot keyboard or mouse it first issues a `SET_PROTOCOL`
control transfer (request type `0x21`, request `0x0B`, value 0 = boot) so the device sends the fixed
boot report layout (`src/orchestrator/binding.rs:30`, `src/orchestrator/binding.rs:33`). It then asks
...
(`src/orchestrator/binding.rs:35`, `src/orchestrator/binding.rs:44`). The boot protocol is what makes
...
(`src/orchestrator/run.rs:20`, `src/orchestrator/run.rs:28`). The loop enumerates once, allocates its
receive and transmit buffers, and then repeats three things each iteration
(`src/orchestrator/poll/run.rs:27`):

1. Service one request with `pump_once` (covered on the [protocol](protocol.md) page)
   (`src/orchestrator/poll/run.rs:40`).
2. Drain every bound endpoint with `drain_endpoints`, which loops `interrupt_in` on each endpoint
   ...
   (`src/orchestrator/poll/run.rs:41`, `src/orchestrator/poll/drain_endpoints.rs:29`,
   `src/orchestrator/poll/drain_endpoint.rs:30`).
3. If no endpoints are bound and it has been idle for a rescan interval of 64 idle polls, re-enumerate
   to pick up a hot-plugged device (`src/orchestrator/poll/run.rs:46`,
   `src/orchestrator/poll/constants.rs:18`, `src/orchestrator/poll/refresh_endpoints.rs:21`).
...
the key and mouse report counters bumped (`src/orchestrator/poll/feed_report.rs:29`). The first time
any endpoint binds, the loop emits `[USB-HID-ENUM] tablet bound` through `mk_debug` exactly once
(`src/orchestrator/poll/run.rs:35`). The parsers themselves are on the [input-post](input-post.md)
page.
