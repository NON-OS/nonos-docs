# Report parsing and the input-post path
...
capsules see the [input path](../../subsystems/input/path.md).

The post-side files are `post_key.rs`, `post_mouse.rs`, and `post_wire.rs`. An older subsystem page
refers to a `hid/publish.rs` file for this driver; there is no such file. The button code, the mouse
publisher, and the `post_failures` bump named there now live in `src/hid/post_mouse.rs`,
`src/hid/post_wire.rs`, and `src/hid/mouse.rs`. The [debugging](debugging.md) page and the
[README](README.md) note the same correction.

## The InputEvent and the syscall

Every post is one `InputEvent`, the shared 32-byte flat record whose userland mirror is
`userland/libc/src/surface_registry/types.rs:44`, carrying a kind, modifier flags, a code, absolute
x/y, relative deltas, and a timestamp. This capsule sets `timestamp_ns` to zero on every event
(`src/hid/post_wire.rs:21`). The `INPUT_KIND_*` constants it uses are defined once, in userland
(`userland/libc/src/surface_registry/types.rs:23`).

`post_wire` is the one place the syscall is made (`src/hid/post_wire.rs:19`). `send` fills a relative
event (kind, flags, code, and dx/dy) and calls `mk_input_event_post`, returning true when the syscall
returns `>= 0` (`src/hid/post_wire.rs:19`, `src/hid/post_wire.rs:22`). `send_abs` fills an absolute
event with x/y and zeroed deltas for the tablet path (`src/hid/post_wire.rs:25`). The wrapper
`mk_input_event_post` is the libc entry for the `MkInputEventPost` syscall
(`userland/libc/src/surface_registry/input_post.rs:22`). Because every event routes through these two
...
the up to six currently-pressed usage codes (`src/hid/keyboard/feed.rs:21`). It stores the modifier
mask, then diffs the six usages against the previous frame `self.prev`
(`src/hid/keyboard/feed.rs:22`, `src/hid/keyboard/feed.rs:24`). A usage present now that was not
present before is a press; a usage that was present and is now gone is a release
(`src/hid/keyboard/feed.rs:25`, `src/hid/keyboard/feed.rs:30`). A usage code of 0 or 1 is filtered as
not a real key (`src/hid/keyboard/is_real_key.rs:17`). The frame is then remembered as the new `prev`,
so a held key repeats no event (`src/hid/keyboard/feed.rs:34`).

Each transition calls `push_key(scancode, pressed)` (`src/hid/keyboard/push_key.rs:22`). On a press it
toggles Caps Lock if the usage is `0x39` (`src/hid/keymap.rs:22`), resolves an ASCII byte from the
usage code, the modifier mask, and Caps Lock through `keymap::ascii`, and builds a `KeyEvent` of
scancode, ascii, modifiers, and pressed (`src/hid/keyboard/push_key.rs:26`,
`src/hid/keyboard/push_key.rs:28`). It publishes the event through `post_key::publish`, bumping
`post_failures` on a failed post, and mirrors the event into the bounded local queue for `OP_POLL_KEYS`
if the queue is under its cap of 64 (`src/hid/keyboard/push_key.rs:29`,
`src/hid/keyboard/constants.rs:17`).

The keymap covers letters, digits, whitespace, and punctuation (`src/hid/keymap.rs:26`). A letter
usage `0x04..=0x1d` is mapped with an upper-case decision that is `shift XOR caps`, so Shift and Caps
Lock combine the way a real keyboard does (`src/hid/keymap.rs:29`, `src/hid/keymap.rs:41`). Digit
usages `0x1e..=0x27` map to their plain or shifted symbols (`src/hid/keymap.rs:46`); Enter, Escape,
Backspace, Tab, and Space are direct (`src/hid/keymap.rs:31`); punctuation usages `0x2d..=0x38` go
through a shifted/unshifted table (`src/hid/punctuation.rs:17`). A usage outside those ranges yields
ASCII 0.

## The keyboard post

`post_key::publish` turns a `KeyEvent` into an `InputEvent` (`src/hid/post_key.rs:32`). The kind is
`INPUT_KIND_KEY_DOWN` on a press and `INPUT_KIND_KEY_UP` on a release
(`src/hid/post_key.rs:33`). The code is chosen in two steps. Navigation usages map to private key
codes `0xE000..0xE008`: arrows, Home, End, Delete, Page Up, and Page Down
(`src/hid/post_key.rs:37`). Otherwise the code is the ASCII byte, with `\n` normalized to `0x0D`, or,
for a key the keymap did not resolve (ASCII 0), the fallback `0x2000 | scancode`
(`src/hid/post_key.rs:52`). The modifier mask is packed into shift/ctrl/alt/meta flag bits by testing
the left and right pairs of the HID modifier byte (`src/hid/post_key.rs:60`). A consumer that sees a
...
(`src/hid/mouse.rs:35`, `src/hid/mouse.rs:40`). It enqueues an event only if something changed:
motion, wheel, or a button transition (`src/hid/mouse.rs:46`). The event carries dx, dy, dz, the
button mask, and a flags byte recording which of those changed (`src/hid/mouse.rs:47`). `push`
publishes it through `post_mouse::publish` with the previous button mask, bumping `post_failures` on a
failed post, and mirrors it into the local queue under a cap of 64 (`src/hid/mouse.rs:65`,
`src/hid/mouse.rs:66`, `src/hid/mouse.rs:69`).

`post_mouse::publish` posts up to three kinds, each a separate `send`
(`src/hid/post_mouse.rs:24`). A non-zero dx/dy sends `INPUT_KIND_POINTER_REL` with `delta_x`/`delta_y`
(`src/hid/post_mouse.rs:26`); a non-zero wheel sends `INPUT_KIND_WHEEL` with `delta_y`
(`src/hid/post_mouse.rs:29`); and each changed button bit sends `INPUT_KIND_BUTTON_DOWN` or
`INPUT_KIND_BUTTON_UP` with `code = bit + 1`, so left is 1, right is 2, middle is 3
(`src/hid/post_mouse.rs:35`, `src/hid/post_mouse.rs:44`). The button diff is `previous XOR current`
masked to the low five bits, so only a bit that flipped emits an event
(`src/hid/post_mouse.rs:36`).

## The tablet path

The absolute tablet path is separate and post-only: it keeps no local queue and is not exposed as a
poll op (`src/hid/tablet.rs:23`). `Tablet::feed` requires at least 5 bytes: byte 0 is the button mask
(low 3 bits), bytes 1 to 2 are a little-endian 16-bit x, bytes 3 to 4 a 16-bit y, and an optional byte
5 is a signed wheel delta (`src/hid/tablet.rs:33`, `src/hid/tablet.rs:36`). It posts an
...
(`src/hid/tablet.rs:40`, `src/hid/tablet.rs:44`, `src/hid/tablet.rs:49`). Absolute coordinates leaving
here are device-space; the router maps them onto the display, as the
[input path](../../subsystems/input/path.md) documents.
