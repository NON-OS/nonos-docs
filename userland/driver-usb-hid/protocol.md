# The service protocol and handlers
...
seven handlers. For the identity and the capability mask see the [README](README.md); for the live
event stream that does not use these ops see the [input-post path](input-post.md).

## The wire format

A request is a 20-byte `NUHI` header followed by an optional payload
(`src/protocol/header.rs:17`). The header is magic `0x4E55_4849` ("NUHI"), version 1, then the op,
the flags, two pad bytes, a request id, and the payload length
(`src/protocol/header.rs:17`, `src/protocol/decode.rs:19`).
...
(`src/protocol/decode.rs:20`, `src/protocol/decode.rs:33`). Only a fully consistent frame yields a
...
(`src/protocol/encode.rs:19`), and its body always begins with a 4-byte signed status word
(`src/protocol/encode.rs:29`). `respond::status` sends just the header and the status;
`respond::payload` sends the header, a zero status, and a body, and is used by the probe, poll, and
state ops (`src/server/respond.rs:21`, `src/server/respond.rs:27`). Both call `mk_ipc_reply` to the
sender pid (`src/server/respond.rs:24`).

The size limits are fixed constants (`src/protocol/limits.rs:17`): the header is 20 bytes and the
...
three values: `E_INVAL` (-22), `E_BAD_OP` (-38), `E_NO_HID` (-61) (`src/protocol/errno.rs:17`).
...
(`src/server/pump_once.rs:26`, `src/server/pump_once.rs:35`). A parsed frame is handed to `dispatch`
with the sender pid, the request, the body, and the shared transmit buffer
(`src/server/pump_once.rs:41`).

`dispatch` matches on the op code (`src/server/dispatch.rs:22`). The ops that take no input are gated
...
returns `E_BAD_OP` (-38) (`src/server/dispatch.rs:30`, `src/server/dispatch.rs:33`).
...
`src/server/handlers/mod.rs:17`. None panics; every failure is a signed status word.

### Healthcheck

`OP_HEALTHCHECK` replies with a zero status and no body (`src/server/handlers/health.rs:20`). It is
the liveness probe.
...
(`src/server/handlers/probe_config.rs:25`, `src/server/handlers/probe_config.rs:29`). A malformed
descriptor is `E_INVAL`; a well-formed descriptor with no boot HID interface is `E_NO_HID` (-61)
(`src/server/handlers/probe_config.rs:32`, `src/server/handlers/probe_config.rs:37`). On success it
bumps `configs_probed`, writes the binding count as a `u32`, then one 8-byte record per binding, and
replies with the payload (`src/server/handlers/probe_config.rs:40`). A record is
`kind, interface, endpoint, interval, max_packet(le16), pad` (`src/descriptors/wire.rs:20`). The walk
is covered on the [enumeration](enumeration.md) page.
...
status (`src/server/handlers/feed_key.rs:22`, `src/server/handlers/feed_key.rs:28`).
`OP_FEED_MOUSE_REPORT` requires 3 to 4 bytes and runs `Mouse::feed` the same way, bumping
`mouse_reports` (`src/server/handlers/feed_mouse.rs:22`, `src/server/handlers/feed_mouse.rs:26`). The
...
(`src/server/handlers/poll_keys.rs:24`, `src/server/handlers/poll_keys.rs:30`). A key record is
`scancode, ascii, modifiers, pressed, then four zero bytes` (`src/hid/key_event.rs:26`).
`OP_POLL_MOUSE` is the mirror over the mouse queue (`src/server/handlers/poll_mouse.rs:24`); a mouse
record is `dx(le16), dy(le16), dz, buttons, flags, pad` (`src/hid/mouse_event.rs:27`). These queues
...
(`src/server/handlers/get_state.rs:23`). The post-failure counts are the diagnostic signal that a
report parsed but `mk_input_event_post` returned negative; a rising count points at a full ring or a
denied `InputSource` gate, covered on the [debugging](debugging.md) page.
