# Debugging capsule_driver_bga

This page lists the concrete failure modes of the BGA capsule and where to look for each. For how the
capsule is put together, read the [README](README.md), the [bring-up](bring-up.md), and the
[mode-set](mode-set.md) pages in this folder.

The capsule has no log markers of its own. It prints nothing, because it registers no service and calls no
debug syscall (`src/main.rs:33`; there is no debug call anywhere in `src/`). What it produces instead is a
...
(`src/discover.rs:51`, `src/setup/sequence.rs:28`, `src/error.rs:27`). Either the QEMU stdvga device is
...
(`src/discover.rs:41`). Confirm the device is enumerated at all before this capsule runs by checking the
broker device table ([device claim](../../subsystems/hardware-broker/claim.md)). The full filter is in the
...
(`src/error.rs:28`, `src/error.rs:25`). The failing call is one of claim, bus-master, or either MMIO map,
in that order (`src/setup/sequence.rs:29`). A claim rejection is usually `AlreadyClaimed` (another capsule
holds the display, for example the virtio-gpu driver) or a stale epoch
([device claim](../../subsystems/hardware-broker/claim.md)); a map rejection is one of the broker's own
ordered `MkMmioMap` checks, such as a stale epoch, a bad range, or the MSI-X clamp
([MMIO grants](../../subsystems/hardware-broker/mmio.md)). The raw negative rc is carried inside the error
value but not in the exit code, which is fixed at `3` (`src/error.rs:20`, `src/error.rs:25`), so trace the
setup call to see which rc it was.

## Panel comes up but at the wrong resolution

The mode is the compile-time constant 1024x768x32 (`src/constants.rs:33`). This capsule does not read EDID
and does not negotiate a mode, so it always programs 1024x768 over the DISPI linear-framebuffer path
(`src/dispi/set_mode.rs:24`). A display or emulated adapter that does not accept that mode shows whatever
the firmware left. Changing the mode today means changing the `MODE_WIDTH` and `MODE_HEIGHT` constants and
rebuilding; there is no runtime path (see the promotion note in [contributing](contributing.md)).
...
assumes the scanline stride equals `width * 4` (`src/dispi/clear.rs:19`, `src/setup/sequence.rs:38`,
`src/setup/sequence.rs:41`). If the adapter's real scanline stride differs from `width * 4`, the fill will
...
broker installs after every grant ([MMIO grants](../../subsystems/hardware-broker/mmio.md)). The capsule
does not read back a hardware stride register; it trusts the packed 32-bit layout the DISPI mode-set
requested. This assumption is the honest gap in the mode-set, documented in [mode-set](mode-set.md).
...
(`src/main.rs:38`, `src/setup/driver.rs:19`). It has no request loop and serves no clients, so it does
nothing further by design. If the panel goes blank, suspect that the `Driver` was dropped, since
`BrokerHandles::drop` unmaps both BARs and releases the device (`src/handles.rs:31`); on the normal
