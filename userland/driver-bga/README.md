...
(`src/main.rs:33`). This folder documents exactly that, and marks every gap between the code and a
...
|------|---------|----------------|
| [bring-up.md](bring-up.md) | `src/setup/`, `src/discover.rs`, `src/handles.rs` | The one-shot bring-up sequence: PCI discovery and the match filter, the exclusive claim and epoch, the single Bus Master Enable write, mapping the register and framebuffer BARs, and RAII teardown. |
| [mode-set.md](mode-set.md) | `src/dispi/`, `src/regs.rs`, `src/constants.rs` | The DISPI mode-set path: the index-addressed register model, the disable/xres/yres/bpp/enable sequence, the linear-framebuffer bit, and the solid-colour framebuffer clear. |
| [contributing.md](contributing.md) | the whole tree | How the capsule would be promoted to a real display driver: adding a `Capsule.mk`, a service, and the generated make targets, plus the code standards a change must keep. |
| [debugging.md](debugging.md) | runtime | The failure modes: no device match, a broker call rejected, wrong resolution, and a garbled fill from the stride assumption. |
...
The [bring-up](bring-up.md) page walks the sequence step by step.
...
absent today. The [mode-set](mode-set.md) page documents the register writes that program that mode.
...
(`src/error.rs:17`): `DeviceNotFound` exits `2` (`error.rs:27`) and `BrokerCallFailed(rc)` exits `3`
...
A promoted capsule would be spawned through [verified spawn](../../security/capsules-and-trust.md): its
...
it; the only way to run it today is a direct build of the crate (see [contributing](contributing.md)).
...
ever released (`src/main.rs:33`, `src/handles.rs:31`).
