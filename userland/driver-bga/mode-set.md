# Mode-set: DISPI registers and the framebuffer clear
...
`src/regs.rs`, and the mode constants in `src/constants.rs`. For how the two BARs it writes here were
obtained, read [bring-up](bring-up.md).

Both steps run inside `setup::run` right after the two BARs are mapped: `set_mode` against the register
window, then `clear` against the framebuffer window (`src/setup/sequence.rs:37`, `src/setup/sequence.rs:39`).
...
`read_volatile`/`write_volatile` at `base + offset` (`src/regs.rs:24`, `src/regs.rs:29`,
`src/regs.rs:33`). The capsule never mints a PIO grant and never issues `in`/`out`.
...
registers sit at MMIO offset `0x500` within that BAR (`src/constants.rs:24`). `dispi_off` turns a DISPI
index into a byte offset as `0x500 + index * 2`, because the registers are 16-bit and packed two bytes
apart (`src/dispi/dispi_off.rs:19`).

| DISPI index | Constant | Meaning | Source |
|---|---|---|---|
| 1 | `DISPI_INDEX_XRES` | horizontal resolution | `src/constants.rs:25` |
| 2 | `DISPI_INDEX_YRES` | vertical resolution | `src/constants.rs:26` |
| 3 | `DISPI_INDEX_BPP` | bits per pixel | `src/constants.rs:27` |
| 4 | `DISPI_INDEX_ENABLE` | enable and mode flags | `src/constants.rs:28` |

## The mode-set sequence

`set_mode` writes five 16-bit registers in a fixed order (`src/dispi/set_mode.rs:24`). The order matters:
...
|---|---|---|---|
| disable | ENABLE (4) | `0` | `src/dispi/set_mode.rs:26` |
| x resolution | XRES (1) | `width` (1024) | `src/dispi/set_mode.rs:27` |
| y resolution | YRES (2) | `height` (768) | `src/dispi/set_mode.rs:28` |
| bit depth | BPP (3) | `32` | `src/dispi/set_mode.rs:29` |
| enable | ENABLE (4) | `DISPI_ENABLED \| DISPI_LFB_ENABLED` (`0x41`) | `src/dispi/set_mode.rs:30` |
...
flat, packed pixel array rather than the banked legacy VGA window (`src/constants.rs:30`,
`src/constants.rs:31`). The `width` and `height` arguments come from the `MODE_WIDTH` and `MODE_HEIGHT`
constants that `setup::run` passes in (`src/constants.rs:33`, `src/setup/sequence.rs:37`); they are not
...
many volatile `u32` stores from the framebuffer base (`src/setup/sequence.rs:38`, `src/dispi/clear.rs:19`).
The colour is the constant `CLEAR_COLOR` (`0x00102A3A`), a dark teal, so a successful bring-up shows a
solid panel rather than uninitialised memory (`src/constants.rs:36`).
...
(`src/setup/sequence.rs:41`). That assumption is correct for the linear-framebuffer mode the mode-set
requested, but it is an assumption the capsule does not verify by reading back a hardware stride register;
see the garbled-fill case in [debugging](debugging.md).
...
`u32`s (`src/dispi/set_mode.rs:24`, `src/dispi/dispi_off.rs:19`, `src/dispi/clear.rs:19`). This is
