# Contributing to capsule_driver_bga
...
the capsule does and how it is put together, read the [README](README.md), the [bring-up](bring-up.md),
and the [mode-set](mode-set.md) pages in this folder.
...
(`src/main.rs:33`). There is no app skeleton and no runtime, because the capsule serves no clients; the
...
|---|---|---|
| `src/setup/sequence.rs` | the ordered bring-up (discover, claim, bus-master, map x2, mode-set, clear) | you change the bring-up order or add a step |
| `src/setup/claim.rs` | `mk_device_claim` and the epoch | you change how the device is claimed |
| `src/setup/pci.rs` | the single Bus Master Enable write | you change the PCI config authority the capsule uses |
| `src/setup/mmio.rs` | mapping a BAR, releasing on failure | you change how a BAR is mapped |
| `src/setup/driver.rs` | `Driver`: the RAII-owning result | you add state the capsule keeps alive |
| `src/discover.rs` | the PCI match filter | you support a different display device or BAR layout |
| `src/dispi/` | the mode-set, the clear, the offset helper | you change the mode-set or the framebuffer fill |
| `src/regs.rs` | the volatile register accessors | you change how registers are read or written |
| `src/handles.rs` | RAII broker teardown | you change what is unmapped or released on drop |
| `src/error.rs` | the error type and exit codes | you add a failure case |
| `src/constants.rs` | every device id, register index, and mode constant | you change the device match, the mode, or the clear colour |
...
4. **Add the client protocol and mode negotiation.** The fixed 1024x768x32 mode is a compile-time constant
   (`src/constants.rs:33`), and the capsule serves no clients. A promoted display capsule would register a
...
- No panics in capsule code. No `unwrap`, `expect`, or `panic!`. `setup::run` returns a `BgaResult` and
  `_start` maps the error to an exit code (`src/main.rs:34`, `src/error.rs:25`); the release profile is
...
- One unit per file. Each bring-up step, each DISPI step, and each type lives in its own file, and `mod.rs`
  is used only for re-exports (`src/setup/mod.rs:23`, `src/dispi/mod.rs:21`).
