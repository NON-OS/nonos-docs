# Client operations and the NUDP protocol
...
`recv`, a payload. For the identity table and the capability mask see the [README](README.md); for the UDP
machinery the handlers reach, see the [datagram](datagram.md) page, and for the port table they touch, the
[state](state.md) page.

## The wire format

A request header is 20 bytes and begins with a magic and a version. The parser rejects anything shorter
than the header, a wrong magic, or a version other than 1 (`src/server/parse_req.rs:27`). The `NUDP` magic
is `0x4E55_4450` (`src/protocol/header.rs:17`).
...
(`src/server/parse_req.rs:40`). On success it returns the parsed `op` and `request_id` and a slice
...
(`src/server/respond.rs:24`). A `recv` reply carries a body after that header; every other op replies with
the header alone. Replies go back to the requesting pid with `mk_ipc_reply` (`src/server/respond.rs:58`).
...
(`src/server/runner.rs:31`). `IPC_PAYLOAD_MAX` is the 1472-byte UDP payload ceiling plus 64 bytes of
op framing slack (`src/protocol/limits.rs:18`).

The loop receives a request with `mk_ipc_recv_from`, which also reports the sender pid
(`src/server/runner.rs:38`). A receive of zero or fewer bytes, or a receive with a zero sender pid, is
...
The opcodes are defined in `src/protocol/ops.rs` and dispatched in `src/server/runner.rs:44`.
...
| `OP_BIND` | `2` | 2-byte local port (LE) | none (status only) | `server/handlers/bind.rs:23` |
| `OP_UNBIND` | `3` | 2-byte local port (LE) | none (status only) | `server/handlers/unbind.rs:23` |
| `OP_SEND` | `4` | 2 src_port + 4 dst IPv4 + 2 dst_port + payload | none (status only) | `server/handlers/send.rs:27` |
| `OP_RECV` | `5` | 2-byte local port (LE) | 4 src IPv4 + 2 src_port + payload | `server/handlers/recv/handle.rs:27` |

The opcode constants are `OP_HEALTHCHECK = 1`, `OP_BIND = 2`, `OP_UNBIND = 3`, `OP_SEND = 4`,
`OP_RECV = 5` (`src/protocol/ops.rs:17`).
...
- `OP_BIND` reads the 2-byte port and inserts a `BindEntry` owned by the sender pid into the port table. A
  ...
  `E_OK` (`src/server/handlers/bind.rs:23`). One owner per port is enforced by the table, described on the
  [state](state.md) page.
- `OP_UNBIND` removes the binding for the sender pid and port. A body under 2 bytes is `E_BAD_LEN`; a
  binding that is not there is `E_NO_PORT`; success is `E_OK` (`src/server/handlers/unbind.rs:23`). Only the
  owning pid can remove its own binding, because `remove` matches on both port and pid
  (`src/state/table.rs:51`).
- `OP_SEND` reads the 2-byte source port, the 4-byte destination IPv4, the 2-byte destination port, and the
  ...
  (`src/server/handlers/send.rs:27`). On success the handler builds the UDP segment and ships it to
  `net.ip`, both covered on the [datagram](datagram.md) page.
- `OP_RECV` reads the 2-byte local port, checks the sender owns it (`E_NO_PORT` otherwise), and dequeues one
  ...
  once; still empty is `E_RX_EMPTY` (`src/server/handlers/recv/handle.rs:27`). A delivered datagram is
  parsed and its source IPv4, source port, and payload are written after the response header
  (`src/server/handlers/recv/deliver.rs:26`).
...
a parse failure is dropped in the request loop rather than answered (`src/server/runner.rs:43`), so a
...
path is defensive (`src/server/respond.rs:22`, `runner.rs:31`).
...
attributable to a caller (`src/server/runner.rs:39`). Port ownership is keyed on the kernel-attested sender
...
(`src/server/handlers/send.rs:41`, `recv/handle.rs:33`, `unbind.rs:29`). A send payload is bounded to the
1472-byte MTU ceiling before it is framed (`src/server/handlers/send.rs:37`). There is no panic path: the
