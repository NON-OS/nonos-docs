# The port table and receive rings
...
[operations](operations.md) page; for the datagram machinery that fills the rings, see the
[datagram](datagram.md) page.

## The shared state object

All mutable state lives behind one process-global `STATE`, a `State` struct with three fields
(`src/state/global.rs:22`):

- `ip_service_port`, an `AtomicU32` holding the resolved `net.ip` service port, written once at setup and
  read on every send and drain (`src/state/global.rs:23`, read through `ip_port()` at `global.rs:37`).
- `local_ipv4`, a `Mutex<[u8; 4]>` holding the cached local IPv4 the checksum path needs
  (`src/state/global.rs:24`).
- `binds`, a `Mutex<BindTable>` holding every port binding and its receive ring (`src/state/global.rs:25`).

`STATE` is a `static` constructed with a `const fn new`, so it needs no runtime initialisation and no heap
allocation to exist (`src/state/global.rs:29`, `global.rs:42`). The atomic port uses `Release` on the store
at setup and `Acquire` on the load, so a reader that sees a non-zero port also sees the setup that produced
it (`src/state/global.rs:38`, `src/setup.rs:43`). The two mutexes are `spin::Mutex`, held only for the
short duration of a table lookup or a ring push and pop.

## The bind table

`BindTable` is a `Vec<BindEntry>` capped at `MAX_BINDS = 64` (`src/state/table.rs:21`, `table.rs:23`). It
...
- `insert` rejects a port that is already present with `TableError::InUse`, rejects a full table with
  `TableError::Full`, and otherwise pushes the new entry (`src/state/table.rs:40`). This is the
...
- `remove` finds the entry matching both the port and the owner pid, returning `TableError::NotFound` if
  there is none, and swap-removes it (`src/state/table.rs:51`). Because the match is on port and pid, one
...
- `find_owned_mut` returns the entry for a given pid and port, the check send and recv use to confirm the
  caller owns the port it names (`src/state/table.rs:36`).
- `find_by_port_mut` returns the entry for a port regardless of owner, used only by the inbound router to
  find where a received datagram belongs (`src/state/table.rs:32`).

`TableError` has three variants, `InUse`, `Full`, and `NotFound` (`src/state/table.rs:62`). The handlers
collapse them to the wire errno set: bind maps `InUse` and `Full` (and, defensively, `NotFound`) to
`E_PORT_IN_USE`, and unbind maps all three to `E_NO_PORT` (`src/server/handlers/bind.rs:30`,
`src/server/handlers/unbind.rs:29`).

## The bind entry and its receive ring

A `BindEntry` records the owner pid, the port, and a receive ring (`src/state/bind.rs:23`). The ring is a
`VecDeque<UdpInbound>` bounded to `RX_RING_DEPTH = 32` entries, pre-sized to that depth at construction
(`src/state/bind.rs:21`, `bind.rs:31`). It is deliberately small and bounded: a bind that stops draining
...
- `push` appends a received segment unless the ring is already at depth, in which case it returns `false`
  and the segment is dropped (`src/state/bind.rs:34`). This is where a full ring drops the tail datagram
  rather than blocking or growing.
- `pop` takes the oldest segment off the front, so delivery is first-in first-out (`src/state/bind.rs:42`).

The inbound router pushes into the ring keyed by the parsed UDP destination port, and a full ring silently
drops the segment (`src/server/handlers/recv/drain.rs:46`). A `recv` op pops from the ring keyed by the
caller's pid and port, so a caller only ever dequeues from a bind it owns
(`src/server/handlers/recv/handle.rs:49`).
