# The UDP Transport Capsule

`capsule_net_udp` is the NONOS UDP transport: a signed ring-3 capsule that owns UDP datagram send and
...
| Service handle | `net.udp` | `Capsule.mk:6`, `src/userspace/capsule_net_udp/spawn.rs:35` |
...
| Kernel mirror | `src/userspace/capsule_net_udp` | `src/userspace/capsule_net_udp/spawn.rs` |
...
|------|---------|----------------|
| [operations.md](operations.md) | `src/protocol/`, `src/server/` | The `NUDP` wire format, the request loop, the five client ops, per-op payloads, the errno set, and the reply path. |
| [datagram.md](datagram.md) | `src/udp/`, `src/ip_client/`, `src/setup.rs` | The UDP header parse and build, the RFC 768 pseudo-header checksum, the `net.ip` client (config, send, poll), and bring-up. |
| [state.md](state.md) | `src/state/` | The port bind table, the one-owner-per-port rule, the per-bind receive ring, and the cached IP link state. |
...
| [debugging.md](debugging.md) | runtime | The boot marker, the setup retry loop, and the runtime errno failure modes: no port, port in use, no IP link, and empty RX. |

For where UDP sits in the wider stack, see the [networking subsystem](../../subsystems/networking/README.md).

## Lifecycle

The capsule is `no_std`/`no_main`. `_start` initialises the heap, then loops calling `setup::run` until it
succeeds, and only then enters the request server, which loops forever (`src/main.rs:32`). `setup::run`
resolves `net.ip` through the service registry and caches the IP service port and the local IPv4 address so
the data path does not pay that round-trip on every datagram (`src/setup.rs:36`); a failed setup yields and
retries (`src/main.rs:40`). The kernel spawns it through
[verified spawn](../../security/capsules-and-trust.md) under the network spawn plan, checking its signature
and attestation and holding its requested capabilities against its manifest ceiling before its ELF is
mapped (`src/userspace/init/spawn_plan/network/spawn_udp.rs:21`). A successful spawn prints
`[NET-UDP] capsule spawned` on the boot log; the [debugging](debugging.md) page covers what that marker
means.
