...
the [operations](operations.md) page, the [datagram](datagram.md) page, and the [state](state.md) page.
...
(`src/userspace/init/spawn_plan/network/spawn_udp.rs:21`), whose `Ok` arm calls `boot_log::ok(prefix,
"capsule spawned")` (`src/userspace/init/capsule_boot/run.rs:29`), which prints `[` + tag + `] ` + message
...
(`src/userspace/init/capsule_boot/run.rs:32`). The line only appears at all when the kernel is built with
the `nonos-capsule-net-udp` feature, the `cfg` gate on the spawn entry
(`src/userspace/init/spawn_plan/network/spawn_udp.rs:18`).
...
yielding 64 times between attempts and retrying forever until it succeeds (`src/main.rs:40`). So a capsule
that spawned but never answers a request is almost always stuck in that loop because `net.ip` has not come
up. `setup::run` fails for one of two reasons (`src/setup.rs:36`):

- `mk_service_lookup("net.ip")` returned non-zero, so the IP capsule is not registered yet
  (`SetupError::IpNotFound`, `src/setup.rs:40`). The fix is upstream: `net.ip` has to be spawned and
...
- The lookup succeeded but `read_ipv4` failed, so `net.ip` answered but the config read did not return a
  usable IPv4 (`SetupError::IpConfigFailed`, `src/setup.rs:49`). That happens when the IP layer has no
...
the 16-bit value at offset 8 of the reply (`src/server/respond.rs:37`).
...
(`src/server/runner.rs:43`), and a receive with a zero sender pid is skipped (`runner.rs:39`). If a client
...
The opcode was not one of the five the dispatch handles (`src/server/runner.rs:50`). Only `1` through `5`
...
at least 2 bytes for the port (`src/server/handlers/bind.rs:24`, `unbind.rs:24`, `recv/handle.rs:28`); send
needs at least 8 bytes for the src port, dst IPv4, and dst port (`src/server/handlers/send.rs:28`) and
rejects a payload over `UDP_PAYLOAD_MAX` (1472) (`send.rs:37`, `src/protocol/limits.rs:18`). A send can also
return `E_BAD_LEN` if the UDP `build` step reports the output buffer was too small or the segment too large
(`src/server/handlers/send.rs:55`, `src/udp/build.rs:38`).
...
(`src/server/handlers/send.rs:41`, `recv/handle.rs:33`, `src/state/table.rs:36`). Unbind returns the same
errno when there is no binding for that pid and port to remove (`src/server/handlers/unbind.rs:29`). The fix
...
(`src/state/table.rs:40`, `src/server/handlers/bind.rs:30`). This is the one-owner-per-port rule: a port
...
(`src/server/handlers/send.rs:46`), or `send_segment` reported a transport failure, a malformed reply, or a
non-zero IP errno (`send.rs:59`). It also surfaces on the recv path if a stored segment fails to reparse on
delivery (`src/server/handlers/recv/deliver.rs:30`). The first case points back at setup and `net.ip`; the
...
checks again; if both are empty it returns `E_RX_EMPTY` (`src/server/handlers/recv/handle.rs:46`). This is
...
avoid (`src/server/handlers/recv/deliver.rs:35`). Note the receive ring is 32 deep and a full ring drops
new arrivals (`src/state/bind.rs:34`), so a client that binds a busy port but polls slowly can miss
