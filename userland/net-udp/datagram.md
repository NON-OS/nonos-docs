# The datagram engine and the net.ip client

This page mirrors `src/udp/` (the UDP header parse and build and the RFC 768 checksum), `src/ip_client/`
(the IPC client that talks to the IP capsule), and `src/setup.rs` (bring-up). It is the machinery a
`send` or `recv` op reaches after the server has parsed and dispatched a request. For that request loop and
the op set see the [operations](operations.md) page; for the port table the receive path fills, see the
[state](state.md) page. For where UDP sits above IP in the stack, see the
...
source and destination ports (`src/udp/header.rs:17`). The length and checksum fields live at fixed offsets
in the wire buffer; the checksum sits at offset 6 (`src/udp/header.rs:18`). Everything else about the header
...
(`src/udp/build.rs:38`). The total length is the 8-byte header plus the payload; a total over `u16::MAX` is
`PayloadTooLarge`, and an output buffer too small for the total is `OutputTooSmall`
(`src/udp/build.rs:38`). The source and destination ports and the total length are written big-endian (the
wire byte order), the checksum field is zeroed, the payload is copied in, and then the checksum is computed
over the finished segment and written back into the zeroed field (`src/udp/build.rs:46`). The caller
supplies the source and destination IPv4 addresses in the `BuildRequest`, because the checksum is sealed
over an IPv4 pseudo-header that includes them (`src/udp/build.rs:20`).
...
returns the parsed header plus the payload slice (`src/udp/parse.rs:31`). A segment shorter than the 8-byte
...
parser read past the buffer (`src/udp/parse.rs:43`). If the wire checksum is non-zero, it is recomputed over
the pseudo-header and the segment, and a mismatch is `BadChecksum`; a wire checksum of zero is accepted
without verification, which RFC 768 reserves as "no checksum" (`src/udp/parse.rs:46`). On success the
payload slice runs from the end of the header to the length the header declared, so trailing bytes past the
declared length are trimmed (`src/udp/parse.rs:52`).

## The checksum

`compute` is the RFC 768 and RFC 1071 checksum: fold 16-bit words over the IPv4 pseudo-header, then the UDP
header and payload (`src/udp/checksum.rs:39`). The pseudo-header is the source and destination IPv4, a zero
byte and the protocol number 17, and the UDP length (`src/udp/checksum.rs:22`, `checksum.rs:43`). Bytes are
summed as big-endian 16-bit words, with a trailing odd byte padded with a zero low byte
(`src/udp/checksum.rs:28`). The 32-bit running sum is folded down to 16 bits by adding the carries back in
until none remain, then complemented (`src/udp/checksum.rs:46`). A computed zero is returned as `0xFFFF`,
which is the RFC rule that lets a zero wire value mean "no checksum" without ambiguity
(`src/udp/checksum.rs:49`). The pseudo-header bytes are summed field by field on the stack, never assembled
...
service uses, distinguished by the magic `NIP4` = `0x4E49_5034` and version 1 (`src/ip_client/wire.rs:21`).
Three IP opcodes matter to UDP: `OP_GET_CONFIG = 2`, `OP_SEND_PACKET = 4`, and `OP_POLL_PACKET = 5`
(`src/ip_client/wire.rs:25`), and the IP protocol byte for UDP is 17 (`src/ip_client/wire.rs:29`).
...
a wrong magic or version by returning `None` (`src/ip_client/header.rs:19`, `header.rs:33`). Request ids
come from a monotonic sequence counter that starts at 1 and skips zero on wraparound, so a reply can be
matched and a zero id is never minted (`src/ip_client/seq.rs:21`).
...
- `read_ipv4` issues `OP_GET_CONFIG` and pulls the interface config back, extracting the 4-byte local IPv4
  ...
  (`src/ip_client/config.rs:35`). A negative transport result is `SendFailed`; a malformed or wrong-op
  response is `BadResponse`; a non-zero IP errno is `Refused` (`src/ip_client/config.rs:41`). UDP needs the
...
- `send_segment` issues `OP_SEND_PACKET` with a body of the 4-byte destination IPv4, the protocol byte 17,
  ...
  (`src/ip_client/send.rs:34`). The IP capsule owns the routing and the actual transmit.
- `poll_segment` issues `OP_POLL_PACKET` with the protocol byte 17 as its body and reads back one queued
  ...
  (`src/ip_client/recv/poll.rs:28`). It maps the IP errno set to a typed error: 0 is a delivered segment, 8
  is `NoConfig`, 10 is `Empty`, and anything else is `Other` (`src/ip_client/recv/poll.rs:50`). It bounds
  ...
  (`src/ip_client/recv/types.rs:21`), capped at a 1500-byte packet (`recv/types.rs:19`).
...
build error or an IP failure becomes `E_BAD_LEN` or `E_NO_IP_LINK` (`src/server/handlers/send.rs:50`). On
...
recv tick retries (`src/server/handlers/recv/drain.rs:24`). Delivery then reparses the stored segment and
writes the source IPv4, source port, and payload into the reply (`src/server/handlers/recv/deliver.rs:26`).
...
does not repeat the round-trip (`src/setup.rs:36`). A failed lookup is `IpNotFound`; a failed config read is
`IpConfigFailed` (`src/setup.rs:40`). `_start` calls setup in a retry loop, yielding between attempts until
it succeeds, so the capsule waits for `net.ip` to come up rather than failing outright
(`src/main.rs:40`). Only after setup succeeds does the server loop start.
