# Contributing to capsule_net_udp
...
[README](README.md), the [operations](operations.md) page, the [datagram](datagram.md) page, and the
[state](state.md) page.
...
(`src/main.rs:32`). The top-level modules are declared there: `ip_client`, `protocol`, `server`, `setup`,
`state`, and `udp` (`src/main.rs:22`).
...
| `src/setup.rs` | resolving `net.ip` and caching the port and local IPv4 | you change bring-up |
...
2. Write the handler as one file under `src/server/handlers/`, exposing a `handle` function that reads the
   body, mutates state through `STATE` if needed, and replies with `respond` (`src/server/respond.rs:44`).
   Follow `bind.rs` (a state mutation with a length-checked body) or `send.rs` (an op that reaches
   `net.ip`). Declare the module in `src/server/handlers/mod.rs:17`. A handler that grows past one concern
   should be split into a folder with a `mod.rs` re-export, the way `recv/` splits into `handle`, `deliver`,
   and `drain` (`src/server/handlers/recv/mod.rs:17`).
3. Wire it into the dispatch match in `src/server/runner.rs:44`. Every op arm passes the sender pid through
   so the handler can attribute the request; an op that owns or acts on a port must check ownership with
   `find_owned_mut` before it acts (`src/state/table.rs:36`), the way send, recv, and unbind do.
...
`cfg` gate on the kernel-side embed (`src/userspace/capsule_net_udp/embed.rs:20`).
...
- One unit per file. New ops are one file per handler under `src/server/handlers/`, and a handler that grows
  ...
  and re-exports, as in `src/protocol/mod.rs:17` and `src/udp/mod.rs:17`.
- Port ownership is keyed on the kernel-attested sender pid. Any op that acts on a port must confirm the
  sender owns it before acting (`src/server/handlers/send.rs:41`), and any inbound path must bound a
  wire-supplied length before it reads a body (`src/ip_client/recv/poll.rs:44`, `src/udp/parse.rs:43`).
