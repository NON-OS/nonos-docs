# The Server Loop and Wipe Discipline
...
[operations.md](operations.md); for the handlers the router calls see [primitives.md](primitives.md); for
the identity and mask see the [README](README.md).

## The loop

`server::run` is the whole runtime and it never returns (`src/server/runner.rs:29`). It allocates one
...
3. Send the reply to the fixed kernel reply endpoint `KERNEL_REPLY_ENDPOINT` = `0x1_0000_0004`
   (`runner.rs:41`, `src/protocol/types.rs:62`). The send result is deliberately discarded; the loop does
...
`dispatch` is a single match from `req.op` to one handler (`src/server/dispatch.rs:29`). Every arm calls a
...
(`dispatch.rs:47`). The handlers are declared and re-exported one per file through `handlers/mod.rs:17`
...
compiler fence (`src/server/wipe.rs:19`). The volatile write and the fence together stop the compiler from
...
the reply (`src/server/handlers/x25519_shared.rs:35`). That is the only per-handler secret wipe; every
...
[transport](transport.md) security discussion.
...
processes (mask decomposed on the [README](README.md), `Capsule.mk:16`). Its positive isolation property
