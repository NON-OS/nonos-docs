# Contributing to capsule_crypto
...
the [README](README.md), the [operation reference](operations.md), the [server loop](server.md), the
[primitives](primitives.md), and the [kernel seam](transport.md) pages in this folder.

## Where the source lives

The capsule is at `userland/capsule_crypto/`. It is a `no_std`/`no_main` service: `_start` inits the heap
and enters `server::run` (`src/main.rs:28`). The two top-level modules are declared there
(`src/main.rs:22`).
...
1. Assign an opcode. If it fits the original block, add the constant to `src/protocol/types.rs:20`;
   otherwise add it to `src/protocol/primitives.rs:17` alongside the later ops, and put any size constant
   next to it. Re-export the new names through `src/protocol/mod.rs:26` or `:34`.
...
2. Write the handler as one file under `src/server/handlers/`, exposing
   ...
   (`src/server/handlers/sha256_hash.rs:23` is the reference), and for a verify op return a status with an
   empty body (`src/server/handlers/ed25519_verify.rs:69`). If the payload has an internal frame, parse it
   with checked `get` calls so a short buffer is an error, not a panic (`aead_frame/common.rs:20`).

3. Declare the module and re-export the handler in `src/server/handlers/mod.rs:17` and `:37`.

4. Wire the opcode into the match in `src/server/dispatch.rs:29`, and add the opcode to the
...
5. If a kernel-side caller should reach it, add a client under `src/security/crypto_capsule/client/`,
   re-export it in `client/mod.rs:37`, call `gate_hash()?` first
   (`src/security/crypto_capsule/capability.rs:22`), and add the syscall plumbing under the crypto syscall
...
`nonos-mk-boot-crypto-hash` runs `tests/boot/crypto_hash_round_trip.sh` (`Makefile:1391`).
...
- The AGPL header sits at the top of every source file, byte for byte the same as the header on
  `src/server/dispatch.rs:1` and every other module.
