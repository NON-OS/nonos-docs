# Debugging capsule_crypto

This page lists the boot marker the crypto pool emits, the request-time status codes and what each maps
to, and where a denial actually comes from. For the shape of the service see the [README](README.md), the
[operation reference](operations.md), the [server loop](server.md), the [primitives](primitives.md), and
the [kernel seam](transport.md) pages in this folder.
...
`boot_log::ok(prefix, "capsule spawned")` (`src/userspace/init/capsule_boot/run.rs:29`,
`src/sys/boot_log/output.rs:33`). If that line is absent the capsule never started, and the `Err` arm
logged an `[ERROR]` line through `boot_log::error` instead (`src/userspace/init/capsule_boot/run.rs:32`),
which is the usual signature, manifest, or capability failure. A present marker means `crypto_pool`
resolves on port 4102, so callers such as the [market](../market/README.md) can reach it.

The capsule itself is silent after that: it holds no `Debug` capability, so it never logs a request or a
plaintext (mask on the [README](README.md), `Capsule.mk:16`). All request-time diagnosis is by the status
code in the reply.

## Request-time status codes

Because the pool is stateless, its failure signatures map cleanly to cause (`src/protocol/errno.rs:17`):
...
- `EINVAL` (-22) is a malformed frame: a short AEAD header, an over-large `aad_len`, a malformed Ed25519
  ...
  mismatch, or a degenerate all-zero AES-GCM nonce (`aead_frame/common.rs:30`, `ed25519_verify.rs:49`,
...
to `Verdict::Refused` (`userland/capsule_market/src/verify/crypto.rs:31`). This is not a capsule bug; it is
...
(`src/security/crypto_capsule/capability.rs:28`). The fix is a capability grant on the caller, not a change
to the capsule. See the [kernel seam](transport.md) page for the gate.
...
(`src/security/crypto_capsule/client/mod.rs:37`). A caller cannot reach them through a shim today, so the
symptom is not a wire `EINVAL` but the absence of any `nonos_libc` entry point. Wiring one is the fifth
step in [contributing.md](contributing.md).
...
(`src/userspace/init/capsule_boot/run.rs:32`). The boot spawn entry for CRYPTO is `spawn_plan/core.rs:60`.
