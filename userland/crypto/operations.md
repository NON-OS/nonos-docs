# The Crypto Wire and Operation Reference
...
[server.md](server.md); for the crate behind each op see [primitives.md](primitives.md); for the identity
and mask see the [README](README.md).

## The NOCX frame

Every message, request and reply, opens with the same 20-byte header (`src/protocol/types.rs:66`), the
same shape as [capsule_entropy](../entropy/README.md):
...
0u16 || request_id || (4 + body.len()) || status || body` (`src/protocol/encode.rs:21`). Status `0` is
success. The reply carries back the request's own `op`, `flags`, and `request_id` so a caller can match a
reply to its call.

The decoder validates before any handler runs (`src/protocol/decode.rs:27`). It rejects, in order, a
...
with op 0 (`src/server/runner.rs:39`). On success it hands the handler a `Request { op, flags,
request_id, payload }` borrowing the payload slice (`types.rs:68`, `decode.rs:50`).

## Status codes

Four codes, all negative, defined in `src/protocol/errno.rs:17`:
...
constants are in `src/protocol/types.rs:20`; opcodes 14 through 20 were folded in later and live in
`src/protocol/primitives.rs:17`. The complete set is seventeen operations, all routed by one match in
`src/server/dispatch.rs:29`. An unknown op is `EINVAL` (`dispatch.rs:47`). Opcodes 7, 8, and 9 are
...
`primitives.rs:17`. All seventeen are re-exported through `protocol/mod.rs:26` and `:34`.
...
of associated data, then the body (`aead_frame/common.rs:20`, `aead_frame/constants.rs:24`). For a seal
the body is the plaintext, bounded by `MAX_PT` (`aead_frame/parse.rs:23`); for an open the body is
`ciphertext || tag`, which must be at least `TAG_LEN` and at most `MAX_PT + TAG_LEN`
(`aead_frame/parse.rs:39`).
...
bytes (`hkdf_sha256.rs:29`, `:32`). See [primitives.md](primitives.md) for the extract/expand
construction.
