...
| Reply inbox (kernel client) | `endpoint.4294967300` = `0x1_0000_0004` | `client/transport.rs:28`, `src/protocol/types.rs:62` |
...
the capsule ever sees the bytes (`src/security/crypto_capsule/capability.rs:22`); the comment in
...
The source under `userland/capsule_crypto/src/` is two top-level modules (`src/main.rs:22`), and the
...
|------|---------|----------------|
| [operations.md](operations.md) | `src/protocol/` | The NOCX wire frame, the decoder and encoder, the four status codes, and the complete seventeen-op reference with every opcode, request layout, reply body, and per-op size limit. |
| [server.md](server.md) | `src/server/` | The receive/dispatch/send/wipe loop, the one-match op router, and the volatile buffer-wipe discipline that makes the pool stateless. |
| [primitives.md](primitives.md) | `src/server/handlers/` | One file per primitive: which crate backs each op, the shared AEAD frame parse and degenerate-nonce guard, and the hand-written HMAC and HKDF. |
| [transport.md](transport.md) | `src/security/crypto_capsule/` | The kernel-side mirror: verified spawn, the per-request `CAP_CRYPTO` gate on the caller pid, the fifteen in-tree clients, and the real callers in the tree. |
| [contributing.md](contributing.md) | the whole tree | Where to work, how to add a primitive end to end, the build and sign targets, and the code standards. |
| [debugging.md](debugging.md) | runtime | The boot marker, the request-time status codes and what each maps to, and where a denial actually comes from. |

## Lifecycle

The capsule is `no_std`/`no_main`. `_start` initializes the heap and, on success, enters `server::run`; a
heap-init failure exits with code 1 (`src/main.rs:28`).

1. The kernel spawns the capsule at boot through `spawn_plan/core.rs:60`, which calls
...
3. `run` allocates one receive buffer of `HDR_LEN + MAX_PAYLOAD_BYTES` and loops: receive, decode,
   ...
   (`src/server/runner.rs:29`). Nothing survives a request, so a request never depends on a prior one and
...
[crypto subsystem](../../subsystems/crypto/README.md)) are distinct bodies of code, and the client that
gates and transports requests is a third. The [transport](transport.md) page covers that seam.
