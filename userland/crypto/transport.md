# The Kernel Seam: Spawn, Gate, and Clients
...
`crypto_pool`. For the capsule the request reaches see the [README](README.md); for the frame on the wire
...
`spawn_crypto_capsule` runs at boot (`src/security/crypto_capsule/spawn.rs:40`). It decodes the baked
...
Every op the kernel client exposes runs `gate_hash` first (`src/security/crypto_capsule/capability.rs:22`).
...
|-------------|----------------|-----------|
| `client/hash_op.rs` | BLAKE3, SHA-256, SHA-512, SHA3-256 | `hash_op.rs:33` |
| `client/aead_op/seal.rs` | ChaCha20 seal, AES-GCM seal | `aead_op/seal.rs:34` |
| `client/aead_op/open.rs` | ChaCha20 open, AES-GCM open | `aead_op/open.rs:34` |
| `client/prf_op.rs` | HMAC, HKDF, X25519 public, X25519 shared | `prf_op.rs:37` |
| `client/verify_ed25519.rs` | Ed25519 verify | `verify_ed25519.rs:48` |
| `client/healthcheck.rs` | healthcheck | `healthcheck.rs:24` |

## The transport and reply inbox

The client sends over the lifecycle transport and receives on a fixed reply inbox
`endpoint.4294967300`, which is `0x1_0000_0004` in hex (`src/security/crypto_capsule/client/transport.rs:28`).
...
(`transport.rs:25`, `src/protocol/types.rs:60`). A `TRANSPORT_LOCK` serializes the send-then-receive so a
reply is matched to its request (`transport.rs:31`).

## Which ops have a kernel client

The kernel client exposes fifteen of the seventeen capsule ops (`src/security/crypto_capsule/client/mod.rs:37`):
...
Callers reach the pool through the `nonos_libc` crypto shims (`userland/libc/src/crypto/mod.rs`), never a
direct crypto dependency:

- The [market](../market/README.md) verifies its signed catalog index through `crypto_ed25519_verify`
  rather than linking Ed25519 itself; a good index returns `rc == 0` and `Verdict::Accepted`
  (`userland/capsule_market/src/verify/crypto.rs:31`).
...
every reply (see [server.md](server.md)) and the reply endpoint is distinct so replies cannot cross-route.
...
- P-256, P-384, and SHA-384 are implemented in the capsule but have no in-tree kernel client
  (`client/mod.rs:37`), so no gated caller reaches them yet.
- The capsule mixes RustCrypto crates with hand-written HMAC and HKDF (see [primitives.md](primitives.md))
  rather than being a single implementation, and it is a separate body of code from the kernel's in-tree
  [crypto stack](../../subsystems/crypto/README.md).
