# The Primitives and Their Backing Crates
...
For the opcodes and payload layouts see [operations.md](operations.md); for the loop that calls these
handlers see [server.md](server.md); for the identity and mask see the [README](README.md).
...
(`handlers/sha256_hash.rs:23` is the reference; SHA-384, SHA-512, SHA3-256, and BLAKE3 follow it). The
...
(`aead_frame/common.rs:20`). `parse_seal` bounds the body by `MAX_PT` (`aead_frame/parse.rs:21`);
`parse_open` requires the body to be at least `TAG_LEN` and at most `MAX_PT + TAG_LEN`
(`aead_frame/parse.rs:37`). `nonce_is_degenerate` ORs every nonce byte and returns true only for an
all-zero nonce (`aead_frame/parse.rs:29`).
...
truncates to `out_len` (`hkdf_sha256.rs:55`). The parse (documented in [operations.md](operations.md))
