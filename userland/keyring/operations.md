...
and its owner checks read [store.md](store.md); for the signers and their memory discipline read
[signing.md](signing.md). For the identity and capability mask read the [README](README.md).
...
(`decode_request`, `src/protocol/decode.rs:19`; `HDR_LEN = 8`, `src/protocol/types.rs:34`).
...
`src/protocol/encode.rs:21`). Status `0` is success; a negative status is one of the errno values below.
...
(`decode.rs:20`, `src/server/runner.rs:40`).
...
|--------|-------|---------|--------|
| `ENOENT` | -2 | no key with that id | `src/protocol/errno.rs:17` |
...
`InvalidArgument -> EINVAL` (for example `src/server/handlers/retrieve.rs:35`).

## Dispatch

`dispatch` matches the opcode to exactly one handler (`src/server/dispatch.rs:28`). An opcode outside
1..=14 replies `EINVAL` and touches nothing (`dispatch.rs:43`). The fourteen opcodes are constants in
`src/protocol/types.rs:17`.
...
validates the key type through `KeyType::from_u8` (`store/types/key_type_from_u8.rs:19`, values 0..=8),
...
the key. The generation, validation, and wiping of these secrets is covered in [signing.md](signing.md).
...
and hash the message, sign, and zero the secret; [signing.md](signing.md) walks each one.

### Wallet rail listing

| Op | Code | Request payload | Reply body | Handler |
|----|------|-----------------|------------|---------|
| `LIST_WALLET_RAILS` | 14 | none | `count(4) \|\| rail records` | `handlers/list_wallet_rails.rs:22` |

`LIST_WALLET_RAILS` reads no pid and touches no key. It serializes a static table of four rails
(`handlers/list_wallet_rails.rs:23`, encoder at `src/server/wallet_rail/encode.rs:21`); the table itself is
described in [signing.md](signing.md).

## The server loop

`run` owns a single `Store` and a 4096-byte receive buffer and loops forever (`src/server/runner.rs:28`):
...
(`runner.rs:46`), each through the volatile `wipe` with a compiler fence (`src/server/wipe.rs:19`). This
...
- [login](../login/README.md) toggles the lock. Its client sends `OP_LOCK = 4` and `OP_UNLOCK = 5` with a
  `caller_pid(4) || key_id(4)` payload and reads the 4-byte status
  (`userland/capsule_login/src/clients/keyring.rs:8`). Login unlocks the wallet key on a successful sign-in
  and locks it again when the session ends, which is how the lock flag becomes an authenticated-session
  gate on signing.
- The [wallet](../wallet-nonos/README.md) drives the wallet ops: `OP_WALLET_GENERATE = 9`,
  `OP_WALLET_ADDRESS = 10`, `OP_SIGN_NOX_APPROVE = 12`, `OP_SIGN_ETH_TRANSFER = 13`, and
  `OP_LIST_WALLET_RAILS = 14` (`userland/capsule_wallet_nonos/src/wallet/ipc/constants.rs:19`), through a
  shared `keyring_call` helper that frames the request and checks the status
  (`.../wallet/ipc/call.rs:23`). The wallet never sees a secret.
- The [payment](../payment/README.md) capsule calls `OP_SIGN_NOX_RECEIPT = 11` to settle a paid request
  (`KEYRING_OP_SIGN_RECEIPT`, `userland/capsule_payment/src/server/consts.rs:21`), passing the owner pid
  and wallet key id (`.../capsule_payment/src/server/handlers/pay.rs:39`).
