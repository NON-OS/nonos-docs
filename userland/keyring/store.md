# The key store and the wiping discipline
...
these methods read [operations.md](operations.md); for the signers that read a secret out of the store read
[signing.md](signing.md).

## The model

The store is a `BTreeMap<u32, KeyEntry>` keyed by an auto-incrementing id, with a `next_id`
(`src/store/types/store.rs:20`). It is constructed empty with `next_id = 1` (`src/store/state.rs:22`). A
`KeyEntry` carries the material and its metadata (`src/store/types/key_entry.rs:20`):
...
(`src/store/types/key_type.rs:18`), with the round-trip through `from_u8` (`key_type_from_u8.rs:19`) and
`to_u8` (`key_type_to_u8.rs:19`). The store is capped at 128 keys and each key at 256 bytes
(`src/store/types/constants.rs:16`), so it cannot grow without bound. Store methods return a `StoreError`
of `NotFound`, `AccessDenied`, `Locked`, `Full`, or `InvalidArgument`
(`src/store/types/store_error.rs:17`), which the handlers map onto the wire errno set.
...
caller (`AccessDenied` on mismatch), then act. It appears in `retrieve` (`src/store/retrieve.rs:24`),
...
up a single id (`count.rs:20`). Combined with the [caller attestation](operations.md#caller-attestation)
...
`store` is the entry point (`src/store/store_key.rs:20`). It rejects an empty or over-256-byte body with
...
(`src/store/retrieve.rs:22`). After the owner check it does two more things before returning any bytes:
...
through `eth_secret` (`src/store/eth_secret.rs:20`), a separate method that re-checks the owner pid, then
...
signature is out; the caller never sees them. [signing.md](signing.md) covers that path.
...
(`src/store/wipe.rs:19`). The `Drop` on `KeyEntry` calls it, so when a key is deleted or the store is torn
down the raw bytes are erased from the heap rather than left in a freed allocation
(`src/store/types/key_entry.rs:31`). `delete` wipes explicitly as well: it verifies the owner, removes the
entry, and volatile-wipes the removed key's `data` before returning (`src/store/delete.rs:29`).
...
(`src/server/zeroize.rs:17`) and the receive buffer after every reply (`src/server/wipe.rs:19`); they are
detailed in [signing.md](signing.md) and [operations.md](operations.md).
