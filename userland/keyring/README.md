...
No other capsule, including the [wallet](../wallet-nonos/README.md), the [login](../login/README.md)
service, and the [payment](../payment/README.md) service, ever holds a private key. They reach the keyring
...
`src/security/keyring_capsule/spawn.rs:55`), but `requested_caps` is only the upper bound the spawn site is
willing to grant for optional caps; the caps installed on the process come from the verified manifest, not
from `requested_caps` (`src/kernel_core/process_spawn/capsule_spawn/runner/verified.rs:23`). The manifest
check returns `required_caps | (optional_caps & granted_caps)`
(`src/security/capsule_manifest/verify/caps.rs:39`), so the installed mask is at least the manifest's
required set, `0x39`.
...
program a DMA engine that could carry a key off the machine. The [store](store.md) page returns to what
each absence buys.

## The three pillars

The source under `userland/capsule_keyring/src/` is three top-level modules, declared in `src/main.rs:22`,
...
|------|---------|----------------|
| [operations.md](operations.md) | `src/protocol/` and `src/server/` dispatch | The wire frame, the fourteen operations and their payloads, the errno set, caller attestation, the server loop, and the receive-buffer wipe. |
| [store.md](store.md) | `src/store/` | The key store, the `KeyEntry` model, the owner-pid checks on every operation, wallet non-exportability at the store boundary, and the wiping discipline. |
| [signing.md](signing.md) | `src/server/` signing subtree | The three signers, address derivation, the EIP-1559 and EIP-712 builders, the RLP encoder, the scratch zeroing on every branch, and the static wallet rail table. |
...
| [debugging.md](debugging.md) | runtime | The boot marker, reading failures from the reply status, and why the keyring emits no diagnostics of its own. |

## The fourteen operations

Every operation is a `u16` opcode dispatched in `dispatch` (`src/server/dispatch.rs:28`); the constants are
in `src/protocol/types.rs:17`. The [operations](operations.md) page gives each payload and reply body.
...
`src/userspace/init/spawn_plan/core.rs:23`), through `spawn_keyring`, which calls
...
(`src/userspace/init/capsule_boot/run.rs:29`, `:32`). Once mapped, `_start` initializes the heap and enters
`server::run`, which never returns (`src/main.rs:29`); the loop receives on inbox 0, dispatches, replies,
and wipes the buffer. The [debugging](debugging.md) page covers what the marker and each reply status mean.
