...
capsule, including the [wallet](../wallet-nonos/README.md), ever holds a private key; the wallet and the
[payment](../payment/README.md) service reach the keyring only over IPC. The source is
...
signing, and zeroing the secret the instant the signature is produced. The lock flag is what [login](../login/README.md)
toggles to gate signing behind an authenticated session, and the receipt signer is what
[payment](../payment/README.md) calls to settle a paid request.
...
[payment](../payment/README.md) capsule calls (`KEYRING_OP_SIGN_RECEIPT = 11`,
...
- [login](../login/README.md) toggles the lock. Its keyring client sends `OP_LOCK = 4` and `OP_UNLOCK = 5` with a
...
- The [wallet](../wallet-nonos/README.md) drives the wallet ops: `OP_WALLET_GENERATE = 9`, `OP_WALLET_ADDRESS = 10`,
