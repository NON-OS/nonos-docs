# Contributing to capsule_keyring
...
[README](README.md), the [operations](operations.md), the [store](store.md), and the [signing](signing.md)
...
(`userland/capsule_keyring/src/main.rs:29`). The three top-level modules are declared there
(`src/main.rs:22`). There is no window, no filesystem, and no network; the capsule holds keys and answers
requests about them.
...
2. Write the handler as one file per op under `src/server/handlers/`, exposing
   ...
   response (`src/server/handlers/retrieve.rs:22` is the reference shape). Length-check the payload first
...
3. Re-export the handler from `src/server/handlers/mod.rs:32` and add a match arm in
   `src/server/dispatch.rs:28` that routes the new opcode to it.

4. If the op touches the store, add the owner-checked method under `src/store/`, one file per operation the
   way `store/lock.rs` and `store/eth_secret.rs` are split, and re-export it through `src/store/mod.rs:30`.
...
If the op handles a secret, wipe every transient copy with `zeroize32` (`src/server/zeroize.rs:17`) or a
volatile loop on every return path, including the error branches, matching the existing signers
(`src/server/handlers/sign_eth_transfer.rs:55`, `:60`). A wallet secret must never appear in a reply body.
...
`nonos-mk-boot-keyring` runs `tests/boot/keyring_round_trip.sh` (`Makefile:1385`) and is part of
