...
from the reply status, and where each error comes from. For the request model see the [README](README.md),
the [operations](operations.md), the [store](store.md), and the [signing](signing.md) pages in this folder.
...
(`src/userspace/init/capsule_boot/run.rs:29`, format in `src/sys/boot_log/output.rs:33`).
...
(`src/userspace/init/capsule_boot/run.rs:32`, `boot_log::error`, `src/sys/boot_log/output.rs:49`), which is
...
values (`src/protocol/errno.rs:17`).

- `EACCES` (-13). Either `resolve_caller` refused a payload pid that did not match the attested sender
  (`src/server/caller.rs:23`), or an owner-pid check refused a key that belongs to another capsule
  (for example `src/store/eth_secret.rs:23`). A wallet that can show an address but cannot sign is often
...
- `EBUSY` (-16). A `retrieve` or a sign against a locked key. The lock check sits in `retrieve`
  (`src/store/retrieve.rs:30`) and in `eth_secret` (`src/store/eth_secret.rs:28`), which every signer uses.
...
- `ENOENT` (-2). A key id that does not exist in the store (`src/store/retrieve.rs:23` returns `NotFound`).
- `EINVAL` (-22). A bad request or a failed crypto step. From `wallet_generate` it means rejection sampling
  ...
  (`src/server/handlers/wallet_generate.rs:43`). From a signer it means a bad request length or a failed
  keccak or sign step (`src/server/handlers/sign_eth_transfer.rs:56`, `:62`). From `store` it means an empty
  or over-256-byte body (`src/store/store_key.rs:28`). It is also the reply for any opcode outside 1..=14
  (`src/server/dispatch.rs:43`).
- `ENOSPC` (-28). The 128-key store is full (`src/store/store_key.rs:31`).
...
as `EACCES`, before it reads any bytes (`src/store/retrieve.rs:27`). There is no operation that exports a
wallet secret; use the signers instead. See [store.md](store.md) and [signing.md](signing.md).
...
the `[ERROR]` line from the boot path for the `SpawnError` (`src/userspace/init/capsule_boot/run.rs:32`); it
