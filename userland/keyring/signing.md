# The signing paths and wallet non-exportability
...
never returned. For the store method that hands a secret to these handlers read [store.md](store.md); for
the request framing read [operations.md](operations.md).
...
`eth_secret` re-checks `owner_pid` inside the store (`src/store/eth_secret.rs:22`). Step 4 is the memory
discipline: the 32-byte secret is zeroed the instant the signature is produced, and on every error branch
too, through `zeroize32`, a volatile write plus a compiler fence (`src/server/zeroize.rs:17`). No signer
returns the secret; they return signatures, hashes, and addresses.

## sign_eth_transfer (op 13)

This is the fullest illustration (`src/server/handlers/sign_eth_transfer.rs:25`). The request is exactly
...
field pulled by `field32`, `src/server/field32.rs:17`), builds the unsigned EIP-1559 payload, Keccak-256
...
(`src/server/handlers/sign_approve.rs:25`), over a 164-byte request (`4 + 4 + 32*5`, `sign_approve.rs:26`).
...
id `1` are fixed constants (`src/server/eip1559/consts.rs:17`). The calldata is a fixed 68 bytes: the
4-byte selector, the 20-byte settlement address right-aligned in a 32-byte word, and the 32-byte amount
(`src/server/eip1559/approve_data.rs:19`).

## sign_receipt (op 11)

`sign_receipt` is the EIP-712 payment path (`src/server/handlers/sign_receipt/sign_receipt.rs:26`), over a
...
the derived address (`src/server/handlers/sign_receipt/read_fields.rs:19`). `struct_hash` lays the type
hash and the eight encoded fields into a 288-byte preimage and Keccak-256 hashes it, with the two 20-byte
address fields right-aligned in their 32-byte words (`src/server/eip712/struct_hash.rs:20`). `receipt_digest`
prefixes `0x19 0x01`, the domain separator, and the struct hash into a 66-byte preimage and hashes that
(`src/server/eip712/digest.rs:19`). The receipt type hash and the domain separator are fixed constants
(`src/server/eip712/consts.rs:17`). This is the op the [payment](../payment/README.md) capsule calls to
settle a paid request.

## Address derivation

`wallet_address` (op 10) and the receipt signer both derive an Ethereum address from a secret without
exporting it. The shared derivation is `address_of` (`src/server/ethaddr.rs:17`): compute the uncompressed
...
(`src/server/handlers/wallet_address.rs:33`, wipe at `:40`).
...
(`src/server/handlers/wallet_generate.rs:37`). `eth_secret_valid` rejects the all-zero scalar and any value
at or above the curve order `n`, by a constant big-endian comparison against `N`
(`src/store/eth_valid.rs:17`). The valid secret is stored as `KeyType::Secp256k1Eth`, and the local scratch
...
wipes the receive buffer after every reply (see [operations.md](operations.md)).
...
maxFee, gas, to, value, data, and an empty access list) (`src/server/eip1559/fields.rs:23`);
`nox_approve_fields` and `eth_transfer_fields` specialize it (`fields.rs:46`, `:65`). `unsigned_*_payload`
prefixes the type byte `0x02` and RLP-encodes the field list (`src/server/eip1559/unsigned.rs:23`);
`signed_*_tx` appends `y_parity`, `r`, and `s` before the same encoding (`src/server/eip1559/signed.rs:23`).

The RLP primitives are one unit per file: `rlp_list` (`src/server/rlp/encode_list.rs:21`), `rlp_string`
...
(`src/server/handlers/list_wallet_rails.rs:22`). Each record is the symbol length, family, status, flags,
chain id, contract address, and symbol bytes (`src/server/wallet_rail/encode.rs:21`), read from the static
`WALLET_RAILS` array (`src/server/wallet_rail/registry.rs:21`):
...
respectively and point at the all-zero pending contract (`src/server/wallet_rail/addresses.rs:22`). The
...
(`src/store/retrieve.rs:27`), and every signer and the address handler read the secret through `eth_secret`
