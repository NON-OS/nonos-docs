...
displays is exactly what those two reads expose, and nothing more. The [sampling](sampling.md) pillar
covers those two reads; the [interface](interface.md) pillar covers the window, the input, and the frame.
...
[interface](interface.md) pillar, not here.
...
|---|---|---|
| [sampling.md](sampling.md) | `src/pm/sample.rs`, `src/pm/state.rs` | The two reads: `mk_proc_stat` for CPU ticks and the delta math, and the service lookup that resolves the eight monitored names to pids. The monitored list and the refresh cadence. |
| [interface.md](interface.md) | `src/pm/manifest.rs`, `src/pm/event.rs`, `src/pm/paint.rs` | The window shape and the `input_kind_mask`, every user action and its handler, and the renderer that draws the title, the table, the status line, the refresh counter, and the sparklines. |
| [contributing.md](contributing.md) | the whole tree | Where to work, how to change what is watched, the build and sign steps, and the code standards. |
| [debugging.md](debugging.md) | runtime | The boot marker and the visual failure modes: an offline row, a flat sparkline, a stuck CPU column, the hardcoded `caps` column. |

## Lifecycle

The capsule is `no_std`/`no_main`. `_start` calls the skeleton's `run(ProcessManager::new)`
(`src/main.rs:27`). The model is a `ProcessManager` holding a `State` and a tick counter
(`src/pm/app.rs:25`).

1. The kernel spawns the capsule at boot through the apps-and-tools fleet plan
   (`src/userspace/init/spawn_plan/apps_tools.rs:20`, behind the `nonos-capsule-process-manager`
   ...
   (`src/userspace/capsule_process_manager/spawn.rs:37`). On success the boot log prints
   `[APP-PROCESS-MANAGER] capsule spawned` (`src/userspace/init/capsule_boot/run.rs:29`).
...
3. `on_tick` fires on the skeleton's timer (a 1000 ms default interval, `app_skeleton/src/app/behavior.rs:30`).
...
The two pillars, [sampling.md](sampling.md) and [interface.md](interface.md), each carry their own source
