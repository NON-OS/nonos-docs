...
This page mirrors `src/pm/sample.rs` and `src/pm/state.rs`: the two reads that give the process manager
...
rows, in this order (`src/pm/state.rs:42`):
...
(`src/pm/state.rs:54`, `state.rs:58`). Changing the watched set means editing this table and keeping those
widths in step, which the [contributing](contributing.md) page covers.

Each `Row` carries the static label and service name, the resolved pid, an online flag, and a 30-sample
circular CPU history (`src/pm/state.rs:31`). `HISTORY` is 30 (`src/pm/state.rs:19`).
...
`lookup_service`; on success it records the pid and marks the row online (`src/pm/state.rs:74`,
`state.rs:81`). The status string is set to `PID from service lookup, caps unavailable`
(`src/pm/state.rs:86`), and a wrapping refresh counter is bumped once per refresh (`src/pm/state.rs:75`).
...
(`userland/app_skeleton/src/discover/lookup_service.rs:21`, `lookup_service.rs:25`). The manager uses only
the pid; it discards the port (`src/pm/state.rs:82`). A row whose service has crashed and not yet
respawned reads offline until the next refresh, which is at most a few ticks away, or immediately if the
user forces a refresh from the [interface](interface.md).
...
- The buffer is a stack array sized `HEADER_LEN + MAX_ENTRIES * ENTRY_LEN`, with `MAX_ENTRIES` fixed at
  64 (`src/pm/sample.rs:21`, `sample.rs:26`). Nothing is heap-allocated per sample.
- `mk_proc_stat(buf, 64)` returns the number of per-process entries written; a return of zero or less
  makes the sampler bail without touching state (`src/pm/sample.rs:27`, `sample.rs:28`). The count is then
  clamped to at most 64 (`sample.rs:31`).
- The header is read out of the buffer with an unaligned read into a `ProcStatHeader`
  (`src/pm/sample.rs:32`). `dt` is `header.total_ticks - state.last_total_ticks`, saturating so it never
  underflows (`sample.rs:34`).
- The first sample is not warmed, because there is no prior baseline: `warmed` requires both a nonzero
  previous total and a positive `dt` (`src/pm/sample.rs:35`). A percentage is therefore only computed from
  the second sample onward.
- For each row, an offline row (pid zero) is skipped, and its last-tick slot is reset to zero
  (`src/pm/sample.rs:37`). For a live row, `find_ticks` locates the pid's `run_ticks` by a linear scan of
...
- When warmed, the per-row delta `d = ticks - last_ticks[row]` (saturating) is turned into a percentage
  of the interval, `min(d * 100 / dt, 100)`, clamped to 100 (`src/pm/sample.rs:43`, `sample.rs:44`). The
...
- Every row's `last_ticks` is updated for the next interval, and finally `state.last_total_ticks` is set
  to the new total (`src/pm/sample.rs:49`, `sample.rs:51`).

The history this fills is what the [interface](interface.md) renders as a sparkline and a newest-sample
percentage.

## What mk_proc_stat is

`mk_proc_stat` is a direct syscall, `N_MK_PROC_STAT`, with no service on the far side
(`userland/libc/src/procstat.rs:36`). The buffer it fills is a `ProcStatHeader` carrying the system total
...
On the kernel side the call lands in `sys_proc_stat` (`src/syscall/microkernel/procstat.rs:43`). It
