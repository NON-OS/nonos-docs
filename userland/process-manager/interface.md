...
This page mirrors `src/pm/manifest.rs`, `src/pm/event.rs`, and `src/pm/paint.rs`: the window the capsule
...
(`src/pm/manifest.rs:19`, `manifest.rs:24`). The window id is `0x504D_4752` and the kind is
`WindowKind::Normal` (`manifest.rs:27`, `manifest.rs:28`).

The manifest also sets `input_kind_mask` to `INPUT_KEY_DOWN_BIT`, which is `1 << 0`
(`src/pm/manifest.rs:22`, `manifest.rs:33`). This is the window's input subscription, and it is a
separate value from the capability mask on the [README](README.md): it selects which `InputKind` values
the compositor delivers, not what the capsule is allowed to do. The bit index matches
`InputKind::KeyDown = 0` (`userland/app_skeleton/src/input/kind.rs:20`), so the window subscribes to
...
Input arrives at `on_event` (`src/pm/event.rs:21`). There are exactly four things a user can do, and none
...
The handler order is: a `ButtonDown` refreshes and repaints and returns early (`src/pm/event.rs:22`);
...
thing, which is to trigger a fresh service lookup (the [sampling](sampling.md) refresh) so the pid column
...
`paint` draws the whole surface each call (`src/pm/paint.rs:29`). It clears to the background colour, then
lays out a fixed frame:

- A title, `process_manager`, at the top left (`src/pm/paint.rs:31`).
...
Rows are laid out from `y = 68` downward at an 18-pixel pitch (`src/pm/paint.rs:40`, `paint.rs:55`), with
...
Each row's CPU history is drawn by `paint_spark` (`src/pm/paint.rs:59`). It walks the 30-slot circular
...
(`src/pm/paint.rs:52`), formatted as a decimal by the `u32_decimal` helper in `src/pm/format.rs:17` and
drawn at `PCT_X` (`paint.rs:53`, `paint.rs:54`). The colours (`BACKGROUND`, `FOREGROUND`, `WARNING`,
`MUTED`) are the four constants in `src/pm/theme.rs:17`.
...
(`src/pm/paint.rs:46`, `paint.rs:49`). This is the one boundary the tool states about itself: the capsule
