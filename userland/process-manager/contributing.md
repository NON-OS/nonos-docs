...
refresh in `state.rs`, the decimal helper in `format.rs`, and the palette in `theme.rs` (`src/pm/mod.rs:17`).
The [sampling](sampling.md) and [interface](interface.md) pillars describe each of these in detail.
...
- To change what the tool watches, edit the `KNOWN` table in `src/pm/state.rs:42`. Each entry is a label
  ...
  fixed at eight, so if you add a row there you must keep those widths in step (`src/pm/state.rs:54`,
  `state.rs:58`).
- To change the sampling window or math, edit `src/pm/sample.rs`: the `MAX_ENTRIES` cap (`sample.rs:21`)
  and the delta computation (`sample.rs:34`, `sample.rs:44`), and `HISTORY` in `src/pm/state.rs:19` for
  the sparkline depth.
- To change the layout, edit `src/pm/paint.rs` and the column offsets at its head (`paint.rs:24`).
- To change the window shape or the input subscription, edit `src/pm/manifest.rs`.
...
that currently renders `unavailable` (`src/pm/paint.rs:46`), and it needs a real kernel read behind it.
...
- No panics, `unwrap`, or `expect` in capsule code. The sampler returns early on a bad read rather than
  trusting it (`src/pm/sample.rs:28`), and the release profile is `panic = "abort"` (`Cargo.toml:26`).
- Modular files, one unit per file, with `mod.rs` used only for re-exports (`src/pm/mod.rs:17`).
- The AGPL header sits at the top of every source file, byte for byte the same as the header on
  `src/pm/sample.rs:1` and every other module.
