# Debugging capsule_power

This page lists what the power capsule does and does not emit, and the concrete failure modes with where to
look for each. For what the capsule is and how it is served, read the [README](README.md) and the
...
(see the [README lifecycle](README.md#lifecycle)).
...
(`src/server/respond.rs:19`). A caller reads that word to know what happened.

| Status | Meaning | Where it comes from |
|--------|---------|---------------------|
| `0` | healthcheck ok, or reboot reply built before reset fired | `src/server/handlers/health.rs:20`, `src/server/handlers/reboot.rs:28` |
| `-95` | `E_NOTSUP`: shutdown is not implemented in the kernel | `src/syscall/dispatch/router/admin/shutdown.rs:19` |
| `-38` | `E_BAD_OP`: opcode was none of the three | `src/server/handlers/router.rs:31`, `src/protocol/errno.rs:17` |
| `-71` | `E_BAD_MAGIC`: first four bytes were not `POWR` | `src/protocol/decode.rs:28`, `src/protocol/errno.rs:18` |
| `-90` | `E_BAD_LEN`: frame short or payload length overran | `src/protocol/decode.rs:20`, `src/protocol/errno.rs:19` |
| `-93` | `E_BAD_VERSION`: version field was not 1 | `src/protocol/decode.rs:31`, `src/protocol/errno.rs:20` |
...
(`src/server/handlers/reboot.rs:28`). In QEMU the reboot path takes effect within milliseconds of any
...
(`src/server/handlers/shutdown.rs:29`, `src/arch/x86_64/acpi/power_sleep.rs:35`). The machine correctly
...
sent against the layout on the [operations](operations.md#the-wire-format) page: magic must be `POWR`,
...
per-caller logic in the handler (`src/syscall/contract/cap_table/admin.rs:24`). There is no debounce and no
