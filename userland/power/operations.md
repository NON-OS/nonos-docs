# Operations and the syscall path
...
ACPI. For what the capsule is and its identity, read the [README](README.md). For status codes seen at
runtime, read [debugging](debugging.md).

## The three operations

Three operations are defined as `u16` opcodes (`src/protocol/ops.rs:17`):

| Op | Opcode | What it does | Handler |
|----|--------|--------------|---------|
| `OP_HEALTHCHECK` | `0x0001` | liveness ping; replies status `0` | `src/server/handlers/health.rs:20` |
| `OP_REBOOT` | `0x0002` | reset the machine | `src/server/handlers/reboot.rs:23` |
| `OP_SHUTDOWN` | `0x0003` | request power off (returns `E_NOTSUP` today) | `src/server/handlers/shutdown.rs:23` |

Health is stateless: it takes `(out, req)` and returns a status-`0` reply, nothing more
(`src/server/handlers/health.rs:20`). The two power ops take the shared `PowerState` so they can record a
...
(`src/protocol/header.rs:19`, `src/protocol/mod.rs:29`). Every field is little-endian
...
(`src/protocol/decode.rs:20`). The `Request` the parser hands back carries only `op`, `flags`, and
`request_id`; those are what the reply echoes (`src/protocol/header.rs:22`). The error statuses
...
status at offset 20, and returns `HDR_LEN + STATUS_LEN` (`src/server/respond.rs:19`,
`src/protocol/encode.rs:19`).

## The request loop

The server is a single fixed-port loop (`src/server/runner.rs:28`):
...
2. Block on `mk_ipc_recv_from(4448, ...)`, which returns the byte count and the sender pid. A receive of
   ...
   than replying to nobody (`src/server/runner.rs:34`, `src/server/runner.rs:41`).
3. Route the received bytes through `route`, which parses the header, dispatches on the opcode, and returns
   ...
   (`src/server/handlers/router.rs:22`, `src/server/handlers/router.rs:31`).
...
(`src/server/handlers/reboot.rs:23`):
...
(`src/syscall/numbers/defs.rs:40`). The admin router matches these two plus `AdminPolicyPush` and, before
...
(`src/syscall/contract/cap_table/admin.rs:24`, `src/syscall/caps/checks/system.rs:24`). Only a caller
holding the Admin bit, which in this fleet is only the power capsule, passes.

Once past the gate the router dispatches (`src/syscall/dispatch/router/admin/route.rs:41`) and marks the
result `audit_required` for both power ops (`src/syscall/dispatch/router/admin/route.rs:47`):

- `AdminReboot` calls `power_reboot::reboot()` and returns `E_OK`
  (`src/syscall/dispatch/router/admin/reboot.rs:21`).
- `AdminShutdown` returns `E_NOTSUP` directly, without side effects and without calling into the ACPI sleep
  path (`src/syscall/dispatch/router/admin/shutdown.rs:19`).
...
(`src/arch/x86_64/acpi/power_reboot.rs:47`, `src/arch/x86_64/acpi/power_reboot.rs:59`).

Shutdown is honest about a missing piece. Entering S5 requires evaluating the `_S5` AML object in the DSDT
to read the `SLP_TYPa`/`SLP_TYPb` values, and NONOS has no AML interpreter, no DSDT walker, and no
...
(`src/arch/x86_64/acpi/power_sleep.rs:27`, `src/arch/x86_64/acpi/power_sleep.rs:35`).

The `mk_time_millis` wrapper the handlers use for the audit timestamp is the `MkTimeMillis` syscall
(`userland/libc/src/time/wall.rs:19`). For the wider admin syscall family, see the
[syscall router](../../subsystems/syscall/router.md).
