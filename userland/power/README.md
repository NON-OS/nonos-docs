...
PM1 value (`src/syscall/dispatch/router/admin/shutdown.rs:19`, `src/arch/x86_64/acpi/power_sleep.rs:35`).
...
syscalls require (`Capsule.mk:13`, `src/syscall/contract/cap_table/admin.rs:24`).
...
(`src/main.rs:29`, `src/server/runner.rs:28`). The two power operations are thin wrappers over the kernel
...
manifest endpoint (`src/server/runner.rs:25`). The reply endpoint declares port `4449`, but the runner does
...
else. It is the same minimal posture as the [policy](../policy/README.md) capsule, which also holds exactly
...
|------|---------|----------------|
| [operations.md](operations.md) | `src/protocol/`, `src/server/` | The three ops, the fixed frame format, the request loop, the reply-then-reset vs shutdown ordering, and the full syscall path from handler to ACPI, including why shutdown returns `E_NOTSUP`. |
...
| [debugging.md](debugging.md) | runtime | The absence of a boot marker, the failure modes, and how to read each status a caller can get back. |
...
rather than given a page of its own (`src/state/mod.rs:17`).
...
(`src/main.rs:29`). The three modules are `protocol` (the wire format), `server` (the loop and handlers),
and `state` (the two timestamps) (`src/main.rs:22`).

Unlike a spawned app such as the [terminal](../terminal/README.md), the power capsule is not launched by
...
not a boot marker. The [debugging](debugging.md) page covers what a caller sees instead.
