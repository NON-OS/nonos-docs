...
the [routing](routing.md) page; the handlers that write them are on the [operations](operations.md) page;
for the overview and the capability identity see the [README](README.md).
...
(`src/state/context/types.rs:19`). It owns the four tables (subscriptions, grabs, key targets, cursor), the
...
at 0, `next_request_id` at 1, and both pointer caches `None` (`src/state/context/new.rs:21`).
...
- `issue_request_id` hands out the next id and wraps past zero back to 1, so a cross-service call always
  carries a nonzero id (`src/state/context/issue_request_id.rs:20`).
- `record` folds a delivery count into the telemetry: a zero count increments `dropped_count`, a nonzero
  count adds to `delivered_count`, both saturating (`src/state/context/record.rs:20`). Every routing arm
  ends in this call.
- `forget_pid` is the single teardown for a dead consumer. It first returns early if the pid is 0 or still
  ...
  cached it (`src/state/context/forget_pid.rs:20`). This is what a failed delivery triggers, so a crashed
...
- `purge_dead` is the periodic sweep: it purges the subscription and grab tables of dead pids and clears
  `shell_pid` if its owner is gone (`src/state/context/purge_dead.rs:20`). The loop calls it every 64th
  tick (`src/server/runner.rs:41`).

## The cursor tracker

`CursorState` is the router's model of where the pointer is on the screen (`src/state/cursor.rs:21`). It
...
so before the compositor answers the cursor lives on a 1024x768 virtual screen centered (`src/state/cursor.rs:31`).
`configure` is called once the compositor reports the real display: it sets the bounds to
`width - 1`/`height - 1` and recenters the cursor (`src/state/cursor.rs:35`). The `configured` flag is what
makes `refresh_display` a one-shot ([routing](routing.md)).

`apply` folds one event into the absolute position (`src/state/cursor.rs:43`). A `POINTER_REL` adds
...
onto the bounds (`ABS_RANGE_MAX = 0x7FFF`, `src/state/cursor.rs:19`). Either way `clamp` pins the result
into `0..max` before it is returned as an unsigned `(x, y)` (`src/state/cursor.rs:56`). The sensitivity
comes from policy: the loop reads `mouse_sensitivity` every two seconds and stores it clamped to `1..4`
into `mult_x2` (`src/server/runner.rs:46`), the read is on the [clients](clients.md) page.

## The grab table

A grab is an exclusive claim on a class of events. `GrabTable` is two `Grab` records, one for keyboard and
one for pointer, each a `holder_pid` and a `kind_mask` (`src/state/grabs/types.rs:17`). `GrabTable::new` is
`const` and starts both empty (`src/state/grabs/new.rs:20`). The split into two records is deliberate: a
...
- `request` splits the requested mask by `KEYBOARD_BITS = 0b11` and `POINTER_BITS = 0b1111_1100` and stores
  ...
  return `false` (`src/state/grabs/request.rs:23`), which the handler maps to `E_BUSY`
...
- `holder_for` shifts `1 << kind` and tests it against the keyboard mask then the pointer mask, returning
  ...
  wrapping (`src/state/grabs/holder_for.rs:20`). This is the first check the router makes on every event.
- `release` clears whichever class the caller holds, keyed on `holder_pid`, so releasing a class you do not
  hold is a no-op; this is why the grab-release handler is unconditional (`src/state/grabs/release.rs:20`).
- `purge_dead` clears either class whose holder pid is no longer alive and returns how many it cleared
  (`src/state/grabs/purge_dead.rs:20`).
...
`kind_mask`, and an `in_use` flag (`src/state/subscriptions/types.rs:17`). `SubscriptionTable::new` is
`const` and starts every slot free (`src/state/subscriptions/new.rs:20`).
...
- `upsert` is what `OP_SUBSCRIBE` calls. It updates the existing entry for a pid, or claims the first free
  ...
  which the handler maps to `E_NOMEM` (`src/state/subscriptions/upsert.rs:20`).
- `allows` is the per-delivery gate: it returns true only if some in-use entry for that pid has the bit for
  this kind set, using `checked_shl` for the out-of-range guard (`src/state/subscriptions/allows.rs:20`).
...
- `match_kind` yields the pids of every in-use entry whose mask covers a kind, and is what the broadcast
  arm iterates (`src/state/subscriptions/match_kind.rs:20`).
- `remove_pid` clears every slot for a pid and reports whether it removed anything; `purge_dead` clears
  every slot whose pid is no longer alive on a sweep (`src/state/subscriptions/remove_pid.rs:20`,
  `src/state/subscriptions/purge_dead.rs:20`).

## The per-key targets

`KeyTargets` records which pid received the key-down for each currently held key, so the matching key-up is
delivered to that same pid even if focus moved while the key was held (`src/state/key_targets.rs:30`). It is
...
that key-up falls back to current focus (`src/state/key_targets.rs:42`). `take` returns and clears the pid
for a code, which is how a key-up finds its target (`src/state/key_targets.rs:53`). `forget_pid` drops every
entry for a dead pid (`src/state/key_targets.rs:61`). The keyboard routing that uses these is on the
[routing](routing.md) page.
...
- `Press` is the implicit pointer grab armed by a button-down inside a window: a `pid` and the window's
  ...
  where deltas equal screen deltas even while the window moves itself (`src/state/press.rs:22`). It is set
  on a `BUTTON_DOWN` over a window and cleared on the matching `BUTTON_UP` ([routing](routing.md)).
- `Hover` caches the topmost window's rect (`pid`, `x`, `y`, `w`, `h`) so pointer motion inside it routes
  without a WM round trip per event; `contains` is the point-in-rect test (`src/state/hover.rs:21`,
  `src/state/hover.rs:30`). The cache is dropped on a press (the window may move itself during a drag) and
  when the cursor exits the rect. The `hover_tick` counter on the `Context` throttles how often the WM is
  re-queried ([routing](routing.md)).
