# Contributing to capsule_input_router
...
read the [README](README.md), the [operations reference](operations.md), the [routing engine](routing.md),
the [state](state.md), and the [clients](clients.md) pages in this folder.

## Where the source lives

The capsule is at `userland/capsule_input_router/`. It is a `no_std`/`no_main` capsule: `_start`
initializes the heap and calls `server::run`, which never returns (`src/main.rs:32`). There is no
`wait_for_setup` step; the router comes up first in the fleet and resolves each service lazily on first use.
The six top-level modules are declared in `main.rs` (`src/main.rs:22`).
...
1. Add the opcode to `src/protocol/ops.rs:17` and, if it carries a body, its request length to
   `src/protocol/limits.rs`.
2. Write the handler as one file under `src/server/handlers/`, checking the exact body length first and
   replying `E_INVAL` on a mismatch, and reply through `respond::status` (`src/server/respond.rs:21`). A
   ...
   (`src/server/handlers/grab_request.rs:25`). Never let an unprivileged caller claim a class or reach a
   consumer it is not entitled to.
3. Re-export the handler from `src/server/handlers/mod.rs` and add the match arm in the opcode dispatch
   (`src/server/drain_ipc.rs:44`). A bodyless verb belongs on an arm guarded by an empty-body check, the way
   `OP_HEALTHCHECK` and `OP_GRAB_RELEASE` are; the two wildcard arms then map an unknown empty-body op to
   `E_BAD_OP` and anything else to `E_INVAL` ([operations](operations.md)).
...
- The order in which grab, pointer, keyboard, and broadcast are tried is `route_event`
  (`src/route/dispatch.rs:28`). Reordering it changes the whole policy, so change it only with the routing
  page in view.
- A new pointer behaviour is a new file under `src/route/pointer/` slotted into the ordered `route_pointer`
  (`src/route/pointer/route_pointer.rs:33`), the same way hover and the drag path are.
- Any new destination must still deliver through the single exit, `deliver_one` (`src/route/deliver.rs:24`),
  so the `NINP` envelope and the 0/1 telemetry stay uniform, and must gate on `subscriptions.allows` before
  delivering to a window so the allow list is never bypassed ([state](state.md)).
- A routing change that reads a new fact from another service is a client change: add the call under
  `src/clients/` over the shared wire and cache its port on the `Context` ([clients](clients.md)).
...
should be added ([README](README.md) identity table).
...
(`src/userspace/capsule_input_router/embed.rs:24`).
...
- No panics in capsule code. No `unwrap`, `expect`, or `panic!`. Every handler returns errors as a status
  ...
  (`src/state/grabs/holder_for.rs:20`), the cursor uses saturating arithmetic and `clamp`
  (`src/state/cursor.rs:45`), the telemetry counters saturate (`src/state/context/record.rs:22`), and the
  request-id counter wraps (`src/state/context/issue_request_id.rs:22`).
