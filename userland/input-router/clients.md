...
make see the [routing](routing.md) page; for the tables the answers land in see the [state](state.md) page;
...
([README](README.md) identity table). A service that does not answer, or answers with the wrong magic or a
...
length, then the payload (`src/clients/wire/build.rs:21`, `HDR_LEN = 20`, `VERSION = 1`,
`src/clients/wire/constants.rs:17`). `call` sends that frame with `mk_ipc_call_timeout` on a 150 ms budget,
requires the reply to be at least the header plus a 4-byte status plus the expected body and to carry the
same magic, copies the body out, and returns the status word (`src/clients/wire/call.rs:28`,
`src/clients/wire/call.rs:47`). A reply that is short or mismatched is an `Err`, which every caller turns
into "no answer".

Two lookup helpers resolve a service by name through `mk_service_lookup`: `lookup_port` returns the port
(rejecting a zero port or pid) and `lookup_pid` returns the pid (`src/clients/wire/lookup_port.rs:19`,
`src/clients/wire/lookup_pid.rs:19`). `send_to` frames a request and fires it one-way with
`mk_ipc_send_to_pid`, no reply expected (`src/clients/wire/send_to.rs:24`). `u32_at` is the bounds-checked
little-endian field reader every reply decode uses (`src/clients/wire/u32_at.rs:17`).

## The window manager client

The router speaks `NWMP` to the `wm` service: magic `0x4E57_4D50`, resolved by the name `wm`
(`src/clients/wm/constants.rs:17`, `src/clients/wm/constants.rs:21`). It makes three calls, each a separate
...
- Focus query. `query_focus` sends `OP_QUERY_FOCUS = 0x000D` with an empty body and reads back the focused
  ...
  no focus (`src/clients/wm/query_focus.rs:20`, `src/clients/wm/constants.rs:18`). The keyboard path calls
  this on every `KEY_DOWN` ([routing](routing.md)).
- Hit test. `query_topmost` sends `OP_QUERY_TOPMOST = 0x000B` carrying the cursor `x`/`y` and decodes a
  ...
  `win_x`/`win_y`/`win_w`/`win_h` (`src/clients/wm/query_topmost.rs:21`, `src/clients/wm/types.rs:17`,
  `src/clients/wm/constants.rs:19`). The pointer path calls this to find the window under a click, touch, or
...
- Focus routing. `route_focus` sends `OP_ROUTE_FOCUS = 0x000C` carrying the target's owner pid and window
  id, and returns whether the wm accepted it (`src/clients/wm/route_focus.rs:21`,
  `src/clients/wm/constants.rs:20`). This is the one wm verb gated to the router alone: the wm resolves
  `input_router` by name and refuses any other caller, documented from the wm side in its
  [clients page](../wm/clients.md#the-input-router-gate). The router calls it when a `BUTTON_DOWN` or
...
`compositor` (`src/clients/compositor/constants.rs:17`, `src/clients/compositor/constants.rs:20`). It makes
...
- Display size. `display_size` sends `OP_DISPLAY_INFO = 0x0008` with an empty body and reads back the
  display width and height, rejecting a zero dimension (`src/clients/compositor/display_size.rs:20`,
  `src/clients/compositor/constants.rs:19`). `refresh_display` calls it once, the first time a pointer event
  is routed, to configure the cursor bounds to the real screen ([routing](routing.md), [state](state.md)).
- Cursor update. `cursor_update` is a one-way send, not a call: it resolves the compositor pid once into a
  ...
  fires it with `send_to` (`src/clients/compositor/cursor_update.rs:24`,
  `src/clients/compositor/constants.rs:18`). The loop calls it after routing a batch whenever the cursor
  moved (`ctx.cursor_dirty`), so the compositor redraws the pointer without the router waiting on a reply
  (`src/server/runner.rs:55`).

## The policy read

`mouse_sensitivity` reads one field from the `policy` service (`src/clients/policy.rs:29`). It uses the
...
success status returns the single byte (`src/clients/policy.rs:21`, `src/clients/policy.rs:43`). The call
has its own 150 ms timeout, and any short or non-`u8` reply clears the port and returns `None`
(`src/clients/policy.rs:56`). The loop reads it at most every two seconds and clamps the result into
`1..4` before storing it in the cursor's `mult_x2` (`src/server/runner.rs:44`), so a missing or misbehaving
...
or the shell), through `deliver_one` and its `NINP` envelope, documented on the [routing](routing.md) and
[operations](operations.md) pages. The clients here only supply the pids and coordinates that decide who
that consumer is. The desktop shell pid itself is resolved once by name (`desktop_shell`) and cached in
`ctx.shell_pid` (`src/route/pointer/shell_pid.rs:22`, `src/route/pointer/constants.rs:17`); the compositor
