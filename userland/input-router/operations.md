...
handlers, the reply). For the routing that follows a drained event see the [routing engine](routing.md);
for the tables the handlers mutate see the [state](state.md) page; for the overview and the capability
identity see the [README](README.md).
...
|---|---|---|---|---|
| `NIRS` request | `0x4E49_5253` | consumer to router, and router to reply | 20-byte header, then payload | `src/protocol/header.rs:17` |
| `NINP` delivery | `0x4E49_4E50` | router to subscriber | 8-byte header, then the 32-byte event, 40 bytes total | `src/protocol/delivery.rs:23` |

### The NIRS request frame

The request header is magic, version 1, op, flags, a reserved word, a request id, and a declared payload
length, in that order (`src/protocol/header.rs:17`, `src/protocol/decode.rs:19`). `parse` checks the magic
...
(`src/protocol/decode.rs:32`). A reply reuses the same header with a 4-byte status appended
(`src/protocol/encode.rs:19`, `src/protocol/encode.rs:29`).

The status values the router returns are `0` for success, `E_INVAL` (-22), `E_ACCES` (-13), and `E_BAD_OP`
(-38) (`src/protocol/errno.rs:17`), plus `E_NOMEM` (-12) from subscribe and `E_BUSY` (-16) from grab
request, both defined locally in their handlers (`src/server/handlers/subscribe.rs:21`,
...
`InputEvent` field by field, little-endian, for 40 bytes total (`src/protocol/delivery.rs:29`). The frame
size is `HDR_LEN + size_of::<InputEvent>()`, computed from the libc struct so the two stay in lockstep
(`src/protocol/delivery.rs:25`). This is the envelope every consumer decodes; the desktop shell's inverse
is documented in the [event path](../../subsystems/input/path.md#delivery-envelope). The delivery path
itself lives with the routing engine (`src/route/deliver.rs`), documented on the [routing](routing.md)
page.

## The four operations

The router exposes four opcodes on its service inbox (`src/protocol/ops.rs:17`).
...
`drain_ipc` reads each pending request without blocking and dispatches it (`src/server/drain_ipc.rs:28`).
It calls `mk_ipc_recv_from` on the service inbox with `RECV_NOWAIT` in a loop; a return of zero or less, or
a zero sender pid, ends the loop and hands control back to the main loop (`src/server/drain_ipc.rs:38`). A
request that fails `parse` is skipped and the loop continues (`src/server/drain_ipc.rs:41`). The dispatch
then matches on the opcode (`src/server/drain_ipc.rs:44`):
...
recognise gets `E_INVAL` (`src/server/drain_ipc.rs:51`).
...
(`src/server/handlers/health.rs:20`). The empty-body requirement is enforced by the caller's match arm, not
the handler.

### Subscribe

The body is exactly 8 bytes: a `u32` kind mask and a `u32` pad (`src/protocol/limits.rs:23`). The mask is a
bitset over the `INPUT_KIND_*` values, so bit `n` set means the subscriber wants events whose `kind == n`
(`src/protocol/limits.rs:20`). The handler validates the length, rejecting anything but 8 bytes with
`E_INVAL`, reads the mask, and `upsert`s it into the subscription table
(`src/server/handlers/subscribe.rs:24`). `upsert` updates an existing entry for that pid or claims a free
slot; a mask of `0` removes the entry, and a full table returns `false`, which the handler maps to
`E_NOMEM` (`src/server/handlers/subscribe.rs:32`). The table and its 16-slot cap are on the
[state](state.md) page.
...
and a non-match is `E_ACCES` (`src/server/handlers/grab_request.rs:25`, `:32`). Only then does it check the
body length (`E_INVAL` on a mismatch) and read the mask; a zero mask is `E_INVAL`
(`src/server/handlers/grab_request.rs:44`). The grab table splits the mask into keyboard and pointer
classes and stores each separately; a class already held by a different pid makes `request` return `false`,
which the handler maps to `E_BUSY` (`src/server/handlers/grab_request.rs:48`). The split and the busy check
are on the [state](state.md) page.

### Grab release

The body must be empty, enforced by the match arm (`src/server/drain_ipc.rs:48`). The handler drops
whichever keyboard and pointer grabs the caller held and replies `0` (`src/server/handlers/grab_release.rs:22`).
...
(`src/server/respond.rs:21`). The reply travels on the reply endpoint, port 4321
(`Capsule.mk:13`). The reply frame is `HDR_LEN + STATUS_LEN` bytes, a 20-byte `NIRS` header plus the
4-byte status (`src/server/respond.rs:24`, `src/protocol/limits.rs:18`).
