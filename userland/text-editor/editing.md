...
(open, save, copy, paste, and the path prompt) is on the [file-io](file-io.md) page. For the capsule's
identity and mask see the [README](README.md).

It mirrors `src/editor/event.rs`, `on_nav.rs`, `insert.rs`, `backspace.rs`, the scroll and layout helpers,
and `paint.rs`.

## The key router

Input arrives as key-down events through the app skeleton. `on_event` is the single entry point and it
routes in a fixed order (`src/editor/event.rs:25`):

1. Anything that is not a key-down returns `Idle` and is ignored; the window subscribes only to key-down
   in the first place (`src/editor/event.rs:26`, `src/editor/manifest.rs:33`).
2. If a path prompt is open, the key goes to the prompt handler and nothing else runs
   (`src/editor/event.rs:29`); that mode is on the [file-io](file-io.md) page.
3. A key with the Ctrl modifier goes to `on_ctrl` (`src/editor/event.rs:32`); the chords are on the
   [file-io](file-io.md) page.
4. A navigation key (arrows, page, home, end) is claimed by `on_nav` (`src/editor/event.rs:36`).
5. Everything else is editing: Esc closes, Backspace deletes, Enter is a newline, and a printable code
   point is inserted (`src/editor/event.rs:39`).

The key constants are the app-skeleton codes shared with every capsule
(`userland/app_skeleton/src/input/keys.rs`, `src/input/modifiers.rs`). The return value is an
`EventOutcome` the runtime acts on: `Idle`, `Repaint`, or `Close`
(`userland/app_skeleton/src/app/event_outcome.rs:19`).
...
[file-io](file-io.md) page. There is deliberately no Left, Right, or Delete-at-cursor binding: the arrow
keys scroll the view, they do not move an edit position.

The key codes above are the app-skeleton constants (`userland/app_skeleton/src/input/keys.rs:17`,
`keys.rs:19`, `keys.rs:20`, `keys.rs:24`, `keys.rs:25`, `keys.rs:28`, `keys.rs:29`, `keys.rs:30`,
`keys.rs:31`); Ctrl is `MOD_CTRL = 1 << 1` (`userland/app_skeleton/src/input/modifiers.rs:18`).
...
(`src/editor/state.rs:17`, `src/editor/state.rs:27`). The caret is implicit at `len`.
...
change, so no edit can overrun the array (`src/editor/insert.rs:20`). A printable key is first encoded to
UTF-8 into a 4-byte scratch and the encoded bytes are what get inserted; a code point that does not decode
to a `char` is dropped (`src/editor/event.rs:43`).
...
character rather than half of one (`src/editor/backspace.rs:20`).
...
and returns `Idle` (`src/editor/event.rs:52`).
...
(`src/editor/on_nav.rs:26`).

- `scroll_up` subtracts from `scroll_line` with a saturating subtract, so it cannot go below zero
  (`src/editor/scroll_up.rs:19`).
- `scroll_down` adds to `scroll_line` and then `clamp_scroll` pins it to the last page
  (`src/editor/scroll_down.rs:20`, `src/editor/clamp_scroll.rs:21`).
- Page Up and Page Down move by `rows - 1` (at least one row), so a page overlaps the previous one by a
  line (`src/editor/on_nav.rs:30`).
- Home sets `scroll_line = 0`; End calls `follow_end`, which sets `scroll_line` to the last scrollable
  line (`src/editor/on_nav.rs:32`, `src/editor/follow_end.rs:21`).

The clamp math is shared. `visual_lines` counts wrapped lines by walking the text and breaking on `\n` or
when the column reaches `wrap_cols` (`src/editor/visual_lines.rs:17`). `max_scroll` is `total - rows`
saturating, the last line the view can rest on (`src/editor/max_scroll.rs:17`). `clamp_scroll` pins
`scroll_line` to that maximum every time it runs (`src/editor/clamp_scroll.rs:21`).
...
resize reflows the text with no separate resize handler (`src/editor/paint.rs:27`).

- `wrap_cols(width)` is `(width - 2*TEXT_LEFT) / GLYPH_ADVANCE` clamped to `32..=160` columns
  (`src/editor/layout.rs:21`). The glyph advance is 9 px and the left margin is 16 px
  (`src/editor/layout.rs:18`, `src/editor/layout.rs:20`).
- `visible_rows(height)` is `(height - FIRST_LINE_Y) / LINE_HEIGHT`, at least one, with the first text row
  at y 76 and a 20 px line height (`src/editor/visible_rows.rs:19`, `src/editor/layout.rs:17`,
  `src/editor/layout.rs:19`).

## Drawing a frame

`paint` runs once per repaint (`src/editor/paint.rs:26`):

1. Recompute `visible_rows` and `wrap_cols` from the surface, then clamp the scroll
   (`src/editor/paint.rs:27`).
2. Clear to the background colour and draw the header rows: the title `text_editor`, the current path,
   and the status line (`src/editor/paint.rs:30`). While a prompt is open a `_` is drawn after the path
   to mark the prompt edit point (`src/editor/paint.rs:33`).
3. Walk the buffer character by character, tracking `line` and `col`, breaking on `\n` or at `wrap_cols`,
   ...
   (`src/editor/paint.rs:37`).
4. Draw the caret. `end_position` computes the line and column of the buffer end over the same wrap rules,
   and if that position is in view a `_` is drawn there (`src/editor/paint.rs:61`,
   `src/editor/end_position.rs:17`).

The colours are four constants: a dark background, a light foreground, a title tint, and a muted tint for
the path and status (`src/editor/theme.rs:17`). The renderer draws one glyph per cell through the
