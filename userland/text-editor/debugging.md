...
(`src/userspace/init/spawn_plan/apps_tools.rs:27`, `src/userspace/init/capsule_boot/run.rs:29`). If that
...
failure surfaced at verified spawn (`src/userspace/capsule_text_editor/spawn.rs:37`).

## The status line as a probe

The editor reports every action in its status line, drawn near the top of the window each frame
(`src/editor/paint.rs:36`). The status word tells you which stage ran and how it ended, so it is the
...
(`src/editor/manifest.rs:33`, `src/editor/event.rs:26`). If keys are dead the editor never sees them, so
...
(`src/editor/event.rs:29`).

### Open fails

The status word names the stage. `open failed` means the editor could not resolve its own pid or the vfs
read errored (`src/editor/ctrl_open.rs:24`, `src/editor/ctrl_open.rs:44`). `file too large` means the
`vfs::stat` size exceeded the 16 KiB buffer, so the read was never attempted
(`src/editor/ctrl_open.rs:30`). `file is not valid utf-8` means the bytes loaded but are not text and were
refused before entering the buffer (`src/editor/ctrl_open.rs:36`, `src/editor/ctrl_open.rs:43`). Because
...
(`src/editor/ctrl_save.rs:23`, `src/editor/ctrl_save.rs:32`). The editor holds no filesystem authority of
...
(`src/editor/ctrl_copy.rs:24`, `src/editor/ctrl_paste.rs:35`). `paste rejected` means the clipboard
returned content that was not valid UTF-8 or did not fit the remaining buffer, so nothing was appended
(`src/editor/ctrl_paste.rs:31`). Paste is capped at 512 bytes per press, so a very large clipboard is
truncated by design (`src/editor/ctrl_paste.rs:23`).
...
expected without the shell and never blocks the save (`src/editor/notify.rs:31`, `src/editor/notify.rs:34`).
The send result is dropped, so a shell that is present but rejects the frame also leaves the save intact
(`src/editor/notify.rs:54`).
...
(`src/editor/layout.rs:21`, `src/editor/visible_rows.rs:19`, `src/editor/paint.rs:27`). A non-ASCII
character renders as `?` by design, so mojibake in the view is expected for multi-byte text, which is
still stored correctly and saved correctly (`src/editor/paint.rs:55`).
