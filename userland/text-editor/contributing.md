# Contributing to capsule_text_editor
...
[README](README.md), the [editing](editing.md) page, and the [file-io](file-io.md) page in this folder.
...
subscription, and paint loop (`src/main.rs:28`). There is one top-level module, `editor`, declared there
(`src/main.rs:22`).

## Module map

The `editor` module is flat: no subfolders, one unit per file, and `mod.rs` holds the module list and a
single re-export of `Editor` (`src/editor/mod.rs:17`, `src/editor/mod.rs:45`). The files group by role.
...
1. Write the handler as its own file under `src/editor/`, following the shape already in the tree. An
   ...
   `backspace` are (`src/editor/insert.rs:20`, `src/editor/backspace.rs:20`). A command that talks to a
   service is a `pub(super) fn action(state: &mut State) -> EventOutcome` that sets a status line and
   returns `Repaint`, the way `ctrl_copy` and `ctrl_paste` are (`src/editor/ctrl_copy.rs:21`,
   `src/editor/ctrl_paste.rs:22`). Register the module in `src/editor/mod.rs:17`.

2. Wire it into the right router. A plain editing key belongs in the `match` in `src/editor/event.rs:39`.
   A scroll or view key belongs in `src/editor/on_nav.rs:26`, which must return `Some(Repaint)` for a key
   it claims and `None` for a key it does not. A Ctrl chord belongs in `src/editor/on_ctrl.rs:24`, and it
   ...
   (`src/editor/path_prompt.rs:23`) and be handled in `path_prompt::on_key` (`src/editor/path_prompt.rs:32`).

Two rules that keep the model honest. Always set a short static status line on the outcome so the result
is visible, the way every existing action does (`src/editor/event.rs:53`, `src/editor/ctrl_save.rs:32`).
...
(`src/editor/insert.rs:20`, `src/editor/ctrl_open.rs:36`, `src/editor/ctrl_paste.rs:25`).
...
vfs client rather than any raw syscall (`src/editor/resolve_owner_pid.rs:21`, `src/editor/ctrl_open.rs:17`).
...
`src/userspace/capsule_text_editor/spawn.rs:50`, and it widens the blast radius, so it is not something a
...
- No panics in capsule code. No `unwrap`, `expect`, or `panic!`; every fallible path returns through a
  ...
  (`userland/capsule_text_editor/Cargo.toml:26`).
- One unit per file. New actions are one file each under `src/editor/`, and `mod.rs` is used only for the
  module list and the single `pub use`, matching the existing tree (`src/editor/mod.rs:17`).
