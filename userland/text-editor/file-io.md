# File I/O, the path prompt, and the clipboard
...
notification to the desktop shell. The in-process editing and view are on the [editing](editing.md) page.
For the capsule's identity and mask see the [README](README.md).

It mirrors `src/editor/on_ctrl.rs`, `ctrl_open.rs`, `ctrl_save.rs`, `ctrl_copy.rs`, `ctrl_paste.rs`,
...
documented under [../vfs/README.md](../vfs/README.md).
...
An unrecognised Ctrl chord returns `Idle` and does nothing (`src/editor/on_ctrl.rs:30`).
...
(`src/editor/path_prompt.rs:23`). While the prompt is pending, `on_event` routes every key to
`path_prompt::on_key` and nothing else runs (`src/editor/event.rs:29`).

The prompt is seeded with the current path, which starts at `/notes.txt` (`src/editor/state.rs:18`,
`src/editor/state_new.rs:21`). Inside the prompt (`src/editor/path_prompt.rs:32`):
...
the prompt (`src/editor/path_prompt.rs:52`). The path field is a fixed 256-byte array with its own length
(`src/editor/state.rs:34`).

## Open a file

On Enter for an `Open`, `ctrl_open` runs (`src/editor/ctrl_open.rs:23`):

1. Resolve the editor's own pid through `resolve_owner_pid`; a failure sets `open failed` and stops
   (`src/editor/ctrl_open.rs:24`).
2. `vfs::stat` the path, and if the reported size exceeds the 16 KiB buffer, refuse with `file too large`
   (`src/editor/ctrl_open.rs:29`).
3. `vfs::read_file` the path, capped at `CAPACITY` bytes. The bytes are accepted only if they are valid
   ...
   scrolls to the end (`src/editor/ctrl_open.rs:35`).
4. A non-UTF-8 body sets `file is not valid utf-8`; any client error sets `open failed`
   (`src/editor/ctrl_open.rs:43`).

`vfs::read_file` opens the path, reads it in chunks advancing the server-side offset, and closes the
handle, returning the assembled bytes (`userland/app_skeleton/src/clients/vfs/read_file.rs:27`).
`vfs::stat` returns the size and a directory flag (`userland/app_skeleton/src/clients/vfs/stat.rs:22`).

## Save a file

On Enter for a `Save`, `ctrl_save` runs (`src/editor/ctrl_save.rs:22`):

1. Resolve the owner pid; a failure sets `save failed` and stops (`src/editor/ctrl_save.rs:23`).
2. `vfs::write_file` the used portion of the buffer to the path. On success the editor notifies the
   desktop shell and sets `saved`; on failure it sets `save failed` (`src/editor/ctrl_save.rs:28`).
...
(`userland/app_skeleton/src/clients/vfs/write_file.rs:28`).
...
(`src/editor/resolve_owner_pid.rs:21`). `resolve_owner_pid` looks up `app.text_editor` and caches the
pid; a zero pid means the lookup failed (`src/editor/resolve_owner_pid.rs:23`).
...
`clipboard unavailable` if the service did not answer (`src/editor/ctrl_copy.rs:21`). The skeleton's
`clipboard_copy` looks up the `clipboard` port, prefixes a text content-type word, and sends `OP_COPY`,
returning an error on a non-zero status (`userland/app_skeleton/src/clients/clipboard/copy.rs:25`).
...
`paste rejected`; a service failure sets `clipboard unavailable` (`src/editor/ctrl_paste.rs:22`). The
skeleton's `clipboard_paste` sends `OP_PASTE` and copies the returned text back into the caller's slice
(`userland/app_skeleton/src/clients/clipboard/paste.rs:26`).

The clipboard wire uses the `NCLP_MAGIC` fingerprint `0x4342_4930`, `OP_COPY` `0x0002`, and `OP_PASTE`
`0x0003` (`userland/app_skeleton/src/wire/constants.rs:21`,
`userland/app_skeleton/src/clients/clipboard/copy.rs:22`,
`userland/app_skeleton/src/clients/clipboard/paste.rs:22`).
...
save still stands (`src/editor/notify.rs:30`). When the port is known it builds a `saved <path>` string
bounded to 54 bytes and calls `send_notify` (`src/editor/notify.rs:37`).
...
`mk_ipc_send` (`src/editor/notify.rs:44`). The send is best-effort; its result is dropped and it never
blocks or fails the save (`src/editor/notify.rs:54`).
...
resolves the clipboard port inside the clipboard client (`src/editor/resolve_owner_pid.rs:17`,
`src/editor/notify.rs:17`, `userland/app_skeleton/src/clients/clipboard/copy.rs:26`). The editor names no
