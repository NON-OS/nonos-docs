...
active the paint pass draws only the preview and nothing else (`src/fm/paint.rs:33`). Opening is reached
from the browse keys on [input.md](input.md).

## Opening a file

Pressing Enter, Right, or `l` on a file, or clicking it, calls `open_preview`
(`src/fm/event_open.rs:31`). It reads the file through the vfs with `read_file`, capped at
`MAX_PREVIEW_BYTES = 256 KiB` so opening a huge file stays bounded (`src/fm/preview.rs:29`, `:45`). On
...
when the read hit the cap, and the scroll offset (`src/fm/preview.rs:33`, `:53`).
...
tab, newline, or carriage return) (`src/fm/preview_is_binary.rs:18`, `:24`, `:32`). An empty file is
...
exhaust the heap (`src/fm/preview_text.rs:22`, `:29`, `:46`).
...
as dots. The dump is capped at `MAX_ROWS = 4096` rows (`src/fm/preview_hex.rs:22`, `:28`, `:31`).
...
(`src/fm/preview_key.rs:24`, `:33`, `src/fm/preview_paint.rs:28`).

`paint_preview` draws the title, the path, the info bar, the visible window of content lines starting at
the scroll offset, and the footer hint (`src/fm/preview_paint.rs:32`). Each line is clipped to
`MAX_COLS = 38` characters by `clip` before it is drawn (`preview_paint.rs:39`, `src/fm/preview_clip.rs:19`).
...
(`src/fm/preview_info.rs:20`, `:26`).

## The vfs ops preview calls

`read_file` goes over the app skeleton's vfs client, service `vfs_pool`, magic `0x4E4F5646`, using
`OP_OPEN` (1), `OP_READ` (3), and `OP_CLOSE` (2) (`.../vfs/types.rs:19`, `:21`, `:20`,
`.../vfs/read_file.rs`). The read is bounded by the `MAX_PREVIEW_BYTES` argument (`preview.rs:45`). The
listing ops are on [listing.md](listing.md) and the action ops on [actions.md](actions.md).
