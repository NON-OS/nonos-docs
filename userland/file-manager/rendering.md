...
[listing.md](listing.md); the preview renderer is on [preview.md](preview.md).
...
(`src/fm/manifest.rs:26`, `:19`, `:35`). `WIDTH` and `HEIGHT` are the constants the paint geometry is
derived from (`manifest.rs:19`).

## The paint pass

`paint` projects the current mode into the surface (`src/fm/paint.rs:28`). Help and preview take over the
whole window and return early (`paint.rs:29`, `:33`); those are on this page (help) and
[preview.md](preview.md). Otherwise it clears the background, draws the `file_manager` title and the
...
`FIRST_ROW_Y = 64`, `ROW_HEIGHT = 22`, and `LIST_VISIBLE = 7` rows (`src/fm/layout.rs:19`).
...
(`src/fm/paint_metrics.rs:19`).

## Header, rows, and footer

The header draws a right-aligned `sort:<mode>` tag, appending `  /<filter>` when a filter is active
(`src/fm/paint_header.rs:24`, `:27`). The sort label comes from `SortMode::label`
(see [listing.md](listing.md)).

`paint_rows` draws the scrolling window of rows (`src/fm/paint_rows.rs:31`). An empty view shows
...
length from the end column (`src/fm/paint_right.rs:21`), and `clip` truncates to a column width
(`src/fm/paint_clip.rs:17`).

The footer draws a right-aligned `cursor/total` counter, the status line, and, in filter or prompt mode,
the live filter or prompt input echoed after the status (`src/fm/paint_footer.rs:24`, `:29`).
...
- `Kind` is the file class: Dir, Code, Image, Doc, Archive, Exec, or Other
  (`src/fm/filetype.rs:17`). `kind_of` returns Dir for a directory and otherwise classifies by extension
  against fixed lists (`src/fm/file_kind.rs:21`, `:26`). `ext` takes the extension after the last dot
  (`src/fm/file_ext.rs:17`).
- `color` maps each `Kind` to a palette color (`src/fm/file_color.rs:19`), and the palette itself
  (background, foreground, selected, directory, muted) is in `theme.rs` (`src/fm/theme.rs:17`).
- `human_size` renders a byte count as an exact value under 1K or one decimal with a K/M/G suffix
  (`src/fm/human_size.rs:21`).
- `fmt_time` renders a unix-millisecond timestamp as a compact `MM-DD HH:MM`, or `--` when the time is
  unknown, using a branchless civil-date conversion (`src/fm/fmt_time.rs:21`, `:35`).

## The help overlay

`?` opens a full-window keybinding reference; `paint_help` clears the window and draws the fixed `KEYS`
table with a `file_manager keys` title and an `any key to close` footer (`src/fm/help.rs:46`, `:27`).
Any key dismisses it back to browsing (`help.rs:58`). The `KEYS` table is the authoritative in-app cheat
sheet and matches the handlers on [input.md](input.md) and [actions.md](actions.md).
