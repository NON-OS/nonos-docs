...
and `src/fm/mod.rs` lists the modules and re-exports `FileManager` (`src/fm/mod.rs:17`, `:80`). The
...
- input: `event_dispatch.rs`, `event_mode.rs`, `event_modes.rs`, `event_browse.rs`, `event_mouse.rs`,
  `event_move.rs`, `event_open.rs`, `event_parent.rs`, `scroll.rs` (see [input.md](input.md)).
...
- listing: `state.rs`, `state_new.rs`, `entries.rs`, `refresh.rs`, `refresh_meta.rs`, `view.rs`,
  `filter.rs`, `view_sort*.rs`, `sort_*.rs` (see [listing.md](listing.md)).
- preview: `preview.rs`, `preview_hex.rs`, `preview_text.rs`, `preview_is_binary.rs`, `preview_key.rs`,
  `preview_info.rs`, `preview_paint.rs`, `preview_clip.rs` (see [preview.md](preview.md)).
- rendering: `manifest.rs`, `paint.rs`, `paint_*.rs`, `layout.rs`, `theme.rs`, `help.rs`, and the
  file-decoration units (see [rendering.md](rendering.md)).
...
1. Write the operation as its own module under `src/fm/`, taking `&mut State`. Get the acting set with
   ...
   (`src/fm/duplicate.rs:28`, `src/fm/perms.rs:29`).
...
1. Add a `PromptKind` variant in `src/fm/state.rs:34`.
2. Add a browse key that calls `start_prompt` with a status hint in `src/fm/event_browse.rs:43`.
3. Add the matching arm in `src/fm/prompt_run_op.rs:28`.
...
- No panics, `unwrap`, or `expect` in capsule code. Every action reports errors as a status line and
  ...
  (`userland/capsule_file_manager/Cargo.toml:26`).
- Modular files, one unit per file, with `mod.rs` used only for re-exports (`src/fm/mod.rs`).
