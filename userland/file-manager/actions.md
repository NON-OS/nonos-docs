...
the keys themselves are on [input.md](input.md), and the vfs ops they call are named at the bottom of
this page. The refresh they run afterward is on [listing.md](listing.md).
...
Sort cycling is a view concern and is covered on [listing.md](listing.md); the rest are below.
...
a selection is active (`src/fm/selection_acting.rs:23`). It returns each path with its directory flag
(`selection_acting.rs:29`, `:33`).

The checkbox selection is a `Vec<String>` of full paths in `State`. Space toggles the entry under the
cursor in and out of it (`src/fm/selection.rs:20`), `a` fills it with every entry in the current view
(`src/fm/selection_select_all.rs:20`), `is_selected` tests membership for the row highlight
(`src/fm/selection_is_selected.rs:19`), and `clear` empties it (`src/fm/selection_clear.rs:19`). Copy,
...
(`src/fm/clipboard.rs:34`, `:42`). An empty acting set leaves `nothing to yank`
(`clipboard.rs:37`).

Paste walks the clipboard and, for each clip, takes the base name and joins it to the current prefix to
form the destination (`src/fm/clipboard_paste.rs:32`). A cut clip is moved with `rename`; a copy clip is
...
(`src/fm/duplicate.rs:28`). `unique_name` builds `name (copy).ext`, then `name (copy 2).ext`,
...
`u` flips the acting set between writable and read-only (`src/fm/perms.rs:29`). For each entry it reads
...
buffer, and shows a status hint (`src/fm/prompt_start.rs:22`):
...
pops, Esc cancels back to browse with a `cancelled` status, and Enter commits (`src/fm/prompt.rs:24`,
...
the resulting status (`src/fm/prompt_commit.rs:23`, `:26`, `:30`).

`run_op` dispatches by prompt kind (`src/fm/prompt_run_op.rs:28`). Rename targets the cursor entry with
...
`vfs_pool`, magic `0x4E4F5646` (`.../vfs/types.rs:17`). The ops the actions on this page use:
...
a status line (`.../vfs/call.rs:21`, `:30`). The listing ops (`OP_LIST`, `OP_STAT`) are on
[listing.md](listing.md), and the preview ops (`OP_OPEN`, `OP_READ`, `OP_CLOSE`) are on
