...
data model every other page draws on. The actions that mutate it are on [actions.md](actions.md); the
paint pass that reads it is on [rendering.md](rendering.md).

## The State model

The whole app is one `State` (`src/fm/state.rs:50`):
...
or Delete), and Preview (`src/fm/state.rs:26`, `:34`). `SortMode` is Name, Size, Date, or Type
(`state.rs:42`). `State::new` starts at the root prefix `/`, in browse mode, with name sort, an empty
listing, and a `loading...` status (`src/fm/state_new.rs:23`).

## The refresh

`refresh` is the one path that reads the directory (`src/fm/refresh.rs:25`). It resolves the owner pid
...
`vfs unavailable`, the listing self-heals once `vfs_pool` comes up (`src/fm/app.rs:43`, `:50`).
...
duplicates and the prefix itself (`src/fm/entries.rs:31`, `:48`). An `Entry` carries a label, the full
...
stall (`src/fm/refresh_meta.rs:21`, `:24`). The writable flag comes from the `0o200` bit of the returned
mode (`refresh_meta.rs:35`).

## The view: filter and sort

`rebuild_view` is called whenever the listing, filter, or sort mode changes (`src/fm/view.rs:28`). It
...
and Enter keeps the filter and returns to browse (`src/fm/filter.rs:24`, `:42`).

`sort_view` always groups directories before files, then orders within a group by the sort mode
(`src/fm/view_sort.rs:24`). `by_mode` implements the four orders (`src/fm/view_sort_mode.rs:25`):
...
(`src/fm/event_actions.rs:37`, `sort_next.rs:20`). `SortMode::label` gives the short name the header
draws (`src/fm/sort_label.rs:20`).
...
`vfs_pool`, magic `0x4E4F5646` (`.../vfs/types.rs:17`):
...
`.../vfs/list_paths.rs`), and `stat_full` returns size, a directory flag, mtime, and mode
(`refresh_meta.rs:32`, `.../vfs/stat_full.rs`). The owner pid is resolved with `lookup_service`
(`refresh.rs:28`, `.../discover/lookup_service.rs`). The action and preview ops are on
[actions.md](actions.md) and [preview.md](preview.md).
