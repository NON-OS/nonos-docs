...
there is no `src/`, no `Cargo.toml`, and no NONOS-authored code. What exists today is an identity and a
...
The intended implementation is not a from-scratch NONOS program either. The `Capsule.mk` names it as
unmodified upstream ripgrep from crates.io, version 14.1.1
(`userland/capsule_ripgrep/Capsule.mk:20`, `Capsule.mk:27`), to be built against the NONOS standard-library
...
is only partly in place, and the parts that are missing are called out on the [contributing](contributing.md)
and [debugging](debugging.md) pages.
...
(`src/userspace/capsule_ripgrep/spawn.rs:47`). It registers the service `tool.ripgrep` on port 4820 and a
reply inbox `endpoint.tool.ripgrep.reply` on port 4821
(`src/userspace/capsule_ripgrep/spawn.rs:27`, `spawn.rs:28`, `spawn.rs:29`, `spawn.rs:30`), which match the
endpoints the `Capsule.mk` fixes. The spawn is compiled in only under the `nonos-capsule-ripgrep` feature
and is an empty function without it (`src/userspace/init/entry.rs:63`, `entry.rs:71`,
`Cargo.toml:69`).

The intended body of the capsule is the unmodified upstream ripgrep binary. The repository Makefile builds
it by running `cargo install ripgrep --version 14.1.1` against the NONOS user target, linked to the
NONOS runtime start object, with no source patched (`Makefile:548`, `Makefile:552`, `Makefile:554`). The
result is the `rg` ELF the kernel mirror embeds under the feature
(`src/userspace/capsule_ripgrep/embed.rs:18`, `embed.rs:19`). So the end state this contract points at is:
upstream ripgrep, running on the NONOS std PAL, spawned as a signed least-privilege tool endpoint that
