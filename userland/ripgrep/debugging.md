# Debugging capsule_ripgrep

Read the [overview](README.md) first. `capsule_ripgrep` is a defined contract with no implementation yet, so
...
error (`src/userspace/init/entry.rs:62`, `entry.rs:63`, `entry.rs:65`, `entry.rs:66`). Without the feature,
...
(`src/userspace/capsule_ripgrep/embed.rs:18`, `embed.rs:19`), and those artifacts only exist after the
...
the NONOS user target with no source patched (`Makefile:548`, `Makefile:552`, `Makefile:554`), but until it
...
capsule are laid out on the [contributing](contributing.md) page.
...
| Kernel spawn mirror | Written, feature-gated | `src/userspace/capsule_ripgrep/spawn.rs:47` |
| Kernel embed of `rg` artifacts | Written, needs signed binary | `src/userspace/capsule_ripgrep/embed.rs:18`, `embed.rs:19` |
| Feature flag | Declared, off by default | `Cargo.toml:69`, `src/userspace/init/entry.rs:62` |
| Boot marker | Only under the feature | `src/userspace/init/entry.rs:65` |
...
[contributing](contributing.md) page are done: build the upstream binary, reconcile the prebuilt path, sign
