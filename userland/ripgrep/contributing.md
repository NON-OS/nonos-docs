# Contributing to capsule_ripgrep

This page is for a contributor who wants to turn the ripgrep contract into a capsule that actually runs.
Read the [overview](README.md) first: the capsule is a defined contract with no implementation yet, so the
...
| Kernel spawn mirror | Written, feature-gated | `src/userspace/capsule_ripgrep/spawn.rs:33` |
| Feature flag | Declared | `Cargo.toml:69` (`nonos-capsule-ripgrep`) |
| Upstream `rg` build recipe | Written | `Makefile:548`..`Makefile:558` |

The intent recorded in `Capsule.mk` is not to write a NONOS ripgrep by hand. It names unmodified upstream
...
1. Build the upstream binary. The Makefile target `nonos-mk-upstream-ripgrep` runs
   `cargo install ripgrep --version 14.1.1` against the NONOS user target, linked to the NONOS runtime
...
3. Sign it. The prebuilt binary is not trusted until it is signed against the baked trust anchor and the
   ...
   the binary name `rg` (`src/userspace/capsule_ripgrep/embed.rs:22`, `embed.rs:26`, `embed.rs:30`), so
...
4. Turn on the feature. The spawn call is compiled in only under `nonos-capsule-ripgrep`; without it
   `run_ripgrep` is an empty function and the capsule never spawns (`src/userspace/init/entry.rs:62`,
...
`src/userspace/capsule_ripgrep/`, and the feature wiring in `src/userspace/init/entry.rs` and `Cargo.toml`.
