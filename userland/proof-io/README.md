...
[what-it-proves](what-it-proves.md) walk-through, a [contributing](contributing.md) page, and a
...
and a `FAIL` marker (`userland/capsule_proof_io/src/main.rs:37`):
...
[what-it-proves](what-it-proves.md) page walks each assertion against the kernel error path it depends on.

## Code pillars

The source is one file, `userland/capsule_proof_io/src/main.rs`, but it has three clear parts:
...
wrappers would refuse to send (`userland/libc/src/lib.rs:81`).

## Lifecycle and how it reports

`proof_io` is spawned through [verified spawn](../../security/capsules-and-trust.md): its id cert,
manifest, and attestation trailer are checked, its requested `0x19` is held against its manifest ceiling,
and only then is its ELF mapped (`src/userspace/capsule_proof_io/spawn.rs:35`). The boot entry calls
`spawn_proof_io_capsule` under the `nonos-user-entry-proof` feature (`src/userspace/init/entry.rs:43`).
