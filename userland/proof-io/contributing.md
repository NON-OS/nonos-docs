# Contributing to proof_io

This page is the map a change follows: where the source is, how it is laid out, how to build and sign it,
and the standards a patch has to meet. For the identity and lifecycle read the [overview](README.md); for
the guarantee the capsule asserts read [what-it-proves.md](what-it-proves.md).

## Module map

`proof_io` is one file, `userland/capsule_proof_io/src/main.rs`, and it stays one file on purpose: the
...
with numbers the safe wrappers would refuse to send (`userland/libc/src/lib.rs:81`).
...
and nothing else (`src/userspace/capsule_proof_io/spawn.rs:49`).
...
`spawn_proof_io_capsule` under the `nonos-user-entry-proof` feature (`src/userspace/init/entry.rs:41`).
