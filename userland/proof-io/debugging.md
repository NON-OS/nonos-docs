# Debugging with proof_io

`proof_io` is itself a diagnostic, so debugging it is short: it reports on two channels, and every outcome
maps to one line in `_start`. For the identity read the [overview](README.md); for the guarantee behind
each check read [what-it-proves.md](what-it-proves.md).
...
empty `debug_tag` (`src/userspace/capsule_proof_io/spawn.rs:52`); the empty tag is only ever printed on an
ELF-load error (`src/kernel_core/process_spawn/capsule_spawn/runner/install/load_elf_into_pid.rs:28`). The
...
yielding `ENOSYS`, so read `lookup_id` and the errno constant (`src/syscall/abi/mod.rs:31`,
`src/syscall/microkernel/errnos.rs:30`). A `FAIL invalid-pointer` or `FAIL invalid-size` means the
`MkDebug` handler stopped guarding its user buffer or its length, so read `sys_mk_debug`
(`src/syscall/microkernel/debug.rs:37`, `src/syscall/microkernel/debug.rs:40`).
...
`src/userspace/init/entry.rs:41`). If the feature is on and there is still no line, the spawn itself was
...
(`src/userspace/capsule_proof_io/spawn.rs:35`).
