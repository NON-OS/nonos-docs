...
`userland/capsule_proof_io/src/main.rs`; this page walks each assertion against the kernel path it
depends on. For the capsule's identity and lifecycle read the [overview](README.md).
...
over the real syscall (`userland/libc/src/lib.rs:82`). Running it 1024 times exercises the full
...
(`userland/libc/src/lib.rs:81`). On the kernel side, an unknown tag has no registry entry, so
`lookup_id` returns `None` (`src/syscall/abi/mod.rs:31`) and the number never resolves to a
`SyscallNumber` (`src/syscall/numbers/convert.rs:22`). The kernel errno for a missing call is
`ERRNO_NOSYS = -38` (`src/syscall/microkernel/errnos.rs:30`), which is exactly the value the proof
...
which is `-14` (`src/syscall/microkernel/debug.rs:40`, `src/syscall/microkernel/errnos.rs:26`). This is
...
anything longer, before it ever touches the buffer (`src/syscall/microkernel/debug.rs:30`,
`src/syscall/microkernel/debug.rs:37`). `257` is one byte over the ceiling, so it is the tightest test
of the bound. `ERRNO_INVAL` is `-22` (`src/syscall/microkernel/errnos.rs:29`). This is what stops a
...
number and yields `ERRNO_NOSYS = -38` (`src/syscall/abi/mod.rs:31`, `src/syscall/microkernel/errnos.rs:30`).
