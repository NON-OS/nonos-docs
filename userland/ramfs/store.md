...
(`src/store/types.rs:29`). A `File` is not a plain byte buffer. It carries its own key material and its
ciphertext (`src/store/types.rs:23`):
...
The three lengths come from `src/store/crypto/constants.rs`: `KEY_LEN = 32`, `NONCE_LEN = 12`, and
`TAG_LEN = 16` (`src/store/crypto/constants.rs:17`). An empty file has an empty `ciphertext` vector, which
...
(`src/store/state.rs:25`). `contains` is a plain map lookup (`src/store/state.rs:29`).
...
(`src/store/state.rs:33`). The key is drawn once, here, and never rotated for the life of that file. The
...
(`src/store/crypto/fresh_key.rs:22`). `fresh_nonce` does the same for 12 bytes
(`src/store/crypto/fresh_nonce.rs:22`). There is no seed baked into the capsule and no key derivation from
...
`ALGO_CHACHA20_POLY1305 = 0` (`src/store/crypto/constants.rs:16`).

`seal` calls `crypto_encrypt(algo, key, nonce, plain, plain_len, cipher)` and returns the number of
ciphertext bytes, or `CryptoFailure` on a negative return (`src/store/crypto/seal.rs:22`). `open` calls
`crypto_decrypt` with the matching arguments and returns the recovered plaintext length
(`src/store/crypto/open.rs:22`). Neither passes any associated data; the AEAD runs with no AAD. The
...
AEAD at all (`src/store/truncate.rs:24`).
...
`f.nonce = fresh_nonce()?` immediately before sealing (`src/store/write.rs:45`), and truncate does the same
(`src/store/truncate.rs:35`). The nonce stored alongside the ciphertext is always the one that produced it,
...
The file map and error type are `src/store/types.rs`; creation is `src/store/state.rs`; the three
operations are `src/store/read.rs`, `src/store/write.rs`, and `src/store/truncate.rs`. The crypto wrappers
