...
returning `None` so the runner drops it silently (`src/protocol/decode.rs:19`). On success it returns a
`Request` borrowing the payload that follows the header (`src/protocol/types.rs:30`,
`src/protocol/decode.rs:29`).
...
(`src/protocol/encode.rs:21`). The status doubles as a length on the read path, where a non-negative value
...
(`src/protocol/decode.rs:32`).

## The five operations

The opcodes are fixed constants (`src/protocol/types.rs:17`):
...
Open carries two flags (`src/protocol/types.rs:23`):
...
`KERNEL_REPLY_ENDPOINT` (`src/server/runner.rs:28`). That endpoint constant is `0x1_0000_0001`, which is
`4294967297`, the value named in the reply endpoint in the manifest (`src/protocol/types.rs:26`).
...
with `EINVAL` (`src/server/dispatch.rs:32`).

## The handle table

An open returns an opaque `u64` handle, not a path. The `HandleTable` maps each handle to a path and the
pid of the process that opened it, capped at `MAX_HANDLES = 1024` (`src/handles.rs:20`). Handle ids start
at 1 and advance with `wrapping_add`, so id 0 is never issued (`src/handles.rs:41`).

Ownership is enforced on every use. `path_for` and `remove` reject a handle whose stored owner pid does not
match the sender, returning `HandleError::Denied` (`src/handles.rs:49`, `src/handles.rs:58`). One process
...
rejects non-UTF-8 paths with `EINVAL` (`src/server/handlers/open.rs:47`). If the file is absent and
`OPEN_FLAG_CREATE` is not set it answers `ENOENT`; otherwise it creates the file through `store.ensure`,
mapping a store error to `EIO` (`src/server/handlers/open.rs:51`). If `OPEN_FLAG_TRUNCATE` is set it
truncates to zero. It then inserts a handle owned by the sender and returns the id, or `EMFILE` if the
table is full (`src/server/handlers/open.rs:62`).

Read parses handle, offset, and count, resolves the path under ownership, and calls `store.read_at`. The
returned bytes are the payload and their length is the status (`src/server/handlers/read.rs:46`).

Write parses handle and offset, treats the remainder of the payload as the data, resolves the path, and
calls `store.write_at`. The status is the number of bytes written (`src/server/handlers/write.rs:47`).

Truncate parses handle and length, resolves the path, and calls `store.truncate`
(`src/server/handlers/truncate.rs:46`).

Close parses the handle and removes it from the table, which also enforces ownership
(`src/server/handlers/close.rs:30`).

## The errno set

The status field carries a small fixed set of signed errno values (`src/protocol/errno.rs:17`):
...
respectively by the read, write, and truncate handlers (`src/store/types.rs:33`).
...
(`runner.rs`, `dispatch.rs`, `handlers/`). The handle table is `src/handles.rs`. The store and crypto
symbols named here are defined under `src/store/`, documented in [store.md](store.md). Every reference above
