# Contributing to the ramfs capsule
...
[operations.md](operations.md) and [store.md](store.md) first; this page assumes you know the layout.
...
| handle ownership or limits | `src/handles.rs` | nothing else |
...
1. Assign the next opcode in `src/protocol/types.rs` and add it to the `pub use` in
   `src/protocol/mod.rs`.
2. Add the matching constant and encoder in the kernel mirror `src/fs/ramfs_capsule/protocol/`.
3. Add a handler under `src/server/handlers/`, re-export it from `handlers/mod.rs`, and add the arm to the
   match in `src/server/dispatch.rs`.
...
(`src/main.rs:17`). Use `core` and `alloc` only. Keep files small and single-purpose, the way the tree
...
`src/handles.rs` and the entry point is `src/main.rs`. Every reference above is verified against those
trees.
