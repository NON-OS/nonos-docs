...
Read [operations.md](operations.md) for the frame layout and the full errno set.

## Read the status field first

On any operation, the four bytes at offset 4 of the reply are a signed `i32` status
(`src/protocol/encode.rs:21`). A non-negative value is success, and on read and write it is also a length.
A negative value is one of five errno codes (`src/protocol/errno.rs:17`):
...
`decode_request` returned `None` and the loop skipped it (`src/protocol/decode.rs:23`); or the IPC receive
itself returned a non-positive length and was skipped (`src/server/runner.rs:35`). Check that the sender is
...
(`src/server/handlers/open.rs:33`), read needs 20 (`src/server/handlers/read.rs:26`), write and truncate
need 16 (`src/server/handlers/write.rs:30`, `src/server/handlers/truncate.rs:30`), and close needs 8
(`src/server/handlers/close.rs:23`). A path that is not valid UTF-8 also yields `EINVAL` on open
(`src/server/handlers/open.rs:47`). An opcode outside 1 through 5 falls through the dispatch match to
`EINVAL` (`src/server/dispatch.rs:38`).
...
truncate, and close (`src/handles.rs:49`). `EACCES` means the sender pid on the request does not match the
...
`EIO` is always a `StoreError::CryptoFailure` (`src/store/types.rs:33`), and there are exactly three places
...
- key or nonce generation failed: `fresh_key` or `fresh_nonce` got a negative `crypto_random`
  (`src/store/crypto/fresh_key.rs:24`, `src/store/crypto/fresh_nonce.rs:24`).
- a seal failed: `crypto_encrypt` returned negative (`src/store/crypto/seal.rs:36`).
- an open failed: `crypto_decrypt` returned negative (`src/store/crypto/open.rs:36`).
...
when the file's ciphertext is empty (`src/store/read.rs:26`), when the offset is at or past the plaintext
length (`src/store/read.rs:32`), or when the clamped window is empty. Check the offset and count against the
actual file size, remembering that a fresh `ensure` leaves the file empty until the first write.

## Short writes and file growth

Write returns the length of the data it was handed, not the new file size (`src/store/write.rs:50`). If a
write extends past the current end the store zero-fills the gap before splicing (`src/store/write.rs:41`),
so a write at a high offset silently grows the file. This is intended; it is not a bug to chase.

## Source map

The status and errno definitions are `src/protocol/encode.rs` and `src/protocol/errno.rs`. The decode and
receive guards are `src/protocol/decode.rs` and `src/server/runner.rs`. The handler length checks are under
`src/server/handlers/`. Ownership is `src/handles.rs`. The crypto failure sites are `src/store/` and
