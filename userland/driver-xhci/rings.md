# The TRB rings, doorbells, and the event engine
...
on the [bring-up](bring-up.md) page; how enumeration drives them is on the [enumeration](enumeration.md)
page; the wire surface above them is on the [operations](operations.md) page.

## The TRB

A TRB is a 16-byte, 16-aligned block of four little-endian dwords (`src/trb/base.rs:16`). Everything else is
...
`get_type` reads the six type bits out of `d3` (`src/trb/get_type.rs:19`, shift `TRB_TYPE_SHIFT = 10`,
mask `TRB_TYPE_MASK`, `src/constants/trb_flags.rs:19`); `set_type` writes them (`src/trb/set_type.rs`).
`get_cycle` / `set_cycle` handle the low bit `TRB_CYCLE` (`src/constants/trb_flags.rs:16`); `get_pointer`
recombines `d0` and `d1` into a 64-bit address (`src/trb/get_pointer.rs:18`), and `set_pointer` splits one.
`completion_code` reads the high byte of `d2`, and `slot_id` reads the high byte of `d3`. Ring memory is
touched only through `read_volatile_at` and `write_volatile_at` (`src/trb/mod.rs:32`), never a plain load
or store, because it is DMA memory the controller reads and writes concurrently.

The TRB type numbers the driver uses are named constants (`src/constants/trb_kinds.rs:16`): Normal `1`,
...
The completion codes are two constants: Success `1` and Short Packet `13` (`src/constants/completion_codes.rs:16`).
...
it follows the link), and the current cycle (`src/trb/builders/link/build.rs:18`,
`src/constants/trb_flags.rs:21`). The rings never actually run into the link at runtime because enqueue
stops one slot short, but the link is what makes the ring circular for the controller.

## The command ring

`CommandRing` is a 64-TRB DMA region, a cycle byte, and an enqueue index (`src/rings/command/state.rs:21`,
`COMMAND_RING_TRBS = 64`, `src/constants/ring.rs:17`). `new` allocates and zeroes the region and writes a
...
(`src/rings/command/enqueue.rs:22`). When the index reaches the last data slot it rewrites the link with the
...
(`src/rings/command/crcr_value.rs:18`).
...
during bring-up (`src/controller/issue_noop_and_wait.rs:22`), and Enable Slot, Disable Slot, Address
...
(`src/controller/ring_doorbell.rs:17`). Doorbell 0 is the command ring; its target is always 0. A device's
...
running drained total (`src/rings/event/state.rs:20`, `EVENT_RING_SEGMENT_TRBS = 64`,
`EVENT_RING_SEGMENT_TABLE_ENTRIES = 1`, `src/constants/ring.rs:18`). `new` allocates both regions, zeroes
...
sets `IMAN.IE` (`src/controller/program_event_ring.rs:19`).
...
(`src/regs/runtime/interrupter_addr.rs:17`, `INTERRUPTER_STRIDE = 0x20`,
`src/constants/runtime_offsets.rs:16`). Within an interrupter: `IMAN` at `0x00`, `IMOD` at `0x04`, `ERSTSZ`
at `0x08`, `ERSTBA` at `0x10`, `ERDP` at `0x18` (`src/constants/runtime_offsets.rs:17`). `erdp_program`
writes the current dequeue device address, low three bits reserved for the segment index, with the
Event-Handler-Busy bit set when clearing (`ERDP_EHB`, `src/regs/runtime/erdp_program.rs:18`,
`src/constants/erdp_bits.rs:16`).

`has_event` reads the TRB at the dequeue index and returns true when its cycle bit equals the consumer
cycle (`src/rings/event/has_event.rs:18`). `advance` zeroes the consumed slot, bumps the drained total,
advances the dequeue index, and flips the consumer cycle on wrap (`src/rings/event/advance.rs:20`).
`current_dequeue_phys` is the device address to write back into `ERDP` after consuming
(`src/rings/event/current_dequeue_phys.rs:19`).
...
(`src/controller/wait_command_completion.rs:25`). `wait_transfer_completion` is the same shape for a
Transfer Event, accepting Success or Short Packet as success (`src/controller/wait_transfer_completion.rs:21`).
Both time out into a distinct error the [debugging](debugging.md) page maps.
...
(`src/server/service_interrupts.rs:19`). `drain_events` pulls up to `DRAIN_BATCH = 32` events, but stops
early on a Transfer Event or Command Completion Event, because those belong to a blocking waiter and must
not be swallowed here (`reserved_for_waiter`, `src/controller/drain_events.rs:30`). It rewrites `ERDP`
once at the end. `ack_irq` clears the interrupt-pending bit in `IMAN` if set and calls `mk_irq_ack` on the
grant so the next MSI-X can be delivered (`src/controller/ack_irq.rs:19`, `IMAN_IP`,
`src/constants/interrupter_bits.rs:16`).
