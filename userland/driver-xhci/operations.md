# xHCI client operations and the IPC surface
...
[bring-up](bring-up.md); for the ring and TRB machinery every op stands on see [rings](rings.md); for the
slot and descriptor work behind the enumeration ops see [enumeration](enumeration.md). Identity and the
capability mask are on the [README](README.md).

## The message format

Every request and reply carries a 20-byte header (`HDR_LEN`, `src/protocol/header.rs:18`) followed by a
...
(`src/protocol/decode.rs:17`); `encode_response_header` writes the same shape back into the reply, echoing
the request op, flags, and request id and zeroing the reserved word (`src/protocol/encode.rs:17`).

A reply is always the 20-byte header, then a 4-byte little-endian status word (`STATUS_LEN`,
`src/protocol/limits.rs:16`), then any op-specific body. `write_status` lays the status word down
(`encode.rs:26`), and the status value is one of the errno constants in `src/protocol/errno.rs`: `0`
success, `E_INVAL = -22`, `E_IO = -5`, `E_NODEV = -19`, `E_AGAIN = -11` (`errno.rs:16`). The fixed body
lengths every handler uses are named constants in `src/protocol/limits.rs`, so the wire layout is defined
in one place rather than scattered as literals.

## The server loop

`server::run` is a single strictly sequential loop (`src/server/runner.rs:29`). It sizes a receive buffer
...
1. `service_interrupts` drains the event ring and acks the interrupter before touching IPC
   (`runner.rs:34`, covered under [rings](rings.md)).
...
`mk_ipc_send` to the fixed `KERNEL_REPLY_ENDPOINT` `0x1_0000_000B` (`src/server/reply.rs:37`).

`dispatch` is a match on `req.op` (`src/server/dispatch.rs:25`). The four empty-body status ops are gated
...
`reply::send` on success (`src/server/error.rs:19`).

## The operations

Opcodes are defined in `src/protocol/ops.rs:16`. All lengths below are body bytes after the status word,
...
- `OP_CONTROLLER_STATUS` reads `USBSTS`, `USBCMD`, and `IMAN` live and packs a 56-byte snapshot: max slots
  ...
  slots (`src/server/handlers/controller_status.rs:23`; `CONTROLLER_STATUS_PAYLOAD_LEN = 56`,
...
- `OP_PORT_STATUS` walks ports `1..=max_ports`, capped at `MAX_PORTS_REPORTED = 255`, reading each `PORTSC`
  ...
  the raw `PORTSC` word) (`src/server/handlers/port_status.rs:22`). Clearing the change bits on read is
...
- `OP_ENABLE_SLOT` issues an Enable Slot command, waits for its completion, and marks the returned slot id
  ...
  returns `E_NODEV` (`src/server/handlers/enable_slot.rs:23`). A command failure is `E_IO`.
- `OP_DISABLE_SLOT` validates the slot is allocated, issues Disable Slot, and on success, if the slot was
  ...
  (`src/server/handlers/disable_slot.rs:20`). An unallocated slot id is `E_INVAL`.
- `OP_ADDRESS_DEVICE` requires the slot allocated but not yet addressed and the port in range
  (`src/server/handlers/address_flow/slot_ready.rs:18`), resets the root port, then runs the Address
  Device flow and replies with slot, port, the xHCI speed id, and the EP0 max packet size
  (`src/server/handlers/address_device.rs:21`, `src/server/handlers/address_reply.rs:20`). Covered in
  detail on the [enumeration](enumeration.md) page.
...
- `OP_GET_CONFIG_DESCRIPTOR` accepts only index `0` and rejects a zero slot (`handle.rs:24`), clamps the
  ...
  `src/server/handlers/config_descriptor/handle.rs:23`, `.../reply.rs:22`).
- `OP_CONTROL_TRANSFER` is the general EP0 control passthrough: it unpacks a USB setup packet
  ...
  read (`src/server/handlers/control_transfer/handle.rs:22`, `.../reply.rs:22`).
...
- `OP_INTERRUPT_IN` polls a slot's interrupt-IN endpoint for one report up to `HID_REPORT_MAX = 8` bytes; a
  ...
  a HID capsule can poll without blocking the driver (`src/server/handlers/interrupt_in.rs:22`,
  `src/server/handlers/interrupt_in/reply.rs:23`).

## The handler layout

Handlers live under `src/server/handlers/`, one module per op (`src/server/handlers/mod.rs:16`). A simple
...
covered on the [enumeration](enumeration.md) page. Each handler takes the `Context` (which owns the one
`Driver`, `src/server/context.rs:17`), the decoded `Request`, the body slice, and the transmit buffer.
...
and the local table before it indexes anything (`src/slots/table/valid.rs:16`), so a caller cannot walk
