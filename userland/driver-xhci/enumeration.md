# Device slots, addressing, and transfers
...
`src/trb/builders/`. The client ops that drive all this are described on the [operations](operations.md)
page, which covers the request and reply bytes; this page covers the internals behind them. The rings and
TRBs everything stands on are on the [rings](rings.md) page.

## The slot table

`SlotTable` holds two 256-wide boolean arrays (allocated and addressed), a vector of per-slot resources,
and a count (`src/slots/table/types.rs:21`, `XHCI_SLOT_TABLE_LEN = 256`). A slot id is valid only when it
is non-zero and no greater than the controller's `max_slots` (`src/slots/table/valid.rs:16`), so a request
...
controller error (`src/controller/issue_enable_slot.rs:22`). The handler then marks the id allocated in the
table, and if the table refuses it (already set), issues a compensating Disable Slot and returns `E_NODEV`
(`src/server/handlers/enable_slot.rs:36`). The device slot exists at this point but has no context and no
address.

## Reset, speed, and the Address Device flow

`OP_ADDRESS_DEVICE` is large enough to be its own subtree, `address_flow/`
(`src/server/handlers/address_flow/mod.rs:16`). The handler first checks `slot_ready`: the port is in
range, the slot is allocated, and it is not yet addressed (`src/server/handlers/address_flow/slot_ready.rs:18`).
...
1. **Reset the port.** `reset_port` first asserts Port Power if the firmware left it off and spins for the
   ...
   nothing connects (`src/controller/reset_port.rs:26`, `power_on` at line 50). It then clears the port's
...
2. **Read the speed.** `port_speed` extracts the four speed bits from `PORTSC`
   (`src/server/handlers/address_flow/port_speed.rs:18`). A zero speed is a device that went away, and
   `address_after_reset` returns `E_NODEV` (`src/server/handlers/address_flow/address_after_reset.rs:30`).
3. **Allocate resources.** `alloc_resources` calls `SlotResources::allocate`, which allocates the output
   ...
   (`src/server/handlers/address_flow/alloc_resources.rs:20`, `src/slots/resources.rs:36`).
4. **Program the DCBAA and run the command.** `complete_address` writes the output-context device address
   ...
   (`src/server/handlers/address_flow/complete_address.rs:24`,
   `src/server/handlers/address_flow/command_address.rs:21`, `src/controller/issue_address_device.rs:22`).
...
5. **Attach and reply.** `attach_resources` records the slot as addressed and stores its resources, or
   ...
   port, speed, and EP0 max packet size (`src/server/handlers/address_flow/attach_resources.rs:23`,
   `src/server/handlers/address_reply.rs:20`).
...
(`src/regs/cap/context_size.rs:19`). A device (output) context is 32 context slots; an input context is 33
(a leading Input Control context plus 32) (`src/contexts/size.rs:16`).

`write_address_device_input` builds the input context for Address Device
(`src/contexts/input.rs:21`): the Input Control context adds the Slot and EP0 contexts (`ADD_SLOT_AND_EP0 =
...
SuperSpeed (speeds 4 and 5), 64 for High Speed (3), 8 otherwise (`src/contexts/ep0.rs:16`).

`write_configure_endpoint_input` builds the input context for Configure Endpoint
(`src/contexts/configure_ep.rs:26`): the Input Control context adds the slot and the endpoint's DCI, the
...
DCI 2, endpoint 1 IN is DCI 3, and so on (`src/slots/dci.rs:16`). Every endpoint doorbell target and every
...
no-data request just Setup and Status (`src/controller/issue_control_transfer.rs:27`, the `direction`
helper at line 56). The Setup Stage TRB packs the setup packet into `d0`/`d1`, the transfer type into `d2`,
and sets the Immediate Data flag (`src/trb/builders/setup_stage_generic.rs:24`). This path backs
...
(`src/controller/get_device_descriptor.rs:26`). The configuration-descriptor and general control-transfer
handlers build on the same three-stage machinery with a caller-sized data buffer, described in the op table
on the [operations](operations.md) page. The driver copies the raw descriptor bytes back and never parses
...
input context, and run the Configure Endpoint command (`src/server/handlers/alloc_transfer_ring/configure.rs:23`,
`src/controller/issue_configure_endpoint.rs:22`). The handler converts the endpoint address to a DCI and
returns it (`src/server/handlers/alloc_transfer_ring/handle.rs:23`).
...
endpoint's doorbell (`src/controller/poll_interrupt_in/mod.rs:25`,
`src/controller/poll_interrupt_in/arm.rs:20`). `scan` then checks the event ring for a Transfer Event
matching the armed TRB: if present it consumes it, disarms, and returns the transferred byte count computed
from the residual; if absent it returns `Pending` (`src/controller/poll_interrupt_in/scan.rs:23`). The
handler turns `Pending` into an `E_AGAIN` status so a HID capsule can poll in a loop without the driver
ever blocking on a device that has nothing to report (`src/server/handlers/interrupt_in.rs:22`).
...
indexes anything (`src/slots/table/valid.rs:16`, `slot_ready`), so a caller cannot walk off the slot arrays
