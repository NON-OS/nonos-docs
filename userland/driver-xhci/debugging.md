...
devices enumerating. For the bring-up sequence itself see [bring-up](bring-up.md); for the ring machinery
the errors come from see [rings](rings.md); for the enumeration path see [enumeration](enumeration.md).

## The bring-up markers

Bring-up narrates its progress through `mk_debug` markers, printed in order as each stage clears
(`src/setup/marker.rs:17`). They land on the serial console, or the framebuffer on a `NONOS_FBCONSOLE=1`
...
process exit code is the negated errno for that failure (`src/main.rs:40`,
`src/error/errno_value.rs:23`). A trace that reaches `running` but never `noop ok` means the controller
started but the command ring, doorbell, event ring, or interrupter is not carrying completions, which is
the machinery on the [rings](rings.md) page.

## The error variants

Every bring-up and runtime failure is one `XhciError` (`src/error/xhci_error.rs:16`), and the mapping to an
exit errno is explicit (`src/error/errno_value.rs:23`). The distinct timeouts are the useful ones, because
...
(`src/controller/wait_command_completion.rs:44`).
...
capability before it resets (noted on the [bring-up](bring-up.md) page), so a controller the BIOS still
...
length (`src/setup/layout.rs:25`, `src/setup/require_window.rs:18`), so a controller whose capability
...
(`src/setup/mmio_map.rs:19`); a controller whose runtime block sits above that window would fail the guard.
The [MMIO debugging](../../subsystems/hardware-broker/mmio.md) markers show how far the mapping itself got.

## No ports, or a port with no device

A `NoDeviceOnPort` from `reset_port` means the port never asserted `PORTSC.CCS`
(`src/controller/reset_port.rs:28`). On QEMU this is almost always a device that is not attached to that
...
(`src/controller/reset_port.rs:50`); a real controller that needs a longer settle than that budget will
...
(`src/controller/issue_enable_slot.rs:33`). An Address Device that fails leaves the slot unaddressed and
rolls back its DCBAA entry (`src/server/handlers/address_flow/complete_address.rs:37`). A descriptor fetch
...
cycle-and-pointer poll, not the interrupt (see [rings](rings.md)), so a stuck transfer is the event ring
not advancing, which is the interrupter routing or the controller not writing completions. The
[IRQ debugging](../../subsystems/hardware-broker/irq.md) section covers the "bound but never fires" case:
...
each loop pass (`src/setup/irq_bind.rs:23`, `src/server/service_interrupts.rs:19`). And the same header's
capability comment spells the mask as `IPC|Memory|Driver|DeviceEnum|Mmio|Irq|Dma` (`Capsule.mk:15`),
omitting the `CoreExec` bit that `0xF8019` also sets, as the [README](README.md) mask decomposition shows.
