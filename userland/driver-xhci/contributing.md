# Contributing to capsule_driver_xhci
...
[README](README.md), the [operations](operations.md) surface, and the [bring-up](bring-up.md),
[rings](rings.md), and [enumeration](enumeration.md) pages in this folder.
...
server loop (`src/main.rs:36`). The thirteen top-level modules are declared there (`src/main.rs:19`). It is
...
| `src/discover.rs` | the PCI match for the controller | you change which devices the driver will bind |
...
1. Assign the opcode in `src/protocol/ops.rs:16`, next to the existing ones. Reuse one of the currently
   unassigned values (`0x000A`, `0x000C`, `0x000D`) rather than inventing a gap; they fall through the
   dispatch default today.
2. Define the fixed body lengths in `src/protocol/limits.rs`, both the request body length and any reply
...
4. Wire it into `dispatch` (`src/server/dispatch.rs:25`). A no-body status op is gated with
...
5. Register the op and its bytes on the [operations](operations.md) page so the wire contract stays
...
- No panics in capsule code. No `unwrap`, `expect`, or `panic!` on a runtime path. Every fallible step
  returns an `XhciError` (`src/error/xhci_error.rs:16`) that maps to an errno at the boundary
  (`src/error/errno_value.rs:23`), and every handler replies with a status word rather than aborting.
