...
the controller from halted to running. That whole path is one ordered sequence in `src/setup/sequence.rs:34`,
and this page mirrors it together with the folders it leans on: `src/setup/` (the sequence and the broker
calls), `src/discover.rs` (finding the device), `src/handles/` (the grant wrappers), `src/dma/` (the DMA
pool), and the `src/controller/` steps the sequence calls. The ring and TRB machinery the last steps drive
is on the [rings](rings.md) page; the per-device slot work that comes later is on the
[enumeration](enumeration.md) page; the wire surface the finished driver serves is on the
[operations](operations.md) page. Identity and the capability mask are on the [README](README.md).
...
`_start` negates into the process exit code (`src/main.rs:40`, `src/error/errno_value.rs:23`), so a
...
1. **Discover.** `find_xhci` calls `mk_device_list` and returns the first record whose broker class is
   ...
   non-empty MMIO BAR0 (`src/discover.rs:27`, `src/discover.rs:49`). It carries forward the device id and
   BAR0 size only. No match is `DeviceNotFound`.
2. **Claim.** `mk_device_claim` on that device id returns a claim epoch (`src/setup/claim.rs:18`). The
   epoch is the token every later broker call must present, so a stale or revoked claim fails cleanly. A
   refusal is `BrokerCallFailed`. See [device claim and epochs](../../subsystems/hardware-broker/claim.md).
3. **Bus master.** `enable_bus_master` writes `MK_PCI_CMD_BUS_MASTER` into the PCI command register through
   `mk_pci_config_write` so the controller may DMA (`src/setup/pci.rs:21`). On failure the device is
   released before the error propagates (`src/setup/sequence.rs:37`).
4. **Map the register window.** `mmio_map` maps BAR index 0, clamped to `REGISTER_WINDOW_LEN = 0x3000`
   ...
   id (`src/setup/mmio_map.rs:18`). The broker withholds the MSI-X table pages from this mapping; see
...
5. **Bind MSI-X.** `irq_bind` calls `mk_irq_bind` with `MK_IRQ_BIND_MSIX` for one vector
   (`src/setup/irq_bind.rs:23`). On failure it rolls back by unmapping the MMIO grant and releasing the
   device before returning `BrokerCallFailed` (`src/setup/irq_bind.rs:25`). This is the point the
   ...
   acknowledges the interrupter every loop pass. The [debugging](debugging.md) page records that
   discrepancy. See [IRQ grants](../../subsystems/hardware-broker/irq.md).
6. **Read the layout.** `read_layout` first calls `refuse_unsupported`, which requires 64-bit addressing
   (`AC64`) and a non-zero `max_slots` before the mapping is trusted (`src/controller/refuse_unsupported.rs:18`).
   It then reads `CAPLENGTH`, `RTSOFF`, and `DBOFF` and checks each derived block start with a one-page
   guard against the mapped length (`require_window`, `src/setup/require_window.rs:18`), producing the
   operational base, doorbell base, primary interrupter base, and the `max_slots` / `max_ports` /
   `max_scratchpad` / `context_size` fields (`src/setup/layout.rs:25`).
7. **Halt.** `halt` clears `USBCMD.RUN` if it is set and polls `USBSTS.HCH` high, bailing after
   `200_000` spins with `HaltTimeout` (`src/controller/halt.rs:20`).
8. **Reset.** `reset` sets `USBCMD.HCRST` and polls it back to zero, `ResetTimeout` on expiry
   (`src/controller/reset.rs:20`). Then `wait_cnr_clear` polls `USBSTS.CNR` (controller-not-ready) low
   before any structure is programmed (`src/controller/wait_cnr_clear.rs:20`).
9. **Scratchpads.** `Scratchpads::allocate` reads `max_scratchpad` and, if non-zero, allocates a pointer
   ...
   the array (`src/controller/scratchpad.rs:26`). A zero count is `Scratchpads::None`. These are the
...
10. **DCBAA.** `program_dcbaa` allocates `(max_slots + 1) * 8` bytes, writes the scratchpad array device
    ...
    to `max_slots` (`src/controller/program_dcbaa.rs:20`, `src/regs/op/dcbaap_program.rs:18`). The Device
...
11. **Command ring.** `CommandRing::new` allocates the 64-TRB ring, writes a wrap link TRB into its last
    ...
    bit (`src/rings/command/state.rs:27`, `src/controller/program_command_ring.rs:18`). Covered on the
    [rings](rings.md) page.
12. **Event ring.** `EventRing::new` allocates the 64-TRB segment and a one-entry ERST, `imod_program`
    ...
    `ERSTBA` and sets `IMAN.IE` (`src/rings/event/state.rs:28`, `src/controller/program_event_ring.rs:19`,
    `src/setup/sequence.rs:60`). Also on the [rings](rings.md) page.
13. **Start.** `start` clears `USBSTS.HSE` and sets `USBCMD.RUN | USBCMD.INTE`
    (`src/controller/start.rs:18`), then `wait_hc_running` polls `USBSTS.HCH` low, `StartTimeout` on expiry
    (`src/controller/wait_hc_running.rs:20`).
14. **No-op probe.** `issue_noop_and_wait` enqueues a No Op command TRB, rings doorbell 0, and waits for its
    completion event (`src/controller/issue_noop_and_wait.rs:22`). This is the end-to-end proof that the
...
slot table (`src/setup/assemble.rs:24`, `src/setup/driver.rs:22`).
...
BIOS-owned bit before the reset in step 8. The [debugging](debugging.md) page notes this as the first
...
|---|---|---|
| Device claim | `BrokerHandles` | the exclusive hold on the PCI function; the root of every other grant (`src/handles/broker_handles.rs:17`) |
| MMIO | `BrokerHandles` | the capability/operational/runtime window mapped as a user VA (`src/setup/mmio_map.rs:20`) |
| IRQ | `BrokerHandles` | one MSI-X vector (`src/setup/irq_bind.rs:21`) |
| DMA | `DmaRegion` | one broker-issued buffer with a user VA and a device address (`src/dma/region.rs:16`) |

The `DmaPool` is just the device id and the claim epoch (`src/dma/pool.rs:17`); every ring, context, and
...
device address (`src/dma/pool_alloc.rs:22`). The capsule programs the controller only with the
broker-issued device addresses (`src/regs/op/dcbaap_program.rs:18`, `src/rings/command/crcr_value.rs:18`),
never a physical address it chose. The USB-host DMA class ceiling is 256 pages, matching the mask's `Dma`
authority; see the ceiling table on [DMA grants](../../subsystems/hardware-broker/dma.md).
...
(`src/handles/broker_handles_drop.rs:18`). Because the server loop never returns, this runs only on an
...
xHCI function it claimed (`src/setup/claim.rs:18`;
[claim.md](../../subsystems/hardware-broker/claim.md)). The MMIO grant is the register window of that
...
(`src/setup/mmio_map.rs:20`; [mmio.md](../../subsystems/hardware-broker/mmio.md)). Each DMA region is a
separate grant with its own device address, capped at the USB-host class ceiling, and the controller is
programmed only with those device addresses (`src/dma/pool_alloc.rs:22`;
[dma.md](../../subsystems/hardware-broker/dma.md)). And the interrupt is programmed by the kernel: the
capsule receives a grant id and a vector, never the MSI-X table
(`src/setup/irq_bind.rs:23`; [irq.md](../../subsystems/hardware-broker/irq.md)).

The honest caveat is the absence of an IOMMU on the current target. Bus mastering is enabled
(`src/setup/pci.rs:21`) and the controller is handed device addresses for its rings, contexts, and data
