...
[frame-pacing.md](frame-pacing.md). Back to the [README](README.md).
...
`wait_for_setup` (`src/wait_for_setup.rs:25`) tries virtio first and only falls back after
`VIRTIO_ATTEMPTS_BEFORE_GOP = 6` failed attempts, yielding between tries, so a machine that has virtio-gpu
always uses it and only hardware without it takes the GOP route (`src/wait_for_setup.rs:23`, `:31`).
`setup::run_virtio` and `setup::run_gop` are the two entry points (`src/setup/mod.rs:22`).

## Virtio bring-up

`run_virtio_once` (`src/setup/prime_once.rs:25`) resolves the gfx endpoint, fetches the primary surface, and
attaches it:

1. `discover::lookup_gfx_endpoint` resolves `driver.virtio_gpu0` through `mk_service_lookup` and returns its
   port; a missing service, dead owner, or zero port is an error (`src/setup/discover.rs:19`, `:28`,
...
`run_gop_once` (`src/setup/prime_gop.rs:36`) is the no-virtio path (`prime_gop.rs:17` describes it). It
...
(`src/gfx_client/wire.rs:24`). `build_request` lays out `magic (u32), version (u16), op (u16), flags (u16),
reserved (u16), request_id (u32), payload_len (u32)` and appends the payload (`wire/build_request.rs:19`).
A call sends the request and reads the reply through `mk_ipc_call_timeout`; a non-positive return is
`Err("gfx ipc call failed")` (`wire/call_with_timeout.rs:31`, `:39`). Two timeouts are used: the boot call
uses `BOOT_REPLY_TIMEOUT_MS = 250` (`wire/call_boot.rs`, `wire.rs:28`), the per-frame calls use
`CALL_REPLY_TIMEOUT_MS = 1000` (`wire/call.rs`, `wire.rs:27`). `read_status` reads the `i32` status word
that follows the reply header (`wire/read_status.rs:17`); `payload_slice` returns the bytes after the status
(`wire/payload_slice.rs:17`).

The ops the compositor issues:

| Op | Code | Body | What it does | Source |
|----|------|------|--------------|--------|
| GET_PRIMARY_SURFACE | `0x000C` | empty | fetch handle, resource id, geometry, format | `gfx_client/get_primary.rs:22` |
| TRANSFER_TO_HOST | `0x0008` | 32 B | copy the dirty rect to the device | `gfx_client/transfer.rs:21` |
| SET_SCANOUT | `0x0009` | 24 B | bind the resource to scanout 0 (first frame) | `gfx_client/set_scanout.rs:21` |
| RESOURCE_FLUSH | `0x000A` | 20 B | flush the dirty rect to the panel | `gfx_client/flush.rs:21` |
...
In `frame_pacer::tick` (`src/frame_pacer/tick.rs:29`), after compositing and the release fence, the present
splits on `gop_mode`:
...
`ctx.scanout_error_reported` and keeps running ([frame-pacing.md](frame-pacing.md)).
...
`mk_surface_release` for the attach cache (`src/state/attach.rs:17`), `mk_surface_register` and `mk_mmap`
for the GOP backing (`src/setup/prime_gop.rs:24`), `mk_surface_present` for the GOP present
(`src/frame_pacer/tick.rs:32`), and `mk_display_vsync_wait` for pacing (`src/frame_pacer/vsync.rs:17`).
These are the same surface primitives the [graphics](../../subsystems/graphics/README.md) subsystem
describes.
</content>
