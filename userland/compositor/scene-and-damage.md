...
write this state are in [operations.md](operations.md); the pass that reads it is in
[frame-pacing.md](frame-pacing.md). Back to the [README](README.md).

## The Context

`Context` (`src/state/context.rs:19`) is the whole runtime state, built once by setup and threaded through
...
The scene table (`src/state/scene/table.rs:19`) is a fixed array of up to `MAX_LAYERS = 32` layers plus a
live count (`src/state/scene/layer.rs:17`). A `Layer` is:
...
- **submit** (`src/state/scene/submit.rs:21`) keys on `owner_pid`: if the sender already has a live layer it
...
- **z_sorted_snapshot** (`src/state/scene/snapshot.rs:21`) copies the in-use layers into a fresh array and
...
- **layers** (`src/state/scene/layers.rs:21`) yields an iterator over the in-use layers, used by
  `SCENE_REMOVE` to collect surface handles.
- **drop_by_pid** (`src/state/scene/drop_by_pid.rs:21`) clears every layer owned by a pid, decrementing the
  count with a saturating subtract, and returns how many it dropped (`drop_by_pid.rs:24`).
- **reap_unattachable** (`src/state/scene/reap_unattachable.rs:21`) is called each frame with the set of
  ...
  ([frame-pacing.md](frame-pacing.md)) consecutive frames and then vanish.

`scene_remove::remove_by_pid` (`src/state/scene_remove.rs:23`) is the helper `SCENE_REMOVE` uses: it walks
...
The `DamageAccumulator` (`src/state/damage.rs:30`) is a single screen-aligned bounding `Rect` plus a
...
The `FocusTable` (`src/state/focus.rs:22`) records one focused pid. `set` overwrites it (`focus.rs:31`); it
starts at zero (`focus.rs:27`). Both `FOCUS_SET` and `INPUT_SUBSCRIBE` write it
([cursor-and-input.md](cursor-and-input.md)). The wm owns z-order and window state; this table just records
...
The `CursorTracker` (`src/state/cursor.rs:24`) holds one `CursorState { x, y, visible }`. It is constructed
...
The `AttachCache` (`src/state/attach.rs:31`) is a `MAX_ATTACH = 32`-slot map from a surface handle to a
...
- **get_or_attach** (`src/state/attach.rs:39`) returns `None` for a zero handle, returns a cached mapping if
...
- **forget** (`src/state/attach.rs:63`) finds the slot for a handle, calls `mk_surface_release`, and clears
...
client's declared surface size ([frame-pacing.md](frame-pacing.md)). Client surface pixels are untrusted:
