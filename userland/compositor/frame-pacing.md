...
paces the loop. The present half of `tick` (the two backends) is detailed in [gpu-client.md](gpu-client.md).
The scene and damage structures the pass reads are in [scene-and-damage.md](scene-and-damage.md). Back to
the [README](README.md).

## The tick

`frame_pacer::tick` (`src/frame_pacer/tick.rs:23`) is called once per loop iteration. It drains the damage
...
[gpu-client.md](gpu-client.md); everything above the fence is this page.

## The compositing pass

`composite::paint` (`src/frame_pacer/composite.rs:26`) draws exactly the dirty rectangle:
...
[scene-and-damage.md](scene-and-damage.md)); for each layer it maps the source surface through the attach
...
[scene-and-damage.md](scene-and-damage.md)). Finally, if the cursor is visible it blits the cursor sprite,
...
declared `byte_len` through one gate, `Surface::row_start` (`src/sw_blitter/mod.rs:33`):
...
`fill_rect` clips the rectangle to the surface and fills whole rows (`src/sw_blitter/fill.rs:20`). It clamps
...
`composite_layer` (`src/sw_blitter/copy_rect.rs:20`) places a source surface at `(at_x, at_y)`, clipped to
...
volatile writes and clipped to the damage rectangle (`src/frame_pacer/cursor.rs:24`). `FG = 0xFFFFFFFF`,
...
([scene-and-damage.md](scene-and-damage.md)); the `CURSOR_UPDATE` op that feeds it is in
[cursor-and-input.md](cursor-and-input.md).

## Vsync pacing

`wait_for_vsync` (`src/frame_pacer/vsync.rs:19`) calls `mk_display_vsync_wait(0)` and returns the vblank
count, or `Err("vsync wait failed")` on a negative return (`vsync.rs:21`). The loop treats an error as a
cue to `mk_yield` rather than spin (`src/server/runner/entry.rs:35`), so a display that cannot signal vblank
