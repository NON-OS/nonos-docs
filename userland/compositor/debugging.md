# Debugging the compositor

This page lists the log markers the compositor and its boot path emit, and the concrete failure modes with
where to look for each. For what the compositor does and how it is built, read the [README](README.md), the
[operations](operations.md), the [frame pacing](frame-pacing.md), the [scene and damage](scene-and-damage.md),
the [GPU client](gpu-client.md), and the [cursor and input](cursor-and-input.md) pages in this folder.
...
presents is stuck in that acquisition (`src/wait_for_setup.rs:25`), not the loop.
...
frame comes from `mark_full` at setup (`src/setup/prime_once.rs:53`, `src/setup/prime_gop.rs:66`), so a
...
mismatch, or a stride/byte-length that does not validate, `src/setup/prime_once.rs:27` through `:51`) and
the fallback to GOP has not fired yet: it only fires after `VIRTIO_ATTEMPTS_BEFORE_GOP = 6` failed virtio
attempts (`src/wait_for_setup.rs:23`).

### A window never appears

A `SCENE_SUBMIT` with a zero-area rectangle, or one that does not fit the display, is rejected with
`E_INVAL` before a layer is created (`src/server/handlers/scene_submit.rs:49`), so a missing window is
usually a geometry rejection, not a lost frame. A submit past 32 live layers also returns `E_INVAL`
(`src/state/scene/submit.rs:28`). Confirm the client is submitting inside the geometry it read from
`DISPLAY_INFO`.
...
(`src/frame_pacer/composite.rs:24`, `:57`, `src/state/scene/reap_unattachable.rs:37`). A window that hangs
...
(`src/gfx_client/transfer.rs:44`), `"gfx scanout: driver rejected"` (`src/gfx_client/set_scanout.rs:44`),
`"gfx flush: driver rejected"` (`src/gfx_client/flush.rs:44`). A compositing loop that runs but shows
...
(`src/frame_pacer/tick.rs:49`). The loop latches the first such error once into `ctx.scanout_error_reported`
and keeps running (`src/server/runner/entry.rs:30`), so the failure will not spam the log.

### No present in GOP mode

`mk_surface_present` returning negative yields `"gop present rejected"` (`src/frame_pacer/tick.rs:32`). This
...
inside the heap VMA and never match (`src/setup/prime_gop.rs:98`, `:102`).
...
repaints it (`src/state/damage.rs:17`). That is the accumulator merging rectangles, expected in v1, not a
bug. The compositor paces to vsync and issues a release fence before each present
(`src/frame_pacer/tick.rs:28`, `src/frame_pacer/vsync.rs:19`), so genuine tearing points at the present
...
follow-up (`src/state/damage.rs:18`).
