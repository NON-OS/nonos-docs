# Contributing to the compositor
...
together, read the [README](README.md), the [operations](operations.md), the [frame pacing](frame-pacing.md),
the [scene and damage](scene-and-damage.md), the [GPU client](gpu-client.md), and the
[cursor and input](cursor-and-input.md) pages in this folder.

## Where the source lives

The capsule is at `userland/compositor/`. It is a `no_std`/`no_main` capsule: `_start` initialises the heap,
waits for setup, registers the `compositor` service, and enters `server::run` (`src/main.rs:37`). The
top-level modules are declared there (`src/main.rs:22`).
...
| `src/setup/`, `src/wait_for_setup.rs` | display bring-up and backend selection | you change how the display comes up |

## Adding or changing an op

To change what clients can request, edit the protocol and a handler:

1. Add the opcode to `src/protocol/ops.rs` as an `OP_*` constant, and, if the op has a fixed request length,
   a constant to `src/protocol/limits.rs`. Re-export both from `src/protocol/mod.rs` (`protocol/mod.rs:24`
...
2. Write the handler as one file under `src/server/handlers/`, taking `(ctx, sender_pid, req, body, tx)`.
   ...
   `handlers/damage_commit.rs`, which checks `body.len() != DAMAGE_COMMIT_REQ_LEN` before reading any field
   (`handlers/damage_commit.rs:28`) and reads fields through the `u32_at` / `u64_at` helpers
   (`handlers/mod.rs:26`). Add its `pub mod` line to `src/server/handlers/mod.rs`.
3. Wire the op into the match in `src/server/runner/dispatch.rs:31`. An op with no payload is gated on
...
`respond::status_payload` (`src/server/respond.rs:21`, `:32`). A handler that must not reply at all returns
`Ok(())` without calling `respond`, the way `CURSOR_UPDATE` does (`handlers/cursor_update.rs:30`).
...
(`src/sw_blitter/mod.rs:33`) so the bounds check against `byte_len` is preserved; do not index a surface
buffer directly.
...
through `read_status`. The shared wire and timeouts live in `src/gfx_client/wire.rs` and its submodules.
...
`make nonos-mk-desktop-gui-prod` (`Makefile:1183`) and `make nonos-mk-zerostate` (`Makefile:1215`), and
...
- No panics in capsule code. No `unwrap`, `expect`, or `panic!`. Every handler returns a status or a
  ...
  guards return `None` rather than index out of bounds (`src/sw_blitter/mod.rs:34`).
