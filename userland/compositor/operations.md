...
the eight handlers. For what a handler mutates once it runs, see [scene-and-damage.md](scene-and-damage.md)
and [cursor-and-input.md](cursor-and-input.md). Back to the [README](README.md).

## The NCMP frame

The inbound frame is `NCMP` (`src/protocol/header.rs:20`): magic `0x4E43_4D50` ("NCMP"), version 1, and a
...
`IPC_PAYLOAD_MAX = 256` bytes (`src/protocol/limits.rs:17`).

`parse` (`src/protocol/decode.rs:19`) is strict. It reads the header fields first so it can build a
...
`response_header` writes the 20-byte header with the reserved field zeroed (`src/protocol/encode.rs:19`);
`write_status` places the `i32` status right after it (`encode.rs:29`). `respond::status` sends header plus
status; `respond::status_payload` appends a data block after the status (`src/server/respond.rs:21`, `:32`).
Both fail closed: a negative return from `mk_ipc_reply` becomes `Err("ipc reply send failed")`
(`respond.rs:46`).

Error codes are defined once (`src/protocol/errno.rs`):
...
The inbox is drained in batches (`src/server/runner/drain.rs:28`). For up to `MAX_BATCH = 16` iterations
...
`dispatch` (`src/server/runner/dispatch.rs:24`) matches on `req.op`. Three ops that take no payload
...
The compositor exposes eight operations (`src/protocol/ops.rs`). Each request is the 20-byte header plus a
fixed-size payload; the reply is the header plus a four-byte status, and `DISPLAY_INFO` adds a 16-byte data
block. Request lengths come from `src/protocol/limits.rs`.

| Op | Code | Request payload | Handler | What it does |
|----|------|-----------------|---------|--------------|
| `HEALTHCHECK` | `0x0001` | empty | `handlers/health.rs:20` | reply status 0, liveness probe |
| `SCENE_SUBMIT` | `0x0002` | 32 B | `handlers/scene_submit.rs:21` | register or replace the caller's layer, damage its rect |
| `DAMAGE_COMMIT` | `0x0003` | 16 B | `handlers/damage_commit.rs:21` | expand the damage box by a rectangle |
| `FOCUS_SET` | `0x0004` | 8 B | `handlers/focus_set.rs:21` | record which pid holds keyboard focus |
| `INPUT_SUBSCRIBE` | `0x0005` | empty | `handlers/input_subscribe.rs:21` | mark the caller focused |
| `CURSOR_UPDATE` | `0x0006` | 16 B | `handlers/cursor_update.rs:22` | move or hide the cursor, damage old and new cells |
| `SCENE_REMOVE` | `0x0007` | 8 B | `handlers/scene_remove.rs:21` | drop the caller's layers and forget their surfaces |
| `DISPLAY_INFO` | `0x0008` | empty | `handlers/display_info.rs:23` | return width, height, stride, pixel format |

Opcodes are the `OP_*` constants in `src/protocol/ops.rs:17` through `:24`.
...
past the 32-slot table returns `E_INVAL` (`state/scene/submit.rs:28`, surfaced at `scene_submit.rs:67`).
...
([cursor-and-input.md](cursor-and-input.md)).
...
fan input out through the compositor; the [input router](../input-router/README.md) does the fan-out. See
[cursor-and-input.md](cursor-and-input.md).
...
short slice rather than indexing out of bounds (`src/server/handlers/mod.rs:26`). A short or malformed frame
