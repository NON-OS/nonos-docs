# The Input Proof Capsule

`capsule_input_proof` is not an application. It is a runtime self-test of the input path, shipped as a
signed, least-privilege NONOS capsule. It draws a plain window, subscribes to the input router, and waits
...
The point of the capsule is to close the loop end to end. The [input subsystem](../../subsystems/input/path.md)
...
| Service handle | `app.input_proof` | `Capsule.mk:8`, `src/userspace/capsule_input_proof/spawn.rs:31` |
...
(`src/userspace/capsule_input_proof/spawn.rs:50`). The one bit that separates this capsule from an ordinary
app such as [snake](../snake/README.md) or the [terminal](../terminal/README.md) is `Debug` (`0x0100`),
...
|---|---|---|---|
| Surface composited | `surface composited` | first `paint` call | `src/proof/app.rs:47` |
| Surface reachable | `surface ready` | first event of any kind | `src/proof/markers.rs:33` |
| Key delivery | `key down code=<n>` | first `KeyDown` | `src/proof/markers.rs:40` |
| Pointer delivery | `pointer motion x=<x> y=<y>` | first `PointerRel`/`PointerAbs` | `src/proof/markers.rs:51` |
| Click delivery | `click dispatch local=<x>,<y>` | first `ButtonDown` | `src/proof/markers.rs:58` |
| Focus routing | `focus routed` | a key that arrives after a click | `src/proof/markers.rs:45` |

Every marker is prefixed `[INPUT-PROOF] ` on the debug surface (`src/proof/emit.rs:19`). The verdict is
positive only: when all six latches are set, the capsule emits `PASS` once and never again
(`src/proof/markers.rs:65`, `src/proof/state.rs:33`). There is no `FAIL` marker in the capsule itself. A
failure is the absence of `PASS` within the harness window, since a stage that never occurs never sets its
latch and `complete()` stays false. The [debugging](debugging.md) page reads each marker as a checkpoint and
...
| File | Role | Source |
|---|---|---|
| `main.rs` | `_start` entry; hands `InputProof::new` to the app skeleton `run` | `src/main.rs:27` |
| `proof/app.rs` | the `App` adapter: manifest, `on_event`, and the `paint` that latches compositing | `src/proof/app.rs:36` |
| `proof/manifest.rs` | window geometry and the input kind mask it subscribes to | `src/proof/manifest.rs:29` |
| `proof/markers.rs` | the event router that latches each stage and emits its line | `src/proof/markers.rs:22` |
| `proof/state.rs` | the `Latches` record and the `complete()` predicate behind `PASS` | `src/proof/state.rs:17` |
| `proof/emit.rs` | the fixed-capacity line builder and the `mk_debug` call | `src/proof/emit.rs:22` |

The [markers page](markers.md) walks each latch and its trigger condition in detail.

## Lifecycle

The capsule is spawned through [verified spawn](../../security/capsules-and-trust.md): its embedded ELF, id
cert, manifest, and attestation trailer are checked, its requested capabilities are held against its
manifest ceiling, and only then is its ELF mapped (`src/userspace/capsule_input_proof/spawn.rs:37`). It is
...
(`src/userspace/init/spawn_plan/apps.rs:29`, `apps.rs:39`). On a successful spawn the kernel logs
`[APP-INPUT-PROOF] capsule spawned` (`src/userspace/init/capsule_boot/run.rs:29`,
`src/userspace/init/spawn_plan/apps.rs:33`).
...
input router with the manifest mask, and primes the first frame (`userland/app_skeleton/src/runner/entry.rs:31`,
`runner/boot.rs:44`). That priming paint is what emits `surface composited`. Each delivered event then flows
into `on_event`, which runs the marker router and returns `EventOutcome::Idle` so the loop does not repaint
on input (`src/proof/app.rs:41`). The verdict is reported the moment the sixth latch closes; nothing about
