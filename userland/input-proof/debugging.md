...
[README](README.md); the exact latch conditions are on the [markers page](markers.md). This page is the
checklist for reading the log.

## What a pass looks like

A pass is the marker sequence on the debug surface, every line prefixed `[INPUT-PROOF] `
(`src/proof/emit.rs:19`), ending in `PASS`. The lines are latched in the order the events happen, not a
...
`surface composited` is latched by the first `paint` (`src/proof/app.rs:47`); the other five stage lines are
latched by the router as events arrive (`src/proof/markers.rs:33`, `markers.rs:41`, `markers.rs:52`,
`markers.rs:59`, `markers.rs:45`). `PASS` is emitted once, when `complete()` returns true, and never again
because the `passed` latch guards it (`src/proof/markers.rs:65`, `src/proof/state.rs:33`). Seeing `PASS`
...
`complete()` is the conjunction of all six stage latches (`src/proof/state.rs:33`), so any one that never
...
spawn (`src/userspace/init/capsule_boot/run.rs:29`, `src/userspace/init/spawn_plan/apps.rs:33`). If that
...
(`src/userspace/init/spawn_plan/apps.rs:29`, `apps.rs:39`, `apps.rs:40`). The verified-spawn path also
...
(`src/userspace/capsule_input_proof/spawn.rs:37`, `spawn.rs:50`); a rejection there also leaves no boot
...
This is the one latch input cannot set. It flips inside `paint` (`src/proof/app.rs:47`), which the skeleton
calls when it primes the first frame during boot (`userland/app_skeleton/src/runner/boot.rs:47`). If this
...
`toolkit` by name (`userland/app_skeleton/src/runner/entry.rs:32`, `entry.rs:36`,
`userland/app_skeleton/src/discover/require.rs:42`). Heap failure exits with code `1` and peer failure with
...
`surface ready` is set by `announce_ready` on the very first event of any kind (`src/proof/markers.rs:33`).
...
and ButtonDown (`src/proof/manifest.rs:23`, `manifest.rs:24`, `manifest.rs:25`, `manifest.rs:26`), so a
missing class points at that device path in the [input subsystem](../../subsystems/input/path.md), driver
through ring through router, rather than at this capsule.

### No `focus routed`: the ordering never held

This latch is a coincidence gate, not a new event (`src/proof/markers.rs:45`). It sets inside `on_key`, but
...
bounds-checked `push` (`src/proof/emit.rs:22`, `emit.rs:28`, `emit.rs:34`); numbers are formatted in place
