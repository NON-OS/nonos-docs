...
and paint loop (`userland/capsule_hello/src/main.rs:28`). All app logic is under `src/hello/`, declared as
one module per concern in `src/hello/mod.rs:17`, which re-exports only `Hello` (`mod.rs:22`).

## File map

| File | Owns | Touch it when |
|---|---|---|
| `src/hello/app.rs` | the `App` impl and the empty `Hello` struct | you add per-instance state or change how it wires to the runtime |
| `src/hello/manifest.rs` | the window request and input mask | you change the title, window size or position, or the subscribed inputs |
| `src/hello/paint.rs` | the colours and the drawn frame | you change what the window shows |
| `src/hello/event.rs` | input | you change which keys the app reacts to |

## Common changes

1. The greeting text or colours: the four colour constants and the five draw calls in `paint`
   (`src/hello/paint.rs:19`, `paint.rs:24`). Adding a line is one more `text` or `text_scaled` call; there
   is no layout to update.
2. The window size or position: the `width`, `height`, `initial_x`, and `initial_y` fields of the manifest
   (`src/hello/manifest.rs:22`). The accent bar in `paint` is drawn 360 wide to match the window, so if you
   change the width, change that `fill_rect` too (`src/hello/paint.rs:26`).
3. A new key: add a branch to `on_event` returning the outcome you want, and if the key is not a key-down,
   widen `input_kind_mask` in the manifest so the app is subscribed to it (`src/hello/event.rs:19`,
   `src/hello/manifest.rs:31`). Return `EventOutcome::Close` to exit and `EventOutcome::Idle` for a no-op.
4. Per-instance state: give `Hello` fields and initialise them in `new`, then read them in `paint`
   (`src/hello/app.rs:25`). The moment you do this the capsule stops being the minimal example, so consider
