...
surface, window, input subscription, and paint loop (`userland/capsule_hello/src/main.rs:28`). The app
...
files by concern, declared in `src/hello/mod.rs:17`, which re-exports only `Hello` (`mod.rs:22`). The whole
...
|-------|---------|----------------|
| `src/main.rs` | the entry point | `_start` hands `Hello::new` to the skeleton's `run` (`main.rs:28`). |
| `src/hello/app.rs` | the `App` impl | the `Hello` struct and the three trait methods that delegate to manifest, event, and paint (`app.rs:31`). |
| `src/hello/manifest.rs` | the window request | title, window id, kind, position, 360x180 size, and the key-down input mask (`manifest.rs:22`). |
| `src/hello/paint.rs` | the frame | the four colours and the five draw calls that produce the greeting (`paint.rs:24`). |
| `src/hello/event.rs` | input | the single rule: key-down Escape closes, everything else is idle (`event.rs:19`). |
| [walkthrough.md](walkthrough.md) | the whole tree | the same files read in order as the teaching example. |
| [contributing.md](contributing.md) | the whole tree | where to work, how to change the greeting or window, and the build and sign steps. |
| [debugging.md](debugging.md) | runtime | the boot marker and what a blank window or a dead Escape key means. |

## Lifecycle

The hello capsule is spawned through [verified spawn](../../security/capsules-and-trust.md): its signature
and attestation are checked, its requested capabilities are held against its manifest ceiling, and only then
is its ELF mapped (`src/userspace/capsule_hello/spawn.rs:34`). A successful spawn prints `[APP-HELLO]
capsule spawned` on the boot log (`src/userspace/init/spawn_plan/apps.rs:53`,
`src/userspace/init/capsule_boot/run.rs:29`); the [debugging](debugging.md) page covers what its absence
