...
scoring and speed-up, and the bounded food placement in `rng`. It mirrors `src/snake/app.rs`,
`state.rs`, `step.rs`, `rng.rs`, and `grid.rs`. For the keys that feed it see the [controls](controls.md),
and for how the resulting state is drawn see the [rendering](rendering.md).

## The App adapter

`SnakeApp` is a thin wrapper that holds exactly one `Game` and forwards the four `App` methods to the free
functions in the submodules (`src/snake/app.rs:25`). There are no globals and no shared mutable state;
each `SnakeApp` owns its `Game` outright (`src/snake/app.rs:31`).

| `App` method | Forwards to | Source |
|---|---|---|
| `manifest` | `manifest()` | `src/snake/app.rs:36` |
| `on_event` | `on_event(&mut game, event)` | `src/snake/app.rs:39` |
| `paint` | `paint(&game, fb)` | `src/snake/app.rs:42` |
| `on_tick` | `step(&mut game)` | `src/snake/app.rs:45` |
| `tick_interval_ms` | `game.interval_ms` | `src/snake/app.rs:48` |

`_start` hands `SnakeApp::new` to the skeleton's `run`, so the runtime owns the surface, the window, the
input subscription, and the loop, and the capsule supplies only these four methods (`src/main.rs:27`).

## The model

The model is a single `Game` struct: the body vector, the current and pending directions, the food cell,
the score, the phase, the current tick interval, and the RNG state (`src/snake/state.rs:55`).

| Field | Type | Meaning | Source |
|---|---|---|---|
| `body` | `Vec<(i16, i16)>` | snake cells, head at index 0 | `src/snake/state.rs:56` |
| `dir` | `Dir` | direction the last tick moved | `src/snake/state.rs:57` |
| `pending` | `Dir` | direction the next tick will adopt | `src/snake/state.rs:58` |
| `food` | `(i16, i16)` | the one food cell | `src/snake/state.rs:59` |
| `score` | `u32` | food eaten this game | `src/snake/state.rs:60` |
| `phase` | `Phase` | `Ready`, `Running`, `Paused`, `GameOver` | `src/snake/state.rs:61` |
| `interval_ms` | `i64` | current tick interval | `src/snake/state.rs:62` |
| `rng` | `u64` | xorshift64 state | `src/snake/state.rs:63` |

`Phase` is the four-state enum that gates everything: `Ready`, `Running`, `Paused`, `GameOver`
(`src/snake/state.rs:47`). `Dir` is the four directions with an `opposite` used by the anti-reversal
guard (`src/snake/state.rs:28`, `state.rs:37`).

`Game::new` seeds the RNG from the wall clock with the low bit forced set so the seed is never zero, then
calls `reset` (`src/snake/state.rs:67`, `state.rs:76`). `reset` recenters a length-3 snake pointing right,
clears the score, restores `START_INTERVAL_MS`, sets the phase to `Ready`, and places the first food
(`src/snake/state.rs:82`). Score is a plain `u32` starting at zero and reset by `reset`; there is no
persisted high score, so it does not survive a restart or a respawn (`src/snake/state.rs:90`).

The board is a fixed 28x18 grid, and the window geometry is derived from the same constants
(`src/snake/grid.rs:18`). `CELL` is 16 px, `COLS` is 28, `ROWS` is 18, and `WIN_W`/`WIN_H` are computed
from those plus the margins, title bar, and header (`src/snake/grid.rs:17`, `grid.rs:26`). Changing
...
that interval to decide when to call `on_tick` again (`src/snake/app.rs:45`, `app.rs:48`,
`userland/app_skeleton/src/runner/entry.rs:55`). Because the interval is read from game state on every
...
so the skeleton skips the repaint (`src/snake/step.rs:22`). The `false` return is why a paused or
game-over board looks frozen. When it does run, `step`:

1. Adopts the buffered `pending` direction into `dir` (`src/snake/step.rs:25`).
2. Computes the next head cell one step in that direction from the current head at `body[0]`
   (`src/snake/step.rs:26`).
3. Ends the game if the next cell is off the 28x18 board or lands on the snake's own body. The wall check
   ...
   (`src/snake/step.rs:33`, `step.rs:34`). On a collision the phase becomes `GameOver` and the tick
   returns `true` so the game-over frame is drawn (`src/snake/step.rs:35`).
4. Otherwise inserts the new head at the front. If the new head is on the food, the score increases by
   ...
   cell is popped so the length stays the same (`src/snake/step.rs:39`, `step.rs:44`).
...
to end (`src/snake/step.rs:33`, `step.rs:42`).

## Scoring and speed

The three tunables live at the top of `state.rs` (`src/snake/state.rs:24`):

| Constant | Value | Meaning | Source |
|---|---|---|---|
| `START_INTERVAL_MS` | 160 | tick interval at the start of a game | `src/snake/state.rs:24` |
| `MIN_INTERVAL_MS` | 80 | fastest the game will ever tick | `src/snake/state.rs:25` |
| `SPEEDUP_MS` | 4 | milliseconds shaved off the interval per food | `src/snake/state.rs:26` |
...
(`src/snake/step.rs:42`). The game starts at 160 ms and never ticks faster than 80 ms.
...
the snake (`src/snake/rng.rs:28`). If all 64 draws collide it falls back to `first_free`, which scans the
grid in row-major order and returns the first free cell, keeping placement finite even when the board is
nearly full (`src/snake/rng.rs:36`, `rng.rs:39`). The generator is a local xorshift64 with the classic
13/7/17 shift triple, seeded once from `mk_time_millis` and advanced in place (`src/snake/rng.rs:19`,
`src/snake/state.rs:76`). Because the fallback is a deterministic scan and the retry count is fixed, the
