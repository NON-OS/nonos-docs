...
and `ok` prints the tag in brackets followed by the message (`src/userspace/init/capsule_boot/run.rs:29`,
`src/sys/boot_log/output.rs:33`). If that line is absent the capsule never started, and the `Err` arm
logged an `[ERROR]` line built from the spawn error instead (`src/userspace/init/capsule_boot/run.rs:32`),
...
(`src/userspace/init/spawn_plan/apps.rs:116`). There is no serial self-test build and no debug marker
...
The game acts only on key-down and returns `Idle` for every other event kind (`src/snake/input.rs:24`). If
...
does not control (`userland/app_skeleton/src/discover/require.rs:31`).
...
(`src/snake/input.rs:48`, `src/snake/step.rs:25`). A single perpendicular turn always takes; only the
...
Enter alone will not start it (`src/snake/input.rs:52`). If the board sits on `PRESS A DIRECTION KEY`, the
...
(`src/snake/input.rs:67`). This is intentional so a stray Enter mid-run cannot wipe the board.

### Motion feels frozen

A paused game and a game-over game both return `false` from the tick, so the skeleton skips the repaint and
the board looks static (`src/snake/step.rs:22`). The overlay text tells the two apart from a running game:
`PAUSED` versus `GAME OVER` (`src/snake/paint/overlay.rs:30`, `overlay.rs:35`). If neither overlay shows,
