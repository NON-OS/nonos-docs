...
`capsule_snake` is the classic snake game as a signed NONOS capsule: a normal GUI window with a real game
...
[capsule_terminal](../terminal/README.md) shows how large an [app-skeleton](../writing-an-app.md) app can
...
(`src/userspace/capsule_snake/spawn.rs:49`). There is no `Network` bit (`0x0004`, `types.rs:58`), no
...
|---|---|---|
| [controls.md](controls.md) | `src/snake/input.rs` | Every keybinding: arrow and WASD steering, Enter restart, Space and P pause, the anti-reversal guard, one-turn buffering, and the phase gates. |
| [game-loop.md](game-loop.md) | `src/snake/app.rs`, `state.rs`, `step.rs`, `rng.rs`, `grid.rs` | The `App` adapter, the owned `Game` model and phases, the self-timed tick, collision and growth, scoring and speed-up, and the bounded food placement. |
| [rendering.md](rendering.md) | `src/snake/paint/` | How a frame is produced: the fit-to-window layout, the score header, the board with food and snake, and the per-phase overlay banners. |
| [contributing.md](contributing.md) | the whole tree | Where to work, the common changes and where they go, the build and sign steps, and the code standards. |
| [debugging.md](debugging.md) | runtime | The boot marker, the failure modes, and where to look when input, motion, or the display misbehaves. |

## Lifecycle

Snake is spawned through [verified spawn](../../security/capsules-and-trust.md): its embedded ELF, id
cert, manifest, and attestation trailer are checked, its requested capabilities are held against its
manifest ceiling, and only then is its ELF mapped (`src/userspace/capsule_snake/spawn.rs:36`). It is
...
(`src/userspace/init/spawn_plan/apps.rs:22`, `apps.rs:111`, `apps.rs:116`). On success the kernel logs
`[APP-SNAKE] capsule spawned` (`src/userspace/init/capsule_boot/run.rs:29`,
...
(`userland/app_skeleton/src/runner/entry.rs:31`, `runner/boot.rs:39`). Each key-down flows into
`on_event`; on the tick cadence the loop calls `on_tick` and repaints only when the game actually moved
or died (`runner/entry.rs:52`). The three pillar pages cover input, the tick, and the frame in turn.
...
on its behalf: opening and sharing its surface (`userland/app_skeleton/src/setup/open.rs:26`), resolving
`compositor`, `wm`, and `input_router` by name (`userland/app_skeleton/src/discover/require.rs:31`,
`require.rs:34`, `require.rs:37`), and subscribing the window to the manifest's input mask
(`userland/app_skeleton/src/runner/boot.rs:46`). The only libc syscall the game itself makes is
`mk_time_millis`, used once to seed the RNG (`src/snake/state.rs:76`, `userland/libc/src/time/wall.rs:19`).
