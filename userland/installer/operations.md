...
call. For why a load is safe once it reaches the kernel, read the [verified-load](verified-load.md) page;
for identity and the mask, read the [README](README.md).
...
(`src/server/runner.rs:26`). Every request is `seq(4) | op(2) | pad(2) | body` and every reply is
`seq(4) | status(4) | payload` (`src/protocol/decode.rs:19`, `src/protocol/encode.rs:19`).
...
(`src/protocol/decode.rs:19`, `src/protocol/types.rs:24`). `encode_response` prepends the little-endian
`seq` and `status` to a payload (`src/protocol/encode.rs:19`). The op and errno constants live in
`src/protocol/types.rs:17` and are re-exported from `src/protocol/mod.rs:23`.

The reply always goes to `KERNEL_REPLY_ENDPOINT`, the constant `0x1_0000_0011`
(`src/protocol/types.rs:22`, `src/server/runner.rs:40`). The receive buffer is 8 MiB, because a capsule
image is large and the by-payload path carries the ELF inline (`src/server/runner.rs:24`). A non-positive
receive or an undecodable frame is skipped without a reply (`src/server/runner.rs:32`, `:37`).

## The four operations

Four operations are defined and dispatched (`src/protocol/types.rs:17`, `src/server/dispatch.rs:26`). Any
other opcode replies `EINVAL` (`-22`) from the fall-through arm (`src/server/dispatch.rs:31`,
`src/protocol/types.rs:26`).
...
Vec<u8>` and returning an `encode_response` (`src/server/handlers/mod.rs:17`). Two roles live in the same
...
(`src/server/handlers/health.rs:21`). A present reply means `mk_service_lookup("installer")` resolves and
the loop is running.
...
[vfs](../vfs/README.md) client, each capped at 16 MiB (`load_by_name.rs:56`, `:28`, `:87`), and builds a
...
certificate ceiling; see the [verified-load](verified-load.md) page.
...
decoded into `InstallReq` (`src/server/fields.rs:17`, `install.rs:33`). A wrong length replies `EINVAL`
(`install.rs:29`). If `price_kind == PRICE_KIND_FREE` (0) it returns a 32-byte zero receipt with status
`0` and contacts no one (`install.rs:42`, `src/server/consts.rs:19`). Otherwise it resolves the `payment`
service by name; if that lookup fails it replies `EAGAIN` (`-11`)
(`install.rs:45`, `src/server/discover.rs:21`, `src/protocol/types.rs:27`). It then calls the
[payment](../payment/README.md) capsule's `OP_PAY` (2) with the owner pid, wallet id, capsule id,
...
(`src/server/pay_call.rs:25`, `:38`, `:41`, `src/server/consts.rs:18`).
...
(`src/server/word32.rs:17`, `src/server/addr20.rs:17`).

## The self-install path

Under the `nonos-autorun-install` feature the module `src/server/selfinstall.rs` is compiled
(`src/server/mod.rs:25`), and `server::run` calls it before the request loop
(`src/server/runner.rs:27`). It waits for the vfs store to answer, then loads `std_proof` and `rg` through
the same `mk_capsule_load` the handlers use, so the runtime install path is proven end to end on a
headless boot (`src/server/selfinstall.rs:30`, `:34`, `:42`, `:72`). It requests `u64::MAX` caps, which,
as the [verified-load](verified-load.md) page explains, can only ever select within each capsule's own
manifest (`src/server/selfinstall.rs:23`, `:64`). It is a build-time feature, not a callable operation.
