# Contributing to capsule_installer
...
[README](README.md), the [operations](operations.md), and the [verified-load](verified-load.md) pages in
this folder.
...
(`userland/capsule_installer/src/main.rs:28`). The two top-level modules are declared there
(`src/main.rs:22`).
...
1. Define the opcode as a `pub const OP_<NAME>: u16` in `src/protocol/types.rs:17`, next to the existing
   four, and re-export it from the `pub use types::{...}` list in `src/protocol/mod.rs:23`.
...
2. Write the handler as one file under `src/server/handlers/`, exposing
   ...
   (`src/server/handlers/health.rs:21` is the smallest reference shape; the load handlers show the
   ...
   load handlers do (`src/server/handlers/load_store.rs:33`,
   `src/server/handlers/load_by_name.rs:43`). Re-export it from `src/server/handlers/mod.rs:17`.

3. Wire it into the match in `src/server/dispatch.rs:26`. Add a `OP_<NAME> => handlers::<name>(req)` arm.
...
syscall (`src/server/handlers/load_by_name.rs:65`, `:78`, `:80`). Never verify anything in the handler;
verification is the kernel's, and duplicating it here would be a second source of truth. See the
[verified-load](verified-load.md) page for why.
...
`src/server/selfinstall.rs`; it adds no callable operation, so it does not change the wire protocol.
