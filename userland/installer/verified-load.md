...
`spawn_verified`. For the wire frame and the operations, read the [operations](operations.md) page.
...
(`src/server/handlers/load_by_name.rs:80`, `src/server/handlers/load_store.rs:64`,
`src/server/selfinstall.rs:72`). The libc shim passes a pointer to that struct through syscall `MCLD` and
returns the new pid, or a stable negative errno (`userland/libc/src/capsule_load.rs:46`,
`userland/libc/src/syscall/numbers/core.rs:19`). The struct mirrors the kernel layout byte for byte: the
four artifact pointer/length pairs, `requested_caps`, and the args pointer/length
(`userland/libc/src/capsule_load.rs:28`).

Kernel side, `SYS_CAPSULE_LOAD` (also `MCLD`) dispatches to `sys_capsule_load`
(`src/syscall/microkernel/dispatch/process.rs:35`, `src/syscall/microkernel/numbers.rs:33`). It validates
the user pointer, copies the struct out of user memory, reads the four blobs and the args, and calls
`load_capsule_from_vfs` (`src/syscall/microkernel/capsule_load/handle.rs:27`, `:34`, `:52`). Loader
...
(`src/syscall/microkernel/capsule_load/errno.rs:23`).
...
path baked capsules use (`src/kernel_core/process_spawn/capsule_spawn/from_vfs/load.rs:36`). Three
...
declares and the caller cannot misname or misroute it (`from_vfs/load.rs:36`, `:42`, `:54`, `:124`). The
...
[NONOS-ID certificate](../../security/certificate-schema.md) against the baked
[trust anchor](../../security/trust-anchor.md), verifies the [manifest](../../security/manifest-schema.md)
...
the [attestation gate](../../security/attestation.md), is spawned (`from_vfs/load.rs:73`); a rejected
...
`manifest:caps_ceiling`, and `attestation` (`from_vfs/load.rs:83`, `:105`).
...
still decides what is actually granted (`from_vfs/load.rs:34`). Two checks enforce this:
...
- The manifest's own declared caps (`required_caps | optional_caps`) must not exceed the certificate
  ...
  (`src/security/capsule_manifest/verify/caps.rs:20`, `from_vfs/load.rs:90`).
- The grant check requires the requested caps to be a subset of what the manifest declares, and the caps
  ...
  (`src/security/capsule_manifest/verify/caps.rs:31`).
...
`src/userspace/capsule_installer/spawn.rs:33`). It cannot be tricked into loading an unsigned or tampered
...
[operations](operations.md) page: the by-name path validates the name to `[A-Za-z0-9_-]` before building
any store path, so it cannot escape `/capsules` (`src/server/handlers/load_by_name.rs:95`), and the
...
(`src/server/handlers/load_store.rs:33`). The artifact blobs are stack-owned across the syscall, so there
is no use-after-free of the bytes the kernel copies (`src/server/handlers/load_by_name.rs:78`).
