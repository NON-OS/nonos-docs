...
the code standards the capsule must meet. Back to the [hub](README.md).
...
2. Write the handler as one file under `src/server/handlers/`, exposing
   ...
   `src/server/handlers/proof_invariants.rs:21`, which does the count-then-tuples pattern with per-entry
   overflow checks.
3. Wire it into the module list and the dispatch match at `src/server/handlers/mod.rs:17` and
...
`src/state/invariants.rs:23`. The count in the reply is derived from `INVARIANTS.len()`, so no other code
changes (`src/server/handlers/proof_invariants.rs:22`). Two rules keep the table honest: the `mechanism`
...
Append a `(name, mask)` tuple to `KNOWN_CAPSULES` in `src/server/handlers/proof_capsule_list.rs:20`. The
...
which the kernel embed includes at build time (`src/userspace/capsule_attest/embed.rs:22`,
...
- Modular files, one unit per file, with `mod.rs` used only for re-exports
  (`src/server/mod.rs:21`, `src/protocol/mod.rs:24`, `src/state/mod.rs:20`).
...
- Do not request a new capability. The mask `0x19` is a documented invariant of this capsule; any change
  that would widen it needs to be justified against the NO LOGS and scope arguments on the [hub](README.md).
</content>
