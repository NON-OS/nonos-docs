...
server-side from constants. The module re-exports `run` from `src/server/mod.rs:21`.
...
The service port is 4444 (`src/server/runner.rs:24`). The receive timeout is 0
...
`src/server/respond.rs` wraps the protocol encoders into the two reply shapes every handler uses:
...
`route` is the single dispatch point (`src/server/handlers/router.rs:24`). It calls `parse`; on a parse
...
(`src/server/handlers/health.rs:20`). It reads no state and never fails, so a client that gets a status-0
...
constants in `state` (`src/server/handlers/proof_summary.rs:21`). It bounds-checks the running write
...
(`proof_summary.rs:33`). The values are covered in [attestation-data.md](attestation-data.md).

### OP_PROOF_INVARIANTS

A 4-byte count followed by six `{name, claim, mechanism}` tuples, each field length-prefixed
(`src/server/handlers/proof_invariants.rs:21`). Before the count and before each tuple it checks the
...
[attestation-data.md](attestation-data.md).

### OP_PROOF_BOOT

A `u64` boot timestamp followed by a length-prefixed fixed label
(`src/server/handlers/proof_boot.rs:22`). This is the only handler that reads a kernel value at request
time: `read_time` calls `mk_time_millis` and clamps a negative return to 0 (`proof_boot.rs:27`,
`proof_boot.rs:39`). The label is the fixed byte string `NONOS bootloader (hybrid Ed25519 + ML-DSA-65)`
...
little-endian mask (`src/server/handlers/proof_capsule_list.rs:40`). The table is the `KNOWN_CAPSULES`
