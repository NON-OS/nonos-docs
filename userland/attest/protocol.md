...
its public surface from `src/protocol/mod.rs:24`.
...
(`src/protocol/header.rs:21`); the magic, version, reserved, and payload_len are validated at parse time
and not carried forward.

## Parsing a request

`parse` takes the received buffer and returns either the `Request` and its payload slice, or the same
`Request` (as far as it could be read) and an error status (`src/protocol/decode.rs:19`). It validates in
this order:
...
(`src/server/handlers/router.rs:25`). The little-endian field readers `u16_le` and `u32_le` are the only
byte arithmetic in the module (`decode.rs:45`, `decode.rs:49`).

## Building a response

Two functions in `src/protocol/encode.rs` build every reply. `response_header` writes the 20-byte header,
...
builders into the status-only and with-payload reply shapes; see [operations.md](operations.md).

## Opcodes

Five opcodes, all with no request payload (`src/protocol/ops.rs:17`):
...
[operations.md](operations.md) and [attestation-data.md](attestation-data.md).

## Error codes

The negative status words the capsule can return (`src/protocol/errno.rs:17`):
...
Two size constants bound the module (`src/protocol/limits.rs:17`):
...
the server allocates once at startup (`src/server/runner.rs:28`); every handler bounds-checks its writes
