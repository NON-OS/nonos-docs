...
is read by the [boot splash](../boot-splash/README.md) through `mk_attest_status`, not by this capsule.
...
|------|---------|----------------|
| [protocol.md](protocol.md) | `src/protocol/` | The 20-byte header, the five opcodes, the error codes, the parse and response builders, and the payload limits. |
| [operations.md](operations.md) | `src/server/` | The receive-reply loop, the header-validating router, and the five read-only handlers with their exact reply layouts. |
| [attestation-data.md](attestation-data.md) | `src/state/` and the two authored handler tables | What the capsule actually serves: the product summary, the six invariants in full, and the seventeen-entry capsule-mask table, with the honest boundary on each. |
| [contributing.md](contributing.md) | the whole tree | Where to work, how to add an op or an invariant, the build and sign targets, and the code standards. |
| [debugging.md](debugging.md) | runtime | The boot marker, the failure modes, and where to look when the capsule does not answer. |
...
`src/userspace/capsule_attest/embed.rs:18`. Its signature and attestation are checked and its requested
...
(`src/userspace/init/capsule_boot/run.rs:29`, `src/userspace/capsule_attest/state.rs:17`,
`state.rs:25`); on failure it
prints an `[ERROR]` line with the decoded `SpawnError` instead (`capsule_boot/run.rs:32`). Inside the
capsule, `_start` initializes the heap, exits with status 1 on failure, and otherwise enters `server::run`,
which never returns (`src/main.rs:29`). The [debugging](debugging.md) page covers what each marker means.
