...
`OP_PROOF_SUMMARY` returns three fields from `src/state/product.rs:17`:
...
(`product.rs:19`). These are re-exported through `src/state/mod.rs:21`.

## The invariants, in full

`OP_PROOF_INVARIANTS` returns the six-entry `INVARIANTS` table (`src/state/invariants.rs:23`). Each entry
is an `Invariant` with a `name`, a `claim`, and a `mechanism`, all authored as constant byte strings
(`src/state/invariants.rs:17`). They are the heart of the response and worth reproducing, because each
...
```
  ...
  NOT LINUX
    ...
    mechanism: Mk* 4-byte ASCII tag syscall ABI; NCMP-style wire across every capsule;
               capability taxonomy is NONOS-native
...
the syscall returns a negative value (`src/server/handlers/proof_boot.rs:39`). The label is the fixed byte
string `NONOS bootloader (hybrid Ed25519 + ML-DSA-65)` (`proof_boot.rs:28`). This is a boot-identity string
...
capability mask it is expected to hold (`src/server/handlers/proof_capsule_list.rs:20`). It is not a live
