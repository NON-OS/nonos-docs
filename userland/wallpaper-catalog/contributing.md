# Contributing to capsule_wallpaper_catalog
...
protocol works, read the [README](README.md) and the [protocol and catalog](protocol.md) page. For
runtime failures, see the [debugging](debugging.md) page.
...
(`src/main.rs:30`). The top-level modules are declared there (`src/main.rs:22`).
...
2. Add an `Entry` to the right group under `src/catalog/entries/`. Pick the group its slug belongs to
   ...
   (`src/catalog/entries/field_focus.rs:20` is the reference shape). `count` and `entry_at` recompute from
   the group lengths on every call (`src/catalog/count.rs:19`, `src/catalog/entries/entry_at.rs:20`), so no
...
`userland/capsule_policy/src/store/defaults/store.rs:45`), so an insert silently repoints existing
...
`src/catalog/entries/special_variant.rs:27`), but keeping them equal avoids confusion.
