# Debugging capsule_wallpaper_catalog

This page lists the markers the catalog and its boot path emit, and the concrete failure modes with where
to look for each. For the wire protocol and the catalog layout see the [README](README.md) and the
[protocol and catalog](protocol.md) page; for how to add an image see the [contributing](contributing.md)
page.
...
instead (`src/userspace/init/capsule_boot/run.rs:32`), which is the usual signature, manifest, or
capability failure.

The capsule also fails itself early with two exit codes before the loop begins. Exit code 1 means
`heap_init` failed (`src/main.rs:31`). Exit code 2 means `bootstrap::register` returned false, that is,
`mk_service_register` did not return 0 (`src/main.rs:35`, `src/bootstrap/register.rs:22`). Either exit
...
(`src/bootstrap/service_name.rs:17`, `src/bootstrap/port.rs:17`). If registration failed, the capsule
...
(`userland/capsule_wallpaper/src/catalog_client/lookup.rs:23`, `lookup.rs:32`).
...
(`userland/capsule_policy/src/store/defaults/store.rs:45`); if the catalog were rebuilt with fewer than
...
lengths (`src/catalog/count.rs:19`), currently 62.
...
`payload_len` (`src/server/handlers/op_get_chunk.rs:31`). The wallpaper client aborts if the running
total ever exceeds the declared size or if the reassembled length does not match the size
(`userland/capsule_wallpaper/src/catalog_client/fetch_image.rs:26`), so a truncated result means a
...
- `E_NOT_FOUND` (91) on size, slug, or chunk means the index is out of the `0..count` range: the accessor
  returned `None` (`src/catalog/entries/entry_at.rs:28`).
- `E_INVAL` (22) means the op code was not one of the four, so the dispatch fell to the default arm
  (`src/server/runner.rs:45`). Check the header's `op` field encoding, remembering the header is
  little-endian (`src/protocol/hdr.rs:29`). Note that the capsule's source `README.md` calls this
...
- `E_BAD_LEN` (90) comes only from the reply path when a payload would exceed the buffer
  (`src/server/respond/ok.rs:25`); a client should never provoke it, since the largest reply is one 4096-
...
index and match the returned bytes against the group tables on the [protocol and catalog](protocol.md)
