...
storage device. It is the source the [wallpaper](../wallpaper/README.md) capsule fetches from.
...
|------|---------|----------------|
| [protocol.md](protocol.md) | `src/server/`, `src/protocol/`, `src/catalog/` | The wire header, the four operations and their exact codes, reply framing, the errno set, and the four embedded image groups with the disk-versus-served discrepancy. |
| [contributing.md](contributing.md) | the whole tree | Where the source lives, how to add a wallpaper, the build and sign steps, and the code standards. |
| [debugging.md](debugging.md) | runtime | The exit codes, the failure modes, and where to look when the desktop comes up with a blank or wrong background. |

## Lifecycle

The catalog is spawned through [verified spawn](../../security/capsules-and-trust.md): its signature and
attestation are checked, its requested capabilities are held against its manifest ceiling, and only then
is its ELF mapped. `_start` (`src/main.rs:30`) initializes the heap, then calls `bootstrap::register`
(`src/bootstrap/register.rs:21`), which invokes `mk_service_register` with the name, its length, and the
port. A heap failure exits with code 1; a failed registration exits with code 2 (`src/main.rs:31`,
`src/main.rs:35`). On success it enters `server::run(SERVICE_PORT)` (`src/main.rs:37`), a poll-yield loop
that never returns.
...
time `_start` runs. The [protocol](protocol.md) page covers the groups and the flat index across them.
