...
the accessors over them). For identity and capabilities see the [README](README.md); for how to add an
image see the [contributing](contributing.md) page; for runtime failures see the
[debugging](debugging.md) page.

## The wire header

Every request and every reply begins with a fixed 16-byte header (`HDR_LEN`, `src/protocol/hdr.rs:17`).
There is no magic number and no version field; the header is the compact catalog form. It is five
little-endian fields in this order (`src/protocol/hdr.rs:29`, `src/protocol/hdr.rs:37`):
...
`run` (`src/server/runner.rs:24`) allocates one stack buffer of `IPC_PAYLOAD_MAX` bytes and loops. Each
pass:

- `recv::poll` (`src/server/recv.rs:21`) calls `mk_ipc_recv_from` with a zero flag, so the receive is
  non-blocking. If it returns nothing, the loop yields with `mk_yield` and retries
  (`src/server/runner.rs:29`).
- A message shorter than the 16-byte header is dropped silently (`src/server/runner.rs:33`).
- `Header::decode` parses the header, and a decode failure continues the loop (`src/server/runner.rs:36`).
- The `op` field selects a handler (`src/server/runner.rs:40`). Anything unrecognized is answered with
  `E_INVAL` (`src/server/runner.rs:45`).
...
Four operation codes (`src/protocol/ops.rs:17`):
...
`OP_GET_COUNT` (`src/server/handlers/op_get_count.rs:22`) always replies with the total catalog count in
`index = 0`, `offset = 0`, and a four-byte body.

`OP_GET_SIZE` (`src/server/handlers/op_get_size.rs:22`) looks up the image bytes and returns their length;
a missing index replies `E_NOT_FOUND`. The size is derived, not stored: `get_size` is
`get_bytes(index).map(len)` (`src/catalog/get_size.rs:19`).

`OP_GET_SLUG` (`src/server/handlers/op_get_slug.rs:22`) returns the slug bytes for the index, or
`E_NOT_FOUND`.

`OP_GET_CHUNK` (`src/server/handlers/op_get_chunk.rs:22`) is the streaming path. It fetches the image
bytes, then:
...
Success replies go through `respond::ok` (`src/server/respond/ok.rs:24`). It rejects a payload larger than
...
`reply::send` (`src/server/reply.rs:19`, which calls `mk_ipc_reply`).

Error replies go through `respond::err` (`src/server/respond/err.rs:21`). An error frame echoes the
original `op` and `index`, sets `offset = 0` and `payload_len = 0`, and carries only the 16-byte header.

## The errno set

The errnos are five constants (`src/protocol/errno.rs:17`):
...
`errno.rs`. The code returns `E_INVAL` (22) for an unknown op (`src/server/runner.rs:45`) and does not
...
Two constants set the sizes (`src/protocol/limits.rs:17`):
...
a byte slice (`src/catalog/entry.rs:17`), each byte slice an `include_bytes!` of a JPEG under
`nonos-data/wallpapers/`. The four groups are concatenated in this order (`src/catalog/entries/groups.rs:23`):

| Group | Const | Entries | Slug range | Source |
|---|---|---|---|---|
| Field focus | `FIELD_FOCUS` | 13 | `field-focus-1` .. `field-focus-13` | `src/catalog/entries/field_focus.rs:19` |
| Hardware aesthetic | `HARDWARE_AESTHETIC` | 14 | `hardware-aesthetic-1` .. `hardware-aesthetic-14` | `src/catalog/entries/hardware_aesthetic.rs:19` |
| Network topology | `NETWORK_TOPOLOGY` | 18 | `network-topology-1` .. `-19`, skipping 12 | `src/catalog/entries/network_topology.rs:19` |
| Special variant | `SPECIAL_VARIANT` | 17 | `-1a`, `-1b`, `-2a`, `-2b`, `-3` .. `-15` | `src/catalog/entries/special_variant.rs:19` |
...
(`src/catalog/entries/mod.rs:24`), walks the groups in order, subtracting each group's length until the
remaining index lands inside a group (`src/catalog/entries/entry_at.rs:20`), so index 0 is
`field-focus-1`, index 13 is `hardware-aesthetic-1`, and so on. `get_slug` and `get_bytes` both resolve
through `entry_by_index` (`src/catalog/get_slug.rs:19`, `src/catalog/get_bytes.rs:19`); `get_size` is
`get_bytes` composed with `len` (`src/catalog/get_size.rs:19`). `count` is the saturating sum of the group
lengths and is the only accessor that walks `ENTRY_GROUPS` directly (`src/catalog/count.rs:19`). All of
