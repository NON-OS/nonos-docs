...
`/capsules` store through the [vfs](vfs.md) client and passes them straight to the kernel; when a load is
...
settlement through the [payment](payment.md) capsule and returns a signed receipt hash; for a free
...
`spawn.rs:48`), the same minimal set as the [vfs pool](vfs.md). There is no `Crypto` bit (32), so the
...
The other outbound call is to the [payment](payment.md) service. The installer resolves it by name with
...
lives on the sibling [market](market.md) capsule, which verifies content itself, whereas the installer
