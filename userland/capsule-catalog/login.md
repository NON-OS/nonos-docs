...
start a session. Starting a session means unlocking a key in the [keyring](keyring.md); once the keyring
...
the [keyring](keyring.md), which owns the key material and applies its own owner-pid check. Login is the
...
`:11`). This is the same `UNLOCK` op the [keyring](keyring.md) documents as op 5; the matching relock is
...
- Login is not the credential authority, and there is no passphrase in it to protect. The `START_SESSION`
  ...
  reaches it. The actual key material and the actual verification live in the [keyring](keyring.md), which
...
[keyring](keyring.md) side (`src/clients/keyring.rs:8`).
