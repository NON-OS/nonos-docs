...
|---------|------------------|------|-----------|
| [vfs](vfs.md) | `service:4104:vfs_pool` | `0x19` | The application filesystem: 15-op store, per-caller FD table, kernel-attested caller pid, `/capsules` read-only. |
| [ramfs](ramfs.md) | `service:4096:ramfs` | `0x38` | The `/ram` filesystem, per-file ChaCha20-Poly1305 with a fresh nonce on every write. |
| [keyring](keyring.md) | `service:4098:keyring` | `0x39` | The key store and wallet signer: owner-pid isolation, secure wipe on drop, ETH/NOX signing. |
| [entropy](entropy.md) | `service:4100:entropy_pool` | `0x39` | The RDRAND-backed random-bytes service with observability counters. |
| [crypto](crypto.md) | `service:4102:crypto_pool` | `0x39` | The stateless crypto compute pool with per-op payload limits and request-buffer wipe. |
| [policy](policy.md) | `service:4108:policy` | `0x219` | The typed config store; reads open, writes gated to the settings and setup-wizard apps. |
| [attest](attest.md) | `service:4444:attest` | `0x19` | System info and stated invariants. Honestly: text claims and a boot label, not cryptographic proofs. |
| [installer](installer.md) | `service:4112:installer` | `0x19` | Loads capsules through the kernel's verified-load syscall; trust is the kernel's. |
| [payment](payment.md) | `service:4110:payment` | `0x19` | Assembles NOX receipts and gets them signed by the keyring; per-payer nonces and a drain queue. |
| [market](market.md) | `service:4106:market.index` | `0x19` | Serves the signed app index; real Ed25519 verifier (reject-all stub under `offline-verify`). |
| [power](power.md) | `service:4448:power` | `0x219` | Reboot and shutdown through the kernel admin syscalls. |
| [process-manager](process-manager.md) | `service:4730:app.process_manager` | `0x1819` | A GUI viewer of running services and CPU usage via `mk_proc_stat`. |
...
[vfs](vfs.md) (`vfs_pool` on port 4104, mask `0x19`) is the application filesystem. Its wire protocol
(`userland/capsule_vfs/src/protocol/types.rs:17`) is `MAGIC 0x4E4F5646` ("NOVF"), version 1, a 20-byte
...
sender when the message came from a real process (`userland/capsule_vfs/src/server/handlers/util.rs:47`),
so the FD table and per-path ownership are keyed on an attested identity rather than a self-declared one.

[ramfs](ramfs.md) (`ramfs` on port 4096, mask `0x38`) is the `/ram` filesystem. Its protocol
(`userland/capsule_ramfs/src/protocol/types.rs:17`) is smaller: an 8-byte header and five operations,
`OP_OPEN`, `OP_CLOSE`, `OP_READ`, `OP_WRITE`, `OP_TRUNCATE` (1 through 5). Every file is stored encrypted:
the store holds a per-file key and nonce (`userland/capsule_ramfs/src/store/types.rs:25`) and rolls a fresh
nonce on each write through `fresh_nonce` (`userland/capsule_ramfs/src/store/crypto/fresh_nonce.rs:22`),
sealing and opening with ChaCha20-Poly1305 (`userland/capsule_ramfs/src/store/crypto/mod.rs:16`). That is
why ramfs is the one filesystem carrying the Crypto bit; vfs does not.

## The security pools

[keyring](keyring.md) (`keyring` on port 4098, mask `0x39`) is both the key store and the wallet signer. Its
protocol (`userland/capsule_keyring/src/protocol/types.rs:17`) has fourteen operations: `OP_STORE`,
...
on explicit delete (`userland/capsule_keyring/src/store/delete.rs:29`) and in the `Drop` for a key entry
(`userland/capsule_keyring/src/store/types/key_entry.rs:30`), so a dropped or overwritten key does not linger
...
[entropy](entropy.md) (`entropy_pool` on port 4100, mask `0x39`) is the random-bytes service. Its protocol
(`userland/capsule_entropy/src/protocol/types.rs:26`) is `MAGIC 0x4E4F454E` ("NOEN"), version 1, a 20-byte
header, four operations: `OP_GET_RANDOM`, `OP_GET_STATS`, `OP_RESEED`, `OP_HEALTHCHECK` (1 through 4). Random
bytes come from RDRAND with a bounded retry loop (`userland/capsule_entropy/src/pool/fill.rs:22`), and the
pool keeps observability counters, `bytes_served` and `last_reseed_request`
(`userland/capsule_entropy/src/pool/new.rs:24`), returned by `OP_GET_STATS`
(`userland/capsule_entropy/src/pool/encode_stats.rs:18`). A single random draw is capped at 4096 bytes
(`MAX_RANDOM_BYTES`, `types.rs:36`).

[crypto](crypto.md) (`crypto_pool` on port 4102, mask `0x39`) is the stateless compute pool. The dispatch
table (`userland/capsule_crypto/src/server/dispatch.rs:28`) routes seventeen operations. The base set lives
in `protocol/types.rs`: `OP_BLAKE3_HASH` (1), `OP_SHA3_256_HASH` (2), `OP_HEALTHCHECK` (3), `OP_SHA256_HASH`
...
`protocol/primitives.rs:17`: `OP_X25519_PUBLIC` (14), `OP_X25519_SHARED` (15), `OP_HMAC_SHA256` (16),
...
(`userland/capsule_crypto/src/server/wipe.rs:19`). The pool holds no keys and keeps no state between
...
[policy](policy.md) (`policy` on port 4108, mask `0x219`) is the typed config store. Its protocol crate
(`userland/policy_proto/src/ops.rs:17`) has two operations, `OP_GET` (0x0001) and `OP_SET` (0x0002), over
...
`E_ACCES` otherwise (`userland/capsule_policy/src/server/handle_set.rs:23`). Fields that mirror into the
kernel are pushed through the `mk_admin_policy_push` syscall (`userland/capsule_policy/src/push/raw.rs:17`),
which is why policy carries the Admin bit.

[attest](attest.md) (`attest` on port 4444, mask `0x19`) reports system state. Its operations
(`userland/capsule_attest/src/protocol/ops.rs:17`) are `OP_HEALTHCHECK` (0x0001), `OP_PROOF_SUMMARY`
...
[installer](installer.md) (`installer` on port 4112, mask `0x19`) is the install authority. Its operations
...
[payment](payment.md) (`payment` on port 4110, mask `0x19`) issues NOX install receipts. Its operations
...
[market](market.md) (`market.index` on port 4106, mask `0x19`) serves the signed app index. Its operations
...
(`userland/capsule_market/src/verify/crypto.rs:17`). The `offline-verify` build feature swaps that for a
development reject-all stub (`userland/capsule_market/src/verify/mod.rs:18`,
`userland/capsule_market/Cargo.toml:35`).

[power](power.md) (`power` on port 4448, mask `0x219`) is the reset rail. Its operations
(`userland/capsule_power/src/protocol/ops.rs:17`) are `OP_HEALTHCHECK` (0x0001), `OP_REBOOT` (0x0002),
...
[process-manager](process-manager.md) (`app.process_manager` on port 4730, mask `0x1819`) is a GUI viewer,
...
and registers it in the lifecycle table (`src/userspace/init/capsule_boot/run.rs:27`). ramfs comes first
because the rest of the store layer needs it (`src/userspace/init/spawn_plan/core.rs:17`); once it is up,
`spawn_after_ramfs` brings keyring, entropy, crypto, and policy in that sequence
(`spawn_plan/core.rs:22`); then vfs and market (`spawn_plan/core.rs:29`, `:35`). The desktop-services pass
adds attest and installer (`src/userspace/init/spawn_plan/desktop_services.rs:20`), and the apps-tools pass
adds the process-manager viewer (`src/userspace/init/spawn_plan/apps_tools.rs:20`).
...
rather than a fine-grained capability (`userland/capsule_policy/src/server/handle_set.rs:23`), a real trust
...
[proof system](../../subsystems/proof-system/README.md) is in the kernel.

## Debugging

Every core service that the init fleet spawns prints a boot marker through `capsule_boot::boot`
(`src/userspace/init/capsule_boot/run.rs:29`), which emits `boot_log::ok(prefix, "capsule spawned")` on
...
(`userland/capsule_installer/src/protocol/types.rs:27`). Each deep page carries that capsule's own
...
(`userland/capsule_entropy/src/protocol/types.rs:26`, `userland/capsule_crypto/src/protocol/types.rs:17`), so
...
(`userland/capsule_vfs/src/server/handlers/util.rs:47`); a caller that forgets to fill in its own pid will
...
- vfs: `userland/capsule_vfs/src/protocol/types.rs`, `userland/capsule_vfs/src/server/handlers/util.rs`.
- ramfs: `userland/capsule_ramfs/src/protocol/types.rs`, `userland/capsule_ramfs/src/store/crypto/`,
  `userland/capsule_ramfs/src/store/types.rs`.
- keyring: `userland/capsule_keyring/src/protocol/types.rs`, `userland/capsule_keyring/src/store/delete.rs`,
  `userland/capsule_keyring/src/store/types/key_entry.rs`.
- entropy: `userland/capsule_entropy/src/protocol/types.rs`, `userland/capsule_entropy/src/pool/`.
- crypto: `userland/capsule_crypto/src/protocol/types.rs`,
  `userland/capsule_crypto/src/protocol/primitives.rs`, `userland/capsule_crypto/src/server/dispatch.rs`,
  `userland/capsule_crypto/src/server/wipe.rs`.
- policy: `userland/policy_proto/src/ops.rs`, `userland/capsule_policy/src/server/handle_set.rs`,
  `userland/capsule_policy/src/push/raw.rs`.
- attest: `userland/capsule_attest/src/protocol/ops.rs`, `userland/capsule_attest/Capsule.mk`.
- installer: `userland/capsule_installer/src/protocol/types.rs`,
  `userland/capsule_installer/src/server/selfinstall.rs`, `userland/capsule_installer/Cargo.toml`.
- payment: `userland/capsule_payment/src/protocol/types.rs`,
  `userland/capsule_payment/src/server/sign_call.rs`, `userland/capsule_payment/src/server/handlers/drain.rs`.
- market: `userland/capsule_market/src/protocol/ops.rs`, `userland/capsule_market/src/verify/crypto.rs`,
  `userland/capsule_market/src/verify/mod.rs`, `userland/capsule_market/Cargo.toml`.
- power: `userland/capsule_power/src/protocol/ops.rs`, `userland/capsule_power/Capsule.mk`.
- process-manager: `userland/capsule_process_manager/src/pm/sample.rs`,
  `userland/capsule_process_manager/Capsule.mk`.
- Boot order and markers: `src/userspace/init/capsule_boot/run.rs`, `src/userspace/init/spawn_plan/core.rs`,
  `src/userspace/init/spawn_plan/desktop_services.rs`, `src/userspace/init/spawn_plan/apps_tools.rs`,
  `src/userspace/init/spawn_plan/boot.rs`.
