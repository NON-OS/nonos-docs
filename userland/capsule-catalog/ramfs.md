...
[vfs pool](vfs.md). This capsule is the store the kernel routes to when a path lives under `/ram`, and it
...
`/ram` is not served by the [vfs pool](vfs.md); it is served by this capsule, and the routing lives in the
