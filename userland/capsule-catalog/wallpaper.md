...
compositor's Z order, reads the selected wallpaper index from the [policy store](policy.md), streams that
image out of the [wallpaper catalog](wallpaper-catalog.md) in bounded chunks, decodes and paints it into
...
The wallpaper capsule does not decide which image to show; the [policy store](policy.md) does. The store
...
is documented in the [wallpaper catalog](wallpaper-catalog.md) page.
...
[wallpaper catalog](wallpaper-catalog.md) page.
