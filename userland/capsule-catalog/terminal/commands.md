...
[terminal overview](README.md); this page is only the engine. Every command a user can type is listed
...
most `MAX_ARGS` tokens into a fixed `Argv` (`parse/split.rs:20`, `parse/types.rs:17`). `MAX_ARGS` is 8,
and `Argv` holds the token slices plus a count, all borrowing the original line
(`parse/types.rs:19`). A token wrapped in matching single or double quotes is taken verbatim between the
quotes, so an argument may contain spaces (`write notes.txt "hello world"`); only the quote bytes are
excluded, and an unterminated quote runs to end of line (`parse/split.rs:35`). The module re-exports just
`parse` and `Argv` (`parse/mod.rs:20`).
...
(`dispatch/statements.rs:29`). `Conn` is the gate: `Always` for `;`, `And` for `&&`, `Or` for `||`
(`dispatch/statements.rs:22`). A single `|` is left untouched here so the pipeline stage can split it
(`dispatch/statements.rs:21`). The caller runs `b` after `a` only when the connector and `a`'s exit
status agree: `&&` needs success, `||` needs failure, `;` always runs.

### The runner

`run` is the top-level entry for one tokenized statement (`dispatch/run.rs:33`). Its order is fixed:

1. An empty argv repaints and does nothing (`dispatch/run.rs:34`).
2. `exit` or `quit` as the first word ends the shell, returning `Outcome::Exit`
   (`dispatch/run.rs:38`, via `builtin/exit_check.rs:17`).
3. `split_input` pulls off a `< file` input redirect, returning the args with the `< path` pair removed
   plus the captured path (`dispatch/run.rs:41`, `dispatch/redirect.rs:31`).
4. `split` scans the remaining args for a `>` or `>>` output redirect, returning the command length, the
   append flag, and the destination path (`dispatch/run.rs:48`, `dispatch/redirect.rs:54`).
5. If there is no input redirect, no `|`, and no output redirect, the command fast-paths straight to
   `exec` and prints to the screen (`dispatch/run.rs:57`).
...
`exec` resolves the first token (`dispatch/exec.rs:24`). A fixed set of verbs is matched directly; every
other word, and anything after `nox`, falls through to the `nox` family (`dispatch/exec.rs:48`). The
direct verbs are `nox`, `help`, `about`, `version`, `whoami`, `capsules`/`caps`, `clear`, `display`,
`echo`, `history`, `market`, `motd`, `ping`, and `service`/`svc` (`dispatch/exec.rs:29` through `:47`).
These are listed as the [top-level verbs](#top-level-verbs) below.

### Redirects

`dispatch/redirect.rs` scans for both redirect directions. `split_input` removes a `< path` pair and
errors with `redirect: expected a file path after <` when the path is missing (`dispatch/redirect.rs:38`).
`split` returns a `Plan`: `Plain`, `Redirect { cmd_len, append, path }` where `append` is true for `>>`,
or `Error` with `redirect: expected a file path after >` (`dispatch/redirect.rs:22`,
`dispatch/redirect.rs:57`). `write_redirect` performs the write: it resolves the path against `cwd`,
...
(`dispatch/write_redirect.rs:30`, `dispatch/write_redirect.rs:46`). Both reads are capped at 64 KiB
(`dispatch/write_redirect.rs:24`).

### Pipes

`dispatch/pipeline.rs` runs a `a | b | c` chain. `run_pipeline` splits the args on `|`, runs the first
segment with its scrollback output captured, then folds each later segment as a filter over the
accumulated lines (`dispatch/pipeline.rs:27`). `run_filters` is the same fold with no producer command,
used when `< file` supplies the seed instead of a leading command (`dispatch/pipeline.rs:53`). The
...
(`dispatch/filter/mod.rs:24`). An unrecognised filter word yields a `pipe: unknown filter <name>` line
(`dispatch/filter/mod.rs:34`). The finite filter set:

| Filter | Syntax | What it does | Handler |
|---|---|---|---|
| `grep` | `grep [-i] [-v] <pattern>` | keep matching lines; `-i` ignores case, `-v` inverts | `filter/mod.rs:26`, `filter/text.rs:22` |
| `sort` | `sort` | sort lines (unstable, byte order) | `filter/mod.rs:27`, `filter/text.rs:35` |
| `uniq` | `uniq` | drop repeated adjacent lines | `filter/mod.rs:28`, `filter/text.rs:42` |
| `cut` | `cut [-d<c>] [-f<n>]` | emit the n-th delim-separated field (1-based; default space, field 1) | `filter/mod.rs:29`, `filter/text.rs:68` |
| `nl` | `nl` | number each line, 1-based, two spaces after the number | `filter/mod.rs:30`, `filter/text.rs:52` |
| `wc` | `wc` | emit the line count | `filter/mod.rs:31`, `filter/count.rs:22` |
| `head` | `head [n]` | keep the first n lines (default 10) | `filter/mod.rs:32`, `filter/count.rs:28` |
| `tail` | `tail [n]` | keep the last n lines (default 10) | `filter/mod.rs:33`, `filter/count.rs:32` |

`grep` with an empty pattern keeps every line; matching is a plain substring scan, case-folded under
`-i` (`filter/text.rs:96`). `head` and `tail` default their count to 10 when the argument is missing or
not a number (`filter/count.rs:37`).
...
body is not itself re-aliased (`dispatch/alias_expand.rs:22`). `expand` substitutes `$NAME` shell
variables, suppressed inside single quotes the way POSIX does, with an undefined variable expanding to
nothing and quote bytes preserved for the tokenizer to strip (`dispatch/expand.rs:23`). A name starts at
a letter or underscore and continues over alphanumerics and underscores (`dispatch/expand.rs:35`).

### Outcome

Every dispatched command returns an `Outcome`, which is either `Repaint` or `Exit` (`dispatch/outcome.rs:17`).
...
`version`, and `whoami` (`builtin/mod.rs:17`). Most commands, though, live in the `nox` family; the
top-level verbs are a small curated set, several of them thin wrappers the `nox` family also exposes.

### Top-level verbs

Matched in `exec` before the `nox` fall-through (`dispatch/exec.rs:29`).

| Command | Syntax | What it does | Source |
|---|---|---|---|
| `nox` | `nox [verb ...]` | dispatch into the `nox` family; no verb prints the help index | `exec.rs:30` |
| `help` | `help` | print the `nox` help index | `exec.rs:31`, `nox/help.rs:19` |
| `about` | `about` | print the capsule self-description (CPL, wire, syscalls) | `exec.rs:32`, `builtin/about.rs:19` |
...
| `capsules` / `caps` | `capsules` | list expected system services, each marked `[live]` or `[absent]` | `exec.rs:35`, `builtin/capsules.rs:47` |
| `clear` | `clear` | clear the scrollback | `exec.rs:38`, `builtin/clear.rs:19` |
...
| `motd` | `motd` | print the banner | `exec.rs:45`, `builtin/motd.rs:20` |
...
(`builtin/capsules.rs:22`).

### builtin/nox: the command family

`nox/dispatch.rs` is the family's dispatch table. An empty argv prints the help index
(`nox/dispatch.rs:27`); otherwise the first word selects a handler and the rest are its arguments
(`nox/dispatch.rs:31`). Each command yields a success boolean that is written to `state.last_status`,
which is what `&&` and `||` gate on; fallible commands return their own result, infallible commands
always report success, and an unknown word is a failure (`nox/dispatch.rs:35`, `nox/dispatch.rs:135`).
The family modules are declared in `nox/mod.rs:17`.

Filesystem, all resolved against the shell's `cwd` and run through the app-skeleton vfs client, caching
the owner pid with `ensure_pid` (`nox/ensure_pid.rs:21`):
...
`find` is a single-level prefix listing, not a recursive walk (`nox/find.rs:31`). `du` sums only the
files the same listing returns, not a recursive tree (`nox/du.rs:41`). `ls` reduces the vfs path list to
immediate children through `children` (`nox/children.rs:20`).
...
| `run` / `open` | `run <app>` | focus an already-running desktop app with an NCTL focus frame | `dispatch.rs:88`, `nox/run.rs:29` |
...
| `echo` | `echo [text ...]` | print the arguments | `dispatch.rs:80`, `nox/echo.rs:21` |
| `apps` / `market` | `market` | list the marketplace catalog | `dispatch.rs:84`, `nox/apps.rs:21` |
| `motd` | `motd` | print the banner | `dispatch.rs:110`, `nox/motd.rs:20` |
| `history` | `history` | print the numbered history | `dispatch.rs:114`, `nox/history.rs:22` |
| `display` | `display` | query the primary display | `dispatch.rs:118`, `nox/display.rs:22` |
| `clear` | `clear` | clear the scrollback | `dispatch.rs:122`, `nox/clear.rs:19` |
...
(`nox/id.rs:21`), `caps` calls `capsules` (`nox/caps.rs:21`), `svc` calls `service` (`nox/svc.rs:21`),
`apps` calls `market` (`nox/apps.rs:21`), `sys` prints `version` then `about` (`nox/sysinfo.rs:21`),
...
reports failure, so `nonexistent || echo no` prints the fallback (`nox/dispatch.rs:130`,
`nox/unknown.rs:21`).

### builtin/nox/ifconfig

The `ifconfig` command is its own subtree because it speaks a DHCP client wire. `run` looks up the lease
and either prints the formatted line or `net0: down`, always reporting success (`nox/ifconfig/run.rs:23`).
It talks to the `net.dhcp.client` service, magic `0x4E44_4843`, `OP_LEASE_STATUS 3`, with a 64 ms timeout,
and treats state 3 as bound (`nox/ifconfig/constants.rs:17`). `lookup` resolves the service
(`nox/ifconfig/lookup.rs:21`) and `call` issues the timed IPC round trip and parses the reply
(`nox/ifconfig/call.rs:24`).

### builtin/nox/install

`install` is the only command that brings a new capsule to life, and it does so at arm's length. `run`
validates the name, packs an argv blob, and calls the installer (`nox/install/run.rs:24`). The name is
constrained to ascii letters, digits, `_`, and `-`, at most 64 bytes, with no path separators, so the
shell cannot point the installer outside the store (`nox/install/run.rs:97`). `call_installer` resolves
the `installer` service and sends `seq(4) | op(2) | pad(2) | REQUESTED_CAPS(8) | name_len(1) | name |
argv`, with `OP_LOAD_BY_NAME 4`; the reply is `seq(4) | status(4) | pid(4)` (`nox/install/call.rs:30`,
`nox/install/call.rs:52`). The `REQUESTED_CAPS = u64::MAX` field is a request, not a grant: the verified
manifest and the identity certificate ceiling are the real bounds, and this field only avoids restricting
a capsule below what it legitimately declares (`nox/install/call.rs:22`). After a successful load the
shell drains the child's stdout from its `proc.<pid>` inbox into the window, bounded by a 5 s deadline so
a child that never exits cannot freeze the shell (`nox/install/run.rs:54`). Debug markers
`[TERMINAL-INSTALL] load ok`, `[TERMINAL-INSTALL] load failed`, and `[TERMINAL-MOUT] output drained`
trace the path (`nox/install/run.rs:38`).
...
(`builtin/market/run.rs:27`). The list op is carried on the shared NCMP wire (see below); the reply body
starts at offset 8 and the response buffer is capped at 4096 bytes (`builtin/market/constants.rs:17`).
...
(`builtin/ping/mod.rs:40`, `builtin/ping/resolve.rs:28`). It uses the `net.ip` service, magic
`0x4E49_5034`, `OP_SEND_PACKET 4` and `OP_POLL_PACKET 5`, ICMP protocol, with a 1 s deadline
(`builtin/ping/mod.rs:30`). The probe loop returns one of reply-with-rtt, no route, not ready,
unreachable (no ARP reply), timed out, or send failed, each mapped to a specific line
(`builtin/ping/probe.rs:47`, `builtin/ping/mod.rs:57`).

## output: formatting a reply

`command/output/` is the thin formatting layer every command writes through. `Output` wraps a mutable
`Scrollback` reference (`output/types.rs:19`), `new` constructs one over the state's scrollback
(`output/new.rs:20`), and `writeln` pushes one line into it (`output/writeln.rs:20`). The module
re-exports only `Output` (`output/mod.rs:21`). Commands that need error styling or raw byte feeds reach
the scrollback directly (for example `push_error` in `nox/read.rs:40` and `feed_raw` in
`nox/install/run.rs:63`); everything else prints through `Output::writeln`.
...
and the payload length, all little-endian (`wire/encode_header.rs:19`, `wire/constants.rs:17`). `HDR_LEN`
is 20 (`wire/constants.rs:19`) and `OP_LIST_APPS` is 2 (`wire/market_op.rs:17`). The market command
builds a header-only request with this encoder and issues the call through `mk_ipc_call`
(`builtin/market/call_list.rs:21`).
...
filesystem commands and both redirect forms; the installer wire in `nox/install/call.rs`; the DNS wire,
magic `0x4E44_4E53`, `OP_RESOLVE_A 2`, 2 s timeout, in `nox/nslookup.rs:23`; the IP wire in
`builtin/ping/mod.rs`; and the DHCP client wire in `nox/ifconfig/constants.rs`. In every case the shell
...
filesystem command that first caches the terminal's own pid (`builtin/service.rs:30`,
`nox/ensure_pid.rs:21`).
