# Debugging capsule_terminal

This page lists the log markers the terminal and its boot path emit, and the concrete failure modes with
where to look for each. For the shell model see the [README](README.md), the
[command reference](commands.md), the [input model](input.md), the [rendering](rendering.md), and the
[terminal emulation](emulation.md) pages in this folder.
...
"capsule spawned")` (`src/userspace/init/capsule_boot/run.rs:29`). If that line is absent the capsule
...
after the installer returns a pid (`src/command/builtin/nox/install/run.rs:38`), and a failure prints the
matching failure marker from the `Err` arm. The child's captured stdout is drained after a load
(`src/command/builtin/nox/install/run.rs:39`).
...
[contributing](contributing.md) page for the `nonos-mk-terminal-test` target.
...
key-down (`src/event/on_event.rs:23`). If keys do nothing at all, the shell never sees them, so the
suspect is the input path into the app (compositor, wm, input_router), not the line editor. Run `caps`
to see whether those services are reported live (`src/command/builtin/nox/caps.rs:21`).

### Command not found

An unrecognised word falls through the dispatch to the `other =>` arm, which runs `unknown` and reports
failure (`src/command/builtin/nox/dispatch.rs:130`). `unknown` prints `nox: unknown verb '<word>' ...`
(`src/command/builtin/nox/unknown.rs:21`). Because every command writes `state.last_status`, the fastest
...
- Filesystem: a failing command sits next to a working one in the same directory (a failing `read` beside
  ...
  for example `src/command/builtin/nox/stat.rs:27` on a usage error. Paths resolve against the shell's
  `cwd`, not the caller's, so check `where` before assuming a vfs bug (`src/term/cwd/resolve.rs`).
- Network: `ping` and `nslookup` are the probes. `ping` prints a distinct line for each stage: `ping: dns
  ...
  (`src/command/builtin/ping/mod.rs:47`). The message names the stage that failed.
- Install: a rejected name never reaches IPC. `valid_name` requires a non-empty stem, at most 64 bytes,
  ...
  (`src/command/builtin/nox/install/run.rs:97`); a name outside that set is refused in the shell. A name
