...
batches. It holds no key material. The signing is delegated to the [keyring](keyring.md), and it holds no
...
The capsule never touches key material. Signing is a synchronous IPC call to the [keyring](keyring.md),
...
Signing, to the [keyring](keyring.md) over a synchronous `mk_ipc_call` (`src/server/sign_call.rs:25`). The
