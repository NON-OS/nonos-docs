...
|---------|------------------|----------------|------|-----------|
| [compositor](compositor.md) | `compositor` :4310 | :4311 | `0x7919` | Owns the screen: scene layers, damage-driven compositing, vsync present. |
| [wm](wm.md) | `wm` :4330 | :4331 | `0x19` | Window lifecycle, z-order, focus, and the hit-test/focus queries the input router uses. |
| [input-router](input-router.md) | `input_router` :4320 | :4321 | `0x19` | Drains the kernel input ring and routes to windows by hit-test and focus, with trusted-only grabs. |
| [desktop-shell](desktop-shell.md) | `desktop_shell` :4410 | :4411 | `0x1819` | The taskbar, tray, toasts, and spotlight; coordinates the desktop services. |
| [wallpaper](wallpaper.md) | `wallpaper` :4340 | :4341 | `0x1819` | The background: color or catalog image, with a fade timeline. |
| [wallpaper-catalog](wallpaper-catalog.md) | `wallpaper_catalog` :4110 | :4111 | `0x19` | Serves built-in wallpaper metadata and chunked image bytes. |
| [image-codec](image-codec.md) | `image_codec` :4412 | :4413 | `0x1819` | Decodes PNG/BMP/JPEG/LZ4 to ARGB with real toolkit decoders; isolated because images are untrusted. |
| [clipboard](clipboard.md) | `clipboard` :4414 | :4415 | `0x19` | Bounded copy history that wipes itself on idle. |
| [login](login.md) | `login` :4416 | :4417 | `0x19` | Session gate: unlocks the keyring (which is authoritative), owner-pid enforced. |
| [toolkit](toolkit.md) | `toolkit` :4610 | :4611 | `0x19` | Stateless theme, animation, and component-render RPC. |
| [boot-splash](boot-splash.md) | `app.boot_splash` (reserved :4796, never bound) | :4797 | `0x1819` | Boot screen that displays the kernel's attestation badge; it displays, it does not verify. It is a pure compositor client. |
| [setup-wizard](setup-wizard.md) | `app.setup_wizard` :4794 | :4795 | `0x1819` | First-run config wizard that commits choices to policy. |
...
(`capsule_boot_splash/src/surface.rs:47`) and looks the compositor up as a client
(`capsule_boot_splash/src/main.rs:78`).

## Capability masks, decoded

Every capability bit is defined in `userland/nonos_cap/src/bits.rs`. The three masks the fleet uses:
...
1. The compositor is passive. It owns the frame (`compositor/src/frame_pacer/vsync.rs`,
   `compositor/src/frame_pacer/composite.rs`) and the scene table
   (`compositor/src/state/scene/mod.rs`), but it never decides placement or focus. Clients submit
...
3. The input router is the single consumer of the kernel input ring. It drains the ring
   (`capsule_input_router/src/server/drain_ipc.rs`) and fans events out to the focused window, so no
...
is the [lifecycle](../lifecycle.md) page. The surface and input mechanisms the fleet rides on are the
[graphics](../../subsystems/graphics/README.md) and [input](../../subsystems/input/README.md)
subsystems, and the raw event source is the [input ring](../../subsystems/input/README.md).
...
- Input isolation. Only the input router reads the kernel input ring. Grabs (taking exclusive input) are
  gated: `capsule_input_router/src/server/handlers/grab_request.rs:25` hardcodes the only three capsules
...
- Untrusted-data isolation. Images are attacker-controlled bytes, so decode is its own capsule
  ([image-codec](image-codec.md)) with only `0x1819`. A decoder bug is contained to a capsule that cannot
  touch input, the keyring, or a device.
- Secret handling. The [login](login.md) capsule is a gate, not a vault: it unlocks the
  [keyring](keyring.md) (`capsule_login/src/clients/keyring.rs`, `OP_UNLOCK = 5`), and the keyring stays
  authoritative. The [clipboard](clipboard.md) bounds its history and wipes on idle
  (`capsule_clipboard/src/protocol/limits.rs:22` `DEFAULT_IDLE_TIMEOUT_MS = 600_000`;
  `capsule_clipboard/src/server/runner.rs:33` `expire_if_idle`), so copied secrets do not linger forever.
...
- Splash never leaves. The boot-splash lingers until the desktop shell registers, then settles, with a
  hard cap so it never hangs (`capsule_boot_splash/src/main.rs`: `SETTLE_MS = 1000`,
...
- Clicks or keys go nowhere. The input router routes by the wm's hit-test and focus. Confirm `wm` :4330
  ...
  frees it (`capsule_input_router/src/server/handlers/grab_release.rs`).
- A window is placed off-screen or overlaps wrongly. That is the wm's placement path
  (`capsule_wm/src/server/handlers/window_open/place.rs`), not the compositor.
- An image will not display. Decode failures come back from image-codec as a negative errno
  (`capsule_image_codec/src/protocol/errno.rs`: `E_UNSUPPORTED = -95`, plus `E_INVAL` and `E_BAD_LEN`
  from `capsule_image_codec/src/server/handlers/decode.rs:57` for bad magic, bad dimensions, too-small
...
- Copied text vanished. The clipboard wipes on idle after `DEFAULT_IDLE_TIMEOUT_MS` (ten minutes) unless
  ...
  (`capsule_clipboard/src/server/handlers/set_idle_timeout.rs`, bounded by `MIN_IDLE_TIMEOUT_MS` and
...
- `userland/compositor/` (main.rs, `frame_pacer/`, `state/scene/`, `state/damage.rs`, `Capsule.mk`)
- `userland/capsule_wm/` (main.rs, `server/handlers/`, `Capsule.mk`)
- `userland/capsule_input_router/` (main.rs, `server/drain_ipc.rs`, `server/handlers/grab_request.rs`,
  `server/handlers/grab_release.rs`, `protocol/errno.rs`, `Capsule.mk`)
- `userland/capsule_desktop_shell/` (`setup/discover/constants.rs`, `state/indicators/`, `Capsule.mk`)
- `userland/capsule_wallpaper/` and `userland/capsule_wallpaper_catalog/` (`Capsule.mk`, `bootstrap/`)
- `userland/capsule_image_codec/` (`protocol/errno.rs`, `server/handlers/decode.rs`, `Capsule.mk`)
- `userland/capsule_clipboard/` (`protocol/limits.rs`, `server/runner.rs`,
  `server/handlers/set_idle_timeout.rs`, `Capsule.mk`)
- `userland/capsule_login/` (`clients/keyring.rs`, `setup/discover/`, `Capsule.mk`)
- `userland/toolkit/` (`protocol/ops.rs`, `Capsule.mk`)
- `userland/capsule_boot_splash/` (main.rs, `surface.rs`, `proto.rs`, `Capsule.mk`)
- `userland/capsule_setup_wizard/` (`Capsule.mk`)
- `userland/nonos_cap/src/bits.rs` (capability bit definitions)
