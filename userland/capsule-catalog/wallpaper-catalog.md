...
MMIO, IRQ, DMA, PIO, filesystem, network, display, or focus-routing grants. The [wallpaper](wallpaper.md)
capsule is the primary client; the [desktop shell](desktop-shell.md) drives selection through the
[policy store](policy.md).
...
by index. Selection lives in the [policy store](policy.md). The store keeps a single `wallpaper: u8`
...
The [wallpaper](wallpaper.md) capsule reads that field over IPC. `get_wallpaper`
...
The [desktop shell](desktop-shell.md) also nudges the wallpaper capsule at prime time through
