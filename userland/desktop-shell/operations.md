...
[surface.md](surface.md); for the state they touch see [state.md](state.md); for the outbound calls the
shell makes in response see [clients.md](clients.md).

## The NDSH frame protocol

The shell serves its own frame protocol on its inbound port. The magic is `NDSH` (`0x4E44_5348`), the
version is 1, and the header is 20 bytes (`src/protocol/header.rs:17`, `header.rs:18`, `header.rs:19`).
...
`E_BAD_MAGIC`, a wrong version is `E_BAD_VERSION` (`src/protocol/decode.rs:20`, `decode.rs:39`,
`decode.rs:45`, `decode.rs:51`). Every reply is a 4-byte status word written by `respond::status`, which
builds the response header and calls `mk_ipc_reply` (`src/server/respond.rs:21`,
`src/protocol/limits.rs:18`). Error codes are the usual negatives (`src/protocol/errno.rs:17`):
...
The shell serves six operations on its `NDSH` port (`src/protocol/ops.rs:17`), routed by `dispatch`
...
`OP_HEALTHCHECK` replies with status 0 and nothing else (`src/server/handlers/health.rs:21`).
...
length (`src/protocol/limits.rs:20`, `limits.rs:24`).
...
- `TRAY_REGISTER` rejects a body of the wrong length or a bad `tray_id`/label length with `E_INVAL`, a
  ...
  sender pid, then repaints the menu bar and commits damage (`src/server/handlers/tray_register.rs:24`,
...
- `TRAY_UPDATE` relabels an entry the caller owns via `find_mut`, or returns `E_NOENT`
  (`src/server/handlers/tray_update.rs:40`).
- `TRAY_REMOVE` drops an owned entry via `remove`, or returns `E_NOENT`
  (`src/server/handlers/tray_remove.rs:32`).
...
(`tray_register.rs:41`, `src/state/tray/table.rs:51`, `table.rs:56`).
...
(`src/server/handlers/notify.rs:26`, `notify.rs:38`, `notify.rs:42`, `notify.rs:48`). The level is one of
Info, Warn, or Error (`src/state/notify.rs:26`). The toast appears above the dock for 4 seconds
(`src/state/toasts.rs:21`). The shell also raises its own `network connected` toast the first time the
link comes up (`src/server/runner/refresh_clock.rs:30`).

### Spotlight

`OP_SPOTLIGHT_OPEN` toggles the spotlight visibility flag, repaints, and commits the spotlight rectangle
(`src/server/handlers/spotlight_open.rs:24`, `spotlight_open.rs:26`). The spotlight is drawn as a
rectangle, but its search UX is not yet wired, so there is no in-panel interaction beyond the toggle
(`src/render/chrome.rs:47`).
...
`launcher_request::request` (`src/server/handlers/launcher_focus.rs:33`). That resolves the target
...
(`src/server/handlers/launcher_request.rs:22`, `launcher_request.rs:29`, `launcher_request.rs:32`,
...
[terminal](../terminal/README.md), which is the capsule that can ask the installer to spawn a store
capsule.
...
(`src/server/wm_notify.rs:26`, `wm_notify.rs:28`, `wm_notify.rs:31`, `wm_notify.rs:37`). `handle`
...
launcher service and matching the pid (`wm_notify.rs:52`, `src/server/wm_notify_app_index.rs:21`), flips
that entry's open state, and raises a toast for the event (`wm_notify.rs:53`, `wm_notify.rs:56`,
`src/server/wm_notify_toast.rs:22`).

## The loop

`server::run` paints the initial chrome, then loops (`src/server/runner/run.rs:30`, `run.rs:35`):

1. `drain` receives inbound frames. It blocks for up to 1000 ms when the shell is idle and polls at 16 ms
   while a subscription is still pending (`src/server/runner/drain.rs:27`,
   `src/server/runner/constants.rs:18`, `constants.rs:19`). Each frame is classified: a `NWMV` lifecycle
