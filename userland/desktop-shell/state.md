...
flag, and the indicator data sources. The [operations](operations.md) handlers and the
[surface](surface.md) renderer both read and write this state; the [clients](clients.md) fill the parts
that come from other services.

## The Context

The `Context` is the whole live state, threaded through every handler as `&mut`
(`src/state/context.rs:19`):
...
service handle it focuses (`src/state/apps.rs:30`, `apps.rs:36`). The nine entries and their handles are
tabulated on the [surface](surface.md) page. The array length is the single source of truth for the dock
size: the taskbar arrays, the dock width, and the hit-test all derive from `LAUNCHER_APPS.len()`
(`src/state/taskbar/types.rs:17`, `src/render/layout.rs:21`).
...
(`src/state/taskbar/types.rs:20`). `active` is a `u8` where `0xFF` means no active entry
...
The tray is a fixed array of 32 owner-tagged slots (`src/state/tray/entry.rs:19`,
`src/state/tray/table.rs:20`). Each `TrayEntry` carries the owner pid, the tray id, a label length, up to
24 label bytes, and an `in_use` flag (`entry.rs:22`, `src/protocol/limits.rs:20`). `insert` rejects a
...
(`src/state/toasts.rs:19`, `toasts.rs:20`, `toasts.rs:21`). `push` truncates the text, fills the first
