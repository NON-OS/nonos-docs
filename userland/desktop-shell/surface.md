...
`src/render/`, which turns the live state into overlay pixels, and `src/server/input.rs`, which is the
only path a pointer or touch takes into the shell. For the served operations that other capsules drive,
see [operations.md](operations.md); for the state these draw from, see [state.md](state.md); for the
identity and lifecycle, see the [README](README.md).
...
clicks, and are covered on the [operations](operations.md) page.
...
(`src/setup/prime/register.rs:25`, `src/setup/prime/register.rs:57`). After that submission the shell
...
`src/compositor_client/damage_commit.rs:22`).

`paint_chrome` is the composite paint (`src/render/chrome.rs:32`). It clears the overlay, paints the menu
bar and its `NONOS launcher` title, paints the dock and its entries only when the dock is visible, paints
...
(`src/render/layout.rs:20`, `layout.rs:34`), the dock is a centered bar 64 pixels tall inset 24 pixels
...
The dock is the launcher. It shows a fixed set of nine apps (`src/state/apps.rs:36`), each a label
...
(`src/render/bottom_taskbar.rs:28`, `bottom_taskbar.rs:71`, `src/render/layout.rs:24`). Every entry draws
...
to the per-app bitmap under `src/render/icons/` (`src/render/icons.rs:35`, `icons.rs:47`).
...
[operations.md](operations.md)), so the dock reflects what is really on screen, not just what was
clicked.

### Launching an app

A button-down or touch inside a dock entry runs `launcher_focus`, which hit-tests the pointer against
each entry's rectangle (`src/server/input.rs:62`, `src/server/handlers/launcher_focus.rs:24`). On a hit
...
(`src/server/handlers/launcher_request.rs:22`, `launcher_request.rs:24`, `launcher_request.rs:29`). On
success the shell marks the entry with a launch pulse for 900 ms and repaints
(`launcher_focus.rs:33`, `src/state/taskbar/mark_launch.rs:21`).
...
The dock auto-hides, and it only paints when it is visible (`src/render/chrome.rs:36`). When it is
hidden, moving the pointer into the bottom 4-pixel band of the screen reveals it
(`src/server/input.rs:71`, `HOVER_REVEAL_BAND` = 4, `input.rs:26`), and a touch or button-down within 18
pixels of the dock's top edge reveals it too (`input.rs:54`, `input.rs:56`). A reveal lasts 1800 ms
(`src/state/taskbar/reveal.rs:21`). When the dock is visible and the pointer moves above it while no
entry is open, it collapses again (`input.rs:81`, `src/state/taskbar/collapse.rs:19`).

## Status indicators

The menu bar carries a right-aligned status area painted by `paint_status` (`src/render/status.rs:28`).
...
|---|---|---|
| Battery | percentage `NN%`, or `AC` when no battery or the reading is out of `0..=100` | `src/state/indicators/battery.rs:19` |
| Network | `NET` when the DHCP lease is bound, `OFF` otherwise | `status.rs:33`, `src/state/indicators/net.rs:28` |
| Date | `YYYY-MM-DD` from the RTC | `src/state/indicators/clock.rs:40` |
| Time | `HH:MM` from the RTC, 12h or 24h per policy | `src/state/indicators/clock.rs:19` |
...
comes from the `policy` service's `CLOCK_FORMAT24` field (`src/state/indicators/policy.rs:24`,
...
the whole status area is skipped (`status.rs:48`). The menu-bar title reads `NONOS launcher`
(`chrome.rs:35`).

## The pointer input path

The shell subscribes to the input router and receives `NINP` frames on its inbox (magic `0x4E49_4E50`,
`src/server/input.rs:29`). `handle` rejects a short buffer or a wrong magic, reads the event kind at
...
pointer-abs, touch, and button-down kinds change anything (`src/setup/prime/run/input_mask.rs:25`).
...
(`src/server/refresh_taskbar.rs:23`), the menu bar handlers use the menu-bar rectangle (for example
`src/server/handlers/notify.rs:50`), the spotlight uses the spotlight rectangle
(`src/server/handlers/spotlight_open.rs:26`), and the once-a-second clock refresh commits the full
screen (`src/server/runner/refresh_clock.rs:36`). The shell never presents a frame itself; the
compositor does, on receipt of the damage commit (`src/compositor_client/damage_commit.rs:35`).
