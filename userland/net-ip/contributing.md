# Contributing to capsule_net_ip
...
the [README](README.md), the [operations](operations.md) page, the [ipv4](ipv4.md) page, the
[icmp](icmp.md) page, the [routing](routing.md) page, and the [state](state.md) page.
...
(`src/main.rs:38`). The top-level modules are declared there (`src/main.rs:22`). It fits into the
decomposed [network stack](../../subsystems/networking/README.md) between the transport capsules and
`net.l2`.
...
| `src/server/authz.rs` | the registry-pid authorization for the control ops | you change who may configure or route |
| `src/ipv4/` | RFC 791 parse and build, the RFC 1071 checksum, address helpers | you touch the datagram format |
| `src/egress/`, `src/ingress.rs` | the outbound and inbound framing that wrap `ipv4` | you change how frames enter or leave |
...
| `src/setup.rs` | bring-up: resolve `net.l2`, read the MAC | you change the startup dependency order |

## Adding a client op

There are three edits, and the dispatch wiring is the load-bearing one.

1. Add the opcode constant to `src/protocol/ops.rs:21` and re-export it from `src/protocol/mod.rs:28`.
   New ops require a constant here; the dispatch table never routes by name (`src/protocol/ops.rs:17`).
...
3. Wire it into the dispatch match in `src/server/runner.rs:45`, passing `sender_pid`, the parsed
...
If the op adds a new failure condition, add its errno to `src/protocol/errno.rs:22` at the end of the
...
- One unit per file. New ops are one file per handler under `src/server/handlers/`, and the existing
  ...
  re-exports, as in `src/ipv4/mod.rs:17` and `src/protocol/mod.rs:22`.
- Keep the kernel spawn mirror in step with the manifest. The requested capabilities in
  `src/userspace/capsule_net_ip/spawn.rs:55` must stay a subset of the manifest ceiling in
