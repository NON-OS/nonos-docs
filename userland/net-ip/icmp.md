...
path that calls this responder see the [ipv4](ipv4.md) page; for the egress the reply is sent through
see the same page.

## The ICMP header

An ICMP message is an 8-byte header followed by a payload (`src/icmp/types.rs:17`). The capsule cares
about two type values, echo reply (0) and echo request (8), and it treats the checksum field as sitting
at offset 2 (`src/icmp/types.rs:19`). The header view it keeps is the type byte, the code byte, and the
4-byte rest field that carries the identifier and sequence for echo (`src/icmp/types.rs:24`).

## Parsing an ICMP message

`icmp::parse` validates the checksum over the entire message, header plus payload, and returns the
header view and the payload slice on success (`src/icmp/parse.rs:29`). It rejects a buffer shorter than
...
and package them with the payload (`src/icmp/echo.rs:29`). `is_echo_request` is the type-and-code test:
...
(`src/icmp/build.rs:29`). It refuses an output buffer too small for the header plus payload
...
(`src/icmp/responder.rs:37`, `src/ingress.rs:61`). It returns quickly for anything that is not an ICMP
...
(`src/ingress.rs:61`). A build failure still returns true, consuming the request rather than surfacing
