...
[README](README.md), the [operations](operations.md) page, the [ipv4](ipv4.md) page, the
[routing](routing.md) page, and the [state](state.md) page.
...
(`src/userspace/init/spawn_plan/network/spawn_ip.rs:21`), which reaches `capsule_boot::boot`, whose
`Ok` arm calls `boot_log::ok(prefix, "capsule spawned")` (`src/userspace/init/capsule_boot/run.rs:29`),
and `ok` formats `[` + tag + `] ` + message (`src/sys/boot_log/output.rs:33`). An absent line means the
capsule never started, usually a signature, manifest, or capability failure; the `Err` arm prints an
`[ERROR]` line instead (`src/userspace/init/capsule_boot/run.rs:32`). The spawn itself only runs when
...
[stack](../../subsystems/networking/stack.md) page).
...
request server once setup succeeds (`src/main.rs:46`). So there is no bring-up exit code to read; a
...
when the MAC read through L2 fails (`src/setup.rs:38`). The fix is upstream: confirm `net.l2` spawned
...
is on the [operations](operations.md) page; these are the ones with a real diagnostic story.
...
address or a zero L2 port (`src/egress/send.rs:35`, `src/egress/send.rs:42`), and the poll path returns
`E_NO_CONFIG` when no L2 port is known (`src/server/handlers/poll_packet/poll.rs:37`). This is the
pre-DHCP state: the address is filled by `OP_SET_CONFIG` from the DHCP client, so the probe is
`OP_GET_CONFIG`, which shows a zero address until the lease lands (`src/server/handlers/get_config.rs:29`).
...
the route table was cleared (`src/server/handlers/set_config.rs:46`, `src/route/table.rs:53`). Add an
...
the caller is expected to back off and retry while the neighbour answers (`src/l2_client/arp.rs:50`,
`src/egress/send.rs:57`). A persistent `E_NO_NEIGHBOUR` points at the L2 or link side: the neighbour is
...
failure and an internal build failure into this one errno (`src/egress/send.rs:31`,
`src/egress/frame.rs:38`, `src/server/handlers/send_packet.rs:39`). A build failure is a local bug
...
the eight-frame L2 poll budget came up empty (`src/server/handlers/poll_packet/poll.rs:48`). A caller
polls again on the next tick. `E_BAD_PACKET` means either the poll filter body was longer than one byte
(`src/server/handlers/poll_packet/select.rs:24`) or a frame drained from L2 failed to parse as IPv4
(`src/server/handlers/poll_packet/route.rs:46`); the latter is a wire or `net.l2` problem, since a
...
`net.dhcp.client` service (`src/server/handlers/set_config.rs:29`), and `OP_ROUTE_ADD` and
`OP_ROUTE_CLEAR` require `net.admin`, which no capsule registers today, so those two are denied by
default (`src/server/authz.rs:38`, `src/server/handlers/route_add.rs:27`). An unexpected `E_PERM` on
