# Client operations and the NIP4 protocol
...
the capability mask see the [README](README.md); for the IPv4 build and parse the packet ops drive see
the [ipv4](ipv4.md) page.
...
shared header (`src/protocol/header.rs:23`). The decoder rejects anything shorter than the header, a
wrong magic, or a version other than 1, and rejects a payload length that would run past the received
bytes, using a checked add so the bound cannot overflow (`src/server/parse_req.rs:27`).
...
than a separate status word, followed by any body (`src/server/respond.rs:24`). Errno `0` (`E_OK`)
means success; a non-zero value is one of the constants below. Replies go straight to the attested
sender pid with `mk_ipc_reply` (`src/server/respond.rs:58`).
...
without splitting (`src/server/runner.rs:33`, `src/protocol/limits.rs:23`). The loop receives a request
...
(`src/server/runner.rs:37`). An unrecognised opcode is answered with `E_BAD_OP`
(`src/server/runner.rs:53`).

## The seven operations

The opcodes are defined in `src/protocol/ops.rs:21` and dispatched in `src/server/runner.rs:45`.

| Op | Opcode | Request body | Reply body after header | Handler |
|---|---|---|---|---|
| `OP_HEALTHCHECK` | `1` | none | none | `server/handlers/health.rs:21` |
| `OP_GET_CONFIG` | `2` | none | 17-byte interface snapshot | `server/handlers/get_config.rs:28` |
| `OP_SET_CONFIG` | `3` | 9-byte address/prefix/gateway | none | `server/handlers/set_config.rs:28` |
| `OP_SEND_PACKET` | `4` | 4 dst + 1 protocol + payload | none | `server/handlers/send_packet.rs:25` |
| `OP_POLL_PACKET` | `5` | 0 or 1 protocol filter byte | src/dst/protocol + payload | `server/handlers/poll_packet/poll.rs:28` |
| `OP_ROUTE_ADD` | `6` | 9-byte network/prefix/gateway | none | `server/handlers/route_add.rs:26` |
| `OP_ROUTE_CLEAR` | `7` | none | none | `server/handlers/route_clear.rs:23` |
...
- `OP_POLL_PACKET` returns one queued or freshly-polled inbound packet as 4-byte source, 4-byte
  destination, 1-byte protocol, then the payload (`poll_packet/deliver.rs:24`). An empty body means
  "any protocol"; a single byte filters to that protocol number; anything longer is `E_BAD_PACKET`
  (`poll_packet/select.rs:20`). The poll path is detailed below.
...
(`poll_packet/poll.rs:33`). Otherwise, if `net.l2` is known, it polls up to `POLL_BUDGET` (8) frames
...
returns an errno (`poll_packet/poll.rs:40`, `poll_packet/route.rs:31`). With no `net.l2` known the op
returns `E_NO_CONFIG`; an exhausted budget with nothing to deliver returns `E_RX_EMPTY`
(`poll_packet/poll.rs:37`, `poll.rs:48`). The queue is bounded, so a push onto a full queue drops the
packet rather than growing memory (`src/state/rx_queue.rs:26`).
...
registry bound to a named service (`src/server/authz.rs:34`).

- `OP_SET_CONFIG` is allowed only when the sender is the `net.dhcp.client` service; any other caller is
  refused with `E_PERM` (`src/server/handlers/set_config.rs:29`). This keeps the interface lease under
...
- `OP_ROUTE_ADD` and `OP_ROUTE_CLEAR` are tier-2 administrative ops, allowed only for the `net.admin`
  service (`src/server/authz.rs:42`, `route_add.rs:27`, `route_clear.rs:24`). No capsule is spawned as
...
contract: a live value is retired and added, never renumbered (`src/protocol/errno.rs:22`).
...
rather than replying (`src/server/parse_req.rs:32`); the ones a client observes on a reply are the
...
(`src/server/parse_req.rs:31`). The two sensitive control ops are pinned to a specific attested caller
service, and the administrative pair fails closed when no such principal exists
(`src/server/authz.rs:34`). Every handler validates its body length against a fixed layout before it
