...
queue) and `src/setup.rs` (the bring-up that resolves `net.l2` and reads the MAC). For the ops that
read and write this state see the [operations](operations.md) page; for how the receive queue is
...
no heap and no init call (`src/state/iface.rs:55`). It holds the MAC, the IPv4 address, the prefix, the
gateway, the MTU, the resolved `net.l2` service port, and the packet identification counter
(`src/state/iface.rs:22`). Each field carries only the synchronization it needs: the MAC, address, and
...
set, so a host can still answer broadcast before DHCP completes (`src/egress/send.rs:35`,
`src/ingress.rs:58`). `OP_SET_CONFIG` is what fills the address, prefix, and gateway
(`src/server/handlers/set_config.rs:42`), and `OP_GET_CONFIG` reads the whole block back out
(`src/server/handlers/get_config.rs:29`). The L2 port is filled once at bring-up
(`src/setup.rs:42`).
...
fetch-add that skips zero on wrap so an id of 0 is never handed out (`src/state/iface.rs:45`). The
egress frame builder pulls one id per transmit (`src/egress/frame.rs:46`). The link client keeps its
own separate request-id counter for `net.l2` calls with the same zero-skip discipline, so the IP
identification space and the L2 request-id space never collide (`src/l2_client/seq.rs:21`).

## The receive queue

Inbound packets that arrive during a poll but do not match the caller's protocol filter are parked in a
bounded queue rather than dropped or returned out of order (`src/state/rx_queue.rs:24`). It is a
...
(`src/server/handlers/poll_packet/poll.rs:33`, `src/server/handlers/poll_packet/route.rs:41`). A queued
`Packet` is source, destination, protocol, and an owned payload (`src/state/packet.rs:21`).

## Bring-up

`setup::run` is the whole bring-up (`src/setup.rs:35`). It resolves the `net.l2` service through the
...
any client error (`src/setup.rs:38`). It deliberately does not set an IPv4 address: the interface comes
...
tries again, never exiting (`src/main.rs:46`). The capsule only enters its request server once setup
has succeeded (`src/main.rs:42`). This is the difference from a driver capsule that exits with a
