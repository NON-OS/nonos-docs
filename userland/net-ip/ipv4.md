...
helpers) and the two framing modules that wrap it: `src/egress/` on the way out and `src/ingress.rs` on
the way in. For the ops that drive this path see the [operations](operations.md) page; for the ICMP
echo hook inside ingress see the [icmp](icmp.md) page; for the route lookup and the ARP resolve that
egress calls see the [routing](routing.md) page.

## The header view

The capsule keeps only the fields the data path needs from a parsed packet: source, destination, and
protocol number (`src/ipv4/header.rs:29`). Everything else on the wire is validated during parse and
...
of 64, and the checksum field offset of 10 (`src/ipv4/header.rs:19`).

## Parsing an inbound packet

`ipv4::parse` validates and decodes a packet, returning the header view and a slice of the payload
trimmed to the declared total length (`src/ipv4/parse.rs:35`). It rejects, in order:
...
total wire length (`src/ipv4/build.rs:40`). The request carries source, destination, protocol,
...
(`src/ipv4/checksum.rs:17`). `fold` sums the buffer as big-endian 16-bit words, adds a trailing odd
...
same two functions (`src/icmp/build.rs:46`).

## Addresses and subnets

An IPv4 address is a plain 4-byte array (`src/ipv4/addr.rs:17`). The one piece of logic here is subnet
comparison: `mask_with_prefix` applies a `/N` mask that saturates at 32, and `same_subnet` returns
true when two addresses share the same masked network (`src/ipv4/addr.rs:20`). This is what the route
table uses for its longest-prefix match (`src/route/table.rs:62`) and what a future on-link check would
build on.
...
(`src/egress/send.rs:26`). The next hop is the route's gateway if it has one, or the destination itself
...
built length lines up with the allocation (`src/egress/frame.rs:24`). The five failure points map to
the five `EgressError` variants (`src/egress/error.rs:17`), which the send handler turns into wire
errnos (`src/server/handlers/send_packet.rs:34`).
...
(`src/ingress.rs:47`). It then applies the local-delivery filter: once an address is configured, a
...
(`ingress.rs:64`, `src/chaos.rs:30`). What survives is an `Inbound` of source, destination, protocol,
and an owned payload, which the poll path converts to a queued `Packet` (`src/server/handlers/poll_packet/route.rs:50`).
