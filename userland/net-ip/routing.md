...
see the [ipv4](ipv4.md) page; for the ops that add and clear routes see the [operations](operations.md)
page.

## The route table

A route is a network, a prefix length, and an optional gateway; a gateway of `None` means the network
is on-link (`src/route/entry.rs:19`). The table is a fixed array of 16 optional slots behind a reader
and writer lock, allocated as a static so it needs no heap (`src/route/table.rs:23`). Three operations
work on it:

- `install` takes the first free slot and returns an error when all 16 are full, which the add handler
  surfaces as `E_TABLE_FULL` (`table.rs:34`, `src/server/handlers/route_add.rs:42`);
...
agree on what a `/N` match means (`table.rs:62`, `src/ipv4/addr.rs:28`). The default route is seeded
...
separate `OP_ROUTE_ADD` (`src/server/handlers/set_config.rs:46`). The egress path turns a lookup result
into a next hop, the gateway when there is one, otherwise the destination itself for an on-link route
(`src/egress/send.rs:49`).
...
(`src/l2_client/wire.rs:23`). `write_request` lays down that header for a request and `parse_response`
validates the magic and version and pulls the op, errno, request id, and payload length back out of a
reply (`src/l2_client/header.rs:19`). Every call carries a request id from a monotonic sequence counter
that skips zero on wrap (`src/l2_client/seq.rs:21`). All four calls use `mk_ipc_call`, a synchronous
request-and-reply to the L2 service port, rather than the fire-and-reply pattern the inbound server
uses (`src/l2_client/tx.rs:42`).
...
- `read_mac` asks L2 for the NIC's hardware address at bring-up; it validates the reply op and a 6-byte
  ...
  refusal (`src/l2_client/mac.rs:30`). Setup stores this into the interface state
  (`src/setup.rs:44`).
...
- `poll_frame` drains one inbound frame, sizing its receive buffer to the L2 header plus a 1514-byte
  ...
  with the L2 header stripped (`src/l2_client/rx.rs:40`). The poll handler calls this up to eight times
  per request and treats `Empty` as the end of the L2 ring (`src/server/handlers/poll_packet/poll.rs:40`).
...
bit that lets it call the L2 service (see the [README](README.md) mask decomposition). The route table
...
stack](../../subsystems/networking/README.md) is built on, the IP layer speaks addresses and routes,
