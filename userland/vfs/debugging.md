# Debugging capsule_vfs

This page lists the boot marker the vfs pool emits, the errno failure signatures a client sees, and where to
look for each. For the shape of the pool see the [README](README.md), the
[wire protocol and operations](protocol.md), and the [store](store.md) pages in this folder.
...
spawned")` (`src/userspace/init/capsule_boot/run.rs:29`), formatted by `src/sys/boot_log/output.rs:33`. If
...
`vfs_pool` service by name (`userland/app_skeleton/src/clients/vfs/types.rs:17`); if the pool never
...
(`src/server/handlers/util.rs:20`) or the two errnos handlers return directly. Each has a distinct cause.
...
- From `write`, `EACCES` means the fd was opened without write permission, which for an existing file means
  its owner-write bit (`0o200`) was clear at open time (`store/fdtable/open.rs:39`,
  `store/fdtable/write.rs:25`). The permission is fixed at open, so re-opening after a `chmod` is the fix,
  not retrying the write.

### EBADF

An fd used by a pid that did not open it, or an fd out of range (`store/fdtable/lookup.rs:24`-`34`). A wrong
...
(`store/fdtable/types.rs:20`-`22`). A create, `mkdir`, or `copy` past 2048 entries, an open past 256 fds, or
...
most ops do not normalize (see the [store](store.md#the-path-model) page), an `ENOENT` on a path that looks
...
A malformed frame (bad magic, wrong version, or a length that overruns the buffer, `src/protocol/decode.rs`),
a bad path length (zero or over 256), a truncated payload, or non-UTF-8 path bytes. A frame that fails to
decode is answered `EINVAL` by the loop rather than dropped (`src/server/runner.rs:40`), and an unrecognised
opcode is answered `EINVAL` by the dispatcher (`src/server/dispatch.rs:44`).
...
(`userland/fs_proofs/src/lib.rs:30`, `src/vfs_protocol/mod.rs:21`, `src/vfs_store/mod.rs:24`,
`src/vfs_path_tests.rs`). A protocol or path regression usually shows up in the proof suite before it boots,
