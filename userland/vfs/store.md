# The store, the fd table, and paths
...
call into, read the [protocol](protocol.md) page. For identity and the mask, read the [README](README.md).
...
`is_dir` set. There is no inode tree, so `find` is an exact-name linear search (`store/fdtable/lookup.rs:20`)
...
(`store/fdtable/new.rs:22`-`28`), and `Default` forwards to it (`store/fdtable/default.rs:20`).
...
rather than a garbage one (`store/fdtable/time.rs:22`-`29`). Occupancy is reported by `usage`: the current
entry count, the total bytes held, and the entry ceiling (`store/fdtable/usage.rs:22`).

## The seed

At startup `seed` (`store/fdtable/seed.rs:27`) creates `/docs` and an empty `/capsules`, plus three files
...
[caller attestation](protocol.md#caller-attestation): attestation stops impersonation at the request, fd
ownership stops it at the descriptor. `read` and `write` both pull the owner pid through `entry` before
touching data (`store/fdtable/read.rs:24`, `store/fdtable/write.rs:22`), `close` frees the slot only through
`slot_mut` (`store/fdtable/close.rs:21`), and the attested pid comes from `split_caller`, not the payload.

## Write permission

Write permission is decided once, at open. `store.open` computes
`writable = write && (self.files[file_idx].mode & 0o200 != 0)` (`store/fdtable/open.rs:39`), so a descriptor
is writable only if the caller asked for write intent and the file's owner-write bit is set. A `write`
through a non-writable fd is `AccessDenied` (`store/fdtable/write.rs:25`), and `O_TRUNC` on a file that is
...
(`src/server/handlers/path/normalize.rs:22`).
...
`is_read_only` (`src/server/handlers/path/is_read_only.rs:17`) treats `/capsules` and everything under it as
non-writable:
...
the ZeroState posture, mirroring the kernel [ramfs](../ramfs/README.md)'s zero-on-drop; the difference is
...
- Only `chmod` and `truncate` normalize the path and apply the `/capsules` read-only guard; the other
  ...
  non-canonical path fails to match rather than reaching a different entry (`store/fdtable/lookup.rs:21`).
