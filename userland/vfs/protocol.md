# The wire protocol and the operations
...
the [store](store.md) page. For the capsule's identity and mask, read the [README](README.md).

## The NOVF frame

A request is a framed message with a 20-byte header (`HDR_LEN`, `src/protocol/types.rs:50`) and a payload.
...
`decode_request` (`src/protocol/decode.rs:27`) rejects a buffer shorter than the header
...
(`src/protocol/encode.rs:21`) with the same header shape, a reserved `u16` written as zero (`encode.rs:28`),
...
Fifteen ops are defined (`src/protocol/types.rs:20`-`34`), each routed by `dispatch`
(`src/server/dispatch.rs:27`) to exactly one handler; an unrecognised op is answered `EINVAL`
...
to POSIX errnos by `map_store_err` (`src/server/handlers/util.rs:20`), so a caller sees familiar codes; the
...
(`open.rs:57`). The store (`store/fdtable/open.rs:23`) creates the file only if `O_CREATE` is set, else
...
(`close.rs:29`). The store `close` (`store/fdtable/close.rs:20`) frees the slot only through `slot_mut`,
...
store `read` (`store/fdtable/read.rs:22`) resolves the fd for its owner (`BadFd` otherwise), copies at most
...
(`store/fdtable/write.rs:20`) requires the fd's `writable` flag (`AccessDenied` otherwise, `write.rs:25`),
...
reported `size` is the count of immediate children, not a byte length (`store/fdtable/query.rs:26`-`37`).
...
skipping any single name over 255 bytes (`store/fdtable/query.rs:47`-`63`). `EINVAL` on an empty rest, an
...
the [store](store.md) page explains why.
...
checks (`mkdir.rs:33`-`38`). The store `mkdir` (`store/fdtable/mkdir.rs:23`) returns `Exists` (`EEXIST`) if
...
store `unlink` (`store/fdtable/unlink.rs:22`) returns `NotFound` if absent (`unlink.rs:23`), `NotEmpty`
...
(`store/fdtable/rename.rs:22`) returns `NotFound` if the source is absent (`rename.rs:23`), `Exists` if the
...
checks (`copy.rs:34`-`50`). The store `copy` (`store/fdtable/copy.rs:28`) returns `NotFound` if the source is
...
store `rmdir` (`store/fdtable/rmdir.rs:27`) returns `NotFound` if absent (`rmdir.rs:28`), `NotEmpty` if the
...
(`store/fdtable/truncate.rs:22`) returns `NotFound` if absent (`truncate.rs:23`), `IsDir` for a directory
...
u64 bytes_used, u32 max_files` (`store/fdtable/usage.rs:22`). The only error path is the attestation failure
...
under `/capsules` (`chmod.rs:43`-`46`). The store `chmod` (`store/fdtable/chmod.rs:21`) returns `NotFound` if
absent (`chmod.rs:22`) and otherwise sets `mode & 0o777` (`chmod.rs:23`).

## Error mapping

`map_store_err` (`src/server/handlers/util.rs:20`) maps the store's errors to the POSIX errnos in
...
for one (`userland/app_skeleton/src/clients/vfs/types.rs:17`-`33`): `OP_OPEN 1` through `OP_CHMOD 15`, the
