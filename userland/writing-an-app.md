...
spawns. The [terminal](terminal.md) is the reference implementation to read alongside this. The
framework is `userland/app_skeleton/`.

## The App trait

A GUI application implements one trait (`userland/app_skeleton/src/app/behavior.rs:21`):
...
`AppManifest` (`app/manifest.rs`) declares the window the app wants:
...
The runtime owns the [surface](../subsystems/graphics/surfaces.md) registration and presentation, the
[input](../subsystems/input/README.md) subscription and delivery, and the window chrome (it draws the
...
```
  ...
  3. sign     produce the capsule's NONOS-ID certificate and manifest (the signing pipeline)
...
Steps 1 and 2 are your code and the [build toolchain](../build/toolchain.md); a `no_std` capsule builds
`core` + `alloc` against the [nonos_std crate](nonos-std.md) or the raw [SDK](sdk.md), and an unmodified
std crate builds through the [std PAL](std-pal.md). Step 3 is the capsule signing pipeline (the
[build](../build/README.md) pages), which produces the certificate and manifest the trust chain checks.
Steps 4 and 5 are the kernel mirror and init: the [lifecycle](lifecycle.md) page covers the embed-and-spawn
side, and the [verified spawn](../security/capsules-and-trust.md) gate is what guarantees only a
