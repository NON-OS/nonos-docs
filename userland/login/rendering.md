# The overlay and its surface

Login draws one thing: a full-screen lock overlay, a solid color with a single decorative bar and no text.
This page covers the painter in `src/render/mod.rs`, the setup path in `src/setup/` that discovers peers and
...
[the unlock flow](unlock.md); for identity and the graphics capabilities login does and does not hold, see
...
`0xFF143A22` and draws the bar lower (`src/render/mod.rs:3`, `:4`, `:7`, `:12`). The bar is a single color
`0xFFEDCB68` (`src/render/mod.rs:5`). There is no font, no cursor, and no text anywhere in this file. This
...
(`src/render/mod.rs:35`, `:17`, `:25`, `:29`). `paint_bar` guards against a surface too small to hold the
bar, returning early if the width is under 32 or the height cannot fit the bar
(`src/render/mod.rs:18`). Both write through `pixel_mut`, which computes a byte address from the surface base,
the row stride, and 4 bytes per pixel (`src/render/mod.rs:46`, `:47`). The locked bar sits at y=0x20 and the
unlocked bar at y=0x38, which is the only visual difference a person sees between the two states
(`src/render/mod.rs:9`, `:14`).

## Setup: discover, size, paint, submit

The overlay is brought up once, in `setup::run`, before the server loop starts (`src/setup/run.rs:21`). The
sequence is:

1. Discover three peers by name through `mk_service_lookup`: the keyring, the desktop shell, and the
   compositor, in that order (`src/setup/run.rs:22`, `:23`, `:24`). Each lookup goes through `lookup_port`,
   which treats a negative return, a zero pid, or a zero port as a failure and returns the string
   `"service lookup failed"` (`src/setup/discover/lookup_port.rs:24`). The service names are literal:
   `keyring`, `desktop_shell`, and `compositor` (`src/setup/discover/constants.rs:17`, `:18`, `:19`).
...
3. Query the display dimensions through `nonos_display_dimensions`; a nonzero return or a zero width or
   height is `"display dimensions unavailable"` (`src/setup/run.rs:26`, `src/setup/display.rs:22`, `:23`).
4. Allocate a private backing surface sized to the screen. `backing::alloc` computes the stride as `width *
   ...
   region read-write (`src/setup/run.rs:27`, `src/setup/backing.rs:20`, `:21`, `:22`). The map flags are the
   local `MAP_PRIVATE_ANON = 0x22` and `PROT_READ_WRITE = 0x3` (`src/setup/constants.rs:17`, `:19`).
5. Build the `Context` and paint the locked overlay into the backing before anyone sees it
   (`src/setup/run.rs:28`, `:37`).
...
`mk_surface_share` to get the handle the compositor references (`src/setup/register.rs:28`, `:37`, `:41`). A
negative register id is `"surface register rejected"` and a non-positive share handle is `"surface share
rejected"` (`src/setup/register.rs:38`, `:42`).

With the handle in hand, `setup::run` calls the compositor's scene submit at `OVERLAY_Z = 1`, covering the
whole screen from (0,0) (`src/setup/run.rs:45`, `src/setup/constants.rs:18`). The submit is where login's
...
(`src/setup/run.rs:41`, `:57`, `:58`, `src/setup/cleanup_backing.rs:20`, `src/setup/cleanup_surface.rs:19`).
Because `setup::run` returns an `Err` on any of these, `wait_for_setup` retries the whole sequence in a
yield loop rather than proceeding with a broken overlay (`src/wait_for_setup.rs:20`, `:23`).

## The compositor client

The compositor client speaks magic `NCMP` (`0x4E43_4D50`), version 1, over a 20-byte header
(`src/clients/compositor/constants.rs:16`, `:17`, `:18`). It uses three ops: `OP_HEALTHCHECK 0x0001`,
`OP_SCENE_SUBMIT 0x0002`, and `OP_DAMAGE_COMMIT 0x0003` (`src/clients/compositor/constants.rs:19`, `:20`,
...
status word otherwise (`src/clients/compositor/status.rs:19`, `:22`).

- `healthcheck` sends an empty body and is the setup liveness probe
  (`src/clients/compositor/healthcheck.rs:24`).
- `push_scene_submit` sends a 32-byte body of surface handle, x, y, width, height, and z, and is how the
  overlay enters the scene at setup (`src/clients/compositor/push_scene_submit.rs:24`, `:42`,
  `src/clients/compositor/constants.rs:22`).
- `ping_damage` sends a 16-byte damage rectangle and is the "present this repaint" call. Login sends it
  ...
  (`src/clients/compositor/ping_damage.rs:24`, `src/clients/compositor/constants.rs:23`,
  `src/server/handlers/start_session.rs:40`, `src/server/handlers/end_session.rs:23`).
...
the compositor picks up the change (`src/server/handlers/start_session.rs:39`, `:40`,
`src/server/handlers/end_session.rs:22`, `:23`). The initial locked overlay is presented by the scene submit
at setup rather than a damage ping (`src/setup/run.rs:45`).
