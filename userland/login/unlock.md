...
(`src/clients/keyring.rs`), and the desktop-shell notify that hands the desktop off
(`src/clients/desktop_shell.rs`). For the wire and dispatch that deliver a request here, see
[the protocol page](protocol.md); for the overlay repaint that follows an unlock, see
[the rendering page](rendering.md).
...
and nothing else, and the `Context` holds no secret field to protect (`src/protocol/limits.rs:4`,
`src/state/context/types.rs:16`). The thing that decides whether an unlock succeeds is the
[keyring](../keyring/README.md), which owns the key material and applies its own owner-pid check on
...
The session state is a two-variant enum on the `Context` (`src/state/context/types.rs:28`). `Locked` carries
nothing; `Unlocked` carries the `owner_pid` that opened the session, the `key_id` it opened, and a per-session
`serial` (`src/state/context/types.rs:28`, `:30`). The `Context` also holds a monotonic `serial` counter that
survives across sessions and starts at 0 (`src/state/context/types.rs:24`, `src/state/context/new.rs:36`). At
construction the machine is `Locked` (`src/state/context/new.rs:37`).
...
- `start_session(owner_pid, key_id)` refuses a second concurrent unlock with `E_BUSY`, then bumps the
  ...
  the serial (`src/state/context/start_session.rs:22`, `:25`, `:27`, `:28`).
...
(`src/clients/keyring.rs:6`, `:12`). The header is the request id, the op, and a reserved `u16`; the body is
the caller pid and the key id (`src/clients/keyring.rs:13`, `:14`, `:15`, `:16`, `:17`). Two ops are used:
`OP_UNLOCK` is `5` and `OP_LOCK` is `4` (`src/clients/keyring.rs:8`, `:9`).
These are the same op numbers the [keyring](../keyring/README.md) documents on its side.
...
word, and a nonzero status is passed back as an error (`src/clients/keyring.rs:21`, `:24`, `:28`, `:32`).
...
(`src/clients/desktop_shell.rs:6`, `:7`, `:9`, `:11`, `:16`, `:17`, `:18`). The message is the fixed text
`login:session_unlocked` (`src/server/handlers/start_session.rs:7`, `:31`). That notify is the handoff that
tells the shell to bring the desktop up. `END_SESSION` sends the mirror message `login:session_locked`
(`src/server/handlers/end_session.rs:7`, `:18`). The client returns the shell's status and maps a short
reply to `-11` (`src/clients/desktop_shell.rs:32`, `:35`).

## START_SESSION in order

The `start_session` handler runs these steps in order, and it is transactional
(`src/server/handlers/start_session.rs`):
...
so a session is never left open with a dead overlay (`src/server/handlers/start_session.rs:34`, `:41`). Each
outbound call carries a request id of `req.request_id ^ serial`, which a peer can correlate back to the
session by xor (`src/server/handlers/start_session.rs:31`, `:35`, `:40`).
...
(`src/state/context/end_session.rs:24`). Reading the key id before ending the session is deliberate: once
the state is `Locked`, `current_key_id` would return `None`, so the handler captures it first and relocks
exactly that key (`src/server/handlers/end_session.rs:10`, `:16`).
...
(`src/state/context/start_session.rs:27`, `src/clients/keyring.rs:27`). It hands no key material to login.
