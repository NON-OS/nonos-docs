...
fill before it serves anything (`src/main.rs:40`, `src/setup/sequence.rs:26`). Once up, it answers two IPC
...
(`README.md:6`). Its relationship to [capsule_entropy](../entropy/README.md) is a layering, not a
...
that client (`src/hardware/virtio_rng_capsule/capability.rs:18`). So the two are complementary entropy
...
cannot be established (`src/main.rs:65`, `README.md:68`).
...
`KERNEL_REPLY_ENDPOINT` the capsule targets (`src/protocol/endpoint.rs:21`, `src/server/error.rs:24`).
That is slot 6 in the per-service reply-inbox numbering that runs ramfs=1, keyring=2, entropy=3, crypto=4,
vfs=5, virtio_rng=6, and the kernel mirror names the same slot (`src/protocol/endpoint.rs:19`,
`src/hardware/virtio_rng_capsule/client/transport.rs:25`). The reply port itself is 4201 (`spawn.rs:34`).
...
|---|---|---|
| [operations.md](operations.md) | `src/protocol/` + `src/server/` | The `NORD` wire frame, the two ops (`fill_random`, `healthcheck`), the handlers, the error codes, and the kernel-side client that calls them. |
| [hardware.md](hardware.md) | `src/discover/` + `src/setup/` + `src/init.rs` + `src/regs/` | Device discovery, the ordered broker bring-up chain (claim, registers, IRQ, DMA), the virtio init handshake, and the MMIO-or-PIO register accessor. |
| [queue.md](queue.md) | `src/queue/` + `src/fill.rs` + `src/constants/queue.rs` | The split virtqueue, how a fill posts a descriptor and pulls bytes, the completion wait, and the DMA buffer bound. |
| [contributing.md](contributing.md) | the whole tree | Where to work, how to add an op, the kernel mirror, and the build, sign, and code standards. |
| [debugging.md](debugging.md) | runtime | The boot marker, the early-exit codes, the broker-phase stalls, and how to read the error codes. |

## Lifecycle

The capsule is spawned at boot through [verified spawn](../../security/capsules-and-trust.md): its
signature and attestation are checked, its requested capabilities are held against its manifest ceiling,
and only then is its ELF mapped (`src/hardware/virtio_rng_capsule/spawn.rs:37`). Inside the capsule,
`_start` initializes the heap, loops on `setup::run` until the whole broker chain succeeds, does one
sanity fill and rejects an all-zero result, and only then enters the server loop (`src/main.rs:35`). A
successful spawn prints `[DRIVER-VIRTIO-RNG] capsule spawned` on the boot log; the [debugging](debugging.md)
