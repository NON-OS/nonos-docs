...
[hardware bring-up](hardware.md) and the [request queue](queue.md) pages; for the identity and mask see the
[overview](README.md).
...
(`src/protocol/header.rs:30`, `header.rs:31`, `header.rs:33`). The `Request` the decoder produces carries
just `op`, `flags`, `request_id`, and `payload_len`; the payload bytes after the header are not parsed in
the decoder, so each handler decides what they mean (`src/protocol/header.rs:37`,
`src/protocol/decode.rs:18`).
...
`EINVAL` rather than act on a stale protocol (`src/protocol/decode.rs:26`, `decode.rs:31`, `decode.rs:35`).
It never panics and never unwraps: every field read is a checked `try_into` that funnels into the `None`
path (`src/protocol/decode.rs:30`).
...
The decoder validates the envelope, then `src/server/runner.rs:51` routes on the op; an unknown op returns
`EINVAL` (`src/server/runner.rs:54`). Two operations are defined (`src/protocol/ops.rs:21`):
...
(`src/protocol/encode.rs:34`, `src/server/error.rs:28`).
...
(`src/protocol/limits.rs:17`). A zero or over-ceiling request is refused with `EMSGSIZE` before the device
is touched (`src/server/handlers/fill.rs:33`). The served count is the smaller of what the caller asked for
and what the device wrote into the used ring, so a short device write returns fewer bytes rather than
padding (`src/server/handlers/fill.rs:44`). The bytes are copied out of the capsule's own DMA grant into
the response buffer under a single-threaded server loop, so no concurrent device write is in flight while
the copy runs (`src/server/handlers/fill.rs:48`, `fill.rs:51`).
...
same errno-to-error mapper it uses for the other capsules (`src/protocol/errno.rs:18`):
...
`run` (`src/server/runner.rs:36`) holds one request in flight at a time. The receive buffer is sized to
...
possible reply always fits without a reallocation (`src/server/runner.rs:33`, `runner.rs:34`). The loop
receives on inbox 0, skips a non-positive receive length, decodes, and dispatches; a decode failure is
answered `EINVAL` and the loop continues (`src/server/runner.rs:40`, `runner.rs:47`).
...
well-formed reply (`src/server/error.rs:27`, `error.rs:33`). The fill handler emits its success reply
inline with the same header and status encoders, then appends the entropy bytes and sends
(`src/server/handlers/fill.rs:46`, `fill.rs:54`).
...
`endpoint.4294967302` under a transport lock (`src/hardware/virtio_rng_capsule/client/transport.rs:27`,
`transport.rs:41`).

- `fill_random` (`src/hardware/virtio_rng_capsule/client/fill_random.rs:27`) gates on `CAP_DRIVER` through
...
(`src/hardware/virtio_rng_capsule/capability.rs:25`, `capability.rs:30`). The wire format has a bit-for-bit
