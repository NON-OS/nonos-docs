# Debugging capsule_driver_virtio_rng
...
driver does and how it is put together, read the [overview](README.md), the [operations](operations.md), the
[hardware bring-up](hardware.md), and the [request queue](queue.md) pages in this folder.
...
entry (`src/userspace/init/spawn_plan/drivers_virtio_io.rs:26`), and the `capsule spawned` suffix is emitted
by the shared capsule boot path on the `Ok` arm through `boot_log::ok(prefix, "capsule spawned")`
(`src/userspace/init/capsule_boot/run.rs:29`). If that line is absent the capsule never spawned, and the
`Err` arm logged an error line through `boot_log::error` instead (`src/userspace/init/capsule_boot/run.rs:32`),
...
driver was never in the image (`src/userspace/init/spawn_plan/drivers_virtio_io.rs:22`, `drivers_virtio_io.rs:32`).
...
bring-up cannot be completed (`src/main.rs:35`). The codes are worth reading directly:
...
(`src/main.rs:51`, `main.rs:60`, `main.rs:65`). In both cases the capsule releases every grant before exiting,
so a failed bring-up leaves the broker with a clean slate rather than a half-claimed device
(`src/main.rs:61`, `main.rs:67`). There is no software fallback that fabricates entropy; a device that cannot
be brought up means no service, by design (see the [overview](README.md)).

Setup itself does not exit. `_start` loops on `setup::run`, and on any `Err` it yields 64 times and retries
rather than giving up (`src/main.rs:40`, `main.rs:44`). So a capsule that spawned but whose device is absent
...
order and each phase returns a distinct error string (`src/setup/sequence.rs:26`):
...
section of [hardware bring-up](hardware.md)). A claim refusal means another capsule holds the device. The
handshake failures come from the device rejecting `FEATURES_OK` or advertising a zero-size requestq, at which
point the capsule writes the `FAILED` status bit and refuses to drive it (`src/init.rs:44`, `init.rs:51`).

Because every failing phase rolls back every earlier grant in reverse order before it returns, a stall never
leaves the broker holding a partial setup, and the next retry starts from a clean claim (`src/setup/dma.rs:57`,
`src/setup/registers/grant_mmio.rs`).
...
signal it returns `Err("virtio-rng: device did not respond")` (`src/fill.rs:23`, `fill.rs:43`). The fill
handler turns that into the wire status `E_IO` (`src/server/handlers/fill.rs:40`). So a wedged device shows up
as an `E_IO` reply, not a caller left waiting forever (see the wait detail in [request queue](queue.md)).

A caller that asks for zero bytes or more than the 4096 ceiling is refused before the device is touched, with
`E_MSGSIZE` (`src/server/handlers/fill.rs:33`).
...
for the other capsules (`src/protocol/errno.rs:18`):
...
an op the runner does not recognise (`src/protocol/decode.rs`, `src/server/runner.rs:54`). A malformed
envelope is still answered, through a synthetic zero-valued request, so a bad frame never leaves the caller
waiting on a reply that never comes (`src/server/error.rs:33`, `src/server/runner.rs:47`). `E_IO` is a device
...
before a real fill (see [operations](operations.md)).
