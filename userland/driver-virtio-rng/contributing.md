# Contributing to capsule_driver_virtio_rng
...
[overview](README.md), the [operations and protocol](operations.md), the [hardware bring-up](hardware.md),
and the [request queue](queue.md) pages in this folder.
...
for policy or a pool is out of scope by design (see the mask in the [overview](README.md)). Second, the wire
...
fill and rejects an all-zero result, then enters the server loop (`src/main.rs:35`). The nine top-level
...
| `src/init.rs` | the virtio legacy init handshake (ACK, DRIVER, features, queue PFN, DRIVER_OK) | you change the device negotiation |
...
| `src/fill.rs` | one virtqueue round trip: notify, bounded wait, IRQ ack, byte count | you change the completion wait |
...
1. Define the opcode. Add the `OP_*` constant next to `OP_FILL_RANDOM = 1` and `OP_HEALTHCHECK = 2`
   (`src/protocol/ops.rs:21`). Pick the next free number; the decoder does not care about the value, only the
   runner's match does.
...
2. Write the handler as one file per op under `src/server/handlers/`, next to `fill.rs` and `health.rs`. A
   ...
   status-only reply (`src/server/handlers/health.rs:25`, `src/server/error.rs:27`). Do not open a new reply
   path; every reply, success or error, goes to `KERNEL_REPLY_ENDPOINT` through those calls so a malformed
   envelope and a real reply are indistinguishable in shape (`src/server/error.rs:27`, `error.rs:33`).

3. Route it. Add a match arm in the server loop next to the two existing ops; an unknown op stays on the
   `_ =>` arm and returns `E_INVAL` (`src/server/runner.rs:51`, `runner.rs:54`). Declare the new handler module
...
- No panics in capsule code. No `unwrap`, `expect`, or `panic!`. The decoder returns `None` on any bad
  envelope rather than unwrapping (`src/protocol/decode.rs`), the fill path returns `Result` on a device that
  does not respond (`src/fill.rs:43`), and setup returns `Result` and rolls back rather than aborting inside a
  grant (`src/setup/dma.rs`).
- `unsafe` is confined to the volatile register and ring accesses and to the DMA buffer read, and each block
  carries a `SAFETY`/`# Safety` note tying it to the grant it operates on (`src/queue/post.rs:39`,
  `src/queue/used.rs:51`). New device memory touches follow the same pattern.
- Every grant a phase takes is released in reverse order on failure, so the broker never holds a partial
  setup (`src/setup/dma.rs:57`), and the live `Driver::release` drops every grant in reverse on teardown
