...
[fields.md](fields.md); the write gate that guards `set` is on [gate.md](gate.md).

## The header

Every request and every reply begins with a fixed 12-byte header, little-endian
(`userland/policy_proto/src/hdr.rs:17`):
...
`KIND_STR 4` (`userland/policy_proto/src/kind.rs:17`). The whole frame is bounded by `IPC_PAYLOAD_MAX = 512`
and a reply payload by `IPC_PAYLOAD_MAX - HDR_LEN` (`userland/policy_proto/src/limits.rs:18`,
`src/server/respond/ok.rs:22`).

## The two operations

The protocol has exactly two operations. There is no separate push or subscribe opcode on this service; the
"push" side is outbound and belongs to the capsule, described on [gate.md](gate.md). Readers do not
subscribe, they poll a `get` when they need a value.

| Op | Opcode | Direction | Payload | Source |
|----|--------|-----------|---------|--------|
| `OP_GET` | 0x0001 | request | none; the reply carries the value | `ops.rs:17`, `src/server/handle_get.rs:21` |
| `OP_SET` | 0x0002 | request | the new value, per kind | `ops.rs:18`, `src/server/handle_set.rs:40` |
...
if the field is not one this kind's getter carries (`src/server/handlers/get_bool.rs:22`):

- bool: one payload byte, `1` or `0` (`src/server/handlers/get_bool.rs:24`, store `src/store/get_bool.rs:21`).
- u8: one payload byte (`src/server/handlers/get_u8.rs:22`, store `src/store/get_u8.rs:21`).
- i8: one payload byte, the value reinterpreted (`src/server/handlers/get_i8.rs:24`, store
  `src/store/get_i8.rs:21`).
- str: up to `STRING_CAP = 64` payload bytes with no trailing NUL (`src/server/handlers/get_str.rs:22`,
  store `src/store/get_str.rs:22`).

### set

`handle_set::dispatch` first applies the write gate (see [gate.md](gate.md)), then routes by kind to the
matching setter (`src/server/handle_set.rs:40`). Each setter length-checks the payload, mutates the store,
...
- u8: exactly one byte or `E_BAD_LEN`; the value is range-checked in the store (`E_INVAL` if over the field
  max) before the mutation (`src/server/handlers/set_u8.rs:22`, `src/store/set_u8.rs:22`).
- i8: exactly one byte or `E_BAD_LEN`; the store enforces `-12..=14` for `Timezone` (`E_INVAL` otherwise);
  on success it mirrors through `push::on_i8_set` (`src/server/handlers/set_i8.rs:23`,
...
- str: at most `STR_MAX = 63` bytes or `E_BAD_LEN`; the store further caps at `STRING_CAP = 64` and
  ...
  `push::on_string_set` (`src/server/handlers/set_str.rs:24`, `src/store/set_str.rs:23`,
...
(`src/server/recv.rs:21`). The reply goes straight back to that sender through `mk_ipc_reply`
(`src/server/reply.rs:19`). A frame shorter than the 12-byte header, or one whose header fails to decode,
is silently skipped rather than answered; only a decodable header with a bad op, a bad field, or a body
that runs past the frame gets an error reply (`src/server/runner.rs:32`, `:36`, `:42`).
...
that would exceed the buffer and downgrades it to `E_BAD_LEN` (`src/server/respond/ok.rs:24`). Every failure
is a header-only reply carrying the op, field, kind, and a status word (`src/server/respond/err.rs:21`).

| Status | Value | When | Source |
|--------|-------|------|--------|
| `E_OK` | 0 | success reply | `errno.rs:17`, `src/server/respond/ok.rs:33` |
| `E_INVAL` | 22 | unknown op, unknown field discriminant, body longer than the frame, or a value the store rejects | `errno.rs:18`, `src/server/runner.rs:42`, `:48`, `:55` |
| `E_BAD_LEN` | 90 | payload length wrong for the kind, or a reply payload that would exceed the buffer | `errno.rs:19`, `src/server/handlers/set_bool.rs:25`, `src/server/respond/ok.rs:26` |
| `E_NOT_FOUND` | 91 | `get` for a field this kind's getter does not carry | `errno.rs:20`, `src/server/handlers/get_bool.rs:28` |
| `E_ACCES` | 93 | `set` from a caller that is not a trusted setter | `errno.rs:22`, `src/server/handle_set.rs:42` |
