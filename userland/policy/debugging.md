...
you are likely to chase. The mechanisms behind each are on [protocol.md](protocol.md),
[fields.md](fields.md), and [gate.md](gate.md).

## Did it start

On a successful boot the kernel prints `[POLICY] capsule spawned` (prefix `POLICY`, message
`capsule spawned`) from the boot log (`src/userspace/init/spawn_plan/core.rs:67`,
...
and readers poll a `get` when they need a value (see [protocol.md](protocol.md)). For the four mirrored
...
and its kernel handler (`src/push/on_bool_set.rs:22`,
`src/syscall/dispatch/router/admin/policy_push/entry.rs:25`). Any other kernel-security toggle is recorded
but not pushed by the current build, so expecting a kernel effect from, for example, `KernelSmep` is
expecting behavior that is not wired yet; that gap is documented on [gate.md](gate.md).

## A write is denied

A `set` from anything but `app.settings` or `app.setup_wizard` returns `E_ACCES`; that is the write gate
doing its job (`src/server/handle_set.rs:42`). If a legitimate writer is being denied, the suspect is
service resolution: `mk_service_lookup` must return that writer's live pid, so confirm the settings app or
wizard actually registered its service name and is running (`src/server/handle_set.rs:37`). The gate
...
- `E_INVAL` means an unknown op, an unknown field discriminant, a body that runs past the frame, or a value
  ...
  (`src/server/runner.rs:42`, `:48`, `:55`, `src/store/set_u8.rs:23`, `src/store/set_i8.rs:25`,
  `src/store/str_validate.rs:19`).
- `E_BAD_LEN` means the payload length did not match the kind: a bool or numeric that was not exactly one
  byte, or a string over 63 bytes (`src/server/handlers/set_bool.rs:25`, `src/server/handlers/set_str.rs:25`).
- `E_NOT_FOUND` means a `get` reached a getter that does not carry that field for its kind
  (`src/server/handlers/get_bool.rs:28`).
...
(`src/server/runner.rs:32`, `:36`). If a client sends and never gets any reply at all, this is the first
thing to rule out: the frame likely arrived under 12 bytes or with a header that did not parse.

## Source map

This page is drawn from `userland/capsule_policy/src/` (`main.rs`, `server/runner.rs`, `server/handle_set.rs`,
`server/handlers/`, `store/`, `push/`) and the kernel-side boot and mirror paths in
`src/userspace/init/spawn_plan/core.rs`, `src/userspace/init/capsule_boot/run.rs`, and
