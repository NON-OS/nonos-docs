...
`src/push/`, and service registration under `src/bootstrap/`. Read [protocol.md](protocol.md),
[fields.md](fields.md), and [gate.md](gate.md) before you change anything; the three describe the surfaces a
...
1. Add the discriminant to the `Field` enum, keeping it in its category range (`0x01xx` user, `0x02xx`
   ...
   (`userland/policy_proto/src/field.rs:19`, `userland/policy_proto/src/category.rs:27`), and add the
   matching arm to `decode_field` so the wire can address it
   (`userland/policy_proto/src/field_decode.rs:19`).
2. Declare its kind in `kind_of` (`userland/policy_proto/src/field_kind.rs:20`). If it is a bounded u8, add
...
3. Add the slot to the `Store` struct (`src/store/types.rs:26`) and its default to the const `store()`
   (`src/store/defaults/store.rs:21`). The default must be a `const` expression because the store is a
   `const fn`.
4. Wire it into the matching store getter and setter arm. For a bool that is `src/store/get_bool.rs:21` and
   `src/store/set_bool.rs:21`; for a u8, i8, or str the corresponding pair. A field that is not wired into
   its getter returns `E_NOT_FOUND` on `get`, and one not wired into its setter returns `E_INVAL` on `set`.
5. If the field must reach the kernel, add a kernel-side id and kind in `src/push/kernel_field.rs:17`, an
   arm to the matching `push::on_*_set`, and a line in `push::seed_kernel` so it is primed at boot
   (`src/push/on_bool_set.rs:22`, `src/push/seed.rs:22`). Then handle the id on the kernel side: add it to
   the `PolicyField` enum (`src/sys/policy/field_id.rs:19`) and to the `policy_push` router
   (`src/syscall/dispatch/router/admin/policy_push/entry.rs:25`). A field that is stored but not mirrored is
...
`app.setup_wizard`, and readable by anyone, with no extra work (`src/server/handle_set.rs:23`).
...
- No panics, `unwrap`, or `expect` in capsule code. Every handler returns an error status through
  `respond::err`, never a panic (`src/server/respond/err.rs:21`); every store setter returns a `bool` the
  handler turns into a status.
- Modular files, one unit per file, with `mod.rs` used only for re-exports (`src/store/mod.rs`,
  `src/server/mod.rs`, `src/push/mod.rs:24`).
...
kernel-side `src/sys/policy/field_id.rs` and `src/syscall/dispatch/router/admin/policy_push/`. Every
