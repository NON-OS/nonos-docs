# The write gate and the kernel mirror

This page mirrors `userland/capsule_policy/src/server/handle_set.rs` (the write gate) and
...
[fields.md](fields.md); the operations are on [protocol.md](protocol.md).
...
one of those services (`src/server/handle_set.rs:25`, `:37`). Every other caller's `set` returns `E_ACCES`
before the store is read or written (`src/server/handle_set.rs:42`). `get` has no such gate: any caller that
can reach the endpoint may read any field. The two writers are documented in
[settings](../settings/README.md) and [setup-wizard](../setup-wizard/README.md).
...
- A compromised writer (`app.settings` or `app.setup_wizard`) can `set` any field, including
  ...
  to `[A-Za-z0-9._-]` (`src/store/set_u8.rs:22`, `src/store/set_i8.rs:25`, `src/store/str_validate.rs:19`).
...
- `on_i8_set` forwards only `Timezone` (`src/push/on_i8_set.rs:22`).
- `on_string_set` forwards `Hostname` and `DomainName` (`src/push/on_string_set.rs:22`).

Each maps the wire `Field` to a small kernel-side id and kind and calls `raw::submit`, which is the
`mk_admin_policy_push` syscall (`src/push/kernel_field.rs:17`, `src/push/raw.rs:19`). The kernel-side ids
...
`KIND_STR 3` (`src/push/kernel_field.rs:17`).
...
kernel admits only for a caller that holds `Admin` (`src/push/raw.rs:19`,
`src/syscall/contract/cap_table/admin.rs:24`). This is why the capsule carries the `Admin` bit and nothing
else elevated; see the mask decomposition on the [hub](README.md). The kernel-side router revalidates the
id and kind before applying the value, mapping the id back through its own `PolicyField` enum and rejecting
an unknown id or kind with `E_INVAL` (`src/syscall/dispatch/router/admin/policy_push/entry.rs:25`,
`src/sys/policy/field_id.rs:19`). The kernel `PolicyField` ids match the capsule's:
`KernelPreempt 0x0001`, `TimezoneOffset 0x0002`, `Hostname 0x0003`, `DomainName 0x0004`
(`src/sys/policy/field_id.rs:20`).
...
that session (`src/main.rs:37`, `src/push/seed.rs:22`). It calls the same `on_*_set` paths the live writes
use, so the seed and a later write travel the identical route into the kernel (`src/push/seed.rs:23`).

## The honest gap

Of the twelve kernel-security fields, only `KernelPreempt` is actually pushed into the kernel today
(`src/push/on_bool_set.rs:22`). The other eleven, ASLR, StackGuard, NX, SMEP, SMAP, Debug, Serial,
...
This page is drawn from `userland/capsule_policy/src/server/handle_set.rs`,
`userland/capsule_policy/src/push/` (`on_bool_set.rs`, `on_i8_set.rs`, `on_string_set.rs`,
`kernel_field.rs`, `raw.rs`, `seed.rs`), and the kernel-side mirror in `src/sys/policy/field_id.rs`,
`src/syscall/dispatch/router/admin/policy_push/`, and `src/syscall/contract/cap_table/admin.rs`. Every
