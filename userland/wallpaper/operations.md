...
the capsule selects and paints an image on its own initiative, read the [pipeline](pipeline.md) page; for
the capability mask and identity, read the [README](README.md).

## The wire header

Requests arrive on the service inbox and are parsed against a 20-byte header carrying magic `NWLP`
(`0x4E574C50`) and version 1 (`src/protocol/header.rs:17`, `src/protocol/header.rs:19`). The header is
...
`payload_len` exactly matches the remaining bytes (`src/protocol/decode.rs:21`). A short frame is rejected
with `E_BAD_LEN`, a wrong magic with `E_BAD_MAGIC`, a wrong version with `E_BAD_VERSION`, and a body
length that does not match the header with `E_BAD_LEN` (`src/protocol/decode.rs:22`,
`src/protocol/decode.rs:38`). Even a rejected frame carries back the op, flags, and request id it could
read, so the error reply is correlatable (`src/protocol/decode.rs:28`).

## The five operations

Five ops dispatch (`src/protocol/ops.rs:17`, `src/server/runner/dispatch.rs:24`):
...
| `OP_SET_WALLPAPER` | 8-byte `argb, _pad`, or a decode-client image | set color or paint image, commit damage | `handlers/set_wallpaper.rs:43` |
...
8-byte body, and the `GET_WALLPAPER` reply body is 24 bytes (`src/protocol/limits.rs:21`,
`src/protocol/limits.rs:24`, `src/protocol/limits.rs:27`, `src/protocol/limits.rs:30`). The inbox and
reply buffers are each sized `HDR_LEN + IPC_PAYLOAD_MAX` with `IPC_PAYLOAD_MAX` at 256 bytes
(`src/protocol/limits.rs:17`, `src/server/runner/entry.rs:28`).

### HEALTHCHECK

`OP_HEALTHCHECK` with an empty body replies status 0 and nothing else (`handlers/health.rs:20`). The
empty-body condition is enforced in the dispatch: a `HEALTHCHECK` op carrying a body falls through to the
unknown arms below (`src/server/runner/dispatch.rs:26`).
...
surface (`handlers/set_wallpaper.rs:26`, `handlers/set_wallpaper.rs:39`, `handlers/set_wallpaper.rs:44`).
...
(`handlers/set_wallpaper.rs:48`, `src/state/context.rs:43`, `src/paint/fill.rs:20`). Any other body length
is handed to the decode client, which parses an inline image header and paints the decoded pixels; a
malformed inline image returns `E_INVAL` (`handlers/set_wallpaper.rs:56`). Either way the handler issues a
fresh request id, commits damage over the whole surface to the compositor, and replies status 0
(`handlers/set_wallpaper.rs:60`). The inline decode path is described on the [pipeline](pipeline.md) page.
...
padding bytes (`handlers/get_wallpaper.rs:22`). It is read-only and open to any caller with IPC rights.
Note that the policy it reports is the stored fit style, which the paint paths do not yet consult; see the
[pipeline](pipeline.md) page for that gap.
...
(`handlers/set_policy.rs:22`, `handlers/set_policy.rs:30`, `handlers/set_policy.rs:34`). `Policy` has
`Fill` (0), `Fit` (1), `Stretch` (2), `Center` (3), and `Tile` (4) (`src/state/policy.rs:19`). Recording
is all it does: the value is stored on the context and reported back through `GET_WALLPAPER`, but neither
paint path reads it. That gap lives on the [pipeline](pipeline.md) page.

### FADE and the pacer

`OP_FADE` takes an 8-byte body of `target_alpha` and `duration_ms`. The handler rejects a wrong length, an
unreadable field, or a target alpha above 255 with `E_INVAL` (`handlers/fade.rs:26`, `handlers/fade.rs:38`).
Otherwise it samples the vsync clock as the start time and arms a linear alpha ramp from the current alpha
to the target over the requested duration (`handlers/fade.rs:42`, `src/state/fade.rs:35`). A zero-duration
fade is defined to snap: the timeline is left inactive, the handler sets the alpha directly, fills, commits
damage, and returns (`src/state/fade.rs:36`, `handlers/fade.rs:46`).
...
the new alpha and commit damage (`src/server/tick.rs:25`, `src/server/tick.rs:33`, `src/state/fade.rs:53`).
...
back to waiting on vsync (`src/server/tick.rs:42`, `src/state/fade.rs:57`, `src/server/runner/entry.rs:33`).
...
(`src/server/runner/drain.rs:27`, `src/server/runner/drain.rs:40`). Dispatch matches the op; a known op
runs its handler, an unknown op with an empty body replies `E_BAD_OP`, and an unknown op that carries a
body replies `E_INVAL` (`src/server/runner/dispatch.rs:33`, `src/server/runner/dispatch.rs:36`).
...
length (`src/server/respond.rs:21`, `src/server/respond.rs:27`, `src/protocol/encode.rs:19`). The full
errno set is (`src/protocol/errno.rs:17`):
