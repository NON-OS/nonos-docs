...
the [policy store](../policy/README.md), streams that image out of the
[wallpaper catalog](../wallpaper-catalog/README.md) in bounded chunks, decodes and paints it into its
...
(`src/userspace/capsule_wallpaper/spawn.rs:50`). There is no `Network` bit (`0x0004`, `types.rs:58`), no
...
|------|---------|----------------|
| [operations.md](operations.md) | `src/protocol/`, `src/server/` | The `NWLP` control protocol: the header and parser, the five ops (`HEALTHCHECK`, `SET_WALLPAPER`, `GET_WALLPAPER`, `SET_POLICY`, `FADE`), the dispatch, the reply framing, the write gate on `SET_WALLPAPER`, and the fade pacer. |
| [pipeline.md](pipeline.md) | `src/subscriber/`, `src/policy_client/`, `src/catalog_client/`, `src/paint/`, `src/decode_client/`, `src/compositor_client/` | Selection from the policy store, the 300-tick re-poll, the chunked catalog fetch, the in-process decode, the always-stretch paint, and the drive to the compositor. |
| [contributing.md](contributing.md) | the whole tree | Where to work, how to change scaling or fetching, the build and sign steps, and the code standards. |
| [debugging.md](debugging.md) | runtime | The boot marker, the failure modes (blank background, wrong wallpaper, no background layer, `E_ACCES`), and where to look for each. |
...
2. Setup discovers the compositor by name, retrying up to 256 times and giving up with
   ...
   `stride * height` (`src/setup/prime/run.rs:31`, `src/setup/prime/backing.rs:34`).
3. It fills the buffer with the default color `0xFF0080FF`, decodes and paints the embedded boot JPEG,
   ...
   (`src/setup/prime/run.rs:33`, `src/setup/prime/run.rs:51`, `src/setup/prime/register.rs:41`). The
...
4. Setup applies the policy-selected wallpaper synchronously if the policy port resolved, marking it
   applied so the subscriber will not repeat the work (`src/setup/prime/run.rs:62`).
...
A successful spawn prints `[WALLPAPER] capsule spawned` on the boot log; the [debugging](debugging.md)
