# The selection and paint pipeline
...
The wallpaper capsule does not decide which image to show; the [policy store](../policy/README.md) does.
...
trusting the byte (`src/policy_client/get_wallpaper.rs:22`, `src/policy_client/get_wallpaper.rs:44`). The
...
- At setup, once the surface exists, `run` reads the policy field and applies it immediately, before the
  ...
  core a few seconds into boot (`src/setup/prime/run.rs:62`). On success the index is recorded in
...
- During the session, the subscriber polls the policy store every 300 pacer ticks
  (`src/subscriber/tick.rs:23`, `src/subscriber/tick.rs:27`). It lazily resolves the policy and catalog
  ports if either is still unset, then reads the `wallpaper` field; if the wanted index differs from
  `applied_wallpaper` it applies it and records the new index (`src/subscriber/tick.rs:30`,
  `src/subscriber/tick.rs:44`). A poll that matches the applied index is a no-op, so a changed wallpaper
  redraws and an unchanged one does not, and a missing policy port simply returns until a later poll
  resolves it (`src/subscriber/tick.rs:36`).
...
(`src/subscriber/apply.rs:22`, `src/subscriber/apply.rs:35`).

The catalog service `wallpaper_catalog` is resolved by name (`src/catalog_client/lookup.rs:23`) and spoken
with a 16-byte compact header of `op u16`, `status u16`, `index u32`, `offset u32`, `payload_len u32`
(`src/catalog_client/proto.rs:23`, `src/catalog_client/proto.rs:37`), each call on a 500 ms reply timeout.
...
`fetch_image` is the streaming client (`src/catalog_client/fetch_image.rs:26`). It calls `OP_GET_SIZE`
...
at least four payload bytes (`src/catalog_client/fetch_size.rs:39`); the chunk call validates the op,
index, and offset, rejects a zero or over-4096 payload length, and rejects a body that runs past the bytes
actually received (`src/catalog_client/fetch_chunk.rs:38`, `src/catalog_client/fetch_chunk.rs:44`,
`src/catalog_client/fetch_chunk.rs:48`). The catalog protocol is described in full on the
[wallpaper catalog](../wallpaper-catalog/README.md) page.
...
- `decode_jpeg` (`src/paint/decode_jpeg.rs:33`) is used for catalog images and the embedded boot image.
  ...
  (`src/paint/decode_jpeg.rs:34`, `src/paint/decode_jpeg.rs:41`, `src/paint/decode_jpeg.rs:42`,
  `src/paint/decode_jpeg.rs:46`).
- The `decode_client` (`src/decode_client/`) handles an image carried inline on a `SET_WALLPAPER` request.
  ...
  length equals the header plus the declared payload (`src/decode_client/header.rs:36`,
  `src/decode_client/header.rs:52`), maps the kind to PNG, BMP, raw LZ4, or JPEG
  (`src/decode_client/header.rs:55`), and decodes through the toolkit into a buffer sized to the backing
  surface (`src/decode_client/wire.rs:27`, `src/decode_client/seq.rs:24`). A decode error becomes `E_INVAL`
  back to the caller (`src/decode_client/seq.rs:26`).
...
path. `SET_POLICY` records the selected style and `GET_WALLPAPER` reports it (`src/state/policy.rs:19`,
`src/server/handlers/set_policy.rs:34`), yet both painters always nearest-neighbor stretch the decoded
...
- The catalog and embedded path goes through `paint_image`, which calls `blit_argb`
  (`src/paint/paint_image.rs:22`). `blit_argb` walks every destination pixel and maps it back to a source
  pixel by the ratio of destination to source dimensions, clamping at the edges: a pure nearest-neighbor
  stretch to the whole surface, with no branch on any policy (`src/paint/blit_argb.rs:31`,
  `src/paint/blit_argb.rs:37`).
- The inline path goes through `decode_and_paint`, whose `paint_stretch` does the same integer-ratio
  ...
  (`src/decode_client/seq.rs:39`, `src/decode_client/seq.rs:51`).
...
functions is the natural first task, and the [contributing](contributing.md) page points at exactly them.

A flat color path bypasses decoding entirely: `fill_argb` writes one ARGB across every pixel of the
backing surface (`src/paint/fill.rs:20`). The default at setup is `0xFF0080FF` (`src/setup/prime/run.rs:25`),
painted before the embedded `special-variant-6-1080p.jpg` is decoded over it (`src/setup/prime/run.rs:26`,
`src/setup/prime/run.rs:51`).

## Redraw on change

Every path that changes what should be on screen paints the backing buffer and then commits damage to the
compositor over the whole surface: the subscriber after a catalog swap (`src/subscriber/apply.rs:37`), the
`SET_WALLPAPER` handler after a color or inline image (`src/server/handlers/set_wallpaper.rs:61`), and the
fade pacer on each alpha step (`src/server/tick.rs:41`). The fade pacer itself lives on the
[operations](operations.md) page.
...
runs on the tighter 16 ms live timeout (`src/compositor_client/wire.rs:13`, `src/compositor_client/wire.rs:14`).
`display_info` rejects a reply whose width, height, or stride is zero or whose format is not
`ARGB8888` (`src/compositor_client/display_info.rs:40`), and the backing surface is registered at Z 0 with
`scene_submit`, rolling back the shared handle if the submit is rejected (`src/setup/prime/register.rs:41`,
`src/setup/prime/register.rs:49`). Each reply is validated to echo the magic, version, op, request id, and
payload length before its status is read (`src/compositor_client/wire/reply.rs:40`,
`src/compositor_client/wire/reply.rs:55`).
