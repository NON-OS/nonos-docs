# Debugging capsule_driver_rtl8139
...
[operations](operations.md) page, the [bring-up](bring-up.md) page, and the [buffers](buffers.md) page.
...
(`src/userspace/init/spawn_plan/drivers_nic.rs:40`), whose `Ok` arm calls `boot_log::ok(prefix, "capsule
...
(`src/userspace/init/capsule_boot/run.rs:32`). The RTL8139 spawn is feature-gated on
`nonos-capsule-driver-rtl8139`, so a build without that feature spawns nothing at all
(`src/userspace/init/spawn_plan/drivers_nic.rs:46`).

## Bring-up exit codes

If the capsule spawns but heap init, `setup::run`, or `init::bring_up` fails, the process exits with a distinct
code, which is the fastest way to tell how far bring-up got (`src/main.rs:36`).

| Exit | Stage | Meaning and where it comes from |
|---|---|---|
| 1 | heap | `heap_init` failed before any device work (`src/main.rs:36`). |
| 2 | setup | A grant step failed: no device matched, or the claim, bus-master write, PIO grant, INTx bind, or DMA map was refused, or a DMA region came back above 32 bits (`src/main.rs:40`, and the failure points in `src/setup/`). |
| 3 | init | The grants were taken but device programming failed: reset never cleared, the MAC was all-zero or all-ones, or a register write was refused; `main` releases every grant first (`src/main.rs:43`). |

Exit `2` is a family of grant failures. Discovery finding no RTL8139 (`src/setup/sequence.rs:25`,
`src/discover.rs:36`) looks the same at the exit-code level as a refused claim (`src/setup/claim.rs:19`), a
refused PIO grant (`src/setup/pio_grant.rs:24`), a refused INTx bind (`src/setup/irq.rs:24`), or a refused or
over-32-bit DMA map (`src/setup/dma.rs:53`). The specific `&'static str` returned names which
...
million polls (`"rtl8139 reset timeout"`, `src/init/reset.rs:28`), a MAC that read back all-zero or all-ones
and so is not a real address (`"rtl8139 invalid mac"`, `src/init/mac.rs:26`), or a `Pio` write that the broker
refused while programming `RBSTART`, `CAPR`, `RCR`, the `TXADDR` registers, `TCR`, `ISR`, `IMR`, or the
command register (`src/init/run.rs:29`).
...
idle case, not an error (`src/rx/recv_one.rs:35`, `src/server/handlers/rx_packet.rs:31`). A stack that polls
...
RX-FIFO-overflow bit was set (`src/rx/recv_one.rs:31`), or a ring descriptor failed validation, meaning its
receive-OK status bit was clear, its raw length was four or fewer bytes, or the frame length exceeded the
Ethernet maximum or the reply buffer (`src/rx/read_frame.rs:37`, `read_frame.rs:42`). An overflow points at a
...
(`src/server/handlers/tx_packet.rs:24`, `tx_packet.rs:28`). These are request-validation errors; no byte
...
underrun, or never completed within the poll budget (`src/tx/poll_done.rs:28`, `poll_done.rs:36`,
`src/server/handlers/tx_packet.rs:37`). An underrun points at DMA starvation, an abort at excessive
...
`OP_LINK_STATUS` returns a `0` byte when the `MSR` link-bad bit is set (`src/server/handlers/link_status.rs:29`).
...
a clear-on-read register, so it is safe to poll while diagnosing (`src/server/handlers/stats.rs:47`).

## The IOMMU caveat

On the current target there is no IOMMU, so bus mastering is enabled but nothing in hardware confines the
NIC's DMA to the ring and slots the driver programmed (`src/setup/pci.rs:23`). The driver mitigates in
