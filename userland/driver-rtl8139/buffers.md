# The receive ring and transmit slots
...
completion poll), together with the sizes in `src/constants/dma.rs` and the register bits in
`src/constants/regs.rs`. How the buffers are first allocated and programmed is on the [bring-up](bring-up.md)
page; the client ops that call into them are on the [operations](operations.md) page.

## The DMA layout

The two DMA regions are sized once (`src/constants/dma.rs:17`):
...
the controller run a frame past the 32 KiB boundary into that slack (`RCR_WRAP`, `src/constants/regs.rs:48`,
set at `src/init/rx_setup.rs:30`). The transmit region is four equal 2048-byte slots, each larger than the
...
3. It reads the command register, and if the `CMD_RX_BUF_EMPTY` bit is set the ring holds no complete frame,
   ...
   (`src/rx/recv_one.rs:35`).
4. Otherwise it hands off to `read_frame` (`src/rx/recv_one.rs:39`).

`read_frame` reads the descriptor at the current `rx_offset` out of the ring, validates it, copies the frame,
and advances (`src/rx/read_frame.rs:28`). It reads the 16-bit status and 16-bit raw length with `ring_u16`,
and rejects a descriptor whose receive-OK status bit is clear or whose raw length is four or fewer bytes,
acking the IRQ and returning an error (`src/rx/read_frame.rs:35`). The real frame length is the raw length
minus the 4-byte CRC the NIC includes, and a frame larger than the Ethernet maximum or the caller's buffer is
refused before any copy (`src/rx/read_frame.rs:41`). A valid frame is copied out with `copy_ring`, then
`advance` moves the cursor and the IRQ is acked (`src/rx/read_frame.rs:46`).
...
runs off the logical end folds back to the start (`src/rx/ring_u8.rs:19`), and `ring_u16` composes two such
reads little-endian (`src/rx/ring_u16.rs:19`). `advance` computes the next offset as the current offset plus
the raw length plus the 4-byte header, rounded up to a 4-byte boundary and taken modulo the data size, then
programs `CAPR` as that offset minus 16, again modulo the data size (`src/rx/advance.rs:22`). The minus-16 is
the RTL8139's quirk: `CAPR` trails the true read position by 16 bytes, and `rx_setup` seeds the same
expression at bring-up so the first read starts clean (`src/init/rx_setup.rs:34`).
...
(`src/init/tx_setup.rs:21`). The driver keeps a software cursor, `tx_cur`, naming the next slot to use.

Sending one frame is `send` (`src/tx/send.rs:24`):

1. It copies the frame bytes into the current slot's user virtual address, `tx_user_va + tx_cur *
   TX_SLOT_BYTES`, with a non-overlapping copy (`src/tx/send.rs:25`).
...
error if neither lands within the budget (`src/tx/poll_done.rs:24`). The status bits are `TX_STATUS_OK`
(bit 15), `TX_STATUS_UNDERRUN` (bit 14), and `TX_STATUS_ABORT` (bit 30) (`src/constants/regs.rs:52`).
...
the copy began (`src/rx/copy_ring.rs:19`, `src/rx/read_frame.rs:42`). On transmit, `send` copies exactly
`frame.len()` bytes into the slot, and the TX handler already bounded that length to the 60..1514 Ethernet
range, which is smaller than the 2048-byte slot, so the copy cannot overrun (`src/tx/send.rs:28`,
`src/server/handlers/tx_packet.rs:28`). The NIC's own DMA stays inside the ring and the slots because those
...
(`src/rx/recv_one.rs:32`, `src/rx/read_frame.rs:48`). Correctness comes from the `CMD_RX_BUF_EMPTY` check and
