# Client operations and the NR89 protocol
...
[net_core](../../subsystems/networking/stack.md) stack binds to: `link_status`, `mac_address`, `tx_packet`,
and `rx_packet` are the four operations a NIC device bridge needs. For the identity table and the capability
mask see the [README](README.md); for how the handlers reach the hardware see the [buffers](buffers.md) page.

## The wire format

A request header is 20 bytes and begins with a magic and a version (`src/protocol/header.rs:17`). The decoder
rejects anything shorter than the header, a wrong magic, or a wrong version, returning `None` so the caller
answers `E_INVAL` (`src/protocol/decode.rs:19`, `src/server/runner.rs:53`).
...
Bytes 10 and 11 are reserved and are written as zero in the response header (`src/protocol/encode.rs:24`).
Every reply is a response header of the same length (`RESP_HDR_LEN = HDR_LEN = 20`), followed by a 4-byte
little-endian status word, then any payload (`src/protocol/header.rs:19`, `src/protocol/limits.rs:19`).
Status `0` means success; a negative status is one of the errno constants below. Replies go to the kernel
reply endpoint `0x1_0000_000D` with `mk_ipc_send` (`src/server/error.rs:26`, `src/protocol/endpoint.rs:17`).
...
holds the largest received frame or the stats block (`src/server/runner.rs:33`). The loop receives a request
on the service inbox, decodes it, and dispatches (`src/server/runner.rs:39`, `runner.rs:45`). A receive of
zero or fewer bytes is skipped, and a decode failure answers `E_INVAL` without touching the device
(`src/server/runner.rs:46`, `runner.rs:53`). Each op routes to its handler, and an unrecognised opcode is
answered with `E_INVAL` (`src/server/runner.rs:58`, `runner.rs:65`).

## The six operations

The opcodes are defined in `src/protocol/ops.rs:17` and dispatched in `src/server/runner.rs:58`.
...
| `OP_LINK_STATUS` | `2` | none | 1-byte link-up flag | `server/handlers/link_status.rs:27` |
| `OP_MAC_ADDRESS` | `3` | none | 6-byte MAC address | `server/handlers/mac_address.rs:25` |
...
| `OP_STATS` | `6` | none | 48-byte register and cursor snapshot | `server/handlers/stats.rs:29` |

`OP_HEALTHCHECK` replies with the response header and a zero status word alone through `reply_with_status`;
it is pure liveness and reads no register (`src/server/handlers/health.rs:20`, `src/server/error.rs:23`).
...
- `OP_LINK_STATUS` reads the `MSR` port register live and reports `1` when the `MSR_LINK_BAD` bit is clear,
  `0` when it is set; a port read failure answers status `-5` (`src/server/handlers/link_status.rs:28`). The
  payload is one byte (`src/protocol/limits.rs:22`).
- `OP_MAC_ADDRESS` returns the six MAC bytes read once at bring-up and cached on the driver
  (`src/server/handlers/mac_address.rs:29`; read at `src/init/mac.rs:21`). The payload is six bytes
  (`src/protocol/limits.rs:21`).
- `OP_TX_PACKET` transmits the request body as one Ethernet frame. It requires the body length to equal the
  ...
  (`src/server/handlers/tx_packet.rs:24`, `tx_packet.rs:28`; bounds at `src/constants/frame.rs:18`). On a
  successful send the status is `0`; a NIC send error is `E_IO` (`src/server/handlers/tx_packet.rs:35`).
- `OP_RX_PACKET` polls the receive ring for one frame. On a frame it writes a 4-byte little-endian length
  ...
  interrupt error answers `E_IO` (`src/server/handlers/rx_packet.rs:29`, `rx_packet.rs:40`). The 4-byte
  prefix is `RX_PAYLOAD_PREFIX_LEN` (`src/protocol/limits.rs:23`).
- `OP_STATS` returns a non-mutating snapshot of twelve little-endian `u32` fields, 48 bytes total: `CMD`,
  ...
  software `tx_cur` cursor (`src/server/handlers/stats.rs:47`; length at `src/protocol/limits.rs:24`). It
  reads only registers that do not clear on access, so telemetry never perturbs the driver's own bookkeeping;
  a port read failure answers `E_IO` (`src/server/handlers/stats.rs:32`).
...
bound, so a client can neither under-run nor over-run the transmit slot (`src/server/handlers/tx_packet.rs:24`,
`tx_packet.rs:28`; the slot is 2048 bytes and 1514 is the ceiling, `src/constants/dma.rs:20`,
`src/constants/frame.rs:19`). The RX handler bounds every copy against the caller's buffer and the Ethernet
maximum inside the ring reader, so a malformed descriptor cannot spill past the reply buffer
(`src/rx/read_frame.rs:42`). There is no panic path: the crate is `panic = "abort"` and every handler returns
