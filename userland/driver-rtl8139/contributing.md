# Contributing to capsule_driver_rtl8139
...
standards a change has to meet. For what the driver does and how it fits together, read the [README](README.md),
the [operations](operations.md) page, the [bring-up](bring-up.md) page, and the [buffers](buffers.md) page.
...
`Driver` to `server::run`, which loops forever (`src/main.rs:35`). The top-level modules are declared there
(`src/main.rs:22`).
...
| `src/discover.rs`, `src/discover/` | the `mk_device_list` scan and the RTL8139 match | you change how the device is found |
...
| `src/pio.rs` | the checked 8/16/32-bit port accessor | you change how a register is read or written |
...
1. Add the opcode constant to `src/protocol/ops.rs:17`, re-export it from `src/protocol/mod.rs:34`, and, if it
   carries a fixed payload, add its length to `src/protocol/limits.rs:17`.
2. Write the handler as one file under `src/server/handlers/`, exposing a `handle` function that encodes the
   ...
   can delegate to `reply_with_status` (`src/server/error.rs:23`). Declare the module in
   `src/server/handlers/mod.rs:17`.
3. Wire it into the dispatch match in `src/server/runner.rs:58`.

An op that carries a client body follows `tx_packet.rs`: check the body length against the header's
`payload_len` and against the size bound before touching the device (`src/server/handlers/tx_packet.rs:24`).

## Keep it PIO-only

The defining architectural rule of this capsule is that it is port-mapped, not memory-mapped. It holds `Pio`
and not `Mmio`, and every register touch goes through the `Pio` accessor (`src/pio.rs:24`), never inline `in`
...
[PIO path](../../subsystems/hardware-broker/pio.md) is the only way in.
...
`nonos-capsule-driver-rtl8139` feature is set (`src/hardware/rtl8139_capsule/embed.rs:17`).
...
- Every grant taken in setup must have reverse-order rollback, which is what the inline failure arms and
  `rollback::after_irq` provide during setup (`src/setup/rollback.rs:21`) and what `Driver::release` provides
  after the driver is built (`src/setup/driver.rs:39`).
- The AGPL header sits at the top of every source file, byte for byte the same as the header on `src/main.rs:1`
