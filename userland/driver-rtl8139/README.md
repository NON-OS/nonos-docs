# The RTL8139 Driver Capsule

`capsule_driver_rtl8139` is the NONOS Realtek RTL8139 Fast Ethernet driver: a signed ring-3 capsule that
...
for `Mmio`. The [drivers](../../subsystems/networking/drivers.md) page places it beside the other NIC
capsules, and the [networking](../../subsystems/networking/README.md) README shows where the frame path goes
next.
...
| Service handle | `driver.rtl8139_0` | `Capsule.mk:7`, `src/hardware/rtl8139_capsule/spawn.rs:32` |
...
| Kernel mirror | `src/hardware/rtl8139_capsule` | `src/hardware/rtl8139_capsule/spawn.rs` |

The reply endpoint number `4294967309` is `0x1_0000_000D`, the kernel reply-inbox constant the capsule sends
every reply to (`src/protocol/endpoint.rs:17`, `src/server/error.rs:26`). The kernel side names the same
inbox `endpoint.4294967309` (`src/hardware/rtl8139_capsule/client/transport.rs:25`).
...
(`src/hardware/rtl8139_capsule/spawn.rs:51`). Because the requested set is a subset of the manifest ceiling,
...
[PIO](../../subsystems/hardware-broker/pio.md) and [claim](../../subsystems/hardware-broker/claim.md) pages
...
|------|---------|----------------|
| [operations.md](operations.md) | `src/protocol/`, `src/server/` | The `NR89` wire format, the request loop, the six client ops (link, MAC, TX, RX, stats, health), per-op payloads, and the errno set. |
| [bring-up.md](bring-up.md) | `src/discover.rs`, `src/setup/`, `src/init/`, `src/pio.rs`, `src/constants/` | Discovery, the broker grants (claim, bus master, PIO, INTx, DMA), the checked port access, and the reset/MAC/enable init sequence with its rollback. |
| [buffers.md](buffers.md) | `src/rx/`, `src/tx/`, `src/constants/dma.rs`, `src/constants/regs.rs` | The circular receive buffer with its `CAPR`/wrap arithmetic, the four transmit descriptors, the DMA layout, and where the frame bytes cross the copy boundary. |
...
| [debugging.md](debugging.md) | runtime | The boot marker, the bring-up exit codes, and the runtime failure modes: no device, reset timeout, TX timeout, and RX-empty. |
...
server, which loops forever (`src/main.rs:35`). If heap init fails it exits `1`; if `setup::run` fails it
exits `2`; if `init::bring_up` fails it releases every grant and exits `3` (`src/main.rs:36`,
`src/main.rs:41`, `src/main.rs:43`). The kernel spawns it under the NIC bring-up plan through
...
(`src/userspace/init/spawn_plan/drivers_nic.rs:37`). A successful spawn prints `[DRIVER-RTL8139] capsule
...
and it does not persist frames; it is the mechanism the [net_core](../../subsystems/networking/stack.md) stack
