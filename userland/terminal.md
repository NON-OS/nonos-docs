...
clearest worked example of an [app-skeleton](writing-an-app.md) application. This page documents it. The
...
skeleton's `run` (`userland/capsule_terminal/src/main.rs`):
...
`Terminal` implements the [`App` trait](writing-an-app.md): a `manifest` (a normal window,
`term/manifest.rs`), an `on_event` that feeds keystrokes to the line editor and the command
...
sense: you can pipe one command into another, redirect output to a [file](std-pal.md), and chain
...
service registry, `display` queries the screen, `ping` exercises the [network](networking-guide.md),
...
[surface](../subsystems/graphics/surfaces.md): a header, a tab strip, the scrollback grid, the input
line with a cursor, and a footer, themed through `term/theme.rs` and a palette. The `term` module holds
...
into the dispatch; to understand the app model it is built on, read [writing an app](writing-an-app.md),
