# Operations, protocol, and the request loop
...
privileged bring-up that produced the `Driver`, see [bringup.md](bringup.md); for the driver's identity
and mask, see the [README](README.md).
...
(`src/protocol/header.rs:17`, `:19`). A request carries an opcode and no body; a reply reuses the same
...
(`src/protocol/decode.rs:19`, `:23`, `:26`). A valid header decodes into a `Request` carrying the op,
flags, request id, and payload length (`src/protocol/header.rs:22`, `src/protocol/decode.rs:29`).
...
little-endian status word that opens every reply body (`src/protocol/encode.rs:19`, `:29`).

## Errno

Only two status values exist (`src/protocol/errno.rs:17`):
...
(`src/server/runner/run.rs:54`). The opcodes are defined in `src/protocol/ops.rs`.

| Op | Opcode | Request | Reply body | Handler |
|---|---|---|---|---|
| `OP_HEALTHCHECK` | 1 | none | status word only | `ops.rs:17`, `handlers/health.rs:20` |
| `OP_CONTROLLER_INFO` | 2 | none | status + 28-byte register record | `ops.rs:18`, `handlers/controller_info.rs:26` |
| `OP_CODEC_MASK` | 3 | none | status + 8-byte mask payload | `ops.rs:19`, `handlers/codec_mask.rs:26` |
| `OP_STREAM_LAYOUT` | 4 | none | status + 4-byte count + 8-byte entries | `ops.rs:20`, `handlers/stream_layout.rs:26` |
| `OP_CODEC_LIST` | 5 | none | status + 4-byte count + 8-byte entries | `ops.rs:21`, `handlers/codec_list.rs:25` |
...
(`src/server/handlers/health.rs:20`). It confirms the service is up and answering without touching a
register.

### OP_CONTROLLER_INFO

Re-reads the HDA global registers live at request time and returns a 28-byte record
(`src/server/handlers/controller_info.rs:26`). It calls `ControllerInfo::read` again, so the reply is the
current controller state, not the setup-time snapshot (`src/server/handlers/controller_info.rs:27`). The
record is laid out in order (`src/server/handlers/controller_info.rs:32`):
...
The four trailing counts are the `GCAP` decode described on the [bringup](bringup.md) page
(`src/controller/info.rs:53`). The payload length is `4 + 28` and the frame is sent to the reply inbox
(`src/server/handlers/controller_info.rs:28`, `:49`).

### OP_CODEC_MASK

Returns the codec-presence mask (`src/server/handlers/codec_mask.rs:26`). It re-reads `ControllerInfo`,
masks `STATESTS` to its low 15 bits, and returns that mask as a `u16`, two reserved bytes, and the
popcount of the mask as a 4-byte set count (`src/server/handlers/codec_mask.rs:28`, `:32`). The 8-byte
payload is therefore the presence bitmap plus the count of codecs the controller sees on the link.

### OP_STREAM_LAYOUT

Returns the computed stream layout (`src/server/handlers/stream_layout.rs:26`). It re-reads
`ControllerInfo`, runs `layout`, and emits a 4-byte entry count followed by one 8-byte entry per stream
(`src/server/handlers/stream_layout.rs:28`, `:35`):
...
The offset is computed, not read from a live descriptor; the derivation is on the [bringup](bringup.md)
page (`src/controller/stream_layout.rs:48`). The reply length grows with the number of streams the
controller advertises.

### OP_CODEC_LIST

Returns the probed codec inventory captured at setup (`src/server/handlers/codec_list.rs:25`). Unlike the
...
(`src/server/handlers/codec_list.rs:26`, `:33`):
...
status-only frame and send it to `KERNEL_REPLY_ENDPOINT` (`src/server/error.rs:23`, `:29`).

## The runtime interrupt poll

`poll_irq` runs at the top of every loop iteration (`src/server/runner/poll_irq.rs:21`). It calls
`mk_irq_poll` on the IRQ grant id; when the returned sequence number differs from the last seen one, it
records the new sequence and acknowledges with `mk_irq_ack` (`src/server/runner/poll_irq.rs:23`). This
...
The service name and the reply inbox are constants (`src/protocol/endpoint.rs:17`, `:18`):
...
Every reply is sent to `KERNEL_REPLY_ENDPOINT`; the [README](README.md) identity table shows that this id
...
(`src/handles/broker_handles.rs:17`, `:25`). Its accessors expose the IRQ grant id for the poll and the
MMIO user VA for the register base (`src/handles/broker_handles_irq_grant_id.rs:20`,
`src/handles/broker_handles_mmio_user_va.rs:20`). Its `Drop` releases the grants in reverse acquisition
order: it unbinds the interrupt, unmaps BAR0, and releases the device claim
(`src/handles/broker_handles_drop.rs:22`). Because the `Driver` lives for the whole program, this cleanup
...
[claim](../../subsystems/hardware-broker/claim.md), [mmio](../../subsystems/hardware-broker/mmio.md), and
[irq](../../subsystems/hardware-broker/irq.md) pages describe how each grant is validated, bounded to the
