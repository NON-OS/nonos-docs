# Contributing to capsule_driver_hda
...
how it is put together, read the [README](README.md), the [bring-up](bringup.md), and the
[operations](operations.md) pages in this folder.
...
with a code derived from the error (`src/main.rs:37`). The top-level modules are declared there
(`src/main.rs:22`). The tree is one unit per file.

## Module map

| Folder | Owns | Touch it when |
|---|---|---|
| `src/discover.rs` | PCI discovery: the candidate predicate over the audio-class device list | you change how the controller is found |
...
Keep raw MMIO behind the `Regs` wrapper (`src/regs/mmio.rs:29`) and keep every fallible path returning an
...
1. Add the opcode to `src/protocol/ops.rs` next to the existing five (`src/protocol/ops.rs:17`), and add
   any fixed reply sizes to `src/protocol/limits.rs`. The transmit buffer is sized from those limits at
   compile time through `max_tx_body`, so a new worst-case body has to be reflected there
   (`src/server/runner/max_tx_body.rs:19`, `src/protocol/limits.rs:22`).
2. Write the handler as one file under `src/server/handlers/`, exposing `pub fn handle(...)`. Build the
   ...
   `KERNEL_REPLY_ENDPOINT`, the way `controller_info` does (`src/server/handlers/controller_info.rs:26`);
   a status-only reply can use `reply_with_status` instead (`src/server/error.rs:23`). Re-export the
   module from `src/server/handlers/mod.rs` (`src/server/handlers/mod.rs:17`).
3. Wire it into the dispatch match in `src/server/runner/run.rs:54` and add the opcode to the `use`
   import list at the top of the runner (`src/server/runner/run.rs:23`).
4. Keep the op payload-free, because the guard at `src/server/runner/run.rs:50` rejects any request with a
...
If the op needs a new register or codec verb, add the offset to `src/constants/regs.rs` and the access
under `src/controller/`, behind `Regs` (`src/regs/mmio.rs:29`).
...
- **A buffer descriptor list.** Each stream needs a BDL in DMA memory describing the sample buffers, plus
  ...
  computes (`src/controller/stream_layout.rs:48`) are exactly where that programming would target, but
...
- **Stream DMA and the run bit.** Programming the stream format, the cyclic buffer length, and the run
  ...
  (`src/server/runner/poll_irq.rs:21`), which today only drains the line, would gain real work.
...
boot (`Makefile:1010`, `src/userspace/init/spawn_plan/drivers_storage.rs:37`). The README documents the
...
- No panics in capsule code. No `unwrap`, `expect`, or `panic!`. Every fallible path returns an
  `HdaError` (`src/error/types.rs:18`) or an errno, and the release profile is `panic = "abort"`
  (`Cargo.toml:26`).
