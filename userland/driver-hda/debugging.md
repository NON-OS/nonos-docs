# Debugging capsule_driver_hda

This page lists the log markers the driver and its boot path emit, the per-error exit codes, and the
concrete failure modes with where to look for each. For the bring-up path see the [bring-up](bringup.md)
page, for the query surface see the [operations](operations.md) page, and for the driver's identity see
the [README](README.md).

## Log markers

The first thing to confirm is that the capsule ran. The driver fleet boots it under the tag `DRIVER-HDA`
(`src/userspace/init/spawn_plan/drivers_storage.rs:40`), and a successful spawn logs
...
tag `[DRIVER-HDA] load_elf_executable error:` (`src/hardware/hda_capsule/spawn.rs:57`).

## Setup exit codes

Setup failures are hard barriers, and each `HdaError` maps to a distinct process exit code, so a driver
that never comes up tells you exactly where it stopped (`src/error/types.rs:29`):
...
`_start` runs `exit_code(e)` on any setup error and exits with that number (`src/main.rs:43`,
`src/error/types.rs:29`), so the exit code alone names the failing step.
...
predicate (`src/discover.rs:50`). The predicate is strict: PCI bus, multimedia class `0x04`, HDA subclass
`0x03`, BAR0 present as MMIO of at least 4 KiB, a non-zero interrupt pin, and a routed interrupt line
(`src/discover.rs:50`). A controller that is present but has no routed line, or whose BAR0 is not yet
assigned, fails here. This is a discovery problem one layer down, in the broker device table, not a
driver bug; the [claim](../../subsystems/hardware-broker/claim.md) page describes the
...
bring-up steps: claim, bus-master write, BAR0 map, or IRQ bind (`src/setup/claim.rs:23`,
`src/setup/pci.rs:23`, `src/setup/mmio.rs:25`, `src/setup/irq.rs:25`). The setup path unwinds cleanly on
each: a failed map or bind releases what was already taken, so a code-3 exit never leaves a dangling
grant (`src/setup/mmio.rs:26`, `src/setup/irq.rs:26`). The broker's own stage markers name which step was
refused: the [MMIO](../../subsystems/hardware-broker/mmio.md) page documents the `[MMIO]` trace and the
[IRQ](../../subsystems/hardware-broker/irq.md) page the bind-error variants. A `StaleEpoch` here means the
claim was lost between claim and grant, a release race worth tracing on the
[claim](../../subsystems/hardware-broker/claim.md) page.
...
the bounded spin (`src/controller/reset.rs:29`). That points at the controller or the BAR0 mapping, not
...
address (`src/controller/codec_probe.rs:56`). The entry still lists the address and marks it present, so a
...
or 6 instead (`src/controller/immediate.rs:46`, `:59`).
...
(`src/controller/codec_probe.rs:36`, `src/server/handlers/codec_mask.rs:28`). `OP_CONTROLLER_INFO` still
answers with the live registers, so read back `STATESTS` there to confirm the controller sees the same
empty link (`src/server/handlers/controller_info.rs:39`).
...
and answered with `E_INVAL` through the decode-failed path (`src/protocol/decode.rs:23`,
`src/server/error.rs:29`). Any op that arrives with a non-zero `payload_len` is answered with `E_INVAL`
before dispatch, and an unknown opcode falls to the same error (`src/server/runner/run.rs:50`, `:60`). So a
...
(`src/server/runner/poll_irq.rs:21`). There is no stream DMA in this slice, so there is no
completion-interrupt work. If you are looking for audio buffer completions, they do not exist here; see
the [contributing](contributing.md) page for what a playback path would add.
