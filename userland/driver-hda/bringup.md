# Controller bring-up and codec enumeration

This page mirrors the privileged half of the driver: `src/discover.rs`, `src/setup/`, `src/controller/`,
...
(`src/setup/sequence.rs:27`). For the request loop that consumes the resulting `Driver`, see
[operations.md](operations.md); for the driver's place in the system, see the [README](README.md).
...
through the whole controller layer (`src/regs/mmio.rs:24`, `:29`). The register offsets and the two bit
constants it uses live in `src/constants/regs.rs`.

## The bring-up sequence

`setup::run` is the one-shot path, in order (`src/setup/sequence.rs:27`):
...
(`src/setup/sequence.rs:30`); `mmio::map` releases the device on a failed map (`src/setup/mmio.rs:26`);
and `irq::bind` unmaps BAR0 and then releases the device on a failed bind (`src/setup/irq.rs:26`). Once
all three grants are held they are handed to a single `BrokerHandles` owner that frees them on drop
(`src/setup/sequence.rs:36`, described on the [operations](operations.md) page).

### Discovery

`find_hda` asks the broker for the audio-class device list and returns the first record that qualifies
(`src/discover.rs:32`). It requests up to 32 records of class `CLASS_AUDIO` (`0x0050`) through
`mk_device_list` (`src/discover.rs:34`, `src/constants/pci.rs:17`). A record is a candidate only when all
of these hold (`src/discover.rs:50`):
...
(`src/discover.rs:22`, `:23`). The returned `Found` carries the `device_id`, the `irq_line`, and the
BAR0 size, which are the three inputs the rest of setup needs (`src/discover.rs:26`). If nothing matches,
`find_hda` returns `None` and setup fails with `DeviceNotFound` (`src/setup/sequence.rs:28`).
...
- **Claim.** `claim` calls `mk_device_claim(device_id)` and returns the claim epoch; a negative return is
  a failure (`src/setup/claim.rs:21`). Every later broker call carries that epoch, and the broker rejects
  a stale one. The claim is the root authority; see the [device claim](../../subsystems/hardware-broker/claim.md)
...
- **Bus master.** `enable_bus_master` sets the PCI command register's bus-master bit through
  ...
  (`src/setup/pci.rs:21`). It is enabled ahead of the future stream-DMA work; in this slice the controller
...
- **Map BAR0.** `mmio::map` maps BAR0, the HDA register window, into the capsule's address space and
  returns a user VA and a grant id (`src/setup/mmio.rs:22`). It passes offset 0 and the full BAR size, so
  the whole register window is mapped (`src/setup/mmio.rs:24`, `src/constants/pci.rs:18`). The broker
  clamps the mapping to the BAR and withholds any MSI-X table pages; see the
  [MMIO](../../subsystems/hardware-broker/mmio.md) page.
- **Bind IRQ.** `irq::bind` binds the controller's interrupt line through `mk_irq_bind` and returns a
  grant id (`src/setup/irq.rs:22`). The interrupt is polled and acked at runtime but gates nothing: every
  query is a synchronous register read. See the [IRQ](../../subsystems/hardware-broker/irq.md) page.

### Leaving reset

`leave_reset` reads `GCTL`, sets `GCTL.CRST` (bit 0) if it is clear, and spins until the controller
reports `CRST` set, meaning it has come out of reset and is running (`src/controller/reset.rs:23`). The
spin is bounded at one million iterations; if `CRST` never reads back set, it returns
`ControllerResetTimeout` (`src/controller/reset.rs:21`, `:35`). The register offset and the `CRST` bit
are `GCTL = 0x08` and `GCTL_CRST = 1 << 0` (`src/constants/regs.rs:22`, `:31`).
...
and the spin observes it immediately (`src/controller/reset.rs:25`).

### Reading controller info

`ControllerInfo::read` snapshots the controller-global registers into a plain `Copy` struct
(`src/controller/info.rs:40`). It reads `GCAP`, `VMIN`, `VMAJ`, `OUTPAY`, `INPAY`, `GCTL`, `STATESTS`,
`GSTS`, `INTCTL`, and `INTSTS`, each at its fixed offset (`src/controller/info.rs:41`,
`src/constants/regs.rs:17`). From `GCAP` alone it derives four counts, described next
(`src/controller/info.rs:53`).

Setup then rejects an unusable controller: if the major version or `GCAP` reads back as zero, it fails
with `UnsupportedController` (`src/setup/sequence.rs:41`). A controller that answers with all-zero global
...
the stream-layout derivation (`src/controller/info.rs:53`).
...
(`src/controller/stream_layout.rs:32`). Each descriptor's offset is `0x80 + global_index * 0x20`, and the
whole list is capped at 64 entries (`src/controller/stream_layout.rs:48`, `src/controller/streams.rs:19`).
...
`global_index`, and the computed `mmio_offset` (`src/controller/streams.rs:20`, `:24`).
...
not a CORB/RIRB verb ring (`src/controller/immediate.rs:17`, `src/constants/regs.rs:27`). The immediate
interface sends one verb at a time and reads one response, which is exactly enough for inventory.

`get_parameter` composes a 32-bit verb and sends it (`src/controller/immediate.rs:23`). The verb is
packed from the codec address, the node id, the 12-bit verb code, and an 8-bit parameter
(`src/controller/immediate.rs:62`):
...
`send` runs the handshake (`src/controller/immediate.rs:27`): it waits for `IRS.BUSY` to clear, writes
...
`ImmediateResponseTimeout` (`src/controller/immediate.rs:37`, `:49`, `:21`). The bit constants are
`IRS_BUSY = 1 << 0` and `IRS_VALID = 1 << 1` (`src/constants/regs.rs:32`, `:33`).

The only verb this slice ever issues is `Get Parameter(Vendor ID)` (`src/controller/codec_probe.rs:48`).
...
(`src/controller/codec_probe.rs:32`, `:21`). A slot is present when its `STATESTS` bit is set: for each
address, `present = (statests >> address) & 1` (`src/controller/codec_probe.rs:36`). An absent slot yields
an all-zero probe with `present = 0`; a present slot is passed to `read_vendor`
(`src/controller/codec_probe.rs:37`).

`read_vendor` issues `get_parameter(address, node 0, PARAM_VENDOR_ID)` on the immediate interface
(`src/controller/codec_probe.rs:48`). On success it splits the 32-bit response: the vendor id is the high
16 bits and the device id the low 16 bits, and it records `ok = 1`
(`src/controller/codec_probe.rs:52`). If the verb times out it records `present = 1, ok = 0` with zero
ids, so a present-but-unresponsive codec is distinguishable from an identified one
(`src/controller/codec_probe.rs:56`). Note the difference in severity: a codec whose immediate interface
...
- Controller discovery, claim, bus-master enable, BAR0 mapping, and IRQ bind
  (`src/setup/sequence.rs:28`).
- Reset release by driving `GCTL.CRST` high (`src/controller/reset.rs:23`).
- Live controller-global register snapshot and `GCAP`-derived stream counts and address width
  (`src/controller/info.rs:40`).
- Codec-presence detection from `STATESTS` (`src/controller/codec_probe.rs:36`).
- Per-codec vendor and device id via the immediate-command `Get Parameter(Vendor ID)` verb
  (`src/controller/codec_probe.rs:48`).
- Computed stream-descriptor offsets `0x80 + i * 0x20` (`src/controller/stream_layout.rs:48`).
