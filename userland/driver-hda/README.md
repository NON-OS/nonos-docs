...
device interrupt to a kernel-delivered slot (`src/capabilities/types.rs:35`). Crucially, there is no `Dma`
...
`Capsule.mk` and `Cargo.toml`), `src/capabilities/types.rs` (the capability bits), the kernel spawn
