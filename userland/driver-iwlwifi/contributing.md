# Contributing to capsule_driver_iwlwifi
...
standards a change has to meet. For what the driver does and its honest state, read the [README](README.md),
the [operations](operations.md) page, the [bring-up](bring-up.md) page, and the [firmware](firmware.md) page.
...
(`src/main.rs:35`). The top-level modules are declared there (`src/main.rs:22`).
...
| `src/discover.rs` | the `mk_device_list` scan and the Intel Wi-Fi match | you change how the device is found |
| `src/init.rs` | the APM clock bring-up and `InitState` | you touch the power-management sequence |
| `src/regs.rs`, `src/constants/` | register access, offsets, CSR and `GP_CNTRL` bits | you touch a register offset or bit |
| `src/firmware/` | blob selection, TLV parse, section staging, the alive poll | you add a family, a TLV type, or the FH transfer |
| `src/driver.rs` | the built `Driver` struct and `stage_firmware` | you add a field to the driver state |

## Adding a client op

There are three edits, and the dispatch wiring is the load-bearing one.

1. Add the opcode constant to `src/protocol/ops.rs:9` and re-export it from `src/protocol/mod.rs:21`.
2. Write the handler as one file under `src/server/handlers/`, exposing a `handle` function that fills a
   ...
   module in `src/server/handlers/mod.rs:9`.
3. Wire it into the dispatch match in `src/server/runner.rs:37`. Add the arm with the `if body.is_empty()`
...
To support another device, add a `Family` variant in `src/firmware/family.rs:11`, its PCI id range in
`family_for_device` (`family.rs:19`), an `include_bytes!` blob and a `blob_for_family` arm in
`src/firmware/blob.rs`, and place the `.ucode` file under `nonos-bootloader/firmware/intel/`. To stage
another TLV section type, add its constant next to `TLV_SEC_RT` in `src/firmware/tlv.rs:5`, add it to the
`matches!` set in `stage_firmware` (`src/firmware/stage/stage_firmware.rs:40`), and count it in
`count_section` (`src/firmware/stage/count_section.rs:20`).

## Where the driver would grow next

The honest gap the [firmware](firmware.md) page describes is the flow-handler transfer. Wiring the firmware
to the device would add the FH transfer-control register offsets to `src/constants/mod.rs`, a transfer step
...
embed and the spawn (`src/hardware/iwlwifi_capsule/embed.rs:17`,
`src/userspace/init/spawn_plan/drivers_bus.rs:23`), and the source `README.md` documents the profile check
...
- Every setup phase that can fail rolls back the grants acquired before it, in reverse order, inline at the
  failure site (`src/setup/mmio.rs:19`, `src/setup/irq.rs:17`, `src/setup/dma.rs:23`).
- The AGPL header sits at the top of every source file, matching the header on `src/main.rs:1` and every
  other module.
