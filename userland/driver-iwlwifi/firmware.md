# Firmware selection, TLV parsing, and staging
...
is prepared but not delivered. For how the DMA buffer was allocated see the [bring-up](bring-up.md) page; for
the op that triggers staging see the [operations](operations.md) page.
...
size of the one that was chosen (`src/firmware/blob.rs:27`, `src/server/handlers/firmware.rs:14`). The five
`.ucode` files are present in the tree, so the includes resolve at build time.

## Family selection

`family_for_device` maps a PCI device id to a `Family` (`src/firmware/family.rs:19`). The ranges are the
...
(`src/firmware/family.rs:20`). The same function is called during discovery to reject an unsupported device
before it is claimed (`src/discover.rs:62`), so the family is already known by the time the driver is built.
...
(`src/firmware/tlv.rs:17`). It requires at least 20 bytes, a leading zero dword, the Intel magic
...
rejected (`src/firmware/tlv.rs:28`, `src/constants/mod.rs:40`). On success it returns the major, minor, and
...
(`src/firmware/tlv.rs:39`).

## Section staging

`stage_firmware` walks the TLV records after the 20-byte header and copies the sections the driver cares
about into the DMA buffer (`src/firmware/stage/stage_firmware.rs:22`). For each record it reads a 4-byte type
...
it stages the section and counts it (`src/firmware/stage/stage_firmware.rs:33`, `src/firmware/tlv.rs:5`). The
...
(`src/firmware/stage/stage_section.rs:17`). For each staged section it writes a 12-byte record header into
...
runtime, and paging section counts into the state record (`src/firmware/stage/count_section.rs:20`), and the
...
interrupt word, all zeroed by `empty()` until set (`src/firmware/stage/state.rs:17`,
`src/firmware/stage/empty.rs:19`).

Staging is triggered on demand by `OP_FIRMWARE_STAGE`, which calls `Driver::stage_firmware` and stores the
result on the driver (`src/driver.rs:36`, `src/server/handlers/firmware_stage.rs:7`). Setup itself starts
with an empty stage record (`src/setup/sequence.rs:35`), so a client must ask for staging; it is not part of
the boot sequence.
...
`src/init.rs` and the interrupt acknowledgement in the alive poll. So the firmware is prepared in RAM and
...
it returns `(false, last_int)` (`src/firmware/alive.rs:4`, `src/constants/mod.rs:36`). Because the firmware
...
(`src/server/handlers/alive.rs:13`). The alive poll is the correct mechanism waiting for a step that the
