...
early power-management sequence. That whole path is one ordered sequence in `src/setup/sequence.rs:19`, and
this page mirrors it together with the folders it leans on: `src/setup/` (the sequence and the broker calls),
`src/discover.rs` (finding the device), `src/init.rs` (the APM register bring-up), `src/regs.rs` (register
access), `src/constants/` (the offsets and bit definitions), and `src/driver.rs` (the built state). The
firmware selection and staging the driver holds are on the [firmware](firmware.md) page; the broker syscalls
themselves are documented on the [claim](../../subsystems/hardware-broker/claim.md),
...
(`src/setup/sequence.rs:19`, `src/main.rs:39`).
...
1. **Discover.** `find_iwlwifi` calls `mk_device_list` and returns the first PCI function whose vendor is
   ...
   line, with a non-zero MMIO BAR0 (`src/discover.rs:34`, `src/discover.rs:57`, `src/constants/mod.rs:17`).
...
2. **Family check.** `family_for_device` maps the PCI device id to one of five families (7265, 8265, 9260,
   ...
   (`src/setup/sequence.rs:21`, `src/firmware/family.rs:19`). Discovery already required this to match, so
   this second check is belt-and-suspenders.
3. **Claim.** `mk_device_claim` on the device id returns a claim epoch (`src/setup/claim.rs:11`). The epoch
...
4. **Map BAR0.** `mk_mmio_map` maps BAR index 0 at offset 0 for its page-rounded size and returns a user
   virtual address, a length, and a grant id (`src/setup/mmio.rs:14`, `src/constants/mod.rs:18`). The length
   is `(bar0_size + 0xFFF) & !0xFFF`, rounding up to a whole page (`src/setup/mmio.rs:16`). On failure the
   device is released and the error propagates (`src/setup/mmio.rs:19`). `Regs::new` later wraps the returned
   address for volatile register access (`src/regs.rs:23`). See
...
5. **Bind IRQ.** `mk_irq_bind` is called with the device's PCI interrupt line and flags `0`, which is the
   legacy INTx path, not MSI-X (`src/setup/irq.rs:15`). This is unlike the [NVMe driver](../driver-nvme/README.md),
   which requests an MSI-X vector. On failure the MMIO grant is unmapped and the device released before the
   error propagates (`src/setup/irq.rs:17`). See [IRQ grants](../../subsystems/hardware-broker/irq.md).
6. **Map DMA staging.** `mk_dma_map` allocates one 64 KiB (`FW_STAGING_SIZE`) region and returns a user
   virtual address, a device address, a length, and a grant id (`src/setup/dma.rs:22`,
   `src/constants/mod.rs:20`). On failure the IRQ is unbound, the MMIO unmapped, and the device released, in
   that reverse order, before the error propagates (`src/setup/dma.rs:23`). See
   [DMA grants](../../subsystems/hardware-broker/dma.md). This is the buffer the firmware staging writes into.
7. **APM bring-up.** `bring_up` runs the early power-management sequence over the mapped registers
   (`src/init.rs:23`). It sets `GP_CNTRL_XTAL_ON`, then `GP_CNTRL_MAC_ACCESS_REQ | GP_CNTRL_INIT_DONE`, then
   polls `CSR_GP_CNTRL` for `GP_CNTRL_MAC_CLOCK_READY` up to `APM_POLL_ITERS` (250000) iterations
   (`src/init.rs:24`, `src/constants/mod.rs:35`). If the clock never comes ready it returns
   ...
   status, and reads the hardware revision from `CSR_HW_REV` (`src/init.rs:29`). It derives an `rf_kill` flag
   from whether `GP_CNTRL_INIT_DONE` is clear in the read-back general-control word (`src/init.rs:37`).
8. **Acknowledge the IRQ and build the driver.** After bring-up, `mk_irq_ack` acknowledges the bound
   ...
   (`src/setup/sequence.rs:28`, `src/driver.rs:12`).
...
flag, the family, the `Regs` accessor, and the firmware stage state (`src/driver.rs:12`).
...
|---|---|---|
| Device claim | `src/setup/claim.rs:11` | the exclusive hold on the Intel Wi-Fi PCI function; the root of every other grant |
| MMIO | `src/setup/mmio.rs:14` | BAR0 mapped into the capsule as a user VA, wrapped by `Regs` |
| IRQ | `src/setup/irq.rs:15` | one legacy INTx binding on the device's PCI interrupt line |
| DMA | `src/setup/dma.rs:22` | one 64 KiB broker-issued staging buffer with a user VA and a device address |
...
it fails (`src/setup/mmio.rs:19`, `src/setup/irq.rs:17`, `src/setup/dma.rs:23`). Because the server loop
...
the DMA buffer (see [revocation](../../subsystems/hardware-broker/revocation.md)).

## The register block

`Regs` is a base address plus volatile 32-bit reads and writes, a `set_bits` read-modify-write, and a
`poll_set` that spins until a mask is set or an iteration cap is reached (`src/regs.rs:22`). The offsets and
bit definitions live in `src/constants/mod.rs`: the control and status registers `CSR_INT_COALESCING` at
`0x004`, `CSR_INT` at `0x008`, `CSR_INT_MASK` at `0x00C`, `CSR_FH_INT_STATUS` at `0x010`, `CSR_GP_CNTRL` at
`0x024`, and `CSR_HW_REV` at `0x028` (`src/constants/mod.rs:22`). The `GP_CNTRL` bits are
`MAC_CLOCK_READY` (`0x2`), `INIT_DONE` (`0x4`), `MAC_ACCESS_REQ` (`0x8`), and `XTAL_ON` (`0x400`)
(`src/constants/mod.rs:28`). All register access is 32-bit; there is no 64-bit access and no doorbell or
...
[NVMe driver](../driver-nvme/bring-up.md), each visible in the sequence above. The claim is device-scoped
and epoch-gated, so the capsule can only act on the one Intel Wi-Fi function it claimed and cannot use a
stale claim (`src/setup/claim.rs:11`; [claim.md](../../subsystems/hardware-broker/claim.md)). The MMIO grant
is exactly BAR0 of that function at a broker-chosen user address, clamped away from the MSI-X table, so the
driver never sees a physical address or another device's registers (`src/setup/mmio.rs:14`;
[mmio.md](../../subsystems/hardware-broker/mmio.md)). The IRQ is a legacy INTx binding the kernel programs;
the capsule receives only a grant id and drives it through ack, never touching the interrupt controller
(`src/setup/irq.rs:15`; [irq.md](../../subsystems/hardware-broker/irq.md)). The DMA region is a single
broker-issued grant with its own device address (`src/setup/dma.rs:22`;
[dma.md](../../subsystems/hardware-broker/dma.md)).
...
(see the [firmware](firmware.md) page). But bus mastering is not disabled either, and the same universal DMA
