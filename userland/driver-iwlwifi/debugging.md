# Debugging capsule_driver_iwlwifi
...
[README](README.md), the [operations](operations.md) page, the [bring-up](bring-up.md) page, and the
...
`DRIVER-IWLWIFI` (`src/userspace/init/spawn_plan/drivers_bus.rs:26`), whose `Ok` arm calls
`boot_log::ok(prefix, "capsule spawned")` (`src/userspace/init/capsule_boot/run.rs:29`), which formats
`[` + tag + `] ` + message. An absent line means the capsule never started, usually a signature, manifest,
or capability failure; the `Err` arm prints an error line instead (`src/userspace/init/capsule_boot/run.rs:32`).
The spawn is feature-gated: without `nonos-capsule-driver-iwlwifi` the spawn function is a no-op and no
marker appears at all (`src/userspace/init/spawn_plan/drivers_bus.rs:34`).
...
(`src/main.rs:37`, `src/main.rs:40`). So exit 2 is the single "bring-up failed" code, and the specific reason
is the `&'static str` the failing step returned. The steps and their messages:

| Message | Where | Meaning |
|---|---|---|
| `iwlwifi: device not found` | `src/setup/sequence.rs:20` | no PCI function matched Intel vendor, class/subclass `02/80`, a supported family id, a valid IRQ pin/line, and a non-zero MMIO BAR0 (`src/discover.rs:34`). |
| `iwlwifi: unsupported device` | `src/setup/sequence.rs:21` | the matched device id has no family mapping; in practice unreachable because discovery already required one (`src/firmware/family.rs:19`). |
| `iwlwifi: device claim failed` | `src/setup/claim.rs:14` | `mk_device_claim` returned non-positive, usually a missing `Driver` capability or a device already claimed. |
| `iwlwifi: mmio map failed` | `src/setup/mmio.rs:20` | `mk_mmio_map` on BAR0 was refused; the device is released before returning. |
| `iwlwifi: irq bind failed` | `src/setup/irq.rs:19` | `mk_irq_bind` (legacy INTx) was refused; the MMIO grant is unmapped and the device released. |
| `iwlwifi: dma staging failed` | `src/setup/dma.rs:27` | `mk_dma_map` for the 64 KiB staging region was refused; the IRQ, MMIO, and claim are rolled back. |
| `iwlwifi: mac clock not ready` | `src/init.rs:27` | the APM `MAC_CLOCK_READY` poll spun out after 250000 iterations without the clock coming ready. |

Because all of these collapse to exit code 2, the way to tell them apart is the console message the failing
step printed, or the broker's own stage markers on the [MMIO](../../subsystems/hardware-broker/mmio.md),
...
and line: discovery skips a device with `irq_pin == 0` or `irq_line == 0xFF` (`src/discover.rs:41`). A
`NONOS_DEVICE_CENSUS=1` build renders the broker device table so you can confirm the device and its IRQ
routing before any driver runs (see the [claim](../../subsystems/hardware-broker/claim.md) page).

### MAC clock not ready

This is the one register-level bring-up failure. `bring_up` sets `XTAL_ON` then `MAC_ACCESS_REQ | INIT_DONE`
and polls `GP_CNTRL` for `MAC_CLOCK_READY` (`src/init.rs:24`). A spin-out means the device did not bring its
...
the [operations](operations.md) page; the two that indicate a real condition rather than a malformed request:

### OP_ALIVE_WAIT returns E_TIMEOUT

`OP_ALIVE_WAIT` returns `E_TIMEOUT` (`-110`) when the alive-bit poll spun out
(`src/server/handlers/alive.rs:13`). On this capsule that is the expected result, not a fault: nothing
programs the flow-handler transfer that would make the firmware run and raise the alive interrupt, so the
bit is not expected to be set (see the [firmware](firmware.md) page). A client should treat a timeout here as
...
(`src/server/handlers/firmware_stage.rs:9`). Two causes: the TLV header failed validation (a wrong magic, or
an API version outside 22 to 77) (`src/firmware/tlv.rs:23`), or the sections overflowed the 64 KiB DMA
staging capacity (`src/firmware/stage/stage_section.rs:27`). The first points at the wrong or a corrupt blob
...
(`src/constants/mod.rs:20`).

### Bad requests

`E_BAD_OP` (`-38`) is an unknown opcode with an empty body, and `E_INVAL` (`-22`) is any request that carried
a non-empty body, since every op is fixed-width and takes no payload (`src/server/runner.rs:47`,
