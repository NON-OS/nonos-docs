# Client operations and the NIWF protocol
...
[README](README.md); for how the bring-up produced the state these handlers report see the
[bring-up](bring-up.md) and [firmware](firmware.md) pages.

## The wire format

A request header is 20 bytes and begins with a magic and a version (`src/protocol/header.rs:9`). The parser
rejects anything shorter than the header, a wrong magic, or a wrong version, returning `None` so the server
skips the request without touching the device (`src/protocol/decode.rs:11`, `src/server/runner.rs:31`).
...
subtraction so the bound cannot underflow (`src/protocol/decode.rs:16`). The body it returns is exactly the
...
(`src/protocol/encode.rs:11`): the magic, the version, the request's opcode echoed back, the signed errno in
...
(`src/server/respond.rs:14`).

## The request loop

`server::run` sizes one receive buffer and one transmit buffer each to the header plus `IPC_PAYLOAD_MAX`
(256 bytes) (`src/server/runner.rs:22`, `src/protocol/limits.rs:9`). The loop receives one request with
`mk_ipc_recv_from` from the service inbox, skips a receive of zero or fewer bytes or one from pid 0, parses
it, and dispatches (`src/server/runner.rs:25`). A parse failure is skipped silently, since a malformed
header has no request id to reply to (`src/server/runner.rs:31`).

`dispatch` matches the opcode with an empty-body guard on every arm: each of the seven ops is handled only
`if body.is_empty()` (`src/server/runner.rs:36`). An opcode that is recognised but carries a body, and any
unrecognised opcode, both fall through: an unknown op with an empty body is answered `E_BAD_OP`, and
anything with a non-empty body is answered `E_INVAL` (`src/server/runner.rs:47`, `runner.rs:50`). So a
client cannot smuggle a payload into a fixed-width query.

## The seven operations

The opcodes are defined in `src/protocol/ops.rs:9` and dispatched in `src/server/runner.rs:37`. Every op
takes no request payload.

| Op | Opcode | Reply payload after the header errno | Handler |
|---|---|---|---|
| `OP_HEALTHCHECK` | `0x0001` | none (errno only) | `server/handlers/health.rs:12` |
| `OP_DEVICE_INFO` | `0x0002` | 32-byte device record | `server/handlers/device.rs:13` |
| `OP_FIRMWARE_INFO` | `0x0003` | name length, blob size, and the name bytes | `server/handlers/firmware.rs:13` |
| `OP_RF_STATE` | `0x0004` | 8-byte RF-kill flag and live `GP_CNTRL` | `server/handlers/rf.rs:13` |
| `OP_DMA_STATE` | `0x0005` | 56-byte grant metadata | `server/handlers/dma.rs:13` |
| `OP_FIRMWARE_STAGE` | `0x0006` | 32-byte staging record, or `E_FW_INVALID` | `server/handlers/firmware_stage.rs:6` |
| `OP_ALIVE_WAIT` | `0x0007` | 8-byte seen flag and interrupt word | `server/handlers/alive.rs:6` |

## Payload detail on each op

- `OP_HEALTHCHECK` replies with the header carrying `E_OK` and no payload (`src/server/handlers/health.rs:13`).
...
- `OP_DEVICE_INFO` packs a fixed 32-byte record: the broker device id (u64), the PCI device id (u16), the
  ...
  `GP_CNTRL` value captured at bring-up as a u32 at offset 20 (`src/server/handlers/device.rs:14`). The hw
...
- `OP_FIRMWARE_INFO` reports the firmware blob that family selection chose: a u32 name length (clamped to
  ...
  reply trimmed to `16 + n` bytes (`src/server/handlers/firmware.rs:14`, `src/protocol/limits.rs:10`). This
...
- `OP_RF_STATE` returns an 8-byte record: the `rf_kill` flag derived at bring-up as a u32, then a live read
  of the `CSR_GP_CNTRL` register (`src/server/handlers/rf.rs:14`). The second word is the one live register
...
- `OP_DMA_STATE` returns the 56-byte broker grant metadata: the DMA grant id, the DMA user virtual address,
  ...
  u64 (`src/server/handlers/dma.rs:14`). This is the staging window a firmware transfer would use.
- `OP_FIRMWARE_STAGE` runs the staging on demand: it calls `Driver::stage_firmware`, and on success returns
  ...
  paging section counts, and the total staged byte count (`src/server/handlers/firmware_stage.rs:15`). On a
  TLV parse or staging failure it returns `E_FW_INVALID` with no payload (`firmware_stage.rs:9`). The
  [firmware](firmware.md) page covers what staging does and does not do.
- `OP_ALIVE_WAIT` polls the interrupt-status register for the firmware alive bit and returns an 8-byte
  ...
  (`src/server/handlers/alive.rs:10`). The errno is `E_OK` when the bit was seen and `E_TIMEOUT` when the
...
All errno words are signed little-endian negatives in the header flags field (`src/protocol/errno.rs`):
...
and skips anything malformed (`src/protocol/decode.rs:11`). The empty-body guard on every dispatch arm means
a query op cannot smuggle a body, and a non-empty body is refused with `E_INVAL` before any handler runs
(`src/server/runner.rs:36`). There is no client-supplied length, offset, or address anywhere in the
...
observed (`src/firmware/alive.rs:9`), which writes back the exact bits it read. There is no panic path: the
