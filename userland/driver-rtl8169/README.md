# The RTL8169 NIC Driver Capsule

`capsule_driver_rtl8169` is the NONOS Realtek RTL8168/RTL8169 gigabit Ethernet driver: a signed ring-3
...
controller only through the [hardware broker](../../subsystems/hardware-broker/README.md), claiming the PCI
...
The frames it moves belong to the [network stack](../../subsystems/networking/README.md): the
[NIC driver page](../../subsystems/networking/drivers.md) describes how `net_core` binds to a driver like
...
| Service handle | `driver.rtl8169_0` | `Capsule.mk:7`, `src/hardware/rtl8169_capsule/spawn.rs:32` |
...
| Reply endpoint | `reply:4215:endpoint.4294967310` | `Capsule.mk:14`, `spawn.rs:34`, `client/transport.rs:25` |
| Capability mask | `0xF8019` | `Capsule.mk:16`, `spawn.rs:51` |
| Binary name | `driver_rtl8169` | `Capsule.mk:10`, `Cargo.toml:14` |
| Kernel mirror | `src/hardware/rtl8169_capsule` | `src/hardware/rtl8169_capsule/spawn.rs` |
...
(`src/hardware/rtl8169_capsule/client/transport.rs:25`), and the capsule hard-codes the same value as
`KERNEL_REPLY_ENDPOINT = 0x1_0000_000E` (`src/protocol/endpoint.rs:17`). The service port `4214` and reply
...
(`README.md:24`), but the shipped code binds the interrupt (`src/setup/irq.rs:23`) and acknowledges it
(`src/rx/recv_one.rs:38`) without ever polling a notification slot; the server loop is a blocking IPC
...
|------|---------|----------------|
| [operations.md](operations.md) | `src/protocol/`, `src/server/` | The `NR69` wire format, the request loop, the six client ops, per-op payloads, the errno set, and the frame bounds. |
| [bring-up.md](bring-up.md) | `src/setup/`, `src/discover*`, `src/init/`, `src/regs.rs`, `src/constants/` | Discovery, the broker grants (claim, bus master, MMIO, IRQ, DMA), the register block, the soft reset, MAC read, and device enable. |
| [rings.md](rings.md) | `src/queue/`, `src/tx/`, `src/rx/` | The transmit and receive descriptor rings, the `OWN` bit handshake, the DMA buffers, and the send and receive paths. |
| [contributing.md](contributing.md) | the whole tree | Where each concern lives, how to add a client op, the build and sign steps, and the code standards. |
| [debugging.md](debugging.md) | runtime | The boot marker, the setup and bring-up exit codes, and the runtime failure modes: link down, RX empty, and a TX timeout on real hardware. |
...
request server, which loops forever (`src/main.rs:40`). `setup::run` acquires the broker grants
(`src/setup/sequence.rs:25`) and `init::bring_up` programs the device (`src/init/run.rs:24`); a failure in
either exits with a distinct code and never serves a request, and a bring-up failure releases the grants
first (`src/main.rs:44`, `src/main.rs:48`). The kernel spawns it through
[verified spawn](../../security/capsules-and-trust.md) under the NIC driver plan, checking its signature and
attestation and holding its requested capabilities against its manifest ceiling before its ELF is mapped
(`src/userspace/init/spawn_plan/drivers_nic.rs:50`). A successful spawn prints `[DRIVER-RTL8169] capsule
spawned` on the boot log; the [debugging](debugging.md) page covers what that and each exit code mean.
...
peer identity, or persist traffic; it is the mechanism the [network stack](../../subsystems/networking/stack.md)
