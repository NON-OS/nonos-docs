# Debugging capsule_driver_rtl8169
...
(`src/userspace/init/spawn_plan/drivers_nic.rs:52`), which delegates to `capsule_boot::boot`
(`src/userspace/init/spawn_plan/boot.rs:26`), whose `Ok` arm calls `boot_log::ok(prefix, "capsule spawned")`
(`src/userspace/init/capsule_boot/run.rs:29`), which formats `[` + tag + `] ` + message
...
(`src/userspace/init/capsule_boot/run.rs:32`). The driver is feature-gated, so also confirm the build
includes `nonos-capsule-driver-rtl8169`, or `spawn_rtl8169` is the empty stub and nothing spawns
(`src/userspace/init/spawn_plan/drivers_nic.rs:59`).
...
is the fastest way to tell how far it got (`src/main.rs:35`).

| Exit | Constant | Meaning and where it comes from |
|---|---|---|
| 1 | `EXIT_HEAP_INIT` | `heap_init` failed before anything else ran (`src/main.rs:41`). |
| 2 | `EXIT_SETUP_FAILED` | `setup::run` failed: no device matched, or a claim, bus-master, MMIO, IRQ, or DMA grant was refused (`src/main.rs:45`, `src/setup/sequence.rs:25`). |
| 3 | `EXIT_BRING_UP_FAILED` | `init::bring_up` failed after the grants were taken: the soft reset timed out or the MAC was invalid; the driver releases every grant first (`src/main.rs:48`, `src/init/run.rs:24`). |
...
If `find_rtl8169` returns `None`, setup fails immediately with exit 2 (`src/setup/sequence.rs:26`). The scan
requires a Realtek vendor id, a matching RTL8169 device id, the network class and Ethernet subclass, a usable
IRQ line, and an MMIO BAR of at least `0x100` bytes (`src/discover/support.rs:24`, `src/discover.rs:47`,
`src/discover/bar_mmio.rs:22`). A device that is present but not matched is a discovery problem one layer
down; a `NONOS_DEVICE_CENSUS=1` build renders the broker device table so you can confirm the NIC is even
enumerated, as described on the [claim](../../subsystems/hardware-broker/claim.md) page.
...
another capsule already holds ([claim.md](../../subsystems/hardware-broker/claim.md)); a refused MMIO, IRQ,
or DMA grant means the epoch check failed or the grant class was denied. Each refusal rolls back the grants
already taken before returning (`src/setup/pci.rs:26`, `src/setup/mmio.rs:25`, `src/setup/irq.rs:24`,
`src/setup/dma.rs:45`), so a failed setup leaves nothing claimed.
...
(`src/init/reset.rs:26`); on real hardware a reset that never completes usually means the register BAR was
mapped but the chip is wedged or the wrong BAR was mapped. The MAC read rejects an all-zero or all-`0xFF`
address (`src/init/mac.rs:26`); an invalid MAC points at a register block that is mapped but not responding
...
(`src/server/handlers/link_status.rs:27`, `src/constants/regs.rs:38`). Check the cable and the peer before
...
frame there yet (`src/server/handlers/rx_packet.rs:32`, `src/rx/recv_one.rs:44`). This is the normal
...
the maximum Ethernet frame (`src/server/handlers/rx_packet.rs:35`, `src/rx/recv_one.rs:37`,
`src/rx/recv_one.rs:48`). The driver rearms the slot and moves on, so a single bad descriptor does not wedge
the ring.
...
(`src/server/handlers/tx_packet.rs:24`, `tx_packet.rs:28`). Those two are refused before the device is
touched. `E_IO` (`-5`) means the send itself failed at the device: the descriptor was still busy, the
completion poll timed out, or the TX-error interrupt bit was set (`src/tx/send.rs:29`,
`src/tx/poll_done.rs:33`, `src/tx/send.rs:59`). A transmit timeout on real hardware usually points at bus
...
only registers that do not clear on access (`src/server/handlers/stats.rs:40`). Use it to see whether the
