# Contributing to capsule_driver_rtl8169
...
enable the device, and hands the built `Driver` to `server::run`, which loops forever (`src/main.rs:40`). The
...
| `src/discover.rs`, `src/discover/` | the `mk_device_list` scan and the Realtek match | you change how the NIC is found |
...
| `src/regs.rs`, `src/constants/` | register access, offsets, and bit definitions | you touch a register offset or bit |

## Adding a client op

There are three edits, and the dispatch wiring is the load-bearing one.

1. Add the opcode constant to `src/protocol/ops.rs:17`, and re-export it from `src/protocol/mod.rs:34`. If
   the op carries a fixed reply payload, add its length to `src/protocol/limits.rs:24` and re-export it from
   `src/protocol/mod.rs:30`.
2. Write the handler as one file under `src/server/handlers/`, exposing a `handle` function that encodes the
   ...
   `reply_with_status` (`src/server/error.rs:23`). Declare the module in `src/server/handlers/mod.rs:17`.
3. Wire it into the dispatch match in `src/server/runner.rs:53`. An unrecognised opcode already falls through
...
`src/hardware/rtl8169_capsule/protocol/ops.rs:17` and add a client method under
...
`src/constants/regs.rs:17`. Add or change a constant there and reference it by name from the `init`, `tx`,
`rx`, or handler code; do not inline a raw offset or bit at a use site. The register accessor itself is
`src/regs.rs:19` and is a thin volatile 8/16/32-bit wrapper, so a new access is a call to `r8`/`r16`/`r32` or
...
`src/hardware/rtl8169_capsule/embed.rs:17`.
...
- One unit per file. New ops are one file per handler under `src/server/handlers/`, and the setup, init, tx,
  ...
  existing tree (`src/protocol/mod.rs:17`, `src/server/handlers/mod.rs:17`).
- Every grant taken in setup must have a matching unmap. The `Driver::release` path unmaps the four DMA
  ...
  (`src/setup/driver.rs:38`), and a mid-sequence DMA failure rolls back the same way through
  `rollback::after` (`src/setup/rollback.rs:21`). A new grant must extend both.
- Keep the driver MMIO-only and raw-frame-only. Do not request a PIO grant, and do not add ARP, IP, socket,
  or firewall logic; that policy belongs above the driver in the [network stack](../../subsystems/networking/README.md).
