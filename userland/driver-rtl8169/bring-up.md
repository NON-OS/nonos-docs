...
MAC, and turn it on. That path is two ordered sequences: the grant acquisition in `src/setup/sequence.rs:25`,
and the device programming in `src/init/run.rs:24`. This page mirrors both together with the folders they
lean on: `src/discover.rs` and `src/discover/` (finding the device), `src/setup/` (the broker calls and the
built `Driver`), `src/init/` (reset, MAC, and enable), `src/regs.rs` (register access), and `src/constants/`
(the offsets and bit definitions). The transmit and receive rings the programming steps set up live on the
[rings](rings.md) page; the broker syscalls themselves are documented on the
...
a partly built driver never serves IPC (`src/main.rs:44`, `src/main.rs:48`).
...
returns the first that passes three filters (`src/discover.rs:37`). A record is supported when its vendor is
...
(`0x00`) (`src/discover/support.rs:24`, `src/constants/pci.rs:17`, `src/constants/pci.rs:18`). It must also
report a usable interrupt, a non-zero IRQ pin and an IRQ line other than `0xFF` (`src/discover.rs:47`), and
it must expose an MMIO BAR of at least `0x100` bytes, the first of which is picked as the register BAR
(`src/discover/bar_mmio.rs:19`). Discovery also records the device's declared BAR command bits, an MMIO or
PIO summary used later to preserve the PCI command register (`src/discover/bar_command.rs:19`). No match
...
`Driver` (`src/setup/sequence.rs:25`).

1. **Claim.** `claim::claim` calls `mk_device_claim` on the device id and returns a claim epoch, failing if
   the broker returns a non-positive value (`src/setup/claim.rs:19`). The epoch is the token every later
...
2. **Bus master.** `pci::enable_bus_master` ors the discovered command bits with `MK_PCI_CMD_BUS_MASTER`
   ...
   controller may DMA (`src/setup/pci.rs:23`). On failure it releases the device and returns an error
   (`src/setup/pci.rs:26`).
3. **Map the register BAR.** `mmio::map` calls `mk_mmio_map` for the discovered BAR index at its full size
   ...
   (`src/setup/mmio.rs:21`). On failure it releases the device (`src/setup/mmio.rs:25`). `Regs::new` wraps
   the returned address for volatile 8/16/32-bit register access (`src/regs.rs:25`). See
   [MMIO grants](../../subsystems/hardware-broker/mmio.md).
4. **Bind the interrupt.** `irq::bind` calls `mk_irq_bind` for the discovered IRQ line and returns a grant id
   and a vector (`src/setup/irq.rs:21`). On failure it unmaps the MMIO grant and releases the device before
   returning (`src/setup/irq.rs:24`). Unlike the NVMe driver's best-effort MSI-X bind, a failed bind here is
   fatal to setup. See [IRQ grants](../../subsystems/hardware-broker/irq.md).
5. **Allocate the DMA rings and buffers.** `dma::map_all` maps four DMA regions through `mk_dma_map`, each
   ...
   (`src/setup/dma.rs:39`, `src/setup/dma.rs:29`). Each region returns a user VA and a broker-issued device
   address. If any of the four fails, `rollback::after` unmaps the DMA regions taken so far, unbinds the IRQ,
   unmaps the MMIO grant, and releases the device, in that order (`src/setup/dma.rs:45`,
   `src/setup/rollback.rs:21`). See [DMA grants](../../subsystems/hardware-broker/dma.md).
...
addresses (`src/setup/sequence.rs:32`, `src/setup/driver.rs:23`).

## Step 7 through 11: bring the device up

With the grants in hand, `init::bring_up` programs the chip (`src/init/run.rs:24`).

1. **Soft reset.** `reset::run` writes `CMD_RESET` (`0x10`) to the command register `0x37` and polls it low,
   spinning up to a million iterations before returning a timeout (`src/init/reset.rs:22`,
   `src/constants/regs.rs:20`, `src/constants/regs.rs:31`). The controller clears the bit itself when the
   reset completes.
2. **Read the MAC.** `mac::read` reads six bytes from `REG_MAC0` (`0x00`) and rejects an all-zero or
   all-`0xFF` address as invalid, caching a valid MAC into `driver.mac` (`src/init/mac.rs:21`,
   `src/init/run.rs:26`). This both learns the hardware address and confirms the register block responds.
3. **Program the RX ring.** `rx_setup::program` fills all sixteen RX descriptors with a buffer device
   ...
   broadcast frames with the DMA burst fields (`src/init/rx_setup.rs:27`). The descriptor layout is on the
   [rings](rings.md) page.
4. **Program the TX ring.** `tx_setup::program` fills all sixteen TX descriptors with a buffer device address
   ...
   the inter-frame-gap and DMA-burst fields (`src/init/tx_setup.rs:25`).
5. **Enable.** `bring_up` clears any pending interrupt by writing `0xFFFF` to the ISR (`0x3E`), unmasks the
   ...
   in the command register, starting the receive and transmit engines (`src/init/run.rs:29`,
   `src/constants/regs.rs:45`).
...
|---|---|---|
| Device claim | `Driver.device_id` | the exclusive hold on the PCI function; the root of every other grant (`src/setup/claim.rs:19`) |
| MMIO | `Driver.mmio_grant` | the register BAR mapped into the capsule as a user VA (`src/setup/mmio.rs:21`) |
| IRQ | `Driver.irq_grant` | the bound device interrupt, acknowledged in the receive path (`src/setup/irq.rs:21`) |
| DMA (x4) | `Driver.rx_ring_grant`, `rx_buffer_grant`, `tx_ring_grant`, `tx_buffer_grant` | the four broker-issued buffers with user VAs and device addresses (`src/setup/dma.rs:39`) |

The capsule programs the controller only with the broker-issued device addresses of its rings and buffers,
never a physical address it chose (`src/init/rx_setup.rs:43`, `src/init/tx_setup.rs:36`).

## The register block

`Regs` is a base address plus volatile 8/16/32-bit reads and writes (`src/regs.rs:19`). The offsets and bit
definitions live in `src/constants/regs.rs`: `MAC0` at `0x00`, the TX descriptor address at `0x20`/`0x24`,
...
descriptor address at `0xE4`/`0xE8` (`src/constants/regs.rs:17`). The command bits (`RESET`, `RX_ENABLE`,
...
(`src/constants/regs.rs:31`).

## Grant teardown

The `Driver` exposes `release`, which unmaps the four DMA regions, unbinds the IRQ, unmaps the MMIO grant,
and releases the device claim, in that reverse order (`src/setup/driver.rs:38`). It is called when
`init::bring_up` fails after the grants were taken (`src/main.rs:49`), and `setup::run` itself rolls back
through `rollback::after` if a DMA allocation fails mid-sequence (`src/setup/rollback.rs:21`). Because the
...
buffer (see [revocation](../../subsystems/hardware-broker/revocation.md)).
...
one NIC it claimed and cannot use a stale claim (`src/setup/claim.rs:19`;
[claim.md](../../subsystems/hardware-broker/claim.md)). The MMIO grant is exactly the discovered BAR of that
function at a broker-chosen user address, so the driver never sees a physical address or another device's
registers (`src/setup/mmio.rs:21`). Each DMA region is a separate grant with its own device address returned
by the broker, and the controller is programmed only with those addresses (`src/setup/dma.rs:39`,
`src/init/rx_setup.rs:43`). Every grant is released on the error path and again by the kernel on process
death (`src/setup/driver.rs:38`).

The honest caveat is the absence of an IOMMU on the current target. Bus mastering is enabled
(`src/setup/pci.rs:23`) and the controller is handed device addresses for its rings and buffers, but nothing
...
[network stack](../../subsystems/networking/drivers.md) keeps this driver in its own capsule holding nothing
