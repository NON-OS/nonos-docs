# Debugging capsule_driver_nvme

This page lists the log marker the driver's boot path emits, the bring-up exit codes, and the concrete
runtime failure modes with where to look for each. For the shape of the driver see the [README](README.md),
the [operations](operations.md) page, the [bring-up](bring-up.md) page, and the [queues](queues.md) page.
...
(`src/userspace/init/spawn_plan/drivers_storage.rs:49`), whose `Ok` arm calls `boot_log::ok(prefix,
"capsule spawned")` (`src/userspace/init/capsule_boot/run.rs:29`), which formats `[` + tag + `] ` + message
...
way to tell how far setup got (`src/error/types.rs:30`). Each maps to one `NvmeError`.

| Exit | Error | Meaning and where it comes from |
|---|---|---|
| 30 | `DeviceNotFound` | No PCI function matched NVMe class/subclass/prog-if `01/08/02` with a `>= 0x4000` MMIO BAR0 (`src/discover.rs:49`). |
| 31 | `ClaimFailed` | `mk_device_claim` was refused, usually a missing `Driver` capability or a device already claimed (`src/setup/claim.rs:23`). |
| 32 | `BrokerCallFailed` | A bus-master write, MMIO map, or DMA map was refused (`src/setup/pci.rs:23`, `src/setup/mmio.rs:26`, `src/dma/region.rs:31`). |
| 33 | `UnsupportedController` | The mapped block did not look like NVMe, or the controller raised `CSTS.CFS` (fatal) during reset or enable (`src/controller/info/is_nvme_register_block.rs:21`, `src/admin/controller.rs:41`). |
| 34 | `UnsupportedPageSize` | The controller's minimum page shift is not 12; the driver only supports 4 KiB pages (`src/admin/controller.rs:30`). |
| 35 | `ControllerTimeout` | A ready poll or a queue completion never landed within the fixed spin limit (`src/admin/controller.rs:49`, `src/admin/queue/wait.rs:36`, `src/nvm/wait.rs:35`). |
| 36 | `AdminCommandFailed` | An admin or IO command completed with a non-zero status field (`src/admin/queue/wait.rs:32`, `src/nvm/wait.rs:31`). |
...
`src/controller/info/is_nvme_register_block.rs:21`), or because the reset or enable poll saw `CSTS.CFS`
raised, meaning the controller declared itself fatal (`src/admin/controller.rs:41`). The two are
...
(`src/admin/queue/constants.rs:22`, `src/nvm/constants.rs:26`). On QEMU this is rare. On real hardware it
...
(`src/server/runner.rs:75`, [queues](queues.md)), a missing MSI-X bind does not cause this; a real
...
NSID 1 did not report a 512-byte LBA with a non-zero size at setup (`src/setup/sequence.rs:63`). Identify and
SMART still answer, so `OP_IDENTIFY_NAMESPACE` is the probe: an absent namespace shows nsid 0
(`src/admin/namespace.rs:30`). This is the "no namespaces" case. It is not a claim or grant failure, the
...
`-22`) (`src/server/handlers/rw_parse.rs:26`). Check the reported capacity with `OP_CAPACITY` first, then
...
(`src/server/handlers/read.rs:33`, `src/server/handlers/write.rs:33`).
...
`E_IO` (`-5`) (`src/server/handlers/read.rs:40`, `write.rs:43`, `flush.rs:27`). This is a device-side error,
