# Contributing to capsule_driver_nvme
...
read the [README](README.md), the [operations](operations.md) page, the [bring-up](bring-up.md) page, and
...
(`src/main.rs:40`). The top-level modules are declared there (`src/main.rs:22`).
...
| `src/discover.rs` | the `mk_device_list` scan and the NVMe match | you change how the device is found |
...
2. Write the handler as one file under `src/server/handlers/`, exposing a `handle` function that encodes the
   ...
   (`src/server/error.rs:23`). Declare the module in `src/server/handlers/mod.rs:17`.
3. Wire it into the dispatch match in `src/server/runner.rs:54`. If the op takes no payload, add it to the
...
command, and waits the completion. The read/write path in `src/nvm/transfer.rs:25` and the flush path in
`src/nvm/flush.rs:23` are the reference shapes; both roll the command id with
...
- Every setup phase must have reverse-order rollback, which is what the `Drop` impls provide: `BrokerHandles`
  unbinds the IRQ, unmaps MMIO, and releases the claim in reverse order (`src/handles/broker_handles_drop.rs:21`),
  and each `DmaRegion` unmaps itself (`src/dma/region.rs:46`).
