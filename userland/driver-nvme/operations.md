# Client operations and the NNVM protocol
...
[README](README.md); for how the handlers reach the device see the [queues](queues.md) page.
...
the caller answers `E_INVAL` (`src/protocol/decode.rs:19`, `src/server/runner.rs:45`).
...
little-endian status word, then any payload (`src/protocol/limits.rs:17`, `src/server/error.rs:24`).
Status `0` means success; a negative status is one of the errno constants below. Replies go to the kernel
reply endpoint `0x1_0000_0011` with `mk_ipc_send` (`src/server/error.rs:26`, `src/protocol/endpoint.rs:18`).
...
largest write and a single send holds the largest read (`src/server/runner.rs:30`). The loop polls the
MSI-X grant, receives a request, decodes it, and dispatches (`src/server/runner.rs:37`). A receive of zero
or fewer bytes is skipped, and a decode failure answers `E_INVAL` without touching the device
(`src/server/runner.rs:40`, `runner.rs:46`).
...
client sends any payload, before the op is handled (`src/server/runner.rs:56`). Each remaining op routes to
its handler, and an unrecognised opcode is answered with `E_INVAL` (`src/server/runner.rs:71`).

## The nine operations

The opcodes are defined in `src/protocol/ops.rs:17` and dispatched in `src/server/runner.rs:54`.
...
word alone through `reply_with_status` (`src/server/error.rs:23`). The six data-bearing ops build a longer
...
- `OP_CONTROLLER_INFO` reads the register block live (`ControllerInfo::read`, not the cached snapshot) and
  ...
  (`src/server/handlers/controller_info.rs:26`). The layout is fixed at 52 bytes
...
- `OP_IDENTIFY_CONTROLLER` returns fields parsed once at bring-up: vendor and subsystem vendor id, the
  ...
  (`src/server/handlers/identify_controller.rs:25`; parsed at `src/admin/identity.rs:35`). Fixed at 88
  bytes (`src/protocol/limits.rs:19`).
- `OP_IDENTIFY_NAMESPACE` returns the cached NSID-1 record: nsid, size, capacity, and used in LBAs, the LBA
  ...
  (`src/server/handlers/identify_namespace.rs:25`; parsed at `src/admin/namespace.rs:43`). Fixed at 36
  bytes (`src/protocol/limits.rs:20`).
- `OP_SMART_HEALTH` returns the cached SMART snapshot: the critical-warning byte, composite temperature in
  ...
  (`src/server/handlers/smart_health/handle.rs:29`; parsed at `src/admin/health/parse.rs:23`). Fixed at
  177 bytes (`src/protocol/limits.rs:21`).
- `OP_CAPACITY` returns the namespace sector count as a little-endian u64, or `E_NODEV` if no IO queue was
  brought up (`src/server/handlers/capacity.rs:27`). Fixed at 8 bytes (`src/protocol/limits.rs:24`).
- `OP_READ_BLOCKS` copies the fetched sectors out of the DMA data region into the reply after the status
  word (`src/server/handlers/read.rs:47`); `OP_WRITE_BLOCKS` copies the request's sector bytes into the DMA
  data region before submitting the write (`src/server/handlers/write.rs:36`).
...
(`src/server/handlers/rw_parse.rs:20`). Both data ops fail with `E_NODEV` first if no IO queue exists
(`src/server/handlers/read.rs:29`, `write.rs:24`). Read then requires the request payload to be exactly the
12-byte header (`src/server/handlers/read.rs:33`); write requires the payload and the received body both to
be the 12-byte header plus exactly `sectors * 512` bytes before it copies anything into the DMA buffer
(`src/server/handlers/write.rs:33`). A parse failure, an oversize count, an out-of-range LBA, or a wrong
...
(`src/server/handlers/read.rs:40`, `write.rs:43`, `flush.rs:27`).
...
rejects anything malformed with `E_INVAL` (`src/protocol/decode.rs:19`, `src/server/error.rs:29`). The
zero-payload guard means a query op cannot smuggle a body (`src/server/runner.rs:56`). The read and write
...
(`src/server/handlers/rw_parse.rs:20`). Write requires an exact payload length before it copies anything
into the DMA buffer (`src/server/handlers/write.rs:33`). There is no panic path: the crate is
