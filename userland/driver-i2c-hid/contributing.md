# Contributing to capsule_driver_i2c_hid
...
together, read the [README](README.md), the [protocol and discovery](protocol.md) page, and the
[report path](input.md) page.
...
exits with code 1 (`src/main.rs:32`). The module tree is declared there (`src/main.rs:23`). The
...
| `src/setup.rs`, `src/state.rs` | bring-up and the shared `State` | you add a field the loop or a handler needs |

Each `mod.rs` under this tree is re-exports only (`src/hid/mod.rs`, `src/protocol/mod.rs`,
`src/i2c_client/mod.rs`, `src/input/mod.rs`, `src/server/mod.rs`), matching the one-unit-per-file layout.

## Adding a server operation

There are four edits, and the dispatch wiring is the load-bearing one.

1. Add the constant in `src/protocol/ops.rs`, next to `OP_DESCRIPTOR`, and re-export it from
   `src/protocol/mod.rs:13`.
2. Write the handler as one file under `src/server/handlers/`, matching the shape of
   `src/server/handlers/descriptor.rs`: build the reply body and call `respond::send` with the status and
   the body. Re-export it from `src/server/handlers/mod.rs`.
3. Wire it into the dispatch match in `src/server/runner.rs:43`. Keep the `if body.is_empty()` guard so an
   op with a stray body still falls to the `E_INVAL` arm (`src/server/runner.rs:53`).
4. If the handler needs new state, add the field to `State` (`src/state.rs:1`) and initialise it in
   `State::new` (`src/state.rs:17`).
...
(`src/input/parse_report.rs:19`, `src/input/publish.rs:28`). A separate branch carries the absolute
...
1. Switch the device into absolute mode. HID-over-I2C sets a feature report to select the input mode. That
   ...
   (`src/i2c_client/transfer.rs:7`); no new capability is needed, since it is still an `OP_TRANSFER` to
   `driver.i2c_pci0`.
...
3. Extend the sample. `MouseSample` holds `buttons, dx, dy, wheel` (`src/input/sample.rs:17`); an absolute
...
4. Post absolute events. `publish` would post `INPUT_KIND_POINTER_ABS` with `x`/`y` set through the `post`
   wrapper (`src/input/post.rs:19`), which already accepts a full `InputEvent`. The `InputSource`
