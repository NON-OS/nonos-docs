...
[input.md](input.md); for identity and the capability mask see the [README](README.md).

## The server loop

`server::run` is the whole capsule after setup, a single loop that does two things per iteration
(`src/server/runner.rs:15`):

1. It receives with `mk_ipc_recv_from` on the service inbox with a 2 ms timeout
   (`src/server/runner.rs:20`, `src/server/runner.rs:13`).
2. Whether or not a request arrived, it calls `input::poll` (`src/server/runner.rs:27`). The report path
   therefore runs on every pass, paced by the receive timeout, not by an operation. See
   [input.md](input.md).
3. If the receive returned nothing, or the sender pid is zero, it loops (`src/server/runner.rs:28`).
4. Otherwise it parses the frame and dispatches (`src/server/runner.rs:31`).
...
declared `body_len` runs past the buffer (`src/protocol/decode.rs:3`). It returns the op and request id
...
body (`src/protocol/encode.rs:3`). Note the header's `body_len` on a reply counts the status word: it is
encoded as `4 + body.len()` (`src/protocol/encode.rs:12`), so a reader recovers the payload by subtracting
the four status bytes. Replies go out through `respond::send`, which formats the frame and calls
`mk_ipc_reply` to the sender pid (`src/server/respond.rs:4`). The payload cap is 96 bytes
(`src/protocol/limits.rs:1`), and the receive and transmit buffers are each sized `HDR_LEN +
IPC_PAYLOAD_MAX` (`src/server/runner.rs:16`).

## Dispatch and validation

`dispatch` matches the op, and each operation is accepted only when its request body is empty
(`src/server/runner.rs:43`):

- A recognised op (`OP_HEALTHCHECK`, `OP_PROBE`, `OP_DESCRIPTOR`) with an empty body runs its handler.
- A recognised or unrecognised op carrying a non-empty body draws `E_INVAL` (`src/server/runner.rs:53`).
- An unrecognised op with an empty body draws `E_BAD_OP` (`src/server/runner.rs:50`).

Ops come from `src/protocol/ops.rs` and error codes from `src/protocol/errno.rs`.

| Op | Value | Request | Reply | Handler |
|---|---|---|---|---|
| `OP_HEALTHCHECK` | 1 | empty | a 56-byte status body | `ops.rs:1`, `src/server/handlers/health.rs:5` |
| `OP_PROBE` | 2 | empty | re-runs the bus probe, replies with the found flag and address | `ops.rs:2`, `src/server/handlers/probe.rs:6` |
| `OP_DESCRIPTOR` | 3 | empty | the address and the cached descriptor bytes, or `E_NOT_FOUND` | `ops.rs:3`, `src/server/handlers/descriptor.rs:5` |

Error codes: `E_OK 0`, `E_NOT_FOUND -2`, `E_INVAL -22`, `E_BAD_OP -38` (`src/protocol/errno.rs:1`).

### OP_HEALTHCHECK

Always succeeds with `E_OK` and a 56-byte body (`src/server/handlers/health.rs:5`). The layout is fixed:
...
[debugging](debugging.md) page uses them to isolate report-path failures.

### OP_PROBE

Calls `reprobe`, which bumps the probe counter and re-runs the address scan, then replies with the found
flag and the selected address (`src/server/handlers/probe.rs:7`, `src/setup.rs:12`).

### OP_DESCRIPTOR

Returns `E_NOT_FOUND` when no descriptor has been read yet, that is when `found()` is false
(`src/server/handlers/descriptor.rs:6`). Otherwise it replies `E_OK` with the address, the descriptor
length, and the descriptor bytes trimmed to `descriptor_len` (`src/server/handlers/descriptor.rs:10`).
...
4. `wire::response` checks the reply for the right magic, a matching request id, and a zero status word
   ...
   and the reply size (`src/i2c_client/wire.rs:28`). Anything else returns `None`, and the transfer is
...
`setup::run` builds the `State` and calls `reprobe` once before entering the loop (`src/setup.rs:5`).
`reprobe` bumps the probe counter and calls `probe_bus`; on success it stores the address, the descriptor
length, and derives the two report registers (`src/setup.rs:12`).

`probe_bus` scans a fixed list of common HID-over-I2C slave addresses and, for each, writes the HID
descriptor register `0x0001` and reads back 30 bytes (`src/hid/probe.rs:6`, `src/hid/probe.rs:9`):

```
  CANDIDATE_ADDRS = 0x10, 0x15, 0x2C, 0x38, 0x4B, 0x4C, 0x20, 0x24
```

covering ELAN, Synaptics, and FocalTech ranges (`src/hid/probe.rs:4`). The first address whose 30 bytes
...
version field is in `0x0100..=0x0111` (`src/hid/descriptor.rs:3`). From the accepted descriptor the capsule
derives:

- the input register, read little-endian from descriptor offset 8..10 (`src/hid/input_register.rs:17`);
