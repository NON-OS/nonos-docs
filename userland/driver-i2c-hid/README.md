...
absolute coordinates (`src/input/parse_report.rs:19`, `src/input/publish.rs:28`).
...
failure exits with code 1 (`src/main.rs:32`). Two things then run in one loop: the capsule answers health,
...
|---|---|---|
| [protocol.md](protocol.md) | `src/protocol/`, `src/server/`, `src/i2c_client/`, `src/hid/` | The `NHID` server wire format, the three operations and their replies, the dispatch loop, the `NI2C` client that calls `driver.i2c_pci0`, and the descriptor discovery that arms the report path. |
| [input.md](input.md) | `src/input/` | The report path from an I2C poll through `parse_report` to `publish` and the `mk_input_event_post` syscall, the relative-pointer decode, the button diff, and the counters. |
| [contributing.md](contributing.md) | the whole tree | Where to work, how the absolute Precision Touchpad path would be added, the build and sign targets, and the code standards. |
| [debugging.md](debugging.md) | runtime | The boot marker, the health counters, and the failure modes when the touchpad is dead or no reports arrive. |
...
(`src/userspace/init/spawn_plan/drivers_bus.rs:52`, `spawn.rs:29`). Bring-up runs in `setup::run`:

1. Resolve the controller. It looks up `driver.i2c_pci0` in the registry through `mk_service_lookup`; a
   missing controller means startup fails closed and the process exits (`src/i2c_client/service.rs:3`,
   `src/main.rs:38`). This is how the driver finds the bus: not by ACPI enumeration in this capsule, but by
...
2. Probe for the device. `reprobe` scans a fixed list of common HID-over-I2C slave addresses and validates
   the first descriptor that parses (`src/setup.rs:12`, `src/hid/probe.rs:4`).
3. Derive the report registers from the accepted descriptor, which is what arms the poll path
   (`src/hid/input_register.rs:17`, `src/hid/input_len.rs:17`).

A successful spawn prints `[DRIVER-I2C-HID] capsule spawned` on the boot log through `boot_log::ok`
(`src/userspace/init/capsule_boot/run.rs:29`); the [debugging](debugging.md) page covers what an absent
...
- The report decode is a relative-pointer format (buttons, dx, dy, wheel), not the absolute multi-contact
  ...
  (`src/input/parse_report.rs:19`). It posts `INPUT_KIND_POINTER_REL`, never `INPUT_KIND_POINTER_ABS`
...
- No interrupt or doorbell pacing. The read cadence is the server loop's 2 ms receive timeout, not a
  hardware interrupt or GPIO doorbell (`src/server/runner.rs:13`, `src/server/runner.rs:27`). The capsule
