...
`nonos_toolkit` (`userland/toolkit/Cargo.toml:4`), that ships two things from the same source tree: a
`#![no_std]` library (`[lib] name = "nonos_toolkit"`, `userland/toolkit/Cargo.toml:12`, `src/lib.rs:17`)
that GUI capsules compile into their own binary, and a small `toolkit` service binary
(`[[bin]] name = "toolkit"`, `userland/toolkit/Cargo.toml:15`, `src/main.rs`) that answers a few
operations over IPC.
...
over four of them. `src/lib.rs:21` declares the library modules and `src/lib.rs:33` re-exports them.

| Page | Mirrors | What it covers |
|------|---------|----------------|
| [library.md](library.md) | `src/font/`, `src/design/`, `src/components/`, `src/decorations/`, `src/image/`, `src/qr/`, `src/animation/`, `src/theme/` | The linked library: the shared drawing interface, the 8x8 font, the design tokens, the widget helpers, window decorations, the image decoders, the QR renderer, the animation store, and the theme snapshot. |
| [service.md](service.md) | `src/main.rs`, `src/server/`, `src/protocol/`, `src/component_dispatch/` | The `toolkit` service binary: the port-4610 receive loop, the NOTK wire protocol, the five operations, and the honest finding that `COMPONENT_RENDER` cannot draw with mask `0x19`. |
| [contributing.md](contributing.md) | the whole tree | Where to work, how to add a widget, the build and sign steps for the `toolkit` slug, and the code standards. |
| [debugging.md](debugging.md) | runtime | The library failure modes that surface in the linking capsule, the service wire signatures, and the spawn marker. |

## The library modules

`src/lib.rs:21` declares eleven modules. `component_dispatch`, `protocol`, and `server` belong to the
service and are covered on the [service](service.md) page; the remaining eight are the linked library and
are covered on the [library](library.md) page.
...
`app_skeleton` (`userland/app_skeleton/Cargo.toml:16`), `capsule_boot_splash`
(`userland/capsule_boot_splash/Cargo.toml:13`), `capsule_desktop_shell`
(`userland/capsule_desktop_shell/Cargo.toml:17`), `capsule_image_codec`
(`userland/capsule_image_codec/Cargo.toml:16`), `capsule_setup_wizard`
(`userland/capsule_setup_wizard/Cargo.toml:13`), and `capsule_wallpaper`
(`userland/capsule_wallpaper/Cargo.toml:27`). `app_skeleton` calls `nonos_toolkit::font::render::draw_text`
(`userland/app_skeleton/src/paint/text.rs:17`) and `nonos_toolkit::decorations::{hit_test, DecorationHit}`
(`userland/app_skeleton/src/runner/decorations.rs:19`); `capsule_image_codec` calls
`nonos_toolkit::image::{bmp, jpeg, lz4_raw, png::decoder, types::DecodeError}`
(`userland/capsule_image_codec/src/server/handlers/decode.rs:18`). That is how the toolkit is used: as
...
already-initialized heap, and calls `server::runner::run` (`src/main.rs:26`), which registers on endpoint
4610 and loops: receive, decode the `NOTK` header, dispatch, encode the reply (`src/server/runner.rs:29`).
A successful spawn logs `[SPAWN] name=toolkit ... caps=0x19`; the [debugging](debugging.md) page covers
