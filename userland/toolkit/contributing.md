...
meet. For what the toolkit is and how it is put together, read the [README](README.md), the
[library](library.md), and the [service](service.md) pages in this folder.

## Where the source lives

The crate is at `userland/toolkit/`. It is one crate with two build products from the same tree: the
`#![no_std]` library `nonos_toolkit` (`userland/toolkit/Cargo.toml:12`, `src/lib.rs:17`) and the `toolkit`
service binary (`userland/toolkit/Cargo.toml:15`, `src/main.rs`). Almost all work is in the library. The
eight library modules are declared in `src/lib.rs:21` and re-exported in `src/lib.rs:33`. The dependencies
are only `alloc`, `nonos_userland_libc`, and `spin` (`userland/toolkit/Cargo.toml:19`).
...
| `src/server/`, `src/protocol/`, `src/component_dispatch/` | the service binary | you change the IPC surface (see the [service](service.md) page) |

## Adding a widget

A widget is one module under `src/components/`, next to the existing twenty (`src/components/mod.rs:1`).
There are two edits: the module itself and its declaration.

1. Write the widget as one file per widget under `src/components/`, for example `src/components/badge.rs`.
   ...
   (`src/components/button.rs:45`), taking `buf`, `stride`, `w`, `h`, `x`, `y`, the geometry, a label, and
   ...
   (`src/components/checkbox.rs:18`) or `ScrollState::clamp` returns a clamped state
   (`src/components/scroll.rs:9`). Give the style struct a `Default` so a capsule can take the defaults or
   override a field, matching `ButtonStyle` (`src/components/button.rs:10`). A drawing widget must clip to
   the caller-supplied `w`/`h` and bound every write against `buf.len()` with saturating arithmetic, the
   way `render_button`'s `fill_rect` does (`src/components/button.rs:27`); the library never writes past
   the buffer the caller handed in.

2. Declare it. Add the module to `src/components/mod.rs` (`src/components/mod.rs:1`), and if the widget or
   its style struct should be reachable as `nonos_toolkit::<name>`, add it to the `pub use components::{...}`
   re-export in `src/lib.rs:34`. A linking capsule then reaches it by path, the way `app_skeleton` reaches
   `nonos_toolkit::font::render::draw_text` (`userland/app_skeleton/src/paint/text.rs:17`). No IPC and no
...
(`src/component_dispatch/kind.rs:24`) and a match arm in `paint` (`src/component_dispatch/paint/paint.rs:39`).
Note that with mask `0x19` the service's `COMPONENT_RENDER` always returns `E_SURFACE` and never reaches
`paint` (see the [service](service.md) page), so a new kind is only exercised if the service is later
...
- No panics in capsule or library code. No `unwrap`, `expect`, or `panic!`. A drawing helper drops or
  clips out-of-range pixels rather than indexing past the slice (`src/font/render/draw_glyph.rs:41`), and a
  decoder returns a `DecodeError` rather than panicking on bad input (`src/image/types.rs:1`).
- One unit per file. New widgets are one module per widget under `src/components/`, and `mod.rs` is used
  only for module declarations and re-exports, matching the existing tree (`src/components/mod.rs:1`,
  `src/lib.rs:33`).
- The AGPL header sits at the top of every source file, byte for byte the same as the header on
  `src/lib.rs:1` and every other module.
