...
The [std PAL](std-pal.md) runs unmodified Rust programs by grafting real `std`. The other option, for a
...
`nonos_std` (`userland/sdk/nonos_std/src/lib.rs`) is `#![no_std]` and layers a std-shaped surface over
`core` and `alloc`:
...
capsules the [std PAL](std-pal.md) uses, so the two share a backend and differ only in whether the
...
familiar shape, `connect`, `bind`, `send`, `recv`, over the [net.sockets](networking-guide.md) service.
It is the native-capsule route to the same networking the std PAL exposes; the
[networking guide](networking-guide.md) covers using it.
...
```
  nonos_std      a capsule written for NONOS; builds core+alloc; use nonos_std::... explicitly
...
and the `no_std` build is lighter. Use the [std PAL](std-pal.md) when porting an existing crate that
