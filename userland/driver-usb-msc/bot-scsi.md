# Bulk-Only Transport and SCSI command construction
...
turns the 13 bytes that come back into a validated status. The [operations page](operations.md) covers how
these get invoked and how the results are counted; the [overview](README.md) covers the capsule as a
whole.
...
`src/bot/cbw.rs` defines the `CommandBlockWrapper` struct and its serializer. The struct carries the tag,
the expected data-transfer length, a direction flag, the LUN, the CDB length, and 16 bytes of CDB
(`src/bot/cbw.rs:23`). `write` lays out the standard 31-byte layout, all little-endian on the multi-byte
fields:
...
(`src/bot/cbw.rs:20`, `:21`). The signature and data length are written with `to_le_bytes`, so the CBW
framing is little-endian even though the CDB it carries is not; that split is called out below.

### The Command Status Wrapper

`src/bot/csw.rs` parses the 13-byte reply. It requires an exact length of 13, the signature `USBS`
(`0x53425355`), and a status byte no greater than 2 (`src/bot/csw.rs:19`, `:29`, `:33`, `:39`). A wrong
length or a bad signature returns `E_INVAL`; a status byte above 2 returns `E_PHASE`
(`src/bot/csw.rs:30`, `:34`, `:40`). On a good parse it returns the echoed tag, the residue, and the
status byte (`src/bot/csw.rs:36`, `:37`, `:38`, `:42`). The parser only validates and decodes; the
accounting of tag mismatch and residue lives in `src/state/ops.rs` and is described on the
[operations page](operations.md).

## The SCSI command blocks

Four CDBs are built, each a small pure function in `src/scsi/cdb.rs` that returns a 16-byte buffer and the
active CDB length.

- INQUIRY: opcode `0x12` at byte 0, allocation length 36 at byte 4, CDB length 6
  (`src/scsi/cdb.rs:17`, `:19`, `:20`, `:21`).
- READ CAPACITY(10): opcode `0x25` at byte 0, CDB length 10 (`src/scsi/cdb.rs:24`, `:26`, `:27`).
- READ(10): opcode `0x28`, CDB length 10, built through the shared block-CDB helper
  (`src/scsi/cdb.rs:30`, `:31`).
- WRITE(10): opcode `0x2A`, CDB length 10, same helper (`src/scsi/cdb.rs:34`, `:35`).
...
(`src/scsi/cdb.rs:38`, `:40`, `:41`, `:42`, `:43`).
...
for a count over the bound (`src/scsi/validate.rs:20`, `:25`, `:28`, `:29`, `src/protocol/limits.rs:23`).
The build handlers call it first and bail with the guard's errno on failure
(`src/server/handlers/build_read.rs:24`, `:25`, `src/server/handlers/build_write.rs:24`, `:25`).

The `data_len` written into the CBW is `blocks * BLOCK_BYTES`, and `BLOCK_BYTES` is a fixed 512
(`src/server/handlers/build_read.rs:29`, `src/protocol/limits.rs:22`). The block size is a constant, not
...
(`src/server/handlers/build_inquiry.rs:24`, `:28`, `src/server/handlers/build_capacity.rs:24`, `:28`).
READ(10) is an IN transfer and WRITE(10) an OUT transfer, each after the 6-byte guard, each with
`data_len = blocks * 512` (`src/server/handlers/build_read.rs:28`, `:29`, `:33`,
`src/server/handlers/build_write.rs:28`, `:29`, `:33`).

Every build handler hard-codes `lun: 0` (`src/server/handlers/build_inquiry.rs:29`,
`src/server/handlers/build_capacity.rs:29`, `src/server/handlers/build_read.rs:34`,
`src/server/handlers/build_write.rs:34`). There is no multi-LUN support: a device that exposes more than
