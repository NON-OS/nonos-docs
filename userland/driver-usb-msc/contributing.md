# Contributing to capsule_driver_usb_msc
...
does and how it fits together, read the [overview](README.md), the [operations reference](operations.md),
and the [BOT and SCSI page](bot-scsi.md).
...
(`userland/capsule_driver_usb_msc/src/main.rs:32`, `:36`). The module trees are declared there
(`src/main.rs:22`).
...
2. Write the handler as one file under `src/server/handlers/`, following an existing one. A handler takes
   ...
   (`src/server/respond.rs:21`, `:27`). A build-style handler writes its payload into
   `tx[HDR_LEN + STATUS_LEN..]`, the way `build_inquiry` does (`src/server/handlers/build_inquiry.rs:33`).
   Register the module in `src/server/handlers/mod.rs:17`.
3. Wire the opcode into the match in `src/server/dispatch.rs:22`. If the op takes no body, add the
   ...
   `E_INVAL` catch-all rather than your arm (`src/server/dispatch.rs:23`, `:35`, `:38`).
4. Keep the capability boundary. Do not import any kernel driver, memory, paging, phys, or hardware path,
   ...
   (`nonos-ci/run-static-checks.sh:1394`, `:1403`).
...
- A transport caller. Something has to hold the xHCI service handle, take the CBW this capsule returns,
  ...
  [claim.md](../../subsystems/hardware-broker/claim.md),
  [mmio.md](../../subsystems/hardware-broker/mmio.md), [dma.md](../../subsystems/hardware-broker/dma.md),
  and [irq.md](../../subsystems/hardware-broker/irq.md).
- Error recovery. `OP_ACCEPT_CSW` counts a phase error and a tag mismatch but takes no action
  (`src/state/ops.rs:37`, `:44`). A real path needs a BOT reset recovery (clear-feature on both bulk
...
- Multi-LUN. Every CBW is built with `lun: 0` hard-coded
  (`src/server/handlers/build_inquiry.rs:29`, `src/server/handlers/build_write.rs:34`). Addressing a
...
(`nonos-ci/run-static-checks.sh:1394`, `:1403`, `:1412`).
...
- No panics in capsule code. No `unwrap`, `expect`, or `panic!`. Every handler returns an errno as a status
  ...
  (`userland/capsule_driver_usb_msc/Cargo.toml:25`).
- One unit per file. New handlers are one op per file under `src/server/handlers/`, and `mod.rs` is used
  only for re-exports, matching the existing tree (`src/protocol/mod.rs`, `src/server/mod.rs`).
