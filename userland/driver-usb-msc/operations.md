# USB MSC operations and the request loop
...
[BOT and SCSI page](bot-scsi.md); for the capsule as a whole and its identity see the
[overview](README.md).

## The server loop

The capsule is a single-threaded request server. `_start` initializes the heap and calls `server::run`,
which never returns (`src/main.rs:32`, `:36`). `run` allocates one receive buffer and one transmit buffer,
each `HDR_LEN + IPC_PAYLOAD_MAX` bytes (20 + 1024), builds a fresh `State`, and enters the loop
(`src/server/runner.rs:28`, `:29`, `:30`, `src/protocol/limits.rs:17`). Every request reuses those two
...
(`src/server/runner.rs:37`, `:38`, `:41`, `:42`). Replies go back to the captured sender pid through
`mk_ipc_reply` (`src/server/respond.rs:24`, `:31`). The capsule makes no outbound IPC calls of its own; it
...
pad, a `u32` request id, and a `u32` payload length (`src/protocol/header.rs:17`, `:18`, `:19`,
`src/protocol/decode.rs:23`, `:28`, `:29`, `:30`, `:31`).
...
received length exactly (`src/protocol/decode.rs:20`, `:25`, `:32`, `:33`). A message that does not parse
is dropped silently by the loop, not answered (`src/server/runner.rs:41`).

Every reply begins with the same header echoed back: the request's op, flags, and request id are copied,
the pad is zeroed, and the reply payload length is written (`src/protocol/encode.rs:19`). After the header
comes a 4-byte little-endian signed status word, then any op-specific payload
(`src/protocol/encode.rs:29`, `src/server/respond.rs:22`, `:29`). Status `0` means success; a negative
status is one of the errno constants below.

## The eight opcodes

The opcode constants live in `src/protocol/ops.rs:17`, and the dispatcher matches on the 16-bit opcode in
`src/server/dispatch.rs:22`.
...
| `OP_PROBE_CONFIG` | `0x0002` | raw USB configuration descriptor | binding count then 8-byte records | `src/server/handlers/probe_config.rs:22` |
| `OP_BUILD_INQUIRY` | `0x0003` | empty | 31-byte BOT CBW for INQUIRY | `src/server/handlers/build_inquiry.rs:23` |
...
`body.is_empty()` arm (`src/server/dispatch.rs:23`, `:25`, `:28`, `:34`). An unknown opcode with an empty
body falls to the `E_BAD_OP` arm, but an unknown opcode carrying a non-empty body falls to a final
catch-all that answers `E_INVAL`, not `E_BAD_OP` (`src/server/dispatch.rs:35`, `:38`). So "unknown op" is
...
the reply (`src/server/handlers/probe_config.rs:23`, `:25`, `:26`).

The parser under `src/descriptors/parse.rs` requires at least 9 bytes with a configuration descriptor type
in `raw[1]`, reads the `wTotalLength` field, and refuses a total under 9 bytes or longer than the buffer
(`src/descriptors/parse.rs:23`, `:26`, `:27`). It then walks fixed-length records, rejecting any record
whose length is under 2 or that runs past the total (`src/descriptors/parse.rs:33`, `:35`).

The walk hands each record to a visitor (`src/descriptors/visitor.rs:21`). An interface record sets the
...
(`src/descriptors/visitor.rs:34`, `:37`, `src/descriptors/wire.rs:20`, `:21`, `:22`). A bulk endpoint
...
(`src/descriptors/visitor.rs:42`, `:47`, `:54`, `src/protocol/limits.rs:19`). A descriptor with zero
bindings answers `E_NO_MSC`; a malformed one answers `E_INVAL` without mutating state
(`src/descriptors/parse.rs:41`, `:43`). The reply is a 32-bit binding count followed by 8-byte records of
`interface, bulk_in, bulk_out, pad, max_packet_in_le16, max_packet_out_le16`
(`src/descriptors/encode.rs:19`, `:23`).
...
parse lives on the [BOT and SCSI page](bot-scsi.md)), folds it into the counters, and returns the CSW's own
status byte as the reply status (`src/server/handlers/accept_csw.rs:23`, `:25`, `:26`). A parse failure
returns `E_INVAL` or `E_PHASE` instead (`src/server/handlers/accept_csw.rs:29`).

The fold is the accounting hook a recovery layer would read (`src/state/ops.rs:36`). A tag that does not
...
(`src/state/ops.rs:37`, `:40`, `:42`, `:43`, `:44`). The capsule does not act on any of this: it counts,
...
counters (`src/state/types.rs:20`). It stores no product strings, serial numbers, raw descriptors, SCSI
...
that skips zero, and records the issued value as `last_tag` (`src/state/ops.rs:29`, `:31`, `:32`). The
starting `next_tag` is 1 (`src/state/types.rs:37`). `install_bindings` replaces the snapshot and bumps the
probe count (`src/state/ops.rs:23`).
...
u32 (`src/state/snapshot.rs:20`, `:21`, `:25`, `:26`, `:27`). That is the introspection surface a caller
...
[BOT and SCSI page](bot-scsi.md) shows the hard-coded LUN in each build handler). The operations layer
