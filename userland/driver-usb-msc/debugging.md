# Debugging capsule_driver_usb_msc
...
[overview](README.md), the [operations reference](operations.md), and the [BOT and SCSI page](bot-scsi.md).
...
reason mapper (`src/userspace/init/capsule_boot/error.rs:21`). Because the plan runs
`spawn_xhci` first, a missing xHCI marker upstream is worth checking too
(`src/userspace/init/spawn_plan/drivers_usb.rs:18`).
...
(`src/descriptors/visitor.rs:34`, `:47`, `src/descriptors/parse.rs:41`). The endpoint snapshot is left
...
(`src/descriptors/parse.rs:23`, `:27`, `:35`). The endpoint snapshot is left untouched. Note this errno
...
(`src/scsi/validate.rs:20`, `:25`, `:28`, `src/protocol/limits.rs:23`). The request body carries the LBA
and count little-endian; if the caller sent them big-endian the count will read as a huge value and trip
`E_OVERFLOW` (`src/scsi/validate.rs:23`).

### A CSW answers `E_INVAL` or `E_PHASE`

`E_INVAL` is a wrong length or a bad `USBS` signature; `E_PHASE` is a status byte above 2
(`src/bot/csw.rs:29`, `:33`, `:39`). A tag that does not match the last issued tag does not error: it
silently bumps the phase-error counter, which you read back with `OP_GET_STATE`
(`src/state/ops.rs:37`, `src/state/snapshot.rs:24`). So a caller that reuses or reorders tags will see
...
(`src/server/dispatch.rs:35`, `:38`). If you are getting `E_INVAL` for an op you expect to exist, check
...
(`src/state/snapshot.rs:20`). It is how you separate the two big classes of failure without any serial
output:
