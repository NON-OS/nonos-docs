...
itself, see the [networking subsystem](../subsystems/networking/README.md); this page is about using it.

## The familiar API

A capsule uses the standard socket types, whether through the [std PAL](std-pal.md) (`std::net`) or the
[nonos_std crate](nonos-std.md) (`nonos_std::net`). The code looks like ordinary Rust:
...
Each socket call becomes an IPC request to the `net.sockets` capsule (`net/connection/nonos.rs` in the
PAL, `net/` in nonos_std). The backend speaks the [sockets op set](../subsystems/networking/sockets.md):
...
The `net.sockets` capsule runs those against the [smoltcp stack](../subsystems/networking/stack.md) in
`net_core`, which in turn exchanges frames with the [NIC driver capsule](../subsystems/networking/drivers.md).
...
[manifest](../security/capsules-and-trust.md) asked for and was granted the capability, and the kernel
...
[std PAL net backend](std-pal.md) and the IPv4 stack):
