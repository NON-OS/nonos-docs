# Debugging capsule_payment

This page covers how to tell whether the capsule is even reachable, and the concrete request-time failure
signatures with where to look for each. For the operation layout see the [operations](operations.md) page,
for the keyring path see the [signing](signing.md) page, and for the design see the [README](README.md).
...
- The keyring port cannot be resolved on a `pay`, so the receipt cannot be signed
  (`src/server/handlers/pay.rs:45`, `src/server/discover.rs:26`). This is the usual signature when the
...
- The keyring reply is short, which the sign helper reports as `-11`
  (`src/server/sign_call.rs:47`).
- The outbox is full at its 1024-record cap, so `push_receipt` refuses the new record
  (`pay.rs:66`, `src/store/outbox.rs:23`). Drain the outbox with `OP_DRAIN_RECEIPTS` to clear it.
...
to sign for (`userland/capsule_keyring/src/server/handlers/sign_receipt/sign_receipt.rs:35`,
...
- A `drain` always returns `status = 0`; a count of zero in its payload means the outbox was empty
  (`src/server/handlers/drain.rs:26`). Otherwise the payload is `count(4 LE)` then that many 297-byte
  records, at most 13 per call.
- A `list_tokens` always returns `status = 0` with the three-entry registry
  (`src/server/handlers/tokens.rs:22`).
- No reply at all means the frame was shorter than the eight-byte header or otherwise undecodable, and the
  runner skipped it with `continue` (`src/server/runner.rs:32`, `src/protocol/decode.rs:20`).
