# Contributing to capsule_payment
...
[README](README.md), the [operations](operations.md) page, and the [signing](signing.md) page in this
folder.

## Where the source lives

The capsule is at `userland/capsule_payment/`. It is a `no_std`/`no_main` service: `_start` initializes
the heap and enters `server::run`, which never returns (`src/main.rs:29`). The three top-level modules are
...
`expiry`) plus `discover` and `sign_call` sit directly under `server/` (`src/server/mod.rs:17`).
...
1. Add the opcode constant in `src/protocol/types.rs:17`, next to the existing `OP_*` values, and
   re-export it from `src/protocol/mod.rs:23`.
2. Write the handler as one file under `src/server/handlers/`, exposing a `pub fn` that takes the
   ...
   the way `pay.rs` and `drain.rs` do. Re-export it from `src/server/handlers/mod.rs:22`.
3. Wire the opcode into the match in `src/server/dispatch.rs:26`.

Keep the receipt marshaling helpers as the single source of the on-wire layout. If an operation touches
the receipt fields, the keyring request in `src/server/sign_call.rs`, the drained record in
`src/server/record.rs`, and the `ReceiptInput` in `src/server/fields.rs` must stay in lockstep with the
field order; the [operations](operations.md) and [signing](signing.md) pages spell out the byte offsets
those three share.
...
because it is not part of any desktop profile and is not spawned at boot; the [README](README.md) states
what that means.
...
- One unit per file. New operations are one handler per file under `src/server/handlers/`, and `mod.rs` is
  used only for re-exports, matching `src/server/mod.rs` and `src/store/mod.rs`.
