...
This page mirrors `src/server/sign_call.rs`, `src/server/discover.rs`, `src/server/fields.rs`, and the
...
[operations.md](operations.md); for the keyring on the other end, see the [keyring](../keyring/README.md).
...
lookup result (`src/server/discover.rs:21`, `KEYRING_SERVICE = b"keyring"` at `src/server/consts.rs:20`).
A `None` here is what the `pay` handler surfaces to its caller as `EAGAIN`
(`src/server/handlers/pay.rs:45`). This is the only service discovery the capsule does, and it happens on
every `pay`, so a keyring that is not yet up is a transient failure rather than a hard one.

## Marshaling the receipt fields

`pay` builds a `ReceiptInput` of seven 32-byte-and-under fields (`src/server/fields.rs:17`): `capsule_id`,
...
- `u64_word` places a `u64` in the low 8 bytes of a 32-byte word, high bytes zero
  (`src/server/u64_word.rs:17`). This is how `nonce`, `epoch`, and `expiry` become 32-byte words.
- `word32` copies a 32-byte field verbatim from the request payload (`src/server/word32.rs:17`). This
...
- `addr20` copies the 20-byte `publisher` address (`src/server/addr20.rs:17`).
...
`KEYRING_OP_SIGN_RECEIPT = 11` (`sign_call.rs:31`, the constant at `src/server/consts.rs:21`). That opcode
matches the keyring's `OP_SIGN_NOX_RECEIPT = 11` (`userland/capsule_keyring/src/protocol/types.rs:27`),
...
(`src/server/handlers/pay.rs:63`), the `struct_hash` becomes the successful reply payload
(`pay.rs:68`), and the `user`, fields, and `signature` are assembled into the 297-byte outbox record by
`build_record` (`src/server/record.rs:21`).
...
(`userland/capsule_keyring/src/server/handlers/sign_receipt/sign_receipt.rs:60`). The payment capsule has
...
(`userland/capsule_keyring/src/server/handlers/sign_receipt/sign_receipt.rs:35`), then fetches the secret
...
capsule not being spawned at boot, is called out on the [README](README.md) and the
