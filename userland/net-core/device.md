...
This page mirrors `src/device/` and `src/setup.rs`. It documents how `net_core` finds a NIC driver capsule,
...
built on top of this device, read the [iface](iface.md) page; for the `NNET` wire format used here, read
the [protocol](protocol.md) page.

## Discovery and bring-up

`setup::run` is the whole bring-up (`src/setup.rs:46`). It discovers a NIC driver by service name, brings
the link up, reads the MAC, builds the smoltcp state, and stores it. If any step fails it returns a
`SetupError` variant (`src/setup.rs:27`), and `main` decides whether to retry or exit on it
([README](README.md), `src/main.rs:45`).
...
(`src/setup.rs:23`, `src/setup.rs:35`). No match returns `SetupError::NicNotFound`, which `main` treats as
retryable so the stack waits for the driver to come up rather than dying (`src/setup.rs:47`, `src/main.rs:49`).
...
- `link_up` sends `OP_LINK_STATUS` and reads a 5-byte body (a 4-byte status word and a 1-byte up flag),
  returning `Some(true)` only when the status is zero and the flag is non-zero (`src/device/link_up.rs:23`).
...
- `mac` sends `OP_MAC_ADDRESS` and reads a 10-byte body (a 4-byte status and the 6-byte MAC), returning the
  MAC only on a zero status (`src/device/mac.rs:23`). A failure returns `SetupError::MacFailed`
  (`src/setup.rs:53`).

Each request carries a fresh request id from a monotonic counter that never reuses zero
(`src/device/seq.rs:21`), so a reply can be matched to its call.

## The phy::Device bridge

`NicDevice` is the smoltcp device the interface drives, and it holds nothing but the driver's IPC port
(`src/device/types.rs:19`). The `Device` impl is the bridge (`src/device/device_impl.rs:23`):

- `receive` polls one frame from the driver and, if one arrives, hands back an rx token holding that frame
  and a tx token (`src/device/device_impl.rs:27`). Returning `None` when no frame is ready is how smoltcp
  learns the receive queue is empty.
- `transmit` always returns a tx token; the frame is not built until smoltcp fills the token
  (`src/device/device_impl.rs:32`).
- `capabilities` reports a 1514-byte MTU over an Ethernet medium (`src/device/device_impl.rs:36`), matching
  the maximum frame the receive path is sized for (`src/device/rx.rs:26`).

## The receive path

`rx::poll_frame` sends `OP_RX_PACKET` and reads a reply sized for a full frame:
`HDR_LEN + 4 + 4 + 12 + 1514` bytes (`src/device/rx.rs:27`). It validates the op, a zero status, and a
...
(`src/device/rx.rs:44`). Any shortfall or non-zero status returns `None`, which smoltcp reads as no frame
available. The rx token simply hands smoltcp a mutable view of that owned frame
(`src/device/rx_token.rs:21`).

## The transmit path

When smoltcp consumes a tx token it asks for a buffer of a given length, fills it with the outbound frame,
and the token forwards it to the driver (`src/device/tx_token.rs:23`). `tx::send_frame` prepends the 20-byte
`NNET` header with `OP_TX_PACKET` and the frame length as the payload length, copies the frame after the
header, sends it, and checks the reply's op and a non-negative status (`src/device/tx.rs:25`). The send
result is not surfaced back into smoltcp's return value; the token returns whatever the fill closure returned
(`src/device/tx_token.rs:28`), which is smoltcp's contract for a fire-and-forget transmit.
...
not ([README](README.md)). `net_core` holds no `DeviceEnum`, `Driver`, `Mmio`, `Irq`, or `Dma` bit; its
