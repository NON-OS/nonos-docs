...
[README](README.md), the [protocol](protocol.md) page, the [server](server.md) page, the
[device](device.md) page, and the [iface](iface.md) page.
...
(`src/main.rs:36`). The top-level modules are declared there (`src/main.rs:22`).
...
| `src/setup.rs` | NIC discovery, link-up, MAC, and building the state | you change the candidate list or the bring-up |
| `src/iface/` | building the smoltcp `Interface`, the poll loop, DHCP, and DNS install | you change address acquisition or the pump |
| `src/state.rs` | `NetState`, the lease, and the locked accessors | you add shared state or an accessor |
| `src/handles.rs`, `src/udp_ports.rs` | the per-client TCP and UDP socket tables | you change socket ownership or capacity |

## Adding a service op

There are three edits, and the dispatch wiring is the load-bearing one. Take a new TCP op as the example.

1. Add the opcode constant to the protocol module, next to the existing ops (`src/protocol/tcp.rs:19`), and
   add any new errno it needs to the same file (`src/protocol/tcp.rs:27`).
2. Write the handler as one file under `src/server/handlers/tcp/`, exposing a `handle(sender_pid, req, body,
   ...
   by calling `reply` (`src/server/respond.rs:21`). `send.rs` and `recv.rs` are the reference shapes
   (`src/server/handlers/tcp/send.rs:25`, `src/server/handlers/tcp/recv.rs:25`). Declare the module in the
   protocol's `mod.rs` (`src/server/handlers/tcp/mod.rs:17`).
3. Wire it into that protocol's dispatch match (`src/server/handlers/tcp/mod.rs:29`). The `_` arm already
...
- No panics in capsule code. No `unwrap`, `expect`, or `panic!`. Every setup path returns a `SetupError`
  (`src/setup.rs:27`) and every request path returns an errno word through `reply`; the release profile is
...
- Every socket op must be scoped to its caller. A TCP op resolves its handle through `handles::get`, which
  checks the owner pid (`src/handles.rs:35`); a UDP op is keyed on the caller pid and local port through
  `udp_ports::get` (`src/udp_ports.rs:43`). A handler that skips that check is a boundary bug.
