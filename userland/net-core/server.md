...
[protocol](protocol.md) page lists. For the wire formats those handlers decode, read the
[protocol](protocol.md) page; for the state tables they read and write, read the [iface](iface.md) page.
...
`parse` validates the 20-byte header before any handler sees the request (`src/server/parse_req.rs:29`). It
...
Every handler ends by calling `reply` (`src/server/respond.rs:21`). It writes the 20-byte header into the
...
`mk_ipc_reply` to the sender pid (`src/server/respond.rs:30`). Echoing the magic and op back lets a client
...
it (`src/handles.rs:35`, [iface](iface.md)). A handle the caller does not own answers `E_NO_SOCKET`.
...
- `connect` allocates an ephemeral local port from 49152 up, builds a smoltcp `tcp::Socket` with 8 KiB rx
  ...
  (`src/server/handlers/tcp/connect.rs:43`). A connect error answers `E_NOT_CONNECTED`; a full socket table
  answers `E_NO_SOCKET` and removes the socket it just added (`src/server/handlers/tcp/connect.rs:78`).
- `send` copies the payload after the handle into the socket's send buffer and returns the byte count smoltcp
  accepted (`src/server/handlers/tcp/send.rs:41`); a failure answers `E_NOT_CONNECTED`.
- `recv` drains up to `IPC_BUF_MAX` bytes from the socket and returns them, or `E_RX_EMPTY` when nothing is
  buffered (`src/server/handlers/tcp/recv.rs:40`).
- `close` calls `close` on the socket and frees the app handle from the table (`src/server/handlers/tcp/close.rs:40`).
- `state` reads the smoltcp connection state and returns the 1-byte code the [protocol](protocol.md) page
  lists (`src/server/handlers/tcp/state.rs:40`).

## The UDP handlers

UDP sockets are keyed on the caller's pid and the local port, not on an opaque handle, through the
`udp_ports` table (`src/udp_ports.rs:43`, [iface](iface.md)).
...
- `bind` builds a smoltcp `udp::Socket` with 16-slot, 4 KiB packet buffers, binds the requested local port,
  ...
  (`src/server/handlers/udp/bind.rs:32`).
- `send` looks up the socket by local port, then sends the payload to the destination IPv4 and port carried
  in the request (`src/server/handlers/udp/send.rs:26`); a send error answers `E_NOT_CONNECTED`.
- `recv` returns one datagram prefixed with the 4-byte source IPv4 and 2-byte source port, clamped to
  `IPC_BUF_MAX - 6` bytes, or `E_RX_EMPTY` when none is queued (`src/server/handlers/udp/recv.rs:26`).
- `unbind` removes the socket from the smoltcp set and the port table (`src/server/handlers/udp/unbind.rs:23`).

## The DNS handler

`resolve_a` reads the hostname as UTF-8, rejecting empty or non-UTF-8 with `E_NAME_INVALID`
(`src/server/handlers/dns/resolve_a.rs:30`). It starts a smoltcp DNS A query against the socket installed
when the lease arrived, answering `E_NO_LEASE` if there is no DNS socket yet
(`src/server/handlers/dns/resolve_a.rs:34`). It then loops, pumping the stack and polling for the answer,
until the query resolves or a 3000 ms deadline passes, cancelling the query and answering `E_SERVFAIL` on
timeout (`src/server/handlers/dns/resolve_a.rs:41`). On success it replies with the first IPv4 address
(`src/server/handlers/dns/resolve_a.rs:59`). This is the one handler that blocks the loop while it waits,
...
state and replying `E_OK` (`src/server/handlers/dhcp_status.rs:49`). `encode_body` is also called from the
DHCP event path as a self-check marker (`src/server/handlers/dhcp_status.rs:32`, [iface](iface.md)).
`health::handle` replies `E_OK` with no body and the request's own magic and op
(`src/server/handlers/health.rs:23`).
