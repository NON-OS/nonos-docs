...
for each. For the shape of the capsule read the [README](README.md), the [protocol](protocol.md) page, the
[server](server.md) page, the [device](device.md) page, and the [iface](iface.md) page.
...
(`src/userspace/init/spawn_plan/network/spawn_core.rs:21`), which delegates to `capsule_boot::boot`
(`src/userspace/init/spawn_plan/boot.rs:26`), whose `Ok` arm calls `boot_log::ok(prefix, "capsule spawned")`
(`src/userspace/init/capsule_boot/run.rs:29`), formatted as `[` + tag + `] ` + message
...
is unbound (`src/server/handlers/dhcp_status.rs:35`).
...
`driver.rtl8139_0`, and returns `NicNotFound` if the registry knows none of them (`src/setup.rs:23`,
`src/setup.rs:35`). `NicNotFound` is retryable: `wait_for_setup` yields and loops rather than exiting, so the
stack waits for the driver capsule to register (`src/main.rs:49`). If no driver is expected to be present,
...
(`src/device/link_up.rs:23`, `src/setup.rs:50`). Like `NicNotFound`, `LinkDown` is retryable and the capsule
keeps polling (`src/main.rs:49`). A malformed or short reply instead returns `LinkFailed`
(`src/setup.rs:51`), and reading the MAC failing returns `MacFailed` (`src/setup.rs:53`); both of those are
hard errors that exit the process with code 2 (`src/main.rs:54`). So a capsule that spawned but is not
...
DNS socket, so `net.dns` returns `E_NO_LEASE` (`src/server/handlers/dns/resolve_a.rs:39`,
[protocol](protocol.md)), and `net.dhcp.client` reports state `1` (unbound)
(`src/server/handlers/dhcp_status.rs:43`).
...
(`src/device/rx.rs:44`); smoltcp reads `None` as an empty receive queue, so a driver that answers
`OP_RX_PACKET` with a malformed body looks exactly like no traffic. The transmit side is `tx::send_frame`,
whose result is intentionally not surfaced back into smoltcp (`src/device/tx_token.rs:28`,
[device](device.md)), so a failing transmit is silent at the stack layer. When a lease never arrives and the
...
the app handle or the pid+port key did not resolve to a socket the caller owns (`src/handles.rs:35`,
`src/udp_ports.rs:43`); `E_NOT_CONNECTED` means smoltcp rejected a send on a socket that is not established
(`src/server/handlers/tcp/send.rs:52`, `src/server/handlers/udp/send.rs:51`); `E_RX_EMPTY` means nothing was
buffered to receive (`src/server/handlers/tcp/recv.rs:51`, `src/server/handlers/udp/recv.rs:61`); and
`E_SERVFAIL` from `net.dns` is a query timeout or empty answer within the 3000 ms deadline
(`src/server/handlers/dns/resolve_a.rs:50`). The full errno sets are on the [protocol](protocol.md) page.
