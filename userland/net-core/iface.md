...
This page mirrors `src/iface/`, `src/state.rs`, `src/handles.rs`, and `src/udp_ports.rs`. It documents how
the smoltcp `Interface` is built over the [device](device.md) bridge, the poll loop that advances it, the
DHCPv4 client that acquires an address and installs the DNS socket, and the shared state and per-client
socket tables every [server](server.md) handler works against. For the ops that drive these sockets, read
the [server](server.md) page.

## Building the interface

`build::build` constructs the smoltcp state from the MAC and the driver port
(`src/iface/build.rs:26`). It draws an 8-byte random seed through `crypto_random`, which is the one use of
the capsule's `Crypto` capability, and fails the build if the draw is short
(`src/iface/build.rs:27`, [README](README.md)). It configures a smoltcp `Config` with the Ethernet address
and that seed, builds the `Interface` at the current millisecond clock over a fresh `NicDevice`, creates an
empty `SocketSet`, and adds the DHCPv4 socket (`src/iface/build.rs:33`). The result is a `NetState` with the
interface, the socket set, the device, the DHCP handle, and an empty DNS handle slot
(`src/iface/build.rs:43`). `setup::run` stores it into the global (`src/setup.rs:54`).

## The shared state

`NetState` bundles the four things that must be advanced together: the `Interface`, the `SocketSet`, the
`NicDevice`, the DHCP socket handle, and the optional DNS socket handle (`src/state.rs:21`). It lives behind
one `spin::Mutex` in a `static`, `NET` (`src/state.rs:34`). Because the capsule has a single execution
context and this mutex is the only access gate, `NetState` is marked `Send` with that reasoning stated at
the `unsafe impl` (`src/state.rs:29`).
...
(`src/state.rs:67`). The DHCP lease is a separate `Lease` behind its own mutex, read through `lease` and
written through `set_lease` (`src/state.rs:36`).

## The poll loop

`poll::pump` is the single advance step, called at the top of every server iteration and again while the DNS
handler waits (`src/iface/poll.rs:23`, [server](server.md)). It locks the state, polls the smoltcp interface
at the current clock so smoltcp exchanges frames with the device and services every socket, then processes
one DHCP event (`src/iface/poll.rs:24`). The interface poll is where the [device](device.md) bridge's
...
- The TCP table `handles` is a 32-slot array of `(owner_pid, SocketHandle)` behind a mutex
  (`src/handles.rs:20`). `alloc` returns the first free index as the opaque app handle, `get` returns the
  smoltcp handle only when the requesting pid matches the recorded owner, and `free` clears a slot only for
  its owner (`src/handles.rs:24`). Every TCP handler resolves its handle through `get`, so a stolen or
  forged app handle answers `E_NO_SOCKET` ([server](server.md)).
- The UDP table `udp_ports` is a 16-slot array of `(owner_pid, local_port, SocketHandle)`
  (`src/udp_ports.rs:20`). It is keyed on the pid and the local port rather than an opaque handle: `insert`
  adds a binding, `get` finds the socket for a pid and port, and `remove` clears it
  (`src/udp_ports.rs:32`). This is why UDP ops carry the local port in their payload rather than a handle
  ([protocol](protocol.md)).

Both tables are fixed-size arrays, so a client cannot exhaust the capsule's memory by opening sockets; a
full table answers `E_NO_SOCKET` at connect or bind ([server](server.md)).
