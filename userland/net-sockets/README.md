...
| Service handle | `net.sockets` | `Capsule.mk:6`, `src/userspace/capsule_net_sockets/spawn.rs:31` |
...
| Feature gate | `nonos-capsule-net-sockets` | `Capsule.mk:8`, `src/userspace/capsule_net_sockets/embed.rs:17` |
| Wire magic | `NSKT` (`0x4E534B54`) | `src/protocol/header.rs:17` |
| Kernel mirror | `src/userspace/capsule_net_sockets` | `Capsule.mk:15`, `src/userspace/capsule_net_sockets/spawn.rs` |
...
(`src/userspace/capsule_net_sockets/spawn.rs:50`), which is `0x1c`; the manifest ceiling adds `CoreExec`
...
|------|---------|----------------|
| [operations.md](operations.md) | `src/protocol/`, `src/server/mod.rs`, `src/server/runner.rs`, `src/server/parse_req.rs`, `src/server/respond.rs`, `src/server/handlers/` | The `NSKT` wire format, the receive loop, the eleven ops and their per-op payloads and handlers, and the errno set. |
| [handles.md](handles.md) | `src/sockets/` | The socket abstraction: the `Kind` families, the `Socket` control block, the per-pid `(pid, handle)` key, the 256-slot table, and the open, lookup, and close operations that mint and reap handles. |
| [transports.md](transports.md) | `src/clients/` | The transport backend clients: the shared IPC envelope, the TCP client, the UDP client, and the multi-file Nym client, and how each op picks a backend by socket kind. |
| [state.md](state.md) | `src/state.rs`, `src/main.rs` | Bring-up and process-global state: the `_start` heap init, the setup-retry loop, and the discovery and caching of the `net.tcp`, `net.udp`, and `net.nym` service ports. |
| [contributing.md](contributing.md) | the whole tree | Where each concern lives, how to add an op or a socket kind, the build and sign steps, and the code standards. |
| [debugging.md](debugging.md) | runtime | The boot marker, the setup retry loop, and the runtime failure modes: no handle, no transport, table full, bad family or kind, not bound, and not connected. |

## Lifecycle

The capsule is `no_std`/`no_main`. `_start` initialises the heap, then loops `state::discover` until it
succeeds, then enters `server::run`, which loops forever (`src/main.rs:31`). If the heap init fails the
process exits with code 1 (`src/main.rs:33`); discovery itself does not exit on failure, it retries after
yielding sixty-four times so a boot that races ahead of the transport capsules recovers on its own
(`src/main.rs:39`). `state::discover` looks up the `net.tcp` and `net.udp` service ports and requires both,
and looks up `net.nym` opportunistically, tolerating its absence (`src/state.rs:45`). The kernel spawns the
capsule through [verified spawn](../../security/capsules-and-trust.md) under the network spawn plan,
checking its signature and attestation and holding its requested capabilities against its manifest ceiling
before its ELF is mapped (`src/userspace/init/spawn_plan/network/spawn_sockets.rs:17`). A successful spawn
prints `[NET-SOCKETS] capsule spawned` on the boot log (`src/userspace/init/capsule_boot/run.rs:29`); the
[debugging](debugging.md) page covers what that and the discovery retry mean.
...
(`userland/sdk/nonos_std/src/net/proto/session.rs:23`, `userland/sdk/nonos_std/src/net/proto/wire.rs:19`).
