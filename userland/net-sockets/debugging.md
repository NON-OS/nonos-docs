...
[README](README.md), the [operations](operations.md), [handles](handles.md), [transports](transports.md),
and [state](state.md) pages.
...
(`src/userspace/init/spawn_plan/network/spawn_sockets.rs:22`), whose `Ok` arm calls
`boot_log::ok(prefix, "capsule spawned")` (`src/userspace/init/capsule_boot/run.rs:29`). An absent line
means the capsule never started, usually a signature, manifest, or capability failure; the `Err` arm prints
an `[ERROR]` line instead (`src/userspace/init/capsule_boot/run.rs:32`). The spawn is feature-gated on
`nonos-capsule-net-sockets`, so an image built without that feature will not print the line at all
(`src/userspace/init/spawn_plan/network/spawn_sockets.rs:18`).
...
a heap init failure, before discovery even runs (`src/main.rs:33`). After that, `wait_for_setup` calls
`state::discover` in a loop, yielding sixty-four times between attempts, and only enters the server once
discovery returns `Ok` (`src/main.rs:39`). So a capsule that spawned but is not answering has almost always
not finished discovery. Discovery fails, and retries, whenever the `net.tcp` or `net.udp` lookup fails or
reports port zero, because both are mandatory (`src/state.rs:46`, `src/state.rs:52`); `net.nym` is optional
and its absence does not block discovery (`src/state.rs:48`).
...
is on the [operations](operations.md) page.
...
when it is exhausted (`src/server/handlers/socket.rs:41`, `src/server/handlers/accept.rs:37`). On a healthy
system this means a client is leaking sockets: it opened handles without calling `OP_CLOSE`, which is what
frees a slot (`src/sockets/table/close.rs:28`). The table is global across callers but keyed per pid, so
...
caller's attested pid (`src/sockets/table/lookup.rs:29`). Three things produce it: a handle the caller never
...
(`src/sockets/table/open.rs:25`), a stale handle after close reliably misses rather than aliasing a live
...
(`src/server/handlers/send.rs:46`, `src/server/handlers/connect.rs:41`). It appears when a `connect`,
...
(`src/clients/envelope.rs:29`, `src/clients/envelope.rs:51`). For a mixnet socket it also covers the case
where `net.nym` was never registered, so `state::nym()` returns zero (`src/server/handlers/connect.rs:60`,
`src/server/handlers/setsockopt.rs:63`). When chasing it, look one layer down: the sockets capsule does not
...
(`src/server/handlers/listen.rs:33`). It is the ordering guard: a client must `OP_BIND` the stream socket to
a local port before `OP_LISTEN`, because listen reads the bound port to pass to `net.tcp`
(`src/server/handlers/listen.rs:35`). A datagram socket never satisfies this arm, since the bound check also
...
remote address (`src/server/handlers/send.rs:52`, `src/server/handlers/recv.rs:54`). It is the "you have a
...
2, or 3 (`src/server/handlers/socket.rs:26`, `src/server/handlers/socket.rs:33`). The capsule is IPv4-only
