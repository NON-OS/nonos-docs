...
mutates in the table, read the [handles](handles.md) page; for how an op leaves for a transport capsule,
read the [transports](transports.md) page.

## The NSKT wire format

Every request and reply begins with a fixed twenty-byte header, defined in one place as `HDR_LEN`
(`src/server/parse_req.rs:19`, `src/server/respond.rs:21`). The magic is `NSKT`, stored as the
little-endian `u32` `0x4E534B54` (`src/protocol/header.rs:17`).
...
(`src/server/parse_req.rs:27`). A reply reuses the same layout: `respond` writes the magic, version 1, the
op, the errno, a zeroed reserved word, the echoed `request_id`, and the reply `payload_len`, then sends the
header plus body back to the sender by pid with `mk_ipc_reply` (`src/server/respond.rs:23`). This is the
same twenty-byte frame the NØNOS standard library builds when it opens a socket
(`userland/sdk/nonos_std/src/net/proto/wire.rs:34`).

## The request loop

`server::run` allocates one receive and one transmit buffer of `HDR_LEN + 1536` and loops forever
(`src/server/runner.rs:26`, `src/server/runner.rs:28`). Each turn does three things in order:

1. Block on `mk_ipc_recv_from` for the next request, recording the sender pid (`src/server/runner.rs:33`).
   The pid is the kernel-attested caller identity, and it is what every handle is scoped to; a receive that
   returns nothing or a zero sender is skipped (`src/server/runner.rs:34`).
2. Parse the request header and body. A parse failure is skipped silently rather than answered, because a
   malformed frame has no trustworthy `request_id` to echo (`src/server/runner.rs:37`).
3. Dispatch on the opcode to a handler (`src/server/handlers/dispatch.rs:23`). A recognized op runs its
   handler and the dispatcher returns `true`; an unknown op returns `false`, and the loop replies `E_BAD_OP`
   with the echoed `request_id` (`src/server/runner.rs:38`, `src/server/handlers/dispatch.rs:36`).
...
The opcodes are `u16` constants (`src/protocol/ops.rs:17`). Handles are the per-socket identifiers the
table mints; see the [handles](handles.md) page. Every handler is one file under `src/server/handlers/`,
and the dispatch maps each op to exactly one (`src/server/handlers/dispatch.rs:24`).

| Op | Value | Request body | Reply body | Handler |
|---|---|---|---|---|
| `OP_HEALTHCHECK` | 1 | none | none | `src/server/handlers/health.rs:21` |
| `OP_SOCKET` | 2 | `u16` family, `u16` kind | `u32` handle | `src/server/handlers/socket.rs:23` |
| `OP_BIND` | 3 | `u32` handle, `[u8;4]` ip, `u16` port | none | `src/server/handlers/bind.rs:25` |
| `OP_LISTEN` | 4 | `u32` handle | none | `src/server/handlers/listen.rs:25` |
| `OP_ACCEPT` | 5 | `u32` handle | `u32` child handle | `src/server/handlers/accept.rs:25` |
| `OP_CONNECT` | 6 | `u32` handle, `[u8;4]` ip, `u16` port | none | `src/server/handlers/connect.rs:25` |
| `OP_SEND` | 7 | `u32` handle, bytes | none | `src/server/handlers/send.rs:25` |
| `OP_RECV` | 8 | `u32` handle | payload bytes | `src/server/handlers/recv.rs:25` |
| `OP_CLOSE` | 9 | `u32` handle | none | `src/server/handlers/close.rs:25` |
| `OP_GETSOCKOPT` | 10 | `u32` handle | 16-byte status block | `src/server/handlers/getsockopt.rs:23` |
| `OP_SETSOCKOPT` | 11 | `u32` handle, `u16` level, `u16` opt | none | `src/server/handlers/setsockopt.rs:28` |

The body fields are read with the bounds-checked helpers `u16_at`, `u32_at`, and `ip4_at`, each of which
returns `E_BAD_LEN` when the requested slice runs past the body (`src/server/handlers/io.rs:19`). A handler
...
anything else is `E_BAD_FAMILY` (`src/server/handlers/socket.rs:24`). The kind maps `1` to `Stream`, `2` to
`Datagram`, and `3` to `Mixnet`, and any other value is `E_BAD_KIND` (`src/server/handlers/socket.rs:29`).
On success it opens a new socket in the table for the caller pid and replies the fresh `u32` handle, or
`E_TABLE_FULL` if the 256 slots are exhausted (`src/server/handlers/socket.rs:36`).

### Bind

`OP_BIND` reads the handle, a four-byte IP that it validates but does not store, and a `u16` port
(`src/server/handlers/bind.rs:42`). For a `Datagram` socket it binds the port on `net.udp` and returns
...
(`src/server/handlers/bind.rs:31`).

### Listen and accept

`OP_LISTEN` reads the handle and requires a `Stream` socket that has already been bound; otherwise it is
`E_NOT_BOUND` (`src/server/handlers/listen.rs:31`). It calls `tcp::listen` on `net.tcp` with the bound
port, stores the returned transport handle, marks the socket listening, and replies `E_OK`, or
`E_NO_TRANSPORT` if the TCP listen fails (`src/server/handlers/listen.rs:35`). `OP_ACCEPT` reads the
...
is full (`src/server/handlers/accept.rs:30`).

### Connect

`OP_CONNECT` reads the handle, the destination IP, and the destination port, and records the remote address
on the socket (`src/server/handlers/connect.rs:30`). A `Datagram` socket needs no connection and returns
...
(`src/server/handlers/connect.rs:35`, `src/server/handlers/connect.rs:59`). Any other kind is `Stream`: it
calls `tcp::connect` on `net.tcp` and stores the returned connection as the transport handle, replying
`E_OK` or `E_NO_TRANSPORT` (`src/server/handlers/connect.rs:44`). An unknown handle is `E_NO_HANDLE`.

### Send and receive

`OP_SEND` requires at least four body bytes for the handle, then treats the rest as the payload
(`src/server/handlers/send.rs:26`). It dispatches by kind: a `Stream` socket with a live transport handle
...
(`src/server/handlers/send.rs:41`). `OP_RECV` reads the handle and dispatches the same way, reading from
...
and a transport failure is `E_NO_TRANSPORT` (`src/server/handlers/recv.rs:42`).
...
failure is `E_NO_TRANSPORT` (`src/server/handlers/close.rs:34`). It then removes the slot from the table
and replies `E_OK`, or `E_NO_HANDLE` if the handle was unknown or already gone
(`src/server/handlers/close.rs:51`).
...
(`src/server/handlers/getsockopt.rs:38`). An unknown handle is `E_NO_HANDLE`. `OP_SETSOCKOPT` reads the
handle, a `u16` level, and a `u16` option; the only accepted pair is the mixnet cover-tick level and option
(`src/server/handlers/setsockopt.rs:25`). It requires a `Mixnet` socket with a live session and a
...
(`src/server/handlers/setsockopt.rs:55`).

## The errno set

Every reply carries a `u16` errno at offset 8 (`src/protocol/errno.rs:17`).
...
(`src/server/parse_req.rs:27`, `src/server/runner.rs:37`).
