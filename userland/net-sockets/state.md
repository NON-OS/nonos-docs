# Bring-up and process state

This page mirrors `src/state.rs` and `src/main.rs`. It covers the small amount of process-global state the
...
itself, read the [handles](handles.md) page; for how those ports are used, read the
[transports](transports.md) page.

## The entry point

The capsule is `no_std`/`no_main`. `_start` is the raw entry: it initialises the heap, and if that fails it
exits the process with code 1, the only exit path in the capsule (`src/main.rs:31`, `src/main.rs:33`).
...
control to `server::run`, which never returns (`src/main.rs:35`).
...
attempts (`src/main.rs:39`). This is the whole of bring-up: unlike the transport capsules there is no
...
(`src/state.rs:21`). `discover` looks up `net.tcp` and `net.udp` by name and requires both to succeed,
...
(`src/state.rs:45`). The lookup helper calls `mk_service_lookup` and treats a nonzero return or a zero port
as a failure (`src/state.rs:52`). The distinction is deliberate: TCP and UDP are mandatory backends, so
...
values discovery stored (`src/state.rs:25`, `src/state.rs:29`). `nym()` is lazier: it returns the cached
value if it is nonzero, and otherwise retries the `net.nym` lookup once and caches a nonzero result
(`src/state.rs:33`). This lets a mixnet socket work even when Nym registered after the sockets capsule
...
(`src/server/handlers/connect.rs:61`).
