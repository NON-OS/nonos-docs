...
forever (`src/main.rs:31`). The top-level modules are declared there (`src/main.rs:22`).
...
| `src/state.rs` | the discovered transport ports and the handle counter home | you change bring-up or discovery |

## Adding an op

There are three edits, and the dispatch wiring is the load-bearing one.

1. Add the opcode constant to `src/protocol/ops.rs:17` and re-export it from `src/protocol/mod.rs:26`.
...
2. Write the handler as one file under `src/server/handlers/`, exposing a `handle` function that reads its
   body with the `io` helpers (`src/server/handlers/io.rs:19`), touches a socket through `SOCKETS.with`
   (`src/sockets/table/lookup.rs:22`), and replies through `respond` (`src/server/respond.rs:23`).
   `health.rs` is the minimal shape and `send.rs` is the shape for an op that dispatches by kind to a
   backend. Declare the module in `src/server/handlers/mod.rs:17`.
3. Wire it into the dispatch match in `src/server/handlers/dispatch.rs:24`. An unrecognized op falls
   through to the `false` arm and the loop answers `E_BAD_OP` (`src/server/handlers/dispatch.rs:36`,
   `src/server/runner.rs:38`), so the new arm is what makes the op reachable.
...
variant to `Kind` (`src/sockets/types.rs:17`), map its wire value in `OP_SOCKET`
(`src/server/handlers/socket.rs:29`), and add the arm to the `match sock.kind` in `send`, `recv`, `close`,
and `connect` (`src/server/handlers/send.rs:41`, `src/server/handlers/recv.rs:42`,
`src/server/handlers/close.rs:34`, `src/server/handlers/connect.rs:32`); leave the sentinel meaning of
...
(`src/clients/envelope.rs:23`); a stateful backend follows the multi-file `nym` split
(`src/clients/nym/mod.rs:17`), a stateless one follows the single-file `udp` shape (`src/clients/udp.rs`).
If the backend is a new service, add its port to `state.rs` and decide in `discover` whether it is
mandatory like `net.tcp` or optional like `net.nym` (`src/state.rs:45`).
...
`nonos-capsule-net-sockets` feature (`Capsule.mk:8`, `src/userspace/capsule_net_sockets/embed.rs:17`), and
the network spawn plan spawns it at boot (`src/userspace/init/spawn_plan/network/spawn_sockets.rs:17`).
...
- One unit per file. New ops are one file per handler under `src/server/handlers/`, new backends stay in
  ...
  (`src/sockets/mod.rs:17`, `src/clients/mod.rs:17`).
- Scope every socket to its owner. The table keys on `(pid, handle)` with the pid stamped by the kernel, not
  the request (`src/sockets/table/lookup.rs:29`); a change that lets a caller name a handle without matching
  its pid breaks the per-pid isolation the [handles](handles.md) page documents.
...
- Bound every structure. The socket table is a fixed 256 slots (`src/sockets/table/types.rs:22`); a full
