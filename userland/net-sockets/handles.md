...
For the ops that drive these handles, read the [operations](operations.md) page; for the transport state a
handle points at, read the [transports](transports.md) page.
...
(`src/sockets/types.rs:17`): `Stream` maps to `net.tcp`, `Datagram` to `net.udp`, and `Mixnet` to
...
port (`src/sockets/types.rs:30`); the capsule keeps IPv4 addressing only, which is why `OP_SOCKET` accepts
only family 4.
...
it is set, alongside the bound, listening, and has-remote bits (`src/server/handlers/getsockopt.rs:53`),
so a client can read a socket's shape without guessing.

## The handle and its owner

A handle is not a bare integer; it is half of a `(pid, handle)` pair (`src/sockets/types.rs:24`). The `pid`
is the kernel-attested sender the receive loop recorded (`src/server/runner.rs:33`), and the `handle` is a
...
(`src/sockets/table/lookup.rs:29`, `src/sockets/table/close.rs:25`). This is the per-pid isolation the
...
(`src/sockets/table/types.rs:42`, `src/sockets/table/open.rs:25`). Because it is monotonic and never
...
counter (`src/sockets/table/types.rs:22`, `src/sockets/table/types.rs:35`). It is a single process-global
`static SOCKETS` constructed at compile time (`src/sockets/table/types.rs:46`), so there is one table for
...
returning the new key or `None` if all 256 are taken (`src/sockets/table/open.rs:24`,
`src/sockets/table/open.rs:38`). It is called by `OP_SOCKET` for a client-created socket and by `OP_ACCEPT`
...
returning the closure's result, or `None` when no slot matches (`src/sockets/table/lookup.rs:22`). Because
...
(`src/sockets/table/close.rs:22`). It is the last step of `OP_CLOSE`, run only after the transport teardown
