...
installed mask, and entry (`src/kernel_core/process_spawn/capsule_spawn/runner/install/spawn_log.rs:18`,
`spawn_log.rs:22`):
...
prints an `[ERROR]` line instead (`capsule_boot/run.rs:32`, `boot_log/output.rs:49`).
...
error codes and their values are on the [protocol](protocol.md) page. Grouped by where they come from:
...
- Frame rejected before decode. A wrong magic returns `E_BAD_MAGIC` (`-74`), a wrong version returns
  ...
  `E_BAD_LEN` (`-90`) (`src/protocol/decode.rs:33`, `decode.rs:52`, `decode.rs:26`, `decode.rs:62`). A
...
- Unknown or misused op. An opcode outside the known set with an empty body returns `E_BAD_OP` (`-38`); a
  ...
  (`src/server/runner.rs:54`, `runner.rs:55`).
...
- Decode failed. A truncated or malformed image, an unsupported feature, or an image that exceeds the
  ...
  output, or truncation (`src/server/handlers/decode.rs:34`, `decode.rs:47`, `decode.rs:56`). A caller that
...
- Surface allocation failed. If the decode succeeded but the surface could not be mapped, registered, or
  ...
  (`src/server/handlers/surface.rs:34`, `surface.rs:51`, `surface.rs:53`, `surface.rs:56`).

- Payload too large. A request whose framed body would exceed `IPC_PAYLOAD_MAX` (128 KiB) cannot fit the
  receive buffer, so it is bounded at the transport before decode (`src/server/runner.rs:29`,
  `src/protocol/limits.rs:17`).

## Reading the loop behaviour

The loop blocks when idle and drains when busy (`src/server/runner.rs:36`). If the capsule appears
...
sees a reply, confirm it is sending to service port 4412 with a valid reply inbox; the [server](server.md)
page walks the dispatch, and the [protocol](protocol.md) page has the frame the caller must send.
...
(`userland/capsule_wallpaper/src/decode_client/wire.rs:21`). If you are debugging a new caller, the
[protocol](protocol.md) page is the contract to target, and the typed errno in the status word is your
first diagnostic.
...
(with `src/sys/boot_log/output.rs`), the capsule's `src/protocol/` parser and `src/server/` loop and
