...
[README](README.md)) is what makes the isolation worth having; the bounds below are what keep a malformed
...
`nonos_toolkit::image` running over them (`src/server/handlers/decode.rs:18`). A full PNG or JPEG parser is
...
error codes are listed on the [protocol](protocol.md) page.

## No panics, no partial surfaces

The capsule builds with `panic = "abort"` in both profiles (`userland/capsule_image_codec/Cargo.toml:19`,
...
(`src/server/handlers/decode.rs:34`, `decode.rs:47`), and the surface is written into the reply only after
...
`MAX_PIXELS = 16384` 32-bit pixels (`src/server/handlers/decode.rs:23`, `decode.rs:26`), and each toolkit
...
- The LZ4-raw decoder rejects zero dimensions (`BadDimensions`), an output larger than the buffer
  ...
  (`userland/toolkit/src/image/lz4_raw.rs:9`, `lz4_raw.rs:11`, `lz4_raw.rs:15`).
- `ImageSize::new` rejects a zero width or height with `BadDimensions` before any decode proceeds
  (`userland/toolkit/src/image/types.rs:1`).
...
(`src/server/handlers/decode.rs:35`, `decode.rs:36`).

## Bounded input

A single request's compressed input is capped at `IPC_PAYLOAD_MAX = 131072` bytes
(`src/protocol/limits.rs:17`). The receive buffer is `HDR_LEN + IPC_PAYLOAD_MAX`
(`src/server/runner.rs:29`), so a frame that would exceed the cap cannot fit and is bounded at the transport
before decode.
...
before it maps anything (`src/server/handlers/surface.rs:29`, `surface.rs:30`). A mapping failure returns
...
(`src/server/runner.rs:29`, `runner.rs:46`). The only shared artifact produced is the surface, owned by the
...
(`src/kernel_core/process_spawn/capsule_spawn/runner/preflight.rs:49`,
`src/kernel_core/process_spawn/capsule_spawn/runner/verified.rs:24`).
