...
The dispatch that reaches these handlers is on the [server](server.md) page; the frame and reply layout are
on the [protocol](protocol.md) page; the containment argument for running the parser here is on the
[safety](safety.md) page.

## The decode handler

`handle` takes the sender pid, the parsed `Request`, the untrusted body, and the transmit buffer
(`src/server/handlers/decode.rs:25`). It allocates a fixed pixel buffer, matches the opcode to a toolkit
...
|--------|-------------|------|
| PNG | `png::decoder::decode_png_argb8888` | `userland/toolkit/src/image/png/decoder.rs:11` |
| BMP | `bmp::decode_bmp_argb8888` | `userland/toolkit/src/image/bmp.rs:13` |
| JPEG | `jpeg::decode_jpeg_argb8888` | `userland/toolkit/src/image/jpeg/decode/decode_jpeg_argb8888.rs:30` |
| LZ4-raw | `lz4_raw::decode_lz4_raw_argb8888` | `userland/toolkit/src/image/lz4_raw.rs:3` |

These are real parsers. The PNG path carries its own zlib inflate and scanline filtering
(`userland/toolkit/src/image/png/inflate/`, `png/scanline.rs`), and the JPEG path carries Huffman table
building, dequantization, IDCT, and MCU walking (`userland/toolkit/src/image/jpeg/`). The image module
re-exports the four format submodules and the shared types through `userland/toolkit/src/image/mod.rs:1`.
The `ImageSize` returned on success carries the decoded `width` and `height`, and `pixel_count` is
`width * height` (`userland/toolkit/src/image/types.rs:1`).

## The LZ4-raw prefix

`OP_DECODE_LZ4_RAW` is the one op that is not a self-describing container, so the handler parses its
dimensions before calling the decoder (`decode_lz4`, `src/server/handlers/decode.rs:49`):
...
The prefix is `DECODE_LZ4_PREFIX_LEN = 8` bytes (`src/protocol/limits.rs:19`). The decoder itself checks
...
otherwise (`userland/toolkit/src/image/lz4_raw.rs:9`, `lz4_raw.rs:11`, `lz4_raw.rs:15`).

## Error mapping

Every toolkit `DecodeError` maps onto a protocol errno in one place (`map_decode_error`,
`src/server/handlers/decode.rs:56`):
...
The `DecodeError` variants are defined at `userland/toolkit/src/image/types.rs:1`. Keeping the mapping in
...
(`src/server/handlers/surface.rs:28`). This is the only place the codec touches its graphics capability:
...
descriptor documented on the [protocol](protocol.md) page (`src/server/handlers/decode.rs:37`).
