...
|------|---------|----------------|
| [protocol.md](protocol.md) | `src/protocol/` | The `GMIN` frame, the five operations, the request and reply layout, the 32-byte decode descriptor, the size limits, and the seven typed error codes. |
| [server.md](server.md) | `src/server/` | The receive/dispatch/reply loop, the blocking-then-drain behaviour, the dispatch table, and the reply builders. |
| [decode.md](decode.md) | `src/server/handlers/` | The parser boundary: the format dispatch, the shared toolkit decoders, the LZ4-raw prefix, the error mapping, and the surface registration path. |
| [safety.md](safety.md) | the untrusted-input posture | Why the mask is narrow, how the wire parser and the decoders are bounded, and what a subverted decoder can and cannot reach. |
| [contributing.md](contributing.md) | the whole tree | Where to work, how to add a format or decoder, the build and sign steps, and the code standards. |
| [debugging.md](debugging.md) | runtime | The boot marker, the typed failure signatures on the wire, and where to look when a decode fails. |
...
(`src/userspace/init/spawn_plan/desktop_services.rs:43`, `src/userspace/capsule_image_codec/spawn.rs:37`).
...
(`userland/capsule_wallpaper/src/decode_client/wire.rs:21`). The service exists as the isolated decode
boundary for callers that want the parser off their own stack; the [protocol](protocol.md) page is the
