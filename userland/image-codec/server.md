...
re-exports `run` through `src/server/mod.rs:21`. The handlers themselves are on the [decode](decode.md)
...
`server::run`, which never returns (`src/main.rs:28`, `src/main.rs:32`). A heap init failure exits with
status 1 before the loop starts (`src/main.rs:29`).

`run` allocates one receive buffer and one transmit buffer, each `HDR_LEN + IPC_PAYLOAD_MAX` bytes, and
loops forever calling `drain_ipc` (`src/server/runner.rs:28`, `runner.rs:31`). The buffers are allocated
...
`drain_ipc` is the core (`src/server/runner.rs:36`). It starts blocking and switches to non-blocking after
the first message:
...
For each received frame the loop parses it and dispatches on the opcode (`src/server/runner.rs:46`):
...
(`src/protocol/decode.rs:19`, `runner.rs:49`). `OP_HEALTHCHECK` is served only with an empty body; a
...
constants and error codes are on the [protocol](protocol.md) page.

## Replies

`respond` builds the two reply shapes over `mk_ipc_reply` (`src/server/respond.rs`):
...
- `payload(sender_pid, req, body_len, tx)` writes the response header with a `STATUS_LEN + body_len`
  ...
  (`src/server/handlers/decode.rs:44`).
...
`handlers` is three modules (`src/server/handlers/mod.rs:17`):

- `health` replies status 0 with no payload (`src/server/handlers/health.rs:20`).
- `decode` is the format dispatch and the parser boundary, covered on the [decode](decode.md) page
  (`src/server/handlers/decode.rs:25`).
- `surface` maps, registers, and shares the decoded ARGB pixels, also covered on the [decode](decode.md)
  page (`src/server/handlers/surface.rs:28`).

## Source map

This page is drawn from `userland/capsule_image_codec/src/main.rs` (the entry) and
