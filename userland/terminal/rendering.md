...
IPC) see [the terminal README](README.md).
...
`paint_tabs` (`src/term/terminal/app_impl.rs:30`, `src/term/terminal/app_impl_paint.rs:22`). The
...
(`app_skeleton/src/runner/paint_once.rs:33`). Event handlers signal that a repaint is needed by
returning `EventOutcome::Repaint`; typing, tab switches, and tab close all do this
(`app_skeleton/src/app/event_outcome.rs:20`, `src/term/terminal/tabs.rs:42`). So a frame is produced on
demand, and each one is a full repaint of the surface, not a delta.

## Surface and framebuffer target (`paint/compose.rs`, skeleton `PaintBuffer`)
...
are the drawable size (`app_skeleton/src/paint/buffer.rs:17`). The buffer is constructed in the runner
by reinterpreting the surface's `backing_va` as a `u32` slice, so every `fill_rect` and `text` call
writes directly into the memory the compositor reads (`app_skeleton/src/runner/paint_frame.rs:49`).

`fill_rect` clips against the buffer bounds and writes `argb` into each covered word
(`app_skeleton/src/paint/fill_rect.rs:20`). `clear` fills the whole buffer with one colour
(`app_skeleton/src/paint/clear.rs`). `text` and `text_scaled` delegate to the toolkit font renderer,
`draw_text` / `draw_text_scaled`, passing the pixel slice, stride, and clip size
(`app_skeleton/src/paint/text.rs:22`, `app_skeleton/src/paint/text_scaled.rs:22`).

`compose::paint` is the frame recipe (`src/paint/compose.rs:34`):

1. `fb.clear(BACKGROUND)` wipes the surface to the theme background `0xFF181A1F`
   (`src/paint/compose.rs:35`, `src/term/theme.rs:17`).
2. `draw_header` paints the top bar (`src/paint/compose.rs:36`).
3. It reads `state.scrollback.grid.alternate` and computes `input_y`, the row where the input bar sits,
   as `height - (FOOTER_H + LINE_HEIGHT)` (`src/paint/compose.rs:37`).
4. It picks one of three body layouts (below), draws the input line unless the alternate screen is
   active, and ends with `draw_footer` (`src/paint/compose.rs:39`).

`paint_tabs` is the wrapper the capsule actually calls: it paints the active tab's `State` with `paint`,
then overlays the tab strip (`src/paint/compose.rs:29`). After `app.paint` returns, the skeleton draws
its own window chrome (titlebar, buttons, border) on top of the same buffer before committing
(`app_skeleton/src/runner/paint_frame.rs:59`), so the terminal owns the interior and the skeleton owns
the frame.

## Layout constants (`paint/constants.rs`)

The geometry is fixed in one file (`src/paint/constants.rs:17`):
...
(`app_skeleton/src/paint/glyph_advance.rs:22`, `toolkit/src/font/atlas.rs:12`). The two agree, but the
grid uses the live value so it tracks any font change, while the header, fetch, and input-bar code use
the constant.

## The three body layouts (`paint/compose.rs`)

`compose::paint` chooses exactly one body path (`src/paint/compose.rs:39`):
...
- Alternate screen (`grid.alternate` true): a full-program TUI is running, so it draws the grid across
  ...
  (`src/paint/compose.rs:40`).
- Fresh tab (`state.fresh` true, no command run yet): it draws the neofetch-style splash instead of the
  grid (`src/paint/compose.rs:43`).
- Normal (a command has run): it draws the block chrome behind the text, then the grid, both bounded by
  `input_y` so scrollback never overlaps the prompt (`src/paint/compose.rs:45`).

## Cell and glyph drawing (`paint/draw_grid.rs`)

`draw_grid` is where the character grid becomes pixels (`src/paint/draw_grid.rs:40`). It reads the column
pitch once as `adv = fb.glyph_advance()`, then walks `row` from 0 to `VISIBLE_ROWS` (15,
`src/term/dimensions.rs:19`). For each row it computes `y = oy + row * LINE_HEIGHT` and stops early if
the row would cross `max_y`, which is how the grid is clipped to the input line or the footer
(`src/paint/draw_grid.rs:42`).
...
is on the active screen (`src/term/grid/view.rs:22`). So scrollback is not a separate render path; it is
the same walk reading a different slice.

Then it walks `col` from 0 to `COLS` (96, `src/term/dimensions.rs:17`). Each `Cell` carries a byte
`ch`, an ANSI foreground index `fg`, an ANSI background index `bg`, and `flags`
(`src/term/grid/cell.rs:21`). Turning one cell into pixels:

- `x = ox + col * adv` places the cell (`src/paint/draw_grid.rs:50`).
- `fg` and `bg` are converted from ANSI indices to ARGB with `ansi_to_argb` (`src/paint/draw_grid.rs:51`).
- If the cell's `F_REVERSE` flag is set, foreground and background are swapped
  (`src/paint/draw_grid.rs:53`, `src/term/grid/cell.rs:19`).
...
`BACKGROUND` so it reads through the block (`src/paint/draw_grid.rs:26`). This is a solid inverse-video
...
(`src/paint/draw_cursor.rs:22`). Both cursors share the `CURSOR` colour `0xFF5FB0C9`
(`src/term/theme.rs:21`).

## Colours and palette (`paint/fetch_palette.rs`, theme, vt/color)

Two colour systems meet in the paint layer. The chrome (header, footer, prompt, cursor, block tints)
uses named theme constants, fixed ARGB words in `src/term/theme.rs:17`. The grid contents use ANSI
...
(`src/term/vt/color.rs:20`). `DEFAULT_BG` is index 0, which is the value `draw_grid` checks against to
decide whether a cell needs a background fill (`src/term/vt/color.rs:18`).
...
(`src/paint/fetch_palette.rs:19`).

## Fetch splash (`paint/fetch.rs`, `paint/fetch_row.rs`, `paint/fetch_uptime.rs`)

A fresh tab shows a neofetch-style card instead of scrollback. `draw_fetch` draws a large scaled NONOS
mark and wordmark with `text_scaled`, then a right-hand column of labelled rows starting at x=172
(`src/paint/fetch.rs:26`). Each row is a `fetch_row` call: the label is drawn in `ACCENT` and the value
is drawn in `FOREGROUND` one label-column to the right, at `x + 9 * CELL_WIDTH`
(`src/paint/fetch_row.rs:22`). The fixed rows are os, kernel, shell, trust, arch, and a live uptime
(`src/paint/fetch.rs:36`). The uptime value comes from `mk_time_millis()` minus the tab's `start_ms`,
formatted as `<m>m <s>s` by `uptime_str` (`src/paint/fetch.rs:46`, `src/paint/fetch_uptime.rs:19`). The
card closes with the swatch strip from `draw_palette` (`src/paint/fetch.rs:53`).

## Block chrome (`paint/block_chrome.rs`, `paint/block_meta.rs`)
...
block owns that line (`src/paint/block_chrome.rs:29`, `src/term/grid/absline.rs:28`). For a row inside a
...
the block's status: `BLOCK_OK`, `BLOCK_ERR`, or `BLOCK_RUN` (`src/paint/block_chrome.rs:41`).
...
positioned in cell multiples from the right edge (`src/paint/block_meta.rs:23`). This is decoration only;
it reads block records and does not change the grid.

## Header, input bar, footer (`paint/header.rs`, `paint/draw_input_line.rs`, `paint/footer.rs`)

The header is a filled `HEADER_H` bar with a one-pixel rule under it, a scaled `NONOS` wordmark on the
left, and the current working directory right-aligned and clipped to its last 34 characters
(`src/paint/header.rs:23`).
...
fit between equal left and right margins (`src/paint/draw_input_line.rs:24`). The prompt is a glyph plus
the cwd (capped to a third of the line so a deep path cannot starve the typing area) plus a trailing
space (`src/paint/draw_input_line.rs:36`). The body text is horizontally scrolled by a `body_cells`-wide
window so the cursor stays on screen, and the caret is placed by `draw_cursor` at the cursor's column
within that window (`src/paint/draw_input_line.rs:43`).

The footer is a filled `FOOTER_H` bar at the bottom carrying a fixed one-line keybinding hint in the
`DIM` colour (`src/paint/footer.rs:22`).

## Tab strip (`paint/tabstrip.rs`)
...
(`src/paint/tabstrip.rs:29`). Each slot has an `x` close mark on its right, and a `+` new-tab affordance
follows the last slot (`src/paint/tabstrip.rs:44`). The strip's `TAB_W`, `STRIP_Y`, `STRIP_H`,
`CLOSE_W`, and `PLUS_W` constants are re-used by the click hit-testing so the drawn geometry and the hot
regions stay in one place (`src/paint/tabstrip.rs:23`, `src/term/terminal/tab_click.rs:20`).
...
the window rectangle to tell the compositor the surface changed (`app_skeleton/src/runner/paint_once.rs:37`).
...
tables in `src/term/theme.rs` and `src/term/vt/color.rs`, and the framebuffer primitives and paint loop
