# Contributing to capsule_terminal
...
[README](README.md), the [command reference](commands.md), the [input model](input.md), the
[rendering](rendering.md), and the [terminal emulation](emulation.md) pages in this folder.
...
subscription, and paint loop (`userland/capsule_terminal/src/main.rs:36`). The four top-level modules are
...
1. Write the command module as one file per verb under `src/command/builtin/nox/`, next to the existing
   ones (for example `nox/stat.rs`). A fallible command exposes
   `pub fn run(state: &mut State, args: &[&[u8]]) -> bool` and returns `false` after pushing an error line
   (`src/command/builtin/nox/stat.rs:25` is the reference shape; it calls `state.scrollback.push_error`
   and returns `false` on a bad argument, `src/command/builtin/nox/stat.rs:27`). An infallible command can
   ...
   (`src/command/builtin/nox/ensure_pid.rs:5`).

2. Wire it into the dispatch table. Add the module to the `use super::{...}` import list at the top of the
   match module (`src/command/builtin/nox/dispatch.rs:17`) and add a match arm in `dispatch`
   (`src/command/builtin/nox/dispatch.rs:35`). A fallible command's arm evaluates to its `bool`; an
   ...
   (`src/command/builtin/nox/dispatch.rs:40`). The value the arm produces becomes `state.last_status`,
   which is what `&&` and `||` gate on (`src/command/builtin/nox/dispatch.rs:135`). A word no arm matches
   falls to the `other =>` arm and runs `unknown`, reporting failure
   (`src/command/builtin/nox/dispatch.rs:130`).

3. If the command should be a top-level verb instead of a `nox` sub-command, add the arm to `exec` rather
   than the `nox` dispatch (`src/command/dispatch/exec.rs:29`). Keep the module under
   `src/command/builtin/`. Most verbs there build an `Output` over `state.scrollback` and call the
   builtin, the way `about` and `echo` do (`src/command/dispatch/exec.rs:32`).

4. Make it discoverable. Add the verb and any alias to the completion table so Tab completes it
   (`src/event/complete.rs:21`), and add a line to the help index so `nox help` lists it
   (`src/command/builtin/nox/help.rs:19`).
...
(`Makefile:1182`); `tests/boot/terminal_round_trip.sh` runs it and restores the plain GUI capsule
...
- The AGPL header sits at the top of every source file, byte for byte the same as the header on
  `src/command/builtin/nox/dispatch.rs:1` and every other module.
