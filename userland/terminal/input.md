...
order. For the wider capsule (commands, identity, IPC, security) see the [terminal overview](README.md).
...
`on_event` (`src/term/terminal/app_impl_on_event.rs:23`). `on_event` itself does one thing: it drops any
event that is not a key-down and passes the rest to the key router (`src/event/on_event.rs:22`).
...
key code against the editing and navigation set (`src/event/on_key.rs:32`). A change to the line returns
Repaint and a no-op returns Idle, which is what `bool_to_outcome` encodes for the edits that report whether
they changed anything (`src/event/bool_to_outcome.rs:19`).
...
inspected separately only to split copy from interrupt (`src/event/on_ctrl.rs:40`). A chord that matches
...
token completes against vfs paths resolved relative to the shell's cwd (`src/event/on_tab.rs:30`). When a
...
(`src/event/complete.rs:21`); `command_candidates` filters it by the typed prefix (`complete.rs:88`) and
...
back to the previous matching entry (`src/event/on_up.rs:21`). Down walks forward through matches and, once
it runs past the newest one, restores the text the search started from (`src/event/on_down.rs:21`).
...
(`src/event/copy_line.rs:21`). Ctrl+V reads the clipboard into a line-width buffer and inserts only the
printable bytes, skipping control bytes, then resets the history cursor and jumps to the bottom if anything
was inserted (`src/event/paste_clipboard.rs:22`).
...
(`src/term/terminal/app_impl_on_event.rs:24`). A click outside the strip's vertical band is ignored so it
never disturbs the active shell (`src/term/terminal/tab_click.rs:24`). Within the strip the x coordinate
...
(`src/term/terminal/app_impl_on_event.rs:29`). It fires only for a key-down with Ctrl held, uses Shift to
guard the open and close chords, and supports up to nine tabs (`src/term/terminal/tabs.rs:29`).
