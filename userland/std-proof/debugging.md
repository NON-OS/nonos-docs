...
the [what it exercises](std-facilities.md) page, and for why it exists at all see the [overview](README.md).
...
capsule in the VFS package store (`src/userspace/init/entry.rs:55`, `entry.rs:56`). That line uses the same
`boot_log::ok` tag-and-message format every capsule marker uses, a bracketed tag and the message
(`src/sys/boot_log/output.rs:33`). Seeing it means the feature is compiled in and the capsule is present and
...
is the empty stub, so nothing was staged (`src/userspace/init/entry.rs:59`). Build with the feature before
looking for anything else.

## Running it

With the staging line present, load the capsule at runtime with `install std_proof`. That path runs it
through [verified spawn](../../security/capsules-and-trust.md): its embedded ELF, id cert, manifest, and
attestation trailer are checked, its three requested capabilities are held against its manifest ceiling, and
only then is its ELF mapped (`src/userspace/capsule_std_proof/spawn.rs:34`). A failure here is a spawn error,
not a proof failure, and it reads as the usual signature, manifest, or capability rejection rather than a
line from `main`.

## The pass

A run is a pass only if one line appears, beginning `NONOS ran crates.io serde_json+regex+base64:` and
carrying every field with its expected value (`src/main.rs:31`):

```
  NONOS ran crates.io serde_json+regex+base64: os=nonos, nums sum=200, ok=true, regex hits=<count>, base64=bm9ub3M=
```

`os=nonos` and `ok=true` are the parsed JSON fields, `nums sum=200` is the array summed through an iterator
(`3+7+11+179`), and `base64=bm9ub3M=` is the base64 of `nonos` (`src/main.rs:16`, `src/main.rs:19`,
`src/main.rs:21`, `src/main.rs:30`). Those exact values are the assertion; a line with a different sum or a
...
the error and returns before the success line (`src/main.rs:11`, `src/main.rs:12`). Because the JSON literal
is a fixed valid string, this failing points not at bad input but at the parser or the allocator underneath
it, so the suspect is the PAL heap the [std PAL](../std-pal.md) page describes rather than the crate.

### The regex failure line

If `Regex::new` returns an error, `main` prints `nonos std proof: regex compile failed:` and returns the
same way (`src/main.rs:26`, `src/main.rs:27`). The pattern is a fixed valid one, so a compile failure again
...
(`src/userspace/capsule_std_proof/spawn.rs:34`), then whether the PAL stdout backend is wired, since the
print is the program's only output (`src/main.rs:31`).
...
field that failed to read (`src/main.rs:16`, `src/main.rs:20`, `src/main.rs:21`). This is a softer signal
