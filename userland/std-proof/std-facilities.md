# What std_proof exercises

This page walks the exact standard-library facilities the [std_proof](README.md) capsule drives, one pillar
...
[std PAL](../std-pal.md) page makes, that only the platform layer is swapped so the portable code above it
is the genuine published code running unchanged. For what the capsule is and why it exists at all, read the
[overview](README.md); for how to read the result off the boot log, read the [debugging](debugging.md) page.

## The three crates and the PAL underneath

The capsule links real upstream `std` and pulls three ordinary crates.io crates with no NONOS-specific
...
|---|---|---|---|---|
| JSON parse | `serde_json` | heap `String`/`Vec`, `Result`, iterators | the global allocator backed by the Mk heap syscalls | `src/main.rs:5`, `src/main.rs:9` |
| Regex match | `regex` | a deep dependency graph over `alloc` and `str` | the same allocator and no OS calls of its own | `src/main.rs:23` |
| Base64 encode | `base64` | trait dispatch on top of `alloc` | the allocator again, purely CPU-bound | `src/main.rs:4`, `src/main.rs:30` |
| Serial print | `println!` | the std stdout writer | the PAL stdout backend, the MDBG debug syscall over serial | `src/main.rs:31` |
...
(`src/main.rs:8`, `src/main.rs:9`). That single call is already a real test of the allocator: the parser
...
panic (`src/main.rs:9`, `src/main.rs:11`).
...
- `v["nums"].as_array()` walks the `[3,7,11,179]` array through `iter().filter_map(Value::as_i64).sum()`,
  which is a real iterator pipeline summing to `200` (`src/main.rs:17`, `src/main.rs:19`). The
  `unwrap_or(-1)` makes a missing or non-array field visible as a sentinel rather than a crash.
- `v["ok"].as_bool()` reads the boolean, again with a fallback (`src/main.rs:21`).
...
it does so (`src/main.rs:23`). The compile is matched the same careful way as the parse: `Ok` keeps the
`Regex`, and `Err` prints `nonos std proof: regex compile failed:` and returns (`src/main.rs:23`,
`src/main.rs:26`). Compiling under std is the check here, not just running a precompiled one.

With the automaton built, `re.find_iter(hay).count()` scans a fixed haystack and counts the words of five or
more lowercase letters, which is a second real iterator pipeline over the match results (`src/main.rs:22`,
`src/main.rs:24`). The count it produces is the regex hit field in the success line.
...
`base64::engine::general_purpose::STANDARD.encode(b"nonos")` (`src/main.rs:4`, `src/main.rs:30`). This is
...
interpolates the four computed values into one line (`src/main.rs:31`). Under std that macro writes to
stdout; on NONOS the PAL routes stdout to the MDBG debug syscall over serial, so the line lands on the boot
log. This is the only OS-visible side effect the whole program has, and it is the thing the
[debugging](debugging.md) page reads to decide pass or fail. There is no exit code and no reply frame; the
...
|---|---|---|
| JSON string field | `os=nonos` | `src/main.rs:16`, `src/main.rs:31` |
| JSON array summed through an iterator | `nums sum=200` (`3+7+11+179`) | `src/main.rs:17`, `src/main.rs:31` |
| JSON boolean field | `ok=true` | `src/main.rs:21`, `src/main.rs:31` |
| Regex compiled and run, hits counted | `regex hits=<count>` | `src/main.rs:23`, `src/main.rs:24` |
| Base64 of `nonos` | `base64=bm9ub3M=` | `src/main.rs:30`, `src/main.rs:31` |
