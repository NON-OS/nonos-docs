# The std_proof Capsule

`capsule_std_proof` is not an application. It is a self-test: a small signed capsule whose only job is to
prove, at runtime on a real NONOS boot, that an unmodified crate pulled straight from crates.io compiles
and runs on NONOS through the [std PAL](../std-pal.md). Its `main` parses JSON with `serde_json`, matches
...
Where the [snake](../snake/README.md) and [terminal](../terminal/README.md) capsules are real
...
| Service handle | `std_proof` | `Capsule.mk:10`, `src/userspace/capsule_std_proof/spawn.rs:28` |
...
(`src/userspace/capsule_std_proof/spawn.rs:48`). There is no `Network` bit (`0x04`, `types.rs:58`), no
...
linked against the `nonos-rt` `_start` shim (`src/main.rs:1`, `.cargo/config.toml`), and it depends on
three ordinary crates.io crates with no NONOS-specific edits (`Cargo.toml:15`). Each one exercises a
different part of the standard library and the PAL:

| Pillar | Crate | Exercises | Source |
|---|---|---|---|
| JSON parse | `serde_json` | heap allocation, `String`/`Vec`, `Result` handling, iterator adapters | `src/main.rs:9`, `src/main.rs:17` |
| Regex match | `regex` | a larger dependency graph compiling and running under std | `src/main.rs:23` |
| Base64 encode | `base64` | trait-based byte encoding on top of `alloc` | `src/main.rs:30` |
| Serial print | `println!` | the PAL stdout backend, which maps to the MDBG debug syscall over serial | `src/main.rs:31` |

The [what it exercises](std-facilities.md) page walks each pillar in detail. The point every pillar shares
is the one the [std PAL](../std-pal.md) page makes: only the platform layer is swapped, so all the
portable code in these crates is the genuine published code running unchanged.

## Lifecycle

std_proof is spawned through [verified spawn](../../security/capsules-and-trust.md): its embedded ELF, id
cert, manifest, and attestation trailer are checked, its requested capabilities are held against its
manifest ceiling, and only then is its ELF mapped (`src/userspace/capsule_std_proof/spawn.rs:34`). It is
compiled into a boot image only under the `nonos-capsule-std-proof` feature; without it the kernel entry
hook is an empty stub (`src/userspace/init/entry.rs:59`).
...
free for that path (`src/userspace/init/entry.rs:50`, `entry.rs:56`).
...
- On success `main` prints one line beginning `NONOS ran crates.io serde_json+regex+base64:` followed by
  the parsed OS string, the summed array (`3+7+11+179`, so `nums sum=200`), the boolean, the regex hit
  count, and the base64 of `nonos` (`src/main.rs:31`). Seeing that line with those fields is the pass.
- On a `serde_json` parse error `main` prints `nonos std proof: serde_json parse failed:` with the error
  and returns without printing the success line (`src/main.rs:11`).
...
signal that the graft or the spawn broke. The [debugging](debugging.md) page maps each of those outcomes
to where to look.
