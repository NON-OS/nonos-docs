# Contributing to capsule_clipboard
...
[README](README.md), the [operations and wire](protocol.md) page, and the [state](state.md) page.
...
(`userland/capsule_clipboard/src/main.rs:13`). The three top-level modules are declared there
(`src/main.rs:6`). It draws no window and subscribes to no input; the whole capsule is a byte store behind
one IPC port.
...
1. Add the opcode. Give it the next value in `src/protocol/ops.rs:17` (the current set ends at
   `OP_SET_IDLE_TIMEOUT = 0x0007`) and re-export it from `src/protocol/mod.rs:32`.
...
2. Write the handler as one file under `src/server/handlers/`, next to the existing ones. A handler has the
   ...
   `respond::status(out, req, E_INVAL)` on a short payload (`src/server/handlers/copy.rs:22` is the
   ...
   (`src/server/handlers/paste.rs:36`). It returns the byte count of the reply.

3. Wire it into the router. Declare the module in `src/server/handlers/mod.rs:17`, add it to the
   `use super::{...}` import at the top of the router (`src/server/handlers/router.rs:17`), and add a match
   arm in `route` keyed on the new opcode (`src/server/handlers/router.rs:30`). A word no arm matches falls
...
4. Decide the idle-wipe posture. If the op is a use of the clipboard's content, call `clipboard.touch(now_ms)`
   ...
   privacy decision, not an incidental one; the [state](state.md) page explains why.

If the op changes a bound, put the constant in `src/protocol/limits.rs` and re-export it from
`src/protocol/mod.rs:28` rather than hard-coding a number in the handler.
...
- No panics in capsule code. No `unwrap`, `expect`, or `panic!`. The request parser decodes by explicit
  little-endian indexing rather than `try_into().unwrap()` (`src/protocol/decode.rs:19`), and every failure
...
- Keep the mask minimal. The clipboard is admitted with exactly `CoreExec | IPC | Memory = 0x19`
  (`Capsule.mk:13`) and the kernel mirror asserts the same (`src/userspace/capsule_clipboard/spawn.rs:35`).
...
- The AGPL header sits at the top of every source file, byte for byte the same as the header on
  `src/protocol/header.rs:1` and every other module.
