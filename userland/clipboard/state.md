# Clipboard storage and the idle wipe
...
read and mutate this store are on the [operations and wire](protocol.md) page; the identity and mask are on
the [README](README.md).

## The entry

Each stored item is an `Entry` (`src/state/entry.rs:19`): a `content_type: u32` and a `Vec<u8>` of the
...
(`userland/app_skeleton/src/clients/clipboard/copy.rs:23`, `.../paste.rs:23`). Any other `u32` works the
same way. Because `OP_PASTE` matches on the exact type value (see the [protocol](protocol.md) page), two
callers using different type tags keep separate most-recent entries inside the one shared FIFO.

## The store

The store is `Clipboard` (`src/state/clipboard/types.rs:21`): a `VecDeque<Entry>` named `items`, a running
...
The bounds come from `src/protocol/limits.rs`:
...
`Clipboard::copy` (`src/state/clipboard/storage.rs:21`) is the only path that grows the store. It pushes
...
The defining behavior is the idle garbage collection (`src/state/clipboard/timer.rs`). The activity
timestamp gates it:
...
(`src/server/runner.rs:33`), so a clipboard left untouched past its timeout is emptied on the next wakeup
