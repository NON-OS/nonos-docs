...
kernel mirror asserts the same value (`REQUIRED_CAPS = 0x19`, `src/userspace/capsule_clipboard/spawn.rs:35`),
...
covered in full on the [operations and wire](protocol.md) page under the security notes.
...
|------|---------|----------------|
| [protocol.md](protocol.md) | `src/protocol/` and `src/server/` | The 20-byte frame, the request parser and reply builders, the seven operations with opcode and reply layout, the errno table, and the server loop that ties them together. |
| [state.md](state.md) | `src/state/` | The entry record, the bounded FIFO and its eviction policy, the content-type model, the byte and depth limits, and the idle wipe. |
| [contributing.md](contributing.md) | the whole tree | Where the source lives, the module map, how to add an operation, the build and sign targets for the `clipboard` slug, and the code standards. |
| [debugging.md](debugging.md) | runtime | The one spawn marker, why there are no runtime markers, and reading the wire status codes as the whole observable surface. |

## Lifecycle

The clipboard is spawned through verified spawn: `spawn_clipboard_capsule`
(`src/userspace/capsule_clipboard/spawn.rs:37`) decodes the baked trust anchor, then hands a
...
stamps `Capability::IPC` on the endpoint (`src/kernel_core/process_spawn/capsule_spawn/runner/install/install.rs:50`),
so a caller needs the IPC capability even to send to the port.

Inside the capsule, `_start` (`src/main.rs:13`) initializes the userland heap through
`nonos_libc::heap_init` and exits with status `1` if that fails, otherwise it hands control to
`server::run` (`src/server/runner.rs:27`), which never returns. The loop wipes the store if it has gone
idle, blocks on the port, dispatches one request, and replies. A successful spawn prints one line on the
boot log; the [debugging](debugging.md) page covers what it means and why it is the only line.
