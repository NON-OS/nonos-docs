...
status codes see the [operations and wire](protocol.md) page; for the store and the idle wipe see the
[state](state.md) page.

## Log markers

The first thing to confirm is that the capsule ran. As it comes up, the kernel install path prints one
spawn line (`src/kernel_core/process_spawn/capsule_spawn/runner/install/spawn_log.rs:17`):
...
(`src/kernel_core/process_spawn/capsule_spawn/runner/install/trace.rs:17`), so a stall during its install
...
empty string (`src/userspace/capsule_clipboard/spawn.rs:51`), so the capsule emits no `MkDebug` output.
...
(`src/server/handlers/copy.rs:27`). A `COPY` with a payload under 4 bytes (no room for the content type)
...
(`src/server/handlers/paste.rs:29`). An empty clipboard is a success, not an error, so a caller that finds
nothing after leaving the machine idle is seeing the idle wipe (`src/state/clipboard/timer.rs:26`), not a
...
returns `E_BAD_LEN` (-90). These come straight from the parser (`src/protocol/decode.rs:19`) and point at
the client's frame encoder, not the store. An op outside 1..7 returns `E_BAD_OP` (-38,
`src/server/handlers/router.rs:38`).
...
own `"clipboard not available"` (`userland/app_skeleton/src/clients/clipboard/copy.rs:26`), not as an error
...
256 KiB total is exceeded (`src/state/clipboard/storage.rs:24`), so an older entry can simply have been
