...
row layout in `src/about/section_render/row.rs` and the palette and geometry in `src/about/theme.rs`. A
...
constants that place everything. For what each section actually contains see [content](content.md); for
the wider capsule see the [about overview](README.md).
...
clamp against the value this paint computed (`frame.rs:28`, `src/about/state.rs:64`). See
[interaction](interaction.md) for the clamp.

## Header

The header fills the top band with the accent color and draws the product name, the tagline, and a
right-aligned `n / 5` breadcrumb (`src/about/paint/header.rs:28`). The breadcrumb is built into a small
...
section array (`header.rs:35`, `src/about/section.rs:26`).

## Tab strip

The tab strip fills its band with the tab-bar color, then lays the five section titles out left to right
(`src/about/paint/tabs.rs:27`). Each tab is its label width (8px per character) plus 14px of horizontal
...
test walks, so a click lands on the tab that was drawn (`tabs.rs:29`, `src/about/event/on_pointer_button.rs:38`).
...
(`src/about/paint/body.rs:23`). `render_section` matches the selected section to its renderer, and
`section_line_count` matches it to its line count for the scrollbar and the End key
(`src/about/section_render/mod.rs:28`, `mod.rs:38`).
...
rows with a running index and draw the ones that fall in the visible window (`src/about/section_render/identity.rs:36`,
...
height (`src/about/section_render/row.rs:24`, `row.rs:29`, `row.rs:33`). Because every renderer offsets its
...
returns without drawing (`src/about/paint/scrollbar.rs:29`). Otherwise it fills a track down the right edge
...
(`src/about/paint/status_bar.rs:23`). The hint is the constant
...
scroll model assumes are the same numbers (`src/about/manifest.rs:19`, `src/about/state.rs:65`).
