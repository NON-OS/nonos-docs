# Debugging capsule_about

This page lists the log marker the about capsule's boot path emits and the concrete failure modes with
where to look for each. For the model see the [README](README.md), the [content reference](content.md),
the [interaction model](interaction.md), and the [rendering](rendering.md) pages in this folder.
...
(`src/userspace/init/capsule_boot/run.rs:29`, `src/userspace/init/spawn_plan/apps.rs:45`,
...
(`src/about/manifest.rs:25`, `src/about/event/router.rs:38`). If keys and clicks do nothing at all, the
...
returned `None` and both Width and Height render as `unavailable` (`src/about/data/display.rs:23`,
`src/about/section_render/display.rs:31`). This happens before the display is up or if the query
capability is missing. Confirm the Authority section lists `GraphicsDisplayQuery` as granted; a mask
without it (`0x0800`) would deny the query at the boundary (`README.md`, `src/about/data/caps.rs:47`).
...
the day/hour/minute/second split falls back to zero (`src/about/data/uptime.rs:19`,
`src/about/section_render/uptime.rs:32`). The clock is read fresh on every paint, so if it later becomes
...
a hand-maintained `LINE_COUNT` (`src/about/state.rs:64`, `src/about/section_render/identity.rs:22`). A
...
when it changed something (`src/about/paint/frame.rs:29`, `src/about/event/on_home.rs:22`). If the section
