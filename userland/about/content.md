...
Uptime, and License (`src/about/section.rs:26`). Every value they show comes from one place: the baked
facts under `src/about/data/`, or one of two live syscalls. This page walks the facts and then each
section row by row. For how those rows reach the screen see [rendering](rendering.md); for the wider
capsule see the [about overview](README.md).
...
| Module | Holds | Source |
|---|---|---|
| `data/product.rs` | product name, tagline, copyright, homepage | `data/product.rs:17` |
| `data/build.rs` | crate version, git SHA, toolchain, target arch | `data/build.rs:17` |
| `data/abi.rs` | the ABI name `Mk` | `data/abi.rs:17` |
| `data/caps.rs` | the full capability table and the app's mask | `data/caps.rs:23`, `data/caps.rs:47` |
| `data/trust.rs` | the trust-chain, scheme, manifest, cert, and status strings | `data/trust.rs:17` |
| `data/display.rs` | the live primary-display size read | `data/display.rs:19` |
| `data/uptime.rs` | the live wall-clock read and its day/hour/minute/second split | `data/uptime.rs:19` |
| `data/license.rs` | the license name, version, URL, and the embedded `LICENSE` text | `data/license.rs:17` |

The version and commit are stamped at build time: `VERSION` is the crate's `CARGO_PKG_VERSION`
(`data/build.rs:17`) and `GIT_SHA` is `ABOUT_GIT_SHA`, resolved by the build script from
`NONOS_BUILD_SHA`, then `GITHUB_SHA`, then `git rev-parse --short=12 HEAD`, falling back to `unknown`
(`data/build.rs:18`, `build.rs:30`). The architecture is a `cfg`-selected string: `x86_64`, `aarch64`,
`riscv64`, or `unknown` (`data/build.rs:21`). The license body is the repository `LICENSE` file embedded
with `include_str!` (`data/license.rs:21`).

## Identity section

Nine label/value rows (`src/about/section_render/identity.rs:25`). Its `LINE_COUNT` is a fixed `9`
(`identity.rs:22`):
...
header and the name of every capability the mask does not grant (`src/about/section_render/authority.rs:31`).
...
filtered by `is_granted` (`authority.rs:52`, `data/caps.rs:49`):
...
Irq, Dma, and Pio (`authority.rs:64`, `data/caps.rs:23`). The row count grows with the table, so adding a
...
flush on its behalf; the about app itself never holds the present or map capability (`data/caps.rs:47`).

## Display section

Four rows (`src/about/section_render/display.rs:33`). Its `LINE_COUNT` is a fixed `4`
...
reads the primary display's size (`userland/libc/src/graphics/display_dimensions.rs:20`). If the call
returns a negative status or a zero dimension, `primary_dimensions` returns `None` and both fields render
as `unavailable` (`data/display.rs:23`, `display.rs:31`).

## Uptime section

Five rows derived from a single wall-clock read (`src/about/section_render/uptime.rs:40`). Its
`LINE_COUNT` is a fixed `5` (`uptime.rs:23`):

| Row | Value | Source |
|---|---|---|
| Wall ms | the raw milliseconds, or `unavailable` | `uptime.rs:41`, `data/uptime.rs:19` |
| Days | milliseconds split into whole days | `uptime.rs:42`, `data/uptime.rs:29` |
| Hours | remaining whole hours | `uptime.rs:43`, `data/uptime.rs:30` |
| Minutes | remaining whole minutes | `uptime.rs:44`, `data/uptime.rs:31` |
| Seconds | remaining whole seconds | `uptime.rs:45`, `data/uptime.rs:32` |

The raw value is `mk_time_millis()` (`userland/libc/src/time/wall.rs:19`); a negative return means the
clock is unreadable, in which case `read_millis` returns `None`, Wall ms shows `unavailable`, and the
split fields fall back to zero (`data/uptime.rs:19`, `uptime.rs:32`). `split_dhms` divides the raw
milliseconds into days, hours, minutes, and seconds (`data/uptime.rs:27`). Each paint reads the clock
...
per line of the embedded text (`src/about/section_render/license.rs:22`, `license.rs:24`):
...
scrollbar exists (`data/license.rs:21`, `license.rs:46`).
...
| Field | Value | Source |
|---|---|---|
| Product name | `NONOS` | `src/about/paint/header.rs:30`, `data/product.rs:17` |
...
| Status hint | `Tab/Shift-Tab cycle sections   Up/Down scroll line   PgUp/PgDn scroll page   Esc close` | `src/about/paint/status_bar.rs:21` |
