# Contributing to capsule_setup_wizard
...
[README](README.md), the [step reference](steps.md), the [state machine](state-machine.md), and the
[policy writes](policy-writes.md) pages in this folder.
...
hands the `Context` to `server::runner::run`, which owns the key loop and never returns (`src/main.rs:16`).
The top-level modules are declared there (`src/main.rs:6`).
...
| `src/state.rs` | `Context`, the whole model | you add a selection field |

## Adding a step

There are four edits, and the two dispatch tables in `screens/mod.rs` are the load-bearing ones.
...
1. Write the screen module as one file under `src/render/screens/`, exposing
   ...
   `default_key` for Enter and Escape. Follow `src/render/screens/network.rs` for a list step,
   `src/render/screens/privacy.rs` for a toggle step, or `src/render/screens/admin.rs` for a text field.
...
3. Register the module and wire both dispatch arms: add the `pub mod` line
   (`src/render/screens/mod.rs:1`), add the `draw` arm keyed on the new step number
   (`src/render/screens/mod.rs:15`) and the matching `on_key` arm (`src/render/screens/mod.rs:31`), add the
   step label to `STEP_LABELS` (`src/render/theme.rs:15`), and raise `DONE` in `src/server/step.rs:1` so
...
4. If the choice should persist, add its `Field` to the shared enum
   ...
   (`src/render/screens/review.rs:26`); the write only lands because the wizard is a trusted policy setter
   (`userland/capsule_policy/src/server/handle_set.rs:23`). Note that several fields today are collected
   without a `set_*` call; the [policy writes page](policy-writes.md) lists those gaps, and closing one is
...
`src/userspace/init/spawn_plan/orchestrator.rs:55`).
...
- No panics in capsule code. No `unwrap`, `expect`, or `panic!`. Every client call returns a `Result` the
  wizard either acts on or discards with `let _ =`, never a panic (`src/render/screens/review.rs:26`,
  `src/server/runner.rs:13`).
- One unit per file. Screens are one step per file under `src/render/screens/`, widgets one per file under
  ...
  (`src/render/screens/mod.rs:1`, `src/clients/compositor/mod.rs:25`).
- The AGPL header sits at the top of every source file, byte for byte the same as the header on
  `src/setup/fill.rs:1` and every other module.
