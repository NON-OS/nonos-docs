...
direct compositor-surface runner, not an [app-skeleton](../writing-an-app.md) window: it owns the whole
...
(`userland/capsule_policy/src/server/handle_set.rs:23`, `handle_set.rs:41`). The input router hard-codes
three trusted grabbers, `app.boot_splash`, `app.setup_wizard`, and `app.input_probe`
(`userland/capsule_input_router/src/server/handlers/grab_request.rs:25`). Those two boundaries live in the
[policy](../policy/README.md) and [input-router](../input-router/README.md) services, gated on the wizard's
name, and the [policy write page](policy-writes.md) sets out what the wizard actually does with that trust.
...
|------|---------|----------------|
| [steps.md](steps.md) | `src/render/screens/` | The ten steps as a user reference: each screen, its keys, and what it stores in `Context`. |
| [state-machine.md](state-machine.md) | `src/server/`, `src/render/`, `src/setup/`, `src/protocol.rs` | The step counter and its clamps, the global keys, the key-driven loop, service bring-up, and how a screen becomes pixels. |
| [policy-writes.md](policy-writes.md) | `src/render/screens/review.rs`, `src/clients/policy.rs` | The review commit, the two-name policy gate, the keyboard grab, and the honest gaps between what the wizard collects and what it writes. |
| [contributing.md](contributing.md) | the whole tree | Where to work, the exact steps to add a wizard step, the build and sign targets, and the code standards. |
| [debugging.md](debugging.md) | runtime | The boot marker, the failure modes, and where to look when bring-up, input, or a policy write misbehaves. |
...
(`src/userspace/init/spawn_plan/orchestrator.rs:56`, `orchestrator.rs:58`, `orchestrator.rs:67`). Boot
verifies the embedded ELF, id cert, manifest, and ZK attestation, registers `app.setup_wizard` on port
4794, and logs `[SETUP-WIZARD] capsule spawned` (`src/userspace/capsule_setup_wizard/spawn.rs:57`,
`src/userspace/init/capsule_boot/run.rs:29`). `_start` initialises the heap, runs `setup::run` to discover
services and put a surface on screen, then hands the resulting `Context` to `server::runner::run`, which
never returns (`src/main.rs:16`). The runner grabs the keyboard, draws, and loops on key deliveries; when
the step counter reaches `DONE`, the review step has already committed the choices, the runner removes the
scene and calls `mk_exit(0)`, and the supervisor then starts the desktop (`src/server/runner.rs:31`,
`src/userspace/init/supervisor/loop_impl.rs:36`). The [state machine page](state-machine.md) walks each
stage in detail.
