# The state machine, bring-up, and render
...
panel, not the per-step screens), and `src/protocol.rs`. For the individual steps see
[steps.md](steps.md); for the final policy write see [policy-writes.md](policy-writes.md).
...
selection, plus the passphrase, admin, and hostname byte buffers and the keygen stage (`src/state.rs:1`).
`Context::new` starts the step at `0`, every selection index at `0`, and `ctx.privacy` at `0b0000_0011`
(`src/state.rs:39`, `src/state.rs:59`).

## Bring-up

`_start` initialises the heap, then calls `setup::run`; a heap failure exits with code 1 and a bring-up
failure exits with code 2 (`src/main.rs:17`, `src/main.rs:22`). `setup::run` does the following, in order,
each step returning its own error string on failure (`src/setup/mod.rs:18`):
...
1. Look up the `compositor` and `input_router` ports, both required, and the `policy` port, optional; a
   ...
   (`src/setup/discover.rs:22`, `discover.rs:26`, `discover.rs:30`, `discover.rs:12`).
2. Health-check the compositor (`mod.rs:22`, `src/clients/compositor/healthcheck.rs:21`).
3. Query the display geometry; a zero width, height, or stride in the reply is rejected
   (`mod.rs:23`, `src/clients/display_info.rs:34`, `display_info.rs:48`).
...
6. Fill the backdrop, submit the surface to the compositor at overlay Z, and commit the first damage
   ...
   `mod.rs:67`, `src/setup/fill.rs:17`).

On success it returns a `Context` carrying the surface and the three ports (`mod.rs:71`).

## The key loop

`server::runner::run` takes the `Context` and never returns (`src/server/runner.rs:12`). It subscribes to
...
underflows or overshoots (`src/server/step.rs:14`, `step.rs:1`). `default_key` turns Enter (`0x0D` or
...
list screen calls `list_nav` first (`src/render/screens/language.rs:16`); a masked-field screen matches
Backspace and the printable range itself before falling back (`src/render/screens/passphrase.rs:28`); the
progress and toggle screens handle Enter or Space specially (`src/render/screens/keygen.rs:24`,
`src/render/screens/privacy.rs:28`).
...
(`src/render/mod.rs:22`, `mod.rs:31`, `mod.rs:32`). The content column and the card start at 38 percent of
the display width (`src/render/mod.rs:18`, `mod.rs:29`). `chrome::panel` draws the wordmark, the
`FIRST-BOOT SETUP` label, and the step list, marking each row done, current, or pending against `ctx.step`
(`src/render/chrome.rs:11`, `chrome.rs:18`, `chrome.rs:26`). The palette and the `STEP_LABELS` list live in
the theme (`src/render/theme.rs:1`, `theme.rs:15`).

Below the frame each screen draws its own widget at the content column: a single-select list
(`src/render/widgets/rows.rs:6`), a toggle list (`src/render/widgets/toggles.rs:6`), a masked field with a
strength bar (`src/render/widgets/field.rs:6`), or a task-and-progress bar
(`src/render/widgets/progress.rs:6`). The primitives underneath are `fill_rect` and a vertical gradient
built by lerping ARGB per row (`src/render/paint.rs:1`, `paint.rs:33`).

## The wire

The input-router wire is defined in `src/protocol.rs`. A request carries a 20-byte header with magic
`0x4E49_5253` ("NIRS") and version 1, followed by the body (`src/protocol.rs:3`, `protocol.rs:15`). A key
...
event fields (`src/protocol.rs:10`, `protocol.rs:27`, `protocol.rs:32`, `protocol.rs:36`). The
compositor and policy wires belong to their own clients and are covered on the
[policy writes page](policy-writes.md) and in the source map below.
