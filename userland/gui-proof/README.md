...
surface through the shared [app-skeleton](../writing-an-app.md). If it opens, draws its label, and counts
...
| Field | Value | Source |
|---|---|---|
| Binary name | `gui_proof` | `userland/capsule_gui_proof/Cargo.toml:15`, `Cargo.toml:16` |
| Crate name | `nonos_capsule_gui_proof` | `Cargo.toml:8` |
| License | `AGPL-3.0` | `Cargo.toml:11` |
| Window title | `std GUI proof` | `src/app.rs:39` |
| Window id | `0x53544447` | `src/app.rs:40` |
| Window kind | `Normal` | `src/app.rs:41` |
| Initial rect | `160,140 360x200` | `src/app.rs:42`, `app.rs:43`, `app.rs:44`, `app.rs:45` |
| Input mask | `0x21` (KeyDown, ButtonDown) | `src/app.rs:46` |
...
(`src/app.rs:46`, matched against `InputKind` in `userland/app_skeleton/src/input/kind.rs:20`,
`kind.rs:25`). The skeleton widens that to also deliver KeyUp, the pointer, wheel, button-up, and touch
before it subscribes the window (`userland/app_skeleton/src/setup/input_mask.rs:27`).

## What the proof exercises

The self-test is the `GuiProof` type in `src/app.rs`, a two-field struct wrapped by the `App` trait. The
...
|---|---|---|
| State | `src/app.rs:23`, `app.rs:28` | A `nonos_std::collections::HashMap<u32, &str>` seeded with one label and a `u32` click counter. |
| Event | `src/app.rs:50` | Button-down increments the counter and asks for a repaint; Escape closes; everything else is idle. |
| Paint | `src/app.rs:61` | Clears the surface, fills a header bar, draws the HashMap label and a `format!`-built count line. |

The [self-test page](self-test.md) walks each of these in turn and states exactly which `nonos_std`
...
The skeleton owns the runtime. `_start` calls `run(app::GuiProof::new)` (`src/main.rs:28`), which
initializes the heap, resolves the desktop peers (`compositor`, `wm`, `input_router`, `toolkit`) by name,
and enters its loop (`userland/app_skeleton/src/runner/entry.rs:31`,
`userland/app_skeleton/src/discover/require.rs:24`). On each delivery it builds a fresh `GuiProof`, opens
the window from the manifest, primes the first frame, and runs the event-and-tick loop
(`userland/app_skeleton/src/runner/boot.rs:39`). The app declares no tick, so it repaints only when
`on_event` returns `Repaint`, which is on every click. Escape returns `Close`, and the skeleton tears the
window down. The [debugging](debugging.md) page covers what to check when the window does not appear.
...
[contributing](contributing.md) describes what that would take.
