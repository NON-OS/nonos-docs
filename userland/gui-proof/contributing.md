# Contributing to the GUI Proof Capsule
...
- `main.rs` is the entry point. `_start` calls `run(app::GuiProof::new)` and nothing else
  (`src/main.rs:28`). You should rarely touch this.
- `app.rs` is the self-test: the `GuiProof` struct, its `new`, and the `App` impl with `manifest`,
  `on_event`, and `paint` (`src/app.rs:23`, `app.rs:29`, `app.rs:36`).

Everything the window can do, open, subscribe to input, paint, close, comes from the shared
[app-skeleton](../writing-an-app.md) under `userland/app_skeleton/`. Read that page before changing how
...
- To change what the proof asserts about `nonos_std`, edit the state and paint together in `app.rs`. If
  ...
  read it back in `paint` so the result is visible. The rule from the [self-test page](self-test.md)
  holds: the assertion has to show up in a drawn frame, because a drawn frame is how this proof reports.
- To change the window, edit the `AppManifest` returned by `manifest` (`src/app.rs:37`): title, initial
  rect, and the `input_kind_mask`. If you add an input kind, add its bit to the mask (bit 0 KeyDown, bit 5
  ButtonDown today, `src/app.rs:46`) and handle it in `on_event`, or it will be delivered and ignored.
...
build. It is `#![no_std]` and `#![no_main]` with `panic = "abort"` (`src/main.rs:17`, `src/main.rs:18`,
...
at the top of every file (`src/app.rs:1`).
