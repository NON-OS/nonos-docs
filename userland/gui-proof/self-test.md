...
This page reads beside `userland/capsule_gui_proof/src/app.rs`, the one file that holds the whole
self-test. `main.rs` only hands the app to the skeleton (`src/main.rs:28`); the assertion lives here. The
...
`GuiProof` has two fields: a `HashMap<u32, &'static str>` and a `u32` click counter (`src/app.rs:23`,
`app.rs:24`). The map is the whole point. `new` builds it with `HashMap::default()` and inserts one entry,
key `0` mapping to the label `"nonos_std drives this GUI"` (`src/app.rs:29`, `app.rs:31`). That
`HashMap` comes from `nonos_std::collections` (`src/app.rs:20`), so constructing it, seeding it, and later
...
The counter is an ordinary `u32` starting at zero (`src/app.rs:32`). It exists to give the paint step a
...
`on_event` is a three-arm match on the incoming `InputKind` (`src/app.rs:50`). A `ButtonDown` increments
the click counter and returns `EventOutcome::Repaint`, which tells the skeleton to call `paint` again
(`src/app.rs:52`). A `KeyDown` whose code is `KEY_ESC` returns `EventOutcome::Close`, which tears the
window down (`src/app.rs:56`; `KEY_ESC` is `0x1B`, `userland/app_skeleton/src/input/keys.rs:20`).
Everything else returns `EventOutcome::Idle` and changes nothing (`src/app.rs:57`).

These are the only two input kinds the manifest subscribes to, bit 0 and bit 5 of the input mask
(`src/app.rs:46`). The `InputKind` values matched here, `ButtonDown = 5` and `KeyDown = 0`, are the same
enum the router delivers (`userland/app_skeleton/src/input/kind.rs:20`, `kind.rs:25`). Handling a real
...
`paint` is where the assertion becomes visible (`src/app.rs:61`). It runs four draw calls on the
skeleton's `PaintBuffer`:

- `fb.clear(0xFF101820)` fills the surface with the dark background (`src/app.rs:62`).
- `fb.fill_rect(20, 20, 320, 60, 0xFF2A7FFF)` draws the blue header bar (`src/app.rs:63`;
  bounds-clamped in `userland/app_skeleton/src/paint/fill_rect.rs:20`).
- `fb.text(34, 44, label.as_bytes(), 0xFFFFFFFF)` draws the label read back out of the HashMap
  (`self.labels.get(&0)`), white on the bar (`src/app.rs:64`, `app.rs:65`). Reading the value back through
...
- `fb.text(34, 120, line.as_bytes(), 0xFFCFE8FF)` draws the click line, where `line` was built with
  `nonos_std::format!("clicks: {}", self.clicks)` (`src/app.rs:66`, `app.rs:67`). Producing that string is
  the `format` half of the proof.

Both text calls go through the toolkit font renderer (`userland/app_skeleton/src/paint/text.rs:22`), and
...
the [debugging](debugging.md) page covers how to read one.

## Source map

Everything here is drawn from `userland/capsule_gui_proof/src/app.rs` and `src/main.rs`, and from the app
