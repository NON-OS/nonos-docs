...
[README](README.md); the mechanics of what each draw call proves are on the [self-test page](self-test.md).
This page is the checklist for when the window is wrong or missing.

## What a pass looks like

A pass is a single small window titled `std GUI proof` (`src/app.rs:39`), opened at `160,140` and sized
`360x200` (`src/app.rs:42`, `app.rs:43`, `app.rs:44`, `app.rs:45`). Inside it: a dark background, a blue
header bar across the top, the white label `nonos_std drives this GUI` on the bar, and a lighter line
reading `clicks: 0` below it (`src/app.rs:62`, `app.rs:63`, `app.rs:65`, `app.rs:67`). Click anywhere in
the window and the count rises by one on the next frame; every click increments the counter and asks the
skeleton for a repaint (`src/app.rs:53`, `app.rs:54`). Press Escape and the window closes
(`src/app.rs:56`). That whole behaviour is the assertion: the label came out of a `nonos_std` `HashMap`,
...
There is no serial line to grep for. Unlike the [input proof](../input-proof/README.md), this capsule
...
that cannot exist yet. [Contributing](contributing.md) describes the `Capsule.mk` and kernel mirror this
would take.
...
(`src/main.rs:28`), and `run` does two things before any window can open: it initializes the heap, and it
resolves the desktop peers by name (`userland/app_skeleton/src/runner/entry.rs:32`, `entry.rs:36`). If the
...
error and the skeleton exits with code `2` (`userland/app_skeleton/src/discover/require.rs:42`,
...
window (`userland/app_skeleton/src/runner/boot.rs:44`, `boot.rs:45`). If `open_window` fails, `boot`
...
once. `paint` first clears to the dark background and fills the header bar (`src/app.rs:62`, `app.rs:63`),
so a window that is all dark with no bar means even the fill did not reach the surface. A window with the
bar but no text means the two `fb.text` calls did not render (`src/app.rs:65`, `app.rs:67`), which points
at the toolkit font path (`userland/app_skeleton/src/paint/text.rs:22`) rather than at this capsule, since
the label and the count go through the same call. The label specifically comes from
`self.labels.get(&0)` (`src/app.rs:64`); if the `HashMap` read failed it would fall back to an empty
...
arriving. `on_event` only increments on `InputKind::ButtonDown` (`src/app.rs:52`), and the manifest
subscribes to just two input bits, KeyDown and ButtonDown (`src/app.rs:46`). A count that never moves means
...
`on_event` returns `Close` only on a delivered `KeyDown` with code `KEY_ESC` (`src/app.rs:56`), so if
Escape does nothing, key events are not reaching the surface either. The dedicated harness for that whole
delivery path is the [input proof](../input-proof/README.md); if both keys and clicks are dead here, debug
it there.
...
Everything here is drawn from `userland/capsule_gui_proof/src/app.rs` and `src/main.rs`, the capsule's own
