# Contributing to capsule_market
...
[README](README.md), the [protocol](protocol.md) page, the [verification](verification.md) page, and the
[readiness](readiness.md) page.
...
by reference to `server::run`, which never returns (`src/main.rs:41`, `src/main.rs:46`, `src/main.rs:49`).
The seven top-level modules are declared there (`src/main.rs:22`).
...
1. Add the op discriminant in `src/protocol/ops.rs:17`. Op numbers are the only routing key, so a new op is
...
2. Write the handler as its own module under `src/server/handlers/`, following the existing shape: pull the
   ...
   `src/server/handlers/get_app/handle.rs:28` is the shortest full example of a data reply, and
   `src/server/handlers/health.rs:20` is the shortest example of a status-only reply. Re-export the handler
   from `src/server/handlers/mod.rs:17`.

3. Wire the op into the dispatch match in `src/server/runner.rs:57`. An unmatched op already falls to the
...
(`src/verify/trait_def.rs:23`) and a matching `#[cfg]` arm in `src/main.rs:34`; do not verify signatures
inline in a handler. See the [verification](verification.md) page for the trait contract.
