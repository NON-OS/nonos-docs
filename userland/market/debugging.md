# Debugging capsule_market

This page lists the log markers the market and its boot path emit, and the concrete failure modes with
where to look for each. For the shell of the design read the [README](README.md), the [protocol](protocol.md)
page, the [verification](verification.md) page, and the [readiness](readiness.md) page.
...
(`src/userspace/init/capsule_boot/run.rs:29`). If that line is absent the capsule never started, and the
...
load_elf_executable error:` tag on an ELF load failure (`src/security/market_capsule/spawn.rs:57`).
...
(`src/userspace/init/spawn_plan/core.rs:42`).
...
this one errno (`src/server/handlers/load_index.rs:42`, `load_index.rs:43`). But in an `offline-verify`
development build every index is refused by `RejectAll` no matter what its signature is
(`src/verify/reject_all.rs:22`), so `E_KEYREJECTED` on a build you expected to accept a valid index is the
first thing to check against the compile-time feature (`src/main.rs:37`, `Cargo.toml:29`). See the
[verification](verification.md) page for the real-verifier-versus-stub swap.
...
(`load_index.rs:41`, `src/ingest/load/load_verified.rs:31`). The very first load runs against a stored
...
needs one length-prefixed string (`src/server/handlers/get_app/handle.rs:35`), and `get_release` and
`install_ready` need a well-formed `(listing_id, release_id)` pair
(`src/server/handlers/get_release/handle.rs:31`). An unknown op also returns `E_INVAL` from the loop's
fall-through arm (`src/server/runner.rs:64`).

### E_NODATA on any query

Either no index has been accepted yet, so `store.current()` is `None`
(`src/server/handlers/install_ready/handle.rs:29`), or the named listing or release is not in the accepted
index (`src/server/handlers/get_release/handle.rs:41`). Check that a `LOAD_INDEX` succeeded before
...
before dispatch (`src/server/runner.rs:51`). On the reply side, a body that does not fit the transmit slot
returns `E_MSGSIZE` from the handler, for example when `list_apps` assembles more than the buffer holds
(`src/server/handlers/list_apps/handle.rs:49`).

### An install that looks ready but the installer refuses it

`INSTALL_READY` returns six independent bytes (`src/server/handlers/install_ready/handle.rs:46`); read them
...
hide. The [readiness](readiness.md) page has the full byte layout and how each field is computed. Remember
