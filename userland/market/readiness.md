# Install readiness and the store
...
For where the verdict is requested on the wire, see `INSTALL_READY` on the [protocol](protocol.md) page;
for how the signature flags it reads were set, see the [verification](verification.md) page.
...
(`src/store/state/accepted.rs:22`). There is exactly one accepted index at a time; a successful
`LOAD_INDEX` replaces the whole value (`src/store/state/install.rs:25`, `install.rs:31`).

Three reads back it:

- `current` returns the accepted index or `None`, which every query handler turns into `E_NODATA` when it
  is `None` (`src/store/state/current.rs:20`).
- `last_serial` returns the accepted index's serial, or `0` when the store is empty
  (`src/store/state/last_serial.rs:20`). The `0` is what makes the very first `LOAD_INDEX` pass the
...
- `publisher_signature_verified` flattens an `(entry, release)` index pair into the linear vector to look
  ...
  returning `false` for an out-of-range index (`src/store/state/publisher_signature_verified.rs:20`,
...
(`src/install_ready/checks.rs:23`). It returns an `InstallReadiness` whose six booleans the handler writes
as six bytes, in a fixed order (`src/server/handlers/install_ready/handle.rs:46`,
`install_ready/constants.rs:17`):
...
The fields are computed as follows (`src/install_ready/checks.rs:28`):
...
`compile_error!` (`src/install_ready/arch.rs:17`, `arch.rs:20`, `arch.rs:23`, `arch.rs:27`). This is what
...
the [verification](verification.md) pillar computed at ingest and the store holds. The other four,
