...
and reply. For the signature gate that guards `LOAD_INDEX` see the [verification](verification.md) page;
for the six-byte verdict that `INSTALL_READY` returns see the [readiness](readiness.md) page. Identity and
the capability mask live on the [README](README.md).

## The wire header

The wire is a fixed 20-byte header (`src/protocol/header.rs:20`). Every request and every reply begins with
it, in this layout:
...
`None` so the caller can reply `E_INVAL` (`src/protocol/decode.rs:20`, `decode.rs:24`, `decode.rs:28`). A
...
(`src/protocol/encode/encode_response_header.rs:19`).

Every reply, success or error, is the header followed by a 4-byte little-endian status word
(`src/protocol/encode/write_status.rs:17`, `STATUS_LEN = 4`, `src/protocol/limits.rs:19`). An error reply
is header plus status only (`src/server/error/reply_status.rs:23`); a data reply appends the body after the
status (`src/server/payload/reply_with_body.rs:23`). A status of `0` is success; a negative status is one
of the errnos below.

## The loop

`server::run` never returns (`src/server/runner.rs:32`). It allocates a receive buffer sized to the maximum
index blob plus 64 bytes and a 64 KiB transmit buffer (`runner.rs:33`, `src/protocol/limits.rs:20`,
...
2. Decode the header; on failure reply `E_INVAL` through `reply_decode_failed`, which synthesizes a
   ...
   `src/server/error/reply_decode_failed.rs:21`).
...
`mk_ipc_send` (`src/protocol/endpoint.rs:17`, `src/server/error/reply_status.rs:26`,
`src/server/payload/reply_with_body.rs:28`).
...
capability of its own. That path is the subject of the [verification](verification.md) page. The market
calls the installer and the payment service for nothing; those are separate capsules and separate concerns.

## The six ops

Six ops are defined (`src/protocol/ops.rs:17`), routed in the loop's match (`src/server/runner.rs:57`):
...
installs the returned index and replies status `0` (`src/server/handlers/load_index.rs:30`,
`load_index.rs:33`, `load_index.rs:38`). The verification pipeline it runs is the
[verification](verification.md) page in full; this handler is the mapping from its four ingest errors to
...
count (`list_apps/handle.rs:35`) followed, per entry, by the length-prefixed `listing_id`, the raw
`capsule_id`, the length-prefixed `name`, and a single readiness byte
(`list_apps/handle.rs:41`, `list_apps/handle.rs:44`). That byte is `1` only if at least one of the entry's
releases evaluates as install-ready (`list_apps/handle.rs:37`). If the assembled body does not fit the
reply slot the handler returns `E_MSGSIZE` (`list_apps/handle.rs:49`).

### GET_APP (op 3)

Requires an accepted index (`E_NODATA`) and a single length-prefixed `listing_id` in the body (`E_INVAL`
on a malformed length prefix) (`get_app/handle.rs:29`, `get_app/handle.rs:35`). The length prefix is
decoded by `read_lp_string`, which needs at least four bytes and a body that fits and is valid UTF-8
(`src/server/handlers/get_app/read_lp_string.rs:17`). An unknown listing is `E_NODATA`
(`get_app/handle.rs:39`). The reply carries the listing's id, capsule id, name, publisher name, publisher
pubkey, description, and release count (`get_app/handle.rs:42`), or `E_MSGSIZE` if it overflows the slot
(`get_app/handle.rs:52`).

### GET_RELEASE (op 4)

Requires an accepted index (`E_NODATA`) and a `(listing_id, release_id)` pair, each length-prefixed, parsed
by `parse_pair` (`E_INVAL` on a bad pair) (`get_release/handle.rs:25`, `get_release/handle.rs:29`,
`src/server/handlers/get_release/parse_pair.rs:19`). `parse_pair` reads two length-prefixed fields back to
back with `take_lp`, each requiring four bytes of prefix and a body that fits
(`src/server/handlers/get_release/take_lp.rs:17`). A pair that names no release is `E_NODATA`
(`get_release/handle.rs:41`). On a hit the reply is the encoded release record, which carries the release
id, both hashes, the package url, the publisher signature, the supported arches, the kernel-abi minimum,
the required capabilities, and the validation record (`get_release/handle.rs:43`,
`src/server/handlers/get_release/encode_release.rs:24`), or `E_MSGSIZE` on overflow
(`get_release/handle.rs:47`).
...
(`src/server/handlers/install_ready/handle.rs:26`, `install_ready/handle.rs:46`). The verdict and its
fields are the [readiness](readiness.md) page.

### HEALTHCHECK (op 6)

Replies status `0` with no body and touches no state (`src/server/handlers/health.rs:20`). It is a liveness
...
(`src/server/payload/body_slot.rs:19`). `body_slot` returns `None` if the body does not fit, which the
...
`mk_ipc_send` (`src/server/payload/reply_with_body.rs:23`). An error reply skips the body entirely and
sends the header plus a negative status through `reply_status`
(`src/server/error/reply_status.rs:23`).
