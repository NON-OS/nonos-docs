...
errors become errnos, see `LOAD_INDEX` on the [protocol](protocol.md) page.
...
accepting only when the return code is `0` (`src/verify/crypto.rs:26`, `crypto.rs:28`, `crypto.rs:37`). The
...
lives behind that boundary, not in this capsule (`Capsule.mk:2`, `src/security/market_capsule/spawn.rs:17`).

`RejectAll` is the stub. Its `verify` returns `Verdict::Refused` unconditionally, ignoring its arguments
(`src/verify/reject_all.rs:22`).
...
(`src/main.rs:34`, `src/main.rs:37`). The `RejectAll` module itself is compiled only under that feature
(`src/verify/mod.rs:18`, `mod.rs:23`), so a default build does not even contain the stub.
...
(`src/verify/reject_all.rs:22`, `src/ingest/load/load_verified.rs:42`). That is intentional. The feature
...
(`src/ingest/load/load_verified.rs:25`):
...
1. Decode the blob with the marketplace ABI. A decode failure is `Malformed`
   ...
   signature (`userland/marketplace_abi/src/codec/decode_index.rs:56`, `decode_index.rs:57`).
...
The four ingest errors (`src/ingest/error.rs:17`) are what `LOAD_INDEX` maps onto errnos. `SignatureRefused`
and `UntrustedOperator` both surface as `E_KEYREJECTED`, so a caller sees one errno for both "the key is not
trusted" and "the signature did not verify" (`src/server/handlers/load_index.rs:42`, `load_index.rs:43`).
...
equals one of the baked operators (`src/bootstrap_trust/check.rs:19`), and there is a single current
operator, `NOX_OPERATOR_V1`, a 32-byte Ed25519 public key (`src/bootstrap_trust/keys.rs:17`,
...
key (`src/ingest/load/verify_publisher_signatures.rs:25`). For each release the check is conservative: a
...
kernel-abi minimum, and the required capabilities, under the domain tag `NONOS.marketplace.release.v1`
(`userland/marketplace_abi/src/codec/release_signing.rs:28`, `release_signing.rs:30`).
...
nothing (`src/main.rs:37`, `src/verify/reject_all.rs:22`). Second, the publisher flags are computed once at
ingest and read back per query, not re-verified on each `install_ready` call
(`src/ingest/load/verify_publisher_signatures.rs:25`). Neither is an authority gap; both are properties of
