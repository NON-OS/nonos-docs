# Contributing to capsule_calculator
...
subscription, and paint loop (`userland/capsule_calculator/src/main.rs:27`). The single top-level module is
`calc`, which declares every submodule and re-exports `Calculator` (`src/calc/mod.rs:17`, `mod.rs:31`).
...
| `src/calc/op.rs`, `src/calc/unary.rs` | the binary and unary arithmetic, the only place the math lives | you change how a computation is done |
| `src/calc/state.rs`, `src/calc/fixed.rs` | the state machine and the fixed-point scale | you change the data model or the numeric precision |
...
| `src/calc/layout.rs`, `src/calc/theme.rs`, `src/calc/manifest.rs` | grid geometry and hit-test, the palette, the window manifest | you change the window size, colours, or input subscription |
...
1. Write the action module. Each operation is one file under `src/calc/actions/`, exposing a
   ...
   errored, the way `memory_recall` and `memory_clear` do (`src/calc/actions/memory_recall.rs:19`). Do the
   math with `checked_*`/`saturating_*` and set `state.error` on failure rather than panicking, the way
   `square` does (`src/calc/actions/square.rs:24`). If it is a pure numeric transform, put the math in
   `src/calc/unary.rs` and keep the action file thin, the way `square` calls `unary::square`
   (`src/calc/actions/square.rs:18`).
2. Register it. Add a variant to the `Action` enum (`src/calc/buttons/kinds.rs:28`), add a match arm in
   the dispatcher (`src/calc/actions/dispatch.rs:24`), and declare the module in `src/calc/actions/mod.rs:17`.
3. Give it a way in. Add a keyboard mapping in the classifier (`src/calc/event/key_classifier.rs:33`) and
   ...
   (`src/calc/buttons/mod.rs:27`, `src/calc/layout.rs:21`).
4. Pick a role for the button so it gets a colour. The five roles are `Number`, `Operator`, `Equals`,
   ...
   (`src/calc/buttons/kinds.rs:19`, `src/calc/paint/button.rs:29`).
...
- No panics in capsule code. No `unwrap`, `expect`, `panic!`, `todo!`, or `unimplemented!`. Every failure
  ...
  (`userland/capsule_calculator/Cargo.toml:26`).
- One unit per file. New operations are one `run` per file under `src/calc/actions/`, new keypad rows are
  ...
  (`src/calc/mod.rs`, `src/calc/actions/mod.rs`).
- No `unsafe` outside the unavoidable `_start` extern (`src/main.rs:27`).
- The AGPL header sits at the top of every source file, byte for byte the same as the header on every
  existing module (`src/calc/state.rs:1`).
