# The Calculator Capsule

The calculator is the model least-privilege NONOS application: a signed userland capsule that draws its
own window, reads pointer clicks and keys, runs a fixed-point arithmetic engine, and reaches nothing
outside its own surface. Where the [terminal](../terminal/README.md) reaches the filesystem, the network,
...
[debugging](debugging.md) page walks the consequences.
...
|------|---------|----------------|
| [input.md](input.md) | `src/calc/event/`, `src/calc/buttons/`, `src/calc/layout.rs` | The full input reference: the 6x5 keypad grid every button by row, the keyboard keymap byte by byte, the pointer hit-test, the router, and how both inputs land on the same dispatch. |
| [engine.md](engine.md) | `src/calc/actions/`, `src/calc/op.rs`, `src/calc/unary.rs`, `src/calc/fixed.rs`, `src/calc/state.rs`, `src/calc/format/` | The evaluation model: the fixed-point scale, the single-pending-operator machine, every operation and its checked arithmetic, the error latch, and number-to-text formatting. |
| [rendering.md](rendering.md) | `src/calc/paint/`, `src/calc/theme.rs` | How a frame is produced: the compose order, the display panel, the memory badge, the keypad grid, per-role button colours, and delivery to the compositor through the skeleton. |
| [contributing.md](contributing.md) | the whole tree | Where to work, how to add an operation, the build and sign steps for the `calculator` slug, and the code standards. |
| [debugging.md](debugging.md) | runtime | The one boot marker, why the capsule emits no serial of its own, and the concrete failure modes with where to look for each. |
...
an arithmetic action, and a `paint` that draws the frame (`src/main.rs:27`, `src/calc/app.rs:34`).
...
1. The kernel spawns the capsule at boot through the desktop fleet plan
   ...
   (`src/userspace/capsule_calculator/spawn.rs:37`, `src/userspace/init/spawn_plan/apps.rs:62`,
...
3. Each event flows in through `on_event`: a button-down goes to `on_pointer_button`, a key-down to
   `on_key`, and anything else is idle (`src/calc/event/router.rs:23`). A click hit-tests to a grid cell
   and runs its action; a key classifies to an action or is ignored (`src/calc/event/on_pointer_button.rs:24`,
   `src/calc/event/on_key.rs:23`). Esc returns `EventOutcome::Close`; any handled action returns
...
4. `paint` composes the frame in a fixed order: background, wordmark, the display value, the memory badge,
   then the keypad grid (`src/calc/paint/frame.rs:26`). The frame lands in the shared surface the
   compositor presents.
...
`src/userspace/capsule_calculator/spawn.rs:31`). It makes no outbound service calls of its own. Unlike the
...
presentation after a `Repaint` outcome (`src/main.rs:27`, `src/calc/manifest.rs:28`, `src/calc/app.rs:38`).
...
of a few integers (`src/calc/state.rs:28`); there is no file it reads, no file it writes, no socket, no
...
a panic (`src/calc/op.rs:29`, `src/calc/unary.rs:20`, `src/calc/state.rs:20`). The release profile is
...
[engine](engine.md) page traces this operation by operation.
