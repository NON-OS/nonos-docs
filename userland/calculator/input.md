...
and `src/calc/layout.rs` (the hit-test geometry). For the arithmetic each action drives, see the
[engine](engine.md) page; for the wider capsule see the [calculator overview](README.md).
...
key-down and goes to the key handler (`src/calc/event/router.rs:23`).
...
`GRID` (`src/calc/buttons/mod.rs:27`). Each `Button` carries a label drawn on screen, a visual `Role` that
picks its colour, and the `Action` it runs (`src/calc/buttons/kinds.rs:48`). This is the complete grid,
...
[rendering](rendering.md) page shows the colour each maps to (`src/calc/buttons/kinds.rs:19`).
...
dispatches its action (`src/calc/event/on_pointer_button.rs:24`). The bounds against `GRID.len()` and
...
`hit_test` is the geometry, in `src/calc/layout.rs:38`. The grid sits below the display panel, so a click
...
`GRID_TOP`) are all defined at the top of the same file (`src/calc/layout.rs:19`), and `cell_size` and
`cell_origin` are the shared functions the [renderer](rendering.md) uses to draw the same grid the
...
(`src/calc/event/key_classifier.rs:26`). The classifier returns one of three outcomes, `Close`, `Action`,
or `Ignored`, and `on_key` turns those into `EventOutcome::Close`, a dispatch plus `Repaint`, or `Idle`
(`src/calc/event/on_key.rs:23`). The full map:
...
(`src/calc/actions/dispatch.rs:24`). A pointer click reads the cell's `action` and calls it
...
each handler does to the arithmetic engine is the subject of the [engine](engine.md) page.
