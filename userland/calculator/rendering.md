...
produced on demand, whenever an event handler returns `EventOutcome::Repaint` (`src/calc/event/on_key.rs:29`,
`src/calc/event/on_pointer_button.rs:37`). This page follows `src/calc/paint/` file by file and
`src/calc/theme.rs` for the palette. For the state a frame projects, see the [engine](engine.md) page; for
the overview see the [README](README.md).

## The compose order

`paint` is the frame recipe, a fixed sequence of five draws in one order (`src/calc/paint/frame.rs:26`):

| Step | What it draws | Source |
|---|---|---|
| `background::paint` | clear the surface to the theme background | `frame.rs:27`, `src/calc/paint/background.rs:21` |
| `wordmark::paint` | the `NONOS calc` wordmark, top-left | `frame.rs:28`, `src/calc/paint/wordmark.rs:24` |
| `display::paint` | the display panel and the current value or `Error` | `frame.rs:29`, `src/calc/paint/display.rs:28` |
| `memory_badge::paint` | the amber `M` badge, only when memory is engaged | `frame.rs:30`, `src/calc/paint/memory_badge.rs:25` |
| `grid::paint` | the 6x5 keypad, cell by cell | `frame.rs:31`, `src/calc/paint/grid.rs:23` |
...
(`src/calc/paint/background.rs:22`, `src/calc/theme.rs:17`). `wordmark::paint` draws the fixed text
`NONOS calc` at the top-left inset by `PADDING`, in the dim `WORDMARK` green `0xFF1F4020`, a quiet brand
mark above the display (`src/calc/paint/wordmark.rs:24`, `theme.rs:35`).

## The display panel

`display::paint` draws the readout (`src/calc/paint/display.rs:28`). It fills a `DISPLAY_H`-tall panel in
...
`src/calc/format/` and is covered in the [engine](engine.md) page.

## The memory badge

`memory_badge::paint` returns immediately unless `state.memory_engaged()` is true, so the badge is drawn
only when the memory register is non-zero (`src/calc/paint/memory_badge.rs:26`). When it is, it draws a
single `M` glyph near the top-left in the `MEMORY_INDICATOR` amber `0xFFD8B45F`, a small persistent hint
that memory holds a value (`memory_badge.rs:29`, `src/calc/theme.rs:22`).

## The keypad grid

`grid::paint` walks the same static `GRID` the [input](input.md) path hit-tests, so the drawn geometry and
the hot regions come from one source (`src/calc/paint/grid.rs:23`). It reads the cell size once from
`cell_size()`, then for every row and column it computes the cell origin with `cell_origin()` and hands the
button to `button::paint` (`grid.rs:24`). Both `cell_size` and `cell_origin` live in `src/calc/layout.rs`,
the same file `hit_test` uses, which is why a click always lands on the button drawn under it
(`src/calc/layout.rs:26`, `layout.rs:32`).

`button::paint` draws one cell (`src/calc/paint/button.rs:28`). It picks a background and foreground colour
...
greens. All of these are fixed ARGB words in `src/calc/theme.rs:17`.

## The palette

The whole colour scheme is one file, `src/calc/theme.rs:17`, a phosphor-green desk-calculator look. The
...
```
  ...
  src/calc/paint/wordmark.rs      the NONOS calc wordmark
