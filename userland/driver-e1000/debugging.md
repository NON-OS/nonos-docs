# Debugging capsule_driver_e1000

This page lists the log marker the driver's boot path emits, the setup exit codes, and the concrete runtime
failure modes with where to look for each. For the shape of the driver see the [README](README.md), the
[operations](operations.md) page, the [bring-up](bring-up.md) page, and the [queues](queues.md) page.
...
(`src/userspace/init/spawn_plan/drivers_nic.rs:26`), whose `Ok` arm calls `boot_log::ok(prefix, "capsule
spawned")` (`src/userspace/init/capsule_boot/run.rs:29`), which formats `[` + tag + `] ` + message
...
(`src/userspace/init/capsule_boot/run.rs:32`). The spawn is feature-gated, so a build without
`nonos-capsule-driver-e1000` compiles the spawn out entirely (`src/userspace/init/spawn_plan/drivers_nic.rs:33`).

## Setup exit codes

If the capsule spawns but `setup::run` or `init::bring_up` fails, the process exits with one of three codes,
which is the fastest way to tell how far bring-up got (`src/main.rs:33`).

| Exit | Constant | Meaning and where it comes from |
|---|---|---|
| 1 | `EXIT_HEAP_INIT` | The capsule heap failed to initialise before any device work (`src/main.rs:39`). |
| 2 | `EXIT_SETUP_FAILED` | The broker handshake failed: no e1000 device matched, the claim was refused, the BAR0 map failed, the IRQ bind failed, or a DMA grant was refused (`src/main.rs:43`, `src/setup/sequence.rs:31`). |
| 3 | `EXIT_BRING_UP_FAILED` | The hardware bring-up failed: the reset bit never self-cleared or an EEPROM word never signalled done. Every grant is released before the exit (`src/main.rs:46`, `src/init/run.rs:26`). |
...
unambiguous: `"no e1000 device"` (discovery found no matching Intel NIC, `src/setup/sequence.rs:32`),
`"claim failed"` (`mk_device_claim` refused, usually a missing `Driver` capability or a device already
claimed, `src/setup/claim.rs:24`), `"mmio map failed"` (`src/setup/mmio.rs:33`), `"irq bind failed"`
(`src/setup/irq.rs:29`), or one of the four `"dma map failed (...)"` strings naming the RX ring, RX buffers,
TX ring, or TX buffers (`src/setup/dma.rs:53`). Discovery requires an Intel vendor id, a matching e1000
device id, network/Ethernet class, a real interrupt line, and an MMIO BAR0 (`src/discover.rs:43`), so a NIC
...
wrong BAR mapped (`src/init/reset.rs:40`). `eeprom::read_mac` returns `"EERD did not signal DONE"` if an
EEPROM word read never completes, which on real hardware usually means the part uses a different EEPROM access
path than the direct `EERD` register the driver drives (`src/init/eeprom.rs:51`).
...
`OP_LINK_STATUS` returns a single byte, `0` when the live `STATUS.LU` bit is clear (`src/server/handlers/link_status.rs:36`).
A persistent `0` means the PHY has not reported link up. Bring-up sets `CTRL.SLU` and `CTRL.ASDE` to set the
link and let the device auto-negotiate (`src/init/reset.rs:49`), so a down link after that is a cable, peer,
or auto-negotiation issue rather than a driver-state issue. Confirm with `OP_STATS`, which returns the live
`STATUS` register as its first word (`src/server/handlers/stats.rs:41`); the link-up bit there and the
...
(`src/queue/rx.rs:49`, `src/server/handlers/rx_packet.rs:31`). A steady stream of `E_AGAIN` while the link is
...
receiver with broadcast-accept and clears the multicast table (`src/init/rx_setup.rs:45`,
`src/init/mac_filter.rs:39`), so unicast to the card's own MAC and broadcast should be accepted.
...
the NIC and reports the error rather than copying a malformed frame out (`src/queue/rx.rs:59`,
`src/server/handlers/rx_packet.rs:35`). This is a device-side or wire-side error, distinct from `E_AGAIN`,
...
one-million-spin budget (`src/server/handlers/tx_packet.rs:50`, `src/queue/tx.rs:67`). On QEMU this is rare.
...
rather than waiting on the interrupt (the IRQ is bound but not serviced, [queues](queues.md)), a missing
...
1514 (`src/server/handlers/tx_packet.rs:31`, `src/constants/frame.rs:26`). `E_MSGSIZE` (`-90`) means the
declared `payload_len` did not match the number of bytes the request actually carried, caught either by the
loop's envelope check or the handler's re-check (`src/server/runner.rs:54`, `src/server/handlers/tx_packet.rs:27`).
