# Client operations and the NE10 protocol
...
[README](README.md); for how the frame handlers reach the rings see the [queues](queues.md) page.

## The wire format

A request header is 20 bytes and begins with a magic and a version (`src/protocol/header.rs:17`). The
decoder rejects anything shorter than the header, a wrong magic, or a wrong version, returning `None` so the
loop answers `E_INVAL` (`src/protocol/decode.rs:19`, `src/server/runner.rs:49`).
...
little-endian status word, then any payload (`src/protocol/header.rs:20`, `src/protocol/limits.rs:24`). The
response header echoes the request's op, flags, and request_id so the kernel client can match the reply, and
carries the reply's payload length (`src/protocol/encode.rs:24`). Status `0` means success; a negative
status is one of the errno constants below. Replies go to the kernel reply endpoint `0x1_0000_000C` with
`mk_ipc_send` (`src/server/error.rs:30`, `src/protocol/endpoint.rs:22`).
...
the largest received frame or the stats snapshot (`src/server/runner.rs:34`). The loop blocks on
`mk_ipc_recv`, skips a receive of zero or fewer bytes, decodes the header, and dispatches
(`src/server/runner.rs:42`). A decode failure answers `E_INVAL` without touching the device
(`src/server/runner.rs:50`).
...
(`src/server/runner.rs:54`). Only then is the body sliced and the op routed. An unrecognised opcode is
answered with `E_INVAL` (`src/server/runner.rs:67`).

## The six operations

The opcodes are defined in `src/protocol/ops.rs:23` and dispatched in `src/server/runner.rs:60`.

| Op | Opcode | Request payload | Reply payload after status | Handler |
|---|---|---|---|---|
| `OP_HEALTHCHECK` | `1` | none | none (status only) | `src/server/handlers/health.rs:22` |
| `OP_LINK_STATUS` | `2` | none | 1 byte, `1` up / `0` down | `src/server/handlers/link_status.rs:32` |
| `OP_MAC_ADDRESS` | `3` | none | 6 bytes, the hardware MAC | `src/server/handlers/mac_address.rs:29` |
| `OP_TX_PACKET` | `4` | one Ethernet frame | none (status only) | `src/server/handlers/tx_packet.rs:26` |
| `OP_RX_PACKET` | `5` | none | 4-byte length prefix then the frame | `src/server/handlers/rx_packet.rs:27` |
| `OP_STATS` | `6` | none | 48-byte register/ring snapshot | `src/server/handlers/stats.rs:27` |

The kernel-side network client calls only ops 1 through 5 (`src/hardware/e1000_capsule/protocol/ops.rs:21`);
`OP_STATS` is a capsule-side telemetry op with no kernel caller yet, noted on the [README](README.md).
...
- `OP_LINK_STATUS` reads the live `STATUS.LU` bit and returns one byte, `1` if link is up and `0` if down.
  ...
  (`src/server/handlers/link_status.rs:36`, `src/constants/status.rs:22`).
- `OP_MAC_ADDRESS` returns the six bytes the driver cached from the EEPROM at bring-up
  (`src/server/handlers/mac_address.rs:33`; read at `src/init/eeprom.rs:28`). The kernel client treats an
...
- `OP_TX_PACKET` copies the request frame into the next TX buffer, posts a descriptor, rings the tail
  ...
  descriptor never completes within the poll budget (`src/server/handlers/tx_packet.rs:38`; the ring is on
  the [queues](queues.md) page).
- `OP_RX_PACKET` consumes the next completed receive descriptor. On a good frame it writes a 4-byte
  ...
  length) it recycles the descriptor and replies `E_IO` (`src/server/handlers/rx_packet.rs:28`,
  `src/queue/rx.rs:47`).
- `OP_STATS` reads seven live registers (`STATUS`, `RCTL`, `TCTL`, `RDH`, `RDT`, `TDH`, `TDT`) and appends
  ...
  packing twelve little-endian `u32` values into the 48-byte record (`src/server/handlers/stats.rs:38`,
  `src/protocol/limits.rs:29`). It is side-effect-free.

## The error set

All errno words are little-endian negatives (`src/protocol/errno.rs`):
...
(`src/server/handlers/tx_packet.rs:27`, `src/constants/frame.rs:26`). Only a frame inside that window is
...
rejects anything malformed with `E_INVAL` (`src/protocol/decode.rs:19`). The envelope-length check means a
client cannot under- or over-declare its payload (`src/server/runner.rs:54`), and the transmit handler bounds
the frame size to the legal Ethernet window before it copies anything into the DMA buffer
(`src/server/handlers/tx_packet.rs:31`). The receive path never trusts the device length: it clamps against
`MAX_ETHERNET_FRAME` and drops any frame the hardware flagged with an error before it leaves the capsule
(`src/queue/rx.rs:59`). There is no panic path: the crate is `panic = "abort"` and every handler returns an
