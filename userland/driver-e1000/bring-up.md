...
(`src/setup/sequence.rs:31`), and `init::bring_up` programs the hardware against those grants
(`src/init/run.rs:26`). This page mirrors both together with the folders they lean on: `src/setup/` (the
grant handshake), `src/discover.rs` (finding the device), `src/init/` (reset, link, MAC, filter, ring
programming), `src/regs.rs` (register access), and `src/constants/` (the offsets and bit definitions). The
descriptor rings the RX/TX steps program live on the [queues](queues.md) page; the broker syscalls
...
maps to a distinct exit code, so a partly built driver never serves IPC (`src/main.rs:42`).

## Stage one: the broker handshake

`setup::run` is the ordered grant sequence (`src/setup/sequence.rs:31`).
...
1. **Discover.** `find_e1000` calls `mk_device_list` and returns the first record whose vendor is Intel
   ...
   network/Ethernet (`02/00`) (`src/discover.rs:32`, `src/discover.rs:55`, `src/constants/pci.rs:17`). The
   ...
   (`src/discover.rs:43`). No match returns `None`, which the sequence turns into `"no e1000 device"`.
2. **Claim.** `claim::claim` calls `mk_device_claim` on that device id and returns the broker's per-claim
   epoch (`src/setup/claim.rs:22`). The epoch is the token every later broker call presents, so a stale or
...
3. **Map BAR0.** `mmio::map` page-rounds the BAR0 size and calls `mk_mmio_map` for BAR index 0, returning a
   ...
   clean (`src/setup/mmio.rs:29`, `src/constants/pci.rs:25`). `Regs::new` wraps the returned address for
   volatile 32-bit register access (`src/regs.rs:31`). See [MMIO grants](../../subsystems/hardware-broker/mmio.md).
4. **Bind the interrupt.** `irq::bind` calls `mk_irq_bind` with the device's INTx line and returns a grant id
   ...
   (`src/setup/irq.rs:26`). MSI-X migration is a separate slice. Note that the grant is taken and released but
   ...
   [README](README.md) discrepancy note). See [IRQ grants](../../subsystems/hardware-broker/irq.md).
5. **Allocate the four DMA regions.** `dma::map_rings_and_buffers` takes four grants in order: the RX
   ...
   through `mk_dma_map` (`src/setup/dma.rs:47`, `src/constants/queue.rs:22`). Each region returns a user VA
   ...
   holds a partial setup (`src/setup/dma.rs:53`, `src/setup/rollback.rs:25`). See
...
accessor, the cached MAC, and the RX and TX ring cursors (`src/setup/driver.rs:28`).
...
address filter, RX ring, TX ring (`src/init/run.rs:26`).
...
1. **Reset and link.** `reset::run` sets `CTRL.RST` and spins until the self-clearing bit drops or a generous
   ...
   to set the link up and let the device auto-negotiate speed and duplex (`src/init/reset.rs:30`,
   `src/constants/status.rs:17`).
2. **EEPROM MAC.** `eeprom::read_mac` reads the first three 16-bit EEPROM words through the `EERD` register,
   ...
   little-endian (`src/init/eeprom.rs:28`, `src/constants/status.rs:36`). A word that never signals done
...
3. **Receive-address filter.** `mac_filter::program` writes the MAC into the primary receive-address pair
   ...
   multicast traffic (`src/init/mac_filter.rs:28`, `src/constants/regs.rs:47`).
4. **RX ring.** `rx_setup::program` zeros the descriptor ring, primes every slot with its buffer device
   ...
   (`src/init/rx_setup.rs:29`). The ring mechanics are on the [queues](queues.md) page.
5. **TX ring.** `tx_setup::program` zeros the descriptor ring, programs the base, length, head, and tail,
   ...
   (`src/init/tx_setup.rs:33`, `src/constants/status.rs:29`).
...
|---|---|---|
| Device claim | the exclusive hold on the PCI function; the root of every other grant | `src/setup/claim.rs:22` |
| MMIO | BAR0 mapped into the capsule as a user VA | `src/setup/mmio.rs:32` |
| IRQ | the device INTx line bound to a broker slot (held, not serviced) | `src/setup/irq.rs:28` |
| DMA RX ring | the receive descriptor ring, with a device address | `src/setup/dma.rs:53` |
| DMA RX buffers | the receive buffer pool | `src/setup/dma.rs:57` |
| DMA TX ring | the transmit descriptor ring, with a device address | `src/setup/dma.rs:61` |
| DMA TX buffers | the transmit buffer pool | `src/setup/dma.rs:65` |

The capsule programs the controller only with the broker-issued device addresses, never a physical address
it chose (`src/init/rx_setup.rs:40`, `src/init/tx_setup.rs:42`).

## Grant teardown

`Driver::release` drops every grant in the reverse order setup took it: the TX buffer, TX ring, RX buffer,
and RX ring DMA regions, then the IRQ, the MMIO grant, and finally the device claim (`src/setup/driver.rs:49`).
Each broker call is best-effort: an error from a doubly-dropped grant is harmless because the broker has
already revoked it. `release` runs on an early bring-up failure (`src/main.rs:47`); the server loop never
returns, and the kernel also revokes every grant tied to the claim when the process dies, so a crash cannot
leak a claim, a mapping, or a DMA buffer (see [revocation](../../subsystems/hardware-broker/revocation.md)).
...
claimed and cannot use a stale claim (`src/setup/claim.rs:22`;
[claim.md](../../subsystems/hardware-broker/claim.md)). The MMIO grant is exactly BAR0 of that function at a
broker-chosen user address, so the driver never sees a physical address or another device's registers
(`src/setup/mmio.rs:32`). Each of the four DMA regions is a separate grant with its own device address
returned by the broker, and the controller's ring bases are programmed only with those addresses
(`src/setup/dma.rs:47`, `src/init/rx_setup.rs:40`). Every grant is released on an early exit and again by the
kernel on process death (`src/setup/driver.rs:49`).
