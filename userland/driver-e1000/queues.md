...
them. How the rings are first allocated and enabled is on the [bring-up](bring-up.md) page; the client ops
that call into the rings are on the [operations](operations.md) page.

## The descriptor layout

Both descriptor types are the legacy 16-byte 8254x form, `repr(C)` with a compile-time size assertion so a
field reorder cannot silently change the wire layout the device reads (`src/queue/layout.rs:46`). The
...
and a special field (`src/queue/layout.rs:23`). A transmit descriptor is a 64-bit buffer address, a 16-bit
length, a checksum-offset byte, a command byte, a status byte, a checksum-start byte, and a special field
(`src/queue/layout.rs:34`).

The status and command bits the driver uses are one file of constants (`src/constants/queue.rs:31`):
...
bytes (`src/constants/queue.rs:17`). The four sizes become the four DMA grants setup takes: an RX ring, an RX
buffer pool, a TX ring, and a TX buffer pool ([bring-up](bring-up.md), `src/setup/dma.rs:47`). The 2048-byte
buffer matches the `RCTL` buffer-size setting programmed at RX enable, so a full 1514-byte Ethernet frame
fits in one buffer with room to spare (`src/init/rx_setup.rs:45`, `src/constants/status.rs:26`).

Each ring cursor knows the user VA of its descriptor ring, the user VA and device address of its buffer pool,
and its position. The RX ring holds a `head` cursor, the TX ring a `tail` cursor (`src/queue/rx.rs:22`,
`src/queue/tx.rs:28`). The per-slot buffer address is computed as the pool base plus the slot index times the
2048-byte buffer length, for both the device address the NIC uses (`buffer_phys`) and the user VA the capsule
copies through (`buffer_va`) (`src/queue/rx.rs:39`, `src/queue/tx.rs:48`).

## The transmit path

`OP_TX_PACKET` validates the frame size, then drives the TX ring (`src/server/handlers/tx_packet.rs:26`,
[operations](operations.md)):

1. Copy the frame into the buffer for the current tail slot with a non-overlapping copy
   (`src/server/handlers/tx_packet.rs:38`).
2. `TxRing::post` writes the descriptor at the tail: the buffer device address, the frame length, and the
   ...
   used (`src/queue/tx.rs:56`).
3. Ring the doorbell: write the register `TDT` with the next tail so the NIC knows a descriptor is ready
   (`src/server/handlers/tx_packet.rs:43`, `src/constants/regs.rs:45`).
4. Poll the posted descriptor's `TX_STATUS_DD` bit until the NIC sets it, up to a fixed budget of one million
   spins; a completion that never lands returns `E_IO` (`src/server/handlers/tx_packet.rs:48`,
   `src/queue/tx.rs:67`). Because `TX_CMD_RS` was set, the NIC writes the done bit back when the frame is out.
...
tail is set to the last valid index, so all 32 slots are available to the NIC (`src/init/rx_setup.rs:35`).
`OP_RX_PACKET` consumes one completed descriptor (`src/server/handlers/rx_packet.rs:27`):

1. `RxRing::consume` reads the descriptor at the head. If its `RX_STATUS_DD` bit is clear the ring is empty
   and it returns `None`, which the handler answers with `E_AGAIN` (`src/queue/rx.rs:47`).
2. On a done descriptor it snapshots the status, error, and length, clears the descriptor's status and error
   so the slot can be reused, and advances the head modulo 32 (`src/queue/rx.rs:52`).
3. It then screens the frame: a descriptor that is not end-of-packet, that has any error bit set, that
   ...
   answers `E_IO` (`src/queue/rx.rs:59`, `src/server/handlers/rx_packet.rs:35`).
4. On a good frame the handler writes a 4-byte little-endian length prefix, copies the frame out of the
   ...
   the reply (`src/server/handlers/rx_packet.rs:42`, `src/constants/regs.rs:39`).
...
mode. This is the discrepancy noted on the [README](README.md) against the source README's "IRQ completion
...
counts (`src/server/handlers/stats.rs:38`). Comparing the device head/tail against the driver cursors is the
direct way to see whether the rings are advancing, which the [debugging](debugging.md) page uses.
