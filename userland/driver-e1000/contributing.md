# Contributing to capsule_driver_e1000
...
the [README](README.md), the [operations](operations.md) page, the [bring-up](bring-up.md) page, and the
...
hands the built `Driver` to `server::run`, which loops forever (`src/main.rs:38`). The top-level modules are
declared there (`src/main.rs:22`).
...
| `src/discover.rs` | the `mk_device_list` scan and the Intel NIC match | you change how the device is found or add a device id |
...
| `src/regs.rs` | the volatile 32-bit MMIO accessor over BAR0 | you change how registers are read or written |
...
1. Add the opcode constant to `src/protocol/ops.rs:23` and re-export it from `src/protocol/mod.rs:34`; if it
   carries a fixed payload, add its length to `src/protocol/limits.rs:22`.
2. Write the handler as one file under `src/server/handlers/`, exposing a `handle` function that encodes the
   ...
   (`src/server/error.rs:27`). Declare the module in `src/server/handlers/mod.rs:17`.
3. Wire it into the dispatch match in `src/server/runner.rs:60`.

If the op is meant for the kernel-side network client, it also needs a client method and an op constant in
the mirror at `src/hardware/e1000_capsule/protocol/ops.rs`; the header wire constants are shared and drift
surfaces there as a protocol mismatch (`src/hardware/e1000_capsule/protocol/header.rs:17`).

## Adding a register or a device id

A new register offset goes in `src/constants/regs.rs:24` and its bit fields in `src/constants/status.rs:17`,
matching the 82540EM / 82545EM naming already used there. Read or write it through the `Regs` accessor
(`src/regs.rs:41`), never with a raw pointer, so the volatile access and the BAR0 base stay in one place. A
new e1000-class device id goes in the `E1000_DEVICE_IDS` table (`src/constants/pci.rs:19`); discovery matches
...
`nonos-capsule-driver-e1000` feature is set (`src/hardware/e1000_capsule/embed.rs:23`).
...
- Every setup phase must roll back in reverse order on failure. Each intermediate step releases the grants it
  already holds before returning the error (`src/setup/mmio.rs:34`, `src/setup/irq.rs:30`), the DMA phase
  funnels through `rollback::after` (`src/setup/rollback.rs:25`), and `Driver::release` is the full
  reverse-order teardown (`src/setup/driver.rs:49`).
- Every `unsafe` block that touches MMIO or DMA memory carries a `SAFETY` note tying the access back to the
  broker grant that guarantees the mapping, matching the notes already in `src/regs.rs:35` and the init and
  queue files.
