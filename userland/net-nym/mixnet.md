...
inside is on the [packet](packet.md) page; the nodes it routes over come from the signed directory on the
[directory](directory.md) page. This is a Sphinx-style construction built on the capsule's own primitives,
...
(`src/route/sphinx/types.rs:19`). A route-selection failure becomes `PacketError::NoRoute`
(`route/header.rs:31`), which the send handler maps to `E_NO_ROUTE`.
...
(`src/route/seed.rs:22`). The seed feeds two things: the topology selector uses its bytes to pick a node at
each layer (`src/topology/select.rs:44`), and the per-hop key schedule and masking mix it in so the layering
...
```
  ...
  hop_key_i  = HKDF-SHA256(salt = credential, ikm = shared_i,
                           info = "NONOS-NYM-SPHINX-HOP-v1" || seed || i)
```

The hop key derivation is in `src/route/sphinx/key.rs:24`: the credential is the salt, the X25519 shared
...
so that on the wire each layer is wrapped by the layers outside it (`src/route/sphinx/build.rs:40`). For hop
index `i` from 4 down to 0, `mask::apply` XORs that hop's 55-byte region with an HKDF-derived keystream
(`src/route/sphinx/mask.rs:24`):

```
  mask_i = HKDF-SHA256(salt = seed, ikm = hop_key_i,
                       info = "NONOS-NYM-SPHINX-MASK-v1" || seed || i)
  region_i ^= mask_i
```
...
produces a fixed-size layered header bound to a session and a signed directory, and the [packet](packet.md)
seal and [state](state.md) replay window sit on top of it. It is a faithful mixnet header in shape, layered
...
is defined by the format rather than exercised by a NONOS mix. The capsule builds and sends real layered
