...
Every request and reply carries the same twenty-byte header, parsed in `src/server/parse_req.rs:26` and
written back in `src/server/respond.rs:23`:
...
forever (`src/server/runner.rs:28`). `IPC_PAYLOAD_MAX` is `WIRE_PACKET_MAX + 64`, that is `365 + 2048 + 64`
bytes, sized to hold a full outbound wire packet plus header slack (`src/protocol/limits.rs:22`). Each pass
...
returning `false` for an unknown op (`src/server/handlers/dispatch.rs:24`). The sender pid threads through
...
`src/server/authz.rs`: `admin(sender_pid)` looks up the pid the registry bound to the service name
...
(`src/server/handlers/gateway.rs:27`), `OP_SET_TOPOLOGY` (`src/server/handlers/set_topology.rs:25`),
`OP_SET_AUTHORITY` (`src/server/handlers/set_authority.rs:24`), `OP_SET_TIMING`
(`src/server/handlers/set_timing.rs:24`), and `OP_SYNC_DIRECTORY` (`src/server/handlers/sync_directory.rs:28`).
...
packet, and hands it to the gateway (`send.rs:29`); the encode path is on the [packet](packet.md) page.
...
The errno constants are in `src/protocol/errno.rs:17`, returned in the reply header at offset 8.
