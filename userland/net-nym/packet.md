...
occupies the tail of every packet is built separately and documented on the [mixnet](mixnet.md) page; the
tables that hold session keys and replay windows are on the [state](state.md) page. For the kernel-side
crypto these wrappers call, read the [crypto capsule](../crypto/README.md).

## Fixed sizing

Every wire packet is exactly `WIRE_PACKET_MAX` bytes: a `NYM_HEADER_BYTES` header of 365 and a
`NYM_PAYLOAD_BYTES` payload of 2048, so 2413 bytes total (`src/protocol/limits.rs:18`). The size is a
...
The header offsets are named constants in `src/packet/header.rs:17`, and `encode` writes them in order
(`src/packet/encode.rs:67`):
...
(`src/route/sphinx/types.rs:19`). The two flag bits are `FLAG_COVER` (`0x01`) and `FLAG_REPLY` (`0x02`)
(`src/packet/types.rs:24`).

## Encode

`encode` takes the session id, the flags, the 32-byte session key, the 32-byte credential material, and the
plaintext, and produces a full wire packet (`src/packet/encode.rs:27`). It runs in order:
...
5. Build the Sphinx route header for this session and credential and write it into the header tail at offset
   56 (`encode.rs:43`, `encode.rs:62`). This is the [mixnet](mixnet.md) page's job.
...
size mismatch returns `PacketError::BadLength` (`src/packet/types.rs:36`).
...
(`src/packet/types.rs:23`). `padded_plaintext` fills the 2032-byte buffer with random bytes, writes the real
length as a little-endian u16 at offset 0, and copies the datagram after it (`src/packet/plain.rs:23`). The
...
real bytes (`src/server/handlers/recv_plain.rs:22`).

## Decode

`decode` is the inverse guard on receipt (`src/packet/decode.rs:24`). It requires the buffer to be exactly
...
window, and opens the ciphertext under the session key (`src/server/handlers/recv_drain.rs:60`).

## The replay tag

The tag is a BLAKE3 hash, not a keyed MAC: `tag::compute` concatenates the session id, the flags byte, the
12-byte nonce, and the full ciphertext, and hashes it to 32 bytes (`src/packet/tag.rs:23`). It serves two
...
(`src/state/replay.rs:32`). Because the tag covers the ciphertext, which covers the random-padded plaintext
...
`src/crypto/mod.rs:24`. It invents no primitives; every function is a bounds-checked wrapper that calls a
...
(`src/crypto/types.rs:17`). Note the directory and credential verify paths call `crypto_ed25519_verify`
directly rather than through this module, because signature verification is not part of the packet crypto;
those live on the [directory](directory.md) and [state](state.md) pages.
