...
the [operations](operations.md), [packet](packet.md), [mixnet](mixnet.md), [directory](directory.md),
...
(`src/userspace/init/spawn_plan/network/spawn_nym.rs:21`), whose `Ok` arm calls `boot_log::ok(prefix,
...
instead (`src/userspace/init/capsule_boot/run.rs:32`). The spawn is feature-gated on `nonos-capsule-net-nym`,
so an image built without that feature will not print the line at all
(`src/userspace/init/spawn_plan/network/spawn_nym.rs:18`).
...
init failure, before setup even runs (`src/main.rs:39`). After that, `wait_for_setup` calls `setup::run` in a
loop, yielding sixty-four times between attempts, and only enters the server once setup returns `Ok`
(`src/main.rs:45`). Setup does exactly one thing: it looks up `net.tcp` and stores its port, and its single
failure is `TcpMissing` when the lookup fails or returns port zero (`src/setup.rs:27`). So a capsule that
...
the [operations](operations.md) page. The important point for debugging is that the capsule has a strict
...
(`src/server/handlers/open.rs:31`, `src/state/table/ops.rs:29`): `E_NO_GATEWAY` means `OP_SET_GATEWAY` has not
...
[operations](operations.md) page lists the ops. `E_TABLE_FULL` means all 32 sessions are in use.
...
(`src/server/authz.rs:38`). Because no capsule registers `net.admin` today, these are denied by default. A
...
(`src/server/handlers/topology_errno.rs:23`): `E_AUTHORITY_MISSING` means no trusted authority was set with
...
(`src/topology/store.rs:39`): pushing a directory whose epoch is not greater than the current one is refused,
...
(`src/server/handlers/send.rs:42`): `E_NO_ROUTE` means route selection found no candidate node for some hop,
usually a directory missing an entry, exit, or a mix layer (`src/topology/select.rs:41`);
`E_CREDENTIAL_EXPIRED`, `E_AUTHORITY_MISSING`, or `E_AUTHORITY_UNTRUSTED` mean the credential lapsed or its
authority changed (`src/state/credential/store.rs:32`); `E_CRYPTO` means a crypto syscall failed inside
...
(`src/server/handlers/gateway.rs:38`). `E_GATEWAY_PROTO` means the WebSocket upgrade handshake failed, the
server did not return a `101` or the `Sec-WebSocket-Accept` did not match (`src/gateway_client/ws/handshake.rs:34`).
...
returns one if any is queued (`src/server/handlers/recv.rs:24`). `E_RX_EMPTY` means nothing is queued for that
...
tag (`src/packet/decode.rs:24`), the drain drops any packet flagged `FLAG_COVER`
(`src/server/handlers/recv_drain.rs:56`), the replay window drops a duplicate tag (`src/state/replay.rs:32`),
