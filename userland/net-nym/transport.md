...
It mirrors `src/gateway_client/`, `src/tcp_client/`, and `src/setup.rs`. The packet these carry is on the
[packet](packet.md) page; the gateway address and transport mode come from `OP_SET_GATEWAY` on the
...
`TcpMissing` when the lookup fails or returns port zero (`src/setup.rs:27`). Every later send or receive reads
that stored port back with `setup::tcp_port` (`src/setup.rs:33`). If the port is zero, meaning setup has not
...
declared length before copying the body out (`src/tcp_client/envelope.rs:23`). On top of that, `ops`
implements the four calls the gateway needs (`src/tcp_client/ops.rs:17`):
...
TCP and `1` is WebSocket, with WebSocket the default (`src/server/handlers/gateway.rs:56`). The `Gateway`
struct carries the IP, port, stream handle, and transport tag (`src/state/gateway.rs:23`). The gateway client
dispatches on that tag for connect, send, receive, and close (`src/gateway_client/ops.rs:22`):

- Raw TCP send and receive pass straight through to the `NTCP` `send_all` and `recv`
  (`gateway_client/ops.rs:33`).
- WebSocket send and receive wrap and unwrap RFC 6455 frames, described below (`gateway_client/ops.rs:34`).

`connect` opens the TCP stream first, and if the transport is WebSocket, runs the upgrade handshake on it
before returning the connected gateway (`gateway_client/ops.rs:22`). `close` sends a WebSocket close frame
first when the transport is WebSocket, then closes the underlying TCP stream (`gateway_client/ops.rs:45`).

## The WebSocket handshake

The upgrade is a real RFC 6455 client handshake, not a stub (`src/gateway_client/ws/handshake.rs:26`). It
...
(`ws/request.rs:22`). It reads the response headers until the `\r\n\r\n` terminator, bounded to sixteen read
...
on the header name (`ws/accept.rs:21`). The SHA-1 is implemented in the capsule (`ws/sha1.rs:21`), as is the
base64 encoder (`ws/base64.rs:19`); neither is a syscall, because they are protocol glue rather than
...
6455 requires of clients (`ws/frame/send.rs:24`). The length field uses the 7-bit form under 126 bytes and
the 16-bit extended form otherwise; a 2413-byte packet takes the extended form (`ws/frame/send.rs:52`).
...
frame (`ws/frame/recv.rs:26`). The frame parser validates the FIN bit, reads the length in its 7-bit or
16-bit form, rejects the 64-bit form, and unmasks the payload if the mask bit is set
(`ws/frame/parse.rs:20`, `ws/frame/read.rs:20`). Control-frame payloads are bounded to 125 bytes into a fixed
buffer, per the spec (`ws/frame/parse.rs:48`).
