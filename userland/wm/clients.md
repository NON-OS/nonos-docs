# Compositor client, setup, and the input-router gate

This page mirrors `src/compositor_client/`, `src/setup/`, `src/wait_for_setup.rs`, and the input-router
gate under `src/server/handlers/route_focus/`: the two outbound calls the wm makes and the two registry
lookups it depends on. The operations that trigger these calls are in [operations.md](operations.md); the
focus state they push is in [layout.md](layout.md).

## The compositor client

The wm speaks `NCMP` to the compositor: magic `0x4E43_4D50`, version 1, a 20-byte header
(`src/compositor_client/wire.rs:10`). `build_request` frames a little-endian header plus payload, and the
two call helpers send and validate the reply (`src/compositor_client/wire.rs:16`). The reply header is
checked for magic, version, op, request id, and payload length before the status word is read, so a
mismatched or short reply is rejected rather than trusted (`src/compositor_client/wire/reply.rs:24`,
`reply.rs:40`).
...
- Display size. `query_display_info` sends op `0x0008` at setup and reads back the display width and
  ...
  bring-up (`src/compositor_client/display_info.rs:19`, `display_info.rs:27`, `src/compositor_client/wire.rs:14`).
- Focus push. `push_focus_set` sends op `0x0004` with the focused pid (0 to clear) and a 16 ms
  steady-state timeout, returning an error if the compositor rejects it (`src/compositor_client/focus_set.rs:19`,
  `focus_set.rs:22`, `src/compositor_client/wire.rs:13`). This is the wm's only influence on pixels: it
...
(`src/compositor_client/health.rs`, re-exported at `src/compositor_client/mod.rs:24`).

## Setup and bring-up

`wait_for_setup` loops until `setup::run` returns a `Context`, yielding 64 times between failed attempts so
a not-yet-ready compositor does not spin the CPU (`src/wait_for_setup.rs:19`, `src/wait_for_setup.rs:24`).
`setup::run` resolves the `compositor` service port by name, probes it with request id 1, then reads the
display size with request id 2 (`src/setup/run.rs:36`, `src/setup/discover.rs:21`). The resulting `Context`
starts `next_request_id` at 3 because ids 1 and 2 were spent probing (`src/setup/run.rs:48`); a failure to
resolve the service returns `"compositor service not announced"` and the loop retries
(`src/setup/discover.rs:30`).
...
is the right length and the sender is the live `input_router` (`src/server/handlers/route_focus/handle.rs:25`).
...
(`src/server/handlers/route_focus/is_input_router.rs:34`, `is_input_router.rs:23`). Once past the gate the
handler validates the two `u32` fields, requires the target window to exist and be focusable, and pushes
`FOCUS_SET` only when focus actually changes (`route_focus/handle.rs:43`, `route_focus/handle.rs:55`).
