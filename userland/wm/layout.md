...
[operations.md](operations.md) call into; the records they read and write are in [state.md](state.md).
...
never overflow (`src/geometry/rect.rs:26`, `src/geometry/rect.rs:33`).
...
(`src/geometry/constrain.rs:25`, `src/geometry/constrain.rs:19`).
...
= 40-pixel steps until it finds a free slot (`place.rs:32`, `window_open/constants.rs:17`). The overlap
predicate iterates the visible normal windows and returns true on the first `Rect::overlaps`
(`window_open/collides.rs:21`).
...
(`place.rs:50`, `window_open/fallback_slot.rs:23`). The whole search is bounded by the display extent and
...
(`src/focus/hit_test.rs:48`). This is exactly what `OP_QUERY_TOPMOST` returns to the input router so it can
...
click even when it sits under the cursor (`src/window/kind.rs:27`).

## The z counter

Stacking is one `u32` counter. `ZStack::allocate` hands out a strictly increasing z on each call and wraps
back to 1 on overflow rather than panicking (`src/z_order/stack.rs:31`). Open, raise, restore, and maximize
...
ordering stays consistent (`src/z_order/stack.rs:17`).
