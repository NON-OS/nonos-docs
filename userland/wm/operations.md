...
[layout.md](layout.md); for the window table and subscriber list it mutates, read [state.md](state.md);
for the outbound compositor calls, read [clients.md](clients.md). Identity and the capability mask live on
the [README](README.md).

## The wire

The request frame is `NWMP` (magic `0x4E57_4D50`, version 1): a 20-byte header (magic, version, op, flags,
request id, payload length) followed by a payload capped at 256 bytes (`src/protocol/header.rs:17`,
`src/protocol/limits.rs:17`). `parse` rejects any frame whose declared payload length does not match the
buffer (`src/protocol/decode.rs:19`). Replies reuse the request's op, flags, and request id and carry a
4-byte status followed by any payload (`src/protocol/encode.rs:19`, `src/server/respond.rs:21`).

Two replies carry more than a status word: `OP_WINDOW_OPEN` returns a 16-byte rect
(`src/server/respond_window_opened.rs:22`), `OP_QUERY_TOPMOST` returns a 32-byte hit descriptor
(`src/server/handlers/query_topmost.rs:49`), and `OP_QUERY_FOCUS` returns an 8-byte
`(owner_pid, window_id)` (`src/server/handlers/query_focus.rs:32`).
...
1 closed), owner pid, window id, and the window's x and y (`src/protocol/notify.rs:21`).

## The run loop and dispatch

`server::run` allocates a receive and a transmit buffer, then loops (`src/server/runner/run.rs:28`). Each
...
the opcode (`src/server/runner/run.rs:32`, `src/server/runner/constants.rs:17`).

`dispatch` is a single match on `req.op` (`src/server/runner/dispatch.rs:33`). The three bodyless verbs
...
otherwise (`src/server/runner/dispatch.rs:52`). The error codes are defined in `src/protocol/errno.rs:17`.

## The fourteen opcodes

Defined in `src/protocol/ops.rs:17`; the body lengths are the `WINDOW_*_REQ_LEN` constants in
`src/protocol/limits.rs:21`, and each verb handler checks its exact length first and replies `E_INVAL` on
a mismatch.
...
focus (`window_open/handle.rs:32`). If the table is full the insert fails and the reply is `E_NOMEM`
(`window_open/handle.rs:51`). The placement policy itself lives in [layout.md](layout.md).
...
- `OP_ROUTE_FOCUS` is the privileged focus path. It is refused with `E_PERM` unless the sender is the live
  `input_router` service, resolved by name and cached (`route_focus/handle.rs:25`,
  ...
  is covered in [clients.md](clients.md).
...
- `OP_QUERY_TOPMOST` hit-tests a point and returns the topmost visible focusable window containing it,
  ...
  (`query_topmost.rs:40`). The hit test lives in [layout.md](layout.md).
...
for the open rect, and the dedicated encoders in the query handlers (`src/server/respond.rs:21`,
`src/server/respond_window_opened.rs:22`). Lifecycle notifications are fire-and-forget sends to each
subscriber pid; a send that fails marks the pid stale and it is dropped from the list
(`src/server/notify_fanout.rs:22`).
