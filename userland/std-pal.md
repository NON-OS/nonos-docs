...
The PAL is applied to the pinned `rust-src` by `toolchain/nonos-std/apply.sh`. It copies NØNOS
...
```
  std facility        NONOS backend                          maps to
  ----------------    -----------------------------------    -------------------------
  ...
  global allocator    sys/alloc/nonos.rs                       the NONOS heap
...
[storage](../subsystems/storage/vfs-capsule.md) page, and the net backend speaks the same
[sockets](../subsystems/networking/sockets.md) op set.

## The threads backend

The thread backend is worth a note because it is real, not emulated. `thread::spawn`
(`sys/thread/nonos.rs`) calls the `MTSP` syscall, which creates a kernel thread sharing the capsule's
...
```
  ...
  # build any std crate for the NONOS std target
...
capsule (see [writing an app](writing-an-app.md) and the [build](../build/toolchain.md) toolchain
