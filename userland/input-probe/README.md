...
capsule through the kernel ring, through [the input router](../input-router/README.md), over IPC, and
onto glass. If a character you type appears in the probe's window, the path is alive; if it does not, the
[debugging](debugging.md) page walks you back up the path to the break.
...
- It then subscribes to keyboard events and requests an exclusive keyboard grab from the router
  (`src/server/runner.rs:13`, `src/server/runner.rs:14`).
- Every routed delivery is decoded, and each printable `KEY_DOWN` (ASCII `0x20..=0x7E`) is appended to a
  64-byte ring and rendered as a row of scaled bitmap glyphs (`src/server/runner.rs:31`,
  `src/server/runner.rs:38`, `src/render/mod.rs:10`).
...
[the input path](../../subsystems/input/path.md) for how a grab bypasses focus and hit testing.
...
| Service handle | `app.input_probe` | `Capsule.mk:8`, `src/userspace/capsule_input_probe/spawn.rs:31` |
...
(`src/userspace/capsule_input_probe/spawn.rs:50`). This is the same leaf-renderer envelope the desktop
...
(`userland/capsule_input_router/src/server/handlers/grab_request.rs:25`). The grant lives in the router,
...
|---|---|---|
| [rendering.md](rendering.md) | `src/server/`, `src/render/`, `src/protocol.rs` | How the probe reads a delivery, filters it, and turns a keystroke into pixels: the receive loop, the delivery decode, the printable filter, the 64-byte history ring, the bitmap font, and the damage commit. |
| [contributing.md](contributing.md) | the whole tree | Where the source lives, which file owns what, how to build and sign the capsule, and the code standards a change has to meet. |
| [debugging.md](debugging.md) | runtime | The failure modes, what a blank versus black-but-unresponsive surface means, and where to look when a keystroke never reaches glass. |

## Lifecycle

The probe is spawned through [verified spawn](../../security/capsules-and-trust.md): its signature and
attestation are checked, its requested capabilities are held against its manifest ceiling, and only then
is its ELF mapped (`src/userspace/capsule_input_probe/spawn.rs:37`). `_start` initializes the heap and
runs `setup::run`, exiting with code 1 on heap failure and 2 on setup failure
(`src/main.rs:16`). Setup looks up the `compositor` and `input_router` services, health-checks the
compositor, queries the display geometry, maps and registers an ARGB8888 surface, submits it as an
overlay, and commits damage (`src/setup/mod.rs:18`). The runner then subscribes to keyboard events, grabs
the keyboard, and enters a blocking receive loop that never returns (`src/server/runner.rs:12`). It is
