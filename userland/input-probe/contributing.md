...
identity read the [overview](README.md); for the receive-and-render internals read
...
which never returns (`src/main.rs:16`). Nothing else calls `mk_exit`.
...
- To change what the probe brings up (a different surface format, a second overlay, a health gate),
  edit `setup/mod.rs`. It is the only file that maps memory, registers a surface, and talks to the
  compositor during bring-up (`src/setup/mod.rs:18`). Keep the stride guard in
  `display_info::query_display_info` intact; it is what stops the fill loop writing past the mapping
  (`src/clients/display_info.rs:50`).
- To change which events the probe acts on, edit `server/runner.rs`. The recv loop filters on
  `INPUT_KIND_KEY_DOWN` and `on_key` gates on the printable ASCII range `0x20..=0x7E`
  (`src/server/runner.rs:31`, `src/server/runner.rs:38`). A new event class is a new arm in the loop,
...
- To change the wire format the probe speaks, edit `protocol.rs`. `encode_request` builds the 20-byte
  ...
  (`src/protocol.rs:15`, `src/protocol.rs:27`). Both sides must move together with the router; the probe
...
- To change how a key is drawn, edit `render/`. `push_and_draw` owns the 64-byte ring and `draw_glyph`
  ...
  (`src/render/mod.rs:10`, `src/render/font.rs:5`).
...
(`src/userspace/capsule_input_probe/spawn.rs:50`). Widening it is a trust-boundary change reviewed
...
- No panics on the runtime path. Setup returns `Result` and the loop swallows a bad delivery rather than
  faulting (`src/setup/mod.rs:18`, `src/server/runner.rs:28`).
- Every raw surface write stays inside the mapping and carries the invariant in a SAFETY comment; the
  clamps in `fill_rect` and `fill` are load-bearing, not decorative (`src/render/mod.rs:47`,
  `src/setup/fill.rs:5`).
