# Client operations and the NL2 protocol
...
authorization, and the per-op handlers), and `src/setup/` and `src/state.rs` (the one-time NIC bind and the
...
the [README](README.md); for how a handler reaches the NIC see the [nic-link](nic-link.md) page.

## The wire format

A request header is 20 bytes and begins with a magic and a version (`src/protocol/header.rs:31`). The magic
is distinct from the NIC protocol's so a stray router cannot mistake one service for the other
(`src/protocol/header.rs:17`). The parser rejects anything shorter than the header, a wrong magic, or a
wrong version, returning the matching errno (`src/protocol/decode.rs:20`).
...
never reads past what the client sent (`src/protocol/decode.rs:35`). Every reply is a response header of the
same 20-byte length with the errno in `flags`, followed by any payload; `write_header` refuses to write into
a buffer shorter than the header (`src/protocol/encode.rs:26`).

## The request loop

`server::run` sizes one receive buffer and one transmit buffer each to the header plus `IPC_PAYLOAD_MAX`, so
a single receive holds a full-MTU frame and a single send holds one back (`src/server/runner.rs:32`).
`IPC_PAYLOAD_MAX` is the 14-byte Ethernet header plus the 1500-byte MTU plus a 64-byte margin, so a caller
can wrap a v1 envelope around a full frame without splitting (`src/protocol/limits.rs:22`).

The loop receives a request with `mk_ipc_recv_from`, which also returns the kernel-attested sender pid
(`src/server/runner.rs:38`). A receive of zero or fewer bytes, or a sender pid of zero, is skipped
(`src/server/runner.rs:39`). A parse failure is dropped silently, since a malformed frame has no valid reply
target (`src/server/runner.rs:43`). Each recognised op routes to its handler, and an unrecognised opcode is
answered with `E_BAD_OP` (`src/server/runner.rs:52`).

## The seven operations

The opcodes are defined in `src/protocol/ops.rs:21` and dispatched in `src/server/runner.rs:44`.
...
| `OP_GET_MAC` | `2` | none | 6-byte MAC | `server/handlers/get_mac.rs:21` |
| `OP_GET_LINK` | `3` | none | 1-byte up/down flag | `server/handlers/get_link.rs:22` |
| `OP_SEND_FRAME` | `4` | raw Ethernet frame | none (status only) | `server/handlers/send_frame.rs:23` |
| `OP_POLL_FRAME` | `5` | none | raw Ethernet frame | `server/handlers/poll_frame.rs:28` |
| `OP_ARP_RESOLVE` | `6` | 4-byte target IPv4 | 6-byte MAC | `server/handlers/arp_resolve.rs:28` |
| `OP_SET_IP` | `7` | 4-byte interface IPv4 | none (status only) | `server/handlers/set_ip.rs:25` |

Every reply carries the errno in the response-header `flags` field and only then the op-specific payload
(`src/server/respond/respond.rs:23`). The status-only ops go through `respond_status_only`, which writes the
header with a zero payload length (`src/server/respond/respond_status_only.rs:19`).
...
- `OP_GET_MAC` copies the six-byte MAC out of capsule state into the reply and answers `E_OK`
  (`src/server/handlers/get_mac.rs:22`). The MAC was read from the NIC at setup.
- `OP_GET_LINK` answers `E_NO_LINK` if no NIC port is bound, otherwise writes a single `1` byte for up and
  answers `E_OK` (`src/server/handlers/get_link.rs:23`).
- `OP_SEND_FRAME` is caller-gated: only `net.ip` or `net.dhcp.client` may send. It rejects any other caller
  ...
  a send failure to `E_TX_BUSY` (`src/server/handlers/send_frame.rs:24`).
- `OP_POLL_FRAME` is caller-gated the same way. On a NIC frame it runs the frame through the ingress
  ...
  `E_RX_EMPTY`, and any other NIC error returns `E_NO_LINK` (`src/server/handlers/poll_frame.rs:29`,
  `poll_frame.rs:49`).
- `OP_ARP_RESOLVE` reads the 4-byte target and rejects a wrong length with `E_BAD_LEN`. On a cache hit it
  ...
  (`src/server/handlers/arp_resolve.rs:29`).
- `OP_SET_IP` is caller-gated to `net.dhcp.client` alone; any other caller gets `E_PERM`. It reads the
  ...
  sender IP into outbound ARP and answer ARP-for-host (`src/server/handlers/set_ip.rs:26`).
...
lookup miss or a pid of zero rejects (`src/server/authz.rs:23`, `authz.rs:32`). `authorized_any` accepts a
match against any service in a list (`src/server/authz.rs:36`). `OP_SEND_FRAME` and `OP_POLL_FRAME` allow
...
All errnos ride in the response-header `flags` field (`src/protocol/errno.rs:20`):
...
(`src/protocol/decode.rs:25`); the rest are handler decisions.
...
first the kernel registry knows (`src/setup/discover.rs:22`, `discover.rs:36`). Adding a NIC class is one
line in that list; nothing upstream of L2 changes (`src/setup/discover.rs:20`). The resolved port and pid go
into capsule state, and the NIC's MAC is read and stored (`src/setup/mod.rs:47`). A missing NIC or a failed
MAC read maps to a `SetupError` that keeps the retry loop running rather than serving a request
(`src/setup/mod.rs:25`, `src/main.rs:42`).
...
cache each under a mutex (`src/state.rs:29`, `src/state.rs:58`). `set_nic` stores the port and pid with
release ordering and `nic_port` loads with acquire, so a handler on another turn sees a consistent bind
(`src/state.rs:48`). The MAC is written once at setup, the IPv4 is written by `OP_SET_IP`, and the cache is
...
rejects a malformed frame without a reply (`src/protocol/decode.rs:20`, `src/server/runner.rs:43`). The
parser bounds the payload slice to the declared length so a handler cannot over-read
(`src/protocol/decode.rs:35`). The three write-capable ops, send frame, poll frame, and set IP, are gated on
a kernel-attested caller pid, not a claim in the request (`src/server/authz.rs:32`). Fixed-length ops check
