# Contributing to capsule_net_l2
...
[cache](cache.md) page, and the [nic-link](nic-link.md) page. For where L2 sits in the stack, see the
...
loops forever (`src/main.rs:34`). The top-level modules are declared there (`src/main.rs:22`).
...
| `src/state.rs` | the NIC port/pid, MAC, IPv4, and ARP cache behind their locks | you add capsule-wide state |
...
| `src/arp/handle.rs` | the inbound ARP decision and the request/reply builders | you change ARP behaviour |
| `src/arp/cache/` | the bounded neighbour cache, learn policy, and pending ring | you change caching or eviction |
| `src/ingress.rs` | the inbound observer that learns ARP and emits replies | you change the inbound side-effect path |
...
1. Add the opcode constant to `src/protocol/ops.rs:21` and export it from `src/protocol/mod.rs:31`.
...
3. Wire it into the dispatch match in `src/server/runner.rs:44`. The unmatched arm already answers
   `E_BAD_OP`, so an opcode with no match is refused rather than mishandled (`src/server/runner.rs:52`).
...
The driver-facing protocol is `NNET`, one op constant per line in `src/nic_client/wire.rs:28`. Add the
...
trusting its payload. The MAC read (`src/nic_client/mac.rs:30`), the transmit (`src/nic_client/tx.rs:32`),
and the receive (`src/nic_client/rx/poll_frame.rs:28`) are the three reference shapes; the receive keeps its
strict payload decode in its own file (`src/nic_client/rx/parse_payload.rs:23`). Match the errno-return shape
...
- One unit per file. New ops are one file per handler under `src/server/handlers/`, and each cache operation
  ...
  declarations and re-exports, as in `src/arp/cache/mod.rs:17` and `src/protocol/mod.rs:17`.
- Keep the boundaries the capsule was designed around: no IP route policy, no transport, no DHCP or DNS, no
  ...
  (`src/userspace/capsule_net_l2/spawn.rs:17`); L2 reaches it only by IPC.
- Keep state bounded. The ARP cache and pending ring are fixed-size by construction
  (`src/arp/cache/constants.rs:17`); do not replace them with a growable structure.
