# The L2 Networking Capsule

`capsule_net_l2` is the NONOS Ethernet and ARP capsule: a signed ring-3 capsule that sits one layer above a
...
fits in the larger stack, see the [networking subsystem](../../subsystems/networking/README.md).
...
| Service handle | `net.l2` | `Capsule.mk:7`, `src/userspace/capsule_net_l2/spawn.rs:35` |
...
| Wire magic | `0x4E4C_3200` ("NL2\0") | `src/protocol/header.rs:31` |
| Kernel mirror | `src/userspace/capsule_net_l2` | `src/userspace/capsule_net_l2/spawn.rs:41` |
...
|------|---------|----------------|
| [operations.md](operations.md) | `src/protocol/`, `src/server/`, `src/setup/`, `src/state.rs` | The `NL2` wire format, the request loop, the seven client ops, the per-op payloads, the caller authorization, the errno set, and the one-time NIC bind. |
| [framing.md](framing.md) | `src/ethernet/`, `src/arp/packet/`, `src/arp/handle.rs`, `src/ingress.rs` | The Ethernet header, the ARP packet parse and build, the request/reply exchange, and the ingress observer that seeds the cache. |
| [cache.md](cache.md) | `src/arp/cache/` | The bounded neighbour cache, the learn policy, the pending-request ring, and oldest-first eviction. |
| [nic-link.md](nic-link.md) | `src/nic_client/` | The `NNET` protocol to the driver capsule, the MAC read, the frame TX and RX paths, and the request-id sequence. |
| [contributing.md](contributing.md) | the whole tree | Where each concern lives, how to add a client op or a NIC op, the build and sign steps, and the code standards. |
| [debugging.md](debugging.md) | runtime | The boot marker, the setup retry loop, and the runtime failure modes: no link, no neighbour, RX empty, TX busy, and a permission reject. |

## Lifecycle

The capsule is `no_std`/`no_main`. `_start` initialises the heap, then loops in `wait_for_setup` until
`setup::run` succeeds, and only then enters `server::run`, which loops forever (`src/main.rs:34`). Setup
resolves the first available NIC driver from the service registry, records its port and pid, and reads its
MAC into capsule state (`src/setup/mod.rs:46`); if no NIC is registered yet it yields and retries rather
than failing (`src/main.rs:42`). The IPv4 binding stays zero until the DHCP client pushes a leased address
down through `OP_SET_IP` (`src/setup/mod.rs:44`, `src/server/handlers/set_ip.rs:25`).

The kernel spawns the capsule under the network spawn plan, checking its signature, its NONOS-ID cert, and
its manifest, and holding its requested capabilities against its manifest ceiling before its ELF is mapped
(`src/userspace/capsule_net_l2/spawn.rs:41`, `spawn.rs:60`). The spawn is gated on the
`nonos-capsule-net-l2` feature (`src/userspace/init/spawn_plan/network/spawn_l2.rs:18`) and prints a
`NET-L2` boot marker on success; the [debugging](debugging.md) page covers what that marker and each failure
state mean.
