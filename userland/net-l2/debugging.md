...
[README](README.md), the [operations](operations.md) page, the [framing](framing.md) page, the
[cache](cache.md) page, and the [nic-link](nic-link.md) page.
...
(`src/userspace/init/spawn_plan/network/spawn_l2.rs:21`), whose `Ok` arm calls
`boot_log::ok(prefix, "capsule spawned")` (`src/userspace/init/capsule_boot/run.rs:29`). An absent line means
the capsule never started: either the `nonos-capsule-net-l2` feature was not built in
(`src/userspace/init/spawn_plan/network/spawn_l2.rs:18`), or the spawn failed, in which case the `Err` arm
prints a `boot_log::error` line instead (`src/userspace/init/capsule_boot/run.rs:32`). A spawn failure is a
signature, NONOS-ID cert, manifest, or capability check inside `spawn_verified`
(`src/userspace/capsule_net_l2/spawn.rs:60`).
...
(`src/main.rs:38`, `src/main.rs:42`). Only heap init failure exits, with code 1 (`src/main.rs:35`). So a
...
- `SetupError::NoNic` (`src/setup/mod.rs:26`): no service in the candidate list, `driver.virtio_net0`,
  `driver.e1000_0`, `driver.rtl8169_0`, `driver.rtl8139_0`, is registered yet
  (`src/setup/discover.rs:22`, `discover.rs:36`). This is normal at early boot, before the NIC driver
  capsule has registered its service; L2 simply retries until it appears.
- `SetupError::MacFailed` (`src/setup/mod.rs:36`): a NIC was found but the `OP_MAC_ADDRESS` call to it failed
  or returned a bad response (`src/nic_client/mac.rs:38`, `mac.rs:47`). This points at the driver capsule,
...
crash. The full set is on the [operations](operations.md) page; these are the ones worth naming.
...
`net.dhcp.client` alone (`src/server/handlers/send_frame.rs:24`, `poll_frame.rs:29`, `set_ip.rs:26`). The
gate compares the kernel-attested `sender_pid` to the owner pid the service registry bound to that service
name (`src/server/authz.rs:32`). An `E_PERM` means the caller is not the registered owner of a permitted
service: either the caller is a different capsule, or the permitted service was never registered so its owner
pid lookup misses (`src/server/authz.rs:26`). It is not a link or NIC problem.
...
setup has not completed (`src/server/handlers/get_link.rs:23`, `send_frame.rs:29`, `poll_frame.rs:34`).
`OP_POLL_FRAME` also returns `E_NO_LINK` for any NIC receive error that is not an empty queue
(`src/server/handlers/poll_frame.rs:43`). If a link op returns `E_NO_LINK` after the capsule has been up for
...
(`src/nic_client/rx/parse_payload.rs:38`, `src/server/handlers/poll_frame.rs:40`). This is the normal idle
case, not an error; a caller polls again. A frame that arrived but did not fit the reply buffer also returns
`E_RX_EMPTY` (`src/server/handlers/poll_frame.rs:54`).
...
maps to `E_TX_BUSY` (`src/nic_client/tx.rs:42`, `tx.rs:63`, `src/server/handlers/send_frame.rs:38`). This is
...
(`src/server/handlers/arp_resolve.rs:46`). If a resolve keeps returning `E_NO_NEIGHBOUR`, the reply is not
coming back: check that the interface IPv4 was set through `OP_SET_IP` so the outbound request carries a real
sender IP (`src/server/handlers/set_ip.rs:36`, `src/arp/handle.rs:68`), and that the target is actually on
...
(`src/server/handlers/arp_resolve.rs:29`, `set_ip.rs:30`). A malformed header, short or wrong magic or
version, is caught earlier in the parser and, for a bad length there, also surfaces as `E_BAD_LEN`
(`src/protocol/decode.rs:21`).
