# Contributing to capsule_driver_virtio_gpu
...
capability mask, read the [README](README.md); for how the three pillars fit together see the
[bring-up](bring-up.md), the [engine](engine.md), and the [client/protocol](client-protocol.md) pages in
this folder.
...
service port 4226, and enters the blocking server loop (`src/main.rs:35`). The nine top-level modules are
declared there (`src/main.rs:19`).
...
| `src/driver.rs` | the `Driver` struct that ties grants, queue, regs, and tables together | you add per-driver state |
...
1. Reserve the opcode. Add a `pub const OP_...: u16` to `src/protocol/ops.rs:16`, next to the existing
   twelve, and add its fixed request or response length to `src/protocol/limits.rs:16` if it carries a
...
2. Write the handler as one file under `src/server/handlers/` and declare it in
   `src/server/handlers/mod.rs:16`. A handler takes `(driver, sender_pid, req, tx)` for an empty-body op
   ...
   (`src/server/respond.rs:20`, `:26`). `create_resource` is the reference shape for a command op: it
   rejects a wrong length with `E_INVAL`, decodes its fields with `le_u32`, validates, calls the device,
   and replies (`src/server/handlers/create_resource.rs:24`).

3. Wire it into the dispatch match (`src/server/runner/dispatch.rs:26`). A read-only or empty-body op is
   gated behind `if body.is_empty()`, the way the seven state ops are, so a stray body falls through to the
   `E_INVAL` arm (`src/server/runner/dispatch.rs:43`). A command op that carries a fixed body is matched
   unconditionally and validates its own length in the handler (`src/server/runner/dispatch.rs:46`). An
...
4. If the op mutates a resource, enforce the two invariants the other command ops enforce: re-check
   ...
   command (`src/server/handlers/get_primary_surface.rs:39` for ownership). A client-supplied physical
...
1. Write the command as one file under `src/device/cmd/` and declare and re-export it in
   `src/device/cmd/mod.rs:16`. Build the 24-byte control header with `Hdr::write`
   (`src/device/cmd/hdr.rs:16`), append the command body, and call `q.submit_sync` with a response buffer.

2. Add the virtio-gpu command type and its accepted response type to `src/constants/mod.rs:60`, then check
   ...
   `RESP_OK_NODATA` (`src/device/cmd/create_resource_2d.rs:42`). Validate arguments before touching the
   queue, as `create_resource_2d` rejects a zero id or dimension first
   (`src/device/cmd/create_resource_2d.rs:29`).
...
- No panics in capsule code. No `unwrap`, `expect`, or `panic!`. Bring-up returns `Result<_, &'static
  ...
  retried rather than aborted (`src/main.rs:39`).
