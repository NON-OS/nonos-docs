# Engine: the virtqueue, the control commands, and the runtime tables
...
scanout, and fence tables, and `src/driver.rs` is the struct that ties them together. The
[bring-up](bring-up.md) page builds all of this; the [client/protocol](client-protocol.md) page calls into
it. For identity and the capability mask see the [README](README.md).
...
primary surface (`src/driver.rs:21`). Its `config()` re-reads the three device config words
(`GPU_CFG_EVENTS_READ 0x14`, `GPU_CFG_NUM_SCANOUTS 0x1C`, `GPU_CFG_NUM_CAPSETS 0x20`) straight from MMIO,
and `display_info()` is an alias of it (`src/driver.rs:40`, `:49`, `src/constants/mod.rs:34`). That is why
...
at 8192, and a 4 KiB staging area at 12288 (`src/constants/mod.rs:22`). `QueueLayout` computes the virtual
and device addresses of each region from the queue's base and size (`src/device/virtqueue/layout.rs`), and
`ControlQueue` wraps that layout with the register accessor (`src/device/virtqueue/control_queue.rs:18`).

`submit_sync` is the whole transaction (`src/device/virtqueue/submit.rs:23`):

1. Reject an empty request or response, and reject a request plus response that overflows the staging area
   (`src/device/virtqueue/submit.rs:30`).
2. Write the request bytes into staging, then zero the response tail behind it
   (`src/device/virtqueue/submit.rs:38`).
3. Read the current used index, take the available head modulo the queue size, and build a two-descriptor
   ...
   `VRING_DESC_F_WRITE` (`src/device/virtqueue/desc.rs`, `src/constants/mod.rs:57`).
4. Publish the head in the available ring, notify the device on queue index 0, and wait for the used ring
   to advance (`src/device/virtqueue/avail.rs`, `src/device/virtqueue/wait.rs`,
   `src/device/virtqueue/submit.rs:52`).
5. Verify the used entry's id matches the head, rejecting a mismatch with `virtio-gpu: used id mismatch`,
   and return the used length (`src/device/virtqueue/submit.rs:57`).

The response is read back out of staging with `read_response`, byte by byte from just past the request
(`src/device/virtqueue/control_queue.rs:34`, `src/device/virtqueue/submit.rs:62`). The queue is driven
...
and a `notify` that kicks the queue (`src/regs/read.rs`, `src/regs/write.rs`, `src/regs/notify.rs`). It
...
the queue (`src/regs/state/with_queue_notify.rs`). Every access is `unsafe` volatile against a broker-mapped
...
by `Hdr::write` and parsed back by `Hdr::parse` (`src/device/cmd/hdr.rs:16`, `:29`, `:36`). The type
constants are in `src/constants/mod.rs:60`.

| Command | Type | Response accepted | Source |
|---|---|---|---|
| `get_display_info` | `0x0100` | `RESP_OK_DISPLAY_INFO 0x1101` | `cmd/get_display_info.rs:45`, `constants/mod.rs:60`, `:67` |
| `create_resource_2d` | `0x0101` | `RESP_OK_NODATA 0x1100` | `cmd/create_resource_2d.rs:42`, `constants/mod.rs:61` |
| `set_scanout` | `0x0103` | `RESP_OK_NODATA 0x1100` | `cmd/set_scanout.rs:44`, `constants/mod.rs:62` |
| `resource_flush` | `0x0104` | `RESP_OK_NODATA 0x1100` | `cmd/flush.rs`, `constants/mod.rs:63` |
| `transfer_to_host_2d` | `0x0105` | `RESP_OK_NODATA 0x1100` | `cmd/transfer_to_host_2d.rs`, `constants/mod.rs:64` |
| `attach_backing` | `0x0106` | `RESP_OK_NODATA 0x1100` | `cmd/attach_backing.rs`, `constants/mod.rs:65` |

`create_resource_2d` rejects a zero id or zero dimension before it touches the queue
(`src/device/cmd/create_resource_2d.rs:29`) and returns `virtio-gpu: create_resource_2d rejected` if the
...
- The resource table is a fixed array of `MAX_RESOURCES` slots. `alloc_id` hands out monotonically
  ...
  `E_NOMEM` (`src/state/resources/table.rs:41`, `:46`, `:55`, `:64`). Each `Resource` carries its id, owner
  pid, geometry, format, and backing range (`src/state/resources/resource.rs`).
- The scanout table is a fixed array of `VG_MAX_SCANOUTS` (16) slots indexed by scanout id, seeded at
  bring-up and read by the mode-list op (`src/state/scanouts.rs:27`, `:42`, `:50`).
- The fence counter issues a monotonic fence id per command, stamped into every control header
  (`src/state/fences.rs`).

The `owner_pid` field is the isolation primitive: the [client/protocol](client-protocol.md) layer sets it
