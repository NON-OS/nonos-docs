# Debugging capsule_driver_virtio_gpu

This page lists the boot marker the driver's spawn path emits and the concrete failure modes with where to
look for each. For the driver's identity see the [README](README.md); for the bring-up sequence a failed
boot walks through see the [bring-up](bring-up.md) page; for the ops that return the runtime `E_*` statuses
see the [client/protocol](client-protocol.md) page.
...
(`src/userspace/init/spawn_plan/drivers_virtio_display.rs:26`); the `Ok` arm calls `boot_log::ok(prefix,
...
logs nothing for the driver (`src/userspace/init/spawn_plan/drivers_virtio_display.rs:22`). The
`nonos-mk-driver-virtio-gpu-prod` image sets that feature (see the [contributing](contributing.md) page).
...
error string, yields 64 times, and tries again (`src/main.rs:39`). The error strings returned all through
`setup/` and `init/` are therefore never printed; a device that cannot be stood up spins in this loop
silently, and the driver never reaches `mk_service_register` (`src/main.rs:50`). The observable signature
...
Where to look, in the order the sequence runs (`src/setup/sequence.rs:24`):
...
- Device not found. `find_virtio_gpu` matched no usable function: vendor `0x1AF4` with device id `0x1010`
  ...
  (`src/discover/match_device.rs:21`, `src/setup/sequence.rs:26`). A device present but with no usable IRQ
...
- Broker grant denied. The claim, bus-master, register map, and queue DMA are each a broker syscall, and
  each phase rolls back the grants it already holds on failure (`src/setup/claim.rs:17`,
  `src/setup/pci.rs:20`, `src/setup/mmio/map_mmio.rs:22`, `src/setup/dma.rs:19`). The mmio and pio map
  errno is decoded to a specific label (`src/setup/mmio/labels.rs:18`), which is the string the failing
  attempt would carry.
- Virtio negotiation rejected. The modern path requires `VIRTIO_F_VERSION_1` and returns `virtio-gpu:
  ...
  (`src/init/modern.rs:38`, `:47`, `:53`); the legacy path returns the matching strings
  (`src/init/legacy.rs:34`, `:40`).

Because none of these are logged, isolating which one fires means reading the sequence against the device
QEMU or the hardware presents; the [bring-up](bring-up.md) page walks the same order in detail.
...
(`src/setup/primary_surface/geometry.rs:23`, `src/setup/primary_surface/create.rs:33`). In that state the
service registers and answers IPC, but `OP_GET_PRIMARY_SURFACE` returns `E_DEVICE` because
`driver.primary` is `None` (`src/server/handlers/get_primary_surface.rs:23`). The compositor gets a live
...
(`src/setup/scanouts.rs:24`, `:61`). If it does, the suspect is `GET_DISPLAY_INFO` returning all scanouts
...
(`src/setup/primary_surface/create.rs:49`, `:54`).
...
line. The codes are POSIX-style negatives (`src/protocol/errno.rs:16`), and each names a distinct cause:
...
- `E_INVAL -22`. A wrong body length, a bad field, a zero dimension, a wrong format, or an out-of-bounds
  ...
  than `VG_FORMAT_B8G8R8A8_UNORM` (`src/server/handlers/create_resource.rs:25`, `:45`); the rect-carrying
...
- `E_BUSY -16`. The caller is not the owner of the resource. `GET_PRIMARY_SURFACE` makes the first caller
  ...
  sender_pid` (`src/server/handlers/get_primary_surface.rs:39`). A second capsule trying to drive a
...
- `E_NOMEM -12`. The resource table is full. `create_resource` returns it when `insert` finds no free slot
  in the `MAX_RESOURCES` (64) array (`src/server/handlers/create_resource.rs:67`,
  `src/protocol/limits.rs:29`).
- `E_DEVICE -110`. The device rejected the control command, or there is no primary surface.
  ...
  (`src/server/handlers/create_resource.rs:51`), and `get_primary_surface` returns it when `driver.primary`
...
- `E_BAD_OP -38`. The opcode is unknown and the body was empty; an unknown opcode with a body is answered
  `E_INVAL` instead (`src/server/runner/dispatch.rs:53`).
