# Client and protocol: the NVGP wire format and the twelve ops
...
see the [README](README.md); for the virtqueue and control commands the command ops drive, see the
[engine](engine.md) page; the compositor's side of this protocol is documented in the
...
the parsed `Request` and a borrowed body slice (`src/protocol/decode.rs:17`, `:31`). Every reply repeats
the header, then a 4-byte signed status word, then an optional body: a status-only reply is `HDR(20) |
status(4)`, and a payload reply is `HDR(20) | status(4) | body` with status 0 (`src/server/respond.rs:20`,
`:26`).

Errors are POSIX-style negative ints in the status word (`src/protocol/errno.rs:16`): `E_INVAL -22`,
...
(`src/server/runner/entry.rs:23`, `src/protocol/limits.rs:16`). Each turn receives on the service inbox
with `mk_ipc_recv_from`, drops an empty frame or a zero sender pid, parses the header, and dispatches;
anything that fails to parse is silently skipped rather than answered (`src/server/runner/loop_once.rs:25`).

The dispatcher matches on the opcode (`src/server/runner/dispatch.rs:26`). The seven read-only and
...
(`src/server/runner/dispatch.rs:53`). Every reply goes back to the sender pid through `mk_ipc_reply`, never
to a fixed reply port (`src/server/respond.rs:23`).

## The twelve ops

The opcodes are defined in `src/protocol/ops.rs:16` and each has one handler file under
`src/server/handlers/`.
...
|---|---|---|---|---|
| `OP_HEALTHCHECK` | `0x0001` | none | status 0 | `ops.rs:16`, `handlers/health.rs` |
| `OP_CONTROLLER_INFO` | `0x0002` | 40 bytes | device_id(8), claim_epoch(8), pci_device(2), queue_size(2), host_features(4), mmio_grant(8), irq_grant(8) | `ops.rs:17`, `handlers/controller.rs`, `limits.rs:18` |
| `OP_DISPLAY_INFO` | `0x0003` | 12 bytes | events_read(4), num_scanouts(4), num_capsets(4), read live from device config | `ops.rs:18`, `handlers/display.rs`, `driver.rs:49` |
| `OP_CONTROLQ_STATE` | `0x0004` | 24 bytes | queue_grant(8), queue_user_va(8), queue_device_addr(8) | `ops.rs:19`, `handlers/controlq.rs`, `limits.rs:20` |
| `OP_QUERY_CAPS` | `0x0005` | 12 bytes | num_scanouts(4), num_capsets(4), events_read(4) | `ops.rs:20`, `handlers/query_caps.rs`, `limits.rs:26` |
| `OP_MODE_LIST` | `0x000B` | 32 bytes per scanout | id(4), enabled(4), width(4), height(4), x(4), y(4), current_resource_id(4), pad(4); one entry per recorded scanout, bounded by the reply buffer | `ops.rs:26`, `handlers/mode_list.rs`, `limits.rs:27` |

`OP_DISPLAY_INFO` and `OP_QUERY_CAPS` both re-read the device config MMIO at call time through
`Driver::config` (`src/driver.rs:40`); they differ only in field order. `OP_MODE_LIST` reports the driver's
own recorded scanout table and refuses to write past the reply buffer even if more scanouts exist than fit
(`handlers/mode_list.rs`).
...
| `OP_ATTACH_BACKING` | `0x0007` | 24 bytes: resource_id(4), pad(4), backing_addr(8), backing_len(8) | status only | `E_INVAL` on bad args, unknown resource, or out-of-region backing, `E_BUSY` if not the owner, `E_DEVICE` on device reject | `ops.rs:22`, `handlers/attach_backing.rs:20` |
...
| `OP_SET_SCANOUT` | `0x0009` | 24 bytes: scanout_id(4), resource_id(4), x(4), y(4), width(4), height(4) | status only | `E_INVAL` on bad args or unknown resource, `E_BUSY` if not the owner, `E_DEVICE` on device reject | `ops.rs:24`, `handlers/set_scanout.rs` |
...
(`handlers/get_primary_surface.rs:27`, `:39`). Every subsequent command op re-checks `owner_pid ==
sender_pid` after its length and resource-lookup checks and returns `E_BUSY` to anyone else
(`handlers/transfer_to_host.rs:33`, `handlers/flush.rs:49`). A second capsule cannot transfer, scanout, or
...
(`handlers/transfer_to_host.rs:37`, `handlers/flush.rs:53`). `CREATE_RESOURCE` additionally refuses any
format other than `VG_FORMAT_B8G8R8A8_UNORM` (`handlers/create_resource.rs:45`). `ATTACH_BACKING` is the one
...
(`handlers/attach_backing.rs:49`, `:72`). The driver cannot be made to hand the device an arbitrary guest
address over IPC.
