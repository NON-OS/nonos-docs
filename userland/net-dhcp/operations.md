# Client operations and the NDHC protocol
...
body. For the identity table and the capability mask see the [README](README.md); for the acquisition ladder
the handlers drive, see the [lease](lease.md) page.

## The wire format

A request header is 20 bytes and begins with a magic and a version. The parser rejects anything shorter than
the header, a wrong magic, or a version other than 1 (`src/server/parse_req.rs:27`). The `NDHC` magic is
`0x4E44_4843` (`src/protocol/header.rs:17`).
...
the received buffer is shorter than that total (`src/server/parse_req.rs:40`). On success it returns the
...
(`src/server/respond.rs:24`). A `lease_status` reply carries an 18-byte body after that header; every other op
replies with the header alone. Replies go back to the requesting pid with `mk_ipc_reply`
(`src/server/respond.rs:58`).

## The request loop

`server::run` sizes one receive buffer and one transmit buffer, each to the header plus 256 bytes, which is
ample for the fixed lease ops and the 18-byte status body (`src/server/runner.rs:30`). The loop receives a
request with `mk_ipc_recv_from`, which also reports the sender pid (`src/server/runner.rs:38`). A receive of
...
| `OP_LEASE_REQUEST` | `2` | none | none (status only) | `server/handlers/lease_request.rs:22` |
| `OP_LEASE_STATUS` | `3` | none | 18-byte lease snapshot | `server/handlers/lease_status.rs:28` |
| `OP_LEASE_RELEASE` | `4` | none | none (status only) | `server/handlers/lease_release.rs:29` |
| `OP_LEASE_RENEW` | `5` | none | none (status only) | `server/handlers/lease_renew.rs:30` |

The opcode constants are `OP_HEALTHCHECK = 1`, `OP_LEASE_REQUEST = 2`, `OP_LEASE_STATUS = 3`,
`OP_LEASE_RELEASE = 4`, `OP_LEASE_RENEW = 5` (`src/protocol/ops.rs:17`).
...
- `OP_LEASE_REQUEST` runs the full DORA ladder synchronously by calling `dora::acquire`, and maps the result
  ...
  (`src/server/handlers/lease_request.rs:22`). The ladder itself is on the [lease](lease.md) page.
- `OP_LEASE_STATUS` reads the client state and the cached lease under their mutexes and writes an 18-byte
  ...
  seconds little-endian (`src/server/handlers/lease_status.rs:28`). It always replies `E_OK`.
- `OP_LEASE_RENEW` reissues a `DHCPREQUEST` for the already-bound address against the original server id and,
  ...
  (`src/server/handlers/lease_renew.rs:30`). It also returns `E_NO_LINK` before doing anything if setup has
...
- `OP_LEASE_RELEASE` sends a `DHCPRELEASE` if a lease is bound, clears the interface in `net.ip`, resets the
  ...
  (`src/server/handlers/lease_release.rs:29`). It returns `E_NO_LINK` only if setup has not completed
  (`lease_release.rs:33`).
...
parse failure is dropped in the request loop rather than answered (`src/server/runner.rs:43`), so a client
...
(`src/server/respond.rs:22`, `runner.rs:31`).
...
length field, and drops anything malformed without replying (`src/server/parse_req.rs:27`). A request with a
zero sender pid is skipped, so every served request is attributable to a caller (`src/server/runner.rs:39`).
...
touching the IP layer (`src/dora/acquire.rs:41`, `acquire.rs:58`). A client that wants a lease managed must
