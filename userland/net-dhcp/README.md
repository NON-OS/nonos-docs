# The DHCPv4 Client Capsule

`capsule_net_dhcp` is the NONOS DHCPv4 client: a signed ring-3 capsule that acquires, renews, reports, and
...
| Service handle | `net.dhcp.client` | `Capsule.mk:6`, `src/userspace/capsule_net_dhcp/spawn.rs:36` |
...
| Kernel mirror | `src/userspace/capsule_net_dhcp` | `Capsule.mk:15`, `src/userspace/capsule_net_dhcp/spawn.rs` |
...
BOOTP `xid` from `crypto_random` (`src/state/global.rs:56`), which is the syscall the `Crypto` bit gates. It
...
only the upper bound the spawn site is willing to grant (`src/kernel_core/process_spawn/capsule_spawn/runner/verified.rs:23`);
the spawn is verified before the ELF is mapped (`src/kernel_core/process_spawn/capsule_spawn/runner/preflight.rs:57`),
...
transport is `net.l2` for the outbound BOOTP exchange (`src/l2_client/wire.rs:22`), with `net.ip` for lease
install (`src/ip_client/wire.rs:21`). This page and the pillar pages describe the code as it is.
...
|------|---------|----------------|
| [operations.md](operations.md) | `src/protocol/`, `src/server/` | The `NDHC` wire format, the request loop, the five lease ops, the status reply body, the errno set, and the reply path. |
| [lease.md](lease.md) | `src/dora/`, `src/dhcp/`, `src/state/`, `src/setup/` | The DISCOVER/OFFER/REQUEST/ACK ladder, NAK and timeout, renew and release, the client state machine, the lease record, and bring-up. |
| [transport.md](transport.md) | `src/l2_client/`, `src/ip_client/` | The `net.l2` raw-frame client (MAC, send, poll, set-ip) and the `net.ip` client that installs the lease with `OP_SET_CONFIG`. |
| [framing.md](framing.md) | `src/dhcp/`, `src/frame/` | The BOOTP fixed region and option build and parse, the subnet-mask-to-prefix conversion, and the Ethernet/IPv4/UDP compose and extract with the RFC 1071 checksum. |
| [contributing.md](contributing.md) | the whole tree | Where each concern lives, how to add a lease op, the build and sign steps, and the code standards. |
| [debugging.md](debugging.md) | runtime | The boot marker, the setup and initial-acquire loops, and the runtime errno failure modes: no link, timeout, and NAK. |

For where DHCP sits in the wider stack, see the [networking subsystem](../../subsystems/networking/README.md).
...
loops forever (`src/main.rs:35`). `setup::run` resolves `net.l2` and `net.ip` through the service registry,
caches both service ports, and reads the NIC MAC from `net.l2` so the data path does not repeat those
round-trips (`src/setup/run.rs:45`); a failed setup yields and retries (`src/main.rs:44`). The initial
acquire runs the DORA ladder up to sixteen times with a yield between tries, so a lease is usually bound by
the time the server accepts requests (`src/main.rs:55`). The kernel spawns it under the network spawn plan,
checking its signature and attestation and holding its manifest capabilities before its ELF is mapped
(`src/userspace/init/spawn_plan/network/spawn_dhcp.rs:20`). A successful spawn prints `[NET-DHCP] capsule spawned`
on the boot log; the [debugging](debugging.md) page covers what that marker means.
