# Contributing to capsule_net_dhcp
...
standards a change has to meet. For what the capsule does and how it fits together, read the [README](README.md),
...
`server::run`, which loops forever (`src/main.rs:35`). The top-level modules are declared there: `dhcp`,
`dora`, `frame`, `ip_client`, `l2_client`, `protocol`, `server`, `setup`, and `state` (`src/main.rs:22`).
...
1. Add the opcode constant to `src/protocol/ops.rs:17`. The current set runs `1` through `5`; a new op takes
   ...
   re-export it from `src/protocol/mod.rs:21`.
2. Write the handler as one file under `src/server/handlers/`, exposing a `handle(sender_pid, req, tx)`
   function that reads state through `STATE` if needed and replies with `respond` (`src/server/respond.rs:44`).
   Follow `lease_status.rs` (a read that writes a body) or `lease_request.rs` (an op that drives the DORA
   ladder). Declare the module in `src/server/handlers/mod.rs:17`.
3. Wire it into the dispatch match in `src/server/runner.rs:44`. Every op arm passes the sender pid through so
...
one-place reset-on-error and reject-does-not-install rules stay intact (`src/dora/acquire.rs:58`,
`src/dora/install.rs:31`).
...
`cfg` gate on the kernel-side embed (`src/userspace/capsule_net_dhcp/embed.rs:22`) and the spawn entry
...
- One unit per file. New ops are one file per handler under `src/server/handlers/`. `mod.rs` is used only for
  module declarations and re-exports, as in `src/dhcp/mod.rs:17` and `src/frame/mod.rs:17`.
- A rejected or incomplete lease must not mutate `net.ip`. The only install path is `dora::install`, reached
  ...
  (`src/dora/acquire.rs:41`, `acquire.rs:58`).
- Any inbound path must bound a wire-supplied length before it reads a body (`src/dhcp/parse/parse_options.rs:37`,
  `src/frame/udp.rs:51`, `src/l2_client/rx.rs:64`).
