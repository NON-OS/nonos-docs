# BOOTP messages and Ethernet framing
...
server reply is peeled back to a BOOTP payload. For the ladder that calls these see the [lease](lease.md)
page; for the IPC clients that carry the finished frames see the [transport](transport.md) page.
...
router, DNS, and lease seconds (`src/dhcp/message.rs:19`). `new_request` seeds a client request from the NIC
MAC and a transaction id, setting the BOOTP op to `OP_REQUEST` and the broadcast flag
(`src/dhcp/message.rs:36`).
...
identifier (54), parameter list (55), and END (0xFF) (`src/dhcp/constants.rs:20`). The DHCP message-type
values are DISCOVER 1, OFFER 2, REQUEST 3, ACK 5, NAK 6, RELEASE 7, and the BOOTP UDP ports are server 67 and
client 68 (`src/dhcp/constants.rs:52`).
...
router, and DNS, and the END marker (`src/dhcp/build.rs:30`). The returned length is the number of bytes
...
and walks the option block (`src/dhcp/parse/parse.rs:23`). `parse_fixed` reads op, xid, flags, `ciaddr`,
`yiaddr`, and `chaddr` from their fixed offsets (`src/dhcp/parse/parse_fixed.rs:20`). `parse_options` walks
...
(`src/dhcp/parse/parse_options.rs:21`). The error set is `TooShort`, `BadCookie`, `BadOption`
(`src/dhcp/parse/error.rs:17`).
...
client's, and the destination MAC broadcast (`src/frame/compose.rs:38`). The Ethernet header is 14 bytes with
ethertype `0x0800` for IPv4 (`src/frame/ethernet.rs:17`). The IPv4 header is a standard 20-byte header with
version 4, IHL 5, TTL 64, protocol 17 (UDP), and a header checksum sealed with `fold` (`src/frame/ipv4.rs:24`).
The UDP header is 8 bytes from client port 68 to server port 67, with a checksum sealed over the IPv4 pseudo-
header by `fold_with_pseudo` (`src/frame/udp.rs:22`).
...
(`src/frame/extract.rs:25`). The UDP parser bounds the declared length against the buffer before returning it
(`src/frame/udp.rs:44`), and the IPv4 parser rejects a version other than 4 and an IHL below 5
(`src/frame/ipv4.rs:52`).

## The checksum

`src/frame/checksum.rs` is the RFC 1071 ones-complement checksum. `fold` sums 16-bit big-endian words with a
trailing odd byte padded, folds carries back in until none remain, and complements, used for the IPv4 header
(`src/frame/checksum.rs:22`). `fold_with_pseudo` prepends the IPv4 pseudo-header (source, destination,
protocol, UDP length) before summing the body, and returns `0xFFFF` for a computed zero so a zero wire value
can keep its RFC "no checksum" meaning, used for UDP (`src/frame/checksum.rs:38`). The DHCP capsule owns its
own copy of this because it cannot route through `net.ip` for the outbound BOOTP exchange, where the source
IPv4 is still `0.0.0.0` (`src/frame/checksum.rs:17`).
...
that runs past the buffer (`src/dhcp/parse/parse_options.rs:37`), the UDP parser refuses a length below the
header or beyond the buffer (`src/frame/udp.rs:51`), and the IPv4 parser refuses a truncated header
(`src/frame/ipv4.rs:64`). The BOOTP parser rejects a short buffer and a wrong cookie before touching options
(`src/dhcp/parse/parse.rs:24`), and the subnet-mask-to-prefix conversion rejects a discontiguous mask rather
than producing a nonsense prefix (`src/dora/mask.rs:26`). All of this runs on frames the capsule itself pulled
