# The DHCP state machine and the lease

This page mirrors `src/dora/` (the DISCOVER/OFFER/REQUEST/ACK ladder and the renew and release paths),
`src/dhcp/client_state.rs` (the RFC 2131 client state), `src/state/` (the shared state and the cached lease),
...
[operations](operations.md) page; for the wire codecs and frames the ladder sends see the [framing](framing.md)
page; for the L2 and IP clients it drives see the [transport](transport.md) page.
...
`net.l2` into shared state (`src/setup/run.rs:45`). A missing `net.l2` is `SetupError::L2NotFound`, a missing
`net.ip` is `IpNotFound`, and a failed MAC read is `L2MacFailed` (`src/setup/run.rs:24`). The lookup helper
returns `NotFound` when the registry call fails or the port is zero (`src/setup/discover.rs:24`). `_start`
calls setup in a retry loop, yielding 64 times between attempts until it succeeds, so the capsule waits for
`net.l2` and `net.ip` to come up rather than failing outright (`src/main.rs:44`). After setup succeeds,
...
(`src/dhcp/client_state.rs:20`). The DORA driver and the renew and release handlers transition between these
explicitly, and `lease_status` maps them to the 1-byte code in its reply body (`src/server/handlers/lease_status.rs:29`).

## The acquisition ladder

`dora::acquire` is the DISCOVER/OFFER/REQUEST/ACK ladder run end to end (`src/dora/acquire.rs:29`):

1. It reads the cached L2 and IP ports and the MAC, and returns `NoLink` if any is unset
   (`src/dora/acquire.rs:32`).
...
3. `discover` sends one `DHCPDISCOVER` and waits for the first matching `DHCPOFFER` for that xid
   (`src/dora/discover.rs:31`). It moves state to `Requesting`.
4. `request` sends a `DHCPREQUEST` for the offered `yiaddr` against the announced `server_id`, then waits for
   either `DHCPACK` (success) or `DHCPNAK`; a NAK is `RequestError::Nak` (`src/dora/request.rs:32`).
...
half-configured (`src/dora/acquire.rs:58`). Discover failures map to `Timeout` (wait exhausted) or `NoLink`
...
L2 send failure is `L2Failed` (`src/dora/send_bootp.rs:31`). The BOOTP buffer is a fixed 576 bytes
...
frames (ARP, ICMP, races) are dropped (`src/dora/wait_reply.rs:37`). The poll budget is bounded to 4000
...
prefix, and router; an IP-side refusal is `IpRefused` (`src/dora/install.rs:31`). The DNS server from the ACK
...
ones plus trailing zeros exactly 32) before it returns a prefix length (`src/dora/mask.rs:20`).
...
a NAK collapses to Init (`src/server/handlers/lease_renew.rs:30`). It requires setup complete and a live lease
...
empty and Init; a release with no lease is idempotent (`src/server/handlers/lease_release.rs:29`). Both
handlers pull the `(l2, ip, mac)` trio through `current`, which returns `None` (answered as `E_NO_LINK`) until
setup has cached all three (`src/server/handlers/xid_mac.rs:22`).
...
so it needs no runtime initialisation (`src/state/global.rs:23`, `global.rs:65`). It holds the L2 and IP
service ports as `AtomicU32` (written once at setup with `Release`, read with `Acquire`), and the MAC, the
client state, and the lease each behind a `spin::Mutex` (`src/state/global.rs:24`). `next_xid` draws four
random bytes through `crypto_random`, returning `None` if the draw is short and substituting 1 for a zero
result so a zero xid is never minted (`src/state/global.rs:42`, `global.rs:56`).

The `Lease` record is the cached lease parameters: the IPv4, the prefix (the subnet mask as a bit count), the
gateway, the server id, the DNS, and the server-supplied lifetime in seconds (`src/state/lease.rs:23`). The
capsule does not run its own expiry timers; renewal is driven by the caller through `OP_LEASE_RENEW`
(`src/state/lease.rs:17`). `Lease::empty` is the all-zero unleased state release resets to (`lease.rs:34`).
