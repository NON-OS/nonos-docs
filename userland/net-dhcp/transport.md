...
these clients see the [lease](lease.md) page; for the frames they carry see the [framing](framing.md) page.
For where these layers sit in the stack, see the [networking subsystem](../../subsystems/networking/README.md).
...
`NL2\0` = `0x4E4C_3200` and version 1 (`src/l2_client/wire.rs:22`). Four L2 opcodes matter to DHCP:
`OP_GET_MAC = 2`, `OP_SEND_FRAME = 4`, `OP_POLL_FRAME = 5`, and `OP_SET_IP = 7`
(`src/l2_client/wire.rs:26`).
...
wrong magic or version by returning `None` (`src/l2_client/header.rs:19`, `header.rs:33`). Request ids come
from a monotonic counter that starts at 1 and skips zero on wraparound, so a zero id is never minted
(`src/l2_client/seq.rs:21`).
...
- `send_frame` issues `OP_SEND_FRAME` with a fully-built ethernet frame as its body and treats a transport
  ...
  header work and ships the frame verbatim (`src/l2_client/tx.rs:35`).
- `poll_frame` issues `OP_POLL_FRAME` and reads back one queued inbound frame, mapping the L2 errno set to a
  ...
  frame is returned as an owned `Vec` (`src/l2_client/rx.rs:42`). It bounds the declared payload length against
...
- `set_ip` issues `OP_SET_IP` with the 4-byte leased IPv4 as its body, so `net.l2` can source ARP from a real
  ...
  `net.ip` (`src/l2_client/set_ip.rs:36`).

## The net.ip client

`src/ip_client/` speaks the `net.ip` v1 request envelope, a 20-byte header distinguished by the magic
`NIP4` = `0x4E49_5034` and version 1 (`src/ip_client/wire.rs:21`). DHCP only ever issues one IP opcode,
`OP_SET_CONFIG = 3`; every other `net.ip` op is for upper transport capsules (`src/ip_client/wire.rs:25`).
The envelope codec and the monotonic request-id counter mirror the L2 client's (`src/ip_client/header.rs:19`,
`src/ip_client/seq.rs:21`).

- `apply_lease` issues `OP_SET_CONFIG` with a 9-byte body that matches the IP capsule's set-config handler
  exactly: 4-byte IPv4, 1-byte prefix, 4-byte gateway (`src/ip_client/set_config.rs:34`). A negative transport
...
- `clear_lease` calls `apply_lease` with an all-zero body, bringing the interface back to unconfigured; after
  it, `net.ip` refuses any send until the next lease lands (`src/ip_client/set_config.rs:65`). Release uses it
...
and `acquire` calls `set_ip` to announce the address to `net.l2` (`src/dora/send_bootp.rs:49`,
`src/dora/wait_reply.rs:43`, `src/dora/install.rs:33`, `src/dora/acquire.rs:45`). On release, the handler
calls `release` (which sends through `send_frame`) and then `clear_lease` (`src/server/handlers/lease_release.rs:42`,
`lease_release.rs:44`).
