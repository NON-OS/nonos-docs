...
the [operations](operations.md) page, the [resolver](resolver.md) page, and the [transport](transport.md)
page.
...
(`src/userspace/init/spawn_plan/network/spawn_dns.rs:21`), which forwards to `capsule_boot::boot`
(`src/userspace/init/spawn_plan/boot.rs:26`), whose `Ok` arm calls `boot_log::ok(prefix, "capsule spawned")`
(`src/userspace/init/capsule_boot/run.rs:29`), which prints `[` + tag + `] ` + message
...
(`src/userspace/init/capsule_boot/run.rs:34`). The line only appears when the kernel is built with the
`nonos-capsule-net-dns` feature, the `cfg` gate on the spawn entry
(`src/userspace/init/spawn_plan/network/spawn_dns.rs:18`), and the DNS capsule is spawned only on the legacy
per-capsule network stack, which is itself gated on `not(feature = "nonos-capsule-net-core")`
(`src/userspace/init/spawn_plan/network/spawn_legacy_stack.rs:18`). A kernel built with the combined
...
yielding 64 times between attempts and retrying forever until it succeeds (`src/main.rs:41`). So a capsule
that spawned but never answers a request is almost always stuck in that loop because `net.udp` has not come
up. `setup::run` fails for one of two reasons (`src/setup.rs:30`):

- `mk_service_lookup("net.udp")` returned non-zero or a zero port, so the UDP transport is not registered
  yet (`SetupError::UdpMissing`, `src/setup.rs:34`). The fix is upstream: `net.udp`, and the IP and L2
...
- The lookup succeeded but the local port could not be minted or bound, so `net.udp` answered but the bind
  failed (`SetupError::BindFailed`, `src/setup.rs:37`). Minting the port needs the kernel entropy source, so
  a failure here also points at the `Crypto` right (`src/state.rs:32`).
...
the 16-bit value at offset 8 of the reply (`src/server/respond.rs:34`).
...
(`src/server/runner.rs:41`), and a receive with a zero sender pid is skipped (`runner.rs:38`). If a client
...
the body actually sent (`src/server/parse_req.rs:27`).

### `E_BAD_OP` (3)

The opcode was not one of the five the dispatch handles (`src/server/runner.rs:48`). Only `1` through `5`
are defined (`src/protocol/ops.rs:17`).

### `E_BAD_LEN` (4)

The body length was wrong for the op. On the request path this is a `payload_len` that overruns the received
buffer (`src/server/parse_req.rs:40`). The one handler that returns it directly is `OP_SET_UPSTREAM`, which
requires exactly 4 bytes of resolver IPv4 (`src/server/handlers/upstream.rs:28`).

### `E_NAME_INVALID` (9)

The host name was empty, longer than 255 bytes, not valid UTF-8, or not encodable as DNS labels. The name
check rejects the first three (`src/server/handlers/resolve_common.rs:29`), and a name that fails to encode
into labels (an empty or over-63-byte label, or a name that overflows the buffer) maps to `E_NAME_INVALID`
on the resolve path (`src/server/handlers/resolve_a.rs:40`, `src/dns/name/encode.rs:27`).

### `E_TIMEOUT` (6)

No usable response arrived within the 3000 ms deadline (`src/server/handlers/resolve_common.rs:26`). The
...
section that held no A or AAAA record (`src/server/handlers/resolve_common.rs:71`, `resolve_common.rs:77`,
`src/dns/response/parse.rs:50`). This is a definitive negative answer, not a transport problem.
...
(`src/server/handlers/resolve_common.rs:74`, `resolve_common.rs:67`, `src/server/handlers/resolve_a.rs:35`).
...
(`src/server/handlers/flush.rs:24`, `src/server/handlers/upstream.rs:24`, `src/server/authz.rs:38`). Because
...
and evicts round-robin when full (`src/dns/cache/ops.rs:41`). A cached entry is served until its TTL expires
against the millisecond clock (`src/dns/cache/ops.rs:31`). The `net.admin` principal can force a full flush
with `OP_FLUSH_CACHE`, which ticks every entry out with `u64::MAX` (`src/server/handlers/flush.rs:28`). The
AAAA path does not cache, so a AAAA answer is always fresh from the upstream
(`src/server/handlers/resolve_aaaa.rs:23`).
