# Client operations and the NDNS protocol
...
[README](README.md); for the DNS engine the resolve handlers reach, see the [resolver](resolver.md) page,
and for the UDP client under it, the [transport](transport.md) page.

## The wire format

A request header is 20 bytes and begins with a magic and a version. The parser rejects anything shorter than
the header, a wrong magic, or a version other than 1 (`src/server/parse_req.rs:27`). The `NDNS` magic is
...
(`src/server/parse_req.rs:40`). On success it returns the parsed `op` and `request_id` and a slice pointing
...
payload length at offset 16 (`src/server/respond.rs:23`). A resolve reply carries an address body after that
header; every other op replies with the header alone. Replies go back to the requesting pid with
`mk_ipc_reply` (`src/server/respond.rs:38`).

## The request loop

`server::run` sizes one receive buffer and one transmit buffer, each to the header plus `IPC_PAYLOAD_MAX`
(`src/server/runner.rs:33`). `IPC_PAYLOAD_MAX` is the 512-byte DNS response ceiling plus 64 bytes of op
framing slack (`src/protocol/limits.rs:17`).

The loop receives a request with `mk_ipc_recv_from`, which also reports the sender pid
(`src/server/runner.rs:37`). A receive of zero or fewer bytes, or a receive with a zero sender pid, is
...
The opcodes are defined in `src/protocol/ops.rs` and dispatched in `src/server/runner.rs:42`.

| Op | Opcode | Request body | Reply body after header | Handler |
|---|---|---|---|---|
| `OP_HEALTHCHECK` | `1` | none | none (status only) | `server/handlers/health.rs:21` |
| `OP_RESOLVE_A` | `2` | host name, 1..=255 bytes | 4-byte IPv4 on success | `server/handlers/resolve_a.rs:24` |
| `OP_RESOLVE_AAAA` | `3` | host name, 1..=255 bytes | 16-byte IPv6 on success | `server/handlers/resolve_aaaa.rs:23` |
| `OP_FLUSH_CACHE` | `4` | none | none (status only), admin only | `server/handlers/flush.rs:23` |
| `OP_SET_UPSTREAM` | `5` | 4-byte IPv4 resolver | none (status only), admin only | `server/handlers/upstream.rs:23` |

The opcode constants are `OP_HEALTHCHECK = 1`, `OP_RESOLVE_A = 2`, `OP_RESOLVE_AAAA = 3`,
`OP_FLUSH_CACHE = 4`, `OP_SET_UPSTREAM = 5` (`src/protocol/ops.rs:17`).

## Per-op behaviour

- `OP_HEALTHCHECK` replies `E_OK` with an empty body; it proves the server is alive and touches no state
  (`src/server/handlers/health.rs:22`).
- `OP_RESOLVE_A` reads the host name from the body, validates it is 1..=255 bytes of UTF-8
  (`E_NAME_INVALID` otherwise, `src/server/handlers/resolve_common.rs:29`), checks the answer cache, and on
  a hit writes the cached IPv4 and replies `E_OK` (`src/server/handlers/resolve_a.rs:30`). On a miss it mints
...
- `OP_RESOLVE_AAAA` is the same shape for a AAAA record but does not consult the IPv4 cache: it builds an
  ...
  (`src/server/handlers/resolve_aaaa.rs:23`). The cache holds A records only.
- `OP_FLUSH_CACHE` is an administrative op. It is denied unless the sender is the `net.admin` principal
  (`E_PERM`, `src/server/handlers/flush.rs:24`); when authorized it expires every cache entry by ticking the
...
- `OP_SET_UPSTREAM` is an administrative op. It is denied unless the sender is `net.admin` (`E_PERM`,
  `src/server/handlers/upstream.rs:24`); it requires a body of exactly 4 bytes (`E_BAD_LEN` otherwise,
...
(`src/server/handlers/flush.rs:24`, `src/server/handlers/upstream.rs:24`). `admin` resolves the owner pid
the registry bound to the service name `net.admin` and compares it to the kernel-attested sender pid; a
lookup miss or a pid of zero denies (`src/server/authz.rs:38`). This is fail-closed by construction: no
...
(`src/server/authz.rs:34`). The two resolve ops and the health check are open to any caller that can reach
the endpoint; only the two ops that change resolver policy are gated.

## The error set

All errno words are unsigned 16-bit values in the response header at offset 8 (`src/protocol/errno.rs`):
...
parse failure is dropped in the request loop rather than answered (`src/server/runner.rs:41`), so a client
...
(`src/server/parse_req.rs:27`). A request with a zero sender pid is skipped, so every served request is
attributable to a caller (`src/server/runner.rs:38`). The two policy-changing ops are gated on the
kernel-attested sender pid matching the `net.admin` principal, and default to denied because no such
principal exists yet (`src/server/authz.rs:38`). A resolve name is bounded to the 255-byte DNS wire limit
and checked as UTF-8 before it is encoded (`src/server/handlers/resolve_common.rs:29`). Transaction ids and
the local source port are drawn from the kernel entropy source, so a response is bound to the query id and
question before it is accepted (`src/state.rs:30`, `src/server/handlers/resolve_common.rs:68`). There is no
