# The UDP client and bring-up

This page mirrors `src/udp_client/` (the IPC client that carries DNS datagrams to `net.udp`) and
`src/setup.rs` (bring-up). It is the layer the resolve exchange reaches when it needs the wire: the DNS
engine builds a query, and this client ships it to the UDP transport capsule and reads the reply back. For
the exchange loop that drives it see the [resolver](resolver.md) page; for the request protocol above it,
the [operations](operations.md) page. For where DNS sits above UDP and IP in the stack, see the
...
(`src/udp_client/header.rs:17`). Three UDP opcodes matter to DNS: `OP_BIND = 2`, `OP_SEND = 4`, and
`OP_RECV = 5` (`src/udp_client/header.rs:20`). The DNS capsule is a client of the UDP capsule exactly the way
...
field, the request id, and the payload length, all little-endian (`src/udp_client/header.rs:24`). `parse`
...
(`src/udp_client/header.rs:33`). The DNS capsule uses fixed request ids per call site (`1` for bind, `2` for
send, `3` for recv) because it issues one call at a time and matches the reply by opcode rather than by id
(`src/udp_client/bind.rs:30`, `src/udp_client/send.rs:39`, `src/udp_client/recv.rs:43`).
...
`mk_ipc_call` (`src/udp_client/bind.rs:28`). A transport error is `Send`, a reply that does not parse or
...
exactly the header plus that body (`src/udp_client/send.rs:37`). It issues `mk_ipc_call` and maps a transport
error to `Send`, a wrong or unparseable reply to `BadResponse`, and any non-zero UDP errno to `Refused`
(`src/udp_client/send.rs:46`). The DNS capsule always sends to the upstream on port 53
(`src/server/handlers/resolve_common.rs:43`), which is the transport page's counterpart to the resolver
...
plus a 512-byte DNS payload, and issues `mk_ipc_call` (`src/udp_client/recv.rs:41`). It maps the UDP errno
set to a typed error: a transport error is `Send`, a wrong opcode or a short reply is `BadResponse`, the UDP
`E_RX_EMPTY` value `8` is `Empty`, and any other non-zero errno is `Refused` (`src/udp_client/recv.rs:47`).
...
(`src/server/handlers/resolve_common.rs:48`).

## The state under the client

The client is stateless; the ports and the upstream it needs live in `src/state.rs`.
...
- `UDP_PORT` is an `AtomicU32` holding the resolved `net.udp` service port, written once at setup with
  ...
  the setup that produced it (`src/state.rs:26`, `src/state.rs:38`).
- `LOCAL_PORT` is an `AtomicU16` holding the capsule's chosen UDP source port. It is minted once, lazily, as
  ...
  callers racing to pick a port converge on one value (`src/state.rs:46`, `src/state.rs:51`).
- `UPSTREAM` is a `Mutex<[u8; 4]>` defaulting to `1.1.1.1`, read by the exchange and written by DHCP
  discovery or the admin op (`src/state.rs:23`, `src/state.rs:28`, `src/state.rs:70`).
- `next_xid` draws a fresh 16-bit DNS transaction id from the same entropy source per query
  (`src/state.rs:58`), and `now_ms` reads the kernel millisecond clock for the cache TTL math, clamping a
  negative raw value to zero (`src/state.rs:62`).

The entropy for the local port and the transaction id comes from `crypto_random`, which is why the manifest
mask carries the `Crypto` bit (`src/state.rs:32`; see the [README](README.md) identity table).

## Bring-up

`setup::run` is the whole bring-up (`src/setup.rs:30`). It resolves `net.udp` through `mk_service_lookup`
and fails `UdpMissing` if the lookup returns non-zero or a zero port (`src/setup.rs:33`). It then mints the
local source port, binds it on `net.udp`, and fails `BindFailed` if either step fails (`src/setup.rs:37`).
On success it stores the UDP service port in shared state and calls `dhcp_upstream::apply` to adopt the
lease's DNS server as the upstream if one is available (`src/setup.rs:39`; the DHCP path is on the
[resolver](resolver.md) page). `_start` calls setup in a retry loop, yielding 64 times between attempts
until it succeeds, so the capsule waits for `net.udp` to come up rather than failing outright
(`src/main.rs:41`). Only after setup succeeds does the server loop start.
