# The DNS engine, the cache, and the resolve exchange

This page mirrors `src/dns/` (the DNS header, name coding, query builders, response parser, and answer
cache), the resolve exchange loop in `src/server/handlers/resolve_common.rs`, and the upstream discovery in
`src/dhcp_upstream/`. It is the machinery a resolve op reaches after the server has parsed and dispatched a
request. For that request loop and the op set see the [operations](operations.md) page; for the UDP client
the exchange rides on, see the [transport](transport.md) page. For where DNS sits above UDP in the stack,
see the [networking subsystem](../../subsystems/networking/README.md).

## The DNS header

A DNS message opens with a 12-byte header (`src/dns/header.rs:17`). The capsule reads the four fields it
needs: the 16-bit id, the flags, the question count, and the answer count, all big-endian
(`src/dns/header.rs:35`). Two flag helpers matter: `is_response` tests the `QR` bit `0x8000`
...
and a question count of one (`src/dns/query.rs:49`).

## Encoding a name

`encode` turns a dotted host name into DNS wire form: a sequence of length-prefixed labels ending in a zero
byte (`src/dns/name/encode.rs:20`). An empty name or a bare `.` encodes as the single zero root
...
`src/dns/types.rs`: `NAME_MAX = 255`, `LABEL_MAX = 63`, and `POINTER_MASK = 0xC0`
(`src/dns/types.rs:17`).
...
(`src/dns/name/skip.rs:28`, `skip.rs:42`), a byte over `LABEL_MAX` is a `BadPointer`, and a step counter
bounded to `NAME_MAX` catches a pointer loop as `LoopDetected` (`src/dns/name/skip.rs:31`, `skip.rs:36`). A
...
`BadPointer`, `LoopDetected`, `TooLong`, `LabelTooLong` (`src/dns/name/error.rs:17`).

## Building a query

`build_a_query` and `build_aaaa_query` frame a complete query for the A (`TYPE_A = 1`) and AAAA
(`TYPE_AAAA = 28`) record types (`src/dns/query.rs:36`, `src/dns/types.rs:21`). The shared `build_query`
...
the 2-byte `CLASS_IN = 1` class (`src/dns/query.rs:44`). A buffer too small for the header or the trailing
...
(`src/dns/response/parse.rs:22`). It parses the header, rejects a message whose `QR` bit is clear as
...
otherwise, `src/dns/response/record.rs:22`). An A record with exactly 4 bytes of data yields an IPv4, a
...
`BadName`, and `NotAResponse`, with a `NameError` folding into `BadName` (`src/dns/response/error.rs:19`).
The parsed `Answer` carries the record type, the TTL, and the optional IPv4 and IPv6
(`src/dns/response/answer.rs:17`).

## The answer cache

The cache is a fixed table of `ENTRY_CAP = 128` slots, each holding a name hash, up to `NAME_BYTES = 64`
bytes of name, the length, the IPv4, and an absolute expiry in milliseconds (`src/dns/cache/entry.rs:19`).
It is a `const fn new` table of `None` slots, so it needs no heap to exist (`src/dns/cache/entry.rs:37`).
...
- `lookup` finds a live entry by comparing the FNV-1a name hash and the stored name bytes, and returns the
  IPv4 only if the entry has not expired against the caller's clock (`src/dns/cache/ops.rs:23`). The hash is
  a case-folding FNV-1a, so `Example.com` and `example.com` collide to the same key
  (`src/dns/cache/hash.rs:17`).
- `insert` reuses the slot already holding the name, or the first empty slot, and otherwise evicts a slot
  ...
  rather than failing (`src/dns/cache/ops.rs:35`, `ops.rs:41`). The expiry is the insert time plus the TTL,
  computed with a saturating add so a huge TTL cannot wrap (`ops.rs:70`).
- `tick` clears every entry whose expiry is at or before the supplied time (`src/dns/cache/ops.rs:45`).
  Passing `u64::MAX` expires the whole table, which is exactly how the flush op empties the cache
  (`src/server/handlers/flush.rs:28`).

## The resolve exchange

`exchange` is the request-response loop against the upstream (`src/server/handlers/resolve_common.rs:36`).
...
(`src/server/handlers/resolve_common.rs:66`, `resolve_common.rs:68`). An `RCODE_NXDOMAIN` becomes
...
`question_matches` confirms a response echoes the query's question (`src/dns/question.rs:19`). It finds the
end of the query name and the response name, compares them case-insensitively label by label, and then
compares the two bytes of record type that follow each name (`src/dns/question.rs:28`). The name walk here
rejects a compression pointer or an over-long label outright rather than following it, and bounds its step
count to `NAME_MAX`, so a crafted question cannot loop or overrun (`src/dns/question.rs:44`). Case folding
uses the same lowercasing rule as the cache hash (`src/dns/question.rs:60`).

## Upstream discovery from DHCP

The upstream resolver defaults to `1.1.1.1` (`src/state.rs:23`), but setup tries to replace it with the DNS
...
the lease-status reply; if it is non-zero it becomes the upstream (`src/dhcp_upstream.rs:26`,
`src/dhcp_upstream/status.rs:19`). A missing DHCP service, a non-`OK` reply, or an all-zero DNS field leaves
the default in place (`src/dhcp_upstream/lookup.rs:23`, `src/dhcp_upstream/status.rs:27`). The
administrative `OP_SET_UPSTREAM` op can override it at runtime (see the [operations](operations.md) page).
