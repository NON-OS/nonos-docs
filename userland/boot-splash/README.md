...
The short version of what it is lives in the [desktop overview](../desktop-fleet.md); the real
attestation check it only displays lives under [attest](../attest/README.md) and the
...
capabilities and no others (`src/userspace/capsule_boot_splash/spawn.rs:50` through `:54`). This is the
same leaf-renderer mask the [wallpaper](../wallpaper/README.md), [login](../login/README.md), and
[setup wizard](../setup-wizard/README.md) capsules carry: it can query the display, register one surface,
...
One identity quirk that the [desktop overview](../desktop-fleet.md) flags: the `Capsule.mk` reserves
...
receive the loop makes is on port 0 (`src/main.rs:112`).
...
|------|---------|----------------|
| [rendering.md](rendering.md) | `paint`, `detail`, `chrome`, `vignette` | What the splash draws: the vignette background, the wordmark and subtitle, the attestation panel and its verdict, the spinner band, and the `D`-key detail view. |
| [contributing.md](contributing.md) | the whole tree | Where to work, how to change the splash or its wire protocol, the build and sign make targets, and the code standards. |
| [debugging.md](debugging.md) | runtime | The boot marker, the exit codes, and where to look when the splash does not appear, does not hand off, or the `D` key does nothing. |
...
no long-lived server loop (`src/main.rs:31`, `:38`):
...
4. The loop animates the spinner, processes any key delivery (the `D` toggle), and watches for
   `desktop_shell` to register (`src/main.rs:97` through `:137`).
5. When `desktop_shell` appears the splash records the time, waits a one-second settle so the desktop
   ...
   (`src/main.rs:103`, `:106`, `:108`; `SETTLE_MS`, `MAX_DWELL_MS`, `MAX_ITERS` at `:25` through `:27`).
...
view with the kernel hash and the ZK program hash (`src/main.rs:52`, `src/paint.rs:56`, `src/detail.rs:33`).
It **displays** the kernel's attestation; it does not verify a proof itself. See
[rendering.md](rendering.md#the-attestation-panel) for the verdict wiring and
...
header-plus-status reply and treats a nonzero status as an error (`src/proto.rs:39`, `:42`, `:46`).
...
(`src/main.rs:112`, `src/input.rs:27`, `:64` through `:73`).
...
(`userland/libc/src/attest.rs:21`, `:30`), called once at `src/main.rs:52`.
...
(`Capsule.mk:17`, `src/userspace/capsule_boot_splash/spawn.rs:50` through `:54`). Beyond mmapping one
...
- It displays, it does not verify. This is the point worth restating. The badge is `mk_attest_status`'s
  `zk_verified` field, read out of the kernel and painted verbatim (`src/main.rs:52`, `src/paint.rs:56`).
  ...
  [attest](../attest/README.md) page and the [proof system](../../subsystems/proof-system/README.md) carry
  the real check.
- The key grab is a router grant, not a splash power. The splash takes an exclusive key grab so it can
  ...
  grab is granted (`userland/capsule_input_router/src/server/handlers/grab_request.rs:25`, `:33`). A
  capsule not on that list gets `E_ACCES`. The authority lives in the router, gated by identity; the
  splash merely qualifies, and it releases the grab and exits when the shell comes up (`src/main.rs:73`).
- Isolation is the kernel's. The surface is a private anonymous mapping the splash registers and later
  releases (`src/surface.rs:24`, `:47`, `src/main.rs:61`); it holds no persistent state and writes no
