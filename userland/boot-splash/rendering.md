...
setup and the compositor wire, see the [README](README.md#protocol-and-ipc). To change any of this, see
...
(`src/surface.rs:24`, `:25`, `:47`, `:51`). Every draw call takes the raw base pointer and a `spx` value,
the stride in pixels, computed as `stride / 4` (`src/paint.rs:31`, `src/detail.rs:34`). Pixels are
...
(`src/main.rs:58`, `:122`, `:133`). The full-frame paints damage the whole surface; the spinner repaint
...
horizontal strip so the spinner can redraw without a full repaint (`src/vignette.rs:20`, `:24`). The
gradient is computed per pixel, no lookup table:

- The center is horizontal middle, vertically at 38 percent of the height (`src/vignette.rs:25`, `:26`),
...
- For each pixel it takes the squared distance from that center, with the vertical delta scaled by 12/10
  ...
  `r2 = w*w / 3`, and clamps (`src/vignette.rs:27`, `:31`, `:32`).
- That parameter `t` drives `lerp` from the core color `0xFF06_201C`, a very dark teal, out to pure black
  `0xFF00_0000` (`src/vignette.rs:17`, `:18`, `:33`). `lerp` blends each of the three color channels
  independently by walking the byte positions 0, 8, 16 (`src/vignette.rs:38` through `:47`).
...
`paint::splash` composes the full boot frame in order (`src/paint.rs:30`):

1. Fill the vignette background (`src/paint.rs:33`).
2. The wordmark `NØNOS`, drawn at scale 5 (`TITLE_SCALE`) through the shared `nonos_toolkit` font atlas,
   ...
   the glyphs on top in `ACCENT` `0xFF00_D4AA` (`src/paint.rs:22` through `:24`, `:27`, `:35` through
   `:40`). The `Ø` is the byte `0xD8` in the title literal (`src/paint.rs:35`). It sits 132 pixels above
   vertical center (`src/paint.rs:38`).
3. The subtitle `zero-state attestation boot`, centered under the wordmark in the dim color `0xFF5B_6B78`
   (`src/paint.rs:22`, `:41` through `:43`).
4. The attestation panel (below).
5. The status band with the spinner (below), painted at frame 0 for the first draw (`src/paint.rs:45`).

### The attestation panel

`attest_panel` draws a 380-pixel-wide box titled `boot-chain attestation`, centered horizontally, 24
pixels above vertical center (`src/paint.rs:48` through `:52`). Inside it are four lines:

- `[+] bootloader  ed25519 verified` in green (`OK` `0xFF00_CC66`) (`src/paint.rs:25`, `:53`).
- `[+] kernel      blake3 verified` in green (`src/paint.rs:54`).
- `[#] capsules    zk attested` in accent teal (`src/paint.rs:55`).
- The live verdict line (`src/paint.rs:56` through `:61`).
...
(`src/main.rs:52`, `src/paint.rs:56`):
...
(`src/main.rs:52`, `:120`). Because the splash only displays this field, a green badge is the kernel's
claim, not the splash's proof; the [README security section](README.md#security) explains why that is
sound.

### The status band

The bottom-of-screen status line is the only animated element. `paint::status` draws it 132 pixels below
vertical center (`src/paint.rs:64`, `:65`):

- It first clears a 16-row band with `vignette::fill_band` so the previous glyphs are erased without a
  full repaint (`src/paint.rs:66`, `src/vignette.rs:24`).
- It draws a spinner glyph from `|/-\` selected by `frame % 4` in accent teal, the label
  ...
  in accent after the label (`src/paint.rs:28`, `:68` through `:74`).
...
glyph (`src/main.rs:128`, `:131`, `:132`). A spinning splash means the capsule is alive and cycling; it is
...
Pressing `D` (upper or lower case) toggles the detail view (`src/main.rs:116`). `detail::detail` repaints
the whole surface (`src/detail.rs:33`):

- Vignette background, then a wide panel from `(40, 60)` spanning `width - 80` by 220 pixels, titled
  `boot-chain attestation` (`src/detail.rs:36` through `:38`).
- The kernel BLAKE3 hash: the label `kernel blake3` in dim, then the 32-byte `att.kernel_blake3` rendered
  as 64 lowercase hex characters by `hex32` (`src/detail.rs:25` through `:31`, `:39` through `:42`).
- The ZK program hash: the label `zk program hash` in dim, then `att.program_hash` in hex
  (`src/detail.rs:43` through `:45`).
- Two colored flags drawn by `flag`, green `OK` when set and amber `WARN` when not: `secure_boot` from
  `att.secure_boot == 1`, and `attested` from `att.zk_verified == 1` (`src/detail.rs:46`, `:47`, `:51`
  through `:53`).
- A footer `press any key to return`, 40 pixels up from the bottom (`src/detail.rs:48`).

The detail view is drawn from the `AttestStatus` the loop already holds, not a fresh read; the same struct
that produced the splash badge produces these hashes and flags (`src/main.rs:51`, `:118`). Any key press
with the detail open toggles it back to the splash (`src/main.rs:116`, `:120`). While the detail view is
open the handoff is suppressed, so a held detail view will not let the splash exit out from under the
reader (`src/main.rs:108`, guarded by `!show_detail`); this is covered under
[debugging.md](debugging.md#the-splash-stays-on-screen-and-never-hands-off).
