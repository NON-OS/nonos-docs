# Debugging capsule_wallet_nonos
...
Work the path from the outside in. For the model see the [README](README.md), the [views](views.md), the
[signing](signing.md), the [network](network.md), and the [rendering](rendering.md) pages in this folder.

## The boot marker

The first thing to confirm is that the capsule ran. On a successful boot the kernel logs
`[APP-NONOS-WALLET] capsule spawned` from the capsule boot path: the `Ok` arm calls
...
`status` line is `keyring pending` (`src/wallet/state/new.rs:54`); the first event or tick runs `hydrate`
once behind the `ready` flag (`src/wallet/app.rs:41`, `:52`). `hydrate` turns the status into
`wallet ready`, `keyring unavailable`, or `rail refresh failed` (`src/wallet/state/hydrate.rs:30`, `:36`,
`:38`).
...
- The `_ready` flags separate a keyring problem from a network problem: `address_ready` with no
  `balance_ready` is an address that loaded but a chain read that did not (`src/wallet/state/types.rs:42`).
...
order and folds the result into `NetStatus` (`src/wallet/net/probe.rs:22`). `probe_rpc_tcp` reports
`resolve`, `socket`, and `connect` as three separate booleans, so a DNS failure, a socket-open failure,
and a TCP-connect failure are distinguishable (`src/wallet/net/probe_rpc_tcp.rs:24`); `probe_tls_rpc` then
attempts the full TLS 1.3 handshake, and `probe_status` turns the combination into the status string the
Home view shows (`src/wallet/net/probe_status.rs:17`).
...
(`src/wallet/tls13/chain_anchor.rs:17`, `gts_r4_anchor.rs:17`), and `cert_valid_now` rejects one outside
its validity window against the RTC (`src/wallet/tls13/cert_valid_now.rs`,
`src/wallet/net/rtc_stamp.rs`). An endpoint that swaps its CA, or a wrong clock, presents exactly this way:
...
`proof_nox_hash` and sets `proof_count`, rendered in the Proofs view (`src/wallet/event/sign_both.rs:57`,
`src/wallet/state/types.rs:65`). The status line reports `transaction signed`, `2 tx proofs ready`,
`transaction sign failed`, or `transaction hash failed` (`src/wallet/event/sign_result.rs:30`, `:35`,
...
`receipt pending`, or `receipt confirmed` (`src/wallet/event/broadcast.rs:27`, `:37`).
...
into the surface (`src/wallet/paint/paint.rs:21`). A blank frame with a live status line points at the
...
button-down, and ignores the rest (`src/wallet/event/on_event.rs:21`).

## Source map

```
  src/userspace/init/capsule_boot/run.rs           [APP-NONOS-WALLET] capsule spawned / error path
