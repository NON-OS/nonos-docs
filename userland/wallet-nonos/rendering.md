...
This page mirrors `src/wallet/paint/` and `src/wallet/manifest.rs`. The renderer is a pure projection of
...
[views](views.md) handlers own the state, and `paint` only reads it. For the whole capsule see the
[wallet overview](README.md).

## The window manifest

The window is described once, in `manifest` (`src/wallet/manifest.rs:26`). It is a Normal window titled
`NONOS Wallet`, opened at `(370, 128)` at the theme's `WIDTH x HEIGHT` (1180 by 720,
`src/wallet/theme.rs:17`).

| Field | Value | Source |
|---|---|---|
| Title | `NONOS Wallet` | `manifest.rs:28` |
...
(`manifest.rs:21`, `:22`, `:23`). Those are exactly the events the [views](views.md) gate handles; the
window subscribes to nothing else.

## The paint dispatch

`paint` is the single entry the skeleton calls each frame (`src/wallet/paint/paint.rs:21`). It draws the
...
(`src/wallet/paint/paint_sidebar.rs:22`). The four nav rows sit at y-offsets 160, 212, 264, and 316, each
...
`src/wallet/event/on_pointer.rs:46`, which is why a sidebar click and the drawn row line up.
...
`hex_hash` (`src/wallet/paint/mod.rs:17`). The palette lives in `src/wallet/theme.rs:19` and is shared with
...
`userland/capsule_wallet_nonos/src/wallet/manifest.rs` and `theme.rs` (the window and palette).
