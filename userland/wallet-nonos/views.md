...
signing, network, rendering) see the [wallet overview](README.md).
...
everything else is ignored (`src/wallet/event/on_event.rs:21`).
...
chain read succeeds (`types.rs:42`). This split is what the [debugging](debugging.md) page reads to
localise a stuck wallet.

## Navigation and views

Key handling is a flat set of single-key shortcuts in `on_key` (`src/wallet/event/on_key.rs:21`); the
pointer handler hit-tests the sidebar, buttons, and Send fields (`src/wallet/event/on_pointer.rs:21`).
...
| Action | Key / click | Effect | Source |
|---|---|---|---|
| Generate a wallet | `g` / `G`, or the Generate button | ask the keyring to create a wallet, fetch its address, switch to Receive | `on_key.rs:35`, `on_pointer.rs:31`, `src/wallet/event/generate.rs:22` |
| Refresh / hydrate | `r` / `R` | re-run the keyring and rail hydrate step | `on_key.rs:22`, `src/wallet/state/hydrate.rs` |
...
The Send form has three fields cycled with Tab and edited in place (`src/wallet/event/send_input.rs:21`).
...
| Edit recipient | hex digit while To is focused | append one hex nibble, up to 40 (a 20-byte address) | `send_input.rs:33`, `send_input.rs:51`, `src/wallet/event/recipient.rs` |
| Edit amount | digit while Amount is focused | build the amount in milli-ETH | `send_input.rs:34`, `src/wallet/event/edit_amount.rs` |
| Edit nonce | digit while Nonce is focused | build the nonce | `send_input.rs:35`, `src/wallet/event/edit_nonce.rs` |
...
reports `amount too large` rather than wrapping (`src/wallet/event/eth_value.rs`, `sign_eth.rs:31`). The
...
|---|---|---|---|
| Sign ETH transfer | `E` (or Enter in Send) | build an EIP-1559 transfer, sign it through the keyring, hash it, store it as a proof, switch to Proofs | `on_key.rs:37`, `src/wallet/event/sign_eth.rs:22` |
| Sign NOX approve | `n` / `N` | request a NOX approval signature from the keyring, hash it, store it as a proof | `on_key.rs:38`, `src/wallet/event/sign_nox.rs` |
...
and switches to the Proofs view (`src/wallet/event/sign_result.rs:24`, `src/wallet/state/record_tx.rs`).
...
`:37`). The signing marshalling itself is on the [signing](signing.md) page.
...
- Demonstration. The EIP-1559 gas parameters in the ETH transfer are fixed constants rather than derived
  from the live fee estimate (`src/wallet/ipc/sign_eth.rs:36`), so a send is a plain 21,000-gas transfer,
  not a general contract call. The NOX path signs a fixed approve template rather than a user-composed
  call (`src/wallet/ipc/sign_nox.rs`). The Send form drives the ETH path end to end; the NOX and sign-both
