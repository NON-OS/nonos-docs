# Debugging capsule_driver_virtio_blk

This page lists the log markers the driver and its boot path emit, and the concrete failure modes with
where to look for each. For the client protocol see the [client](client.md) page, for the device side the
[bring-up](bringup.md) page, and for the request engine the [queue](queue.md) page.
...
(`src/userspace/init/spawn_plan/drivers_virtio_io.rs:39`, `src/userspace/init/capsule_boot/run.rs:29`). If
...
an ELF or mapping failure surfaces under that prefix (`src/hardware/virtio_blk_capsule/spawn.rs`).
...
serves nothing is stuck in bring-up (`src/main.rs:34`). The usual causes are broker refusals: the claim is
...
is present before any driver runs ([claim](../../subsystems/hardware-broker/claim.md)). A DMA refusal prints
a `[DMA]` line naming the failed check ([dma](../../subsystems/hardware-broker/dma.md)). If discovery itself
...
same as an absent one (`src/setup/sequence.rs:24`, `src/discover.rs:37`).

### Bring-up aborts on negotiation

`bring_up` returns a specific string for each virtio failure: `virtio-blk: features-ok rejected`,
`virtio-blk: requestq missing`, or `virtio-blk: unsupported requestq size` (`src/init.rs:39`,
`src/init.rs:45`, `src/init.rs:49`). On these paths the driver has already set the device's `FAILED` status
...
device model rather than the driver (`src/setup/sequence.rs:52`).

### A request returns an error status

The status word in the reply is the diagnosis (`src/protocol/errno.rs:16`).
...
- `E_MSGSIZE` (-90): the body length or `payload_len` did not match the declared sector count. Read requires
  ...
  `12 + nsectors * 512` (`src/server/handlers/read/request.rs:33`, `src/server/handlers/write/request.rs:42`).
- `E_INVAL` (-22): a bad opcode, a header that failed to decode, a zero or over-64 sector count, or a device
  that reported the request unsupported (`src/server/runner.rs:51`, `src/server/handlers/read/request.rs:38`,
  `src/server/handlers/flush.rs:25`).
- `E_NXIO` (-6): the `lba + nsectors` range ran past the probed capacity, or the addition overflowed
  (`src/server/handlers/read/request.rs:41`).
- `E_IO` (-5): the device reported an I/O error, or the completion wait failed
  (`src/io/submit.rs:59`, `src/server/handlers/read/handle.rs:45`).

### A request hangs

`submit` gives up with `Timeout` after `MAX_YIELDS` = 200000 yields waiting for the used ring index to reach
the target or the interrupt sequence to change (`src/io/submit.rs:44`). A `Timeout` surfaces to the handler
...
(`src/server/handlers/capacity.rs:29`, `src/server/handlers/read/reply.rs:28`). If a read reply looks short,
...
an overrun (`src/queue/used.rs:27`).
