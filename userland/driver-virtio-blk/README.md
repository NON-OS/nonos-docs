...
exposes; the [bring-up](bringup.md) page covers that fork.
...
|------|---------|----------------|
| [client.md](client.md) | `src/protocol/`, `src/server/` | The NBLK wire format, the receive loop and opcode dispatch, the five operations and their bounds, and the fixed kernel reply endpoint. |
| [bringup.md](bringup.md) | `src/discover.rs`, `src/setup/`, `src/init.rs`, `src/regs/` | Device discovery, the ordered claim/register/irq/dma grant transaction and its rollback, the legacy virtio handshake and feature negotiation, and the MMIO-versus-PIO register abstraction. |
| [queue.md](queue.md) | `src/queue/`, `src/io/` | The split-virtqueue layout, the header/data/status descriptor chain, the available-ring publish, and the notify/wait/ack completion path. |
...
| [debugging.md](debugging.md) | runtime | The boot marker, the bring-up and per-request failure modes, and where to look for each. |
...
(`src/main.rs:30`). There is no policy in the loop: the driver discovers exactly one virtio-blk device,
...
the virtio status handshake, probe the sector count, and hand back a `Driver` (`src/setup/sequence.rs:23`).
Each step depends on the grant the previous one produced, and each failure rolls back the grants already
taken, so a failed bring-up leaves the device unclaimed and no grants leaked. The [bring-up](bringup.md)
...
(`src/queue/post/descriptors.rs:28`, `src/setup/sequence.rs:41`). A request is serialised through the
...
(`src/hardware/virtio_blk_capsule/spawn.rs:37`). A successful spawn prints
`[DRIVER-VIRTIO-BLK] capsule spawned` on the boot log (`src/userspace/init/capsule_boot/run.rs:29`); the
[debugging](debugging.md) page covers what each later marker means.
...
sees them ([claim](../../subsystems/hardware-broker/claim.md),
...
rejected rather than truncated (`src/server/handlers/read/request.rs:33`,
...
sector past the device, or overrun the reply. The [client](client.md) page details each bound.
...
DMA to any physical address regardless of the grant ([dma](../../subsystems/hardware-broker/dma.md)). The
...
caller holding `CAP_DRIVER` may reach the block surface (`src/hardware/virtio_blk_capsule/capability.rs:30`).
