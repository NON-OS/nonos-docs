...
virtio handshake, and probes capacity. It mirrors `src/discover.rs`, `src/setup/`, `src/init.rs`, and
`src/regs/`. For the request path that runs on top of a live device see the [queue](queue.md) page; for the
client protocol see [client](client.md). The broker's own view of each grant is in
[claim](../../subsystems/hardware-broker/claim.md), [mmio](../../subsystems/hardware-broker/mmio.md),
[irq](../../subsystems/hardware-broker/irq.md), and [dma](../../subsystems/hardware-broker/dma.md).

## Discovery

`find_virtio_blk` lists broker devices through `mk_device_list` into a fixed 32-entry buffer and returns the
first that matches (`src/discover.rs:27`). A match is virtio vendor `0x1AF4` on a PCI bus with device id
`0x1001` (transitional) or `0x1042` (modern) (`src/discover.rs:52`, `src/constants/pci.rs:16`). A matching
record with no interrupt pin or an unrouted line (`irq_line == 0xFF`) is skipped, so the driver never binds
to a device it cannot get interrupts from (`src/discover.rs:37`). It then picks the first BAR with a
non-zero size whose kind is PIO or MMIO, and returns a `Found` carrying the device id, the interrupt line,
and the register BAR index, kind, and size (`src/discover.rs:57`).

## The bring-up transaction

`setup::run` is the ordered transaction (`src/setup/sequence.rs:23`). Each step depends on the grant the
...
(`src/setup/claim.rs:17`). The epoch is the broker's anti-stale linchpin: every later grant quotes it, and
a grant quoting an old epoch after a release is rejected
([claim](../../subsystems/hardware-broker/claim.md)). The claim is exclusive, so once it succeeds no other
capsule can be mapping this device's BARs or taking its interrupts underneath the driver.

### Registers

`registers::grant` maps the register BAR according to its kind (`src/setup/registers.rs:42`). An MMIO BAR
goes through `mk_mmio_map` with the length page-rounded up (`src/setup/registers.rs:51`); a port BAR goes
through `mk_pio_grant` (`src/setup/registers.rs:63`). Either way the result is wrapped in a `RegisterGrant`
enum, and `RegisterGrant::regs` turns it into a `Regs` handle so the rest of the driver reads registers the
same way regardless of BAR kind (`src/setup/registers.rs:29`). A map failure releases the device claim
before returning (`src/setup/registers.rs:54`). Register access is uncached device memory, never RAM
...
(`src/setup/irq.rs:19`). A total failure releases the register grant and then the device claim before
returning (`src/setup/irq.rs:27`). The grant delivers the device interrupt on a kernel-owned vector; the
...
- the virtqueue ring, `VQ_REGION_SIZE` = 16384 bytes (`src/setup/dma/map_queue.rs:20`);
- the request header buffer, `HEADER_BUF_LEN` = 4096 bytes (`src/setup/dma/map_header.rs:20`);
- the data buffer, `DATA_BUF_LEN` = 64 * 512 = 32768 bytes (`src/setup/dma/map_data.rs:20`,
  `src/constants/queue.rs:26`).
...
([dma](../../subsystems/hardware-broker/dma.md)). The rollback chain is layered: `rollback::base` unbinds
the irq, releases the registers, and releases the device; `rollback::queue` unmaps the queue then calls
`base`; `rollback::header` unmaps the header then calls `queue` (`src/setup/dma/rollback.rs:18`). The
ordering is the reverse of acquisition, so teardown mirrors bring-up.

## The virtio handshake

`bring_up` walks the legacy virtio status handshake over the register BAR (`src/init.rs:25`). The status
...
(`src/init.rs:21`, `src/init.rs:32`); everything else is masked off, so the device runs in its simplest
legacy mode. The queue size must be at least 3 (a chain needs header, data, and status descriptors) and at
most `Queue::max_supported_size()` = `MAX_QUEUE_SIZE` = 256 (`src/init.rs:47`, `src/constants/queue.rs:16`).
The status-register writes on the two abort paths set the `FAILED` bit so the device knows the driver gave
up (`src/init.rs:38`, `src/init.rs:44`, `src/init.rs:48`).

## Probe and arm

After `DRIVER_OK`, `setup::run` reads `capacity_sectors` from the legacy config capacity register at offset
`0x14` as a 64-bit value and rejects a zero capacity (`src/setup/sequence.rs:50`,
`src/constants/regs.rs:23`). `r64` is assembled from two 32-bit reads, low word then high
(`src/regs/state/r64.rs:20`). It then acknowledges the initial interrupt with `mk_irq_ack` and returns a
`Driver` holding the IRQ grant id, the `Queue`, the `Regs` handle, and the capacity
(`src/setup/sequence.rs:54`, `src/setup/driver.rs:18`). From this point the [queue](queue.md) engine and the
[client](client.md) server take over.
...
pointer or a PIO grant id (`src/regs/io.rs:16`, `src/regs/state/types.rs:18`). The sized accessors
...
width in bytes (`src/regs/state/r32.rs:23`, `src/regs/pio.rs:17`). This is why the mask carries both `Mmio`
