# The virtqueue request and DMA engine
...
protocol that feeds it is on the [client](client.md) page; the one-time queue setup that programs the ring
into the device is on the [bring-up](bringup.md) page.
...
index the driver has consumed (`src/queue/layout.rs:18`). `Queue::new` computes the ring offsets from the
negotiated queue size and zeroes the whole region before use (`src/queue/layout.rs:32`).

The region is a legacy split virtqueue laid out in one contiguous DMA region:

- the descriptor table at offset `VQ_DESC_OFFSET` = 0 (`src/constants/queue.rs:19`);
- the available ring right after the descriptor table, at `VQ_DESC_OFFSET + queue_size * 16`
  (`src/queue/layout.rs:66`);
- the used ring page-aligned after the available ring, at `align_up(avail + 6 + queue_size * 2, 4096)`
  (`src/queue/layout.rs:69`).

The whole region is `VQ_REGION_SIZE` = 16384 bytes (`src/constants/queue.rs:20`), which holds the largest
...
the chain, all under one `unsafe` block (`src/queue/post.rs:23`). The header is a 16-byte virtio-blk request
header at header-buffer offset `HEADER_OFFSET` = 0: a 32-bit request type, a reserved 32-bit word, and the
64-bit LBA (`src/queue/post/header.rs:21`, `src/constants/queue.rs:23`). The request type comes from the
`Direction`: `VIRTIO_BLK_T_IN` (0) for read, `VIRTIO_BLK_T_OUT` (1) for write, `VIRTIO_BLK_T_FLUSH` (4) for
flush (`src/queue/post/direction.rs:24`, `src/constants/request.rs:16`). The status byte at
`STATUS_OFFSET` = 64 is pre-set to `0xFF` before submission, so a device that never writes it back is
distinguishable from `S_OK` = 0 (`src/queue/post/header.rs:26`, `src/constants/queue.rs:25`).

## The descriptor chain

`write_descriptor_chain` builds the chain from the descriptor table base, which is the region base plus
`VQ_DESC_OFFSET` (`src/queue/post/descriptors.rs:28`). Each descriptor is 16 bytes: an 8-byte physical
address, a 4-byte length, a 2-byte flags word, and a 2-byte next index (`src/queue/post/descriptors.rs:39`).
...
written at descriptor slot 1 (`src/queue/post/descriptors.rs:32`). For read and write the data descriptor is
slot 1 and the status descriptor is slot 2 (`src/queue/post/descriptors.rs:36`). The read case sets
`VRING_DESC_F_WRITE` (2) on the data descriptor so the device fills the buffer; write leaves it
device-readable with only `VRING_DESC_F_NEXT` (1) (`src/queue/post/descriptors.rs:47`,
`src/constants/queue.rs:17`). The data length is `nsectors.saturating_mul(SECTOR_SIZE)`, so an implausible
sector count saturates rather than wrapping (`src/queue/post/descriptors.rs:46`). The status descriptor is
always device-writable and always the tail with next index 0 (`src/queue/post/descriptors.rs:60`).

## Publishing the chain

`publish_avail` hands the chain to the device by bumping the available ring
(`src/queue/post/publish.rs:19`). The available ring is `[flags: u16, idx: u16, ring: [u16; queue_size]]`.
The handler writes the ring flags to 0, reads the current index, writes descriptor 0 into the ring slot at
`idx % queue_size`, then increments the index with a wrapping add (`src/queue/post/publish.rs:22`). Because
the driver runs one request at a time, the slot the chain occupies always references descriptor 0.

## Submit, wait, ack

`submit` drives one request end to end (`src/io/submit.rs:27`). It posts the chain, kicks the device with a
...
(`src/io/submit.rs:38`, `src/io/read_seq.rs:19`). On completion it records the new `last_used`, reads the
...
byte treated as `Io` (`src/io/submit.rs:52`, `src/queue/used.rs:24`, `src/constants/request.rs:19`).

## Reading the result

The used-ring and status accessors live in `src/queue/used.rs`. `used_idx` reads the 16-bit used index at
the used-ring offset plus 2 (past the flags word) (`src/queue/used.rs:21`). `status_byte` reads the device's
status byte back from the header buffer (`src/queue/used.rs:24`). The data accessors are the safety hinge:
...
that walks past it (`src/queue/used.rs:27`). The read handler copies out through `data`, so an over-large
length can never overrun the DMA region ([client](client.md)).
