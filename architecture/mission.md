...
NONOS exists to make the trusted path something you own rather than something you hope for. On a
...
signs a transaction, or runs an autonomous agent that spends money. NONOS is built for that second
...
NONOS inverts all three, and the inversions are the architecture, not a configuration.

## What NONOS does instead

**Drivers run in ring 3, as capsules, with lent authority.** A device driver on NONOS is a signed
userspace capsule that holds no hardware privilege of its own. It reaches its device only through the
[hardware broker](../subsystems/hardware-broker/README.md), which lends it narrow, revocable grants:
...
model](../security/capabilities-and-tokens.md) is not a policy layered on top; it is the only way a
...
path](../security/capsules-and-trust.md) is the gate, and it is fail-closed: a bad signature, an
over-broad capability request, or a failed attestation means the capsule does not run.

## Why NONOS is different from everything else

- **Versus monolithic OSes (Linux, Windows, macOS):** their drivers run in the kernel; NONOS drivers
  run in ring-3 capsules with brokered grants. Their authority is ambient and subtractive; NONOS
...
- **Versus other microkernels (seL4, QNX):** NONOS shares the microkernel thesis, a small kernel that
  ...
  minimal verified kernel you assemble a system around; NONOS ships a complete capability system, the
  ...
  separate model (see below). NONOS is a system, not only a kernel.
- **Versus "secure OS" and "hardened OS" products:** their isolation is configured (sandboxes,
  containers, MAC policies) and can be misconfigured or bypassed; NONOS isolation is structural, a
  capsule cannot reach what it was not granted, and the grant machinery is the only path. Their driver
  model is still in-kernel; NONOS drivers are confined capsules.
- **Versus "formally verified" marketing:** many projects claim verification and prove little, or prove
  a model with no link to the running code. NONOS states exactly what is proven, proves it over the
...
one key; NONOS gives it to the whole system, in software, enforced by the same capability model that
...
(NONOS carries an in-kernel STARK and a Pedersen attestation gate, see [proof
system](../subsystems/proof-system/README.md) and [attestation](../security/attestation.md)), the
...
allowed to authorize a spend, and how you bound and attest it, becomes the whole game. A NONOS capsule
...
placeholders, and re-run on every push. NONOS deliberately proves properties over the real Rust source
rather than a detached model, which is the gap where defects usually hide. The full, honest scope of
what is and is not established is the [Verification](verification.md) page. That page is written to be
audited, not believed.

## Where to read next

- The layered [architecture overview](overview.md): what the kernel owns and what capsules own.
- The [hardware broker](../subsystems/hardware-broker/README.md): how a ring-3 driver reaches hardware.
- The [capability model](../security/capabilities-and-tokens.md) and [verified
  spawn](../security/capsules-and-trust.md): how authority is granted and gated.
- The [Verification](verification.md) page: exactly what is proven, and what is not.
