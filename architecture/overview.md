...
`Arch` implements the `ArchOps` trait (`src/arch/abi.rs:36`), the set of leaf
...
context is built. `arch/x86_64/context/setup.rs:36` rejects any entry point or
...
(`kernel_core/init/memory.rs`). Allocation hands out single frames; the higher
...
step is `init_unified_vm` at `memory/unified/init/run.rs:35`, which does the
following in order:
...
```
  ...
  +------------------------+
  +------------------------+   NONOS-ID certificate: the capsule's
...
`src/hardware/ps2_kbd_capsule/embed.rs:24`. The signed certificate and manifest
...
An endpoint declaration (`schema/endpoint.rs`) is a kind (Service or Reply), a
...
A publisher signature (`schema/publisher_sig.rs`) carries an algorithm id, a key
...
`kernel_core/process_spawn/capsule_spawn/runner/verified.rs:26`, which runs
...
`crypto/asymmetric/alg_id/verify.rs:23` and dispatches on algorithm: Ed25519
...
(`syscall/contract/dispatch.rs:31`):
...
`resolve` runs a chain (`syscall/contract/resolver/resolve.rs:31`): the token's
...
table at `syscall/contract/cap_table/mk.rs:20`. A few rows from it:
...
A capability token (`capabilities/token/types.rs:23`) is not a bearer secret
...
(`capabilities/token/material.rs`). The comparison is constant time
(`crypto/util/constant_time/compare.rs`) so verification does not leak the
...
codes, is the [ABI reference](../abi/). This section is the shape; that page is
the contract.
...
Sleeping and waking (`scheduler/dispatch/sleep.rs`):
...
(`arch/x86_64/context/switch/dispatch.rs:39`) loads it and executes `iretq` to
...
(`kernel_core/surface_registry/types.rs`): kind, flags, code, absolute x and y,
relative deltas, and a nanosecond timestamp.

The kernel owns one multi-producer single-consumer ring
(`kernel_core/surface_registry/input_ring.rs`). Many driver capsules post into
...
(`kernel_core/surface_registry/types.rs`) is width, height, stride in bytes,
